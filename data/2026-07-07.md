...
### [1] [EEG-Based Imagined Speech Decoding Using a Hybrid CNN-SNN Architecture](https://arxiv.org/abs/2607.03844)
*Fatima Shalhoub,Mariam Al Mawla,Kabalan Chaccour,Iván López-Espejo,Hoda Fares*

Main category: cs.SD

TL;DR: 本文提出了一种结合CNN和脉冲神经网络(SNN)的混合框架，首次将SNN应用于基于EEG的想象语音解码。


<details>
  <summary>Details</summary>
Motivation: 脑电信号具有高变异性和低波幅特点，且现有深度学习模型无法有效利用生物神经元的脉冲时间动力学特征。

Method: 设计了CNN-SNN混合架构，先通过CNN提取时间表征，再利用SNN进行脉冲时间分类。

Result: 在2020年BCI竞赛III基准数据集上实现80.13%的准确率，远超现有方法（70.19%）。

Conclusion: 研究证实了基于脉冲的解码方法在脑电想象语音识别中的有效性，为新一代神经形态脑机接口应用提供了新方向。
...
### [2] [Adaptive Loss Balancing for Multi-Task Bioacoustic Classification of Bird Species and Call Types](https://arxiv.org/abs/2607.03304)
*Paria Vali Zadeh,Sven Tomforde*

Main category: cs.SD

TL;DR: 本研究探讨了在鸟类物种和鸣叫类型联合分类的多任务学习中，不同预训练模型、微调深度及损失平衡策略的交互影响。


<details>
  <summary>Details</summary>
Motivation: 被动声学监测中鸟类鸣声分析需要处理多个且不平衡的标注目标，现有方法在多任务联合分类和损失平衡上面临挑战。

Method: 扩展BirdCallNet以进行联合分类，评估了四种鸟类领域编码器在三种适应深度（线性探测、注意力探测、全微调）下的表现，并对比了多种自适应损失加权策略（不确定性加权、DWA、GradNorm等）。

Result: 多任务学习提升了鸣叫类型识别；全微调并非总是最优选择，冻结骨干网络的调整在某些任务上表现更好；不同的自适应加权策略表现取决于具体架构和任务，且GradNorm计算成本较高。

Conclusion: 最佳的损失平衡策略高度依赖于骨干网络、适应模式和目标任务，冻结骨干网络比全微调能提供更好的性能与效率权衡。
...
### [3] [Trajectory Variance: AnUnsupervised Measure of Developmental Vocal Plasticity in Birdsong](https://arxiv.org/abs/2607.03496)
*Kanghwi Lee*

Main category: cs.SD

TL;DR: 提出了一种名为“轨迹方差”的无监督可塑性评分方法，用于量化动物发声在发育过程中的变化程度。


<details>
  <summary>Details</summary>
Motivation: 旨在解决如何在不需要人工类型标签的情况下，定量评估单个发声在不同发育阶段的变化量（可塑性）。

Method: 利用位移模型预测自编码器潜在空间中受年龄调节的发声变化，并以不同目标年龄预测值的方差（轨迹方差）来量化其可塑性。

Result: 在斑胸草雀数据上的评估表明，该指标成功区分了习得性鸣声与先天呼叫（AUC为0.58-0.67），且与谱平坦度呈负相关（r为-0.48至-0.75），表现优于无参数基线方法。

Conclusion: 轨迹方差是一种有效的无标签发声可塑性度量工具，证实了发育中变化较大的发声通常具有更清晰的音调和结构化频谱。
...
### [4] [Metronome: Bound the Cache, Keep the Beat for Real-Time Interaction Model Serving](https://arxiv.org/abs/2607.02640)
*Jiaying Meng,Bojie Li*

Main category: cs.SD

TL;DR: 本文揭示了实时交互大模型因KV缓存耗尽导致系统静默崩溃的“亚稳态崩溃”现象，并提出了Metronome系统，通过KV窗口化和在线准入控制解决了该问题。


<details>
  <summary>Details</summary>
Motivation: 实时交互模型在持续负载下，随着KV缓存单调增长，会因耗尽KV池而突然崩溃，且这种崩溃具有隐蔽性，传统的延迟指标无法预警。

Method: 设计了Metronome系统，采用引擎内KV窗口限制会话状态，保留少量关键注意力汇（attention-sink）Token，并结合在线准入控制器进行并发调度。

Result: Metronome完全消除了系统崩溃（运行崩溃率从14/20降至0/20），使延迟能真实反映负载，且一阶模型能高精度预测崩溃时间，同时保证了生成质量。

Conclusion: 通过限制会话的驻留KV状态，可以有效恢复实时交互模型服务的稳定性和可观测性，避免系统级静默崩溃。
...
### [5] [DETECT-3B-Omni is Agnostic of Content and Demographics](https://arxiv.org/abs/2607.03418)
*Nicolas M. Müller,Aditya Tirumala Bukkapatnam,Dominik Schnieders,Zohaib Ahmed*

Main category: cs.SD

TL;DR: 评估表明，DETECT-3B-Omni 深度伪造音频检测器的准确率不受说话内容或说话人特征的影响。


<details>
  <summary>Details</summary>
Motivation: 为了确保深度伪造音频检测器符合 GDPR 且值得信赖，其决策必须基于声学伪影，而非说话内容或说话人身份。

Method: 使用由8个AI语音克隆系统生成的10,240个音频样本（涵盖不同性别、年龄和地区），通过等效性检验评估检测器在不同内容和说话人特征下的准确率差异。

Result: 在99%的置信度下，不同说话内容、性别、年龄或地区群体之间的检测准确率差异不超过2个百分点。

Conclusion: 该检测器在识别AI生成音频时具有语义独立性，其准确率不受音频内容或说话人身份的影响。
...
### [6] [Taste-aware music retrieval from audio embeddings](https://arxiv.org/abs/2607.03296)
*Matteo Spanio,Antonio Rodà*

Main category: cs.SD

TL;DR: 本文构建了从音频预测味道的跨模态检索基准，评估了多种音频编码器，证明其预测味道共识的能力超越了人类平均水平。


<details>
  <summary>Details</summary>
Motivation: 声音与味道之间的跨模态对应关系在心理学中已确立，但在基于内容的媒体检索领域尚未得到应用。

Method: 将“从音频预测味道”定义为音乐信息检索任务，对比了十种音频编码器在多任务回归头和门控晚期融合下的表现，并使用RMSE和皮尔逊相关系数等指标进行评估。

Result: 最佳系统的预测宏RMSE达到0.134，显著优于人类评分者偏差（0.28）和前人基线（0.219），且在检索任务中远超CLAP文本基线。

Conclusion: 该研究证实了通过音频表示预测味道的可行性，模型能比个体人类更准确地契合群体味道共识，为跨模态检索开辟了新方向。
...
### [7] [Information-Geometric Superposed Vowel Evaluation: Part 1. Moraic Syllabary (Japanese)](https://arxiv.org/abs/2607.04154)
*Yusei Tamura,Shigekazu Ishihara,Ken Ito*

Main category: cs.SD

TL;DR: 提出一种通过分析元音频谱分布、利用Wasserstein距离和持续同调（persistent homology）来区分生成式AI合成语音与人类真实语音的新方法。


<details>
  <summary>Details</summary>
Motivation: 生成式AI合成的语音由于训练数据有限导致其元音多样性不足，而人类自然语音因发音器官的灵活性而具有更多样化的频谱分布。本研究旨在区分这两种语音。

Method: 以日语元音为例，将语音频谱归一化为模拟人类耳蜗接收频率的概率密度函数，使用Wasserstein度量计算频谱间距离，并利用持续同调进行拓扑映射以实现聚类。

Result: 合成语音元音之间的Wasserstein距离较短，而通过持续同调可以将合成语音和真实语音的频谱概率密度函数成功分解为不同的聚类。

Conclusion: 利用频谱分布、Wasserstein距离和拓扑数据分析（持续同调）可以有效区分AI合成语音与人类自然语音。
...
### [8] [TokAN: Accent Normalization Using Self-Supervised Speech Tokens](https://arxiv.org/abs/2607.03928)
*Qibing Bai,Shuai Wang,Yuhan Du,Bohan Li,Yannan Wang,Haizhou Li*

Main category: cs.SD

TL;DR: 本文提出了一种名为TokAN的口音归一化框架，利用离散语音标记和强化学习，在无需平行语料或合成目标的情况下实现高质量的非母语口音转换并保留说话人身份。


<details>
  <summary>Details</summary>
Motivation: 现有的口音归一化技术要么需要稀缺的自然L1-L2平行语音数据，要么在以合成语音作为监督目标时面临音质下降的问题。

Method: 提出TokAN框架，通过自监督VQ分词器进行标记转换，引入基于GRPO算法的强化学习后训练（以字错率和口音分类器置信度为奖励），并结合流匹配合成器与时长预测器以支持时长感知合成。

Result: 在7种英语口音的实验中，TokAN将字错率（WER）从12.40%降至9.89%（微调后），并在强化学习后进一步降至9.23%，在口音消除和清晰度上均显著优于现有基线方法。

Conclusion: TokAN是一种高效且无需平行语料监督的口音归一化方法，不仅提升了语音转换的质量，还适用于配音和直播等对时长敏感的实际应用场景。
...
### [9] [Doppelganger: Sound Effects and Their Synthetic Twins](https://arxiv.org/abs/2607.04337)
*Elliott Ash*

Main category: cs.SD

TL;DR: 引入了Doppelganger基准测试，用于将合成音效与其原始真实音频进行匹配，并研究了提升跨虚实边界匹配能力的方法。


<details>
  <summary>Details</summary>
Motivation: 随着音频生成技术的发展，真实与合成音频并存，但目前缺乏评估表征模型能否将合成音频与其对应的真实源录音进行准确匹配的基准。

Method: 构建了包含10,420个真实-合成配对音频的Doppelganger基准数据集；评估了现成音频编码器在类别标签训练与配对（真实音频与其合成孪生音频）训练下的泛化表现，并引入了人类听觉标注进行对比。

Result: 现成编码器表现不佳。采用配对训练在未见声音事件上的匹配准确率从61%提升至约80%，且该匹配具有生成器特异性。人类匹配表现低于模型，有29%的概率将合成音误判为真实音，而生成器特异性检测器能完美区分两者。

Conclusion: 针对真实与合成配对音频的训练能显著提升模型跨越虚实边界的匹配能力，该能力对特定生成器具有特异性，且专用的检测器在区分虚实音频方面远超人类。
...
### [10] [Sampling Bias Compensation for Robust Evaluation of Audio Classification Systems with Partially Labeled Evaluation Datasets](https://arxiv.org/abs/2607.04463)
*Javier Naranjo-Alcazar,Annamaria Mesaros,Tuomas Virtanen,Pedro Zuccarello*

Main category: cs.SD

TL;DR: 本文研究了如何利用重要性权重技术，在有限标注预算下减轻音频机器学习评估中的样本选择偏差。


<details>
  <summary>Details</summary>
Motivation: 在实际部署中全面标注大量音频数据十分困难，而仅依赖少量标注子集进行评估会引入采样偏差，导致评估指标失真。

Method: 引入并对比了三种密度比估计方法（KDE、逻辑回归和kNN）以计算重要性权重，并结合五种基于主动学习的采样策略模拟真实场景。

Result: 在音频场景分类基准上的实验表明，重要性权重方法显著缩小了子集评估指标与真实性能之间的差距，提供了更真实的准确率估计。

Conclusion: 重要性权重技术能够有效缓解音频系统评估中的选择性偏差，提升在有限标注预算下的评估可靠性。
...
### [11] [Auto-AEG: Scalable Data Construction for Open-Vocabulary Audio Event Grounding](https://arxiv.org/abs/2607.04383)
*Zihan Zhang,Xize Cheng,Wenhao Yan,Tong Zhang,Dongjie Fu,Boyun Zhang,Yongbo He,Tao Jin*

Main category: cs.SD

TL;DR: 本文提出了Auto-AEG，一个通过自动数据构建和强化学习微调来提升大音频语言模型（LALM）开放词汇音频事件定位能力的流水线。


<details>
  <summary>Details</summary>
Motivation: 现有的大音频语言模型缺乏精确的时间定位能力，而传统的音频事件检测局限于闭集标签，且由于人工标注成本高昂，导致开放词汇的时序定位监督数据严重匮乏。

Method: 引入Auto-AEG流水线，将程序合成的音频（提供精确边界用于有监督冷启动）与真实音频上的多模型伪标签（提供强化学习的奖励信号）相结合进行微调，并推出了难度分级的评估基准AEGBench。

Result: 该方法在DESED基准测试和新提出的AEGBench上均取得了显著的性能提升。

Conclusion: 自动构建的数据与区间感知的奖励函数设计相结合，是扩展大音频语言模型时序定位能力的有效数据端途径。
...
### [12] [Training-Free Model Selection and Domain-Aware Score Calibration for First-Shot Anomalous Sound Detection](https://arxiv.org/abs/2607.04526)
*Grach Mkrtchian*

Main category: cs.SD

TL;DR: 本文提出了一种无需训练的后处理方法，通过分位数校准和领域平衡准则，解决DCASE异常声音检测中源/目标域失衡及评估集预测难的问题。


<details>
  <summary>Details</summary>
Motivation: 解决DCASE任务中源域与目标域AUC负相关，以及开发集性能无法预测评估集实际表现的问题。

Method: 在冻结音频嵌入之上，采用基于先验强度的每域分位数校准来平衡源域与目标域，并结合一种仅使用正常训练数据的无标签交叉验证领域平衡准则来筛选和排序配置。

Result: 在DCASE 2025上，该准则与评估得分的相关性达0.91（而开发集仅为0.06），使评估得分提升至61.05（相当于第4名）。在2023和2024年复现中，同样证实开发集无预测力，但在这两年固定完全均衡设置的效果更好。

Conclusion: 所提的后处理校准和选择方法显著提高了异常检测性能和评估预测准确性，但在不同年份的数据分布下，自动选择策略与固定默认设置的优劣存在差异。
...
### [13] [CARD: Cross-component Audio Representation Distillation for Encoder-Free Audio Captioning](https://arxiv.org/abs/2607.04619)
*Ganesh Pavan Kartikeya Bharadwaj Kolluri,Yuchen Zhang,Michael Kampouridis,Ravi Shekhar*

Main category: cs.SD

TL;DR: CARD是一个在推理阶段无需音频编码器的音频描述模型，通过将教师模型的表征合理分流蒸馏到投射器和LLM中，降低了推理成本。


<details>
  <summary>Details</summary>
Motivation: 传统的自动音频描述系统依赖冻结的音频编码器，这带来了高昂的推理开销，并因固定的声学特征限制了模型性能。

Method: 提出CARD模型，推理时丢弃编码器。在训练时，将预训练音频教师模型（CLAP-HTSAT）的表征分流蒸馏：知觉阶段送入投射器（Projector），语义阶段送入带LoRA适配器的LLM。

Result: 相比仅在LLM蒸馏的模型，CARD在AudioCaps上的CIDEr-D提升了12.18，在Clotho上提升了5.21，在无编码器推理下达到55.4分（接近保留编码器的66.4分上限）。

Conclusion: 知识蒸馏的注入位置与其是否存在同等重要，通过合理的跨组件表征路由可以实现高效且性能优秀的无编码器音频描述。
...
### [14] [SynSFX: Multi-Model Sound Effects Synthesis Dataset for Deepfake Detection and Evaluation](https://arxiv.org/abs/2607.04848)
*Linxi Li,Yuncong Yu,Qianwei Guo,Liwei Jin,Yechen Wang,Carsten Maple*

Main category: cs.SD

TL;DR: 本文推出了SynSFX，一个包含43374个片段的大规模合成音效数据集，旨在解决音频伪造检测在音效上泛化性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的音频伪造检测器在合成音效上的泛化能力有限，且现有数据集在规模和生成来源上存在局限，无法有效支持孤立音效伪造的研究。

Method: 构建了包含43,374个音频片段（26,452个合成，16,922个真实）的SynSFX数据集，涵盖了7种主流的文本转音频模型。

Result: 成功提供了一个大规模、来源丰富的音效伪造检测数据集，填补了现有数据集在规模和多样性上的空白。

Conclusion: SynSFX数据集为研究和提升针对合成音效的伪造检测技术提供了强有力的支持。
...
### [15] [Adaptive Diversity-Uncertainty Active Learning with Redundancy Control for Bioacoustic Event Classification](https://arxiv.org/abs/2607.04868)
*Gabriel Dubus,Hugo Magaldi,Anatole Gros-Martial*

Main category: cs.SD

TL;DR: 提出一种结合预测不确定性、多样性和去冗余的自适应主动学习策略，用于提高生物声学事件分类的标注效率。


<details>
  <summary>Details</summary>
Motivation: 现有的样本选择策略依赖静态标准，无法适应模型训练中预测可靠性的动态变化，导致探索与利用的折中效果欠佳且存在样本选择冗余。

Method: 提出一种自适应权重方案，根据模型置信度动态平衡多样性探索与不确定性利用，并结合贪婪最大边缘相关性（MMR）算法减少批次内的样本冗余。

Result: 在陆地（BirdSet）和海洋（ATBFL）生物声学基准测试中，该方法在mAP和学习曲线下面积（AULC）上均表现出持续的效率提升，尤其在陆地声景中效果显著。

Conclusion: 融合不确定性估计、空间多样性与去冗余批次构建的自适应主动学习策略，为异构声学域下的生物声学监测提供了一种高效且鲁棒的解决方案。
...
### [16] [Towards Robust Uncertainty-Aware Speaker Modeling](https://arxiv.org/abs/2607.04937)
*Junjie Li,Yang Xiao,Kong Aik Lee*

Main category: cs.SD

TL;DR: 本文提出了一种稳健的说话人嵌入不确定性建模框架，以提高说话人识别的准确性与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在面临领域偏移时，存在不确定性估计不准和不确定性校准失真的问题。

Method: 引入了类间和类内感知的不确定性Softmax以及不确定性校准域自适应（UCDA）框架。

Result: 在域内和跨域基准测试上的实验表明，该方法显著提升了不确定性估计的可靠性和说话人识别的鲁棒性。

Conclusion: 通过从估计和自适应两个维度进行优化，可以有效解决说话人识别中的不确定性失准问题。
...
### [17] [Listen, Think, Transcribe: Continuous Latent Test-Time Scaling for ASR](https://arxiv.org/abs/2607.05051)
*Ho Lam Chung,Yiming Chen,Dau-Cheng Lyu,Hsiao-Tsung Hung,Hung-yi Lee*

Main category: cs.SD

TL;DR: 本文提出了LatentASR，一种在冻结的ASR模型中通过测试时迭代优化潜在前缀来提升难样本转录准确率的参数高效方法。


<details>
  <summary>Details</summary>
Motivation: 传统的端到端ASR模型进行单次解码，无法重新处理困难输入，且在极小数据集上使用传统微调方法（如LoRA）容易发生性能退化（WER上升）。

Method: 在冻结的Qwen3-ASR-0.6B模型上添加约4M参数的两个模块：Latent Adapter（在测试时迭代优化潜在前缀）和Value Head（预测是否需要额外计算并提前终止），仅用500条语音进行训练激活。

Result: 在FLEURS和VoxPopuli上分别实现2.54% and 0.47%的相对WER下降，在口音及混合语料（ASCEND）上实现16.0%的相对CER下降，同时动态终止机制可跳过约一半的语料以节省计算量。

Conclusion: 研究表明，利用极小的激活集即可在不损害原模型的前提下，为冻结的ASR模型引入测试时缩放机制，实现按需分配的高效输入相关计算。
...
### [18] [Context-Aware ASR for Mandarin Technical Lectures](https://arxiv.org/abs/2607.05058)
*Ho-Lam Chung,Yiming Chen,Hung-yi Lee*

Main category: cs.SD

TL;DR: 针对中英混合讲座中的专业术语识别问题，本文提出了一种基于双阶段解码的自建术语表提示方法，并引入了更准确的术语中心评估指标。


<details>
  <summary>Details</summary>
Motivation: 技术讲座中中英混杂的专业术语承载核心语义，但因字数较少，其识别错误常被传统的字符错误率（CER）所掩盖。

Method: 构建了一个术语丰富的中文AI/ML讲座基准数据集并定义了术语中心指标；提出了一种双阶段免参考解码方法，在第一阶段提取高频术语构建临时词表，在第二阶段用其提示ASR模型。

Result: 在五种ASR模型上均提升了术语召回率并保持或降低了CER。例如在Breeze-ASR-25上，将术语召回率从52.50%提升至60.13%（结合外部词表可达62.05% recall和82.73% precision）。

Conclusion: 利用模型自身输出恢复的讲座上下文能有效辅助技术术语识别，且以术语为中心的评估能揭示传统CER无法反映的识别错误。
...
### [19] [Internal Pluralism and the Limits of Pairwise Comparisons](https://arxiv.org/abs/2607.02672)
*Bailey Flanigan,Michelle Si*

Main category: cs.AI

TL;DR: 本文指出传统的局部成对比较方法在捕捉人类复杂的多元价值偏好时存在缺陷，并证明允许用户表达“未决定”以及直接引导其优先级能显著提升偏好学习的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统的偏好学习（如对齐和参与式设计）依赖于局部成对比较，这假设了局部信息足以代表整体偏好且人们总能做出果断选择。然而，在面对内部多元主义（即个体根据多个冲突的原则进行决策）时，这些假设会失效。

Method: 构建了一个关于决策规则多元偏好的形式化模型，并利用该模型分析了强制局部成对比较的失效模式，同时评估了允许用户报告“未决定（indecision）”状态的替代方案。

Result: 研究发现，局部比较无法捕获如比例原则或平均主义等全局性优先级，且强迫选择在有内部冲突时会导致行为扭曲；相反，允许报告“未决定”能大幅减少准确学习偏好所需的查询数量。

Conclusion: 偏好学习方法应转向直接启发和获取用户的核心优先级，从而更真实、更具可解释性地反映人类的价值观。
...
### [20] [iFLYTEK-Embodied-Omni Technical Report](https://arxiv.org/abs/2607.02542)
...
TL;DR: 本文提出了iFLYTEK-Embodied-Omni，一个将视觉、语言和动作统一建模的具身智能多模态基座模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常在推理、世界建模或动作生成上各自独立，级联式管道容易带来接口瓶颈和累积预测误差。

Method: 利用共享的多模态自注意力机制构建“大脑-小脑”协同架构（VLM/VGM作为大脑负责规划预测，AGM作为小脑执行动作），并采用四阶段渐进式训练策略。

Result: 实现了在统一框架中融合指令理解、任务规划、未来状态预测以及可执行动作块的直接生成。

Conclusion: 该框架通过一体化建模有效克服了级联误差，为构建通用具身智能体提供了有效的新途径。
...
### [21] [Object-Centric Environment Modeling for Agentic Tasks](https://arxiv.org/abs/2607.02846)
*Yiyang Li,Tianyi Ma,Zehong Wang,Yijun Ma,Yanfang Ye*

Main category: cs.AI

TL;DR: 本文提出OCM（面向对象环境建模），将大模型智能体的经验组织为可执行的面向对象环境模型，以提高经验利用率。


<details>
  <summary>Details</summary>
Motivation: 传统的自由文本记忆难以维护和复用，而现有的符号方法往往受限于局部过程或简化的动态假设。

Method: OCM维护对象知识（Python类）和过程知识（交互模式）两个代码库，进行在线反射与一致性验证，并采用渐进式知识披露机制进行检索。

Result: 实验表明，OCM在各基准测试中取得了最佳平均排名，并有效减少了无效动作的生成。

Conclusion: 构建面向对象的可执行环境模型能显著提升智能体在复杂环境中的决策效率和准确性。
...
### [22] [Automated Data Readiness for Scientific AI](https://arxiv.org/abs/2607.02771)
...
TL;DR: 本文提出了REDI框架，用于将大规模科学数据集自动、可追溯地转换为适用于AI训练的数据。


<details>
  <summary>Details</summary>
Motivation: 现有的科学数据处理框架无法统一实现自动化转换、数据就绪评估、数据溯源跟踪和智能体原生部署。

Method: 设计了包含五个阶段（摄取、预处理、转换、结构化、输出）的统一管道框架REDI，并结合配套工具SetGo实现FAIR合规性与目录发布，支持智能体部署。

Result: 在气候、蛋白质组学等多个领域成功应用，在Frontier超级计算机上实现了近乎理想的100节点并行扩展，并指出文件I/O和格式选择是主要的优化瓶颈。

Conclusion: REDI作为一个跨领域平台，能够有效解决科学AI的数据准备瓶颈，并生成可重复、可重用的社区资产。
...
### [23] [ASK in the Dark: Uncertainty-Gated LLM Assistance under Partial Observability](https://arxiv.org/abs/2607.02686)
*Juarez Monteiro,Nathan Gavenski,Guilherme Lima,Francisco Galuppo,Odinaldo Rodrigues,Adriano Veloso*

Main category: cs.AI

TL;DR: 本文提出了 ASK+ 方法，通过为小语言模型（SLM）提供轨迹感知上下文和结构化思维链推理，成功解决了部分可观测环境下强化学习代理难以有效结合 SLM 指导的问题。


<details>
  <summary>Details</summary>
Motivation: 在部分可观测环境中，传统的基于不确定性门控的 SLM 辅助强化学习方法由于缺乏足够的上下文信息，导致 SLM 无法给出有效的独立动作建议。

Method: 提出 ASK+ 方法，为 SLM 引入包含局部地图、历史路径和动作记录的轨迹感知上下文，结合结构化思维链，并将预测熵用于衡量 POMDP 中的动作不确定性以进行选择性查询。

Result: ASK+ 在 DoorKey 任务中成功率提升至 93%，在 FourRooms 中由 53% 提升至 70%，在 HigherLower 中达到 73.7% 的准确率，且 Qwen3.5-2B 的表现比肩或超越 4B 模型。

Conclusion: 合理的提示词设计和选择性门控机制相比于模型规模更为关键，这使得在无需大参数量模型的情况下也能实现高效的强化学习引导。
...
### [24] [SwarmResearch: Orchestrating Coding Agents for Open-Ended Discovery](https://arxiv.org/abs/2607.02807)
*Yuvraj Virk,Zack Edds,Chunqiu Steven Xia,Lingming Zhang*

Main category: cs.AI

TL;DR: 本文提出SwarmResearch，一种结合“牧羊人”编排器与多个分支搜索智能体的框架，旨在解决编码智能体容易过早收敛并陷入局部最优的问题。


<details>
  <summary>Details</summary>
Motivation: 长期运行的编码智能体往往会收敛于单一的高层方法并仅进行低级修改，这主要是由于在单智能体中累积上下文以及仅暴露单一程序状态造成的限制。

Method: 引入SwarmResearch框架，使用一个掌握全局上下文的 Shepherd 智能体来引导多个在独立 Git 分支中运行（仅具局部上下文）的 Search 智能体，并基于搜索深度动态调整并行度。

Result: 在15个开放式优化任务中，SwarmResearch在13个任务上取得了优于或相当于最先进LLM引导进化和多智能体技术的效果，展现出更强的全局探索能力。

Conclusion: 通过全局编排与多分支局部探索相结合，SwarmResearch能够有效避免局部最优，在开放式代码优化任务中实现更高效的探索和更佳的解决方案。
...
TL;DR: 本文提出了一个新的医学计算评估基准 MedCalc-Pro，并设计了一个能够处理复杂临床计算任务的通用智能体框架。


<details>
  <summary>Details</summary>
Motivation: 现有的医学计算评估基准大多基于简化场景，无法反映现实临床中多计算器联合评估、嵌套计算及模糊查询的复杂需求。

Method: 提出了包含 2,268 个真实临床案例的基准 MedCalc-Pro，并开发了一个支持多工具选择、嵌套调用以及通过结构化验证和证据审查抑制误差传播的智能体框架。

Result: 在开源、闭源及医学专用大语言模型的对比实验中，所提框架在单计算器、多计算器和嵌套计算三种任务设置下均取得了最佳性能。

Conclusion: 本工作为在复杂临床计算场景中评估和应用大语言模型提供了新的基准和有效的方法支撑。
...
### [26] [Oyster-II: Reinforcement Learning for Constructive Safety Alignment in Large Language Models](https://arxiv.org/abs/2607.02914)
*Jiyang Guan,Yong Xie,Jun Chen,Jiexi Liu,Zipeng Ye,Defeng Li,Jiayu Shen,Jialing Tao,Hui Xue*

Main category: cs.AI

TL;DR: Oyster-II 提出了一种基于强化学习的建设性安全对齐框架，旨在解决大语言模型在安全防护中的过度拒绝和泛化不足问题。


<details>
  <summary>Details</summary>
Motivation: 传统的拒绝式安全对齐和基于监督微调（SFT）的建设性安全方法（如 Oyster-I）存在分布外泛化能力不足，以及将安全思维链过度套用到无害查询中（导致有用性下降）的问题。

Method: 提出了 Oyster-II 框架，采用零样本强化学习（Zero-RL）范式与多阶段强化学习相结合的方法来进行建设性的安全对齐。

Result: Oyster-II 在安全维度上全面超越了 Qwen3-14B 和前代 Oyster-I，取得了可媲美 Qwen3-Max 和 Qwen3.5-397B 等更大规模模型的性能表现。

Conclusion: 通过强化学习实现的建设性安全对齐能够更有效地解决安全泛化与过度泛化问题，在保证模型安全性的同时提升其服务用户真实意图的实用性。
...
### [27] [VERITAS: Towards a General-Purpose Replication Tool for Scientific Research](https://arxiv.org/abs/2607.02931)
*Haokun Liu,Filbert Aurelian Tjiaranata,Chenhao Tan*

Main category: cs.AI

TL;DR: VERITAS 是一个通用的科学论文复现框架，利用 CLI 编码代理自动运行实验并评估论文声明的真实性，实现了 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 人工复现学术论文耗时且昂贵，而现有的自动化复现工具大多局限于特定基准，缺乏通用的科学复现工具。

Method: 提出基于 CLI 编码代理的 VERITAS 框架，输入论文和代码后，它能自动提取声明、运行并修复代码，并输出复现评分及修复后的代码。

Result: 在包含 65 篇跨学科论文的 CORE-Bench 和 ReplicationBench 上评估，VERITAS 的表现超越了两个强劲的 Claude Code 基线，在所有指标上均达到 SOTA。

Conclusion: VERITAS 证明了通用编码代理在自动化科学复现中的有效性，为解决学术复现性问题提供了实用的工具。
...
### [28] [A Sliding-Window-Based Reinforcement Learning for Dynamic Assembly Flow Shop Scheduling with Multi-Product Delivery](https://arxiv.org/abs/2607.02941)
*Junhao Qiu,Jianjun Liu,Ting Liu,Rongjie Liao,Zhantao Li,Qingfu Zhang*

Main category: cs.AI

TL;DR: 提出了一个基于滑动窗口强化学习（SWRL）的框架，用于解决具有复杂配套约束的混合制造系统实时调度问题。


<details>
  <summary>Details</summary>
Motivation: 动态订单到达改变了供应依赖和机器分配，使得多产品配套交付的实时调度面临巨大挑战。

Method: 将问题建模为异构图MDP，并提出SWRL框架，整合了滑动窗口过滤机制、时空图编码网络以及带约束等待的动态动作映射模块。

Result: 在真实家电制造场景下的实验表明，SWRL在降低交期延迟方面显著优于传统规则和现有DRL方法，且在不同资源和负载配置下表现出强鲁棒性。

Conclusion: SWRL框架能够有效应对具有复杂配套约束的柔性装配流水线在线调度难题，具备高效性和鲁棒性。
...
### [29] [Evaluating Generative Agents with Actions Grounded in Socially Distributed Task Environments using Incognita](https://arxiv.org/abs/2607.02975)
*Dan C. Hsu,Luke Lu*

Main category: cs.AI

TL;DR: 本文提出了Incognita评估框架，用于测试智能体在社交分布式任务环境中结合社交沟通与落地执行的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的评估基准要么侧重于可执行的落地操作，要么侧重于社交互动，缺乏将两者结合的、需要通过与不同角色沟通来获取任务关键知识并执行操作的环境。

Method: 定义了社交分布式任务环境，并开发了基于Concordia的Incognita框架，该框架将社交交互与落地执行分离。基于此构建了Incognita-Retail基准，并在18个任务上评估了三种生成式智能体模型。

Result: 评估表明智能体的表现有所提升（成功率从0%提升至8.9%和17.2%，过早结束任务的比例下降），更强的模型能获取更多隐藏知识并尝试更多操作，但整体可靠性依然较低。

Conclusion: 社交分布式任务环境能够有效暴露智能体在达到可靠成功之前的中间行为缺陷，如知识启发、来源选择和过早完成任务的信念。
...
### [30] [Reinforcement Learning for Evidence-Seeking Diagnostic Reasoning with Large Language Models](https://arxiv.org/abs/2607.02983)
*Shengyi Hua,Kangzhe Hu,Conghui He,Xiaofan Zhang,Shaoting Zhang*

Main category: cs.AI

TL;DR: 本文提出了一种结合强化学习（RLVR）与高保真临床模拟器（RAGES）的框架，将医疗诊断重塑为迭代式证据获取任务，使大模型实现从被动推理到主动临床调查的转变。


<details>
  <summary>Details</summary>
Motivation: 现有推理大模型多为被动推理，而真实临床诊断是一个需要主动、迭代获取证据的动态过程。

Method: 将诊断形式化为迭代证据寻求任务；使用带可验证奖励的强化学习（RLVR）进行优化；开发了基于检索增强生成的检查模拟器（RAGES）提供逼真的临床反馈。

Result: 该框架使大模型具备了自主诊断能力，效果比肩更大型的推理模型，且RAGES在生成合理临床反馈上显著优于传统大模型。

Conclusion: 通过结合RLVR和RAGES，成功推动了大语言模型向自主、策略性的临床智能助手演进。
...
### [31] [Beyond Forecasting: The Belief-to-Trade Layer in Prediction-Market Agents](https://arxiv.org/abs/2607.03015)
*Yishu Wang,Yuxuan Wang,Jiaqi Deng,Hanyang Tang*

Main category: cs.AI

TL;DR: 提出了首个用于预测市场的自主交易智能体 Raven-Agent，在回放测试中实现了唯一的正收益。


<details>
  <summary>Details</summary>
Motivation: 虽然预测市场是评估AI预测能力的天然场所，但交易不仅需要预测，且现有模型的校准概率得分与实际交易结果之间存在显著差距。

Method: 开发了 Raven-Agent，这是首个用于预测市场的自主交易智能体架构。

Result: 在归档决策集的受控回放测试中，Raven-Agent 实现了所有测试策略中唯一的正收益和唯一的正风险调整后收益。

Conclusion: Raven-Agent 证明了自主交易智能体在预测市场中能够有效连接预测能力与实际交易收益。
...
TL;DR: 本文提出了HCRA人机协同决策框架，通过随机博弈建模和迭代语言反馈来对齐AI与人类期望。


<details>
  <summary>Details</summary>
Motivation: 人类对AI推荐存在过度或不足信赖的问题，且当前AI系统与人类期望的校准较差。

Method: 将协同决策建模为人类与AI的随机博弈，并提出整合了人类校准模型与强化学习的以人为中心反射式架构（HCRA）。

Result: 实验结果表明，HCRA提升了决策的有效性，并能提供高质量的推荐建议。

Conclusion: 该研究成功构建了一种增强人类能力并使AI代理符合人类偏好与期望的协同决策框架。
...
### [33] [Silicon Sampling via Cross-Survey Transfer](https://arxiv.org/abs/2607.03091)
*Chan-Tung Ku,Chan Hsu,Pei-Cing Huang,Frank Cheng-shan Liu,I-Ling Cheng,Yihuang Kang*

Main category: cs.AI

TL;DR: 本研究提出了一种“跨调查迁移”评估框架，用于测试大语言模型在个人层面上模拟人类问卷受访者的能力。


<details>
  <summary>Details</summary>
Motivation: 传统的评估方法依赖于群体分布对比，容易混淆宏观模式匹配与连贯的个体层面预测，因此需要更严格的个体级预测评估方法。

Method: 提出跨调查迁移框架（基于受访者对部分问题的回答预测其对其他问题的回答），并利用TEDS 2024数据集、三个开源大模型（27B-120B）以及监督机器学习基线（如随机森林）进行实验对比。

Result: 零样本LLM在未见问题上实现了52%的预测准确率，与监督学习基线仅相差6个百分点；不同主题的可预测性存在差异（党派态度67%至主权问题23%）；方差崩溃和安全对齐效应对模型的影响比以往报道的更为复杂。

Conclusion: 该研究明确了利用大语言模型进行硅基采样（模拟人类受访者）的潜力与边界。
...
### [34] [APeB: Benchmarking Personalization Ability of Large Language Model Agents](https://arxiv.org/abs/2607.03162)
...
TL;DR: 本文提出了用于评估LLM智能体在模糊查询和复杂历史下个性化能力的基准APeB，并引入了一种历史感知的查询细化管道VQRA以提升其表现。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试多使用已精炼的查询或简化历史，无法有效评估智能体在面对用户原始、未具体说明的查询时，从嘈杂历史中推断潜在意图和个性化偏好的能力。

Method: 构建了基于实际操作日志的个性化商品搜索基准（APeB），评估了前沿LLM在多步工作流中的表现，并提出了一种历史感知的查询细化管道（VQRA）来辅助智能体。

Result: 评估显示，模型在处理需要意图和偏好发掘的早期模糊查询时表现较差，主要归因于对历史记录的利用不足；而采用VQRA管道能带来显著且一致的性能提升。

Conclusion: 个性化智能体需要专门的历史信息利用模块，APeB基准和VQRA为提升LLM智能体在现实模糊场景下的个性化能力提供了重要参考。
...
### [35] [Organizational Memory for Agentic Business Process Execution](https://arxiv.org/abs/2607.03228)
*Lukas Kirchdorfer,Adrian Rebmann,Christian Warmuth,Timotheus Kampik,Theiss Heilker,Gregor Berg*

Main category: cs.AI

TL;DR: 提出了针对智能体业务流程执行的“组织记忆”架构，以解决大模型缺乏企业特定知识且现有方式难以扩展的问题。


<details>
  <summary>Details</summary>
Motivation: 通用大模型缺乏执行特定业务所需的组织知识，而现有的将知识写入提示词或独立检索的方法在企业中难以扩展，易导致知识孤岛和更新困难。

Method: 提出了一个共享、受控且智能体可直接利用的“组织记忆”参考层，并设计了用于其构建和消费的系统架构。

Result: 通过一个采购场景的原理验证（PoC）展示了该架构在自动化业务流程执行中的有效性。

Conclusion: 引入“组织记忆”作为企业级程序性知识的统一管理层，能够有效解决多智能体协同中的知识同步与可靠执行问题。
...
### [36] [Quantum-Inspired Harmonic Decision Models: A Computational Framework for Music Generation](https://arxiv.org/abs/2607.05007)
*Josef Pavlíček,Petra Pavlíčková,Martin Molhanec*

Main category: cs.AI

TL;DR: 本文介绍了一种用于音乐和声决策的量子启发式计算框架，将和声生成视为受限空间中的优化问题。


<details>
  <summary>Details</summary>
Motivation: 旨在探索利用量子启发式方法来解决音乐创作等创造性领域中复杂的决策与多约束优化问题。

Method: 结合了基于干涉的量子启发式和声化阶段（用于并行评估多种和声候选）与基于传统调性和声的经典优化程序（用于精细化序列）。

Result: 实验表明，该优化阶段显著降低了和弦密度、提高了和声稳定性与功能组织性，且专家评估强调了风格上下文对自然感的影响。

Conclusion: 该框架表明量子启发式方法能够有效建模创造性领域中的复杂决策过程，为结合领域知识与干涉搜索机制提供了可行模型。
...
### [37] [Embodied Operators and Benchmarking: Toward Reusable and Deployable Embodied Intelligence Systems](https://arxiv.org/abs/2607.03283)
...
TL;DR: 本文定义并系统性地研究了具身智能系统中的“具身算子”（可复用的功能模块），提出了分类体系和多维度评估基准。


<details>
  <summary>Details</summary>
Motivation: 具身智能系统不仅需要端到端策略，还需要能将多模态观测和任务上下文转化为结构化表示与控制指令的可复用功能模块。

Method: 定义了具身算子的边界，构建了涵盖五个类别的分类法，并提出了一个包含正确性、效率和可靠性等多维度指标的评估基准框架。

Result: 系统地归纳了各类别算子的代表性功能、技术范式和局限性，并探讨了工作流级加速及算子组合等开放性挑战。

Conclusion: 具身算子应作为整体可部署组件进行优化和评估，这为构建可复用、可扩展且可验证的具身智能系统奠定了基础。
...
### [38] [When Aggregate Alignment Misleads: Auditing Policy Repair Without Per-State Expert Actions](https://arxiv.org/abs/2607.03386)
*Peiying Zhu,Sidi Chang*

Main category: cs.AI

TL;DR: 本文研究了在仅有区域级诊断反馈而无专家标签的酒店定价模拟器中，使用大语言模型（LLM）进行策略修复的效果，证明其能有效恢复收入并减少策略偏差。


<details>
  <summary>Details</summary>
Motivation: 智能体AI系统在修复决策策略时，往往缺乏逐状态的专家动作标签，仅能获得粗粒度的诊断反馈，这使得评估和优化策略修复变得非常困难。

Method: 在酒店定价模拟器中，使用多重启LLM编辑器，仅根据区域级诊断反馈对目标动作表进行受限修改，并与非语义搜索、混淆控制以及树编辑器等方法进行对比。

Result: LLM编辑器实现了108.47的RevPAR（接近基准的108.75），并显著降低了情境组合距离。而非语义搜索表现较差，树编辑器虽行为对齐度高但收入表现明显下滑。

Conclusion: 评估智能体策略修复时不应仅依赖单一的行为距离指标，而应看诊断反馈是否能转化为可靠的闭环实际结果。
...
### [39] [Reflective Dialogue or Prompt Refinement? Effects of Tutor Scaffolding on Students' Independent LLM Use for Programming](https://arxiv.org/abs/2607.03303)
*Jerome Brender,Laila El-Hamamsy,Kim Uittenhove,Aitor Perez,Patrick Jermann,Francesco Mondada,Engin Bumbacher*

Main category: cs.AI

TL;DR: 研究比较了苏格拉底式引导（SG）和提示词优化（PR）两种LLM辅导模式，发现SG更有助于提升学生的长期学习效果和建立理解导向的提示策略。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM能提供个性化学习支持，但其效果取决于学生的交互方式，因此需要研究不同LLM导师设计对学生提示行为和学习成效的长期影响。

Method: 在研究生课程中开展两阶段研究：先让66名学生使用SG或PR导师6周，随后让52名学生在无限制的LLM环境下完成3周项目，对比分析其学习增益和提示策略。

Result: 两组在引导期的任务表现相似，但SG组学生在后期获得了更高的学习增益，且在使用无限制LLM时更倾向于使用理解驱动的提示策略，尽管他们主观认为SG效率较低。

Conclusion: 苏格拉底式引导能够有效培养学生长期利用LLM进行自主学习的能力，这为未来LLM教学助手的设计提供了重要指导。
...
### [40] [From Mobile Data to Business Insights: An End-to-End Analytics Framework for Large-Scale Urban Mobility Analysis and Decision Support](https://arxiv.org/abs/2607.03394)
*Thiago Andrade,Shazia Tabassum,Miguel E. P. Silva,Ricardo Dinis,Joao Gama*

Main category: cs.AI

TL;DR: 本文开发了一个基于移动应用定位数据的模块化分析平台，用于捕捉城市用户的出行动态并辅助决策。


<details>
  <summary>Details</summary>
Motivation: 旨在利用移动应用实时位置数据解决旅游规划、交通管理和商业零售等领域的城市挑战，克服多场景应用下的数据处理难题。

Method: 构建了包含数据匿名化、ETL管道、Google BigQuery和Vertex AI的模块化架构，并结合Power BI进行交互式可视化，应用了多种迁移率分析模型。

Result: 成功在细粒度的时空分辨率下捕获了用户出行动态，并生成了支持多利益相关者用例的数据产品。

Conclusion: 该框架为城市规划和战略商业决策提供了可操作的洞察，证明了模块化分析平台在处理复杂城市移动性任务中的有效性。
...
### [41] [Efficient bias mitigation in T2I diffusion models using Concept Graphs](https://arxiv.org/abs/2607.03397)
*Mansi,Avinash Kori,Francesco Leofante*

Main category: cs.AI

TL;DR: 提出CO-ALIGN方法，通过对齐文本编码器和去噪器的内部概念本体，在保持图像生成质量的同时，有效减少文生图扩散模型的偏见。


<details>
  <summary>Details</summary>
Motivation: 现有的文生图偏见缓解技术往往仅干预文本编码器或在推理阶段进行引导，容易导致生成的图像出现语义不连贯的问题。

Method: 引入基于概念图对齐的CO-ALIGN方法，在文本编码器、去噪器以及两者联合的三个层面上对齐模型的内部概念本体。

Result: CO-ALIGN使公平性提高了30%，图像质量ΔFID提升11.4，保真度提升2.8%，语义不连贯输出减少88%，并增强了概念遗忘任务的鲁棒性。

Conclusion: 通过对齐内部概念本体，CO-ALIGN不仅能显著减少模型偏见并保留生成质量，还能有效提升其他下游任务（如概念遗忘）的性能。
...
### [42] [Personalized Causal Recourse: A Human-In-The-Loop Approach](https://arxiv.org/abs/2607.03425)
*Denise Tampieri,Giovanni De Toni,Paolo Giudici*

Main category: cs.AI

TL;DR: 本文提出了一种人机协同框架，通过交互式贝叶斯推理逐步逼近用户的结构因果模型，以提供个性化的算法追索建议。


<details>
  <summary>Details</summary>
Motivation: 传统的算法追索方法通常依赖最接近的反事实解释或假设先验的因果结构，忽略了个人背景和特征之间的具体交互。

Method: 采用人机协同框架，通过交互式查询和贝叶斯推理迭代地近似用户的结构因果模型，并利用人类反馈改善因果效应的识别。

Result: 在模拟评估中，该框架在解析线性和非线性因果模型方面表现出良好效果，但在捕捉复杂非线性结构时仍存在挑战。

Conclusion: 利用人类反馈可实现更个性化、低成本的因果追索，但未来研究需关注更精确的因果近似和鲁棒的噪声分布建模。
...
### [43] [Demonstrating Generalization Failures via Mixtures of Conditional Policies](https://arxiv.org/abs/2607.03478)
*Jou Barzdukas,Jack Peck,Julian Schulz,Paulius Rauba,Steven Basart,Lennie Wells*

Main category: cs.AI

TL;DR: 提出了一种构建语言模型（“模型生物”）的简单方法，使其在强化学习训练后以可控的方式发生泛化失败，用以研究泛化与对齐问题。


<details>
  <summary>Details</summary>
Motivation: 前沿语言模型在训练和部署环境之间存在分布偏移，导致难以理解的泛化失败。为了更好研究这一现象，需要构建简化且可控的泛化失败示例。

Method: 通过监督微调（SFT）混合了多种“条件策略”的数据集来构建模型，随后使用强化学习（RL）进行训练，使其在特定训练分布上选择能获得最高奖励的策略。

Result: RL训练会选择性地提高训练分布上的表现，却将另一分布上的性能降至零；同时展示了任务覆盖率和时间上下文分布偏移导致的新型泛化失败。

Conclusion: 该方法构建的“模型生物”可用于对齐压力测试和泛化科学研究，证明了训练成功与泛化能力可能会以结构化的方式走向分裂。
...
### [44] [Applying Answer Set Programming with Fuzzy Membership Functions: a Case Study](https://arxiv.org/abs/2607.03550)
*Luca Ferragina,Ilenia Galati,Lorena Gullone,Francesco Scarcello*

Main category: cs.AI

TL;DR: 一种基于模糊逻辑的回答集程序设计（ASP）定性扩展框架，旨在桥接数值数据与定性符号推理。


<details>
  <summary>Details</summary>
Motivation: 人类推理常用模糊且依赖上下文的定性标签，传统方法缺乏有效连接数值信息与定性推理的鲁棒机制。

Method: 提出一种模糊逻辑定性ASP扩展框架，利用基于学习的隶属度函数和语义丰富谓词，整合数值输入（如机器学习输出）与符号推理。

Result: 通过典型用例展示了该框架如何避免刚性阈值，成功将数值数据与定性标签符号推理相融合。

Conclusion: 该框架在统一的声明式设置中实现了专家知识、上下文和主观解释的结合，为模糊环境下的鲁棒推理提供了可行方案。
...
### [45] [MentalThink: Shaping Thoughts in Mental SVG World](https://arxiv.org/abs/2607.03530)
...
TL;DR: 介绍了一种名为 MentalThink 的视觉符号推理范式，通过生成和渲染 SVG 作为中间视觉表征，赋予多模态大模型“心理”成像与空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 旨在克服多模态大模型在复杂空间理解和多轮推理中的局限，模拟人类通过心理表象进行空间假设和检验的过程。

Method: 采用 think-with-SVG 管道，结合监督微调（SFT）进行语法对齐，以及多轮强化学习（RL）实现视觉假设的迭代修改与优化。

Result: 在空间推理基准测试中取得显著提升（VSIBench 达到 55.1%，MindCube 达到 76.0%），验证了矢量图形作为可验证视觉工作空间的有效性。

Conclusion: MentalThink 证明了可执行的矢量图形能有效作为多模态大模型的视觉工作区，显著增强了模型的动态空间与几何推理能力。
...
### [46] [The Role of Rigor in Artificial Intelligence](https://arxiv.org/abs/2607.03634)
*Timothy Nguyen*

Main category: cs.AI

TL;DR: 本文提出了一个包含概念、认识和操作严谨性的三维框架，用于分析和引导人工智能走向成熟的科学体系。


<details>
  <summary>Details</summary>
Motivation: 现代AI虽然发展迅速，但缺乏成熟学科所需的理论和科学基础，主要依赖性能驱动的迭代和经验尝试。

Method: 提出并应用了一个由“概念严谨性”、“认识严谨性”和“操作严谨性”组成的三部式分析框架。

Result: 分析了不同智能概念、深度学习的实证局限性、基准测试的利弊以及理论发展的障碍，揭示了操作严谨性在现代深度学习中的主导地位。

Conclusion: AI的快速发展与不确定性源于不同严谨性形式的相互作用，该框架为将AI转化为成熟科学和可靠技术提供了清晰的路径。
...
### [47] [How to Avoid Debate: Scalable AI Safety via Doubly-Efficient Interactive Proofs](https://arxiv.org/abs/2607.03561)
*Liyan Chen,Yael Tauman Kalai,Zoe Xi*

Main category: cs.AI

TL;DR: 本文研究了用于AI安全性的单证明者交互式证明系统，表明无需双模型辩论即可实现交互式验证。


<details>
  <summary>Details</summary>
Motivation: 现有的“辩论”验证机制假设两个AI模型能力对等且其中一个是诚实的，这不切实际；而传统的单证明者证明无法适用于包含外部预言机（如人类判断或数据库查询）辅助的计算。

Method: 提出了针对预言机辅助计算的双重高效单证明者交互式证明，适用于两种设定：(1) 对少数预言机错误具有容忍性的鲁棒计算，(2) 低度多项式预言机。

Result: 证明了在结构化或容错的预言机访问条件下，单证明者交互式验证是可行的。

Conclusion: 交互式AI安全性验证可以在没有辩论的情况下实现，为AI验证提供了一种无需双模型对抗的新途径。
...
### [48] [Robust Feasible Route Construction through Collaborative Partition Optimization](https://arxiv.org/abs/2607.03694)
*Oguzhan Karaahmetoglu,Hyong Kim*

Main category: cs.AI

TL;DR: 提出协同路由构建器（CoRC）框架，通过允许子问题在优化过程中交换客户和车辆，有效解决大规模CVRP中的需求未满足问题。


<details>
  <summary>Details</summary>
Motivation: 传统的大规模CVRP划分独立求解方法容易导致部分客户需求因分区固定而无法被服务，即使其他区域仍有闲置资源。

Method: 设计了CoRC框架，使各独立求解的子问题在优化过程中能够动态交换客户和车辆，避免依赖固定的分区或后期全局重优化。

Result: 在包含多达20万客户的测试中，CoRC一致性地构建出可行解，而传统的划分方法则无法做到；同时，在相同计算预算下，其效果优于现有的端到端路由框架。

Conclusion: 子问题间的协同为解决大规模可行性路径规划提供了一种具有高鲁棒性和可扩展性的新途径。
...
### [49] [Agent Reinforcement Learning via Pivotal-Aware Self-Feedback Retry](https://arxiv.org/abs/2607.03702)
*Weiyang Guo,Zesheng Shi,Longhui Zhang,Zeen Zhu,Min Zhang,Jing Li*

Main category: cs.AI

TL;DR: 提出PivoARL框架，通过定位关键错误状态进行局部重试，以降低LLM智能体在失败轨迹中的交互成本并提升经验利用率。


<details>
  <summary>Details</summary>
Motivation: LLM智能体难以有效利用失败轨迹：完全重试会导致高昂的交互成本，而经验检索又会稀释关键经验信号。

Method: 通过结构化反思定位关键错误步骤，并仅从该状态进行局部重试以复用正确前缀；同时设计了关键感知信用分配机制，并利用隐式反思回报优化反思质量。

Result: 在多项任务中，PivoARL在Pass@2/3上相比MetaRL平均提升约11.5%，在扫雷任务中相比GiGPO性能提升超45%且交互轮数减少约42%。

Conclusion: PivoARL通过局部关键重试和信用分配，有效解决了LLM智能体利用失败经验时交互成本高和信号稀释的问题。
...
### [50] [Explainable Reinforcement Learning for Adaptive Traffic Signal Control](https://arxiv.org/abs/2607.03703)
*Dickens Kwesiga,Nishu Choudhary,Angshuman Guin,Michael Hunter*

Main category: cs.AI

TL;DR: 本文提出了一种兼顾高优化性能与决策可解释性的实体中心强化学习交通信号控制框架。


<details>
  <summary>Details</summary>
Motivation: 传统深度强化学习算法在交通控制中具有黑盒特性，缺乏可解释性与安全保障，难以获得交通监管部门的信任和实际部署。

Method: 将路口状态解耦为车道实体和相位配置以保留空间拓扑，采用双阶段注意力网络动态提取关联性，并结合动作遮罩机制以确保安全约束。

Result: 在微观交通仿真评估中超越了现有基线方法，有效降低了车辆延迟，且其注意力权重与经典交通工程原理高度吻合。

Conclusion: 该框架为下一代自适应交通信号控制提供了一种可审计、高安全且易于部署的透明化解决方案。
...
### [51] [Can Conversational Temporal Dynamics Improve Depression Detection in Dyads? A Preliminary Investigation in Multi-Modality Perspectives](https://arxiv.org/abs/2607.03744)
*Hanie Kang,Huang-Cheng Chou,Sudarsana Reddy Kadiri,Shrikanth Narayanan*

Main category: cs.AI

TL;DR: 提出一种利用对话轮替时序特征进行抑郁症检测的方法，证明其是比声学特征更轻量且有效的辅助模态。


<details>
  <summary>Details</summary>
Motivation: 传统的自动抑郁症检测忽略了临床医生与患者之间对话互动的时序动态特征。

Method: 设计了一个24维的对话时序特征模块，并使用凸加权晚期融合策略将其与自监督编码器（WavLM-large和RoBERTa-large）进行融合，在DAIC-WOZ数据集上进行评估。

Result: 时序模块在开发集上取得单模态最优性能；融合策略使开发集和测试集的macro-F1分别达到0.804和0.669，且融合中声学特征的权重被有效归零。

Conclusion: 对话时序特征可以作为一种轻量、可解释的互补模态，有效辅助双人对话中的抑郁症筛查。
...
### [52] [Bridging Interleaved Multi-Modal Reasoning as a Unified Decision Process](https://arxiv.org/abs/2607.03748)
...
TL;DR: 提出BRAID框架，将多轮图文交织推理建模为统一的马尔可夫决策过程，实现文本与图像生成的联合强化学习优化。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅在文本步骤应用强化学习，将图像生成边缘化为监督替代任务，导致策略梯度无法跨异构模态的完整轨迹传播。

Method: 将推理过程建模为统一的MDP，计算共享的轨迹级优势并将其传播至文本Token和图像去噪路径，并引入VLM裁判为中间图像提供密集的反馈评分。

Result: 在空间推理和视觉感知基准测试中，BRAID一致且显著地优于现有的各类基准方法。

Conclusion: 将图文多模态推理视为统一的MDP，并结合视觉思维引导，对于提升多模态模型的推理能力至关重要。
...
### [53] [Folding, Reasoning, and Scaling with Open-source Drug Discovery Engine](https://arxiv.org/abs/2607.03787)
*Aureka AI OpenDDE project*

Main category: cs.AI

TL;DR: 本文介绍开源的全原子生物分子基础模型 OpenDDE，旨在通过协同折叠和开源生态加速 AI 辅助药物研发。


<details>
  <summary>Details</summary>
Motivation: 准确建模生物分子相互作用是当前生物学和药物研发的主要瓶颈。

Method: 引入 OpenDDE 模型，融合了全原子架构、原子潜推理、推理优化和大规模数据处理，并探索了协同折叠模型的缩放定律。

Result: 达到了与 IsoDDE 相当的共折叠准确率，并开源了完整的训练代码、推理管线、模型权重和基准测试。

Conclusion: OpenDDE 为下一代药物发现系统奠定了开源基础，推动了从单纯结构预测向药物分子设计和优化的转变。
...
### [54] [Evaluating LLM Uncertainty in Long-Form Generation Using Deterministic Ground Truth](https://arxiv.org/abs/2607.03870)
*Ido Amit,Ido Galil,Ran El-Yaniv*

Main category: cs.AI

TL;DR: 本文引入了SALT基准测试，用于在细粒度上评估大语言模型长文本生成的不确定性，并揭示了模型置信度估计的多个核心特性。


<details>
  <summary>Details</summary>
Motivation: 现有的长文本不确定性评估方法受限于不完美的标签，缺乏无噪声且具有确定性标准答案的基准测试。

Method: 提出了SALT基准，包含6个程序生成的任务，提供单一且确定性的长文本标准答案，支持在无需外部裁判的情况下进行单位级评估。

Result: 评估了50多个LLM，发现原子级置信度排序基本失效（但行级较好）；识别出未来错误的两个独立驱动因素；表明推理虽能提高准确率但会降低置信度排序能力。

Conclusion: 本研究的发现对于需要可靠错误识别和缓解的关键风险应用具有重要的指导意义。
...
### [55] [Harness-Aware Self-Evolving: Co-Evolving Model Weights, Harness, and Task Solutions](https://arxiv.org/abs/2607.03935)
*Haochen Luo,Yi Huang,Sichun Luo,Fengyuan Liu,Lei Li,Zefa Hu,Junlan Feng,Qi Liu*

Main category: cs.AI

TL;DR: 本文提出了HASE框架，这是一种能够同时优化任务解决方案和测试基准（harness）的单模型自演化增强学习框架。


<details>
  <summary>Details</summary>
Motivation: 传统的自演化框架在优化任务解决方案时，通常将周围的测试基准视为固定不变，限制了系统的演化潜力。

Method: 引入HASE框架，允许单个模型在多轮动作空间中，既能生成解决方案，又能编辑和修复测试基准的组件。

Result: 使用Qwen3-8B模型，HASE在文本分类任务中达到了GPT-OSS-120B的水平，在因子挖掘中超越了基线，并在圆包装算法发现中成功修复了评估组件并达到SOTA性能。

Conclusion: HASE证明了通过统一的Agent流程协同提升测试基准和解决方案的可行性与高效性。
...
### [56] [Online Linear Programming for Multi-Objective Routing in LLM Serving](https://arxiv.org/abs/2607.03948)
*Zixi Chen,Yinyu Ye,Zijie Zhou*

Main category: cs.AI

TL;DR: 提出一种基于在线线性规划的多目标优化路由框架，以提升大语言模型服务中的延迟与吞吐量性能。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM服务路由主要依赖启发式算法，缺乏与服务水平目标（SLO）的显式关联，且无法有效控制延迟与吞吐量之间的权衡。

Method: 将路由问题建模为带可解释奖励的在线线性规划，提出基于影子价格的报价控制策略，并开发了满足毫秒级决策需求的热启动投影一阶更新算法。

Result: 在Vidur模拟器上的实验表明，该方法在端到端延迟、首字时间（TTFT）、吞吐量及尾部性能等多项SLO指标上均显著优于传统基线。

Conclusion: 研究表明，相比于传统的启发式方法，基于数学优化的科学方法在LLM在线路由中表现出更优越的性能。
...
### [57] [Explainable AI for Screening Abuse-Related Trauma in Bangladeshi Children: A Training-Free Multimodal Framework Evaluated on Noise-Aware Synthetic Data](https://arxiv.org/abs/2607.04010)
*Salma Hoque Talukdar Koli,Fahima Haque Talukder Jely*

Main category: cs.AI

TL;DR: 本研究开发了 ShishuRaksha AI，这是一个结合四种模态的临床决策支持框架，用于在资源匮乏的孟加拉国早期筛查儿童受虐心理创伤。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国极度缺乏儿童心理健康专业人员，且缺乏针对本地语言和文化定制的儿童受虐心理创伤早期筛查工具。

Method: 融合了筛查问卷、孟加拉语叙述文本、房树人（HTP）绘画特征和面部表情四种模态，采用无训练、临床加权的融合机制；利用合成数据集评估了树集成替代模型的性能，并提供双语报告和可解释性分析。

Result: 融合模型在合成数据集上的 AUC 达到 0.874，显著优于仅基于问卷（SDQ）基线模型的 0.756。

Conclusion: 该研究证实了在低资源环境下以合乎伦理的方式部署多模态儿童保护筛查工具的可行性，并提供了相应的设计贡献。
...
### [58] [What is Left for Us? Second Scholarship Against the Degradation of Research by AI](https://arxiv.org/abs/2607.04049)
*Claudio Novelli,Luciano Floridi*

Main category: cs.AI

TL;DR: 本文探讨了生成式AI对学术研究的潜在危害，指出它侵蚀了培养学术判断力和信任的实践过程，并倡导回归“第二学术”以重塑学术手艺。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的高效输出促使研究人员将核心任务外包，导致其失去在研究实践中获得智力塑造和学术成长的机会。

Method: 通过理论论证和概念辨析，提出了基于默会知识、个人承诺、社会化和深度阅读四要素的“第二学术”（second scholarship）框架。

Result: 证明了仅让研究者充当AI的提示者或质检员不足以保留研究的育人价值；明确了学术研究作为一种经历摩擦、逐渐形成判断的“体验实践”的必要性。

Conclusion: 学术共同体必须珍视并坚守那些无法且不应被自动化替代的学术核心实践，以维持研究的本真价值。
...
### [59] [PLACEMEM: Toward a Compute-Aware Memory Plane for Lifelong Agents](https://arxiv.org/abs/2607.04089)
*Sukanta Ganguly*

Main category: cs.AI

TL;DR: 本文提出了 PLACEMEM，一种将终身智能体内存表示为版本化胶囊的系统方案，以实现持久且可修正的内存管理。


<details>
  <summary>Details</summary>
Motivation: 现有的长上下文和检索方案在终身智能体中会导致重复计算或重用过期状态，智能体需要一种能够持续演进且易于修正的内存机制。

Method: 引入版本化“胶囊”（capsules）概念统一内存属性，并构建了基于 vLLM 的原型系统，包含并发安全失效机制、兼容 OpenAI 的路由边车及基准测试套件。

Result: 成功实现了一个可运行的控制平面原型，能够有效进行纠错感知路由，并在基准测试中展现了低首字延迟和高效的内存重用。

Conclusion: PLACEMEM 证明了纠错感知控制平面的可行性，为未来支持重放的终身智能体服务系统整合提供了具体路线图。
...
### [60] [Language models guide symbolic equation discovery by controlling search](https://arxiv.org/abs/2607.04156)
*Zikai Xie,Wenmei Li,Man Luo,Jun Jiang,Linjiang Chen*

Main category: cs.AI

TL;DR: 提出将大语言模型（LLM）作为搜索控制器来引导符号回归（LLM-PySR），实现了准确性、复杂度和成本的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 传统的符号回归面临组合搜索空间爆炸的问题，而大模型直接生成方程又缺乏数值严谨性，需要寻找两者结合的最佳分工方式。

Method: 对比了LLM作为方程作者、决定者或搜索控制器的不同角色，并提出LLM-PySR：由LLM指定变量、算子和搜索深度等先验，由符号回归负责具体的表达式拟合。

Result: 在74个AI-Feynman物理方程和多个复杂任务中，LLM-PySR（搜索控制模式）取得了最佳的综合表现，并在实际电池数据集中成功发现了有效的物理关系。

Conclusion: LLM应该扮演引导假说探索的“控制器”角色，而非直接生成或决定最终方程的留存。
...
### [61] [Forethought: Verifiable Reasoning from Neurosymbolic Primitive Programming](https://arxiv.org/abs/2607.04096)
*Vishvesh Bhat,Jay Vaghasiya,Emmanuel Anaya Gonzalez*

Main category: cs.AI

TL;DR: Forethought 是一种神经符号推理系统，它将推理过程转化为显式且可验证的程序，以极低的成本显著提升了语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时缩放方法将推理能力隐式绑定在模型权重中，导致无法逐步验证且推理成本高昂。

Method: 提出 Forethought 系统，通过领域特定语言（DSL）将符号与神经原语组合，构建出可检查和修改的显式推理程序。

Result: 在五个基准测试中将基础模型准确率相对提升了约 30%，使小模型性能媲美前沿模型，且相比专用推理模型减少了三个数量级的后训练成本。

Conclusion: 将推理表示为显式程序是一种模型无关、可审计且高效的方案，能够有效替代传统的权重缠绕式推理缩放。
...
### [62] [A Clustering-Based Framework for Identifying Suspicious Trading Patterns in Capital Market](https://arxiv.org/abs/2607.04184)
*Asif Zaman,Romona Magdalene Sarkar,Sabiha Khair Ohi,Iftekharul Mobin*

Main category: cs.AI

TL;DR: 本研究提出了一种基于K-Means++聚类和市场启发式阈值的无监督欺诈检测方法，用于识别和分类金融交易中的市场操纵行为。


<details>
  <summary>Details</summary>
Motivation: 市场操纵行为损害了交易平台的信任度，亟需在缺乏标注数据的情况下有效识别欺诈交易。

Method: 采用K-Means++聚类算法对2012至2024年间约100万笔金融交易数据进行分析，并结合市场启发式阈值构建检测与分类管线。

Result: 成功将2.02%的交易标记为可疑交易（其中51.10%为虚假申报，0.10%为拉高出货，0.55%为内幕交易，1.43%为假突破，46.83%未分类），模型轮廓系数达到0.561。

Conclusion: 该聚类管线方法在无需地面真值标签的情况下，仍能有效识别并分类潜在的市场操纵行为。
...
### [63] [Agentic IoT: Architectures, Applications, and Challenges Toward the Internet of Agents](https://arxiv.org/abs/2607.04219)
*Rümeysa Hilal Sevinç,Bahaeddin Türkoğlu,İbrahim Kök*

Main category: cs.AI

TL;DR: 本文探讨了智能体物联网（Agentic IoT）这一新范式，将自主AI智能体的认知能力与物理信息系统融合，以构建分布式智能生态系统。


<details>
  <summary>Details</summary>
Motivation: 传统的AIoT系统依赖于特定任务的传感器推理模型，缺乏实时推理、自适应规划和自主协调等高级认知与系统级决策能力。

Method: 通过系统性文献综述，界定Agentic IoT与传统AIoT、边缘智能等概念的区别，提出整体架构框架，并分析了具体应用领域及挑战。

Result: 定义了Agentic IoT的范式转换，构建了跨“设备/边缘-雾-云”的架构，并梳理了该领域的关键技术瓶颈与未来研究方向。

Conclusion: Agentic IoT代表了物联网向分布式认知代理生态系统的演进方向，能有效提升网络化物理系统的自主性与协同效率。
...
### [64] [Unsupervised Features Mining via Activation Geometry](https://arxiv.org/abs/2607.04222)
*Amit LeVi,Elad David,Max Fomin*

Main category: cs.AI

TL;DR: 本文提出了一种名为MAG的无监督框架，通过比较加入指令前后激活值的几何变化来提取大语言模型的推理特征。


<details>
  <summary>Details</summary>
Motivation: 传统可解释性方法依赖带有偏见的人类标记数据，因此需要一种简单、无监督的方法来直接提取大模型内部的推理特征。

Method: 提出“激活几何挖掘”（MAG）框架，通过在输入前添加特定指令，并测量单点读出激活值的变化量 m(Q|p) - m(p) 来提取特征。

Result: 提取的特征不仅能预测模型判断，还可以通过激活引导改变模型决策；在提示注入分类的数据集选择中，该方法实现了94.7%的Top-1和100%的Top-2准确率。

Conclusion: MAG是一个简单且有效的无监督框架，能成功提取和引导LLM的内部推理特征，并在下游数据集选择任务中表现优异。
...
### [65] [Biological Motifs for Agentic Control](https://arxiv.org/abs/2607.04240)
*Bogdan Banu*

Main category: cs.AI

TL;DR: 本文提出了一种基于系统生物学基序和范畴论的LLM智能体架构设计框架，旨在解决现有智能体系统的安全与可靠性挑战。


<details>
  <summary>Details</summary>
Motivation: 当前的LLM智能体系统多为临时构建，容易面临幻觉级联、无限循环和提示词注入攻击等可靠性与安全性问题。

Method: 利用多项式演算和接线图，建立基因调控网络与智能体系统的类型接口对应关系，将生物基序映射为软件设计模式，并构建认识拓扑层以推导知识算子。

Result: 开发了具有误差抑制边界的智能体操作子，推导了四个多智能体扩展定理（预测结果与现有基准一致），并提供了一个包含1,813个测试的参考实现。

Conclusion: 该研究通过引入生物系统控制机制的形式化数学框架，为构建安全、可靠和可扩展的LLM多智能体系统提供了新途径。
...
### [66] [Shortcut Learning in Legal Judgment Prediction: Empirical Evidence from the UK Employment Tribunal](https://arxiv.org/abs/2607.04261)
*Joe Watson,Joana Ribeiro de Faria,Marcus Tomalin,Måns Magnusson,Huiyuan Xie,Hao Tian Yeung,Felix Steffek*

Main category: cs.AI

TL;DR: 本文研究了法律判决预测（LJP）中由于依赖事后司法材料而导致的捷径学习与信息泄露问题，并提出了相应的评估与优化方法。


<details>
  <summary>Details</summary>
Motivation: 现有的法律判决预测模型主要依赖判决后的文本，这使得模型更容易进行回顾式分类而非真正的预测，存在因语言伪影导致性能被夸大的风险。

Method: 利用33,158件英国就业裁判所的申诉数据，采用从TF-IDF到大语言模型的多种方法进行预测，并通过人工评估数据泄露程度对测试集进行分层，对比了掩膜泄露特征前后的模型表现。

Result: 实验表明模型的强劲表现很大程度上由文本中嵌入的泄露线索驱动，仅用4%的泄露特征训练的模型即可超越人类专家；但掩膜这些特征后，模型的预测表现仅出现微幅下降。

Conclusion: 虽然法律判决预测模型倾向于利用捷径，但通过主动审计和掩膜泄露特征，模型在消除这些干扰后仍能提取出有效的预测信号。
...
### [67] [Progress- and Reliability-Oriented Group Policy Optimization for Agentic Reinforcement Learning](https://arxiv.org/abs/2607.04242)
*Mingxuan Fan,Peiyang Liu*

Main category: cs.AI

TL;DR: 提出ProGPO，一种无需学习Critic的、上下文一致的步骤级组强化学习方法，旨在优化大语言模型Agent在交互任务中的策略更新。


<details>
  <summary>Details</summary>
Motivation: 步骤级组强化学习在分组时存在两难：按宽泛状态分组会引入不一致的历史对比，而强制历史一致性则会导致分组碎片化、丢失比较信号。

Method: 提出ProGPO方法，保持精确前缀的动作对比，利用基于Rollout状态势能的转移信用值补充稀疏的比较信号，并通过语义扩展和逆方差融合来可靠估计势能。

Result: 在ALFWorld和WebShop任务上，ProGPO在使用Qwen2.5-1.5B-Instruct时以相近的计算开销超越了对比基线，并在Qwen2.5-3B-Instruct上展现了良好的可扩展性。

Conclusion: ProGPO为大模型Agent的步骤级强化学习提供了一种无需Critic、上下文一致且高效可扩展的解决方案。
...
### [68] [Agentic SABRE: An Uncertainty-Aware Neuro-Symbolic Multi-Agent Framework for Adaptive Ransomware Detection](https://arxiv.org/abs/2607.04292)
*Henry Kabuye,Biju Issac,Jeyamohan Neera*

Main category: cs.AI

TL;DR: 提出了 Agentic SABRE，一种结合语义与行为特征、具备不确定性感知和可解释性的多智能体勒索软件检测框架。


<details>
  <summary>Details</summary>
Motivation: 传统的静态特征和单体分类器在面对勒索软件的概念漂移、逃避和行为多态性时难以有效泛化。

Method: 融合语义表征与行为时域遥测数据，利用蒙特卡洛 Dropout 量化不确定性，通过决策层协调器进行风险与不确定性分流，并引入多种后验可解释性机制。

Result: 在测试中实现了语义数据集上 1.0 的 AUC，在保持召回率的同时减少了高达 4.9% 的错误上报，并通过反事实分析证明了决策边界的稳定性。

Conclusion: Agentic SABRE 提供了一种鲁棒、可解释的勒索软件检测方案，在自主响应与人工审计之间建立起了灵活平衡的合作机制。
...
### [69] [HAS-Bench: Evaluating LLM-Based Human-Agent Systems under Configurable Human Participation](https://arxiv.org/abs/2607.04329)
...
TL;DR: 本文提出了HAS-Framework和HAS-Bench，用于建模和评估人机协同系统中人类与大模型智能体的合作表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地与人类进行主动协作，但缺乏一个将人类和智能体都视为一等公民并明确其角色与权限的系统性框架和评估基准。

Method: 提出了基于图的HAS-Framework来定义人类与智能体的角色和权限，并构建了HAS-Bench基准，在不同的人类参与配置下评估任务结果和协作过程。

Result: 在六个领域的实验表明，人类参与能显著提升任务完成度和错误恢复能力，但实际收益取决于人类输入的时间、方式和主体。

Conclusion: 该研究为理解和优化人类-智能体协同系统提供了系统性的建模与评估工具，展示了合理配置人类参与对提升系统性能的关键作用。
...
### [70] [Do GUI Agents Believe Their Eyes? Diagnosing State-Belief Reliance on Pixels versus Structure](https://arxiv.org/abs/2607.04334)
*Guijia Zhang,Harry Yang*

Main category: cs.AI

TL;DR: 本文研究了多模态GUI智能体在理解界面状态时，究竟是依赖视觉像素还是序列化结构（如DOM），并提出了一种量化这种依赖的方法。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对智能体状态信念来源（像素 vs. 结构）的评估，无法确定智能体是否真正实现了视觉接地。

Method: 提出了“视觉状态依赖”概念，通过310个多端探针进行单通道干预实验，并引入“感知-融合差距”（Perception-Fusion Gap）指标进行定量评估。

Result: 实验表明，五款主流模型在面临冲突时更倾向于信赖结构文本而非视觉像素，导致感知-融合差距为正值，且这种偏好会导致实际任务执行失败；而基于坐标动作的智能体受此影响较小。

Conclusion: 视觉状态依赖是诊断智能体视觉接地能力的有效指标，暴露了模型对结构文本的过度依赖及其引发的动作错误。
...
### [71] [Nemotron-Labs-3-Puzzle-75B-A9B: Compressing Hybrid MoE LLMs](https://arxiv.org/abs/2607.04371)
...
TL;DR: 本文介绍了Nemotron-Labs-3-Puzzle-75B-A9B，这是Nemotron-3-Super的压缩版本，旨在提升交互式部署的服务器吞吐量。


<details>
  <summary>Details</summary>
Motivation: 旨在在严格的用户吞吐量限制下最大化服务器吞吐量，并提升超长上下文部署中的并发处理能力。

Method: 结合了Iterative Puzzle压缩框架、知识蒸馏、强化学习、量化及多Token预测（MTP）技术，联合优化异构MoE剪枝、活动参数预算和Mamba剪枝。

Result: 在8xB200节点上实现了约2倍的服务器吞吐量提升，在单张H100上将1M上下文并发量从1个请求提升至8个，同时保留了强大的下游任务准确率。

Conclusion: 大型混合MoE模型可以在大幅提升部署和推理效率的同时，有效维持其强大的下游能力。
...
### [72] [Server-side Anti-cheat in FPS games for Aimbot detection using Deep learning and Machine learning](https://arxiv.org/abs/2607.04336)
*Siddhesh A. Dhinge,Shubham G. Sukum,Harsh S. Ranjane,Ruturajsingh R. Rajput,Jyoti H. Jadhav*

Main category: cs.AI

TL;DR: 本文提出了一个名为YAACS的服务器端FPS游戏反作弊系统，专门用于检测自动瞄准（aimbot）外挂。


<details>
  <summary>Details</summary>
Motivation: 多玩家FPS游戏中作弊行为日益增多，破坏了游戏公平性，亟需有效的自动瞄准检测方法以保障玩家体验。

Method: 提取玩家的时序数据（如瞄准速度、射击数）和行为数据，构建包含解析器、深度学习模型（堆叠LSTM）及接口工具的YAACS系统进行检测。

Result: 堆叠LSTM模型实现了88.6%的分类准确率，且误报率低至0.97%，其误报控制能力明显优于决策树基线模型（误报率为2.68%）。

Conclusion: 在FPS游戏作弊检测中，通过序列建模引入时间上下文对于将误报率降至最低以避免误判至关重要。
...
TL;DR: 提出了WALLA，一种用于去中心化LLM预测聚合的优势对齐下注机制，以模型自主学习的下注额作为聚合权重。


<details>
  <summary>Details</summary>
Motivation: 在去中心化设置中聚合多个LLM的预测时，需要在不访问模型私有信息的情况下确定权重，并能抵抗模型的策略性报告行为。

Method: 设计了WALLA机制，模型提交预测和下注额。通过在净支付函数中引入留一法（leave-one-out）基线，实现激励相容性、优势-下注对齐以及独立于预测的下注优化，并实例化了两种机制变体。

Result: 在问答和预测基准测试中，WALLA在预测性能上达到了与中心化聚合方法相当的水准，同时实现了去中心化学习、不确定性感知和激励相容的预测。

Conclusion: WALLA为去中心化、异构LLM的预测聚合提供了一种安全、鲁棒且高效的解决方案。
...
### [74] [LLM-as-a-Tutor: Policy-Aware Prompt Adaptation for Non-Verifiable RL](https://arxiv.org/abs/2607.04412)
...
TL;DR: 本文提出了LLM-as-a-Tutor框架，通过动态调整提示词难度来解决强化学习中静态提示词与策略能力不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 在不可验证指令遵循的强化学习中，静态的训练提示词会导致提示词难度与策略能力失配，使LLM裁判因无法在策略输出中产生质量差异而失效。

Method: 引入LLM-as-a-Tutor框架，让单一LLM同时担任考官（对比策略输出以识别无挑战的提示词）和生成器（通过追加原子约束以递增式提高提示词难度）。

Result: 在三个复杂的指令遵循基准测试中，该方法一致优于策略无关的基线方法以及仅调整评估标准或重写提示词的已有自适应方法。

Conclusion: 提示词自适应是不可验证强化学习中实现策略感知的关键且此前被忽视的维度。
...
### [75] [MechMath Agent Team: LLM Driven Agents for Mathematical Research](https://arxiv.org/abs/2607.04394)
*Yichuan Cao,Ruichen Qiu,Junqi Liu,Jiaqi Wang,Dakai Guo,Ruyong Feng,Lihong Zhi,Xiao-Shan Gao*

Main category: cs.AI

TL;DR: 本文介绍了MechMath智能体团队（MMAT），这是一个旨在辅助全周期数学研究的大语言模型驱动多智能体系统。


<details>
  <summary>Details</summary>
Motivation: 传统AI推理系统难以应对数学研究中非线性推导路径、严密逻辑要求以及漫长探索周期带来的挑战。

Method: 设计了将系统职责解耦为控制、执行和增强三层的“三元马具架构”（Harness Architecture），并实例化了知识库管理、自然语言证明和形式语言证明三个专用智能体进行闭环协作。

Result: 在数论、代数复杂度理论等多个领域的开放问题上进行了评估，在为期两个月的部署中成功解决了11个开放数学问题。

Conclusion: MMAT展现了其作为数学研究全周期协作者的能力，证实了解耦的马具架构在多智能体数学推理中的有效性。
...
### [76] [Agent Step Value: State-Transition Measurement with State-Grounded LLM Evaluators](https://arxiv.org/abs/2607.04419)
*Andrew Zhang,Chengzhan Li*

Main category: cs.AI

TL;DR: 本文提出了Agent Step Value (ASV)，这是一种用于评估智能体单步操作对状态转换贡献的细粒度度量框架。


<details>
  <summary>Details</summary>
Motivation: 传统的智能体评估通常只关注最终结果或整条轨迹的得分，无法诊断具体是哪一步操作带来了有用或有害的状态改变。

Method: 引入ASV框架，通过对比动作前后的状态投影，利用无状态大语言模型（LLM）评估器计算候选结果的对数得分，并采用无标签推理步骤来隔离评估器深思过程与单Token选项评分。

Result: 在包含1100个步骤和2200个状态的100个开放式问答检索任务中，ASV成功测得平均黄金边际增益为-2.335，熵变动为0.000，贝叶斯惊奇度为2.693。

Conclusion: ASV能够有效定位传统最终得分和仅依靠熵的步骤指标所遗漏的建设性及破坏性信念转折点，并已开源该评估工具包。
...
### [77] [ResearchStudio-Idea: An Evidence-Grounded Research-Ideation Skill Suite from ML Conference Outcomes](https://arxiv.org/abs/2607.04439)
*Qihao Zhao,Yangyu Huang,Yalun Dai,Lingao Xiao,Jianjun Gao,Xin Zhang,Wenshan Wu,Scarlett Li,Yang He,Yan Lu,Yap Kim Hui*

Main category: cs.AI

TL;DR: 本文提出了 ResearchStudio-Idea 技能套件（核心为 IdeaSpark），利用从近两千篇顶会论文中提取的 15 种可重用构思模式，辅助研究人员生成高质量、有依据且经过查重审计的研究提案。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型降低了研究构思的门槛，但要形成有效的研究方案，仍需在文献定位、瓶颈识别、差异化对比和风险评估等“第一英里”环节进行深度论证，这正是目前 AI 辅助研究的痛点。

Method: 开发了包含文献检索、新颖性检测及端到端构思（IdeaSpark）的工具套件。IdeaSpark 基于 2021-2025 年间 ICLR/ICML/NeurIPS 的 1,947 篇论文，归纳出 15 种构思模式，并通过证据评估、模式引导生成、冲突检索和审计工作流输出研究提案卡。

Result: 盲法自动评审结果显示，IdeaSpark 产生的科研提案在质量上显著优于无技能和通用技能基线，同时在学术新颖性上保持了竞争力。

Conclusion: 该工作证明了通过结构化的可重用构思模式和严谨的审计工作流，能够有效提升 AI 生成研究提案的质量和可追踪性。
...
### [78] [Why Pure Reasoning is Not Enough: Nature as the Source of Mathematical Innovation](https://arxiv.org/abs/2607.04505)
*Charanjit S. Jutla,Vimal Sharma*

Main category: cs.AI

TL;DR: 人类数学推理受限于逻辑计算的复杂性，因而依赖源自自然物理世界的跨领域模式匹配；这一结论为当前大规模语言模型的合理性提供了理论支持。


<details>
  <summary>Details</summary>
Motivation: 探究人类如何在面对逻辑不可判定性和计算不可行性屏障时，实现数学推理和创造。

Method: 通过追溯傅里叶变换的历史演变（如热传导方程），并分析不同逻辑系统的计算复杂性上限进行论证。

Result: 历史和逻辑分析表明，纯形式推理无法预测许多关键数学工具的诞生，而物理启发式的模式匹配是克服逻辑计算屏障的认知必然。

Conclusion: AI若要实现人类级别的数学创造力，不能仅依赖纯逻辑推理，必须融入海量的跨领域模式，这合理解释了如今大语言模型为何需要庞大的参数规模。
...
### [79] [Compressing the Validation Bottleneck: An Agentic Self-Driving Lab for Scientific Discovery](https://arxiv.org/abs/2607.04508)
*Kyunghoon Hur,Chihun Lee*

Main category: cs.AI

TL;DR: 本文提出了一个双重策略的智能体框架，旨在通过减少实验轮次和降低单次实验成本来加速自动驾驶实验室（SDL）的科学发现进程。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶实验室目前面临两大瓶颈：一是在低价值实验上耗费过多轮次，二是高分辨率实验的单次成本过高。

Method: 引入单智能体系统，包含：（1）先验感知实验设计（DOE）循环，结合领域知识与历史结果推荐可行实验；（2）成本感知代理模型，基于不确定性在低成本和高成本测量间做出选择，并用低成本数据预测高成本结果。该方法在生物和材料领域进行了应用验证。

Result: 该智能体系统在生物和材料领域成功减少了所需的实验循环次数，并降低了每次实验的平均成本。

Conclusion: 结合先验感知与成本感知决策的智能体能够有效解决自动驾驶实验室的物理瓶颈，显著加速AI驱动的科学研究。
...
### [80] [VLA Grounder: Language-Conditioning Space Optimization for Black-Box VLA Models](https://arxiv.org/abs/2607.04517)
*Damir Shodiev,Aleksei Staroverov,Nikita Kachaev,Alexey K. Kovalev,Aleksandr I. Panov*

Main category: cs.AI

TL;DR: 通过强化学习将人类指令转化为更易于VLA模型执行的指令，在不修改VLA模型权重的情况下提升机器人的操作成功率。


<details>
  <summary>Details</summary>
Motivation: VLA模型对指令的具体措辞高度敏感，这表明可以通过优化语言输入而不是更新复杂的动作权重来提升模型表现。

Method: 提出一种语言条件空间优化方法。在冻结VLA模型的前提下，引入一个语言策略将人类指令转化为精简的落地命令，并基于失败先验通过强化学习和稀疏奖励对该策略进行优化。

Result: 在RL4VLA和VL-Think上的实验表明，该方法显著提升了机器人在指令敏感、符号化和多物体操作任务中的成功率。

Conclusion: 语言可以作为机器人基础模型的一个可优化变量，通过优化语言指令空间可以有效提升冻结VLA模型的表现。
...
### [81] [Heaviside Continuity of Rolling Coefficients for Eliminating Epistemic Entropy in Large Language Models](https://arxiv.org/abs/2607.04562)
*MY Pitsane,Hope Mogale*

Main category: cs.AI

TL;DR: 本文提出了一种名为HCRC的验证优先LLM执行框架，通过门控状态转换阻止错误传播，显著降低了错误生成率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型常生成流利但错误的输出，且自回归解码缺乏中间推理的验证机制，导致错误容易在后续步骤中累积和传播。

Method: 引入HCRC框架，将推理重构为受Heaviside门控制的状态转换，结合模型置信度与并行工作架构的独立验证信号，仅在满足预设正确性断言时允许状态推进。

Result: 在软件工程和推理任务中，HCRC将强模型的错误完成率（FCR）从4-7%降至0%，且延迟保持竞争力；对弱模型则能将其转化为诚实停机，防止破坏下游状态。

Conclusion: 研究表明，通过原则性的执行控制而非单纯依赖模型规模，可以有效实现可靠的LLM推理，为验证驱动的LLM执行提供了通用框架。
...
### [82] [Measuring Harness-Induced Belief Divergence in Multi-Step LLM Agents](https://arxiv.org/abs/2607.04528)
*Haiwen Yi,Xinyuan Song*

Main category: cs.AI

TL;DR: 本文研究了评估测试框架（harness）对软件智能体多步信念的影响，并提出了解析与对齐此类偏差的诊断工具及协议。


<details>
  <summary>Details</summary>
Motivation: 传统的智能体基准测试仅关注最终成败，忽略了控制交互界面的测试框架在任务和模型固定时仍会改变智能体内部信念的问题。

Method: 提出了信念展开诊断工具以提取K步轨迹，定义了跨框架信念散度，并设计了无需训练的BIWM协议来规范观察并对齐不同框架下的信念轨迹。

Result: 实验表明，阻止操作和压缩修复等框架设计虽然能保持最终成功率，但会显著改变智能体后续决策的信念；BIWM协议能有效对齐这些信念。

Conclusion: 测试框架的设计是智能体评估中一个重要且会引入偏差的实验变量，而不仅仅是无关紧要的实现细节。
...
### [83] [Attention Limited Reward Learning](https://arxiv.org/abs/2607.04590)
*Wenqian Xing*

Main category: cs.AI

TL;DR: 论文指出基于Bradley-Terry模型的RLHF忽略了人类有限注意力对偏好选择的影响，并提出人类反馈应被视为注意力受限的测量过程。


<details>
  <summary>Details</summary>
Motivation: 现有的RLHF偏好建模（如Bradley-Terry模型）混淆了“候选方案价值接近”与“受限于注意力难以区分”这两种模糊性，可能导致推断出错误的偏好排名。

Method: 提出一个受理性不关注（Rational Inattention）启发的简化评估通道模型，并结合Chatbot Arena的人类投票数据和感知比较实验进行案例分析。

Result: 研究表明有限注意力会扭曲偏好恢复；标准模型在注意力异质时会产生误导性排名；Chatbot Arena数据中存在标量奖励无法解释的循环偏好，且反应时间和视线能够提供标签之外的差距信息。

Conclusion: 人类偏好反馈不能简单等同于直接的价值偏好显现，弱偏好信号可能源于隐藏的评估难度而非真正的无差异。
...
### [84] [Detecting Answer-Driven Reasoning in LLM-Based Educational Tutors via Truncated Chain-of-Thought Auditing](https://arxiv.org/abs/2607.04572)
*Bonan Shen,Dingyan Shang,Youting Wang,Tao Ning*

Main category: cs.AI

TL;DR: 本文研究了LLM导师在拥有标准答案等私有信息时是否会产生“答案驱动”的解释，并提出了一种截断推理评估方法（TRACE）来检测此现象。


<details>
  <summary>Details</summary>
Motivation: LLM导师即使能接触到标准答案等私有信息，也应当能够从问题出发进行推理，但实际中其生成的解释可能会受到最终答案的隐形驱动，因此需要评估和诊断这种“答案驱动”的推理行为。

Method: 提出TRACE（截断推理AUC评估）方法，在生成思维链解释的各个截断节点强制模型立即给出答案，以此评估模型多早能获取最终答案。研究基于GSM8K数据集，对比了仅问题、正确答案和错误答案三种上下文环境。

Result: 引入答案键后，Qwen2.5-3B-Instruct的中位数TRACE AUC从0.375提升至0.900，在99.7%的案例中，模型在解释生成仅10%时就已知道正确答案。

Conclusion: 截断思维链审计可以作为一种轻量级的过程级诊断工具，用于检测和评估数学辅导中LLM的答案驱动推理问题。
...
### [85] [Governed Individuation: Cryptographically Decoupling an Agent's Learning from Its Authority](https://arxiv.org/abs/2607.04613)
*Xue Qin,Simin Luan,Cong Yang,Zhijun Li*

Main category: cs.AI

TL;DR: 本文提出了一种名为“受控个体化”的执行架构，通过密码学身份绑定和语义效果门控，为自主Agent的部署后行为提供确定性（非概率性）的安全限制保证。


<details>
  <summary>Details</summary>
Motivation: 随着自主Agent在部署后持续学习并操作真实世界基础设施，现有的概率性对齐技术无法保证Agent在适应环境后仍不超出授权范围，亟需一种能够提供确定性安全限制的系统架构。

Method: 引入“受控个体化（Governed individuation）”架构：在启动时将Agent绑定到密码学冻结的身份摘要，并根据动作的实际“语义效果”（而非动作名称）设立安全门控来审查每一次操作。

Result: 理论证明了Agent的自我学习无法在未经授权时扩大其权限；实验表明，该方法成功将非法操作的执行次数降至零并保持了任务成功率，且在对抗性评估中将错误准许率从75%降低至零。

Conclusion: 该研究将对部署后学习Agent的安全信任，从对其持续对齐的概率性押注，转变为可在启动时进行验证的确定性机制。
...
### [86] [Formal Disco: Scalable Open-Ended Generation of Formally Verified Programs](https://arxiv.org/abs/2607.04631)
*Gabriel Poesia,Simon Henniger,Tzu-Han Hsu,Yilun Du,Nada Amin*

Main category: cs.AI

TL;DR: 提出了 Formal Disco，一个通过多智能体协同大规模生成形式化验证程序的分布式系统，并发布了高质量数据集和微调模型。


<details>
  <summary>Details</summary>
Motivation: 形式化验证能提供极强的程序质量保证，但由于人类编写的形式化验证语言代码极其稀缺，限制了 AI 模型在这一领域的应用。

Method: 构建了包含发起者（initiators）、修复者（fixers）和扩展者（extenders）三类 LLM 智能体的分布式协作系统，并结合最大熵原理进行迭代监督微调（SFT）以提升程序多样性。

Result: 发布了 Dafny、Verus 和 Frama-C 的大型合成验证程序数据集，且微调后的开源模型在验证相关任务上达到或超越了 Claude Opus 4.5 的性能。

Conclusion: 该研究为形式化推理领域大规模生成合成数据提供了一条可行路径，有效克服了长期存在的数据壁垒。
...
### [87] [MRMS: A Multi-Resolution Memory Substrate for Long-Lived AI Agents](https://arxiv.org/abs/2607.04617)
*Jizhizi Li,Amy Shi-Nash*

Main category: cs.AI

TL;DR: 提出了一种多表征、多时间尺度的记忆基底（MRMS），通过结构化、向量和图关系的同步管理，实现AI Agent的长期个性化与记忆连续性。


<details>
  <summary>Details</summary>
Motivation: 长期生存的AI Agent需要跨交互的连续性，但仅靠延长提示词窗口无法解决这一问题，Agent需要有效地保留、检索、区分和修正记忆。

Method: 提出了一种双轴设计的记忆基底架构：表征轴（结构化记录、向量表示、图关系）与时间轴（短期痕迹、中期抽象、长期语义），并实现了轻量级原型MRMS。

Result: 原型系统在受控的长期交互场景下，成功实现了生成前的记忆选择、修正、边界强化和证据归因等核心机制。

Conclusion: 实现可靠的个性化是一个记忆设计问题，有效记忆应是结构化、选择性暴露、持续巩固并带有认识论标签的，而非简单的对话历史堆积。
...
### [88] [Integrated Altruistic and Fairness Preference Induces Advanced Mutual Cooperation in Sequential Social Dilemmas](https://arxiv.org/abs/2607.04710)
*Yu Wei,Yukiko Ogura,Yoshiyuki Ohmura,Ildefons Magrans de Abril,Hoshinori Kanazawa,Yasuo Kuniyoshi*

Main category: cs.AI

TL;DR: 提出了一种结合利他与公平偏好（AFP）的奖励共享机制，以解决多智能体强化学习在社会困境中的合作难题。


<details>
  <summary>Details</summary>
Motivation: 在多智能体强化学习中，智能体在社会困境（个人利益与集体利益冲突）下难以实现合作。

Method: 设计了一种融合利他偏好（关注他人奖励）与公平偏好（关注平等）的效用函数（AFP），作为奖励共享机制。

Result: 在序列社会困境游戏中，AFP智能体比基线方法实现了更高的集体奖励和公平性；利他偏好促进了公共品贡献，公平偏好诱导了互惠行为。

Conclusion: 结合利他与公平偏好能有效引导分布式智能体在社会困境中达成互惠合作。
...
### [89] [FORGE: Research-Trajectory Hijacking Attacks on Deep Research Agents](https://arxiv.org/abs/2607.04718)
*Yue Pan,Ziheng Zhang,Junxiang Lei,Changhao Jia,Qingyi Si,Hongcheng Guo*

Main category: cs.AI

TL;DR: 本文揭示了深度研究智能体在规划层面的安全漏洞，提出了FORGE攻击方法、评估指标PRISM以及防御机制RQA。


<details>
  <summary>Details</summary>
Motivation: 深度研究智能体在多轮检索和规划中，易受对抗性文档影响，导致子任务规划被劫持并污染最终报告。

Method: 设计了结合文档内推理伪造与文档间协同的FORGE攻击，引入了认知类型加权的PRISM评估指标，并提出了将生成绑定到根查询的RQA防御方法。

Result: FORGE攻击实现了26.4%的PRISM污染率；而采用RQA防御后，PRISM指标从38.5%显著降低至18.3%。

Conclusion: 规划层中毒对智能体构成严重威胁，而根查询锚定（RQA）是一种能够有效缓解此类攻击的轻量级防御方案。
...
### [90] [FM-ChangeNet: Learning Change through Pathwise Feature Transport](https://arxiv.org/abs/2607.04750)
*Roie Kazoom,George Leifman,Genady Beryozkin*

Main category: cs.AI

TL;DR: 提出FM-ChangeNet，将双时相变化检测重新定义为特征空间中的连续路径传输（速度场建模），而非传统的静态终点对比。


<details>
  <summary>Details</summary>
Motivation: 传统的变化检测方法依赖静态终点对比，监督信号稀疏且模糊，难以捕捉时间演变过程，且容易受到光照变化和空间失配等非结构性噪声的干扰。

Method: 提出路径监督的FM-ChangeNet框架，通过构建中间潜在状态来学习时间调控的速度场；采用多尺度层次化架构进行跨时相配准与流解码，并结合流监督、轨迹一致性及分割损失进行联合优化。

Result: 在遥感基准数据集上取得了最先进的性能（SOTA），能够产生更具结构性和鲁棒性的变化表征，有效区分真实的结构变化与环境干扰（如光照和对齐误差）。

Conclusion: 将变化检测任务转化为特征空间中的连续流传输问题，不仅能显式捕获时间演变，还通过速度场幅值提供了一种可解释且具鲁棒性的变化表征方式。
...
### [91] [AgenticPD: A Stage-Aware Agentic Framework for Physical Design QoR Optimization](https://arxiv.org/abs/2607.04758)
*Shuo Ren,Zijin Cheng,Yaohui Han,Libo Shen,Leilei Jin,Wanting Tian,Rongliang Fu,Chao Wang,Bei Yu,Tsung-Yi Ho*

Main category: cs.AI

TL;DR: 本文提出 AgenticPD，一种阶段感知的智能体框架，用于高效优化物理设计的 QoR。


<details>
  <summary>Details</summary>
Motivation: 传统的物理设计优化方法成本高昂，且现有方法将其孤立地视为扁平参数微调或需要重复运行完整流程的脚本生成任务。

Method: 引入裁判智能体引导搜索，结合阶段专用智能体在局部进行决策，并利用智能体装配管理上下文以实现中间状态的分支与检查点复用。

Result: AgenticPD 取得了强劲的布线后时序表现，同时在功耗和面积指标上保持了竞争力。

Conclusion: 阶段感知的多智能体协作框架能显著降低 EDA 流程的开销，是物理设计 QoR 优化的有效方案。
...
### [92] [CARL: Constraint-Aware Reinforcement Learning for Planning with LLMs](https://arxiv.org/abs/2607.04854)
...
TL;DR: 提出CARL框架，通过对比有无约束输入下的输出分布生成奖励，显著提升大语言模型在规划任务中的内在约束遵守能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在规划时易违反约束，且现有方法（如依赖外部工具或任务分解）未能有效提升模型内在的约束感知能力。

Method: 设计了约束感知强化学习（CARL）框架，通过对比有约束和无约束输入下的模型输出分布来构建约束感知奖励，进行端到端微调。

Result: 在BlocksWorld、TravelPlanner和T-Eval等基准测试中，CARL在约束遵守和规划性能上显著优于标准强化学习微调基线及最先进的推理模型。

Conclusion: CARL能够有效增强大语言模型对约束的内在关注，为实现可扩展的约束感知规划提供了一种高效且无需外部求解器的方案。
...
### [93] [Toward Trustworthy Large Language Model Agents in Healthcare](https://arxiv.org/abs/2607.05055)
*Hadi Hasan,Safaa Salman,Adam Tai Abou Dargham,Ammar Mohanna,Ali Chehab*

Main category: cs.AI

TL;DR: 提出CareConnect，一个安全优先的医疗对话智能体，利用大语言模型高效、安全且低成本地实现医疗预约挂号的自动化。


<details>
  <summary>Details</summary>
Motivation: 传统医疗预约系统因人工协调、系统碎片化和高管理开销而存在效率瓶颈，限制了医疗服务获取并增加了行政负担。

Method: 开发了结合LLM函数调用、检索增强生成（RAG）和分层确定性安全护栏的系统，调用8个特定领域工具处理预约逻辑，并设计了紧急情况和医疗意图的拦截机制以杜绝提供医疗建议。

Result: 在680个场景的基准测试中，系统实现了91.8%的任务完成率，中位数延迟为2.2秒，安全合规率达96.0%，每次预约平均成本仅为0.0324美元，显著低于人工成本。

Conclusion: 通过严格限制范围和实施安全保障，基于LLM的智能体能够在确保安全合规的前提下，可靠地自动化复杂的医疗运营工作流，实现显著的降本增效。
...
### [94] [STAPO: Selective Trajectory-Aware Policy Optimization for LLM Agent Training](https://arxiv.org/abs/2607.04963)
*Qiuyi Qi,Tian Liang,Mutian Bao,Jinjian Zhang,Dongnan Liu,Wei Zhou,Linjian Mo,Ming Kong,Jie Liu,Feng Zhang,Qiang Zhu*

Main category: cs.AI

TL;DR: 提出STAPO框架，利用归一化熵识别并优化长程任务中大语言模型智能体的“轨迹忽略”问题，实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 在长程任务中，稀疏和延迟的奖励会导致LLM智能体出现“轨迹忽略”（即丢失对任务目标和历史信息的关注），而现有的香农熵指标因混淆状态复杂性与置信度而无法准确评估决策可靠性。

Method: 提出“归一化熵”来准确衡量智能体的置信度偏差以定位轨迹忽略的步骤，并设计了分层组强化学习框架STAPO，通过结合轨迹感知奖励和轨迹无关惩罚来联合优化这些异常步骤。

Result: 在ALFWorld、WebShop和检索增强问答等任务上的实验表明，STAPO取得了最先进的性能，并显著缓解了轨迹忽略问题。

Conclusion: STAPO框架为解决大模型智能体在长程任务中的轨迹忽略问题提供了一种有效且鲁棒的强化学习解决方案。
...
### [95] [Medi-Gemma: A Hybrid Clinical Decision Support System Integrating Deterministic EMR Analytics and Retrieval-Augmented Generation](https://arxiv.org/abs/2607.04907)
*Mohammed Saim Ahmed Quadri,Yunzhe Xue,Justin W. Ady,Usman Roshan*

Main category: cs.AI

TL;DR: 本文介绍了Medi-Gemma，一个用于伤口病理分流的临床决策支持系统，通过解耦框架提高LLM在临床应用中的安全性和数据真实性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLM）在临床应用中面临结构性幻觉、表格数据推理能力弱以及向量检索遗漏等局限性。

Method: 提出一种解耦框架，采用多阶段管道，结合数据管理器、意图路由、Pandas查询引擎、检索增强生成（RAG）引擎，并引入真实数据注入模块和安全验证器。

Result: 该架构有效消除了语义上下文漂移，防止了数据库崩溃，并显著提升了临床数据检索的事实准确性。

Conclusion: Medi-Gemma为临床决策支持提供了一种更安全的设计模式，在确保结构化数据保真度、检索接地及确定性安全防御方面具有重要意义。
...
### [96] [Diffusion-Guided Uncertainty-Aware Delayed Policy Optimization](https://arxiv.org/abs/2607.05064)
*Junqi Tu,Zejiao Liu,Fangfei Li,Yang Tang*

Main category: cs.AI

TL;DR: 本文提出了DUPO算法，利用扩散模型建模延迟状态与真实状态之间的关系，并基于偏差估计加权策略，以解决随机环境延迟反馈下的强化学习性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界强化学习常面临反馈延迟，而现有方法忽略了随机MDP中延迟状态与真实状态之间的固有偏差，这在理论上会导致最优策略性能下降。

Method: 提出“扩散引导的不确定性感知延迟策略优化（DUPO）”，通过扩散模型显式建模延迟信息与当前状态的关系，并利用得到的偏差估计来加权延迟策略。

Result: 在包含多种随机延迟的连续机器人控制任务中，DUPO显著优于现有方法，且在长期和随机延迟场景下依然保持高效。

Conclusion: 理论与实验证明了延迟状态偏差对策略的负面影响，而通过扩散模型建模和不确定性感知加权能有效缓解随机延迟带来的性能退化问题。
...
### [97] [ASSEMCAD: Production-Ready CAD Assembly Generation from Natural Language](https://arxiv.org/abs/2607.05123)
*Yurui Dong,Shu Zou,Siqi Li,Nianchen Deng,Hongbin Zhou,Xuemeng Yang,Pinlong Cai,Licheng Wen,Xinyu Cai,Botian Shi*

Main category: cs.AI

TL;DR: 提出AssemCAD框架，通过公理化装配规范与几何执行，实现从自然语言生成高物理有效性与可重用性的三维机械装配体。


<details>
  <summary>Details</summary>
Motivation: 单部件CAD生成已取得进展，但复杂的机械装配体生成仍面临组件协调、物理一致性及接口匹配等挑战，直接生成CAD代码难以保证装配的有效性。

Method: 引入公理化装配规范（定义部件、接口与配合关系），并结合基于端口和配合的CAD装配库，通过确定性几何变换与验证来合成参数化组件。

Result: 在AssemBench基准测试上，AssemCAD在装配保持度及物理有效性上显著优于传统的代码生成基线，且适用于多种大模型底座。

Conclusion: 通过将公理化装配推理与确定性几何执行相结合，AssemCAD成功将Text-to-CAD技术从单一零件生成扩展至工业级机械装配设计。
...
### [98] [TacReasoner: A Dynamic Tactile-Language Framework for Interactive Reasoning in Real-World Scenarios](https://arxiv.org/abs/2607.05131)
*Kailin Lyu,Di Wu,Long Xiao,Jianning Zeng,Jianwei He,Chang Lin,Lianyu Hu,Lin Shu,Jie Hao,Ce Hao*

Main category: cs.AI

TL;DR: 本文提出了TacReasoner，一个旨在解决动态触觉信号建模不足和触觉大模型幻觉问题的动态触觉-语言推理框架。


<details>
  <summary>Details</summary>
Motivation: 多模态推理系统在触觉集成上面临两大挑战：动态触觉信号建模不足限制了时变属性的推理，且缺乏显式推理机制导致触觉大模型容易产生幻觉。

Method: 提出了包含动态感知触觉编码器的TacReasoner框架，并构建了首个触觉思维链（CoT）数据集TouchCoT-10k以及评估基准DynTac-Bench。

Result: TacReasoner在多个数据集上表现优异，仅以7B参数规模就在大多数子任务上超越了14B的VTV-LLM模型。

Conclusion: 该研究验证了TacReasoner在动态触觉感知和真实世界常识推理中的有效性与高效性。
...
### [99] [The Changing Role of Symbolic Methods in Artificial Intelligence](https://arxiv.org/abs/2607.05168)
*Jun Sun*

Main category: cs.AI

TL;DR: 本文探讨了符号推理在现代大模型时代的定位，提出显式符号推理是弥补模型简化信息丢失的手段，并指出其未来将从计算引擎转型为连接人机的符号接口。


<details>
  <summary>Details</summary>
Motivation: 现代基础模型的成功引发了对符号推理在智能系统中所扮演角色的重新思考。

Method: 提出了“压缩原理”（模型是现实的简化，符号推理弥补了丢失的信息）和“建模-推理权衡”这一理论框架。

Result: 统一解释了传统符号AI的成功与现代大模型的有效性，并指出符号方法在人类对AI进行规范、验证和建立信任中正变得越发重要。

Conclusion: 符号方法的未来并非作为智能系统的核心计算引擎，而是作为人类与高能力AI系统之间进行沟通、监管和建立信任的符号接口。
...
### [100] [DSpark: Confidence-Scheduled Speculative Decoding with Semi-Autoregressive Generation](https://arxiv.org/abs/2607.05147)
...
TL;DR: 本文提出了 DSpark 框架，一种结合半自回归架构与自适应验证的大模型投机解码方案，显著提升了推理速度与吞吐量。


<details>
  <summary>Details</summary>
Motivation: 传统并行起草器因缺乏 Token 间依赖导致接受率快速衰减，且无差别验证长序列在并发服务中会造成严重的计算资源浪费。

Method: 采用半自回归架构（并行骨干网络结合轻量顺序模块）以建模块内依赖，并使用置信度调度验证机制来根据生存概率动态调整验证长度。

Result: 在 DeepSeek-V4 真实流量测试中，相比 MTP-1 基线，DSpark 在同等吞吐量下将单用户生成速度提升了 60% 至 85%。

Conclusion: DSpark 成功解决了投机解码中的接受率衰减与验证资源浪费问题，显著优化了高并发大模型服务的性能与 Pareto 前沿。
...
### [101] [AgentGym2: Benchmarking Large Language Model Agents in De-Idealized Real-World Environments](https://arxiv.org/abs/2607.05174)
...
TL;DR: AgentGym2 是一个全新的评估框架，用于在真实且具挑战性的端到端环境中评估大语言模型智能体，揭示了当前先进模型与实际需求之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试多在简化和理想化的设置中评估智能体，低估了真实部署中噪声、不确定性和需要主动探索工具的难度。

Method: 提出 AgentGym2 评估框架，通过真实的端到端任务，评估智能体在工具探索与组合、端到端执行以及对噪声和不完整信息的鲁棒性等方面的能力。

Result: 对15个主流开源和闭源模型进行实验，发现即使是 Gemini 和 GPT-5 等最先进的模型在 AgentGym2 上也表现不佳。

Conclusion: 当前的智能体能力与真实世界应用的实际需求之间仍存在巨大的鸿沟。
...
### [102] [CP-WSP: A Declarative CP-SAT Framework for Configurable Multi-Constraint Workforce Scheduling](https://arxiv.org/abs/2607.05177)
*Vipul Patel,Anirudh Deodhar,Dagnachew Birru*

Main category: cs.AI

TL;DR: 本文提出了 CP-WSP，一个基于 CP-SAT 的声明式员工排班框架，旨在解决实际排班中复杂的硬性约束和多目标优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有的排班模型过于简化，缺乏对强制休息时间、工作量公平性、细粒度时间、跨周稳定性和跨午夜班次等实际复杂约束的支持。

Method: 采用 CP-SAT 求解器，引入班次窗口变量分解、网格偏移预处理技术及多粒度时间分辨率，通过统一的加权惩罚函数和 JSON 配置文件实现灵活排班。

Result: 在 INRC-II 基准测试和 36 个合成配置上进行了多粒度评估，并提供了一套包含 36 个配置的可复现基准测试集。

Conclusion: CP-WSP 有效填补了理论排班模型与实际复杂需求之间的空白，提供了一种无需修改代码即可灵活配置的高效排班解决方案。
...
### [103] [Rethinking On-Policy Self-Distillation for Thinking Models](https://arxiv.org/abs/2607.05184)
*Simran Kaur,Narutatsu Ri,Yinghui He,Liam Fowl,Sanjeev Arora*

Main category: cs.AI

TL;DR: 研究表明，特权自蒸馏会显著损害思考模型在长推理链上的表现，导致准确率下降，因其抑制了模型的自我纠错和多路径探索能力。


<details>
  <summary>Details</summary>
Motivation: 自蒸馏是模型自我提升的常用策略，通常认为思考模型能很好地吸收特权信息，但其在长推理场景下的实际表现和潜在缺陷仍待探究。

Method: 在多个数学基准（AIME24/25, HMMT25）上评估了5款Qwen3和OLMo思考模型，对比了特权上下文蒸馏与在策略蒸馏（OPD），并诊断了分叉率及自我纠错标记的变化。

Result: 特权自蒸馏使模型准确率最高下降17%，且在长rollout时恶化更严重；特权上下文降低了高熵分叉率，减少了验证、回溯等纠错行为，这与对指令微调模型的正面作用形成对比。

Conclusion: 特权自蒸馏对强思考模型有副作用。未来针对此类模型的自蒸馏设计需要更加关注Token级别的信号，尤其是纠错和推理步骤的处理。
...
### [104] [Reason, Reward, Refine: Step-Level Errors Corrections with Structured Feedback for Physics Reasoning in Small Language Models](https://arxiv.org/abs/2607.05199)
*Raj Jaiswal,Dhruv Jain,Rishabh Dhawan,Sree Krishna Uppalapati,Shin'ichi Satoh,Tanuja Ganu,Rajiv Ratn Shah*

Main category: cs.AI

TL;DR: 提出一种步骤级奖励框架，通过自动定位首个推理错误并生成针对性反馈来训练模型进行自我修正，显著提升了小语言模型的物理推理能力。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在多步物理推理中容易因单步错误累积、幻觉和领域知识不足而导致整体推理失败。

Method: 使用步骤级奖励框架定位首个错误并提供结构化反馈，通过带KL正则化的策略梯度算法训练模型进行修正，无需依赖真实答案作为生成目标或偏好数据。

Result: 在五个物理基准测试中，该方法比CoT提升了17-20%的准确率，显著减少了计算错误（从56.9%降至23.5%）和理解错误。

Conclusion: 该框架能有效改善小模型的物理推理和纠错能力，但概念性错误仍然是当前最难克服的瓶颈。
...
### [105] [ClassicLogic: A Knowledge-Driven Benchmark of Classic Puzzle Games for Evaluating Compositional Generalization](https://arxiv.org/abs/2607.05185)
*Mahnoor Shahid,Hannes Rothe*

Main category: cs.AI

TL;DR: 本文介绍了 ClassicLogic，一个基于四种经典逻辑谜题（数独、KenKen、Kakuro 和 Futoshiki）的新型基准测试，旨在评估 AI 的组合泛化与多步推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的组合泛化基准测试大多集中在语言任务上，缺乏复杂且显式的组合结构，无法有效评估智能体学习和组合解题策略的能力。

Method: 引入了 ClassicLogic 基准测试，其核心创新是为每个谜题构建层级化、显式的知识库，将复杂的解题策略正式定义为基础策略的组合。

Result: 该基准能够对智能体的推理能力进行细粒度评估，涵盖从理解基本规则到应用多步组合策略解决不同数学难度谜题的各个阶段。

Conclusion: ClassicLogic 提供了一个极具挑战性的开源测试平台，有望推动神经符号及其他先进 AI 推理系统的研究与发展。
...
### [106] [EvoAgentBench: Benchmarking Agent Self-Evolution via Ability Transfer](https://arxiv.org/abs/2607.05202)
...
TL;DR: 提出了 EvoAgentBench，一个用于评估智能体通过跨领域程序性能力转移实现自我进化的全新基准。


<details>
  <summary>Details</summary>
Motivation: 当前的评估方法未能孤立出程序性能力的转移与复用，现有基准多侧重于单次任务解决或信息记忆，而非经验过程的复用。

Method: 引入 EvoAgentBench 基准，通过从智能体执行轨迹中提取能力、规范化为操作单元，并构建连接具有程序重叠任务的领域特定“能力图”。

Result: 实验表明，人工筛选的能力内容在不同模型家族间转移可靠，但目前尚无自动方法能在所有设置下持续获得正向收益。

Conclusion: EvoAgentBench 将智能体自我进化的评估从单纯的准确率比较转变为对经验编码、路由和吸收的细粒度诊断。
...
### [107] [MoP-JEPA: Hard-Assigned Predictor Mixtures for Stochastic JEPA World Models](https://arxiv.org/abs/2607.05238)
*Zhi Song,Ximing Xing,Zhenchao Tang,hanbo Huang,Tianxu Lv,minghao Yang,Zhongzheng Niu,He Bing,Lusheng Wang,Jianhua Yao*

Main category: cs.AI

TL;DR: 本文指出传统JEPA世界模型在随机环境中因预测条件均值而失效的问题，并提出了硬分配的多预测器混合方法（MoP-JEPA）来有效预测多模态状态转移。


<details>
  <summary>Details</summary>
Motivation: 传统的JEPA世界模型使用单一确定性预测器，在面对随机环境的分支转换时，会输出不对应任何真实状态的条件均值，导致规划失败。

Method: 提出具有硬分配的预测器混合模型（MoP-JEPA），使其收敛为转移分布的量化器；同时设计了一套包含代码簿控制、打乱上下文测试和验证路由标准的验证协议，以防止评估中的覆盖舞弊。

Result: 在OGBench离线数据上，规划成功率从单预测器的0.02-0.09大幅提升至0.85，在验证路由标准下比最强备选方法高出2-5倍，并在真实环境的最难迷宫任务中获得第二名。

Conclusion: 多模态动力学决定了JEPA世界模型能否进行有效规划，而采用硬分配的预测器混合机制是解决该问题的一种极简且可验证的有效方案。
...
### [108] [MetaSkill-Evolve: Recursive Self-Improvement of LLM Agents via Two-Timescale Meta-Skill Evolution](https://arxiv.org/abs/2607.05297)
*Zefeng Wang,Minxi Yan,Jinhe Bi,Sikuan Yan,Volker Tresp,Yunpu Ma*

Main category: cs.AI

TL;DR: 本文提出了 MetaSkill-Evolve，一个通过双时间尺度框架实现任务技能和元技能递归自我进化的 LLM 智能体系统。


<details>
  <summary>Details</summary>
Motivation: 现有的自我改进智能体仅能改进具体任务技能（“做什么”），而改进技能的元流程（“怎么改进”）是固定不变的，限制了智能体适应多样化任务的能力。

Method: 引入双时间尺度框架，使任务技能在快速循环中进化，而包含五个组件（分析、检索、分配、建议、进化）的元技能在慢速循环中通过自身管道递归进化，且所有组件共享同一个冻结的骨干网络。

Result: 在 OfficeQA、SealQA 和 ALFWorld 三个基准上，MetaSkill-Evolve 显著优于无技能、静态技能和单层进化基线，测试准确率较原始骨干网络分别提升了 23.54、16.09 和 1.92 个百分点。

Conclusion: 实验证明，让智能体的自我改进程序本身具备递归进化能力（元进化），可以有效提升大语言模型智能体在长周期开放任务中的表现与自适应能力。
...
### [109] [OptiAgent: End-to-End Optimization Modeling via Multi-Agent Iterative Refinement](https://arxiv.org/abs/2607.05346)
...
TL;DR: OptiAgent是一个多智能体框架，能将自然语言描述的运筹学问题转化为数学公式和可执行代码，并通过多环验证提高准确性与透明度。


<details>
  <summary>Details</summary>
Motivation: 旨在解决将自然语言描述的运筹学问题自动且准确地转化为数学模型和代码的挑战，并提升建模过程的透明度与可审计性。

Method: 提出OptiAgent框架，优先进行数学建模（提取变量与约束），并引入包含四种反馈机制的多环验证架构进行迭代自我纠错。

Result: 在LP、MILP和非线性规划任务的4个基准数据集中，OptiAgent在3个上取得了最先进的（SOTA）性能，在剩余1个上也极具竞争力。

Conclusion: OptiAgent通过模块化设计和多环验证，不仅提高了优化问题求解的准确性，还显著增强了建模过程的透明度和可解释性。
...
### [110] [Evaluating and Understanding Model Editing for Medical Vision Language Models](https://arxiv.org/abs/2607.05310)
*Guli Zhu,Chenwei Wu,Liyue Shen*

Main category: cs.AI

TL;DR: 本文推出了M3Bench，这是一个用于多模态医学视觉语言模型编辑的临床基准测试集，旨在评估模型编辑在临床环境下的可靠性和泛化性。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态模型编辑基准多针对通用任务，无法反映临床领域的实际需求和多变性，难以保证模型部署后修正的安全性和有效性。

Method: 构建了包含16,276个医学问题的M3Bench基准，通过在6个医学与通用VLM上评估4种代表性编辑器的表现（包含单次和连续编辑），并分析了其潜在空间几何结构的变化。

Result: 评估发现没有单一方法在所有指标上胜出。梯度编辑器迁移性强但会破坏局部性；记忆编辑器能保留局部性，但缺乏组合泛化性且对超参数高度敏感。

Conclusion: M3Bench为多模态模型编辑提供了一个严格的临床压力测试，并为医学模型部署后的安全自适应提供了实践指导。
...
### [111] [Graph Sparse Sampling: Breaking the Curse of the Horizon in Continuous MDP Planning](https://arxiv.org/abs/2607.05359)
*Idan Lev-Yehudi,Vadim Indelman*

Main category: cs.AI

TL;DR: 提出图稀疏采样（GSS）算法，通过在决策间共享采样未来，将树状搜索转化为无分支的图结构，以解决连续空间规划中的指数级分支难题。


<details>
  <summary>Details</summary>
Motivation: 传统树状搜索算法（如MCTS）在连续空间中由于分支结构，其采样需求随规划深度呈指数增长，计算成本极高。

Method: 提出图稀疏采样（GSS）算法，通过在多个候选决策间共享采样未来构建无分支的图，便于GPU批量计算，并引入启发式引导和支持连续/离散动作空间的平滑备份机制。

Result: 证明了GSS在特定条件下对规划时域具有多项式级（而非指数级）的样本复杂度边界，并在连续控制仿真中显著优于传统树状规划器，达到近乎最优的性能。

Conclusion: 无分支的图规划是一种有效的在线控制设计原则，能有效规避树状采样中的指数级时域依赖。
...
### [112] [SovereignPA-Bench: Evaluating User-Owned Personal Agents under Evolving Intent, Platform Mediation, and Consent Constraints](https://arxiv.org/abs/2607.05363)
*Dylan Zongmin Liu*

Main category: cs.AI

TL;DR: 本文提出了SovereignPA-Bench，一个用于评估个人AI智能体在保护用户主权（包括隐私、同意权、抗操纵性等）方面表现的可执行基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准主要关注任务完成度和工具使用，缺乏对智能体在维护用户主权（如保护隐私、尊重同意、抵抗平台操纵和平衡用户负担）方面的评估。

Method: 引入SovereignPA-Bench基准，该基准将智能体可见的“可观测状态”与评估器专用的“隐藏标签”分离。研究者在120个主权压力场景下评估了4个模型系列和8种策略基线，生成了3,840条运行轨迹，并进行了包含人类审计的对比分析。

Result: 全主权支架（Full-sovereign scaffolding）策略相比其他基线显著提高了主权得分，减少了隐私泄露、同意违规和被操纵的风险；人类审计在隐私和同意方面一致性较高，但在操纵判断上存在主观分歧。

Conclusion: 个人智能体的评估必须超越单纯的任务完成度，转向代表用户利益、具备同意意识和基于证据的行动评估。
...
### [113] [LLM-as-a-Verifier: A General-Purpose Verification Framework](https://arxiv.org/abs/2607.05391)
*Jacky Kwok,Shulu Li,Pranav Atreya,Yuejiang Liu,Yixing Jiang,Chelsea Finn,Marco Pavone,Ion Stoica,Azalia Mirhoseini*

Main category: cs.AI

TL;DR: 本文提出了 LLM-as-a-Verifier，这是一种将验证（Verification）作为全新缩放维度的通用框架，利用连续概率分数提供细粒度反馈。


<details>
  <summary>Details</summary>
Motivation: 为了在无需额外训练的前提下，提升大语言模型智能体任务的验证能力，解决传统 LLM 裁判输出离散分数且难以多维度扩展的问题。

Method: 引入 LLM-as-a-Verifier 框架，通过计算评分 Token 的 Logit 分布期望来生成连续分数，并在分数粒度、重复评估和指标分解三个维度进行扩展，并结合高效的排序算法。

Result: 在 Terminal-Bench V2 (86.5%)、SWE-Bench Verified (78.2%) 等多个基准测试中达到 SOTA 性能，并能有效提升 Claude Code 任务监控及强化学习（SAC 和 GRPO）的样本效率。

Conclusion: LLM-as-a-Verifier 证明了验证是一个行之有效的模型能力扩展轴，其细粒度反馈机制不仅能提升智能体表现，还能作为强化学习的密集奖励信号。
...
### [114] [DELTAVID: Enhancing Fine-Grained Spatiotemporal Perception with Cross-Video Differences](https://arxiv.org/abs/2607.02551)
*Yankai Yang,Yancheng Long,Bin Wen,Fan Yang,Tingting Gao,Han Li,Shuo Yang*

Main category: cs.CV

TL;DR: 提出 DELTAVID 框架，通过跨视频“找不同”任务，增强视频多模态大模型的细粒度时空感知与推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视频大模型缺乏精准的局部时空感知能力，难以识别和区分全局语义高度相似但局部存在微小差异的视频。

Method: 设计了可验证的代理任务框架 DELTAVID，将跨视频差异识别转化为训练信号，并构建了配套的 DELTAVID-10K 数据集与 DELTAVID-Bench 评估基准。

Result: 该方法显著提升了模型在跨视频差异理解上的表现，并在多个通用视频理解主流基准（如 Video-MME、MMVU 等）上实现了良好的泛化与性能提升。

Conclusion: 跨视频差异对比不仅能有效诊断模型的感知缺陷，还可作为一种高效的代理监督手段，推动模型从粗粒度语义理解走向细粒度时空证据推理。
...
### [115] [Learning 3D Affordances for Blade Insertion in Cluttered Stowing](https://arxiv.org/abs/2607.02549)
*Tianyu Li,Harpreet Sawhney,Minju Jung,Aditya Mehrotra,Kunal Mehrotra,Mudit Agrawal*

Main category: cs.CV

TL;DR: VulcanVoxel通过将示能推理保留在3D空间（体素）域中，利用掩码自编码器重构工具占用空间，显著提升了机器人装载任务中的多模态空间可行性预测。


<details>
  <summary>Details</summary>
Motivation: 传统方法将示能推断为SE(3)位姿分布，这缺乏几何结构，无法推理未观察到的位置是否可行，且难以从单模态数据中恢复其他几何替代方案。

Method: 提出VulcanVoxel，利用3D占用场上的掩码自编码器，结合场景几何重构铲刀的占用空间，在体素级别局部计算可行性；同时引入蒸馏的学生网络以实现超快速的RGB到体素推理。

Result: 在1万个真实仓库装载回合上训练，VulcanVoxel的Top-5覆盖率达到0.89（对比最佳位姿基线的0.71），蒸馏模型的推理时间从1.4秒大幅缩短至30毫秒。

Conclusion: 将示能推理保持在空间几何域（而非位姿域）能够实现更好的几何推理和多模态预测，显著提升机器人整理与装载任务的效率。
...
### [116] [Interpretable machine learning predicts Parkinson's disease severity using motion-corrected QSM MRI and multiband multiecho fMRI features](https://arxiv.org/abs/2607.02553)
*Aixa X. Andrade*

Main category: cs.CV

TL;DR: 本研究利用可解释机器学习，结合磁共振定量易感性图谱（QSM）与静息态功能磁共振成像（ReHo）特征，实现了对帕金森病运动严重程度（MDS-UPDRS Part III）的有效预测。


<details>
  <summary>Details</summary>
Motivation: 旨在寻找客观的神经影像生物标志物，以捕获临床检查无法直接观察到的脑部变化，从而改善帕金森病的运动功能评估。

Method: 提取28名参与者的区域QSM和ReHo特征，设计了13种特征集实验（包含影像、临床及多模态组合），采用SVR、Elastic Net、随机森林和XGBoost进行嵌套交叉验证评估，并引入SHAP进行可解释性分析。

Result: 融合全fMRI、全QSM和临床变量的模型表现出最强的全局拟合度（解释了45.4%的方差）；而精选的QSM结合临床变量模型实现了最接近临床实际的预测（75%的患者预测偏差在±5分以内）。SHAP分析指出了小脑、丘脑、纹状体、岛叶和运动皮层等关键特征。

Conclusion: QSM和ReHo能捕获帕金森病运动严重程度中不同且互补的维度，表明结构和功能神经影像在实现不同临床预测目标时具有独特的贡献。
...
### [117] [Do Diabetic Foot Ulcer Segmentation Models Generalize? A Cross-Dataset Benchmark of CNN and Transformer Architectures](https://arxiv.org/abs/2607.02555)
*Abderrahmane Benfatah*

Main category: cs.CV

TL;DR: 研究评估了不同深度学习架构在糖尿病足溃疡（DFU）分割中的跨医院泛化能力，发现基于Transformer的SegFormer-B2在外部数据集上的表现显著优于卷积网络模型。


<details>
  <summary>Details</summary>
Motivation: 现有的DFU分割模型大多在相同数据集上进行训练和测试，导致其在不同临床来源的未知数据上的泛化表现难以评估。

Method: 采用统一的无数据泄漏协议，在FUSeg/AZH混合数据集上训练U-Net、DeepLabV3+和SegFormer-B2三种模型，并在不进行微调的情况下，在DFUC2022和Medetec两个独立的外部数据集上进行直接评估和失效分析。

Result: 所有模型在内部数据上均表现良好（Dice 0.80-0.83），但在外部数据集上均有退化。SegFormer-B2的泛化表现最好（DFUC2022 Dice 0.557，Medetec Dice 0.786），且严重失败率最低（31.1%）；较复杂的DeepLabV3+表现逊于较简单的U-Net。

Conclusion: 架构类型（如Transformer对比CNN）而非模型复杂度，是决定模型跨医院泛化能力的关键驱动因素。
...
### [118] [Reliability-Aware Monocular Depth Supervision for Sparse-View Neural Reconstruction](https://arxiv.org/abs/2607.02554)
*Wei-Teng Chu,Yashasvini Gopalan,Changju Yuan*

Main category: cs.CV

TL;DR: 本文探讨了在稀疏视角神经重建中如何有效应用单目深度先验，提出通过光度掩码选择性应用深度监督，并在Mip-NeRF-360和Splatfacto上进行了评估。


<details>
  <summary>Details</summary>
Motivation: 户外驾驶场景下的稀疏视角神经重建面临多视角重叠有限的挑战。虽然单目深度估计可提供密集几何先验，但其预测存在噪声且在不同区域的可靠性不均匀。

Method: 使用Depth Anything V2作为单目深度先验，通过尺度-偏移拟合对齐至度量深度，并利用基于仅RGB基线模型生成的光度掩码进行选择性深度监督。

Result: 该方法显著提升了Splatfacto的表现（PSNR从14.903升至15.932，RMSE从0.542降至0.100），但对Mip-NeRF-360效果有限。消融实验证实提升源于筛选出了低误差的可靠区域。此外，在多视角覆盖充足时，深度监督虽能改善几何，但可能会损害渲染质量。

Conclusion: 单目深度先验对于欠约束的稀疏视角重建非常有用，但应当进行选择性应用并配合适度的权重。
...
### [119] [How many labels do you need? A decision framework for cross-habitat marine species recognition](https://arxiv.org/abs/2607.02559)
*Alzayat Saleh,Mostafa Rahimi Azghadi*

Main category: cs.CV

TL;DR: 提出一个评估生态图像识别模型迁移时标注工作量与准确率权衡的决策框架，发现结合冻结的DINOv2与线性分类器仅需每物种10-20张标注图像即可在新站点实现可靠部署。


<details>
  <summary>Details</summary>
Motivation: 生态学家在将自动图像识别系统部署到新生态站点时，缺乏关于需要多少标注数据和工作量的定量指导。

Method: 构建了跨越5个数据集、3个海洋和3个分类群的基准，评估了4种模型（DINOv2、CLIP等）在4种微调策略（线性探测、LoRA等）和3种迁移协议下的表现。

Result: 冻结的DINOv2加线性分类器表现优异，能学习到栖息地不变的特征，避免了基线模型的栖息地快捷方式偏见；每个物种仅需10-20张标注图像（减少约一个数量级的工作量）即可在新站点实现可靠部署。

Conclusion: 通过将预训练的DINOv2与简单线性分类器结合，并仅标注极少数图像（每个站点约需1-4小时），即可在新站点高效且可靠地部署海洋生态监测识别系统。
...
### [120] [Double-Helix Active Geometry: LiDAR-Anchored Multi-View Depth with Selective Abstention](https://arxiv.org/abs/2607.02561)
*Jinwen Wen*

Main category: cs.CV

TL;DR: 提出了DH-Active，一种轻量级、无需训练的几何后端，利用消费级深度传感器进行快速、高精度的稀疏深度恢复。


<details>
  <summary>Details</summary>
Motivation: 消费级深度传感器（如iPhone的LiDAR）有效工作距离短且数据稀疏，难以直接用于远场深度测量。

Method: 将传感器视为度量标尺，利用近场深度通过PnP确定两视图的相对位姿，对无深度的视觉追踪点进行三角测量，并利用视差/重投影门限制进行显式弃权（留空）。

Result: 在CPU上的中值延迟仅为1.11 ms；在TUM RGB-D和ARKitScenes等基准测试中，恢复深度的中值相对误差为1.4%至6.7%；远场评估中实现了64.2%的覆盖率和13.4%的误差。

Conclusion: 该方法成功实现了零学习参数、具备显式弃权机制且计算成本极低（近毫秒级CPU运行）的度量稀疏深度恢复。
...
### [121] [Inpainting U-Net for seamless pedestrian-level wind prediction across urban morphologies](https://arxiv.org/abs/2607.02560)
*Jingzi Huang,Claire E. Heaney,Tao Li,Xinzhe Li,Graham O. Hughes,Maarten van Reeuwijk*

Main category: cs.CV

TL;DR: 提出了一种用于快速预测城市人行道高度风速的双阶段U-Net深度学习框架。


<details>
  <summary>Details</summary>
Motivation: 传统高精度流体模拟（如LES）计算成本高，难以满足城市规划中对风环境进行快速评估的需求。

Method: 设计了双阶段框架：第一阶段（M1）进行分块预测以适应任意城市尺寸，第二阶段（M2）利用更大上下文窗口并通过高斯-赛德尔迭代方案消除分块边界的不连续性。

Result: M1成功捕获了风速的空间分布，M2显著减少了边界伪影并提升了空间连贯性。模型在未知城市形态上表现出良好的泛化能力，但对最大风速有所低估。

Conclusion: 该框架为真实城市形态下的高分辨率 pedestrian-level 风速预测提供了一种高效、灵活且实用的代理模型。
...
### [122] [Entropy-Coded MS-VQ-VAE with Learned Priors for Ultra-Low Bitrate Video Compression](https://arxiv.org/abs/2607.02562)
*Manikanta Kotthapalli,Banafsheh Rekabdar*

Main category: cs.CV

TL;DR: 本文提出一种基于离散潜在表示（VQ）的超低比特率视频编解码方案，通过自回归先验和EMA稳定码本更新，在低于0.1 bpp的极低带宽下实现了优于H.265的感知重建质量。


<details>
  <summary>Details</summary>
Motivation: 基于连续潜变量的视频编解码器在低于0.1 bpp的超低比特率下，由于缺乏可微速率信号，拉格朗日优化难以有效平衡重建质量与码率。

Method: 基于MS-VQ-VAE架构，利用离散码本施加硬性信息上限，采用EMA稳定码本更新及死码重启机制解决训练崩溃问题，并结合自回归先验对幂律分布的码本进行高效熵编码。

Result: 在UCF101上实现了0.043-0.064 bpp的极低码率（比H.265极限低5-7.6倍），感知质量（LPIPS）全面超越H.265 CRF 36，且熵效率达70-85%。

Conclusion: 离散潜在表示与稳定化码本更新机制相结合，有效解决了超低码率下的优化难题，证实了其作为原理性视频熵编码器的可行性。
...
### [123] [Attention Dynamics in Diffusion Models: A Visual Analytics Framework for Human-AI Collaboration](https://arxiv.org/abs/2607.02563)
*Yiran Xiao,George Legrady*

Main category: cs.CV

TL;DR: 本文提出了一个可视化分析框架，用于探索和解释扩散模型生成图像过程中的交叉注意力动态变化。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型能生成复杂图像，但其语义结构的出现和演变难以解释，且现有分析方法割裂了时间变化与空间证据。

Method: 引入一个交互式可视化分析框架，整合定量测量与数据驱动的阶段识别，分析跨生成步骤的词元级交叉注意力图的时空关系。

Result: 在包含60个提示词的Stable-Diffusion基准上进行案例研究，发现了循环出现且可解释的注意力模式，验证了联动时空视图对理解生成过程的有效性。

Conclusion: 该框架通过促进对生成过程的直观观察与讨论，支持了更有效的人机协作。
...
### [124] [From Raw Segmentations to Simulation-Ready Cardiac Meshes: An Automated Framework for Anatomical Reconstruction and Virtual Cohort Generation](https://arxiv.org/abs/2607.02564)
...
TL;DR: 本文介绍了一种将CT图像分割快速转化为适用于多物理场模拟的心脏网格的半自动流水线，用以构建虚拟患者队列。


<details>
  <summary>Details</summary>
Motivation: 传统心脏计算模型局限于单一或特定患者，且医学图像分割中常含有伪影和网格缺陷，难以直接用于多物理场模拟，限制了群体水平变异性的研究。

Method: 结合深度学习分割与基于模板的配准方法，利用倒角距离（Chamfer-distance）变形策略，将高质量的心脏网格模板变形以匹配患者个体解剖结构，同时保持拓扑一致性。

Result: 在58例健康心脏CT扫描上成功验证，生成了无缝、同拓扑且具有点对点对应关系的高质量网格；并通过主成分分析（PCA）和高斯混合模型（GMM）实现了统计形状模型的构建和合成解剖结构的生成。

Conclusion: 该开源框架实现了从原始分割到模拟就绪心脏几何体的快速转化，为大规模计算机临床试验中构建解剖一致的虚拟队列奠定了基础。
...
### [125] [Coordinate Singularities Break Conformal Coverage for Gaze and Head Pose](https://arxiv.org/abs/2607.02565)
*Mohammadreza Jamalifard,Yaxiong Lei,Parastoo Azizinezhad,Javier Andreu-Perez*

Main category: cs.CV

TL;DR: 指出在弯曲空间（如3D旋转和视线方向）中使用扁平坐标系进行保形预测会导致奇异点附近覆盖率严重下降，并提出使用测地线评分来解决此问题。


<details>
  <summary>Details</summary>
Motivation: 传统的保形预测在处理流形/弯曲空间输出时，常在扁平坐标图（如俯仰角或欧拉角）中定义评分，这在坐标奇异点附近（如万向节锁或高俯仰角）会引入系统性几何畸变，导致局部条件覆盖率崩溃。

Method: 利用黎曼体积密度诊断覆盖率崩溃发生的区域，证明了标量阈值化无法消除这种几何畸变，并提出了一种无需重新训练且计算成本极低的无坐标测地线评分（geodesic scoring）方法。

Result: 实验表明，在多个数据集（ETH-XGaze, Gaze360等）的奇异值区域，传统方法的局部条件覆盖率从90%的名义目标骤降30-50个百分点（最低至38.9%），而测地线评分成功消除了这种畸变并恢复了覆盖率。

Conclusion: 流形上的保形预测必须考虑几何度量，所提的测地线评分方法在不增加重训和计算开销的前提下，有效解决了弯曲空间中的几何畸变和覆盖率崩溃问题。
...
### [126] [MAGE: View-guided Point Cloud Completion with Efficient Modality Alignment and Adaptive Geometry Enhancement](https://arxiv.org/abs/2607.02568)
*Weize Quan,Zhengwei Wu,Kai Wang,Dong-Ming Yan*

Main category: cs.CV

TL;DR: 提出了一种名为 MAGE 的几何感知框架，通过高效的模态对齐和自适应几何增强，解决了视图引导的点云补全中的跨模态几何不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有的视图引导点云补全方法由于模态对齐弱和自身几何增强有限，导致存在跨模态几何不一致和性能受限的问题。

Method: 提出了一个统一的几何感知框架，包含：1) 结合共享自注意力与跨模态重建监督的几何感知模态对齐；2) 融合局部几何注意力与空间变化特征的自适应几何感知自注意力模块；3) 几何感知锚点细化模块。

Result: 在合成和真实世界数据集上的实验表明，该方法在点云补全任务上取得了优于现有方法的先进性能。

Conclusion: 该框架通过加强跨模态对齐和几何特征感知，有效提升了单视图引导的点云补全质量。
...
### [127] [Uncertainty-Aware Last-Layer Adaptation of RETFound for Referable Diabetic Retinopathy Screening Under Dataset Shift](https://arxiv.org/abs/2607.02569)
*Karim Mardhani*

Main category: cs.CV

TL;DR: 本文对使用视网膜基础模型进行糖尿病视网膜病变筛查的多种不确定性感知最后一层分类头进行了以安全为中心的评估，强调了跨数据集验证的必要性。


<details>
  <summary>Details</summary>
Motivation: 旨在评估不确定性感知自适应方法在可转诊糖尿病视网膜病变筛查中的安全性和选择性转诊表现，弥补仅依赖综合准确率评估的不足。

Method: 以RETFound为冻结特征编码器，在APTOS和DDR数据集上对比了缓存特征Softmax、事后温度缩放、变分贝叶斯、对角拉普拉斯近似及SNGP等最后一层分类头。

Result: 在APTOS上，不确定性感知方法显著改善了选择性转诊（推迟20%病例可使接受病例的假阴性降至0）；但在DDR跨数据集测试中，这些优势有所减弱，且SNGP模型的迁移表现较差。

Conclusion: 不确定性感知分类头虽能提升内部安全性指标，但要确保视网膜筛查的可靠性，必须进行显式的安全覆盖评估以及跨数据集的数据偏移验证。
...
### [128] [Dual-Adaptive SAM3: Hierarchical Routing over Low-Rank Expert Layers for Parameter-Efficient Medical Image Segmentation](https://arxiv.org/abs/2607.02571)
*Ying Chen,Jinyue Li,Kun Wang,Qiankun Li,Yang Liu*

Main category: cs.CV

TL;DR: 本文提出了DA-SAM3框架，通过双重自适应机制实现了高效且低开销的医疗图像开放词汇分割。


<details>
  <summary>Details</summary>
Motivation: 将SAM3模型应用于医疗图像领域时，面临直接微调参数效率低下以及传统专家混合（MoE）方法计算开销过大的挑战。

Method: 提出DA-SAM3方法，结合了联合推理图像与文本提示的动态专家路由器（DER），以及通过共享冻结基底和轻量低秩增量减少80%以上参数开销的分解参数化专家（DPE）设计。

Result: 在多个公开医疗图像分割数据集上，该方法不仅优于全微调和标准MoE模型，还比当前最先进（SOTA）的方法提升了5%的分割准确率度度度度度度。

Conclusion: DA-SAM3在显著降低参数和计算开销的同时，提供了出色的分割精度和可解释性，为医疗图像高效适配大模型提供了可行方案。
...
### [129] [$C^3$ASD: Multi-Level Consistency-Driven Representation Learning](https://arxiv.org/abs/2607.03018)
*Jin Hong,Jisoo Park,Junseok Kwon*

Main category: cs.CV

TL;DR: 提出 $C^3$ASD 框架，通过引入多级一致性约束，显著提升主动说话人检测（ASD）在面对现实世界干扰（如噪声和遮挡）时的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的视听融合方法在现实干扰下（如噪声、遮挡等）性能易退化，这源于缺乏显式一致性约束导致模型学习了脆弱的模态特异性捷径。

Method: 提出 $C^3$ASD 框架，包含三种一致性约束：嵌入级跨模态一致性（对齐视听表示）、序列级模态内一致性（通过轨迹感知对比学习分离说话和非说话簇）以及预测级一致性（通过知识蒸馏稳定融合）。

Result: 在多种音频、视觉及联合干扰下实现了显著的性能提升，同时在干净数据上保持了极具竞争力的表现。

Conclusion: 通过多级一致性约束可以有效避免多模态模型对脆弱捷径的依赖，显著增强了主动说话人检测系统在现实复杂环境中的鲁棒性。
...
### [130] [Additive Causal Construction for Transferable and Reconfigurable Cross-System Learning in Multi-Source Image Fusion](https://arxiv.org/abs/2607.02572)
*Zhizhong Fu,Wei Zhou,Zhaoyang Jiang,Yulong Lin,Yifu Hou,Xiaorong Ding,Qiang Yan,Yifan Chen*

Main category: cs.CV

TL;DR: 本文提出了一个加性因果构建（ACC）框架及其具体实现ACC-CRL，用以解决多源图像融合中的跨系统差异和纠缠问题，提升分布外（OOD）泛化性能。


<details>
  <summary>Details</summary>
Motivation: 多源图像融合中异构输入导致的跨系统差异（CSD）和跨系统纠缠（CSE）容易引起分布外（OOD）预测性能的显著下降。

Method: 提出了ACC框架（包含CGT和CGR），并实现为ACC-CRL方法。该方法采用内容-机制解耦、基于共享锚点的响应对齐以及结构不确定性调节来优化融合过程。

Result: 在ColorMNIST和多中心医疗影像（MVI预测）数据集上的实验表明，该方法在维持分布内（ID）性能的同时，显著提升了分布外（OOD）的泛化能力。

Conclusion: 实验证实了基于机制对齐和不确定性建模的ACC-CRL策略在开放环境下的有效性和鲁棒性。
...
### [131] [Symmetry-Structured Neural Completion of Islamic Geometric Patterns from Sparse Control Geometry](https://arxiv.org/abs/2607.02573)
*Hassan Ugail,Irfan Mehmood*

Main category: cs.CV

TL;DR: 提出一种将旋转对称规则嵌入神经网络的框架，用于精确补全伊斯兰几何矢量图案。


<details>
  <summary>Details</summary>
Motivation: 传统的深度学习方法难以保证生成几何图案时所需的精确旋转对称性和严格的构造规则。

Method: 利用循环群将候选晶格的边组织为旋转轨道，并在这些轨道内约束神经网络的预测或在推理中进行投影。

Result: 该方法在保证100%精确对称性的同时不损失图像保真度，即使在输入缺失的情况下也能实现零对称性违规。

Conclusion: 结合几何对称结构与数据增强可有效保障矢量图案生成的几何有效性与高保真度。
...
### [132] [UniSkip-Mamba: A Frequency-Aware State Space Model for Audio-Visual Temporal Forgery Localization](https://arxiv.org/abs/2607.04498)
*Cangjin Qiu,Quan Zhang,Dan Jiang,Ke Zhang*

Main category: cs.CV

TL;DR: 提出了UniSkip-Mamba框架，通过频域分析和跳跃扫描机制，聚焦中低频伪造特征，实现高效鲁棒的音视频时序篡改定位。


<details>
  <summary>Details</summary>
Motivation: 现有音视频伪造定位方法未区分频率成分，易拟合高频噪声，且在现实数据退化下鲁棒性不足；研究发现伪造判别模式主要集中在低/中频段。

Method: 提出UniSkip-Mamba框架，结合统一多模态序列融合以保持跨模态相位关系，并利用通过组-扫描-合并（Group-Scan-Merge）机制实现频域正则化的Skip-Scanning Mamba模块。

Result: 在LAV-DF数据集上取得63.4% AP@0.95（提升9.8%），在AV-Deepfake1M上取得63.58% mAP（提升14.32%），同时推理速度提升6倍。

Conclusion: 频域分析与跳跃扫描机制从信号处理角度证明了聚焦中低频特征能有效提升音视频伪造检测的准确率与鲁棒性。
...
### [133] [Criterion-Conditional In-Context Learning: Evaluating Criterion-Shift Adaptation in Vision-Language Models](https://arxiv.org/abs/2607.02575)
*Kaiyun Yang,Ruilin Yang,Zhimin Yao,J. Wang,Wei Ge*

Main category: cs.CV

TL;DR: 本文引入了准则条件上下文学习（CC-ICL）这一新设置及评估基准 CC-Bench，旨在评估和提升视觉语言模型根据上下文动态调整决策准则的能力。


<details>
  <summary>Details</summary>
Motivation: 虽然标准上下文学习能够诱导任务，但在现实应用中，任务的决策准则常随具体需求而变化，现有模型难以应对这种准则漂移。

Method: 提出了CC-ICL设置及“准则不变性”和“准则敏感性”两项评估指标，构建了包含双层数据层级的多领域基准CC-Bench，并引入了一种多准则微调训练策略。

Result: 实验发现多数模型存在刚性边界偏见，难以对齐隐式准则；而采用简单的多准则训练策略能显著改善这一问题，使7B模型在不损失通用性能的前提下超越闭源模型。

Conclusion: CC-ICL和CC-Bench能有效评估模型在动态准则下的适应性，且多准则训练是缓解模型刚性边界偏见的简便高效方案。
...
### [134] [Classroom Behavior Monitoring with YOLO An Empirical Study in Higher Education Settings](https://arxiv.org/abs/2607.02580)
*Sinh Vu Trong,Dung Nguyen Manh,Hieu Hoang Minh,Hieu Pham Trung,Thu Pham Ha,Nhu Le Hoang*

Main category: cs.CV

TL;DR: 本文介绍了一个真实的课堂行为视频数据集（BAV-Classroom），并评估了多种计算机视觉模型在自动课堂监控中的应用效果。


<details>
  <summary>Details</summary>
Motivation: 传统的课堂观察方法主观性强且难以推广，而课堂行为监控对评估学生参与度和提升教学质量至关重要。

Method: 收集并标注了包含9种行为类别的BAV-Classroom数据集，并对比评估了多种先进的计算机视觉模型（如YOLOv11）。

Result: YOLOv11模型取得了最佳性能；实验发现学生在课程后半段的注意力会显著下降。

Conclusion: 计算机视觉技术可有效应用于自动化课堂监控，为教学质量管理提供了有价值的见解。
...
### [135] [Evaluating Intellectual Property Guardrails of Generative Image Models: A Technical Report](https://arxiv.org/abs/2607.02582)
...
TL;DR: 评估了14个主流图像生成模型的知识产权（IP）保护机制，发现所有模型在不同程度上仍能生成包含受保护IP的图像。


<details>
  <summary>Details</summary>
Motivation: 生成式图像模型容易生成与受知识产权（IP）保护的内容高度相似的图像，因此需要评估现有模型的IP保护栏（guardrails）及其生成IP图像的倾向。

Method: 开发了一个基准和自动化评估流程，针对虚构角色、名人类比和商业标志三类IP，对14个主流文本生成图像模型（3个开源和11个闭源模型）进行评估。

Result: 所有闭源模型均存在不同程度的拒绝生成情况，其中商业标志被拒绝的频率最低且生成成功率最高；截至2026年3月，所有测试模型仍能轻易生成包含可识别IP的图像。

Conclusion: 尽管各模型对不同IP类别的拦截率有所差异，但现有的IP保护机制仍不完善，所有受试模型均能生成包含可识别知识产权的图像。
...
### [136] [RotateAttention: RoPE-Aware Rotation and Range Rectification for INT4 Quantized Attention in Video Generation](https://arxiv.org/abs/2607.02584)
*Yaofu Liu,Wanli Lan,Jinxi Li,Binhang Yuan,Harry Yang*

Main category: cs.CV

TL;DR: 针对使用3D RoPE的DiT视频生成模型，提出了混合精度INT4 FlashAttention框架（RotateAttention），在保持视频质量的同时显著提升计算速度。


<details>
  <summary>Details</summary>
Motivation: 3D RoPE的DiT视频生成模型中注意力机制计算瓶颈大，而现有低比特量化方法难以兼容RoPE，且对称量化会浪费非负注意力矩阵一半的INT4动态范围。

Method: 提出RotateAttention框架，采用RoPE感知旋转以缓解离群值，利用范围优化的P量化来充分利用INT4范围，并对精度敏感的模块进行选择性FP16回退。

Result: 在保持与全精度基线几乎相同的视频生成质量的同时，实现了高达1.68倍的端到端速度提升和2.2倍的内核级加速。

Conclusion: RotateAttention有效解决了3D RoPE在DiT视频模型中的量化难题，实现了高效且高质量的视频生成。
...
### [137] [Reliability-Aware CT-MRI Registration: A Quality Engineering Framework with Stability Analysis and Risk Classification](https://arxiv.org/abs/2607.02585)
*Nisreen Albzour*

Main category: cs.CV

TL;DR: 提出一种基于可靠性评估的CT-MRI图像配准框架，通过红/黄/绿风险分类实现单例配准的质量控制。


<details>
  <summary>Details</summary>
Motivation: 多模态配准流程通常仅提供整体平均质量指标，缺乏针对单个病例的可靠性反馈。

Method: 融合Delta NMI、Delta SSIM、Dice重叠度、稳定性及逆一致性误差构建综合评分R，并利用数据学习的阈值对配准质量进行分类评估。

Result: 仿射配准表现优于刚体配准（绿色分类占比44%对33%），可靠性筛选能有效改善平均对齐质量，且Dice重叠度是评分的主导特征。

Conclusion: 该框架为多模态图像配准提供了一个可解释的质量控制层，但其阈值划分目前基于统计学验证而非临床验证。
...
### [138] [Homer: Understanding Long-form Videos with Hierarchical Memory and Agentic Reasoning](https://arxiv.org/abs/2607.02588)
*Yixin Ji,Fanghua Ye,Juntao Li,Bo Zhao,Zexuan Qiu,Zhaopeng Tu,Liefeng Bo,Min Zhang*

Main category: cs.CV

TL;DR: 本文提出了Homer框架，通过构建分层在线记忆与智能体推理机制，解决了长视频在线理解中多步叙事推理困难的问题。


<details>
  <summary>Details</summary>
Motivation: 现有在线长视频分析方法缺乏语义结构或显式的时空因果关联，使得大模型在处理复杂的多步叙事推理时效率低下且效果不佳。

Method: 设计了分层记忆结构（涵盖感知、实体到具有因果时空关系的事件），并引入智能体推理器，通过多轮检索、验证与修正来模拟人类探索记忆并回答问题。

Result: Homer在M3-Bench-robot、M3-Bench-web和Video-MME-Long基准上分别超越此前最佳方法5.5、10.8和4.4个百分点，并显著提升了三种不同LLM主干网络的效果。

Conclusion: Homer提供了一种与具体模型无关的结构化长视频检索能力，证明了分层记忆组织与主动式智能体推理相结合的有效性。
...
### [139] [CPR: Chained Perceptual Refinement for Coarse-to-Fine Medical Image Classification](https://arxiv.org/abs/2607.02591)
*Si-Yuan Lu,Hanruo Zhu,Ziquan Zhu,Gaojie Jin,Zeyu Fu,Lu Yin,Ke Li,Lu Liu,Tianjin Huang*

Main category: cs.CV

TL;DR: 提出了一种名为CPR的由粗到精的医学图像分析框架，通过动态定位并融合局部高分辨率细节，实现了精度与计算效率的优异平衡。


<details>
  <summary>Details</summary>
Motivation: 高分辨率医学图像中的关键诊断细节分布稀疏，直接处理全图会导致极高的计算与内存开销，而简单下采样又会丢失关键细节。

Method: 设计了链式感知精细化（CPR）框架，将图像分析建模为从全局到局部的顺序决策过程，在保持主干网络输入尺寸不变的前提下，动态预测并融合局部高分辨率特征与全局上下文。

Result: 在五个医学影像数据集上的实验表明，CPR比现有最先进方法提升了高达2.27个百分点，同时在同等精度下减少了高达19.6倍的GFLOPs。

Conclusion: CPR在保持常数级GPU峰值内存的同时，有效解决了高分辨率医学图像分析中精度与效率的冲突，具有显著的实用价值。
...
### [140] [H-OPD: Confidence Aware Heterogeneous Multi-Teacher Multimodal On-policy Distillation](https://arxiv.org/abs/2607.02592)
*Qixiang Yin,Huanjin Yao,Yuchen Cai,Jianghao Chen,Ziyi Wang,Min Yang,Fei Su,Zhicheng Zhao*

Main category: cs.CV

TL;DR: 本文提出了一种名为 H-OPD 的多模态推理在线策略蒸馏（OPD）框架，通过 Token 级别的动态多导师仲裁机制提升蒸馏性能。


<details>
  <summary>Details</summary>
Motivation: 现有的 OPD 方法采用静态导师路由，忽视了不同解码步骤对视觉定位和抽象推理需求的动态变化，导致单一导师难以胜任整条生成轨迹的指导。

Method: 提出了 H-OPD 框架，采用“视觉-语言描述迁移”使纯文本导师获取视觉语义，并结合置信度感知仲裁机制在 Token 级别动态融合多模态和纯文本导师的监督信号。

Result: 在 11 个主流推理基准测试上的广泛评估表明，该方法取得了优异的性能表现。

Conclusion: 通过在 Token 级别引入异构导师的动态协作，能够有效提升多模态推理模型的蒸馏效果。
...
### [141] [Token-level Response-visual Attention Guidance for Multimodal LLMs Knowledge Distillation](https://arxiv.org/abs/2607.02593)
*Jaehyun Jang,Eunseop Yoon,Hee Suk Yoon,SooHwan Eom,Mark A. Hasegawa-Johnson,Chang D. Yoo*

Main category: cs.CV

TL;DR: 本文提出了一种名为TRAG的轻量化多模态大模型蒸馏方法，通过聚焦于“回复-视觉”注意力和Token级自适应权重调整来提升蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 传统的输出概率蒸馏不足以有效压缩多模态大模型，且现有的注意力蒸馏方法未区分不同注意力信号的作用，忽略了Token间注意力的差异性。

Method: 引入TRAG方法，将蒸馏重点转向“回复-视觉”注意力，并基于注意力熵对不同Token的KL散度进行自适应加权。

Result: 在多个基准测试上的实验表明，TRAG显著优于现有的多模态大模型蒸馏基线方法。

Conclusion: 通过优化“回复-视觉”注意力并实施Token级自适应引导，可以更有效地引导学生模型模仿教师模型的视觉关注点，从而提升模型压缩性能。
...
### [142] [An automated method of identifying incorrectly labelled images based on the sequences of loss functions of deep learning networks](https://arxiv.org/abs/2607.02594)
*Zhipeng Zhang,Wenhui Shou,Wengting Ma,Dongjia Xing,Qingqing Xu,Li-Qun Xu,Qingxia Fan,Ling Xu*

Main category: cs.CV

TL;DR: 提出了一种基于训练损失序列自动识别医疗图像错误标签的方法，有效提升了数据集质量和模型性能。


<details>
  <summary>Details</summary>
Motivation: 医疗图像人工标注中存在高达10%的错误率，这严重降级了深度学习模型的性能，亟需自动化的标签质控方法。

Method: 通过分析深度学习分类网络在多个训练周期中产生的损失函数序列，自动识别出可能存在标注错误的图像。

Result: 在眼底图像数据集上，该方法成功识别了75.31%的故意翻转错误标签，误报率仅为4.85%；纠正这些标签并重训模型后，测试准确率从95.93%提升至96.50%，接近无噪声时的96.57%。

Conclusion: 该方法能够有效通过自动化标签质量控制来纠正标注噪声，从而提升医学图像分类模型的性能。
...
### [143] [CIPHER: Causal Intervention Pathways for Healthcare Equity and Robustness](https://arxiv.org/abs/2607.02596)
*Xinyu Jia,Weidong Guo,Wangyuan Zhao,Yi Guo,Zeju Li,Yuanyuan Wang*

Main category: cs.CV

TL;DR: 本文提出CIPHER框架，通过因果干预和扩散模型生成逆事实数据，以减少医疗诊断模型在不同敏感群体间的性能差距。


<details>
  <summary>Details</summary>
Motivation: 医疗深度学习模型在不同敏感亚型（如种族、性别）间存在性能差异，而现有方法未能有效处理敏感属性影响图像特征的多种复杂因果通路。

Method: 利用结构因果模型（SCM）分析医疗图像生成过程，并提出基于扩散模型（结合无分类器引导和空文本反转）的CIPHER框架，对四条因果通路进行干预以合成患者特异性逆事实图像。

Result: 在胸部X光和皮肤镜数据集的实验中，CIPHER将最差群体的性能差距平均降低了35.8%，并同时提升了整体诊断准确率。

Conclusion: 系统性地对多条因果通路进行干预和逆事实合成，能够有效提升医疗AI模型的公平性与鲁棒性。
...
### [144] [Evaluating Agentic Harness Systems for Autonomous Computational Pathology](https://arxiv.org/abs/2607.02598)
*Jie Lin,Zongyi Chen,Qiaoling Zheng,Liuyi Wang,Hengyi Jiang,Jiabao Chen,Xiang Liu,Yinghong Yang,Liansheng Wang*

Main category: cs.CV

TL;DR: 本文提出了 ACP-Bench 框架，用于评估和审计自主计算病理学（ACP）智能体系统的工作流能力。


<details>
  <summary>Details</summary>
Motivation: 实现自主计算病理学需要将通用的智能体系统适配到病理学特定的任务、工具、证据标准和临床边界中。

Method: 开发了 ACP-Bench 框架，对 9 个模型和 3 个工具链在 41 项病理工作流任务上的 369 个执行轨迹进行了多维度评估（包括专家审计和安全审查）。

Result: 智能体系统在任务启动和报告撰写方面较成熟，但在工具执行、结果绑定和反射修正方面能力不足，端到端完全自主完成任务的情况仍然罕见。

Conclusion: ACP-Bench 为在智能体系统声称获得可靠临床自主权之前，审计其能否实际运行病理工作流提供了一个可复用的标准。
...
### [145] [CV-DCLR: Causal-Visual Dynamic Label Refinement for Robust Zero-Shot Learning](https://arxiv.org/abs/2607.02601)
*Can Wang,Jiangnan Li,Mingyu Li,Yining Song,Kangrui Ren,Min Gan,Jinfu Fan*

Main category: cs.CV

TL;DR: 本文提出CV-DCLR框架，通过引入因果干预来解决零样本学习中的语义纠缠问题。


<details>
  <summary>Details</summary>
Motivation: 零样本学习中存在语义纠缠问题，导致模型容易混淆视觉相似的概念并过度拟合伪相似性。

Method: 提出双流相互纠错机制（结合视觉似然与反事实因果干预）以及自适应门控机制来过滤干扰。

Result: 在CUB、SUN和AWA2基准上，CV-DCLR在语义纠缠的高模糊场景下表现出显著优于现有方法的鲁棒性。

Conclusion: 该框架证明了因果机制在消除语义混淆、实现鲁棒零样本分类中的有效性。
...
### [146] [DynaWM: A Base-VLA-Guided World Foundation Model for Moving-Object Manipulation](https://arxiv.org/abs/2607.02604)
*Chongkei Chang,Zhidong Deng*

Main category: cs.CV

TL;DR: 提出DynaWM，一种由基础VLA模型引导的世界基座模型，用于提升动态移动物体的操作性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理动态移动物体时，将动力学模型强行融入基础VLA中进行微调，容易导致预训练VLA模型性能退化。

Method: 提出DynaWM架构，使用Mamba-3编码动作、V-JEPA 2.1编码多视角视觉、以及本体感觉编码，共同调节流匹配DiT以重新生成动作轨迹；并构建了DynaGrasp-32基准和DynaGrasp-1600数据集。

Result: 在动态抓取任务中，DynaWM显著提升了微调（提升1.88%~45.31%）和粗调（提升26.13%~44.06%）基础VLA模型的表现，消融实验证实了各模块的有效性。

Conclusion: DynaWM能够有效适配各种基础VLA模型，显著增强了机器人在操纵动态移动物体时的鲁棒性与成功率。
...
### [147] [Privacy-Preserving Industrial Ergonomics: mmWave-Based Automated REBA Scoring and Pose Estimation](https://arxiv.org/abs/2607.02611)
*Xuhan Zhang,Zhuangzhuang Dai,Luis J. Mans,Victor Chang*

Main category: cs.CV

TL;DR: 本文提出了一种基于毫米波雷达和多任务学习的隐私保护人体工效学评估（REBA评分）框架。


<details>
  <summary>Details</summary>
Motivation: 传统的工效学评估（REBA）依赖人工且耗时，而基于摄像头的方法存在隐私泄露风险。

Method: 使用毫米波雷达，结合时空骨干网络重建3D人体骨骼以预测REBA评分，并利用多目标损失函数及过采样策略解决点云稀疏和高危姿态样本不平衡的问题。

Result: 在MMFi数据集上实现了77.78%的分类准确率，推理延迟仅5.70毫秒，高危REBA评估的平均绝对误差（MAE）达到0.93，性能显著优于对比方法。

Conclusion: 该方法为工业场景下的非侵入式人体工效学评估提供了一种兼顾隐私保护与实时性的鲁棒解决方案。
...
### [148] [Latent Visual Cache for Video Reasoning](https://arxiv.org/abs/2607.02607)
*Yongheng Zhang,Zhipeng Xu,Hao Wu,Yinghui Li,Di Yin,Xing Sun,Philip S. Yu*

Main category: cs.CV

TL;DR: 本文提出了 Latent-VC（隐式视频缓存），一种插入解码器中的循环隐式视觉缓存机制，以解决视频大模型在推理过程中视觉定位衰减的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的视频推理模型大多采用“一次读取，多次生成”的模式，导致在生成文本的过程中视觉定位能力逐渐减弱（即视觉锚定衰减）。

Method: 引入 Latent-VC 缓存，并利用有监督对比缓存对齐以及带有隐式定位奖励的视觉定位 GRPO 算法进行训练，保持训练与推理的严格对齐。

Result: 基于 Qwen3.5-9B 构建的 Latent-VC 在六个视频基准测试上均优于 CoT 和 SFT+GRPO 基线，在长视频和定位密集型任务中提升尤为显著，且生成响应更短。

Conclusion: 隐式视觉缓存能够通过有效保留视觉证据来提升视频推理能力，而无需依赖冗长的文本推理链。
...
### [149] [SE-UNet: Singular Equivariant Imaging for Real-World Constrained Generation](https://arxiv.org/abs/2607.02628)
*Kanishk Awadhiya*

Main category: cs.CV

TL;DR: 本文提出 SE-UNet，一种结合几何等变性与奇异值约束的无监督图像逆问题求解框架。


<details>
  <summary>Details</summary>
Motivation: 传统的扩散模型在处理图像逆问题时，面临需要海量训练数据以及难以引入物理约束的局限性。

Method: 将图像生成视为优化问题，并利用几何等变性（$D_4$群）与奇异值门控作为强归纳偏置进行约束。

Result: 在 CIFAR-10 的零样本图像修复任务（缺失80%像素）中，PSNR 超越 DIP 基线 4 dB 以上，并表现出快速收敛的“奇异突变”特性。

Conclusion: SE-UNet 为受约束的图像生成提供了一种高数据效率的解决方案，成功桥接了理论先验与实际部署。
...
### [150] [Fusion: A Framework for Unified Sequential Token AdaptatIon in VisiOn TraNsformers](https://arxiv.org/abs/2607.02612)
*Aravind Pradeep,Samira Nazari,Mahdi Taheri,Christian Herglotz*

Main category: cs.CV

TL;DR: 本文提出了 Fusion 框架，通过协同 Token 合并、提前退出和 Token 剪枝，实现高效且灵活的 Vision Transformer 自适应推理。


<details>
  <summary>Details</summary>
Motivation: 现有的 ViT 自适应推理方法在结合 Token 压缩与提前退出时，往往会导致中间表示不稳定和分类准确率下降。

Method: 设计了分阶段的 Fusion 框架（依次进行 Token 合并、置信度评估与 Token 剪枝），并引入轻量级路由模块，支持在推理时动态调节准确率与延迟的权衡而无需重新训练。

Result: 在 ImageNet-1k 上，该方法在保持或超越最先进自适应 ViT 性能的前提下，将校准误差降低了高达 4 倍，推理能耗减少了 48%，并在多个数据集上展现出良好的迁移能力。

Conclusion: Fusion 证明了通过合理的阶段性设计可以使多种自适应推理机制协同工作，为 ViT 提供了一种高效、节能且免微调的推理解决方案。
...
### [151] [CLABTOOLKIT: An Open-Source Toolkit for Routine Processing, Manipulation, and Visualization of Neuroimaging Data](https://arxiv.org/abs/2607.02638)
*Yasser Alemán-Gómez,Nino Hervé,Patric Hagmann*

Main category: cs.CV

TL;DR: 介绍了CLABTOOLKIT，一个旨在将异构神经影像数据结构与处理工具统一到一个高效Python框架中的开源工具包。


<details>
  <summary>Details</summary>
Motivation: 神经影像学研究需要处理多种异构的数据结构和不兼容的文件格式，迫使研究人员不断重复实现一些技术上要求高且常规的操作。

Method: 开发了CLABTOOLKIT Python包，设计了五个核心数据结构（Parcellation、Surface、AnnotParcellation、Tractogram和Connectome），并集成了BIDS管理、FreeSurfer集成、扩散MRI处理、网络分析及GPU加速可视化等19个模块。

Result: 该工具包提供了13个面向对象的类（含234个方法和207个独立函数），成功将色彩管理、分区操作、多表面可视化和纤维束追踪等功能整合在单一框架中，并支持通过JSON进行无代码的工作流定制。

Conclusion: CLABTOOLKIT为神经影像学研究提供了一个兼容性强（支持Python 3.9-3.12）、功能全面的开源解决方案，有效减少了重复开发并简化了工作流。
...
### [152] [BiSLW: Bi-Spectral Latent Watermarking for Generative Diffusion Models](https://arxiv.org/abs/2607.02643)
*Aryan Pandit*

Main category: cs.CV

TL;DR: 本文提出了一种名为 BiSLW 的双光谱潜在水印框架，用于保护扩散生成模型的版权。


<details>
  <summary>Details</summary>
Motivation: 现有的水印技术易受生成对抗攻击和扩散随机性的影响，且未能利用潜在空间频段的结构特征进行冗余嵌入。

Method: 通过可学习的编解码器，将对齐的水印信号分别嵌入到扩散潜在空间的低频（语义）和高频（纹理）互补光谱带中，并使用跨频一致性约束进行对齐。

Result: 相比之前的方法，PSNR 提升了 3 dB 以上，在面对重生成和常见图像失真时保持了接近完美的比特提取准确率，且计算开销极小。

Conclusion: BiSLW 有效平衡了图像保真度与水印鲁棒性，为扩散模型提供了一种可靠的防伪与追溯方案。
...
### [153] [EmoteGPT: 3D Human Facial Expressions from Natural Language Descriptions](https://arxiv.org/abs/2607.02674)
*Haoran Wang,Mohit Mendiratta,Christian Theobalt,Adam Kortylewski*

Main category: cs.CV

TL;DR: 本文提出 Txt2Emote 数据集与 EmoteGPT 框架，实现了高精度、细粒度的文本驱动 3D 人脸表情合成。


<details>
  <summary>Details</summary>
Motivation: 现有文本生成 3D 方法难以实现细粒度的表情控制，且缺乏将详细文本与精确 3D 表情参数相配对的数据集。

Method: 将表情合成建模为 3DMM 参数空间的回归问题；利用 GPT-4o 和人脸追踪器构建 Txt2Emote 数据集；开发基于 MLLM 的 EmoteGPT 框架，并引入大规模图像到 3D 数据的增强训练。

Result: 在情绪识别指标和表达力上超越了现有最先进的方法，成功支持写实与风格化 3D 化身生成，以及 3D 一致的 2D 人脸合成。

Conclusion: 通过结合多模态大模型与解耦的 3DMM 参数空间，可以有效实现精准且多样化的文本驱动 3D 脸部表情控制。
...
### [154] [K9-Bench: Evaluating Multimodal LLMs on Canine-Centric Videos](https://arxiv.org/abs/2607.02680)
*Khush Attarde,Yusuf Ali,Megha Thukral,Divye Bhutani,Thomas Ploetz,Zsolt Kira*

Main category: cs.CV

TL;DR: 本文提出了一个针对家犬行为和交互理解的多模态视频基准测试集 K9-Bench，旨在评估和提升大模型对动物行为的理解能力。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大模型在多领域表现出色，但其在宠物（如犬类）行为理解和人宠互动等实际场景中的评估依然处于空白状态。

Method: 开发了一种基于 VLM/LLM 的可扩展数据生成流水线，自动挖掘并清洗了包含 907 个视频、约 5000 个问答对的基准数据集，并引入了偏见缓解策略。

Result: 实验表明，当前前沿多模态大模型在犬类任务上的零样本表现有限，尤其是面对长视频中细微姿态变化和交互的复杂推理时，表现依然吃力。

Conclusion: K9-Bench 填补了宠物场景多模态评估的空白，且其通用的数据集构建方法可推广应用于其他低数据领域的基准建设。
...
### [155] [S-EMBER: A Large-Scale Benchmark for Streaming Egocentric Memory Retrieval](https://arxiv.org/abs/2607.02689)
...
TL;DR: 本文提出了S-EMBER，一个用于评估可穿戴AI流式第一人称视角情节记忆检索的新型大规模基准。


<details>
  <summary>Details</summary>
Motivation: 现有的评估方法多依赖离线完整视频访问，无法真实模拟可穿戴设备中实时流式的情节记忆检索需求。

Method: 引入S-EMBER基准，包含388小时真实生活视频和9,448个问答对，定义了流式情节检索任务，要求进行精准的时间定位。

Result: 基准测试发现前沿模型存在“定位悖论”：虽然语义推理能力随模型规模提升，但时间定位精度遇到了无法通过单纯扩大规模解决的架构瓶颈。

Conclusion: S-EMBER为开发具备可靠情节记忆的下一代可穿戴AI智能体提供了硬件真实的测试基准。
...
### [156] [An Automated Multimodal Glaucoma Detection Framework Using ViT and a Stacking-Based Ensemble](https://arxiv.org/abs/2607.02692)
...
TL;DR: 提出了一种结合眼底图像与临床数据的多模态自动青光眼检测框架，在样本和患者级别均实现了高精度诊断，并已部署为Web辅助平台。


<details>
  <summary>Details</summary>
Motivation: 传统的青光眼诊断方法费时且依赖专家经验，限制了大范围筛查的效率，因此急需自动化的快速筛查手段。

Method: 使用Vision Transformer (ViT)提取眼底图像特征，并与临床数据融合；采用经典机器学习分类器以及堆叠集成（Stacking Ensemble）方法进行预测；在样本级和患者级两种设置下进行评估。

Result: 在PAPILA数据集上，样本级检测准确率达97.47%（F1值97.50%），患者级检测准确率和F1值均达到98.97%。

Conclusion: 该多模态框架诊断性能优异，已被开发为端到端Web平台，能有效支持临床决策和自动化青光眼筛查。
...
### [157] [Property-Constrained 3D Porous Media Reconstruction from 2D Images via Conditional Generative Adversarial Networks](https://arxiv.org/abs/2607.02693)
*Ali Sadeghkhani,Brandon Bennett,Arash Rabbani*

Main category: cs.CV

TL;DR: 本文提出了一种基于2D薄片图像生成具有受控孔隙率的3D多孔介质体积的条件对抗生成网络（cGAN）框架。


<details>
  <summary>Details</summary>
Motivation: 旨在消除对昂贵3D训练数据的依赖，同时实现对生成的3D结构岩石物理性质（孔隙率）的精确控制。

Method: 采用包含3D生成器和2D判别器的混合cGAN架构，利用多轴切片提取技术从2D图像学习3D一致性结构，并结合改进的U-Net进行孔隙率标签提取。

Result: 在碳酸盐岩样本上成功生成了逼真的3D结构，孔隙率控制的 $R^2$ 达到 0.93，非均匀和均匀样本的平均绝对误差分别为 0.019 和 0.010。

Conclusion: 该框架证明了仅利用2D图像即可重建出具有高度可控孔隙率的高质量3D多孔介质体积。
...
### [158] [Aircraft Detection in Satellite Imagery using Deep Learning Object Detectors](https://arxiv.org/abs/2607.02699)
*Mujahir Hussain Abbasi,S. Beghin,A. T. R. Krishna Priya,P. Jamuna,G. Anandhakumari,S. Jaanaa Rubavathy*

Main category: cs.CV

TL;DR: 结合Gabor滤波器图像增强与YOLOv11的目标检测框架，实现了高精度（95% mAP）的卫星图像飞机检测。


<details>
  <summary>Details</summary>
Motivation: 卫星图像目标检测面临噪声、图像质量波动和复杂背景的挑战，需要提高检测准确性。

Method: 采用Gabor滤波器提取关键特征并降低噪声，进行数据归一化，并利用YOLOv11模型进行高效的目标特征学习与检测。

Result: 该方法在飞机检测中取得了95%的mAP、97%的精确率、85%的召回率和91%的F1分数。

Conclusion: 该框架能够准确识别卫星图像中的飞机，适用于监视、空中交通管制和遥感分析等实时应用领域。
...
### [159] [VLRC: Vision-Language Reprojection Consistency as a scalable signal for better feed-forward 3D pretraining](https://arxiv.org/abs/2607.02707)
*Marwane Hariat,David Filliat,Antoine Manzanera*

Main category: cs.CV

TL;DR: 本文提出了VLRC，一种利用冻结的视觉语言表示进行跨视图特征一致性监督的辅助目标，旨在提升前馈3D模型的重建和开放词汇语义理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的前馈3D模型训练受限于昂贵的几何监督或不完整的自监督光度目标，缺乏有效的学习信号。

Method: 引入视觉语言重投影一致性（VLRC），通过跨视图重投影稠密的视觉语言特征并强制对应图像位置的特征一致，无需3D标注。

Result: 实验表明，该方法在室内外基准上均提升了深度和相机估计的准确性，并改善了零样本开放词汇3D语义分割的性能。

Conclusion: 通过将几何结构与语言特征对齐，VLRC有效增强了3D重建质量和多视图语义融合的连贯性。
...
### [160] [When Does Resolution Help a Frozen Backbone? Global Attention at Resolution Predicts Scalable Adaptation for Camouflaged and Marine Animal Segmentation](https://arxiv.org/abs/2607.02708)
*Tyler Rust,Chandra Kambhamettu*

Main category: cs.CV

TL;DR: 研究探讨了冻结视觉基座模型在细粒度分割微调中，骨干网络架构（全局注意力 vs. 局部窗口）如何影响分辨率到准确率的转化，并提出了高效微调管线SALT。


<details>
  <summary>Details</summary>
Motivation: 探究在低秩适应（LoRA）下，为什么不同骨干网络（如各向同性ViT与层次化网络）在处理高分辨率输入时的性能表现存在显著差异。

Method: 通过对六种骨干网络进行对照研究和结构编辑（如调整池化与全局注意力），并提出了一种包含侧边网络、注意力门控U-Net和低秩微调的管线SALT。

Result: 发现全局注意力是使低秩适配器将高分辨率转化为高精度的关键；基于SALT的方法在多个伪装物体、显著目标及海洋生物数据集（MAS3K mIoU 达 0.878）上创下新SOTA。

Conclusion: 在低秩微调下，骨干网络在高分辨率Token上保留全局注意力至关重要，而传统的层次化局部窗口设计会限制分辨率带来的精度增益。
...
### [161] [RayTun3R: Online Camera Adaptation in 3D Foundation Models](https://arxiv.org/abs/2607.02711)
*Daniil Sinitsyn,Nikita Araslanov,Daniel Cremers*

Main category: cs.CV

TL;DR: 本文提出 RayTun3R，一种极轻量级的相机自适应方法，旨在解决 3D 基础模型在鱼眼相机下的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 预训练 3D 基础模型在针孔图像上表现良好，但在鱼眼相机下由于位置编码中存在针孔相机偏差而性能急剧下降。

Method: 引入 RayTun3R，保持预训练网络冻结，仅微调与标记位置和相机几何相关的组件（仅 10,752 个可训练参数），学习对绝对和旋转位置编码的残差修正，并结合无参数标记化和预测网格坐标修正。

Result: 在多种鱼眼数据集上，旋转误差降低了 2-12 倍，性能优于 LoRA 且参数量少约 14 倍，同时在不增加推理成本的情况下提升了姿态估计并保持了深度精度。

Conclusion: RayTun3R 能够高效、低成本地使针孔 3D 基础模型自适应鱼眼相机几何，实现良好的泛化表现。
...
### [162] [Diagnosing Aerial-View Object Detectors with Foundational Image Generative Models](https://arxiv.org/abs/2607.02718)
...
TL;DR: 本文提出一个利用可控合成图像诊断和提升航空车辆检测模型性能的评估框架。


<details>
  <summary>Details</summary>
Motivation: 利用生成模型诊断航空遥感视觉系统的潜力尚未被探索，且真实数据集难以隔离不同环境条件以进行细粒度评估。

Method: 结合文本引导生成、属性控制编辑和自动属性验证，构建了一个可控的合成测试平台，对预训练检测器进行多场景评估。

Result: 合成测试中的性能趋势与真实世界弱点一致；基于诊断结果进行针对性数据补充，使检测器AP50提升高达13%，且所需样本量更少。

Conclusion: 可控的合成探测能够有效预测真实领域的性能差距并指导高效的数据收集，具有良好的模块化和扩展性。
...
### [163] [Global Pose Control for Generative View Synthesis in Normalized Object Coordinate Space](https://arxiv.org/abs/2607.02712)
*Zhibing Li,Amogh Gupta,Behnoosh Parsa,Dan Casas*

Main category: cs.CV

TL;DR: 本文提出了一种在归一化对象坐标空间（NOCS）中实现精确相机控制的新型新视角合成（NVS）方法。


<details>
  <summary>Details</summary>
Motivation: 现有新视角合成方法由于依赖输入相对相机姿态或仅能生成稀疏的全局视角，缺乏全局且直观的目标视角控制能力，限制了下游任务的应用。

Method: 将NVS重新表述为图像编辑问题，通过多模态上下文调节机制注入相机Token，并结合文本描述来明确定义NOCS的正则坐标系以减少歧义。

Result: 该方法能够从任意无姿态的单张或少数图像中，生成具有精确一致方向的高质量新视角，在图像质量和逼真度上均达到最先进水平。

Conclusion: 通过在自定义NOCS空间中进行相机控制，并结合图像编辑模型与文本引导，有效解决了新视角合成中全局姿态控制的难题，并显著增强了对未见物体的泛化能力。
...
### [164] [GRCD: Grounded Region Change Detection for Multi-Finding Chest X-Ray Pairs](https://arxiv.org/abs/2607.02719)
*OFM Riaz Rahman Aranya,Peyman Najafirad,Kevin Desai*

Main category: cs.CV

TL;DR: 本文提出了GRCD框架，用于从成对胸部X光片中生成多诊断且具空间定位的随访报告。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法在生成多诊断报告时同时实现空间定位和准确的时序变化推理。

Method: 构建了清洗后的时序X光片数据集，并提出区域引导变化标记模块，通过双路径策略将区域级时序变化信号融入语言模型。

Result: GRCD在多诊断测试集上的文本生成、临床准确性和变化检测指标均优于现有基线方法。

Conclusion: 通过结合区域级时序编码与双路径注入策略，GRCD有效解决了多诊断时序定位报告生成的难题。
...
### [165] [Provable Pruning for Efficient 3D Gaussian Splatting via Coresets](https://arxiv.org/abs/2607.02721)
*Waseem Mousa,Alaa Maalouf*

Main category: cs.CV

TL;DR: 提出了一种针对3DGS具有理论保证的加权核心集（coreset）构建方法，以实现无需或仅需极少微调的高效压缩。


<details>
  <summary>Details</summary>
Motivation: 现有的3DGS降维方法多为启发式且缺乏理论上的近似保证，导致压缩后需要耗费大量计算资源进行微调以恢复渲染质量。

Method: 引入了首个针对特定渲染分辨率的3DGS加权核心集构建定理，通过计算灵敏度（重要性得分）对高斯分布进行采样，并将其转化为渲染质量保证。

Result: 在仅剪枝（不微调）和超短时间微调的极限压缩场景下，该方法取得了SOTA（最先进）的性能表现。

Conclusion: 证明了基于原则性重要性估算来压缩3DGS不仅在理论上可行，而且在实际硬件部署中也非常实用。
...
### [166] [Signal from Space: Detecting Schools and Towers to Bridge the Digital Divide](https://arxiv.org/abs/2607.02724)
*Zakarya Elmimouni,Sandor Farkas,Fares Fourati,Vladimir Daigele,Walid Mathlouthi,Mohamed-Slim Alouini*

Main category: cs.CV

TL;DR: 本文提出一个基于卫星图像和目标检测的视觉框架，用于自动定位学校与通信基站并评估网络覆盖情况。


<details>
  <summary>Details</summary>
Motivation: 旨在解决发展中国家学校网络连接不足的问题。传统数据稀疏且嘈杂，需要一种高效、可扩展且无需依赖第三方辅助数据的学校与通信基础设施评估方法。

Method: 提出一种纯视觉框架，利用高分辨率卫星图像和迁移学习（仅需少量标注数据微调预训练目标检测模型）来自动识别学校和手机信号塔，并通过空间分析评估其网络连通性。

Result: 在莱索托（Lesotho）的真实卫星图像上进行了验证，展示出强大的跨区域检测与评估性能。

Conclusion: 该图像驱动的方法能够实现大规模基础设施制图，减少对外部辅助数据的依赖，并为欠发达地区的网络建设投资决策提供数据支持。
...
### [167] [Biomechanics-aware Multi-view Markerless Motion Capture of Dexterous Hand Movements](https://arxiv.org/abs/2607.02796)
*Pouyan Firouzabadi,J. D. Peiffer,Kunal Shah,Anton Sobinov,Lee E. Miller,R. James Cotton,Wendy M. Murray*

Main category: cs.CV

TL;DR: 评估了一种结合生物力学模型的梯度优化无标记运动捕捉方法，用于追踪灵巧且无约束的手部运动，其表现优于传统的两阶段重构法。


<details>
  <summary>Details</summary>
Motivation: 无标记运动捕捉技术在复杂手部运动分析中的应用滞后，需要更有效的算法来精确追踪灵巧的手部动作并克服遮挡等挑战。

Method: 使用8摄像头系统采集121段手部运动视频，对比了所提出的将生物力学模型融入循环的梯度优化端到端重构方法与传统的“2D关键点三角化+逆运动学”两阶段法。

Result: 所提方法对所有121段视频均成功求解，而两阶段法在15%的视频中无法收敛；所提方法生成的手部运动学数据更符合生物力学合理性，且对物体遮挡更具鲁棒性。

Conclusion: 该端到端方法能更有效地利用2D关键点信息，实现自动且具生物力学意义的手部运动追踪，有助于临床评估、康复监测及人类运动控制研究。
...
### [168] [Track the Noise, Move the World:3D-Grounded Motion-Consistent Noise for Controllable Video Generation](https://arxiv.org/abs/2607.02798)
*Long Vu,Tan Ngo,Animesh Karnewar,Amir Habibian,Binh-Son Hua,Hung Bui,Minh Hoai Nguyen,Phong Nguyen-Ha*

Main category: cs.CV

TL;DR: 提出了 UniCaMo，通过在 3D 空间中直接操纵输入噪声，实现了对视频生成中物体轨迹和相机运动的精确协同控制，且无需修改模型架构。


<details>
  <summary>Details</summary>
Motivation: 现有的视频扩散模型在单个生成过程中缺乏对物体运动和相机运动的精确且统一的控制能力。

Method: 构建共享的 3D 运动一致噪声空间，结合局部轨迹引导的噪声扭曲（用于物体运动）和全局球面噪声采样（用于相机运动），仅修改输入噪声并结合轻量级 LoRA 微调，不改变原有模型结构。

Result: 在 Wan 2.1 (14B) 等大型视频扩散模型上应用，在标准可控视频生成基准上取得了视频质量和运动可控性的双重 SOTA 结果。

Conclusion: UniCaMo 证明了通过直接操纵输入噪声能够高效、低成本地实现高几何与时间一致性的双重运动控制（物体+相机）。
...
### [169] [Conversational Human Audio-visual Talking Dialogue Generation](https://arxiv.org/abs/2607.02799)
...
TL;DR: 本文提出了CHAT框架，能够仅通过单个文本提示词生成多样化且相互响应的双人互动视听对话片段。


<details>
  <summary>Details</summary>
Motivation: 收集大规模双人互动视听对话（DIAD）数据耗时、昂贵且存在伦理敏感性，限制了交互式虚拟人技术的发展。

Method: 提出了CHAT框架，该框架将大语言模型和说话人脸模型与互动音频及面部行为细化模块相统一，从而生成对齐的双人对话片段。

Result: 实验表明，CHAT在客观和主观评估中均优于现有方法，且合成的CHAT-AVD-50k数据集能有效提升下游交互式头像生成模型的性能（在REACT 2024上改进了PerFRDiff和ReactDiff）。

Conclusion: CHAT为解决昂贵且伦理敏感的真实双人互动数据收集问题，提供了一种极具扩展性的替代方案。
...
### [170] [Prior Bias in Vision Language Models on UML Diagram Interpretation](https://arxiv.org/abs/2607.02853)
*Zaiyu Cheng,Khai-Nguyen Nguyen,Antonio Mastropaolo*

Main category: cs.CV

TL;DR: 评估了多模态大模型在理解UML类图时是依赖视觉符号还是预训练先验，发现面对与先验冲突的图表（反转箭头）时，模型性能出现显著下降。


<details>
  <summary>Details</summary>
Motivation: 探究多模态大模型在处理UML类图等软件工程制品时，是真正理解了图中的视觉关系，还是仅仅依赖预训练中关于类关系的先验知识进行猜测。

Method: 引入了一个受控的UML基准测试，通过保持类名和布局一致但反转关系箭头来制造先验冲突，并对InternVL3.5、Qwen3等开源模型以及GPT-5.4系列闭源模型进行了评估。

Result: 反转箭头使开源模型的逻辑方向准确率平均下降33.48%（三类类图场景下下降45.28%），GPT-5.4系列也受到明显影响（分别下降10%和18.62%），且模型参数规模的扩大对该问题的改善作用有限。

Conclusion: 该研究揭示了多模态大模型在理解图表软件制品时存在严重的先验驱动失效，表明模型目前仍缺乏对视觉细节的精确理解能力。
...
### [171] [E-TraMamba: A New Paradigm for Efficient Long-Term 3D Feature Tracking with Event Cameras](https://arxiv.org/abs/2607.02866)
*Juwei Shen,Yujie Wu,Changwen Chen*

Main category: cs.CV

TL;DR: 本文提出了E-TraMamba，首个基于Mamba的事件数据3D特征跟踪框架，显著提升了跟踪长度与精度。


<details>
  <summary>Details</summary>
Motivation: 现有的CNN和Transformer追踪器在实时性要求下，难以有效捕获稀疏、多噪点事件流中的长程时空依赖关系。

Method: 引入线性状态空间模型进行高效长程建模，集成轻量级仿射变换预测器应对运动模糊与遮挡，并设计了多尺度线索融合机制，同时构建了EvD-PointOdyssey数据集。

Result: E-TraMamba实现了SOTA性能，在严格精度阈值下使特征生存期延长2倍以上，且具有更高的跟踪成功率和更低的RMSE。

Conclusion: 该框架在低延迟视觉里程计、实时SLAM和交互式机器人领域具有极大的应用潜力。
...
### [172] [Cancelable Biometric Template Protection Based on Multi-Instance Fusion: A Contralateral Iris Approach](https://arxiv.org/abs/2607.02860)
*Jittarin Chaivong,Nicha Vikromrotjananan,Teekatat Piriyapittaya,Waree Kongprawechnon,Suradej Duangpummet*

Main category: cs.CV

TL;DR: 提出一种无需外部令牌的可撤销双侧虹膜模板保护方案，通过融合左右眼虹膜信息同时提高系统的安全性和识别准确率。


<details>
  <summary>Details</summary>
Motivation: 生物特征模板一旦泄露无法重发，而现有的可撤销生物特征保护方案大多依赖外部密钥或令牌，引入了额外的攻击面。

Method: 利用多数表决融合稳定单眼图像，结合基于用户ID的加盐位排列，并通过按位异或（XOR）将左右眼排列模板融合为单一保护模板。

Result: 在CASIA等多个数据集上的测试表明，该方法的等错误率（EER）最低可达0.36%，表现优于无保护的基线方法，且与现有先进方法相当。

Conclusion: 该方案首次在符合ISO/IEC 24745标准的前提下，实现了无令牌的多实例融合与双侧虹膜绑定，有效提升了系统的识别性能与抗攻击能力。
...
### [173] [SPLIT: Training-Free AI-Generated and Partially Edited Video Detection via Spatial Patch-Level Incoherence and Temporal Roughness](https://arxiv.org/abs/2607.02886)
*Jongyeop Hyun,Hyounghun Kim*

Main category: cs.CV

TL;DR: 提出了一种无需训练的AI生成视频检测方法SPLIT，通过分析时空特征不一致性，在极低误报率下实现了高效准确的视频检测。


<details>
  <summary>Details</summary>
Motivation: 实际应用中AI视频检测需要极低的误报率（FPR）以避免误判真实视频，而传统指标（如AUROC）无法有效评估该区间下的性能。

Method: 利用冻结视觉编码器的特征，提出双步时间粗糙度（TTR）和局部空间运动不协调度（LSMI）两个信号，进行乘性融合与伽马校正，并采用面向服务对齐的固定FPR评估协议。

Result: 在FakeParts、GenVideo和ViF-Bench等基准上，SPLIT在FPR=0.1%时取得了最高的虚假召回率，显著优于现有基线方法，且具有极低的计算开销。

Conclusion: SPLIT为超低误报率约束下的AI生成与编辑视频检测提供了一种高效、免训练且鲁棒的解决方案。
...
### [174] [ProLaViT: Learning Progressive Latent Visual Thoughts in Structured Latent Space](https://arxiv.org/abs/2607.02907)
*Peiming Li,Yifan Wang,Xiaotian Zhang,Zhiyuan Hu,Shiyu Li,Zheng Wei,Yang Tang*

Main category: cs.CV

TL;DR: ProLaViT是一个允许多模态大语言模型在连续隐空间中进行渐进式视觉思考以提升复杂推理能力的框架。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂视觉推理上面临挑战，而现有方案要么计算成本高，要么依赖外部模型且缺乏严密的逻辑支撑。

Method: 采用内生自蒸馏机制进行隐式思考监督，设计了粗到细因果链和辩证推理链两种推理范式，并结合距离权重多样性损失以防止特征退化。

Result: 在多项视觉核心基准测试上超越了基线模型，显著提升了推理的准确性、可解释性和运行效率。

Conclusion: 通过在隐空间中引入结构化的渐进式视觉思考，ProLaViT成功增强了多模态大语言模型在复杂视觉任务中的逻辑推理与感知能力。
...
### [175] [Holo-Captioning: Toward the Text Equivalent of 3D Scenes](https://arxiv.org/abs/2607.02908)
*Kun-Yu Lin,Chengke Bu,Zhenguo Li,Kai Han*

Main category: cs.CV

TL;DR: 本文提出了一种全新的三维场景描述任务“holo-captioning”，并引入了新模型HoloScribe、基准数据集以及评估指标HoloScore。


<details>
  <summary>Details</summary>
Motivation: 旨在解决如何为3D场景生成包含实体语义、空间位置、属性和实体间关系等综合结构化文本描述的挑战。

Method: 开发了实例和关系描述生成引擎，构建了15K+场景的基准数据集；提出采用实例感知解耦流水线和锚点感知实例链接的HoloScribe模型；设计了HoloScore评估指标及人工标注测试集。

Result: 实验结果表明，HoloScribe在3D场景描述生成任务上显著优于现有的最先进3D稠密描述模型和3D大语言模型（LLM）通用体。

Conclusion: 本工作成功定义了holo-captioning任务，并通过创新的模型、数据集和评估指标，为3D场景的细粒度结构化文本理解提供了有效的解决方案。
...
### [176] [Learning Taxonomic Trees with Hierarchical Representation Regularization for Large Multimodal Models](https://arxiv.org/abs/2607.02909)
*Hulingxiao He,Zhi Tan,Yuxin Peng*

Main category: cs.CV

TL;DR: 提出了一种名为 $HiR^2$ 的即插即用正则化方法，通过双曲空间和单位球面的正则化设计，显著提升了大模态模型（LMM）在分层视觉识别中的分类学一致性。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在微调时通常仅依赖语言建模目标，缺乏显式的分类学正则化，导致其缺乏分类学知识且分层视觉识别一致性较低。

Method: 提出 $HiR^2$ 正则化器：1) 构建语义感知视觉树，从LLM中间层提取文本引导的粗到细视觉特征；2) 引入双曲蕴含损失（利用洛伦兹模型中的双曲蕴含锥强制执行层级关系）；3) 引入判别性分散损失（在单位球上分离相似语义的嵌入而不破坏径向层级）。

Result: 实验表明，$HiR^2$ 在多种主流LMM和微调方法上均能有效捕获分类学结构，显著提升了分层视觉识别的性能和一致性。

Conclusion: $HiR^2$ 是一种通用且高效的即插即用解决方案，成功为多模态大模型引入了分类学正则化，改善了其分层表示能力。
...
### [177] [See the Emotion: A Facial Emoji Proxy Modeling for EEG Emotion Recognition](https://arxiv.org/abs/2607.02912)
*Jingjing Hu,Guo Dan,Haofan Cheng,Ying Zeng,Zhan Si,Jinxing Zhou,Meng Wang*

Main category: cs.CV

TL;DR: 本文提出了一种将脑电（EEG）信号转化为匿名面部表情包的框架，实现了可解释且保护隐私的情感识别。


<details>
  <summary>Details</summary>
Motivation: 现有的EEG情感识别模型缺乏语义可解释性，是一个难以理解的“黑盒”，需要建立神经特征与人类可理解状态之间的联系。

Method: 提出了面部表情包代理建模框架，包含用于建模空间协同效应的FMENet，以及将表情包重建作为语义正则化器的面部表情包学习分支（FELB）。

Result: 在EAV和MMER数据集上取得了仅基于脑电模型的最先进（SOTA）准确率，并成功生成了能够准确反映情感演变的面部动画。

Conclusion: 该方法通过将高维脑电信号转化为直观的面部表情包，为大脑情感的理解提供了一个透明、保护隐私的全新视角。
...
### [178] [R3D: Quantitative 3D Spatial Reasoning for Egocentric Wearables](https://arxiv.org/abs/2607.02921)
*Maxwell Horton,Wei Lu,Quan Tran,Yury Astashonok,Kirmani Ahmed,Babak Damavandi,Anuj Kumar,Xiao Zhang,Seungwhan Moon*

Main category: cs.CV

TL;DR: 本文引入了全新的R3D-Bench基准和R3D空间工具调用框架，旨在提升第一人称RGB-D视频下的定量3D空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的空间推理基准缺乏对真实第一人称视角视频、带有位姿的RGB-D输入以及具有挑战性的定量3D空间推理问答的支持。

Method: 构建了包含3,033个问题的R3D-Bench基准；并提出了R3D框架，该框架通过图像分割和深度投影构建3D场景，并为大语言模型（LLM）提供八个可组合的空间工具进行推理。

Result: 基于Qwen3-VL 235B的R3D在R3D-Bench上达到了73.5%的平均相对准确率，显著超越了现有的深度基线模型（61.9%）和纯RGB基线模型（46.5%）。

Conclusion: R3D-Bench填补了定量3D空间推理评估的空白，而基于显式3D表示与工具调用的R3D框架展示了在解决复杂3D空间推理问题上的优越性。
...
### [179] [VideoSearcher: Empowering Video Deep Research with Multi-Tool Agentic Reasoning via Reinforcement Learning](https://arxiv.org/abs/2607.02927)
...
TL;DR: 本文提出了 VideoSearcher，一个面向视频深度研究（VDR）的闭环智能体框架，结合了多工具推理与强化学习优化算法。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态搜索智能体主要针对静态图像，且当前的视频深度研究（VDR）基准依赖文本检索，忽视了关键的视觉信息。

Method: 提出 VideoSearcher 框架（整合时空定位与多模态搜索）、BiSPO 强化学习算法（解耦工具调用与答案准确性优化）以及 VideoSearch-QA 基准。

Result: VideoSearcher 在各类搜索导向和多模态理解基准测试中显著超越了现有的开源智能体基线。

Conclusion: 该研究证明了结合多工具推理与解耦强化学习优化能有效提升智能体在开放世界视频检索和推理中的表现。
...
### [180] [STAC: Selective Spatiotemporal Aggregation and Compression for Video Reasoning Segmentation](https://arxiv.org/abs/2607.02922)
*Syed Ariff Syed Hesham,Yun Liu,Guolei Sun,Jing Yang,Henghui Ding,Xue Geng,Xudong Jiang*

Main category: cs.CV

TL;DR: 提出STAC方法，利用状态空间模型（SSM）的时空上下文感知能力进行高效的视频Token层次化压缩，显著降低了视频推理分割的计算开销。


<details>
  <summary>Details</summary>
Motivation: 视频推理分割在处理长视频时会产生密集的时空Token，导致二次方的自注意力计算成本过高；而现有Token压缩方法由于缺乏时间上下文，无法准确评估内容冗余度。

Method: 提出选择性时空聚合与压缩（STAC）方法，通过解耦的双向空间和因果时间扫描（基于状态空间模型）增强特征，并基于循环推导的冗余度进行自适应阈值的层次化压缩。

Result: 在零样本和流式传输设置下，STAC实现了85%的Token削减和1.8倍的加速，同时在推理分割基准测试中超越了无压缩的基线模型。

Conclusion: STAC通过引入状态空间模型，在降低长视频推理分割计算复杂度的同时，提升了模型的处理效率和分割准确性。
...
### [181] [MatPhaseBench: A Semantics-Guided Benchmark for Materials Phase Diagrams Understanding](https://arxiv.org/abs/2607.02934)
*Hanwen Wang,Sihan Liang,Zhiwei Liu,Yangang Wang,Wei Yan,Yuqin Liu,Zongguo Wang*

Main category: cs.CV

TL;DR: 本文提出了MatPhaseBench，一个针对材料相图的高质量复杂科学图像理解基准。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在理解逻辑复杂的材料相图（需要深层热力学机制推理）方面的系统性评估仍然有限，亟需相关基准。

Method: 从3681篇材料学论文中筛选并经人工严格校验，构建了包含200个高质量相图-文本对的基准，覆盖189个材料系统和70种元素。

Result: 实验显示当前VLM的理解能力远落后于人类专家，主要局限于表面视觉感知，缺乏深度的热力学推理、领域认知及细粒度区分能力。

Conclusion: MatPhaseBench作为一个极具挑战性的研究级基准，为复杂科学图像理解、相图分析和科学多模态AI的发展奠定了基础。
...
### [182] [CL-Anomaly: Layer-Adaptive Mixture-of-Experts with Multimodal Large Language Model for Continual Learning in Anomaly Detection](https://arxiv.org/abs/2607.02930)
*Wen Dong,Zhao Wang,Shuangqing Zhang,Kai Sun,Ben Li,Guo-Sen Xie,Caifeng Shan,Fang Zhao*

Main category: cs.CV

TL;DR: 提出了一种名为 CL-Anomaly 的参数高效微调框架，通过隔离-共享协作机制实现多模态大模型的持续异常检测。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型全参数重训成本高，且现有持续学习面临参数空间语义纠缠问题；异常检测具有模态、领域和尺度的三重异构性，使得跨任务知识迁移尤为困难。

Method: 引入任务私有专家 PrivLoRA 隔离任务特定空间以防语义纠缠；利用层自适应共享专家维护跨任务表征；采用基于动量机制的层自适应知识迁移策略，动态选择和更新共享专家以促进知识迁移。

Result: 在类增量、跨领域和跨模态三种持续异常检测场景的广泛实验中，CL-Anomaly 的性能均优于现有的最先进方法。

Conclusion: CL-Anomaly 框架成功解决了持续异常检测中的语义纠缠与多任务知识迁移挑战，为多模态大模型的持续学习提供了高效的解决方案。
...
### [183] [Pooling-Based Context Modeling for Convolution-Free Deep Image Prior](https://arxiv.org/abs/2607.02952)
*Gihyun Kim,Jong-Seok Lee*

Main category: cs.CV

TL;DR: 提出了一种无卷积的 Pool-DIP 架构，通过池化对比建模高效捕获空间上下文，提升了图像去噪与恢复的性能并降低了计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 深度图像先验（DIP）的超参数化架构在优化过程中容易拟合噪声，且计算复杂度较高。

Method: 提出 Pool-DIP 架构，去除卷积操作，引入基于池化的对比建模来捕获空间上下文，并抑制优化过程中的错误高频信号。

Result: 在多个数据集上取得了竞争性的去噪效果，显著减少了参数量和计算复杂度，并能推广至超分辨率和图像修复任务。

Conclusion: Pool-DIP 证明了无需卷积即可有效利用图像先验，为高效、防过拟合的图像恢复任务提供了一种新可行方案。
...
### [184] [ReLo-IRR: Reflection-Guided LoRA Framework for Image Reflection Removal](https://arxiv.org/abs/2607.02957)
*Chaoqun Wang,Yuehuan Wei,Haoxiang Cao,Shaobo Min*

Main category: cs.CV

TL;DR: 提出ReLo-IRR，一种基于整流流模型的反射引导LoRA框架，用于自适应、鲁棒地消除单张图像中的反射。


<details>
  <summary>Details</summary>
Motivation: 现有的反射消除方法采用统一的抑制策略，无法根据反射的严重程度进行调整，导致重度反射清除不彻底或轻度反射丢失图像细节。

Method: 设计了轻量级反射强度评估器以实现图像依赖的LoRA调制，并引入时间条件机制将反射描述符与时间步嵌入融合，实现随去噪过程动态演变的LoRA调节。

Result: 在挑战性基准测试中取得了优异的去反射效果，展现出强大的泛化能力和对不同反射程度的鲁棒抑制。

Conclusion: 联合建模反射强度与去噪动态可以有效解决反射消除中的过度或不足抑制问题，显著提升了图像恢复的质量。
...
### [185] [MORE: A Multilingual Document Parsing Benchmark and Evaluation](https://arxiv.org/abs/2607.02956)
*Long Xu,Binghong Wu,Tinghao Yu,Hao Feng,Zhenyu Huang,Haoqing Jiang,Yunhao Wang,Shuo Huang,Feng Zhang*

Main category: cs.CV

TL;DR: 提出了MORE，一个包含149种语言、支持复杂结构化元素且源于真实数据的多语言文档解析评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有文档解析基准主要集中在英语和中文等高资源语言，缺乏评估多模态大模型在其他长尾语言上性能的数据和标准。

Method: 构建了MORE基准，其具有三大特点：覆盖149种语言；包含代码块、表格和目录等复杂结构元素；通过模型辅助和人工精炼的管线从真实世界文档中收集数据。

Result: 使用MORE对前沿模型进行了评估，建立了长尾语言的新性能基准，并验证了该基准在诊断模型真实多语言文档解析能力方面的有效性。

Conclusion: MORE填补了多语言文档解析评估的空白，为在现实、多样化场景下诊断和提升大模型的多语言处理能力提供了重要工具。
...
### [186] [Incentivizing Vision Language Models to Search for Long Video Question Answering](https://arxiv.org/abs/2607.02959)
*Harsh Goel,S P Sharan,Sahil Shah,Minkyu Choi,Joungbin An,Kristen Grauman,Sandeep P. Chinchali*

Main category: cs.CV

TL;DR: VSeek是一个基于强化学习和神经符号奖励的智能体框架，将长视频问答（LVQA）转化为主动的多轮检索过程。


<details>
  <summary>Details</summary>
Motivation: 传统的长视频问答采用单次被动感知，且由于缺乏可验证的奖励信号，限制了强化学习（RL）在长视频推理中的应用。

Method: 将复杂查询编译为形式化时序逻辑规范，分解为关键目标和动作的时序清单，为RL训练提供基于视觉原语检索的可验证密集奖励。

Result: 在长视频理解基准上，VSeek相比基础模型将Pass@1得分提升了高达8%，Pass@4得分提升了15%。

Conclusion: 通过显式优化可验证的证据检索行为，结合神经符号反馈，能显著增强长视频问答的推理和检索能力。
...
### [187] [Overloading Large Vision-Language Models for Jailbreaking](https://arxiv.org/abs/2607.02961)
*Haoyu Zhang,Yangyang Guo,Mohan Kankanhalli*

Main category: cs.CV

TL;DR: 本文提出了一种利用信息过载（结合复杂文本与多维图像）对多模态大语言模型（LVLMs）进行越狱攻击的新方法。


<details>
  <summary>Details</summary>
Motivation: 现有的LVLM越狱方法设计过于简单，难以应对不断升级的安全对齐机制，且在不同模型架构间的迁移性较差。

Method: 设计了一种信息过载方法，采用基于递归的图像-排版布局，融合大量文本与多维图像攻击，以指数级增加多模态信息复杂度并干扰安全对齐。

Result: 该方法在开源模型上达到88.6%的平均越狱成功率（ASR），在商业模型上达到84.0%，且展现出极强的跨模型迁移能力。

Conclusion: 研究表明信息过载是LVLM面临的实际安全威胁，亟需针对复杂多模态越狱输入开发更强的防御机制。
...
### [188] [Parallelized Autoregressive Decoding for Omni-Modal Dense Video Captioning](https://arxiv.org/abs/2607.02963)
*Wenzheng Zeng,Siyi Jiao,Chen Gao,Hwee Tou Ng,Mike Zheng Shou*

Main category: cs.CV

TL;DR: 本文提出了一种名为 PadCaptioner 的并行自回归框架，通过重构因果依赖图实现密集视频描述的无损并行生成，兼顾了高效率与高性能。


<details>
  <summary>Details</summary>
Motivation: 传统的自回归视频大模型在密集视频描述中采用逐标记（token-by-token）生成，导致推理效率低，难以扩展至长视频和高密度事件。

Method: 利用不同事件间较弱的局部依赖关系重构因果依赖图，设计了潜在全局规划机制（自动学习事件结构并引导并行解码）和事件因子化并行解码机制（平衡局部聚焦与全局感知）。

Result: 在多个基准测试上，该方法在全模态事件定位与描述任务的效率和性能上均表现出明显优势。

Conclusion: 该研究证明了利用事件间的弱依赖性进行并行化重构可以实现无损且高效的视频自回归生成，为大规模视频理解提供了可行方案。
...
### [189] [Awakening Diffusion Transformers: Eliciting Stronger Generation and Understanding via Massive Activation Modulation](https://arxiv.org/abs/2607.02968)
*Chaofan Gan,Zicheng Zhao,Yuanpeng Tu,Xi Chen,Ziran Qin,Tieyuan Chen,Supavadee Aramvith,Mehrtash Harandi,Weiyao Lin*

Main category: cs.CV

TL;DR: 本文系统分析了扩散Transformer (DiTs) 中的大激活值 (MAs)，并提出了一个无需训练的框架 EMA，以同时提升 DiT 的生成与表征能力。


<details>
  <summary>Details</summary>
Motivation: 尽管大激活值 (MAs) 在Transformer模型中被广泛发现，但其在扩散Transformer (DiTs) 中的结构和功能作用尚未得到充分的研究和理解。

Method: 分析并定位了 MAs 的维度分布与时间步调制特性；基于此设计了 EMA 框架，包含用于生成的 MA 驱动细节引导 (DG) 和用于理解的 MA 调制表征提取 (MREP)。

Result: 实验结果表明，EMA 在无需训练的情况下，显著提升了 DiT 的图像细节生成质量，并增强了其密集特征的判别与表征能力。

Conclusion: 大激活值在 DiT 中起到了关键的调制信号作用，利用 EMA 框架可以协同提升 DiT 的生成和理解性能。
...
### [190] [RePos: Relative-to-Absolute Output Factorization for Cross-Environment WiFi-Based 3D Human Pose Estimation](https://arxiv.org/abs/2607.02986)
*Zhangcheng Hou,Tomoaki Ohtsuki*

Main category: cs.CV

TL;DR: 本文提出了解耦人体姿态与位置的RePos框架，显著提升了基于WiFi CSI的跨场景3D人体姿态估计精度。


<details>
  <summary>Details</summary>
Motivation: 现有的WiFi CSI人体姿态估计方法在跨场景部署时泛化性差，因为绝对姿态的回归导致人体结构信息与特定环境的位置线索发生耦合，容易产生过拟合。

Method: 提出RePos框架，将根节点相对姿态估计（利用身体部位感知的Token和骨架引导建模）与根节点定位（利用振幅网络和可微空间分解模块）进行解耦。

Result: 单场景下RePos-D取得SOTA精度（86.9 mm MPJPE）；跨场景下RePos相比现有方法实现了10-21%的误差降低（254.4-296.1 mm MPJPE），并在多种评估协议下保持一致的优越性。

Conclusion: 解耦人体结构与空间定位可以使相对姿态表示具有环境不变性，从而有效提高WiFi 3D人体姿态估计的跨场景泛化能力。
...
### [191] [Cross-device Collaborative Test-time Adaptation with Zeroth-order Optimization and Model Merging](https://arxiv.org/abs/2607.02988)
*Yu Mitsuzumi,Akisato Kimura,Yasuhiro Fujiwara,Hisashi Kashima*

Main category: cs.CV

TL;DR: 本文提出了一种适用于资源受限边缘设备的跨设备协作测试时自适应（CDC-TTA）方法，通过零阶优化和模型合并来降低计算与内存需求。


<details>
  <summary>Details</summary>
Motivation: 传统的TTA方法依赖高内存消耗的反向传播（BP），导致其无法在资源受限的边缘设备上部署。

Method: 在CDC-TTA框架下结合零阶优化（ZOO）与模型合并，通过将合并系数作为优化目标来降低ZOO的优化维度，并采用剪枝策略减少模型间的冗余信息。

Result: 在常见图像损坏和风格迁移基准数据集上的实验证实了该方法的有效性。

Conclusion: 该方法成功规避了反向传播的资源消耗，为资源受限设备实现高效的测试时自适应提供了可行方案。
...
### [192] [GuideMe: Multi-Domain Task Guidance and Intervention in Streaming Video](https://arxiv.org/abs/2607.02991)
*Fang Liu,Jinpeng Chen,Ke Xu,Yuhao Liu,Huankang Guan,Xudong Lu,Bo Yang,Gerhard Hancke,Rui Liu,Rynson W. H. Lau*

Main category: cs.CV

TL;DR: 本文引入了GuideMe，首个用于训练和评估多模态大模型（MLLMs）在流式视频中进行闭环交互式任务指导的多领域基准。


<details>
  <summary>Details</summary>
Motivation: 旨在研究多模态大模型在充当实时程序教练时，持续监控执行、检测错误并提供纠正引导的能力。

Method: 构建了包含2,458个视频和47,775个交互样本的GuideMe基准，并设计了包含时间-语义双边匹配、行为分类和LLM裁判的三组件评估框架。

Result: 实验发现现有模型存在性能不对称：虽擅长指导下一步，但难以识别执行错误并给出纠正反馈。

Conclusion: 现有模型在闭环交互任务指导中（特别是错误检测和纠正）仍存在关键缺陷，GuideMe为这一方向的研究奠定了基础。
...
### [193] [REAL-OW: Rehearsal-free Open World Object Detection with Low-Rank Adaptation and Dual-Stage Objectness Modeling](https://arxiv.org/abs/2607.03004)
*Huazhong Zhang,Xiaowen Fu,Yang Zhang,Linlin Shen,Jinbao Wang*

Main category: cs.CV

TL;DR: 本文提出 REAL-OW，一个无需样本重放的开放世界目标检测框架，通过协同适配器和双阶段目标性建模解决灾难性遗忘与表征漂移问题。


<details>
  <summary>Details</summary>
Motivation: 现有的开放世界目标检测方法严重依赖样本重放来缓解灾难性遗忘，但这在实际应用中会带来数据隐私风险和高额的内存开销。

Method: 提出 REAL-OW 框架，利用基于 LoRA 的协同适配器架构解耦增量知识，并引入双阶段目标性建模（DSOM）结合校准高斯负对数似然（CG-NLL）距离来稳定目标性分布。

Result: 实验结果表明，REAL-OW 在检测精度和未知目标发现方面均超越了现有的样本重放方法，取得了最先进的性能。

Conclusion: 该研究成功为无样本重放的开放世界目标检测奠定了新的基准。
...
### [194] [VISTA: Auditing Semantic Divergence in Vision-Language Models](https://arxiv.org/abs/2607.02995)
*Junchi Liao,Jiawen Deng,Fuji Ren*

Main category: cs.CV

TL;DR: 提出VISTA，一种针对视觉语言模型（VLM）的黑盒交叉审计方法，用于检测由视觉概念（如人口特征、标志）引起的模型行为异常与偏见。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型容易表现出由视觉概念调节的分歧行为（产生特定偏见或反应），而这种行为无法被传统的纯文本审计手段所检测。

Method: 提出VISTA方法，这是一种黑盒跨模型审计工具，通过结合语义熵与基于分布的分歧指标来筛查模型特有的异常行为。

Result: 在受控实验中成功检测出植入的偏差立场；审计6个VLM发现142个高可疑案例（1.2%），并识别出选择性拒绝这一新分歧模式（各群体拒绝率达0%至65%）。

Conclusion: VISTA能有效检测出多模态模型中由视觉概念引起的隐蔽偏见，为评估多模态模型的安全性和一致性提供了重要工具。
...
### [195] [CONFLUX: A Latent Diusion Model for 3D Chest-CT Synthesis with RL Post-Training](https://arxiv.org/abs/2607.02998)
*Max Van Puyvelde,Halil Ibrahim Gulluk,Wim Van Criekinge,Olivier Gevaert*

Main category: cs.CV

TL;DR: 本文提出CONFLUX，一种结合三维潜层扩散模型与强化学习后训练的可控三维胸部CT图像生成模型。


<details>
  <summary>Details</summary>
Motivation: 生成可控的三维医学影像需要同时具备高保真度、原生三维结构以及对临床属性条件的精确遵循，当前方法在这三者结合上存在挑战。

Method: 使用3D变分自编码器（VAE）压缩体积，通过整流流变换器（Rectified-flow Transformer）在潜空间进行条件生成，并引入基于群组相对策略优化（GRPO）的在线强化学习后训练来强化属性控制。

Result: 三平面FID达到32.3，显著优于基线模型MAISI（74.6）；后训练优化消除了47%与真实扫描可靠性之间的差距，并开源了模型及约20万例合成胸部CT数据集。

Conclusion: 结合强化学习的CONFLUX模型显著提升了三维医学图像生成的保真度与属性可控性，为临床数据合成提供了强大的工具。
...
### [196] [PosterHarness: Turning Scientific Poster Generation into an Auditable Instruction-Following Benchmark](https://arxiv.org/abs/2607.03006)
*Tianyi Yang,Dawei Fu,Youpeng Wu,Zixun Kou,Linrui Chen,Ruobing Jiang,Zijian Wang,Qiang Li*

Main category: cs.CV

TL;DR: 本文提出了 POSTERHARNESS，一个用于评估学术海报生成模型是否遵循学术规范（如避免虚假图表）的可审计评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有模型在生成海报时容易编造科学图表，且缺乏量化评估其是否遵守学术通信契约（如标签清晰度、比例合规等）的方法。

Method: 引入“占位符优先”机制，将布局设计与图表生成分离。模型仅规划包含标签的空白占位符，由确定性合成器插入真实图表，从而实现对占位符精度、防伪造等指标的量化审计。

Result: 实验表明，该机制将合成虚假图表数量降至0；识别了四类阻碍契约的失败类型；与Paper2Poster相比，生成的海报分辨率更高、视觉偏好更强，但运行速度稍慢。

Conclusion: POSTERHARNESS 为学术海报生成提供了可度量的指令遵循评估方案，有效遏制了学术图表造假，促进了该领域的规范化发展。
...
### [197] [HyperVAttention: Efficient Sparse Attention with Spatio-Temporal Clustering for Video Diffusion](https://arxiv.org/abs/2607.03012)
*Dongyeun Lee,Amir Zandieh,Vahab Mirrokni,Junmo Kim,Insu Han*

Main category: cs.CV

TL;DR: 本文提出了 HyperVAttention (HVA)，一种用于视频扩散 Transformer (VDiT) 的免训练稀疏注意力框架，旨在解决长视频生成中的计算瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有的基于聚类的稀疏注意力方法在生成长视频时，面临聚类开销过大以及因不规则聚类块导致 GPU 硬件（CTA）利用率低下的双重瓶颈。

Method: 引入了三项核心设计：1) 结合定制 Triton 内核的 3D 局部窗口聚类；2) 仅在锚点步进行完整聚类的混合聚类策略；3) 优化 GPU 执行效率的硬件感知聚类合并。

Result: 实验表明，HVA 在文本到视频生成任务中将端到端延迟降低了高达 2.13 倍，同时在生成质量上优于现有的免训练稀疏注意力基准方法。

Conclusion: HVA 通过协同优化算法与硬件执行效率，为视频扩散模型中的免训练稀疏注意力机制确立了新的性能帕累托前沿。
...
### [198] [MambaLIE: Scene Light Intensity-Boosted Low-Light Image Enhancement with State Space Model](https://arxiv.org/abs/2607.03013)
*Wanshu Fan,Xiangyu Li,Cong Wang,Kin-man Lam,Xin Yang,Haiyan Zhang,Dongsheng Zhou*

Main category: cs.CV

TL;DR: 本文提出了一种基于状态空间模型（SSM）的高效低光照图像增强方案 MambaLIE，实现了高性能与低计算成本的平衡。


<details>
  <summary>Details</summary>
Motivation: CNN难以建模长距离依赖，而Transformer计算复杂度过高，限制了低光照图像增强算法在资源受限设备上的应用。

Method: 引入场景光强引导机制，并提出局部增强状态空间模型（LESSM），通过SSM分支建模长距离依赖，通过局部增强分支捕获局部特征。

Result: 在多个合成与真实数据集上，该方法在图像恢复质量、运行速度和参数量上均优于现有的CNN和Transformer方法。

Conclusion: MambaLIE在保持高性能的同时降低了计算开销，非常适合实际部署在资源受限的移动端设备上。
...
### [199] [Natural Language Camera Movement Understanding](https://arxiv.org/abs/2607.03043)
*Yuwen Tan,Joey Huang,Jin Huang,Haoxiang Li,Boqing Gong*

Main category: cs.CV

TL;DR: 本文研究了自然语言对相机运动的理解，指出当前视觉语言模型（VLM）在这一任务上的不足，并提出了新的分类法、基准测试、训练数据集以及微调模型。


<details>
  <summary>Details</summary>
Motivation: 现有的VLM在理解相机运动时经常出现混淆（如平移与旋转、左右方向、物体与相机运动），而理解相机运动对于视频生成模型等应用至关重要。

Method: 引入了两级电影摄影分类学，建立了包含真实和合成视频的基准测试，构建了经过针对性相机运动增强的大规模多源训练集，并对VLM-8B进行微调。

Result: 微调后的VLM-8B在真实和合成视频基准上分别比Gemini 3.1 Pro高出10%和11%。

Conclusion: 虽然模型性能有所提升，但与人类表现相比仍有较大差距，表明自然语言相机运动理解领域仍需进一步深入研究。
...
### [200] [OmniDS: Dual-Stream Context Fusion for Omnidirectional Depth from Fisheye Cameras](https://arxiv.org/abs/2607.03038)
*Chaesong Park,Jihyeon Hwang,Muyeol Sung,Jongwoo Lim*

Main category: cs.CV

TL;DR: OmniDS是一个通过动态上下文融合、多视角共识和知识蒸馏解决多鱼眼相机深度估计中遮挡与刚性投影问题的迭代精细化框架。


<details>
  <summary>Details</summary>
Motivation: 多鱼眼相机系统的宽基线会导致视线冲突（不同相机观测到物体的不同部分）。在固定等距柱状投影（ERP）下进行刚性特征聚合，容易在遮挡边界和薄结构处产生模糊的匹配线索。

Method: 使用CNN与DINOv3双流编码器提取几何和语义特征，通过可变形交叉注意力进行动态ERP重投影；构建多视角共识体积（由3D U-Net正则化）以捕获全局一致性；最后将双流表示蒸馏至轻量化单流MobileNet编码器中以实现快速推理。

Result: 在OmniThings、OmniHouse和Sunny等基准数据集上均达到了最先进的（SOTA）性能，同时保持了极具竞争力的推理速度。

Conclusion: OmniDS通过引入动态上下文融合与共识感知机制，有效解决了多鱼眼系统中的可见性冲突和刚性采样限制，并通过特征蒸馏实现了高效的实际部署。
...
### [201] [CURE: Controllable Unified Image Restoration for Complex Degradations](https://arxiv.org/abs/2607.03044)
*Boseong Kim,Donghyeon Cho*

Main category: cs.CV

TL;DR: 本文提出CURE框架，通过学习解耦且可调的表示，实现了在复杂复合退化场景下的可控图像修复。


<details>
  <summary>Details</summary>
Motivation: 复合退化中各退化因素相互交织影响，导致难以在恢复过程中进行选择性控制和精确修复。

Method: 提出由四个目标驱动的CURE训练框架：1) 恒等嵌入与重构约束，2) 基于混合比例的连续强度控制，3) 监督分步退化消除的中间损失，4) 消除顺序影响的排列不变损失。该框架仅修改训练策略，可无缝集成到现有模型中。

Result: CURE在复合退化基准测试中取得了最先进的性能，并允许用户通过调节嵌入比例进行灵活的选择性修复或联合修复。

Conclusion: CURE是一个即插即用的图像修复框架，成功解决了复合退化下可控且一致的图像恢复难题。
...
### [202] [Text-to-Image Generation for Projector-Camera System Registration](https://arxiv.org/abs/2607.03046)
*Xinyu Chen,Yuqi Li,Jiabao Li,Pinyan Tang,Chong Wang,Aditi Majumder*

Main category: cs.CV

TL;DR: 本文提出了一种基于深度神经网络的投影仪-相机（procam）配准方法，通过使用由文本提示生成的兼具自然度与丰富特征的单张自然图像，实现了高精度且低视觉干扰的配准。


<details>
  <summary>Details</summary>
Motivation: 传统的结构光配准方法虽精确但效率低且视觉干扰大，而现有的自然图像方法在面对环境光照变化和表面纹理时配准精度不足。

Method: 开发了一种深度神经网络，在模拟了投影相机系统几何和光度畸变的合成数据集上进行训练，利用文本生成的特征丰富且自然图像来预测投影仪与相机图像之间的像素对应关系。

Result: 该方法在多种系统配置下显著提高了配准精度，有效减少了投影内容的视觉破坏，并在用户调研中证实了其在自然度和实用性上的优势。

Conclusion: 该方法成功兼顾了投影图像的自然美观与配准的高精度，在实际投影应用中具有极高的实用价值。
...
### [203] [RIGS-Refiner: Risk-Guided Recursive Refinement in Prediction Space for Colonoscopy Polyp Segmentation](https://arxiv.org/abs/2607.03058)
*Jiachi Zhang,Zhuoyu Wu,Wenqi Fang*

Main category: cs.CV

TL;DR: 提出了一种轻量级的结直肠镜分割后处理插件 RIGS-Refiner，通过风险引导的递归机制以极低的计算开销优化局部预测边界。


<details>
  <summary>Details</summary>
Motivation: 现有的后处理方法计算成本高且参数量大，而结直肠镜分割的错误主要集中在边界等局部区域，因此需要一种针对预测空间的轻量化选择性局部修复方法。

Method: 设计了 RIGS-Refiner 插件，利用冻结的主预测网络提取图像先验和预测线索，通过共享的递归单元实施风险引导的更新，仅引入 519 个参数和 0.631 GFLOPs 计算量。

Result: 在 Kvasir、ClinicDB、ColonDB 和 ETIS 数据集上结合 PraNet 和 SegFormer-B0 进行评估，结果表明该方法在保持极高效率的同时，显著且一致地提升了分割精度。

Conclusion: RIGS-Refiner 证明了在预测空间进行轻量级递归局部修复的可行性，为结直肠镜图像分割提供了一种高效且实用的后处理解决方案。
...
### [204] [Lightweight Polyp Segmentation via a Gain-Aware Prediction-Space Recursive Controller](https://arxiv.org/abs/2607.03062)
*Jiachi Zhang,Zhuoyu Wu,Quanjun Wang,Wenhui Ou,Wenqi Fang*

Main category: cs.CV

TL;DR: 提出一种预测空间递归控制器，通过直接对主干网络logits进行轻量级修正，提升息肉分割的准确率与部署效率。


<details>
  <summary>Details</summary>
Motivation: 现有轻量级分割模型的性能提升往往依赖于更重的编码器或复杂的细化分支，难以孤立评估轻量级修正机制本身的有效性。

Method: 将细化过程建模为预测空间递归修正任务，设计了一个在固定计算预算下运行的递归控制器，通过聚合差异与不确定性证据来应用加性残差修正。

Result: 在多个数据集和7个轻量级主干网络上的实验表明，该方法在保持极低参数量、计算量和内存开销的同时，显著提升了分割精度，效果媲美更重的结构细化方法。

Conclusion: 该递归控制器是一种轻量、便携且易于部署的有效息肉分割细化方案。
...
### [205] [PixCon: Clean-Positive Contrastive Learning for Foundation-Model Semi-Supervised Segmentation](https://arxiv.org/abs/2607.03068)
*Ebenezer Tarubinga*

Main category: cs.CV

TL;DR: 提出PixCon框架，通过构建无污染的正样本记忆库，提升了基于基座模型的半监督语义分割（SSSS）的特征表示能力与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 基座模型（如DINOv2）使伪标签质量大幅提升，半监督分割的关键从“伪标签过滤”转向“如何构建类间嵌入空间”。然而，现有对比学习方法依赖的记忆库仍存在伪标签带来的样本污染问题。

Method: 提出PixCon框架，其记忆库仅吸纳学生网络已正确分类的“有标签像素”，从而在结构上保证正样本集完全无污染（$ρ_F=0$）。该方法不增加推理参数，且无需特定阈值。

Result: 在一阶梯度分析中证明了污染对InfoNCE损失的负面影响。在Pascal VOC、Cityscapes和ADE20K上，PixCon匹配或超越了强基线UniMatch V2（例如在Pascal-1/8上平均mIoU达到87.90）。

Conclusion: 零污染保证（$ρ_F=0$）主要在教师模型变弱时提供鲁棒性，而精度提升则源于更干净的正样本监督，这使得清洁正样本对比成为基座模型下半监督语义分割的高效且鲁棒的默认方案。
...
### [206] [SafeGuard: A Multi-Agent Perception-Reasoning Framework for Social-Risk AI-Generated Video Detection](https://arxiv.org/abs/2607.03069)
*Wenlin Wu,Sheng Zhou,Peipei Song,Wenhao Wang,Junbin Xiao,Xun Yang*

Main category: cs.CV

TL;DR: 本文提出了一个结合取证感知与语义推理的多智能体框架 SafeGuard，以及一个专注于物理和社会逻辑合理性检测的新基准数据集 SafeVid。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成视频检测方法存在“感知-推理差距”：偏向感知伪影的检测器缺乏语义推理能力，而视觉语言模型则对微观取证线索不敏感。

Method: 提出了 SafeGuard 框架（由分层感知求解器和自反思验证器组成）以构建证据链，并推出了包含2万个视频、涵盖10大社会风险类别的 SafeVid 评估基准。

Result: SafeGuard 在 SafeVid 数据集上的检测准确率提升了 18.7%，并在其他四个公开基准上持续超越现有主流方法。

Conclusion: 该研究成功桥接了视频伪造检测中的感知与推理鸿沟，显著提升了检测的泛化能力与可解释性。
...
### [207] [Attention-Guided Efficientnet Architecture For Precise Criminal Identification in Surveillance Images](https://arxiv.org/abs/2607.03073)
*Savitha N J,Lata B T*

Main category: cs.CV

TL;DR: 提出了一种基于注意力机制的AG-EfficientNet框架，旨在解决复杂监控图像下的罪犯人脸识别难题。


<details>
  <summary>Details</summary>
Motivation: 监控场景下的人脸识别由于受低分辨率、光照变化、模糊及遮挡等环境因素影响，识别难度极大。

Method: 将EfficientNet-B0与卷积块注意力模块（CBAM）融合，采用多尺度特征融合策略，并结合混合Softmax-Triplet优化机制进行训练。

Result: 在LFW和SCFace数据集上取得了98.2%的准确率和0.99的ROC-AUC，性能显著优于AlexNet、VGG16和ResNet50等传统网络。

Conclusion: 实验证明，AG-EfficientNet框架及注意力引导特征学习策略能有效提升恶劣监控图像下的罪犯识别精度与鲁棒性。
...
### [208] [PotatoGANs: Utilizing Generative Adversarial Networks, Instance Segmentation, and Explainable AI for Enhanced Potato Disease Identification and Classification](https://arxiv.org/abs/2405.07332)
*Fatema Tuj Johora Faria,Mukaffi Bin Moin,Mohammad Shafiul Alam,Ahmed Al Wase,Md. Rabius Sani,Khan Md Hasib*

Main category: cs.CV

TL;DR: 本文提出PotatoGANs，利用GAN生成合成土豆病害图像以增强数据，并结合可解释性AI算法提高了分类模型的泛化与可解释性。


<details>
  <summary>Details</summary>
Motivation: 深度学习在农作物病害分割中易出现过拟合，且传统数据增强方法泛化能力有限，因此需要更有效的方法来快速准确识别影响产量的土豆病害。

Method: 提出PotatoGANs方法，利用CycleGAN和Pix2Pix从健康土豆图像生成病害图像；并结合三种可解释性AI算法（GradCAM, GradCAM++, ScoreCAM）和三种CNN架构进行分类与可解释性分析。

Result: 实验表明PotatoGANs能生成高质量的真实图像，其中CycleGAN在黑痣病和疮痂病上分别取得了1.2001和1.0900的更高Inception分数（IS），优于Pix2Pix。

Conclusion: 合成数据能有效改善大型神经网络训练，降低数据采集成本并提高泛化能力，结合可解释性AI也提升了模型的透明度。
...
### [209] [SNR-Adaptive Unified Diffusion for Multi-Task Medical Image Segmentation](https://arxiv.org/abs/2607.03103)
*Jiahao Liu,Hang Wei,Shuai Wu*

Main category: cs.CV

TL;DR: UniT-Diff 是一个统一的扩散心脏图像分割框架，通过解决联合训练中的标签冲突和梯度失衡，在多个基准上超越了独立训练的专用模型。


<details>
  <summary>Details</summary>
Motivation: 临床心脏影像通常为不同数据集和模态部署独立模型，导致训练冗余且无法共享知识。尝试将多种学习范式整合至单一模型时，会面临标签语义冲突和梯度失衡的挑战。

Method: 提出 UniT-Diff 框架，包含三个核心机制：11通道任务特定输出空间（物理隔离标签）；信噪比自适应任务调节（SATC，动态缩放任务Token以抑制领域偏差）；以及任务类型感知条件丢弃（TTACD，为泛化输入移除任务Token以路由至共享解剖通路）。

Result: 在单一参数配置下，UniT-Diff 在三个基准上均超越了独立训练的基线模型：LA 提升 0.87%，MMWHS 提升 1.77%，MNMS 提升 0.88%。

Conclusion: UniT-Diff 成功克服了多源医学影像联合训练中的负迁移问题，证明了用统一扩散模型处理多任务心脏图像分割的高效性与可行性。
...
### [210] [Vidu S1: A Real-Time Interactive Video Generation Model](https://arxiv.org/abs/2607.03118)
...
TL;DR: 本文介绍了Vidu S1，一个支持语音交互控制的实时视频生成模型，能以高帧率生成无限长度且无失真的视频。


<details>
  <summary>Details</summary>
Motivation: 旨在解决现有视频生成模型在实时交互、长视频生成质量（防模糊和漂移）以及消费级GPU上低延迟推理方面的挑战。

Method: 开发了基于TurboDiffusion和TurboServe的Vidu S1模型，支持通过语音指令实时控制数字化角色，并支持自定义图像输入和音调选择。

Result: 在普通消费级GPU上实现了高达42 FPS的540p实时视频生成，实验表明其在各项测试指标上均达到最优，且无模糊或视觉失真。

Conclusion: Vidu S1成功实现了高效、高质量且具备实时交互能力的视频生成，证明了其在个性化数字角色控制领域的实用价值。
...
### [211] [ExpoMotion: A Large-Scale Benchmark and A Householder Projection Network for Multi-Exposure Fusion](https://arxiv.org/abs/2607.03110)
*Yao Liu,Lishen Qu,Shihao Zhou,Jie Liang,Hui Zeng,Yabin Peng,Huipeng Lin,Lei Zhang,Jufeng Yang*

Main category: cs.CV

TL;DR: 本文提出了一个大型多曝光融合去鬼影基准数据集 ExpoMotion，并设计了基于豪斯霍尔德正交投影的 HOP 网络以消除动态场景鬼影。


<details>
  <summary>Details</summary>
Motivation: 多曝光融合（MEF）易产生运动鬼影，且现有基准数据集缺乏高质量的动态场景以及可靠的真实标签（GT）。

Method: 构建了包含10,909张图像的 ExpoMotion 数据集；提出 HOP 网络，利用 GPIA 模块进行曝光对齐，并通过 HOA 模块将鬼影作为正交扰动进行投影过滤。

Result: 实验证明 ExpoMotion 数据集能显著提升模型的泛化与细节恢复能力，同时验证了 HOP 方法在去鬼影方面的高效性与先进性。

Conclusion: 本工作通过发布高质量基准数据集和提出创新的数学投影网络，为解决多曝光融合中的动态鬼影问题提供了有效的解决方案。
...
### [212] [SiamixFormer: a fully-transformer Siamese network with temporal Fusion for accurate building detection and change detection in bi-temporal remote sensing images](https://arxiv.org/abs/2208.00657)
*Amir Mohammadian,Foad Ghaderi*

Main category: cs.CV

TL;DR: 提出了一种名为SiamixFormer的双生Transformer模型，结合灾前和灾后图像进行特征融合，显著提升了建筑物及变化检测的性能。


<details>
  <summary>Details</summary>
Motivation: 现有建筑物检测模型多仅使用单张灾前图像，忽略了灾后图像中的时序特征，且传统CNN在融合特征时难以维持大感受野。

Method: 设计了双编码器的层次化Transformer架构，利用时序Transformer将灾前图像（Query）与灾后图像（Key/Value）进行特征融合，并在各阶段使用简单MLP解码器输出结果。

Result: 在xBD、WHU建筑物检测数据集以及LEVIR-CD、CDD变化检测数据集上的评估结果表明，该模型性能超越了现有的先进方法。

Conclusion: 通过结合时序Transformer的双生结构，能更有效地利用双时相图像的时空特征，是大感受野特征融合与检测的有效方案。
...
### [213] [A Multi-Task Deep Learning Framework for Real-Time Intelligent Video Surveillance with Temporal Event Validation](https://arxiv.org/abs/2607.03131)
*Estera Dumitru,Stelian Spînu*

Main category: cs.CV

TL;DR: 提出一个集成多种安全检测任务的统一深度学习框架，通过时间事件验证机制显著降低误报并实现实时监控。


<details>
  <summary>Details</summary>
Motivation: 现代视频监控产生的数据量远超人工监控能力，亟需自动化、多任务的智能分析系统来实时检测安全事件。

Method: 构建了一个统一的多任务框架，融合了人脸、车牌、武器、火灾及异常行为检测模块，并引入基于多帧确认和置信度加权投票的时间事件验证架构以过滤帧级预测。

Result: 武器检测mAP达0.947，防范破坏行为识别率达94.33%；火灾误报率从52%降至4%，车牌识别准确率从66.7%提升至81.8%，单帧延迟低于100毫秒。

Conclusion: 将专用深度学习模型与时间事件验证相结合，为可靠的实时智能视频监控提供了一种切实可行的解决方案。
...
### [214] [DistillH-Mamba: A Hypergraph-Mamba-Based Knowledge Distillation Model for Efficient Impact Fall Detection](https://arxiv.org/abs/2607.03156)
*Tresor Y. Koffi,Youssef Mourchid,Mohammed Hindawi,Yohan Dupuis*

Main category: cs.CV

TL;DR: 提出了一种名为 DistillH-Mamba 的轻量级且高效的跌倒冲击检测架构，旨在实现高精度的实时跌倒监测。


<details>
  <summary>Details</summary>
Motivation: 准确识别跌倒时的撞击瞬间非常困难，且现有的深度学习算法模型复杂度高、计算量大，难以满足实时部署的需求。

Method: 提出 DistillH-Mamba 架构，包括：引入超图（Hypergraph）捕捉多关节间的高阶复杂空间交互；将 Mamba 架构与超图结合以加速处理并捕获突变运动特征；采用关系知识蒸馏技术降低计算量以实现实时检测。

Result: 在 UP-Fall 和 UMAFall 数据集上的评估显示，该模型在跌倒冲击检测中达到了 97.38% 的准确率，且推理时间较教师模型缩短了 73.8%，在精度和效率上均超越了现有最先进的方法。

Conclusion: DistillH-Mamba 成功解决了跌倒冲击检测中精确度与实时性之间的矛盾，为老年人跌倒监测提供了一种高效且实用的解决方案。
...
### [215] [Text as Partial Constraint: Core-Residual Alignment for Robust Vision-Language Learning](https://arxiv.org/abs/2607.03143)
*Chengzhen Yu,Canran Xiao,Siyuan Ma,Yang Liu*

Main category: cs.CV

TL;DR: 提出TPC框架，将文本视为局部约束，以提高视觉-语言表示在不完全文本监督下的稳定性与可靠性。


<details>
  <summary>Details</summary>
Motivation: 自然语言描述通常是不完整的，导致视觉-语言对齐在面对同义改写或细节缺失时匹配脆弱且容易过度自信。

Method: 提出“文本即局部约束”（TPC）框架，通过提取共识语义核心作为对齐目标，抑制未提及残差对相似度的影响，并引入不确定性感知对比目标来缓和对齐。

Result: 在ImageNet上达到81.42%的分类准确率和64.05%的鲁棒准确率，并在Avg-14迁移集和大视觉语言模型（LVLM）迁移中取得了显著的性能提升。

Conclusion: 将文本建模为局部约束是解决不完全语言监督、构建更可靠视觉-语言表征的一种可行且有效的途径。
...
### [216] [OpenGlass: A Sensing-Computing Split Architecture for Local MLLM-Driven Real-Time Visual Assistance](https://arxiv.org/abs/2607.03213)
*Mengzhang Li,Yuan Yao*

Main category: cs.CV

TL;DR: OpenGlass是一个开源、本地优先的低延迟多模态视觉辅助系统，旨在保护隐私的同时为视障用户提供辅助。


<details>
  <summary>Details</summary>
Motivation: 云端大模型助理存在隐私泄露风险和高网络延迟，而可穿戴眼镜设备因算力和功耗限制无法直接运行大模型。

Method: 采用“感知-计算分离”架构，由ESP32眼镜端负责图像捕获，附近的消费级设备负责本地大模型推理与语音输出。

Result: 调整分辨率后中值延迟为993毫秒（97.5%低于2秒），原始720p图像中值延迟为1625毫秒（93.3%低于2秒）。

Conclusion: OpenGlass为障碍物感知和物体查询等任务提供了一个低延迟、保护隐私的本地化视觉辅助参考平台。
...
### [217] [FairFlow: Demystifying and Mitigating Stereotype Bias in Text-to-Image Diffusion Transformers](https://arxiv.org/abs/2607.03180)
*Chen Chen,Yuanmin Huang,Zhenfei Zhang,Mi Zhang,Xiaohan Zhang,Yun Xiong,Xiaoyu You,Min Yang*

Main category: cs.CV

TL;DR: 本文揭示了多模态扩散Transformer（MM-DiT）中偏见产生的内部机制，并提出了一种名为FairFlow的轻量级偏见缓解框架。


<details>
  <summary>Details</summary>
Motivation: MM-DiT模型存在严重的刻板印象偏见，且现有的去偏策略主要针对旧的U-Net架构，难以直接应用于MM-DiT。

Method: 通过机制分析定位了负责偏见传播的稀疏语义绑定枢纽层，并在这些特定层中引入稀疏引导，注入属性特定的公平方向。

Result: 在FLUX.1-dev和Stable Diffusion 3上的评估表明，FairFlow能有效消除性别、种族等偏见，且推理开销几乎为零，实现了良好的公平与保真度平衡。

Conclusion: FairFlow为大规模部署的MM-DiT系统提供了一种轻量且实用的偏差消除方案。
...
### [218] [Semantic Segmentation-Driven Image-Level Diagnosis of Liver Cancers in Hematoxylin and Eosin Histopathology Images](https://arxiv.org/abs/2607.03253)
*Ivica Kopriva,Dario Sitnik,Arijana Pacic,Karolina Krstanac,Irena Veliki Dalic,Marijana Popovic Hadzija*

Main category: cs.CV

TL;DR: 本研究提出了一种结合染色归一化和语义分割的深度学习框架，用于高精度自动分类三种肝癌类型的H&E染色全视野图像。


<details>
  <summary>Details</summary>
Motivation: 组织切片制备、染色和扫描的高度变异性，以及病理专家像素级标注的不确定性，使得基于H&E图像的自动诊断极具挑战性。

Method: 利用nnU-Net架构在专家标注的肝癌数据集上训练语义分割模型，结合染色归一化技术，以分割输出中占比最大的像素类别作为整张图像的诊断结果。

Result: 在五折交叉验证中，该方法对肝细胞癌（HCC）、胆管细胞癌（CCA）和结直肠癌转移性腺癌（CMA）的均衡准确率分别达到0.975、0.950和1.000，表现优于基于图像块的多款深度学习模型。

Conclusion: 该框架通过结合染色归一化和语义分割，有效缓解了域偏移并降低了标注噪声敏感性，能有效辅助病理医生优化免疫组化检测选择，降低诊断成本。
...
### [219] [BVS: Bayesian Visual Search with Multimodal Large Language Model for Fine-grained Perception](https://arxiv.org/abs/2607.03184)
*Geng Li,Yuxin Peng*

Main category: cs.CV

TL;DR: 提出BVS（贝叶斯视觉搜索）框架，通过结合先验引导与后验修正，提升多模态大模型对超高分辨率图像中微小物体的细粒度感知能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在处理超高分辨率图像中的细粒度感知时面临两难：要么采用低效的无先验扫描，要么依赖缺乏后验修正的静态先验，难以应对杂乱场景中的微小物体。

Method: 提出BVS框架，将感知建模为连续空间-尺度流形上的全局优化问题。利用多模态大模型的提前终止注意力展开构建推理感知先验，并结合尺度感知非平稳核与GP-UCB算法进行动态噪声修正与信息恢复。

Result: 提供了亚线性遗憾界限（sub-linear regret bounds）的理论保证，实验表明BVS在精度和效率的权衡上显著超越了现有的先进基准方法。

Conclusion: BVS框架成功桥接了先验引导与后验修正，为多模态大模型在超高分辨率图像下的高效、精准细粒度感知提供了新方案。
...
### [220] [Rethinking Brain Decoding with CLIP: The Role of Adversarial Robustness](https://arxiv.org/abs/2607.03165)
*Byeongseo Bok,Futa Waseda,Jun Liu,Isao Echizen*

Main category: cs.CV

TL;DR: 本文研究了对抗鲁棒性表征对提升基于CLIP的fMRI脑解码性能的作用，发现使用鲁棒CLIP变体能显著提高解码准确率和表征对齐度。


<details>
  <summary>Details</summary>
Motivation: 标准的CLIP模型并非针对大脑对齐进行优化，其表征可能包含脑活动中未反映的非鲁棒特征，限制了解码效果。

Method: 在固定fMRI解码器的前提下，对比标准CLIP与对抗鲁棒CLIP作为目标表征在NSD和GOD数据集上的表现，并进行归因分析。

Result: 采用对抗鲁棒的CLIP变体一致性地提高了fMRI-图像检索和零样本分类的性能，并增强了表征对齐度，重构了特征重要性。

Conclusion: 目标表征的选择显著影响神经解码性能，对抗鲁棒性可作为脑解码模型选择的有效标准。
...
### [221] [Seeing Through WiFi: Lightweight Human Pose Estimation with Dynamic Kernel Attention](https://arxiv.org/abs/2607.03196)
...
TL;DR: 本文提出了一种基于WiFi CSI信号的轻量级高效人体姿态估计框架WiLHPE。


<details>
  <summary>Details</summary>
Motivation: 旨在解决WiFi人体姿态估计（HPE）在保护隐私的同时，如何在资源受限的设备上平衡模型性能与计算效率的问题。

Method: 利用动态卷积核学习与通道/频率空间注意力机制构建新型神经网络，并在训练中结合相机模型辅助；同时采用树状巴岑估计器（TPE）进行高效超参数优化。

Result: 在MM-Fi和WiPose数据集上分别实现了85.96%和94.27%的PCK50准确率，计算开销极小，且在AWGN噪声干扰下仍能保持约80%的PCK50。

Conclusion: WiLHPE在保持高效轻量化的同时，性能显著优于现有前沿方法，且对噪声环境具有较强的鲁棒性。
...
### [222] [Brand-as-Memory: Vision-Language Models Encode Causal, Mechanistically Localizable Credibility Priors for News Sources](https://arxiv.org/abs/2607.03365)
*Chih-Ting Liao,Xin Cao*

Main category: cs.CV

TL;DR: 论文揭示并分析了多模态大模型（VLM）在评估内容可信度时，过度依赖媒体来源标识（如Logo、域名）而非内容本身的“来源信誉偏见”。


<details>
  <summary>Details</summary>
Motivation: 随着VLM越来越多地通过图像阅读网页内容，研究者旨在探究模型是否对发布者身份存在先入为主的偏见，以及这种偏见如何影响其对事实的客观判断。

Method: 提出了诊断基准 CueTrust 及来源覆盖指数（SOI）；开展了跨模型与跨线索的基准测试；通过层级定位、稀疏自编码器分析及激活转向等方法对偏见的产生机制进行了因果剖析。

Result: 发现修改报头会显著改变模型评估的可信度（与专业媒体评级相关性达0.88）；该偏见定位在模型的第19-21层；通过激活转向成功将来源覆盖效应降低了41%并表现出良好的泛化性。

Conclusion: 现有的VLM在阅读时会优先考虑来源身份而非眼前的实际证据，这是一种可以通过定位和干预来缓解的系统性可靠性缺陷。
...
### [223] [Fast 3D Foundation Model Initialized Gaussian Splatting](https://arxiv.org/abs/2607.03209)
*Anurag Dalal,Daniel Hagen,Kjell G. Robbersmyr,Kristian Muri Knausgård*

Main category: cs.CV

TL;DR: 一种无需传统SfM的快速高质量3D高斯泼溅（3DGS）重建方法，训练时间缩短至约3分钟。


<details>
  <summary>Details</summary>
Motivation: 传统的3DGS依赖于耗时且计算量大的运动恢复结构（SfM）进行初始化，限制了其在快速或稀疏视角场景下的应用。

Method: 利用3D基础模型（3DFMs）进行相机姿态和点云初始化，采用深度引导损失联合优化姿态与高斯基元，并引入MLP姿态精细化模块。

Result: 在Mip-NeRF 360等数据集上实现了竞争力的重建质量（23.61 dB PSNR），同时将单场景训练时间缩短至约3分钟。

Conclusion: 该方法能够在极短时间内生成高质量的3DGS模型，适用于机器人、VR和自主导航等近实时应用。
...
### [224] [Latent Clarity: Bridging World-Model Kinematics to Semantic Manifolds for Video Anomaly Anticipation](https://arxiv.org/abs/2607.03558)
*Abu Anas Ibn Samad*

Main category: cs.CV

TL;DR: 提出PULS，一种通过将物理运动张量映射到文本对齐潜空间并利用前瞻性状态预测（ASP）来实现视频异常检测的连续语义世界模型。


<details>
  <summary>Details</summary>
Motivation: 当前的视频异常检测主要依赖反应性的多示例学习（MIL），这会将时空特征退化为标量分数，缺乏对未来语义状态的预测能力。

Method: 引入PULS管道，包含将V-JEPA 2物理张量映射至Qwen3-VL文本对齐超球面的KSD桥接模块，以及优化未来预测潜在表征的前瞻性状态预测器（ASP），并采用三轨提前期协议进行验证。

Result: 无MIL情况下，在UCF-Crime上实现0.8994 AUROC，零样本泛化至XD-Violence达0.8162 AUROC；ASP使零样本VQA准确率提升至44.5%，并在T-0.5s时带来+8.9 pp的前瞻性优势。

Conclusion: 验证了“潜在清晰度假设”，即预测的未来表征比当前观察更具语义可分性，且基于物理运动的表征具有优异的域外泛化能力。
...
### [225] [EPRA U-Net: An Efficient Pyramid Residual Attention Framework for Accurate Infarct Segmentation in Diffusion-Weighted MRI](https://arxiv.org/abs/2607.03568)
*Hasan Ulutas,Muhammet Emin Sahin,Mustafa Fatih Erkoc,Esra Yuce,Turker Tuncer,Sengul Dogan,Serkan Kiranyaz*

Main category: cs.CV

TL;DR: 本文提出了一种名为EPRA U-Net的新型深度学习架构，用于在DWI脑磁共振图像中精确分割急性缺血性脑梗死病灶。


<details>
  <summary>Details</summary>
Motivation: 在脑血管疾病管理中，准确识别和量化DWI图像上的急性缺血性脑梗死灶对于临床决策支持至关重要。

Method: 设计了EPRA U-Net，该模型结合了轻量化EfficientNet编码器、残差循环（R2）模块、空洞空间金字塔池化（ASPP）和双重注意力机制，并采用Tversky损失函数优化以提高病灶检测的灵敏度。

Result: 在包含4,895张DWI切片的数据集上，EPRA U-Net取得了0.8984的像素聚合Dice值；相较于UNet++、DeepLabV3+和TransUNet，其漏诊率分别降低了16%、25%和29%。

Conclusion: EPRA U-Net在急性脑梗死灶分割中表现优异，显著降低了病灶漏诊率，具有重要的临床应用价值。
...
### [226] [Learning to Suppress SPAD-based LiDAR Flare](https://arxiv.org/abs/2607.03247)
*Xuanya Zhu,Linghao Shen*

Main category: cs.CV

TL;DR: 本文提出了一种物理启发的语义分割方法PILF，用于抑制SPAD LiDAR中的耀斑干扰，提升了深度感知精度。


<details>
  <summary>Details</summary>
Motivation: SPAD LiDAR中的耀斑会导致深度估计错误和几何畸变。现有方法缺乏泛化性，且传统分割模型无法有效利用SPAD信号的多回波特征。

Method: 将耀斑抑制重构为语义分割问题，提出PILF方法，将第一、第二回波及环境光作为不同模态进行跨回波信息聚合，联合编码几何与光度特征。

Result: 实验表明，PILF在多场景下表现显著优于对比模型，mIoU达到了79.32%。

Conclusion: PILF为SPAD LiDAR耀斑抑制提供了一种有效且实用的数据驱动解决方案。
...
### [227] [PLGSA-Transformer: Periocular Landmark-Guided Attention with Occlusion-Adaptive Cosine Thresholding for Cross-Modal Masked and Unmasked Face Recognition](https://arxiv.org/abs/2607.03581)
*Dana A Abdullah*

Main category: cs.CV

TL;DR: 提出PLGSA-Transformer框架，利用眼周地标引导注意力、CNN-Transformer混合架构和自适应阈值解决口罩遮挡下的人脸识别问题。


<details>
  <summary>Details</summary>
Motivation: 口罩的普及导致传统人脸识别系统在面部遮挡时识别率下降，固定阈值和非自适应网络难以应对真实世界的多变场景。

Method: 结合眼周地标引导空间注意力（PLGSA）与EfficientNetB3特征，使用混合CNN-Transformer分支建模区域间依赖关系，并引入遮挡自适应余弦阈值（OACT）动态调整匹配标准。

Result: 在多源数据集上实现了97.22%的验证准确率和1.0000的ROC AUC，显著优于MUFM（95.0%）和HOG分类器（85.0%）等现有方法。

Conclusion: 研究证实，将眼周几何结构融入注意力机制并结合自适应阈值，是实现鲁棒且可扩展的跨模态遮挡人脸识别的有效途径。
...
### [228] [OmniLayout: A Schematic-Coupled Multimodal Benchmark for Constraint-Aware Geometric Reasoning in PCB Layout](https://arxiv.org/abs/2607.03261)
...
TL;DR: 本文推出了首个用于评估大语言模型（LLM）在PCB布局放置推理能力的基准测试OmniLayout，并发现现有LLM在应对真实世界电路设计约束时存在显著局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在空间推理上取得了进展，但在电子设计自动化（EDA）工作流中，面对严格的空间和功能约束进行密集元器件放置的能力仍未得到探索。

Method: 提出了包含1,681个工业级PCB布局的基准测试OmniLayout，设计了四项核心任务：IC物理放置的几何推理、可布线布局推理、电学功能保留以及工具增强的智能体推理。

Result: 实验结果显示，当前LLM在PCB布局任务中表现出较弱的几何推理能力、较差的可布线优化效果，以及在保留电学功能一致性上的不足。

Conclusion: 当前大语言模型在处理复杂的PCB布局和实体设计推理任务时仍有较大局限性，OmniLayout为此类研究提供了重要的评估平台。
...
### [229] [Responsibility Distribution Estimation in Ego-View Accident Videos with Multimodal Large Language Models](https://arxiv.org/abs/2607.03591)
*Ryosei Tamura,Andrew Shin*

Main category: cs.CV

TL;DR: 本文提出了一项新任务——基于驾驶员视角视频的交通事故责任比例预测，并构建了相应的标注管线与基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有的交通事故分析方法成本高且无法反映驾驶员的实际视线，而驾驶员视角视频更适合评估事故的可避免性和责任归属。

Method: 引入责任分布估计任务，构建了LLM辅助的责任标注管线，并在多模态输入（原始图像、分割增强、文本描述）下微调了多模态大模型。

Result: 实验建立了一个强有力的初始基准，证明了多模态大模型能够有效进行这种复杂的、受约束的责任推理任务。

Conclusion: 第一人称视角的事故视频为具有社会和法律意义的多模态推理研究奠定了可行基础。
...
### [230] [A Decomposable Probe for Few-Step Diffusion Models: Prompt, Latent, and Score Selectivity across Backbone Families and Distillation Paradigms](https://arxiv.org/abs/2607.03256)
*Patrick Mu Haojie*

Main category: cs.CV

TL;DR: 本文提出了一种可分解探针方法，用于分析少步蒸馏扩散模型内部各层对条件响应的影响，以替代传统的单一标量评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有的单标量指标（如FID/CLIP）无法深入揭示蒸馏扩散模型的行为变化具体是由架构、蒸馏目标还是扩散过程本身所导致的。

Method: 在模型的三个层（提示编码器、去噪器输入和输出）注入受控扰动，利用统一的Bures W2^2选择性比率估算器，对23个主流模型进行横向对比分析。

Result: 提示层反映了提示词的均值响应；潜在层可作为整流流（Rectified Flow）架构（如SD3.5、FLUX）的近乎二元检测器；分数层则能有效区分不同的蒸馏目标。

Conclusion: 该探针成功地将宏观指标分解为可归因的模型层与设计决策，为理解和诊断扩散模型及其蒸馏机制提供了精细化的工具。
...
### [231] [Token-Based Affordance Grounding with Large Vision-Language Models](https://arxiv.org/abs/2607.03595)
*Seung Il Lee,Qinqian Lei,Daguang Xu,Dong Yang,Robby T. Tan,Yixin Chen,Bo Wang*

Main category: cs.CV

TL;DR: 本文提出了TokAG，一个利用大视觉语言模型（LVLM）Token级语义-空间信号进行零样本示能定位（Affordance Grounding）的框架。


<details>
  <summary>Details</summary>
Motivation: 传统的弱监督方法难以应对视觉模糊和共存动作，且缺乏动作的丰富语义细节；而现有的LVLM虽包含丰富语义，却无法直接提供动作特定的空间定位。

Method: 提出了TokAG框架，通过引入空间感知Token选择机制，评估并选择在目标物体上具有主导激活的注意力图Token，将LVLM的隐式语义信号转化为示能热力图。

Result: TokAG在多个基准测试中均优于先前的弱监督方法，在AGD20K未见划分上的NSS指标提升了10.7%，在HICO-IIF上提升了29.7%。

Conclusion: 该研究表明，通过有效提取和选择LVLM内部的Token级注意力信号，无需外部监督即可实现高精度的零样本示能定位。
...
### [232] [Defending from GeoLocalization through Adversarial Road Trips](https://arxiv.org/abs/2607.03277)
*Niccolò Niccoli,Federico Becattini,Lorenzo Seidenari*

Main category: cs.CV

TL;DR: 本文提出了一种名为RoadTrip Attack (RTA)的图像地理定位靶向对抗攻击方法。


<details>
  <summary>Details</summary>
Motivation: 深度学习图像地理定位技术的快速发展引发了隐私泄露担忧，需要通过对抗攻击来保护用户隐私。

Method: 将对抗攻击过程建模为寻找通往特定目标位置的虚拟路径，利用束搜索算法迭代生成细微扰动以误导定位模型。

Result: 该方法在黑盒设置下表现优异，具有高可迁移性且生成的图像伪影极少。

Conclusion: RTA是一种高效、隐蔽且适用于黑盒场景的地理定位对抗攻击方案，能有效保护用户位置隐私。
...
### [233] [A Step Towards Robust Unsupervised Domain Adaptation via Fine-Tuning and Reinforcement Learning](https://arxiv.org/abs/2607.03600)
*Sushant Dagaji Desale,Rahul Mishra,Ashutosh Kumar Sinha*

Main category: cs.CV

TL;DR: 本文提出了一个结合监督微调与强化学习的两阶段框架（SFT+RL），以提升无监督域适应在对抗攻击下的鲁棒性与分类准确率。


<details>
  <summary>Details</summary>
Motivation: 在无监督域适应中，由于噪声伪标签以及源域和目标域之间的分布偏差，模型在面对对抗攻击时难以平衡鲁棒性与准确率。

Method: 提出SFT+RL两阶段方法：首先通过基于PGD的对抗微调线性分类器并微调CLIP投影层；随后引入置信度引导的伪标签策略，并在混合的干净与对抗样本上进行对抗训练。

Result: 在OfficeHome、PACS和VisDA等基准数据集上，该方法使干净准确率平均提升10.2%，对抗鲁棒性平均提升15.8%，优于现有主流方法。

Conclusion: 该研究表明，通过结合CLIP预训练先验与精心设计的两阶段微调及伪标签筛选策略，可以有效解决无监督域适应中的对抗防御难题。
...
### [234] [LBTCap: A Lightweight Bilateral Transformer for Real-Time Remote Sensing Image Change Captioning](https://arxiv.org/abs/2607.03320)
*Licheng Zhang,Siew-Kei Lam,Naveed Akhtar*

Main category: cs.CV

TL;DR: 本文提出了一种轻量级遥感图像变化描述框架LBTCap，通过双边Transformer在降低计算开销的同时保持了高精度的变化描述生成。


<details>
  <summary>Details</summary>
Motivation: 现有的遥感图像变化描述（RSICC）方法通常忽略了计算效率和推理速度，限制了其在实际实时部署中的应用。

Method: 设计了LBTCap框架，采用共享查询和键矩阵、级联值向量以及可学习双边权重的双边注意力机制，并结合截断骨干网络与分组查询注意力（GQA）来精简模型。

Result: 在两个公开数据集上的实验表明，LBTCap仅含有39.99M参数，在保持与主流SOTA方法相当的准确率的同时，显著提升了推理速度，且在低资源场景下优势明显。

Conclusion: LBTCap在准确率与效率之间实现了优异的平衡，为实际的遥感变化描述任务提供了一种极具实用价值的轻量级方案。
...
### [235] [RADIO1D: Elastic Representations for Condensed Vision Modeling](https://arxiv.org/abs/2607.03624)
...
TL;DR: 本文挑战了多模态大模型（VLM）需要固定2D图像块特征的假设，提出了RADIO1D方法，将图像压缩为可变长度的1D Token序列，以实现更高的计算效率与准确率。


<details>
  <summary>Details</summary>
Motivation: 研究发现视觉编码器在VLM训练过程中，其表征会变得越来越抽象且空间相干性降低，暗示了固定2D特征并非必要，从而激发了探索更高效图像表征的研究。

Method: 引入了RADIO1D，利用多教师知识蒸馏和自编码器设计，将图像压缩为紧凑、长度可变的一维Token序列。

Result: RADIO1D实现了强大的分层总结能力（甚至单Token场景理解），改善了图像检索性能，并在多个多模态基准测试中提供了灵活的精度与效率权衡，降低了计算开销。

Conclusion: 该研究表明，通过自适应的一维Token序列替代传统的2D块特征，可以在降低计算成本的同时，维持或提升视觉语言模型的性能。
...
### [236] [From General Actions to Domain-Specific Monitoring: Prior-Adaptive Transfer for Skeleton-Based Action Recognition](https://arxiv.org/abs/2607.03327)
*Hao Wang,Di Yang,Jiangtao Wang*

Main category: cs.CV

TL;DR: 本文提出了PATS框架，通过选择性地保留任务相关的运动先验并过滤冗余信息，实现了通用骨骼动作识别模型向特定领域任务（如医疗监控）的高效迁移。


<details>
  <summary>Details</summary>
Motivation: 将通用骨骼动作识别模型直接迁移到特定领域任务时效果不佳，因为这些特定任务通常仅涉及通用运动先验的一个子集，保留无关的预训练模式不仅会阻碍模型适应，还会增加计算成本。

Method: 引入了自适应先验迁移（APT）模块，通过迭代修剪和细化的模型压缩技术作为先验选择机制，只保留与目标任务相关的运动先验。

Result: 在阿尔茨海默病检测和摔倒检测两个特定动作识别任务上的实验表明，PATS在性能和计算效率上均一致优于现有的竞争基线。

Conclusion: PATS通过有效的自适应先验选择与压缩机制，成功解决了骨骼动作识别模型在特定领域迁移中的性能与效率瓶颈。
...
### [237] [GrowFields: Compositional 4D Neural Fields for Topology-Changing Plant Growth](https://arxiv.org/abs/2607.03330)
*Joaquin Gajardo,Michele Volpi,Marko Mihajlovic,Siyu Tang,Lukas Roth,Sergey Prokudin*

Main category: cs.CV

TL;DR: 提出 GrowFields，一种用于从点云时间序列中进行器官级 4D 植物生长建模的组合动态神经场表示方法。


<details>
  <summary>Details</summary>
Motivation: 植物生长伴随复杂的非刚性变形、拓扑结构变化（新器官产生）及异步生长，传统三维重建方法难以有效应对这些挑战。

Method: 将植物分解为不同器官并对齐至各自的规范坐标系，利用共享的连续神经变形场和器官隐变量，建模各个器官的生长动态。

Result: 在四种植物数据集上的实验表明，该方法在空间精度、时间连贯性和形态保真度方面均显著优于现有方法。

Conclusion: GrowFields 能够有效处理植物的异步发育，为实用的器官级植物追踪和生长分析提供了强有力的解决方案。
...
### [238] [Moonstone: A Multimodal Foundation Model and Benchmark for Lunar Remote Sensing](https://arxiv.org/abs/2607.03644)
*Ayush Prasad,Swarnalee Mazumder*

Main category: cs.CV

TL;DR: 本文推出了首个用于月球遥感的多模态基准模型 Moonstone，包含一个大规模全球多模态数据集和一种新型自编码器模型 MG-MAE。


<details>
  <summary>Details</summary>
Motivation: 现有的月球遥感数据零散分布在不同归档中，且缺乏用于评估月球数据机器学习算法的统一基准。

Method: 构建了28通道的全球月球数据集，并提出了模态分组掩码自编码器（MG-MAE），结合了分组卷积分词器、共享ViT以及针对缺失模态的注意力掩码，并设计了六个下游任务的评估基准。

Result: MG-MAE 预训练特征在所有下游任务（分类、回归、分割）中均优于从头训练的基线，且显著超越了 ImageNet 预训练和普通 MAE 基线。

Conclusion: Moonstone 为月球遥感领域提供了首个多模态基模型与基准，展示了自监督预训练在处理异构月球遥感数据中的巨大潜力。
...
### [239] [Beyond Post-Quantization: Native Hash Learning with a Dedicated HASH Token](https://arxiv.org/abs/2607.03328)
*Xinze Liu,Ding Wang,Hengjie Zhu,Dayan Wu*

Main category: cs.CV

TL;DR: 提出HashViT，通过在Transformer内部引入原生HASH Token，解决了传统深度哈希中特征与编码不一致的问题，实现了高效的图像检索。


<details>
  <summary>Details</summary>
Motivation: 传统的深度哈希方法多采用后量化范式，导致连续优化的特征空间与离散的汉明空间之间存在特征与编码的不一致性。

Method: 引入包含哈希寄存器和语义工作空间的HASH Token，并设计哈希细化适配器进行层间交互，结合多任务统一目标进行优化。

Result: 在三个基准数据集上均取得了最先进或极具竞争力的检索性能，同时保留了紧凑汉明码的检索效率。

Conclusion: 原生哈希Token学习有效消除了特征与编码间的偏差，证明了在Transformer骨干网络中直接演化哈希表示的可行性与优越性。
...
### [240] [ClinOCR-Bench: A Comprehensive Clinical Scanned Document Dataset for Optical Character Recognition Model Evaluation](https://arxiv.org/abs/2607.03650)
*Enshuo Hsu,Jin Zhou,Kirk Roberts*

Main category: cs.CV

TL;DR: 本文介绍并开源了ClinOCR-Bench，这是一个包含真实扫描伪影的临床OCR基准数据集，用于评估和提升视觉语言模型在医学文档解析中的性能。


<details>
  <summary>Details</summary>
Motivation: 目前的临床OCR研究大多依赖私有数据，缺乏公开的基准数据集，且现有数据集无法反映影响OCR性能的常见扫描伪影，导致无法进行系统性评估。

Method: 构建了包含6个子集（如手写、旋转、表格等）、共384张扫描图像的ClinOCR-Bench数据集，并使用多种主流的开源和商业视觉语言模型进行基准性能测试。

Result: 成功创建了一个无隐私泄露风险、具备多样化布局和常见扫描伪影的临床基准数据集，并提供了主流视觉语言模型的基准测试结果。

Conclusion: ClinOCR-Bench填补了临床OCR公开基准的空白，为评估和优化医疗文档数字化技术提供了重要的评估平台。
...
### [241] [ViPo-MLLM: Visual-Pose Multimodal LLM for Gloss-Free Sign Language Translation](https://arxiv.org/abs/2607.03657)
*Ahmed Abul Hasanaath,Bicheng Xu,Mir Rayat Imtiaz Hossain,Leonid Sigal,Hamzah Luqman*

Main category: cs.CV

TL;DR: 提出了一种集成RGB和人体姿态特征的无词标辅助手语翻译（SLT）框架ViPo-MLLM。


<details>
  <summary>Details</summary>
Motivation: 现有的无词标辅助手语翻译方法多使用单模态或弱融合特征，限制了其在处理手势、身体和面部细粒度线索时的性能。

Method: 提出ViPo-MLLM框架，融合时空RGB和人体姿态特征，利用专用编码器和跨模态注意力机制捕获依赖关系，并通过结构化提示词和双重目标（对比学习与语言建模）训练大语言模型。

Result: 在PHOENIX14T和CSL-Daily数据集上均取得了新的最先进（SOTA）结果，且与有词标辅助的方法相比具有极具竞争力的性能。

Conclusion: 该研究证实了引入姿态线索和跨模态注意力机制在提升无词标辅助手语翻译性能方面的有效性。
...
### [242] [Pathways of Visual Information Flow in Vision-Language Models](https://arxiv.org/abs/2607.03358)
*Israfel Salazar,Stella Frank,Dan Oneata,Desmond Elliott,Constanza Fierro*

Main category: cs.CV

TL;DR: 本文研究并揭示了视觉语言模型（VLM）中视觉信息传输的双路径机制（直接路径与文本介导路径）及其在干预下的动态灵活性。


<details>
  <summary>Details</summary>
Motivation: 探索视觉语言模型内部如何路由和处理视觉信息，以解决图像相关的查询任务。

Method: 在合成和真实数据集上采用了因果修补（Causal Patching）、注意力消融（Attention Knockouts）和受损输入修补等分析方法。

Result: 发现了“直接路径”和“文本介导路径”两条信息通道，其选择具有任务依赖性且受数据和提示词影响；同时发现这些路径具有弹性，当常用路径被消融时，文本介导路径可作为后备通道。

Conclusion: 该研究提供了VLM视觉信息流的机制性刻画，展示了其内部机制的灵活性，并指出消融实验揭示的往往是模型的潜在能力而非默认行为。
...
### [243] [Attending to Multimodal Generation One Token at a Time](https://arxiv.org/abs/2607.03738)
*Varun Gupta,Vineet Gandhi,Makarand Tapaswi*

Main category: cs.CV

TL;DR: 本文分析了多模态大模型在生成过程中的逐标记注意力动态，并提出了一种提升其性能的测试时干预方法。


<details>
  <summary>Details</summary>
Motivation: 以往关于多模态大模型可解释性的研究多关注空间维度（层与电路），而忽略了时间维度（生成过程中注意力随时间的变化）。

Method: 引入OTaT方法追踪注意力转移，设计了多模态上下文切换任务，实施了因果注意力阻断实验，并开发了测试时注意力增强干预。

Result: 揭示了注意力在图像、指令和已生成文本间的动态规律，验证了其因果功能，并通过测试时干预显著提高了多模态任务的性能。

Conclusion: 掌握多模态大模型的token级注意力动态有助于解释其决策机制，并能通过简单的干预手段直接优化其多模态生成表现。
...
### [244] [Present but Not Remembered: Auditing How Frozen VLAs Encode, Deploy, and Steer Visual History](https://arxiv.org/abs/2607.03372)
*Chih-Ting Liao,Xin Cao*

Main category: cs.CV

TL;DR: 本文研究了视觉-语言-动作模型（VLA）如何表示和利用历史信息，发现其存储的历史多为当前的冗余副本，并揭示了不同架构在调用历史时的策略差异。


<details>
  <summary>Details</summary>
Motivation: 以往研究主要关注如何为VLA增加记忆，却较少探究现有模型本身是如何表征和使用历史观测信息的。

Method: 采用层分辨率线性探测和因果交换干预，对两类架构的三个VLA模型进行分析，并引入了一种无需训练的时间部署审计方法。

Result: 研究发现历史帧内容在整个网络中均可线性解码，但其内容几乎只是当前帧的冗余复制；只有在当前帧严重退化时模型才会依赖历史，且不同架构的依赖和引导（steerability）策略有所不同。

Conclusion: VLA并未遗忘过去，而是未能将历史表示为独立于当前的信息。未来的记忆增强研究应致力于注入过去特有的独特信息，而非单纯增加历史长度。
...
### [245] [EmCom-Diffusion: Probing Visual Reflection in Emergent Languages via Image Generation](https://arxiv.org/abs/2607.03752)
*Haruumi Omoto,Tadahiro Taniguchi*

Main category: cs.CV

TL;DR: 本文提出EmCom-Diffusion框架，通过图像重建来直接评估涌现语言（emergent languages）对视觉内容的保留程度（即视觉反射）。


<details>
  <summary>Details</summary>
Motivation: 现有衡量涌现语言视觉反射的指标均为间接测量（如人类定义概念或图文匹配），容易漏掉视觉内容或误判不相关内容。

Method: 在“图像-涌现消息”对上微调预训练的文本生成图像扩散模型，重构输入图像，并通过计算重构图与原图的感知相似度来进行直接且生成式的评估。

Result: 在MS-COCO数据集上通过指代游戏（Referential Game）进行了验证，对比四种现有指标和基线，该方法成功捕捉到了其他指标遗漏的视觉内容。

Conclusion: EmCom-Diffusion为评估涌现语言的视觉反射提供了一种更直接、有效的生成式评估新范式。
...
### [246] [TemporalGS: Training-Free Plug-and-Play Acceleration for 3D Gaussian Splatting Rendering via Temporal Priors](https://arxiv.org/abs/2607.03390)
*Yuhongze Zhou,Zihao Yang,Xinxin Zuo,Juwei Lu*

Main category: cs.CV

TL;DR: TemporalGS是一种无需训练、即插即用的3DGS渲染加速方法，通过利用时间先验实现高达1.48倍的渲染提速。


<details>
  <summary>Details</summary>
Motivation: 尽管3DGS渲染保真度高，但在处理包含大量高斯椭球的场景时，实现高帧率和低延迟的渲染仍然面临挑战。

Method: 利用几何与外观缓冲区等时间先验，减少相邻帧在预处理、排序和光栅化阶段的冗余。主要包含两种策略：(1) 时间动态剔除（过滤贡献较小的高斯体）；(2) 选择性渲染（仅渲染无法被时间先验近似的少量图块）。

Result: 在保持竞争性渲染质量的前提下，TemporalGS使多种3DGS方法的渲染速度提升高达1.48倍，表现优于或相当于现有的免训练/后处理加速方法，并成功推广至硬件光栅化3DGS。

Conclusion: TemporalGS为3DGS渲染加速提供了一种简单、高效且具有良好移植性的免训练即插即用解决方案。
...
### [247] [Rethinking Depth Pruning for Vision Transformers: A Heterogeneity-Aware Perspective](https://arxiv.org/abs/2607.03784)
*Zhenfeng Su,Kang Zhao,Han Bao,Tao Yuan,Zhongzhe Hu,Xianzhi Yu,Wenxuan Wang*

Main category: cs.CV

TL;DR: 本文提出了HetDPT，一种解决层间异构性并避免维度不匹配的视觉Transformer（ViT）深度剪枝方法，显著提升了运行速度且几乎无精度损失。


<details>
  <summary>Details</summary>
Motivation: 现有的ViT深度剪枝方法往往忽略了不同层之间的异构性，导致剪枝后精度难以恢复且存在维度不匹配问题，限制了联合宽度与深度剪枝的加速效果。

Method: 提出了一种异构性感知的深度剪枝方法（HetDPT），并可与宽度剪枝相结合（HetDPT+），在避免维度不匹配的同时实现高效剪枝。

Result: HetDPT使DeiT-B和DeiT-S分别实现1.58倍和1.39倍的加速且几乎无精度损失；结合宽度剪枝后（HetDPT+），在特定配置下将加速比从4.24倍提升至5.19倍，创下新的SOTA。

Conclusion: HetDPT通过解决层间异构性问题，成功克服了ViT深度剪枝的精度恢复难题，为ViT的高效加速提供了新方案。
...
### [248] [Self-Improving Diffusion Classifiers with Minority Preference Optimization](https://arxiv.org/abs/2607.03770)
*Hyunsoo Kim,Jungmyung Wi,Soobin Um,Donghyun Kim,Suhyun Kim*

Main category: cs.CV

TL;DR: 本文提出MiPO方法，通过少数样本偏好优化提升扩散分类器在低密度（少数派）数据区域的零样本分类能力。


<details>
  <summary>Details</summary>
Motivation: 扩散分类器在多数高密度区域表现优异，但在少数低密度区域准确率较低，且少数样本生成采样与分类器感知能力之间的关系尚未被充分研究。

Method: 提出MiPO方法，无需额外图像数据或外部模型，仅利用文本数据生成样本，并通过少数样本偏好奖励、LoRA和群组相对策略优化（GRPO）微调预训练扩散模型。

Result: 在五个标准数据集上的实验表明，MiPO实现了稳定、自适应的少数样本采样，并成功将低密度生成覆盖转化为更强的零样本扩散分类性能。

Conclusion: 扩散分类器的感知偏见可以通过少数样本偏好优化来缓解，从而有效提升其在边缘和少数数据区域的识别能力。
...
### [249] [Handwriting Trajectory Recovery with Diffusion Models](https://arxiv.org/abs/2607.03422)
*Hiroki Nagamatsu,Shoji Toyota,Seiichi Uchida*

Main category: cs.CV

TL;DR: 提出首个基于扩散模型的脱机手写图像笔画轨迹恢复框架，实现了高精度和强泛化能力的在线轨迹重建。


<details>
  <summary>Details</summary>
Motivation: 脱机手写图像到在线笔划轨迹的恢复（笔画恢复）在笔画级编辑和司法鉴定中具有重要应用，现有方法在复杂字符的形状保真度和时间相似度上面临挑战。

Method: 将轨迹恢复公式化为图像条件生成任务，利用去噪扩散模型采样出与观测到的墨迹相一致的笔画轨迹。

Result: 在CASIA-OLHWDB数据集上的评估表明，该方法在时间相似度（DTW/LDTW）和形状保真度（AIoU）上显著优于PEN-Net和Cross-VAE，并且展示了出色的跨语系泛化能力（如将中文训练的模型应用于拉丁字母）。

Conclusion: 扩散模型非常适合用于手写轨迹恢复，不仅能准确重建复杂的多笔画字符，还能捕获通用的笔画顺序规律并具备强大的泛化能力。
...
### [250] [WorldBagel: Uncovering the Power of Unified Multimodal Models for Vision-Language-Action-World Modeling](https://arxiv.org/abs/2607.03461)
*Zelin Zhao,Min Shi,Bo Yuan,Haotian Xue,Jialuo Li,Lama Moukheiber,Humphrey Shi,Yongxin Chen*

Main category: cs.CV

TL;DR: 本文提出 WorldBagel 框架，系统研究了统一多模态模型在视觉-语言-动作-世界（VLAW）建模中的关键作用与优势。


<details>
  <summary>Details</summary>
Motivation: 尽管统一视觉-语言模型展示了强大的多模态生成能力，但其作为世界模型的潜力仍未得到充分探索。

Method: 引入基于 BAGEL 架构的统一 VLAW 框架 WorldBagel，并在多任务机器人操纵及跨领域任务中进行评估。

Result: WorldBagel 在 LIBERO、Language Table 和 Franka 等实验中一致优于特定任务的替代方案，并学习到更具结构性和语义对齐的动作表示。

Conclusion: 统一性不仅是架构上的便利，更是构建高效 VLAW 模型的关键因素，能带来持续的实证提升。
...
### [251] [CineMobile: On-Device Image-to-Video Diffusion for Cinematic Camera Motion Generation](https://arxiv.org/abs/2607.03803)
*Xuyao Huang,Zelai Deng,Xu Wang,Xizhong Xiao,Zhijie Deng*

Main category: cs.CV

TL;DR: CineMobile是一个专为移动端设计的图像到视频生成模型，通过剪枝、蒸馏和量化实现了40倍的加速，支持在移动设备上高效生成高质量视频。


<details>
  <summary>Details</summary>
Motivation: 移动设备对电影级视频生成需求增加，但现有的Diffusion Transformer (DiT) 模型参数量大且迭代步数多，难以在移动端高效运行。

Method: 提出三步优化策略：基于蒸馏引导的剪枝压缩模型、结合扩散蒸馏与强化学习实现4步快速生成、以及通过混合后训练量化将模型体积压缩至1GB以下。

Result: 相比Wan 2.1教师模型实现了40倍加速并保持同等画质；在联发科天玑8400平台上生成49帧480p视频仅需20秒，峰值内存仅1.8 GB。

Conclusion: CineMobile成功克服了DiT模型在移动端的计算和内存瓶颈，证明了在移动设备上进行实用图像到视频创作的可行性。
...
### [252] [PhysMirror: Physics-Aware Mirror Object Generation](https://arxiv.org/abs/2607.03470)
...
TL;DR: 本文提出了PhysMirror框架，通过引入3D空间先验，解决了文本到图像扩散模型在生成物理一致的镜面反射图像时面临的几何失真问题。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型难以处理严苛的物理几何约束，导致生成的镜面反射存在幻觉，限制了其作为具身智能和机器人感知合成训练数据的实用性。

Method: 将输入物体转化为3D网格并在三维模拟环境中构建精确的镜面场景，以此提取深度图和分割图等2D引导信号辅助扩散模型生成，并引入了无参考的自动评估指标MCS。

Result: 在MirrOB数据集上的实验表明，该方法在反射准确性和物理真实性上显著优于现有基线，同时保持了良好的文本与图像语义对齐。

Conclusion: PhysMirror为具身智能的数据生成提供了一条可靠且符合物理规律的镜面反射图像生成管线。
...
### [253] [Q-TriM: Question-Guided Tri-Modal Attention for Audio-Visual Question Answering](https://arxiv.org/abs/2607.03825)
*SungHun Kim,SeungJun Baek*

Main category: cs.CV

TL;DR: 提出Q-TriM，一种用于视听问答（AVQA）的浅层并行三模态融合框架，通过避免深层顺序注意力堆叠来减少误差累积并取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统的AVQA系统依赖深层且顺序堆叠的多模态注意力层，这容易导致微小的跨模态信息丢失以及误差在层间累积。

Method: 提出Q-TriM框架，以文本为条件，采用并行且浅层的方式融合视频和音频。引入三模态注意力机制（Query、Key、Value来自不同模态），并在单阶段并行结合这些注意力表示。

Result: 在三个AVQA基准测试上取得了最先进的（SOTA）性能，尤其是在MUSIC-AVQA-R上表现出显著提升。

Conclusion: Q-TriM通过并行融合有效缓解了深度多模态融合中的误差累积问题，展示了强大的鲁棒性和分布外泛化能力。
...
### [254] [Learning to Generate Multiple Objects from Dense and Occluded Layouts](https://arxiv.org/abs/2607.03488)
*Bach-Hoang Ngo,Si-Tri Ngo,Hieu Le,Trung-Nghia Le*

Main category: cs.CV

TL;DR: 针对文本生成图像模型在密集场景中物体计数不准的问题，本文提出了布局感知注意力偏差与无外形平衡损失，并引入了全新评估基准。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成密集且重叠的物体时容易出现“实例归属崩溃”，即重叠物体间词token过度交互以及被遮挡物体监督信号微弱，导致无法准确计数。

Method: 引入了布局感知注意力偏差以限制跨实例的信息泄露，并采用无外形平衡损失来根据遮挡程度放大梯度；同时推出了包含 amodal 监督的 OverlapDepth-45K 基准数据集。

Result: 实验表明，该方法在保持高图像质量的前提下，显著提升了密集场景下的物体计数准确率，并有效防止了实例重叠合并。

Conclusion: 结合布局偏置和遮挡感知损失可以有效克服扩散模型在密集重叠场景中的计数失效问题。
...
### [255] [How Do Diffusion Classifiers Decide? A Bias-Centric Evaluation](https://arxiv.org/abs/2607.03831)
*Saba Fathi,Fardin Ayar,Maryam Abdolali,Ehsan Javanmardi,Manabu Tsukada,Mahdi Javanmardi*

Main category: cs.CV

TL;DR: 引入ASOB-Bench评估扩散分类器在属性绑定、尺寸顺序和背景依赖三个维度的偏见，揭示了其独特的决策机制和偏见特征。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散分类器应用广泛，但其如何做出分类决策的机制尚不清楚，需要系统地评估其偏见以理解其工作原理。

Method: 提出ASOB-Bench基准，利用重建误差热图和U-Net交叉注意力可视化，从属性绑定、尺寸顺序及背景依赖三个维度诊断分类偏见。

Result: 扩散分类器较OpenCLIP baseline更不易发生属性误绑定，但更容易受到尺寸顺序捷径的影响，且在ImageNet-B上对背景信息表现出更强的依赖性。

Conclusion: 扩散分类器展现出与视觉语言模型截然不同的偏见特征，为构建更鲁棒的扩散模型提供了指导。
...
### [256] [Towards Standardized Light Field Quality Assessment: Hybrid Subjective Benchmarking and Objective Metric Evaluation](https://arxiv.org/abs/2607.03494)
...
TL;DR: 本文介绍了一种用于光场图像质量评估的标准化工作流，结合了基准生成、混合主观评估和客观指标分析。


<details>
  <summary>Details</summary>
Motivation: 旨在解决沉浸式媒体编码中缺乏可靠、可重复的主观质量评估程序，以及现有客观指标在处理视角合成等新型失真时准确性不足的问题。

Method: 提出了一个整合了包含合成失真的基准生成、结合参考锚定与配对比较的混合主观评估，以及对多种客观指标进行系统性评估的标准化框架。

Result: 实验显示，现有客观指标对仅编码失真的评估一致性较高，但在加入视角合成失真后性能明显下降，并强调了视角池化策略对未来指标设计的重要性。

Conclusion: 该工作为光场质量评估提供了一个可复制且达到标准化要求的框架，并揭示了当前客观指标在新型编码流程中的主要局限性。
...
### [257] [Perceptual Flow Matching for Few-Step Generative Modeling](https://arxiv.org/abs/2607.03524)
*Chuyang Zhao,Yifei Song,Hongfa Wang,Jianlong Yuan,Yuan Zhang,Siming Fu,Zhineng Chen,Huilin Deng,Haoyang Huang,Nan Duan*

Main category: cs.CV

TL;DR: 本文提出了感知流匹配（PFM）框架，通过在预训练感知特征空间中监督流匹配，实现了高效的少步图像与视频生成。


<details>
  <summary>Details</summary>
Motivation: 传统的流匹配模型生成步骤较多，且现有的加速和蒸馏方法通常需要教师模型或辅助网络，增加了复杂度。

Method: 引入预训练感知模型，在感知特征空间中监督流匹配，而非在传统的VAE隐空间中进行速度回归。

Result: 成功将生成步数从35-50步缩减至4-8步，在保持生成质量的同时显著减少了伪影，且无需教师模型。

Conclusion: 研究表明，在合适的表征空间中进行监督，标准的流匹配训练能够自然地转化为高质量的少步生成器。
...
### [258] [Flex-Forcing: Towards a Unified Autoregressive and Bidirectional Video Diffusion Model](https://arxiv.org/abs/2607.03509)
*Xinyin Ma,Julius Berner,Chao Liu,Arash Vahdat,Weili Nie,Xinchao Wang*

Main category: cs.CV

TL;DR: Flex-Forcing是一个统一的视频扩散模型框架，通过灵活的分块机制结合了双向和自回归生成的优势，实现了高效且具备长期一致性的视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有的视频生成受限于刚性的推理范式：双向扩散模型全局一致性好但推理慢，而自回归模型效率高但存在长期一致性差和曝光偏差的问题。

Method: 提出了Flex-Forcing框架，其核心是联合定义在时间轴和去噪步骤上的灵活分块机制，使模型能够同时支持双向与自回归生成，并允许任意顺序和时间步的生成。

Result: 实验表明，Flex-Forcing在多个基准测试上均优于传统刚性推理模型，不仅提升了视频质量和长视频稳定性，还加快了推理速度。

Conclusion: Flex-Forcing成功克服了视频生成中推理速度与全局一致性之间的矛盾，为灵活高效的视频扩散生成提供了有效方案。
...
### [259] [When Simpler Is Better: Evaluating Translation Pipelines for Medieval Latin Manuscripts](https://arxiv.org/abs/2607.03836)
*Nguyen Kim Hai Bui,Md. Easin Arafat,Tamás Gábor Orosz,Mufti Mahmud*

Main category: cs.CV

TL;DR: 本文评估了视觉语言模型在中世纪拉丁语手稿翻译中的表现，并提出了一种简单高效的“专用OCR+大模型”流水线。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在处理历史手稿时面临缩写、连画和纸张降解等噪声挑战，且缺乏端到端的图像到翻译评估基准。

Method: 提出了系统性评估框架，引入了包含1383行对齐图像与翻译的IPC数据集，并对比了专用OCR、通用大模型以及结合RAG和纠错的多组件流水线。

Result: 专用OCR的字符错误率比通用大模型低4.3倍；“专用OCR + VLM”的最简流水线效果最好，而加入RAG或后纠错反而因误差传播导致性能下降。

Conclusion: 在低资源历史文献翻译中，简单的“专用OCR+大模型”架构优于复杂的流水线，本文为该领域提供了新基准和实用部署指南。
...
### [260] [GeoSelect: Spatial-Program Execution for Training-Free Referring Remote Sensing Image Segmentation](https://arxiv.org/abs/2607.03869)
*Yuhang Jiang,Guohui Deng,Miaozhong Xu,Chao Ruan,Jinling Zhao,Linsheng Huang*

Main category: cs.CV

TL;DR: 提出GeoSelect，一种无需训练的遥感图像指代分割方法，通过将文本转换为可执行的空间程序来实现精准的目标定位。


<details>
  <summary>Details</summary>
Motivation: 现有的免训练方法主要依赖隐式的图文相似度，缺乏对遥感图像中至关重要的空间、对比和顺序关系（如“最大”或“左边第二个”）的控制能力。

Method: 利用冻结的纯文本大模型将自然语言查询解析为特定领域的空间程序，并通过确定性执行器结合连续几何场与离散集合/排序算子进行显式执行，同时引入可靠性降级机制。

Result: 在RRSIS-D和RISBench测试集上分别达到58.86 mIoU和55.27 mIoU，比之前最好的免训练方法性能提升了一倍以上，且无需指代监督，仅需单GPU运行。

Conclusion: 显式空间程序执行（而非骨干网络）是提升性能的关键，该方法实现了高可解释性、鲁棒性以及显著的零样本分割性能提升。
...
### [261] [iVISION-2DCD: A Long-Term Change Detection Dataset for Large-Scale Outdoor Construction Monitoring](https://arxiv.org/abs/2607.03553)
*Dayou Mao,Yuchen Lin,Ashkan Ebadi,John Zelek,Alexander Wong,Yuhao Chen*

Main category: cs.CV

TL;DR: 本文推出了 iVISION-2DCD，一个用于施工现场视点鲁棒性二维变化检测（2DCD）的大型合成基准数据集。


<details>
  <summary>Details</summary>
Motivation: 施工进度监测需要支持任意相机视角的变化检测，但真实场景下的无人机数据采集受安全和航迹限制，领域内缺乏开源的基准数据集。

Method: 基于密集的 LiDAR 点云合成高保真图像，利用新视角合成技术解决双时态对齐与视角多样性难题，并结合半自动语义分割生成精确的变化标签。

Result: 成功构建了包含逼真图像和准确标注的 iVISION-2DCD 数据集，并通过最先进的 2DCD 算法基准评估展示了其带来的新研究挑战。

Conclusion: 该数据集填补了施工自动化与 2D 变化检测交叉领域的空白，为计算机视觉和机器人领域提供了强有力的研究基准。
...
### [262] [Full Glyph Images Beat Token Embeddings: A Controlled Study for Transformers](https://arxiv.org/abs/2607.03994)
*Shuyang Xiang,Hao Guan*

Main category: cs.CV

TL;DR: 本文提出用字符序列的光栅化图像代替传统离散Token嵌入来训练中文语言模型，实现了显著的性能提升和更快的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 挑战现代语言模型中默认使用离散Token嵌入的传统假设，探索视觉输入（图像）在中文语言建模中的潜力和优势。

Method: 将字符序列转化为单张光栅化图像，通过ResNet和浅层ViT进行编码；采用双分支对照框架，使视觉模型与Token基线共享完全相同的解码器和训练配置，以隔离输入模态的影响。

Result: 视觉模型在所有测试中均优于基线（准确率提升21%），收敛速度提高约一倍，且对字符损坏具有鲁棒性；消融实验表明该优势依赖于空间连贯的输入，且在中文上效果显著，但在英文上不明显。

Conclusion: Transformer对输入模态具有更强的适应性，离散Token化并非中文语言建模的必需条件，中文独特的视觉和部首结构是实现该优势的关键因素。
...
### [263] [Enhancing Implicit Neural Representations with Image Feature Embedding for Unsupervised Cardiac Cine MRI Reconstruction](https://arxiv.org/abs/2607.04069)
*Donghang Lyu,Marius Staring,Yiming Dong,Keupp Jochen,Hildo J. Lamb,Mariya Doneva*

Main category: cs.CV

TL;DR: 本文提出了一种名为I-FP-INR的图像域双分支隐式神经表示（INR）框架，用于提升无监督心脏电影磁共振成像（MRI）的重建质量。


<details>
  <summary>Details</summary>
Motivation: 传统的监督学习重建方法高度依赖难以获取的完全采样参考数据，而现有的无监督INR方法在特征表示能力上仍有优化空间。

Method: 设计了双分支INR架构，通过引入额外的特征处理分支来提取互补的特征嵌入，从而增强图像重建的表示能力。

Result: 在公开和内部数据集上的实验表明，该方法在重建质量上显著优于基线方法，并在多种场景下展现出强大的鲁棒性。

Conclusion: I-FP-INR框架成功提升了无监督心脏电影MRI重建的性能，为无需完全采样数据的医学图像重建提供了有效的解决方案。
...
### [264] [XPlainVerse: A Million-Scale Benchmark for Explainable Deepfake Detection](https://arxiv.org/abs/2607.03562)
*Abhijeet Narang,Kartik Kuckreja,Shreya Ghosh,Muhammad Haris Khan,Jianfei Cai,Abhinav Dhall*

Main category: cs.CV

TL;DR: 本文引入了XPlainVerse，一个用于联合深度伪造检测和以人为本解释的大规模基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有的深度伪造检测模型解释缺乏视觉证据支撑，且评估指标仅关注分类准确率，忽视了模型解释与实际篡改的一致性。

Method: 构建了包含百万图像的XPlainVerse基准，设计了Edit-Check多阶段过滤管道，提供双重解释风格，并提出EntityScore和EvidenceScore两个新评估指标。

Result: 成功构建了高质量的数据集，通过2000对人工标注验证了数据质量，并能有效评估解释的保真度。

Conclusion: XPlainVerse将解释质量确立为深度伪造检测的一个可衡量维度，有助于推动可信、可解释深度伪造检测系统的研究。
...
### [265] [Seeing Once is Enough? Online Geometry-Aware Token Pruning for 3D Question Answering](https://arxiv.org/abs/2607.04079)
*Ruei-Chi Lai,Bolivar Solarte,Chin-Hsuan Wu,Yi-Hsuan Tsai,Min Sun*

Main category: cs.CV

TL;DR: 本文提出了一种针对3D问答任务的在线Token剪枝方法，可在不需额外训练的情况下，降低多模态大模型的计算开销和内存占用。


<details>
  <summary>Details</summary>
Motivation: 将多模态大模型扩展到3D问答需要处理多视角图像，带来高昂的计算成本，而现有剪枝方法多为离线预处理，缺乏高效的在线处理方案。

Method: 利用深度和相机位姿将输入帧投影到共享体素空间，识别帧间空间重叠，并在进入语言模型前在线剪枝冗余的图像Token。

Result: 减少了高达50%的Token使用量，在ScanQA、SQA3D和OpenEQA-HM3D基准测试上提升了Qwen2.5-VL-7B和Qwen3-VL-8B的性能。

Conclusion: 该方法实现了一种无需训练、低内存占用的高效在线3D问答推理流程，有效平衡了计算效率与模型性能。
...
### [266] [Vision Non-Causal Trapezoidal Mamba: Eliminating Directional Scanning in Vision SSMs with Second-Order Dynamics](https://arxiv.org/abs/2607.03589)
*Anvitha Ramachandran,Dhruv Parikh,Haoyang Fan,Rajgopal Kannan,Viktor Prasanna*

Main category: cs.CV

TL;DR: 提出VNCT，一种二阶非因果视觉状态空间模型，通过消除方向性标记扫描来减少空间偏差并提升视觉任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉状态空间模型多采用源自因果序列建模的方向性扫描，这引入了空间偏差并对图像方向变化过于敏感。

Method: 设计了视觉非因果梯形Mamba（VNCT），作为一种二阶非因果视觉SSM，它允许所有图像Token在单次前向传播中进行交互，消除了方向性扫描。

Result: VNCT表现出更强的方向鲁棒性，边界IoU提升达3.7个百分点，并在ImageNet-1K分类、COCO检测分割和ADE20K语义分割上全面超越了传统的方向性扫描和一阶非因果SSM。

Conclusion: 方向性扫描对于构建高性能视觉SSM并非必需，二阶非因果状态空间建模是一种更简单、高效且鲁棒的视觉识别替代方案。
...
### [267] [IDEAL-Bench: Indoor Dataset and Evaluation suite for Analyzing 3D Layout reasoning](https://arxiv.org/abs/2607.03614)
*Yuening Cai,Junwei Zhou,Youran Qu,Yu-Wing Tai*

Main category: cs.CV

TL;DR: 本文提出了IDEAL-Bench，一个用于评估多模态大模型（VLM）整体三维布局推断能力的诊断性基准。


<details>
  <summary>Details</summary>
Motivation: 现有的空间问答（QA）评估方法忽略了对VLM在三维空间布局推断（预测物体姿态和范围）能力的评估。

Method: 开发了包含1000个Blender生成场景的IDEAL-Scenes数据集，并建立IDEAL-Bench，通过五维数值指标和渲染对比协议评估15个主流VLM。

Result: 实验表明，当前最强模型仅获得62.1分，且模型普遍存在“重识别、轻测量”的几何回归缺陷，其排名与传统QA基准存在差异。

Conclusion: IDEAL-Bench能有效诊断出QA评估无法发现的几何与结构能力缺陷，为评估未来VLM的真实空间智能奠定了基础。
...
### [268] [SOV-CAD: Stepwise Orthographic Views Guided CAD Modeling Sequence Reconstruction](https://arxiv.org/abs/2607.04119)
*Zhaopeng Feng,Chen Zhi,Xuhong Zhang,Zhengwen Feng,Xinkui Zhao*

Main category: cs.CV

TL;DR: 提出SOV-CAD框架，通过离线强化学习和逐步视觉反馈，实现更符合人类设计流程的CAD模型序列重建。


<details>
  <summary>Details</summary>
Motivation: 现有的CAD重建方法通常是整体生成序列，忽略了人类设计中迭代和反馈驱动的特性。

Method: 将CAD重建建模为序列决策任务，采用Decision Transformer与离线强化学习，并引入逐步视觉监督和几何对齐奖励。

Result: 实验表明，SOV-CAD在CAD序列重建中超越了现有最先进的方法，并表现出强大的数据效率。

Conclusion: 引入连续的视觉反馈能够实现更准确、更类似于人类的CAD建模过程。
...
### [269] [HCSU: A Dataset and Benchmark for Fine-Grained Historical Calligraphy Style Understanding](https://arxiv.org/abs/2607.04147)
*Yinsheng Yao,Yan Liu,Chen Ye*

Main category: cs.CV

TL;DR: 本文推出了HCSU，首个用于历史书法风格细粒度理解的综合数据集，旨在解决大视觉语言模型在书法审美感知上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有书法数据集存在模态混合（如墨迹字帖与碑刻拓片混杂）和标签扁平化的问题，限制了模型进行深度的书法风格理解。

Method: 构建了包含39,307张图像、涵盖49位书法家的HCSU数据集，实现了字帖与碑帖的解耦，并引入层级化专家美学描述及双重评估协议。

Result: 实验显示，现有的先进多模态模型虽有一定效果，但易受字体、文本和数据源干扰，且难以给出基于细粒度笔画的解释性美学判断。

Conclusion: HCSU数据集揭示了当前多模态架构在细粒度视觉推理上的局限，为未来文化遗产保护领域的专家级视觉感知研究提供了基准。
...
### [270] [SAF3R: Dynamic Sparse Attention for Feed-Forward 3D Reconstruction Transformers](https://arxiv.org/abs/2607.03612)
*Jianing Deng,Yuanzhe Li,Jialu Wang,Song Wang,Tianlong Chen,Huanrui Yang,Jingtong Hu*

Main category: cs.CV

TL;DR: 本文提出SAF3R，一种针对前馈三维重建（F3R）Transformer的免训练动态稀疏注意力框架，用以解决长图像序列下的计算瓶颈。


<details>
  <summary>Details</summary>
Motivation: 前馈三维重建（F3R）Transformer在处理长图像序列时，跨视角全局注意力的二次计算复杂度成为主要瓶颈，且现有方法未能有效利用注意力的稀疏性与动态特性。

Method: 设计了SAF3R框架，通过引入定制的稀疏注意力机制、离线多头分析以及在线自适应策略，动态匹配输入相关的注意力行为，无需任何额外训练。

Result: 在维持高稀疏率的同时，SAF3R有效保留了相机位姿估计与三维重建的质量，实现了显著的端到端运行加速。

Conclusion: SAF3R成功通过动态稀疏化方法缓解了F3R Transformer的计算限制，为高效三维重建提供了新思路。
...
### [271] [SeeMe: Mitigating Hallucinations in Large Vision-Language Models through Effective Visual Token Engineering](https://arxiv.org/abs/2607.04163)
*Kai Tang,Jinhao You,Bohua Zhang,Yichen Guo,Yiding Sun,Dongxu Zhang,Chenxi Li,Xiande Huang,Shanghang Zhang*

Main category: cs.CV

TL;DR: 本文提出了SeeMe，一种无需训练的框架，通过三阶段视觉Token重构有效缓解大型视觉语言模型中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法多在解码阶段干预，忽视了无关或噪点视觉Token误导解码这一导致幻觉的关键因素。

Method: 引入传统机器学习的特征工程概念，设计了三阶段Token工程流程，对视觉Token进行重构以抑制噪声并保留关键视觉信息。

Result: 在MME、POPE和AMBER基准上对四种LVLM进行评估，结果表明SeeMe能有效减少幻觉并提升输出一致性。

Conclusion: SeeMe为缓解视觉语言模型的幻觉问题提供了一种无需训练的Token工程新视角。
...
### [272] [A Vision Based System for Guided and Collaborative Reconstruction of Fragmented Documents](https://arxiv.org/abs/2607.03621)
*Oliver Krumpek,Diana Leo*

Main category: cs.CV

TL;DR: 本文开发并评估了一个结合协作机器人与AI图像匹配技术的纸质文献碎片实时协同重建系统。


<details>
  <summary>Details</summary>
Motivation: 旨在解决文化遗产保护中，脆弱及破损纸质文献碎片重建过程中的安全抓取与精准定位难题。

Method: 采用配备特制真空吸盘的协作机器人进行碎片定位，并结合AI图像分割与局部特征匹配技术（特别是SE2-LoFTR算法），提供手动与全自动双重建模式。

Result: 系统实现了8cm²碎片0.57mm的定位重复精度；在各类文档旋转、尺度变化和破损测试中，SE2-LoFTR局部特征匹配算法表现出最佳的鲁棒性。

Conclusion: 该协同系统结合先进AI与机器人技术，为脆弱历史档案碎片的修复与重建提供了一种高效、安全且精准的解决方案。
...
### [273] [CritiqueDriveVLM: From Verifier-Guided Reinforcement Learning to Latent Thought Distillation for Autonomous Driving](https://arxiv.org/abs/2607.04179)
*Zhaohong Liu,Hao Ye,Xianlin Zhang,Mengshi Qi*

Main category: cs.CV

TL;DR: 提出了CritiqueDriveVLM框架，通过强化学习训练高精度教师模型，并将其推理能力蒸馏至无CoT的学生模型，以解决自动驾驶中VLM的推理效率与可靠性冲突。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的端到端VLM常面临推理幻觉与保守偏差，而现有的思维链（CoT）和工具辅助方法因高延迟和高Token消耗无法满足实时部署需求。

Method: 提出三阶段框架：首先通过多维度验证器引导的批判驱动多轮强化学习训练System-2教师模型；随后通过隐空间思维蒸馏，将教师的推理状态对齐并压缩至快速的System-1学生模型中。

Result: 在DriveLMM-01基准上，教师模型将多选题质量（MCQ）提升至76.54%；蒸馏后的学生模型在保持推理能力的同时，平均生成长度减至28个Token，推理延迟降低了88%（从3482 ms降至416 ms）。

Conclusion: 该框架成功将复杂的推理能力内化并压缩，在不依赖脆弱外部工具的前提下，为低延迟、高可靠性的自动驾驶部署开辟了新途径。
...
### [274] [Hierarchical Multi-to-Single-Modal Knowledge Distillation for Disruption Prediction in EAST](https://arxiv.org/abs/2607.04241)
...
TL;DR: 提出一种多到单模态知识蒸馏框架，在推理阶段仅用时间序列信号即可获得多模态（图像+时间序列）的破裂预测精度，显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 虽然可见光图像能提供破裂预测的互补空间信息，但其在推理阶段会带来巨大的计算开销。

Method: 构建了多模态教师模型与单模态学生模型，通过图结构级、表征级和决策级蒸馏，将多模态知识传递给时间序列学生模型。

Result: 在EAST数据集上的评估显示，该方法在大幅降低推理成本的同时，保留了多模态学习的高精度优势。

Conclusion: 该框架为托卡马克装置中高效、实时的等离子体破裂预测提供了一种有效的解决方案。
...
### [275] [Probing Identity-Specific Motion Signatures: A Controlled Diagnostic Study](https://arxiv.org/abs/2607.03633)
...
TL;DR: 本文探讨了视频模型在身份识别中对运动特征的利用，指出模型倾向于依赖静态外观捷径，但通过抑制外观可以迫使其学习更鲁棒的运动微模式。


<details>
  <summary>Details</summary>
Motivation: 探究在身份特异性运动线索存在时，现代视频模型在多大程度上会利用这些动态线索进行身份识别，而非仅仅依赖静态外观。

Method: 引入了受控的篮球罚球基准数据集 BALLER120，并通过对比分析 RGB 输入与外观抑制（仅轮廓或骨骼）输入下视频模型的表现与关注点。

Result: RGB 模型主要依赖面部和球衣等静态捷径；而外观抑制模型则能转向关注脚部位置和肘部弯曲等运动微模式，并在面临外观变化时展现出更强的鲁棒性。

Conclusion: 身份特异性运动特征具有可学习性与信息量，但现代视频模型极易走静态捷径，需要显式抑制外观才能释放运动识别的潜力。
...
### [276] [Do Medical Vision Language Models Actually See? A Counterfactual Grounding Framework and Hard-Negative Contrastive Training for Visually-Reliant Medical VLMs](https://arxiv.org/abs/2607.03647)
*Anas Zafar,Leema Krishna Murali,Siddhant Bharadwaj,Ashish Vashist,Jia Wu*

Main category: cs.CV

TL;DR: 本文提出了一个反事实评估框架来评估医疗VLM对视觉证据的依赖性，并开发了CORAL模型，通过对比学习显著减少了视觉幻觉并提高了问答准确度。


<details>
  <summary>Details</summary>
Motivation: 虽然医疗视觉语言模型在问答中表现优异，但尚不清楚它们是依赖视觉证据进行推理，还是仅利用了文本捷径。

Method: 提出了包含图像替换（如空白、打乱及硬负样本）的反事实评估框架以计算视觉依赖指标，并开发了基于对比接地目标（CGO）进行微调的CORAL模型。

Result: CORAL在四个医疗VQA数据集上使宏观准确率提升了6.7个百分点，视觉幻觉率（VHR）降低了8.0个百分点，表现优于Qwen2.5-VL-7B基线及MedVLThinker模型。

Conclusion: 研究揭示了医疗VLM中存在的视觉接地差距，并证实了对比学习方法在缓解这一问题和提高医疗问答可靠性方面的有效性。
...
### [277] [HASSL: Hierarchy-Aware Self-Supervised Learning Framework for Single Cell Microscopy](https://arxiv.org/abs/2607.04353)
...
TL;DR: 本文提出了一种层级感知的自监督学习框架，用于在生物细胞图像中发现由细微形态特征驱动的生物学子聚类。


<details>
  <summary>Details</summary>
Motivation: 当前的自监督学习模型在处理细胞图像时常抑制层级结构，粗粒度因素（如成像模态）容易在潜在空间中掩盖细粒度的形态属性。

Method: 结合了两个核心组件：一是引入分割教师的蒸馏框架以增强潜在空间的形态感知能力；二是基于HDBSCAN的层级感知对比损失，以优化不同层级相似亚型之间的决策边界。

Result: 在包含208个细胞类别的230万个单细胞数据集上进行评估，该方法将平均Top-K准确率提高了2.8%，深层级数据集上的Top-9检索率提高了6.3%，药物分类的F1分数提高了7.8%。

Conclusion: 该方法有效减少了自监督学习对粗粒度因素的过度关注，使图像嵌入更好地与语义和形态特征对齐，从而提取出更具生物学意义的细粒度特征。
...
### [278] [From Geometric Labels to Semantic Understanding of Indoor Building Components Using Multimodal Large Language Models](https://arxiv.org/abs/2607.03661)
*Shuju Jing,Chao Yin*

Main category: cs.CV

TL;DR: 提出Building-MLLM，首个针对室内建筑构件的点云多模态大语言模型，实现了三维点云理解与自然语言交互。


<details>
  <summary>Details</summary>
Motivation: 现有的点云识别方法仅能输出离散标签，无法解释构件功能，也缺乏自然语言交互的能力，难以满足智能设施运维的需求。

Method: 开发了Building-MLLM模型，采用点信息增强器、几何保留正则化、固定文本前缀和多维LoRA四种领域特定机制；并构建了一个包含4198个对象、37,782个指令对的合成点云-文本数据集。

Result: 该模型在简单识别、复杂描述和多工程问答任务中分别取得了88.00%、65.10%和68.14%的优秀表现，并在真实世界数据集的迁移推理中展示出泛化能力。

Conclusion: Building-MLLM成功地将点云与大语言模型结合，显著提升了对室内构件的语义理解与推理能力，为建筑运维提供了新的技术路径。
...
### [279] [IPDiff: Diffusion-driven ORSI Salient Object Detection with Information Reconstruction and Multi-Prior Guidance](https://arxiv.org/abs/2607.03696)
*Gongyang Li,Zhen Bai,Runmin Cong,Dan Zeng,Weisi Lin,Xiao-Ping Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种基于扩散模型（Diffusion Model）的动态优化遥感图像显著性目标检测方法 IPDiff。


<details>
  <summary>Details</summary>
Motivation: 现有的遥感图像显著性目标检测方法多采用静态推理策略，参数固定，导致测试阶段产生的误差无法被进一步优化。

Method: 将检测任务建模为条件扩散问题，通过信息重构驱动的注意力模块提取显著性与层次先验，引导去噪网络在动态时步下迭代去噪，并结合空间与频域的混合损失函数进行协同监督。

Result: 在ORSSD、EORSSD和ORSI-4199三大公开数据集上的实验结果表明，IPDiff的性能超越了46种代表性的前沿方法。

Conclusion: 基于扩散模型和多先验引导的动态推理机制能有效解决传统静态推理的局限性，显著提升遥感图像显著性目标检测的精度。
...
### [280] [IRIS: An Intelligent Vision-Language System for Ocular Surface Diseases via Topic Tree and Scene-Driven VQA Generation](https://arxiv.org/abs/2607.04344)
*Hao Wei,Wenjin Qi,Dasen Dai,Minqing Zhang,Wu Yuan*

Main category: cs.CV

TL;DR: 本文提出了IRIS系统和最大规模的眼表疾病（OSD）VQA数据集IRIS-120K，通过结构化临床知识注入，使4B参数模型的效果超越了34B参数的大模型。


<details>
  <summary>Details</summary>
Motivation: 通用视觉语言模型（VLM）在眼表疾病等专科领域的临床推理能力，严重受限于高质量、多模态指令微调数据的匮乏。

Method: 构建了IRIS-120K数据集，并提出双分支数据生成范式（包含用于精确医学推理的主题寻找树TFT，以及场景驱动的对话合成），以此微调4B参数的轻量级VLM。

Result: IRIS取得了最先进的性能表现，全面超越了参数量高达34B的通用和医学专用VLM。

Conclusion: 结构化知识注入比单纯追求模型参数规模更有效，为在移动边缘设备上部署低资源、专家级的眼表疾病筛查AI奠定了基础。
...
### [281] [Transferability Between Understanding and Generation in Unified Multimodal Models](https://arxiv.org/abs/2607.04423)
*Jiwon Kang,Heeji Yoon,Jaewoo Jung,Jaewon Min,Minkyeong Jeon,Biyeon Hwang,Sangwon Jung,Seungryong Kim*

Main category: cs.CV

TL;DR: 本文研究了统一多模态模型（UMMs）中图像理解与生成任务之间的迁移性，发现共享架构能促进跨任务迁移，并提出通过训练理解任务来间接提升生成能力的新策略。


<details>
  <summary>Details</summary>
Motivation: 尽管统一多模态模型整合了理解与生成任务，但两者之间的相互作用及无显式监督下的能力迁移机制仍处于未被充分研究的状态。

Method: 通过控制变量实验探究不同架构对迁移性的影响，并提出一种通过微调对应的理解能力来提升生成能力（如计数、空间关系、文本生成）的训练策略。

Result: 实验表明，完全共享骨干网络和视觉编码器的架构具有更强的跨任务迁移性；利用该特性训练理解任务，能在不破坏生成图像质量的前提下，显著提升相应的生成能力。

Conclusion: 统一多模态模型中的跨任务迁移性与架构设计高度相关，且这种迁移性可以被系统性地利用，为提升多模态模型性能提供了新途径。
...
### [282] [ResearchStudio-Reel: Automate the Last Mile of Research from Paper to Poster, Video, and Blog](https://arxiv.org/abs/2607.04438)
...
TL;DR: ResearchStudio-Reel 是一个能将学术论文自动转化为可编辑海报、演讲视频和双语博客的集成系统。


<details>
  <summary>Details</summary>
Motivation: 现有的论文推广制品（海报、视频、博客）生成工具多为孤立运行，生成的格式不可在 PowerPoint 或 Word 中二次编辑，且质量控制不足，导致推广过程依然依赖繁琐的手动工作。

Method: 提出一种基于技能组合的流水线架构，包含共享资产提取器（Paper2Assets）、三个生成可编辑制品（PPT/Word）的生成器（Paper2Poster, Paper2Video, Paper2Blog），以及一个将三者集成为交互式 HTML 视轨的融合层（Paper2Reel）。

Result: 在 Paper2Poster 基准测试中，该系统在美观度和信息展示上击败了现有自动化系统和单发前沿大模型，在 84% 至 93% 的论文中获得优胜，且是唯一能同时输出三种可编辑制品的系统。

Conclusion: 通过共享提取与硬性渲染门控的组合设计，ResearchStudio-Reel 实现了高质量、可编辑且保持事实一致性的学术成果自动化多模态传播。
...
### [283] [Leveraging Pathology Co-occurrence for Test-Time Adaptation in Chest X-Ray Diagnosis](https://arxiv.org/abs/2607.03715)
*Woojin Jeong,Yujin Choi,Dongbin Kim,Soyeon Park,Jaewook Lee*

Main category: cs.CV

TL;DR: 提出了CoWA方法，通过利用疾病共现模式来改进多标签医学图像的测试时自适应（TTA）。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时自适应方法忽略了多标签医学图像中病理共现的结构化特征，导致在临床部署面临域偏移时，容易受到噪声预测的干扰。

Method: 提出共现加权自适应（CoWA）方法，从模型预测中估计标签共现结构，并降低偏离预期模式样本的权重，从而提高自适应的可靠性。

Result: 在胸部X光基准数据集上进行评估，CoWA在域偏移场景下相比现有基线方法取得了持续的性能提升。

Conclusion: 利用疾病共现规律作为可靠性信号，能有效提升医学影像多标签分类模型在未知目标域的自适应能力。
...
### [284] [ProxyUp: Training-Free Proxy-Conditioned Video Generation for Controllable Dynamics](https://arxiv.org/abs/2607.03732)
*Zanwei Zhou,Jiazhong Cen,Jiemin Fang,Yumeng He,Chen Yang,Sikuang Li,Fanpeng Meng,Zhikuan Bao,Wei Shen,Qi Tian*

Main category: cs.CV

TL;DR: 本文提出了 ProxyUp，一个无需训练的视频生成框架，利用粗糙的代理视频（物理模拟或真实录像）来精确控制视频中的复杂动态和交互。


<details>
  <summary>Details</summary>
Motivation: 仅靠文本提示难以控制精细且符合物理规律的视频动态，而获取成对的代理与目标视频进行训练又极其困难。

Method: 提出了 ProxyUp 框架，采用区域潜空间加噪（Region-wise Latent Noising）来保留关键运动，并引入随机流松弛（Stochastic Flow Relaxation, SFR）以缓解拼接带来的分布不匹配问题。

Result: 在物理模拟和真实视频代理的测试中，ProxyUp 在动态保真度和文本对齐度上均显著优于现有的视频编辑和运动转移基线方法。

Conclusion: ProxyUp 证明了无需训练即可利用粗糙物理代理实现对视频生成的高效、精确动力学控制。
...
### [285] [Wan-Streamer v0.2: Higher Resolution, Same Latency](https://arxiv.org/abs/2607.04443)
...
TL;DR: 本文介绍了 Wan-Streamer v0.2，这是一个在保持超低延迟（约200ms）的同时，将实时音视频互动模型分辨率提升至 640x368 的升级版本。


<details>
  <summary>Details</summary>
Motivation: 旨在提升交互视频的分辨率，使角色的姿态、视线、手势及场景细节在实时对话中更加清晰，同时避免增加用户可感知的延迟。

Method: 采用双阶段架构：Thinker 作为单 GPU 低延迟路径处理感知与状态计算；Performer 采用多 GPU Ulysses 上下文并行组处理高分辨率视频生成，通过将视频序列分片去噪并以 K/V 键值对形式传递条件，减少通信开销。

Result: 在 25 FPS 下实现了 640x368 视频流输出，模型端延迟维持在约 200 ms，结合 350 ms 网络预算后的总交互延迟控制在约 550 ms。

Conclusion: 通过合理拆分 Thinker-Performer 边界与并行化生成，该架构成功在不牺牲实时交互性的前提下大幅提升了视觉生成质量。
...
### [286] [Exploring SAM Supervision for Fine-Grained UAV Target Segmentation under Data Scarcity](https://arxiv.org/abs/2607.03754)
*Le-Anh Tran*

Main category: cs.CV

TL;DR: 本文提出了一种利用SAM3大模型生成伪标签来训练轻量级无人机目标分割网络（IPS-Seg）的双阶段框架。


<details>
  <summary>Details</summary>
Motivation: 无人机图像中目标尺寸小、背景复杂且缺乏密集标注数据，这限制了轻量级分割模型在实际中的应用。

Method: 利用SAM3通过“粗定位-精细化”双阶段生成伪标签，并基于此训练由IdentityFormer、ASPP和PixelShuffle组成的轻量级分割网络IPS-Seg。

Result: IPS-Seg在分割精度和计算效率之间取得了优异的平衡，且伪标签生成策略显著提升了模型性能。

Conclusion: 研究证实了利用大型基础模型作为数据标注源，来训练低标注视觉领域中轻量级特定任务网络的高效性与潜力。
...
### [287] [GeoSAM-Lite: A Lightweight Foundation Model for Onboard Remote Sensing Segmentation](https://arxiv.org/abs/2607.03760)
*Yongcong Wang,Jie Zhang,Rui Jiang,Xubing Yang,Ting Yun,Li Zhang*

Main category: cs.CV

TL;DR: 本文提出了GeoSAM-Lite，一个通过领域初始化和特征融合实现的高效、轻量级免提示遥感图像分割框架，在显著减少参数量的同时保持了高精度。


<details>
  <summary>Details</summary>
Motivation: 旨在解决在资源受限的边缘设备上部署大模型（如SAM）时，所面临的计算成本过高以及自然图像与遥感图像之间的领域偏差问题。

Method: 引入了地理空间领域初始化（Geo-Init）策略以通过知识蒸馏弥合领域差距，并设计了特征融合层（FFL）以校准空间特征并恢复高频边界信息。

Result: 实验表明，GeoSAM-Lite在保持高精度分割的同时，比重量级模型RSAM-Seg减少了92.8%的参数量。

Conclusion: GeoSAM-Lite在效率与保真度之间取得了优异的平衡，为边缘设备上的实时遥感图像分割提供了一种实用的解决方案。
...
### [288] [PulmoSight-XAI: An Explainable Multi-View Attention Ensemble with Gradient Boosting Meta-Learning for Multi-Label Chest X-Ray Classification](https://arxiv.org/abs/2607.04478)
*Moshiur Rahman,Shafqat Alam,Tasnia Binte Mamun*

Main category: cs.CV

TL;DR: 提出一种用于胸部X射线分类的可解释分层多视角集成框架，实现了最先进的分类性能与出色的临床解释性。


<details>
  <summary>Details</summary>
Motivation: 自动胸部X射线分类面临着严重的类别失衡、并发病理以及传统架构中局部特征丢失的挑战。

Method: 采用正侧位独立建模的多视角训练，结合多尺度特征融合与通道空间注意力机制（CBAM），并使用非对称与自适应焦点混合损失函数进行优化，最后通过元学习器进行分层集成，并引入事后归因技术进行解释性分析。

Result: 在CheXpert风格的数据集上，正面和侧面胸片的宏平均AUROC分别达到了0.9319和0.9154，且可视化解释具有极高的临床一致性。

Conclusion: 该框架通过结合架构多样性、多尺度注意力机制、分层元学习和可解释性分析，为胸部疾病分类提供了一个透明、高精度且符合临床实用需求的辅助诊断系统。
...
### [289] [Sparse-View Surface Reconstruction using Gaussian Splatting through High-Confidence Depth Propagation with Normal Priors](https://arxiv.org/abs/2607.03765)
*Liang Han,Bangcai Wei,Junsheng Zhou,Yu-Shen Liu,Zhizhong Han*

Main category: cs.CV

TL;DR: 提出了一种基于3D高斯泼溅（3DGS）的新方法，通过法线引导的深度传播和边缘感知正则化，实现了稀疏视图下的高质量3D表面重建。


<details>
  <summary>Details</summary>
Motivation: 尽管3DGS在稀疏视图的新视角合成中表现出色，但由于几何线索有限和高斯点的离散性，从稀疏视图重建高质量的几何表面仍然存在巨大挑战。

Method: 引入了法线引导的深度传播方法，将深度信息从高置信度区域扩展到低置信度区域以约束深度；同时提出了异常深度边缘感知正则化，以解决高斯离散性导致的深度不连续问题。

Result: 在DTU和Tanks-and-Temples数据集上的实验表明，该方法在稀疏视图表面重建任务中超越了现有的最先进（SOTA）方法。

Conclusion: 该方法通过有效解决深度传播和不连续性问题，显著提升了基于3DGS的稀疏视图高保真表面重建质量。
...
### [290] [CRISP: A Spatiotemporal Camera-Radar Backbone for Driving via Forecasting-Based World-Model Pretraining](https://arxiv.org/abs/2607.04541)
*Jingyu Song,Yi Liu,Katherine A. Skinner*

Main category: cs.CV

TL;DR: 本文提出了CRISP，一种通过预测未来LiDAR点云来预训练相机-雷达（CR）融合骨干网络的方法，以实现可复用的多模态表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有的相机-雷达融合模型通常采用特定任务的监督进行训练，这限制了通用和可复用表示的学习。

Method: 使用历史多视角图像和雷达数据，通过预测未来的LiDAR点云（仅在预训练时作为特权监督）来学习统一的鸟瞰图（BEV）表示。方法包含增强的雷达编码器、雷达增强的时序自注意力以及模态创新门控的特征渲染。

Result: 在nuScenes数据集上，CRISP不仅提升了长时点云预测的性能，还能有效迁移至3D检测、跟踪、在线建图、运动预测和规划等多个下游任务。

Conclusion: 基于预测的相机-雷达预训练是实现实用传感器配置下可扩展自动驾驶表征的一种极具前景的途径。
...
### [291] [Explainable Novel Category Discovery in Semantic Concept Space](https://arxiv.org/abs/2607.04548)
*Ifrat Ikhtear Uddin,Yang Zhou,KC Santosh,Longwei Wang*

Main category: cs.CV

TL;DR: 本文提出了 xNCD，一种可解释的新类别发现（NCD）框架，通过在结构化语义概念空间中进行类别发现，提供人类可读的解释。


<details>
  <summary>Details</summary>
Motivation: 现有新类别发现方法大多在不透明的隐式特征空间中运行，难以解释区分新类别的具体语义依据。

Method: 利用预训练多模态模型的视觉-语言相似性先验，将视觉特征对齐到无标签的概念表示空间，并对概念空间 logits 应用统一的自标记目标。

Result: 在 CIFAR-10 上达到 92.63% 的准确率，在 CIFAR-100 上将准确率提升至 76.45%，同时提供了清晰的群组和实例级解释。

Conclusion: xNCD 证明了通过语义概念瓶颈不仅能限制假设空间以引导语义合理的分类，还能在不牺牲发现性能的前提下实现内在可解释性。
...
### [292] [City-Level 3D Surface Reconstruction with Viewpoint Orientation Partitioning and Scene Completion](https://arxiv.org/abs/2607.03771)
*Liang Han,Wenyuan Zhang,Junsheng Zhou,Yu-Shen Liu,Zhizhong Han*

Main category: cs.CV

TL;DR: 提出一种基于视点方向的分区方法（VOP-GS），用于高效、高质量的大规模场景3D表面重建。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的大规模场景表面重建面临几何结构复杂、优化时间长和内存受限等挑战。

Method: 提出基于视点方向的场景分区方法以实现多GPU并行计算和更准确的深度估计；同时引入了一种检测并修复因稀疏视点或无纹理引起的初始点云缺失区域的策略。

Result: 在GauU-Scene、MatrixCity和UrbanScene3D等数据集上的实验表明，该方法在重建质量和效率上均优于现有的最先进方法。

Conclusion: 该方法通过视点方向分区和点云修复策略，有效解决了大规模场景表面重建中的效率与质量瓶颈。
...
### [293] [G$^2$TAM: Geometry Grounded Track Anything Model](https://arxiv.org/abs/2607.03789)
*Chenming Zhu,Peizhou Cao,Jingli Lin,Wenbo Hu,Yunlong Ran,Jiangmiao Pang,Tai Wang,Xihui Liu*

Main category: cs.CV

TL;DR: 本文提出了G$^2$TAM，一个利用三维几何特征进行跨视角和视频目标跟踪与重建的统一框架。


<details>
  <summary>Details</summary>
Motivation: 现有的视频分割模型依赖显式的外观记忆，易受视角变化和长期遮挡的影响，缺乏空间一致性。

Method: 提出基于几何定位的跟踪模型G$^2$TAM，使用空间对齐的几何表示作为隐式记忆，结合跨模态空间编码器融合视觉与文本提示，并构建了InsTrack数据集。

Result: G$^2$TAM在跨视角一致性、可提示实例空间跟踪、视频对象分割和空间重建方面表现出强劲的性能。

Conclusion: 该方法为交互式、基于几何的空间推理奠定了基础，有效解决了多视角和时序下的物体一致性识别与定位问题。
...
### [294] [TORINO: Token Reduction via Interpretable Concept Overlap in Vision-Language Models](https://arxiv.org/abs/2607.04593)
*Riccardo Renzulli,Gabriele Spadaro,Shruthi Gowda,Alaa Eddine Mazouz,Van-Tam Nguyen*

Main category: cs.CV

TL;DR: 本文提出了TORINO，一种用于视觉语言模型（VLM）的即插即用自适应视觉Token减少框架，通过稀疏自编码器（SAE）在可解释的隐空间中合并或剪枝冗余Token。


<details>
  <summary>Details</summary>
Motivation: VLM的大量视觉Token导致了高昂的计算成本，而现有的Token减少方法仅依赖注意力分数或成对相似度，缺乏对Token显式语义表示的分析。

Method: 利用稀疏自编码器将视觉Token投影到可解释空间，通过“概念重叠”（即共享的SAE隐层激活）对语义相近的Token进行分组，并采用修剪或合并的方式动态减少Token数量，无需对原模型进行微调。

Result: 在多个视觉语言基准测试中，TORINO实现了优异的效率与准确率权衡，在显著减少视觉Token数量的同时，仅带来极小的性能损失。

Conclusion: TORINO证明了利用可解释的语义重叠可以在不牺牲模型性能的前提下，实现高效且自适应的视觉Token减少，为降低VLM的计算开销提供了新途径。
...
### [295] [LCPNet: Latent Consistent Proximal Unfolding Network for Infrared Small Target Detection](https://arxiv.org/abs/2607.04603)
*Tianfang Zhang,Fengyi Wu,Lei Li,Chang Liu,Zhenming Peng,Huaping Zhang,Xiangyang Ji*

Main category: cs.CV

TL;DR: 提出LCPNet，一种在潜在空间中进行深度展开的红外小目标检测方法，通过结合物理分解结构与深度学习，实现了高效且低虚警率的检测。


<details>
  <summary>Details</summary>
Motivation: 现有的红外小目标检测方法未充分利用目标与背景的物理分解结构，且深度展开方法大多限于图像域，且更新与记忆机制与优化过程脱节。

Method: 提出潜一致近端展开网络（LCPNet），主要包含：1) 在潜在表示空间中利用低秩先验进行展开；2) 设计潜一致近端（LCP）求解器直接演化潜在变量；3) 引入共享优化记忆（SOM）协调各展开阶段。

Result: 在四个公开数据集上的实验表明，LCPNet在检测精度和鲁棒性上超越了最先进的方法，同时保持了低虚警率和高运行效率。

Conclusion: LCPNet证明了在潜在空间中结合物理先验与深度展开的可行性，为红外小目标检测提供了一种精准且高效的解决方案。
...
### [296] [G2VD: Generalizable AI-Generated Video Detection via Counterfactual Intervention and Causal Disentanglement](https://arxiv.org/abs/2607.04607)
*Meng Du,Hongchang Chen,Ran Li,Junjie Zhang,Qi Ouyang,Shuxin Liu*

Main category: cs.CV

TL;DR: 本文提出了G2VD框架，利用反事实干预和因果解耦来提升AI生成视频检测器的跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有检测器在未知生成器上性能下降严重，因为它们容易依赖于特定域的虚假线索（快捷学习），而非本质的伪造痕迹。

Method: 引入反事实干预管道（CFIPipeline）生成受控反事实样本进行频域和像素域对齐，并使用基于HSIC约束的因果解耦分类器来分离任务相关线索与特定域偏差。

Result: 在四个公开数据集上表现出强劲的跨域性能，在GenVidBench上仅用10%的训练数据就实现了超过90%的准确率和接近0.95的AUC。

Conclusion: G2VD通过克服快捷学习和域偏见，显著提升了AI生成视频检测的泛化性能。
...
### [297] [InfraNet: Quality-Aware RGB Guidance for Efficient Infrared Object Detection](https://arxiv.org/abs/2607.03795)
...
TL;DR: 提出一个以红外为中心的质量感知目标检测框架InfraNet，通过动态调控RGB引导，实现灵活且高效的多模态或单红外检测。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态融合检测方法过度依赖RGB和红外输入的对称结合，在RGB模态不可靠或缺失时鲁棒性较差。

Method: 设计了非对称架构InfraNet，利用主红外路径进行预测，辅助RGB路径提供监督信号，并引入质量感知融合模块QualGate以抑制不可靠的RGB引导。

Result: 在四个主流基准数据集（LLVIP、FLIR-Aligned、M3FD和DroneVehicle）上取得了优异的检测准确率，并且在仅红外推理模式下表现出极高的计算效率。

Conclusion: 该方法有效提升了恶劣视觉条件下的多模态目标检测鲁棒性，兼顾了检测准确率与计算效率。
...
### [298] [Probabilistic Robustness in Medical Image Classification](https://arxiv.org/abs/2607.03797)
*Yi Zhang,Siddartha Khastgir,Xingyu Zhao*

Main category: cs.CV

TL;DR: 本文探讨了医学图像分类中比对抗鲁棒性更契合实际的概率鲁棒性（PR），并在MedMNIST v2数据集上系统评估了常用深度学习模型在自然损坏下的表现。


<details>
  <summary>Details</summary>
Motivation: 深度学习在临床医学图像分类部署中面临可信度挑战，而现有的对抗鲁棒性研究偏向最坏情况，与实际医学应用场景不够契合。

Method: 引入概率鲁棒性作为更实用的评估指标，构建了医学图像的自然损坏设置，并在MedMNIST v2数据集上对常用深度学习模型进行了系统评估。

Result: 系统性地评估了多种常用深度学习模型在自然损坏下的表现，为医学图像分类模型的可信度评估提供了统计学依据。

Conclusion: 本研究为评估深度学习模型的可信度提供了一个有统计学基础的新视角，有助于推动其在临床医学成像中的可信部署。
...
### [299] [Targeted Structure Completion for Sparse-View 3D Reconstruction in Autonomous Driving](https://arxiv.org/abs/2607.04661)
*Guoqing Wang,Pin Tang,Xiangxuan Ren,Liping Hou,Chao Ma*

Main category: cs.CV

TL;DR: 提出FocusGS框架，通过将计算集中在几何不确定区域，实现了高效且结构完整的自动驾驶3D场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有的体素高斯方法因采用均匀体素处理而产生大量计算冗余，需要弥合像素级方法的高效性与体素级方法的结构完整性之间的鸿沟。

Method: 提出FocusGS框架，构建3D几何模糊流形以精准定位易遮挡和高不确定性区域，并设计轻量级的定向结构补全模块，仅在此稀疏拓扑子空间内实例化和优化连续的高斯查询。

Result: 在自动驾驶基准测试上达到最先进性能，同时将高斯函数总量减少了约74%，并将渲染时间缩短了约34%。

Conclusion: FocusGS通过将结构补全与确定性区域解耦，成功实现了自动驾驶场景重建中更优的效率与质量权衡。
...
### [300] [Hierarchical Evidence-Driven Reasoning for Long Document Understanding](https://arxiv.org/abs/2607.04625)
*Junyu Xiong,Yonghui Wang,Rongjian Gu,Chenyu Liu,Bing Yin,Wengang Zhou,Houqiang Li*

Main category: cs.CV

TL;DR: 本文提出了 HIEVI-RAG，一个用于闭域文档理解的分层、证据驱动的多模态检索增强生成（RAG）框架。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要问题：一是检索容易引入无答案的干扰页面，二是单次检索机制容错率低、易导致级联错误。

Method: 提出四阶段协同管道：分层问题分解、粗粒度视觉检索、基于 EVIAGENT（用 GRPO 训练）的细粒度页面验证以及记忆引导的迭代生成。

Result: 在四个基准测试上，HIEVI-RAG 显著优于现有开源基线，平均准确率提升了 8.05%。

Conclusion: HIEVI-RAG 通过系统性的分层与证据驱动机制，有效提升了多模态 RAG 在复杂长文档理解任务中的鲁棒性与准确性。
...
### [301] [TestMate: Test-Time Domain Adaptation Aided by Lightweight Vision Foundation Model](https://arxiv.org/abs/2607.03810)
*Dimitrios Fotiou,Vasileios Mygdalis,Ioannis Pitas*

Main category: cs.CV

TL;DR: 本文提出了一种名为 TestMate 的无需反向传播的实时测试时域自适应（TTDA）框架，利用视觉基础模型提升语义分割的自适应能力。


<details>
  <summary>Details</summary>
Motivation: 现有的 TTDA 方法中，基于熵最小化的方法计算成本高且易导致灾难性遗忘，而记忆库方法在应对持续域偏移时适应较慢。

Method: 利用轻量级零样本 YOLOv8-seg 模型生成多尺度掩膜建议，并采用基于尺寸排序的启发式竞争融合机制将其与主模型预测相结合，实现无反向传播的实时自适应。

Result: 在两个基准数据集上取得了最先进的性能，并在 TTDA、无源域自适应（SFDA）和在线 TTDA 任务中均证明了其有效性。

Conclusion: TestMate 能够实现即时自适应，有效避免灾难性遗忘并保留精细的目标边界，可作为独立优化模块或与现有 TTDA 方法无缝集成。
...
### [302] [Global Logic and Local Search: Dual-Stream Multimodal In-Context Learning for Verifiable Industrial Anomaly Detection](https://arxiv.org/abs/2607.03817)
*Runzhi Deng,Yundi Hu,Yiming Zhong,Zhao Wang,Xixi Liu,Hongsong Wang,Caifeng Shan,Fang Zhao*

Main category: cs.CV

TL;DR: 本文提出了GLLS，一种免训练的多模态上下文验证框架，用于解决少样本情况下的工业异常检测问题。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测面临缺陷微小、分辨率有限以及缺乏视觉证据等难题，且现有的微调方法在部署初期因缺乏缺陷样本而难以应用。

Method: 提出GLLS框架，通过“局部感知视觉逻辑图谱”组织参考标准，结合“全局与逻辑流”（利用SAM 3提取局部视觉事实）和“细粒度与动作流”（利用MCTS在有限预算下检索局部证据裁剪图）进行推理。

Result: 在MMAD-QA等多个异常检测数据集上，该方法均优于对比基线，并能生成可追溯至具体视觉证据的诊断决策。

Conclusion: GLLS为无需训练的少样本工业异常检测提供了一种高精度且具可解释性的新方案。
...
### [303] [EventCoT: Event-centric Video Chain-of-thought for Reasoning Temporal Localization](https://arxiv.org/abs/2607.04872)
*Youngkil Song,Yoonjae Baek,Dongwon Kim,Inho Kim,Dongkeun Kim,Suha Kwak*

Main category: cs.CV

TL;DR: 本文提出了EventCoT，这是一种基于事件中心视频思维链的框架，旨在高效解决推理时间定位（RTL）任务。


<details>
  <summary>Details</summary>
Motivation: 推理时间定位（RTL）需要模型在生成答案的同时定位支持该答案的时间区间，这要求模型兼具高阶推理能力与精准的时间定位能力。

Method: EventCoT首先通过事件中心化分词将视频转化为紧凑的事件Token以识别相关事件；接着在事件内进行推理生成答案；最后通过嵌入匹配将占位符Token与视觉嵌入对齐，从而实现时间区间的精准定位。

Result: EventCoT在ActivityNet-RTL数据集上达到了SOTA性能，且使用的视觉Token数量显著减少；同时在ReXTime基准测试中展现出强大的零样本泛化能力。

Conclusion: EventCoT是一个高效且实用的视频思维链框架，证明了事件中心化处理在推理时间定位和多模态视频问答中的有效性。
...
### [304] [TRISTAR: Triple-Signal Stair Recognition and Vision-Only Indoor Navigation for Search-and-Rescue Micro-UAVs](https://arxiv.org/abs/2607.03818)
*Octavian Gîngu,Stelian Spînu*

Main category: cs.CV

TL;DR: 本文介绍了一种仅基于单目视觉的低成本无人机自主室内搜救与导航框架，实现了走廊探索与自主楼梯攀升。


<details>
  <summary>Details</summary>
Motivation: 传统室内自主导航依赖LiDAR或双目相机，导致硬件复杂且成本高昂，难以在无GNSS信号且环境危险的室内搜救场景中快速推广。

Method: 利用DJI Tello平台，结合Depth Anything V2单目深度估计与轻量化深度学习，设计了走廊探索机制，并提出基于三源融合（Sobel结构、Gabor纹理及单目深度）的TRISTAR算法用于楼梯识别与攀升。

Result: 在真实室内飞行中，深度估计误差降至10%以下，门检测精确率达0.93（F1值0.91），且多传感器融合显著提升了楼梯识别在复杂光照和反射表面下的鲁棒性。

Conclusion: 在资源受限且无专业测距硬件的平台上，仅靠单目视觉也能实现可靠的室内探索与楼梯遍历，为快速搜救部署提供了一种高性价比的解决方案。
...
### [305] [Graph Representation Learning of Longitudinal Medical Imaging Trajectories for Treatment Response Prediction](https://arxiv.org/abs/2607.04912)
...
TL;DR: 提出了一种基于3D时空图神经网络和自监督学习的框架，用于预测乳腺癌患者新辅助化疗后的病理完全缓解（pCR）。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌患者对新辅助化疗的反应差异显著，需要准确的预测模型来评估个体化治疗反应，以辅助临床决策。

Method: 融合了先进的图神经网络与时间关系建模，并结合三种新型自监督治疗轨迹表征学习目标，构建了3D时空纵向影像预测框架。

Result: 在ISPY-2数据集（585名患者）上的实验表明，该方法在多项分类指标上显著优于传统视觉和自监督学习基线，并建立了pCR预测基准。

Conclusion: 研究证实了纵向医学影像建模在预测新辅助化疗疗效中的重要临床价值，并通过开源代码促进了研究的可复现性。
...
### [306] [FDR-Occ: Factorized Dense Routing for Full-Spectrum 3D Occupancy Prediction](https://arxiv.org/abs/2607.03822)
*Dubing Chen,Huan Zheng,Tianyi Yan,Yucheng Zhou,Runzhou Tao,Zhongying Qiu,Jianfei Yang,Jianbing Shen*

Main category: cs.CV

TL;DR: 提出了一种基于因子化密集路由（FDR）和分辨率-上下文解耦架构的3D占用预测框架，打破了传统显式投影的局部性瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有2D到3D视图转换依赖显式物理投影，导致局部性瓶颈，且在相机外参不准确或缺失时模型鲁棒性较差。

Method: 提出因子化密集路由（FDR）以实现全局感受野，并引入分辨率-上下文解耦架构，将3D空间分解为全局拓扑锚点和局部几何平面。

Result: 在Occ3D-nuScenes和Occ3D-Waymo基准上取得SOTA性能，并在无外参校准的严苛设置下展现出更强的结构鲁棒性。

Conclusion: 该方法通过全局路由与解耦架构，实现了全局语义推理与精确表面定位的互补，显著提升了3D占用预测的精度和对相机外参变化的鲁棒性。
...
### [307] [Efficient Perception in Automotive Detection and Tracking Using Neuromorphic Computing](https://arxiv.org/abs/2607.04921)
*Manish Kolachalam,Rani Malhotra*

Main category: cs.CV

TL;DR: 本文首次全面评估了脉冲神经网络（SNN）在真实汽车多目标检测和跟踪中的表现，证明了其在自动驾驶感知任务中高效应用的可行性。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习的高能耗和计算需求限制了其在边缘设备上的部署，而脉冲神经网络（SNN）作为低功耗、高并行的替代方案，非常适合用于自主机器的感知任务。

Method: 采用基于SpikeYOLO架构的迁移学习方法，在KITTI和BDD100K MOT2020数据集上进行多目标检测与追踪的全面评估。

Result: 在目标检测中，KITTI和BDD100K数据集的mAP分别达到0.937和0.771；在目标跟踪中，HOTA得分分别为0.701和0.445，其性能与传统的深度学习方法相当。

Conclusion: SNN能够以高能效的方式提供高性能的目标检测与追踪，证实了其在真实世界自主系统感知应用中的可行性。
...
### [308] [ContiStain: Cross-Domain Relation-Preserving Distillation for Continual Multi-Domain Virtual IHC Staining](https://arxiv.org/abs/2607.03851)
*Fuqiang Chen,Yifeng Wang,Hongpeng Wang,Yongbing Zhang*

Main category: cs.CV

TL;DR: 本文提出了 ContiStain 框架，通过混合专家架构和关系保持蒸馏，解决了多领域虚拟染色在持续学习过程中的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 在临床应用中，新生物标志物的数据通常是顺序获取的，直接对新数据进行微调会导致模型在已学习的生物标志物上出现严重的性能退化（灾难性遗忘）。

Method: 提出了 ContiStain 框架，首先使用混合专家（MoE）提取器构建领域感知的结构化特征空间以减少干扰，然后通过关系保持蒸馏策略维持跨领域 Token 级余弦相似度矩阵的一致性。

Result: 在 MIST 数据集的四领域顺序虚拟染色实验中，ContiStain 表现出更高的稳定性，相比顺序微调，FID 和 ConchFID 分别降低了 11.1 和 60.9。

Conclusion: ContiStain 能够有效缓解遗忘问题并保持对新领域的适应性，实现了可扩展且鲁棒的多领域虚拟染色。
...
### [309] [MemPose: Category-level Object Pose Estimation with Memory](https://arxiv.org/abs/2607.04930)
*Xiao Lin,Minghao Zhu,Yun Peng,Liuyi Wang,Qiyi Wang,Chengju Liu,Qijun Chen*

Main category: cs.CV

TL;DR: 本文提出MemPose，一种利用外部记忆缓冲区动态存储和更新几何特征的记忆增强型类别级物体姿态估计框架。


<details>
  <summary>Details</summary>
Motivation: 现有的类别级姿态估计方法主要将特征编码为固定的形状先验或静态权重，限制了对多样化实例的泛化与扩展能力。

Method: 引入外部记忆缓冲区，用于存储和动态更新先前观测实例的结构化表示，使模型能够利用累积的经验来辅助当前姿态感知。

Result: 在REAL275、CAMERA25、Housecat6D和Wild6D四个挑战性基准上，该方法表现均优于现有的最先进方法。

Conclusion: 将类别级几何记忆引入姿态估计管道，能有效提升模型在面对多样化实例时的鲁棒性和泛化能力。
...
### [310] [Comparison of Loss Functions for Robust Deep Learning-based Echocardiography Segmentation when Learning with Partially Labelled Data from Multiple Domains](https://arxiv.org/abs/2607.05008)
*Iman Islam,Esther Puyol-Antón,Bram Ruijsink,Andrew J. Reader,Andrew P. King*

Main category: cs.CV

TL;DR: 评估并对比了三种损失函数（aCCE、边缘损失和aBCE）在处理多领域、部分标注的超声心动图分割任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 多源超声心动图数据集通常存在部分标注（标签缺失）的问题，这给训练高效的多结构自动分割模型带来了挑战。

Method: 全面对比了自适应分类交叉熵（aCCE）损失、边缘损失（marginal loss）和自适应二分类交叉熵（aBCE）损失在域内/跨域任务、单/多标签缺失以及不同标注比例等多种场景下的性能。

Result: 三种损失在域内分割任务中均表现优异。在跨域任务中，针对单一标签缺失，aBCE和边缘损失效果较好；针对多个标签缺失，边缘损失表现出最强的鲁棒性并优于其他方法。

Conclusion: 针对不同的标注缺失场景选择合适的损失函数对于优化分割模型至关重要，该研究为解决跨领域部分标注的超声心动图分割问题提供了重要参考。
...
### [311] [CogRad: A Cognitively-Inspired Multi-Agent Framework for Radiology Report Generation](https://arxiv.org/abs/2607.03853)
*Saif Ur Rehman Khan,Hasaan Maqsood,Sebastian Vollmer,Andreas Dengel,Muhammad Nabeel Asim*

Main category: cs.CV

TL;DR: 本文提出了CogRad，一个受认知启发的多智能体放射报告生成框架，通过模拟医生的四个阅读与核对阶段来提升报告的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有自动放射报告生成系统通常采用单次生成，缺乏比对图像进行核实及修正已生成内容的机制。

Method: 构建了包含Scout（侦察）、Investigator（调查）、Writer（撰写）和Verifier（验证）四个智能体的框架，结合插槽注意力机制与视觉蕴含损失实现自我检验与重写。

Result: 在CheXpert Plus和IU X-Ray数据集上均取得了领先的BLEU-4和CIDEr评分，且在临床准确性指标（RadGraph/CheXbert F1）和幻觉分析中表现优异。

Conclusion: 通过认知启发的多智能体协同与推理期自我纠错，CogRad显著提升了自动放射报告生成的质量、临床准确性及视觉基准能力。
...
### [312] [RUFNet: Query-Guided Support Mask Refinement and Uncertainty Fusion based on Hybrid Mamba for Few-Shot Brain Tumor Segmentation](https://arxiv.org/abs/2607.05035)
*Dongyi He,Xiangkai Wang,Binbing Xu,Bin Jiang,Hongjie Yan,Weixiang Liu,Wai Ting Siok,Nizhuan Wang*

Main category: cs.CV

TL;DR: 本文提出了RUFNet，一种结合支持掩膜细化与不确定性感知后验融合的混合Mamba少样本脑肿瘤分割框架。


<details>
  <summary>Details</summary>
Motivation: 针对少样本脑肿瘤分割中面临的支持掩膜噪声、患者间图像差异以及缺乏像素级置信度估计等挑战。

Method: 提出RUFNet框架，包含线性复杂度的混合Mamba交互骨干网络、用于校准支持掩膜的注意力引导掩膜细化模块（AGMR），以及用于评估像素级方差并融合预测的不确定性感知后验融合模块（UAPF）。

Result: 在BraTS 2020数据集上，RUFNet在1-shot和5-shot设置下分别实现了84.3%和86.1%的Dice系数，性能超越了现有的先进方法。

Conclusion: 研究表明，混合Mamba交互、掩膜细化和不确定性建模能有效提高少样本医学图像分割的鲁棒性。
...
### [313] [PRISM3D: Probabilistic Refinement and Robust Initialization for Physically Consistent Scene Modeling under Extreme Motion Blur](https://arxiv.org/abs/2607.03855)
*Gopi Raju Matta,Reddypalli Trisha,Vemunuri Divya Madhuri,Kaushik Mitra*

Main category: cs.CV

TL;DR: 提出PRISM3D框架，首次实现直接从极端运动模糊图像中进行鲁棒的3D三维重建，并支持RGB与事件融合的多模态扩展。


<details>
  <summary>Details</summary>
Motivation: 传统SfM管线在极端运动模糊下会失效，且现有解模糊重建方法依赖不切实际的清晰图像监督。

Method: 使用基于深度稠密跟踪（VGGSfM）的鲁棒初始化，结合基于MCMC的概率几何加密与贝塞尔轨迹物理模糊建模，并引入RGB+事件的多模态扩展（PRISM3D-E）。

Result: 在纯RGB和多模态设置下均达到最先进（SOTA）性能，并贡献了PRISM3D-E基准数据集。

Conclusion: PRISM3D成功攻克了从严重模糊图像中重建3D场景的难题，展示了概率几何与物理建模结合的强大潜力。
...
### [314] [Ghosts Beneath Textures: Texture-Relation Cues for Cross-Paradigm AI-Generated Image Detection](https://arxiv.org/abs/2607.03862)
*Haoyu Wang,Yiming Qin,Zhongjie Ba,Ziping Dong,Jishen Zeng,Peng Cheng,Kui Ren*

Main category: cs.CV

TL;DR: 本文针对跨范式（无图像与有图像条件生成）AI生成图像检测问题，提出了通过建模语义无关的局部-全局纹理关系来进行检测的DTS-Det框架，并构建了新基准ConImageGen。


<details>
  <summary>Details</summary>
Motivation: 现有的AI生成图像检测器主要针对无图像条件生成，在面对图像条件生成时泛化能力不足，这导致跨范式检测成为一个被长期忽视的关键挑战。

Method: 首先构建了跨范式检测基准ConImageGen；接着通过抑制语义干扰，可视化了跨范式的语义无关纹理模式；最后提出DTS-Det框架，通过建模这些局部与全局的纹理关系来进行通用检测。

Result: DTS-Det在ConImageGen基准上达到99.6%的准确率（比最佳基线提升10.5%），在跨数据集评估中达到93.2%/94.1%的准确率，并在重建和黑盒对抗攻击下保持高鲁棒性。

Conclusion: 将检测关注点从显式伪影转向建模语义无关的局部-全局纹理关系，是实现通用且鲁棒的跨范式AI生成图像检测的有效途径。
...
### [315] [Shifting from Discrete to Continuous Reference Data: QSM-Derived Horizontal Tree Biomass Distribution for Deep Learning Biomass Estimation](https://arxiv.org/abs/2607.05260)
*Nils Griese,Christoph Kleinn,Nils Nölke*

Main category: cs.CV

TL;DR: 提出一种基于定量结构模型（QSM）连续绘制水平生物量分布（HBD）的方法，有效解决了利用LiDAR估算地上生物量时小样地面临的边界效应问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于LiDAR的地上生物量（AGB）估算依赖于离散的样地级调查汇总，这在小样地中会引入严重的边界效应误差，从而降低模型估算精度。

Method: 基于模拟的阔叶林结构训练稀疏3D U-Net，对比了标准森林调查（FI）汇总、无边缘效应的QSM汇总和连续HBD映射三种AGB参考源，并评估了100至2500平方米不同样地尺寸下的表现。

Result: 在小样地尺寸下，基于QSM的模型系统性优于FI方法。特别是在100平方米的小样地上，HBD参考相比FI基线使相对均方根误差（RRMSE）降低了16.84%，并将R²提高了0.22。

Conclusion: 用HBD作为AGB参考代替传统的样地级汇总，能够有效纠正边界效应，并显著提升小尺寸样地下的生物量估算模型性能。
...
### [316] [SharpSplat: Edge-Regularized 3D Gaussian Splatting for High Fidelity Urban Building Reconstruction from UAV images](https://arxiv.org/abs/2607.03872)
*Porus Vaid,Shivam Chopra,Vaibhav Kumar*

Main category: cs.CV

TL;DR: 本文提出了一种语义边缘正则化框架，通过引入边缘监督，显著提升了3D高斯 splatting (3DGS) 在建筑物三维重建中的边界几何保真度。


<details>
  <summary>Details</summary>
Motivation: 现有的3DGS技术在重建建筑物外立面时，难以有效捕捉清晰锐利的几何过渡和结构边界。

Method: 利用SAM 3生成精确的建筑掩膜并提取关键边缘，在训练中将渲染图像的梯度与提取的边缘进行对齐，约束高斯分布收敛至清晰的结构几何面。

Result: 在校园、密集城市及住宅区等多场景数据集上的实验表明，该方法在不修改3DGS管线的前提下显著提升了边缘保真度，表现出强鲁棒性。

Conclusion: 该研究证明了结合语义边缘正则化可以有效解决3DGS在建筑重建中边界模糊的问题，适用于大规模数字孪生建设。
...
### [317] [MACRO: Training-free Multi-plane Attention for Closeup Render Optimization](https://arxiv.org/abs/2607.03875)
*Nitzan Hodos,Roy Amoyal,Lior Fritz,Ianir Ideses,Sagie Benaim,Netalee Efrat*

Main category: cs.CV

TL;DR: 本文提出MACRO，一种无需训练的特写渲染优化方法，解决了3D高斯泼溅在近距离观察时因尺度差异导致的渲染伪影问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在特写镜头下的渲染质量会下降，而基于扩散的增强方法由于特写视图与参考视图之间的尺度差异（且VAE和注意力机制非尺度等变/共变），会导致特征匹配错误并产生伪影。

Method: 提出MACRO（多平面注意力特写渲染优化）：利用3D结构将特写图分解为多个深度平面，在图像空间对参考图进行裁剪和缩放以匹配各平面的尺度，并使用深度感知注意力掩码限制Token只关注尺度匹配的参考区域。

Result: 在两个新建立的特写新视角合成基准上，该方法在重建和感知指标上均取得了SOTA（最先进）的成果，显著优于现有的3DGS和基于扩散的方法。

Conclusion: MACRO在无需修改架构和无需训练的情况下，通过在图像空间解决尺度偏差，有效提升了3DGS的特写新视角合成质量。
...
### [318] [Steering Optimisation Trajectories in Diffusion Representation Learning](https://arxiv.org/abs/2607.05319)
*Rajat Rasal,Avinash Kori,Tian Xia,Ben Glocker*

Main category: cs.CV

TL;DR: 本研究揭示了扩散自编码器在重建与解耦表征之间的优化动力学权衡，并提出了SteeringDRL方法以优化潜在表征质量。


<details>
  <summary>Details</summary>
Motivation: 探究扩散自编码器在生成相似图像质量时，为何会学习到截然不同的潜在结构。

Method: 分析优化轨迹，并提出SteeringDRL方法（结合门控残差U-Net与噪声水平曝光课程学习）来引导优化过程。

Result: SteeringDRL在解耦基准上提升了表征质量并降低了种子敏感性，同时在空间解耦的物体中心学习中提高了分割质量。

Conclusion: 通过控制U-Net捷径和早期噪声暴露，可以有效引导优化动力学，在重建与解耦表征之间取得更好平衡。
...
### [319] [SGF-CDNet: A Consistency-Discrepancy Graph Network over Semantic-Geometric Fused Nodes for Face Forgery Detection](https://arxiv.org/abs/2607.03883)
*Jiayao Jiang,Bin Liu,Nenghai Yu*

Main category: cs.CV

TL;DR: 提出SGF-CDNet，一种基于语义-几何融合和一致性-差异性图网络的人脸伪造检测方法。


<details>
  <summary>Details</summary>
Motivation: 深伪技术发展迅速，伪造人脸虽无明显瑕疵，但面部区域间常存在微妙的不协调。

Method: 构建融合语义与几何信息的SGF节点，并通过双路径图网络（CD-GNN）在一致性和差异性两个维度上并行推理面部区域关系。

Result: 在公开数据集上的实验表明，SGF-CDNet取得了优异的检测性能。

Conclusion: SGF-CDNet能有效识别面部组件间的不协调关系，是可靠的人脸伪造检测方案。
...
### [320] [Topological Shape Representation for Aneurysm -- Bifurcation Detection](https://arxiv.org/abs/2607.05317)
*Akshay Gokhale,Mansi Dhamne*

Main category: cs.CV

TL;DR: 提出一种基于平滑欧拉示性标变换（SECT）的拓扑感知虚警过滤框架，显著提升了小型颅内动脉瘤（<3mm）的检测准确率。


<details>
  <summary>Details</summary>
Motivation: 自动检测颅内动脉瘤时，CNN常因依赖局部像素强度而混淆动脉瘤与血管分叉，导致高虚警率，尤其是对于小于3mm的小病变检测敏感性极低。

Method: 引入平滑欧拉示性标变换（SECT）来编码全局3D血管几何特征，构建了一个即插即用的拓扑感知虚警去除框架，并在RSNA 2025数据集上进行验证。

Result: SECT实现了0.943的AUC，在小于3mm的病变中保持了0.943的AUC和78.5%的敏感性，并且在跨扫描仪验证中表现出优异的泛化能力（平均AUC达0.927）。

Conclusion: SECT通过捕获非对称几何不变量成功解决了结构性混淆问题，证明了拓扑特征在克服医学图像检测中虚警问题上的有效性，适合作为深度学习管线的下游过滤器。
...
### [321] [Multiplayer Interactive World Models with Representation Autoencoders](https://arxiv.org/abs/2607.05352)
...
TL;DR: 本文推出了首个用于高动态、复杂物理交互环境的多人世界模型，并在《火箭联盟》游戏上进行了验证。


<details>
  <summary>Details</summary>
Motivation: 传统的单人世界模型将其他玩家视为环境的一部分，无法准确将场景变化归因于特定玩家，也无法在多种动作组合下保持画面一致性。

Method: 使用1万小时的游戏数据，训练了一个拥有50亿参数的潜扩散模型（Latent Diffusion Model），并根据多个玩家的动作流进行条件约束输入。

Result: 该模型能在单张Nvidia B200 GPU上以20 fps的速率实时生成四人比赛画面，且生成的视频流可稳定持续数分钟甚至数小时而不崩溃。

Conclusion: 研究证明了多人世界模型在大规模数据和参数下的可行性与物理理解力，并开源了相关数据集和代码。
...
### [322] [SiamJEPA: On the Role of Siamese Student Encoders in JEPA](https://arxiv.org/abs/2607.04044)
*Makoto Yamada*

Main category: cs.CV

TL;DR: 提出了SiamJEPA，通过在JEPA框架中引入孪生（Siamese）学生编码器，提升了自监督表征学习的效率与性能。


<details>
  <summary>Details</summary>
Motivation: 现有JEPA方法通常在学生网络中采用单编码器，而更符合大脑启发式表征学习框架的孪生编码器在JEPA中的作用尚未得到探索。

Method: 提出SiamJEPA，该模型将掩码孪生学生编码器与指数移动平均（EMA）教师网络相结合，是脑启发表征学习模型PhiNet的JEPA化实现。

Result: 孪生编码器起到了有效的正则化作用，加速了早期训练并提高了表征的可分性。在ImageNet线性探测评估中，SiamJEPA在有限训练预算下优于单编码器JEPA变体，且超越了训练时间更长的MAE。

Conclusion: 孪生学生编码器为预测性表征学习引入了重要的归纳偏置，为优化JEPA模型设计和改进自监督表征学习提供了一种简单而有效的新途径。
...
### [323] [The Multipath Blind Spot: $K$-Agnostic Robust Calibration for Sparse-Anchor Metric Depth from Frozen Foundations](https://arxiv.org/abs/2607.04101)
*Sohag Roy,Rajesh Misra,Swami Shastravidyananda,Tamal Maharaj*

Main category: cs.CV

TL;DR: 提出MRAC，一种无参数的推理期包装器，能有效过滤噪声锚点，实现鲁棒的单目深度绝对尺度校准。


<details>
  <summary>Details</summary>
Motivation: 现有的稀疏锚点深度校准方法假定锚点数据是干净的，但实际传感器常会产生异常值（如多路径干扰），导致现有校准方法失效。

Method: 提出多路径鲁棒锚点校准（MRAC），在校准前利用Theil-Sen拟合和绝对中位偏差（MAD）测试，根据模型自身的相对深度顺序对锚点进行一致性筛查过滤。

Result: MRAC无需额外学习参数，CPU运行时间仅约50微秒。在基准测试中胜率达84%，在KITTI多路径误差测试中将绝对相对误差（AbsRel）降低了3.2倍（从0.489降至0.151），且无需重新训练。

Conclusion: MRAC提供了一种无需重训、极低计算开销且能有效抵御真实传感器异常值干扰的单目相对深度尺度校准方案。
...
### [324] [FRFDet: Efficient UAV Small Object Detection with Symmetric Sampling and Scalable Fusion](https://arxiv.org/abs/2607.04125)
*Yunzhong Si,Huiying Xu,Xinzhong Zhu,Yang Liu,Yao Dong,Wenhao Zhang,Hongbo Li*

Main category: cs.CV

TL;DR: 本文提出了一种名为FRFDet的轻量级单阶段无人机目标检测算法，有效提升了恶劣环境下微小目标的检测性能。


<details>
  <summary>Details</summary>
Motivation: 无人机图像中的微小目标检测在恶劣天气、低光照和噪声等复杂环境下，面临背景杂乱、细节退化以及特征融合不足的挑战。

Method: 引入了两个即插即用模块：逆双向采样（IBS）以保留关键空间细节并改善特征对齐，以及尺度特征关系交叉融合（SFRCF）以显式建模并优化多尺度特征融合行为。

Result: 在VisDrone、UAVDT、HazyDet和MS COCO数据集上的实验表明，该方法在保持低计算量、参数量和高推理速度的同时，达到了轻量级检测器的前沿水平。

Conclusion: FRFDet实现了性能与效率的良好平衡，非常适合部署在资源受限的无人机平台上。
...
### [325] [Real-Time LiDAR Gaussian Splatting SLAM](https://arxiv.org/abs/2607.04127)
*Seungjun Tak,Yewon Jeon,Jaeik Hwang,SukMin Hwang,Seongbo Ha,Hyeonwoo Yu*

Main category: cs.CV

TL;DR: 一种实时LiDAR高斯泼溅SLAM框架，通过耦合G-ICP与几何感知的密集建图实现高效定位与重建。


<details>
  <summary>Details</summary>
Motivation: 旨在利用LiDAR几何特征而非视觉外观，解决大规模场景下高斯泼溅SLAM的实时性与建图精度问题。

Method: 耦合快速G-ICP配准与球面光栅化建图；利用跟踪协方差初始化高斯尺度并导出法向量进行优化；引入协方差几何得分指导自适应剪枝与加密；将优化结果反馈以增强跟踪鲁棒性。

Result: 在Newer College数据集上实现超20 FPS的实时运行速度，且在线轨迹的F-score达到86.78%，在多个数据集上验证了其稳定性和可扩展性。

Conclusion: 该框架成功将LiDAR几何先验与高斯泼溅结合，为大场景实时三维重建和定位提供了一种高效、稳健的解决方案。
...
### [326] [Topology-Driven Transferability Estimation for 3D Medical Vision Foundation Models](https://arxiv.org/abs/2607.04199)
*Jiaqi Tang,Shaoyang Zhang,Fandong Zhang,Shu Zhang,Yang Liu,Qingchao Chen*

Main category: cs.CV

TL;DR: 一种快速且非参数化的拓扑驱动框架，用于在不进行微调的情况下评估医学视觉基座模型在3D分割任务中的可迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有的模型可迁移性评估方法通常依赖高成本的微调，且主要针对图像级分类，无法有效捕捉3D医学图像分割所需的空间关系和精细边界细节。

Method: 提出一种基于最小生成树（MST）的拓扑驱动框架，通过局部边界感知拓扑一致性（LBTC）和全局表示拓扑发散（GRTD）双尺度评估特征与标签的对齐度，并利用随机初始化的解码器作为空间投影仪以稳定评估。

Result: 在包含11.4万个3D医学卷轴的大规模基准上，该方法将可迁移性评估的加权Kendall系数提升了0.36，同时将评估速度提高了56倍。

Conclusion: 该框架证明了拓扑分析在无微调情况下评估密集预测任务可迁移性的有效性，为医学视觉基座模型的高效选择提供了实用方案。
...
TL;DR: 提出HeartVolMesh，一种基于模板变形的3D CNN-GNN方法，用于从三维医学图像中重建高精度的患者特异性心脏四面体网格。


<details>
  <summary>Details</summary>
Motivation: 传统的“先分割后建模”方法容易模糊心脏的薄壁解剖结构，且缺乏病例间的网格对应关系。

Method: 将模板顶点表示为各向异性高斯核，使用3D CNN-GNN预测顶点位移与协方差，并通过分阶段对齐、非刚性配准和变形传播将固定四面体模板变形至目标表面。

Result: 在表面网格精度和体网格保真度上，该方法相比于其他基于变形的基线方法取得了持续的性能提升。

Conclusion: HeartVolMesh有效解决了拓扑一致性和薄壁结构重建的难题，为计算机模拟临床试验提供了高保真的心脏体网格。
...
### [328] [Perceiving Better Moments: Cover Frame Reselection and Enhancement for Live Photos with the Live2K Dataset](https://arxiv.org/abs/2607.04151)
*Junyu Lou,Kai Chen,Weiyi You,Hui Zeng,Lei Zhang,Shuhang Gu*

Main category: cs.CV

TL;DR: 本文定义了实况照片封面帧重选与增强（LPRE）任务，并提出了首个数据集和基线模型以消除视频帧与封面图之间的质量差距。


<details>
  <summary>Details</summary>
Motivation: 智能手机Live Photo中高质量封面图与低质量视频帧之间存在显著画质差距，导致用户重选封面帧时图像质量严重退化。

Method: 定义了LPRE任务，构建了包含2,042个实况照片的Live2K数据集，并开发了集成多帧融合、引导色彩增强和超分辨率的一阶段基线网络。

Result: 成功建立了首个Live Photo增强研究的基准，有效结合空间、色彩及时间线索提升了重选帧的质量。

Conclusion: 本研究为解决实况照片封面重选的画质退化问题提供了系统性的任务定义、数据集和有效基准。
...
### [329] [Beyond Scene Priors: Fine-Grained Traffic Scene Reasoning with Benchmarking and Query-Guided Small-Object Focus](https://arxiv.org/abs/2607.04149)
*Waikit Xiu,Qiang Lu,Zian Wang,Xinjie Yang,Zhiwei Chen,Chen Sun,Xiying Li*

Main category: cs.CV

TL;DR: 针对交通安全场景中多模态大模型易忽略微小关键视觉信息的问题，本文提出了细粒度推理基准FGTR-Bench和文本引导微小目标推理模型TSR-MLLM。


<details>
  <summary>Details</summary>
Motivation: 标准多模态大模型在复杂交通场景中容易过度关注背景，导致关键微小目标被稀释；同时，现有VQA数据集缺乏对这种局部证据定位能力的大规模评估。

Method: 引入包含约4.5万个多选问题的FGTR-Bench数据集；提出TSR-MLLM模型，利用查询条件下的文本引导微小目标关注（TG-SOF）机制，在解码端为关键视觉槽添加稀疏Top-K门控残差。

Result: TSR-MLLM在FGTR-Bench上以74.1%的准确率超越最强4B基线模型2.1个百分点，且在DriveQA-V上保持了极强的竞争力。

Conclusion: 该研究通过新数据集和无需外部检测器的端到端新架构，有效解决了交通推理中的“关键证据稀释”问题。
...
### [330] [Beyond Random Sampling: Distribution-Aware Alignment for Semi-Supervised Medical Image Segmentation](https://arxiv.org/abs/2607.04249)
*Weihao Yan,Yeqiang Qian,Yi Dong,Ming Yang*

Main category: cs.CV

TL;DR: 提出了一种基于分布对齐的高数据效率半监督医学图像分割框架，在极低标注比例下实现了优异的分割性能。


<details>
  <summary>Details</summary>
Motivation: 传统的半监督分割方法依赖随机采样，在低数据量场景下会导致严重的表征偏差，无法有效捕获异构的医学数据流形。

Method: 引入了基于视觉基座模型（VFM）和Density-K-Center算法的分布感知样本选择策略，以及用于跨域对齐的记忆导向复制粘贴（MCP）模块，并配合由易到难的渐进式调度以减少伪标签噪声。

Result: 在6个不同的2D和3D医学图像数据集上，该方法展示了强大的分割性能，特别是在极低标注比例（如1/16）下表现突出。

Conclusion: 通过结合主动样本选择与语义一致性对齐，该框架有效缓解了数据分布偏差和伪标签噪声，显著提升了半监督医学图像分割的数据利用效率。
...
### [331] [AdaptiveSplat:Texture Aware Controllable 3D Gaussian Allocation for Feed-Forward Reconstruction](https://arxiv.org/abs/2607.04256)
*Badrinath Singhal,Srihari K G,Sreehari Iyer,Ankit Dhiman,Venkatesh Babu Radhakrishnan*

Main category: cs.CV

TL;DR: 提出一种结合局部纹理信息的自适应高斯剪枝与属性预测方法，在保持前向传播效率的同时减少3D重建中的冗余高斯基元。


<details>
  <summary>Details</summary>
Motivation: 当前前向3D重建方法生成的高斯基元存在高度冗余，而传统剪枝方法会导致严重伪影且需要耗时的推断时微调。

Method: 利用局部纹理信息控制高斯数量，包含三个核心组件：用于捕获空间细节变化的纹理估计、移除低频区域冗余高斯的纹理感知剪枝，以及预测保留基元属性的自适应高斯头。

Result: 在RE10K、ACID、DL3DV、Tanks and Temples和DTU等多个数据集上的实验和消融研究证实了该方法的有效性。

Conclusion: 该方法在不破坏前向传播范式的前提下，通过结合纹理特征成功实现了高效且高质量的3D高斯表示剪枝。
...
### [332] [AquaStereo: Enabling Underwater Stereo Matching via Depth-Conditioned Diffusion and Geometry Self-Distillation](https://arxiv.org/abs/2607.04303)
*Qizhe Wei,Yingping Liang,Shaodi You,Ying Fu*

Main category: cs.CV

TL;DR: 提出AquaStereo框架，通过数据模拟、自蒸馏和时序特征融合，有效解决了水下双目匹配中数据匮乏和特征退化的问题。


<details>
  <summary>Details</summary>
Motivation: 水下双目匹配面临两个主要挑战：缺乏真实的水下训练数据，以及水下图像退化（浑浊、低纹理）导致难以提取有效的匹配对应关系。

Method: 1) 利用深度条件扩散管道生成保持几何一致性的水下双目图像；2) 采用自蒸馏策略，通过干净图像上训练的教师模型指导水下退化图像下的学生模型学习；3) 引入可学习感知帧，融合辅助视图的时序线索与语义特征以增强特征稳定性。

Result: 实验表明，该方法在具有挑战性的水下场景中显著提升了双目匹配的鲁棒性和零样本泛化能力。

Conclusion: AquaStereo通过协同数据仿真和感知增强，为解决水下双目立体匹配难题提供了一种有效的泛化方案。
...
### [333] [EMPURPLE: A Free Lunch for Diffusion Distillation based on the Information Bottleneck](https://arxiv.org/abs/2607.04276)
*Zilai Li,Lujia Bai*

Main category: cs.CV

TL;DR: 提出了一种名为EMPURPLE的免训练方法，通过回收原始模型的中间潜变量，显著改善了蒸馏扩散模型的图像生成多样性和FID指标。


<details>
  <summary>Details</summary>
Motivation: 扩散模型蒸馏虽然加速了采样速度，但往往会导致生成图像的FID指标退化（多样性降低），本研究旨在分析其背后成因并予以解决。

Method: 通过PAC泛化界限分析指出早期步骤的强力重定向导致了中间潜变量的分布失配，并据此设计了免训练的中间潜变量回收方法EMPURPLE。

Result: EMPURPLE具有模型无关性，在DMD2、Hyper-SD、FlashSD和SDXL-Lightning上将FID指标提升了7%至20%。

Conclusion: 解决蒸馏过程中中间潜变量的分布不一致问题是提升生成质量的关键，且该问题可以通过简单的无训练回收机制得到有效缓解。
...
### [334] [Road-Aware Anomaly Segmentation with Query-Guided Polygons and CLIP in Autonomous Driving](https://arxiv.org/abs/2607.04304)
*Zhiran Yan,Gordon Elger*

Main category: cs.CV

TL;DR: 一种无需重训的轻量级后处理道路异常分割框架，结合空间先验与语义验证来检测自动驾驶中的未知物体。


<details>
  <summary>Details</summary>
Motivation: 传统语义分割模型在闭集假设下工作，难以识别开放世界自动驾驶中未知的道路异常（OOD物体），带来安全隐患。

Method: 利用掩码生成网络中的查询级置信度与多边形道路先验定位异常区域，并结合基于CLIP的零样本语义过滤模块来抑制误报。

Result: 在Fishyscapes、SMIYC和RoadAnomaly基准上表现优异，超越了Maskomaly基线，并在Fishyscapes LostAndFound上取得最高AP。

Conclusion: 该方法无需重新训练、OOD数据或辅助监督，具有极高的实用性与可部署性，有助于提升自动驾驶系统的安全性。
...
### [335] [Aura: Consistent Multi-Subject Video Generation via VLM-Grounded Semantic Alignment](https://arxiv.org/abs/2607.04311)
...
TL;DR: 本文提出了Aura，一个用于高保真和身份一致性视频生成的统一框架，有效解决多主体视频生成问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在保持身份一致性以及建模多主体间的复杂关系方面存在不足。

Method: 引入AI导演级描述，利用带可学习查询的VLM提取特征，设计两阶段对齐策略、主体感知RoPE-Shift机制、主体感知Token、内存Token及推理期的Progressive-APG，并构建了专用数据集。

Result: 在单主体和多要素视频生成任务中均取得了最先进（SOTA）的性能表现。

Conclusion: Aura通过一系列创新的对齐与控制机制，显著提升了可控视频生成中的身份一致性和多主体交互建模能力。
...
### [336] [Last-Meter Precision Navigation for UAVs: A Diffusion-Refined Aerial Visual Servoing Approach](https://arxiv.org/abs/2607.04352)
*Yaxuan Li,Jiarui Zeng,Shaofei Huang,Zhedong Zheng*

Main category: cs.CV

TL;DR: 提出DreamNav框架，通过粗到精的双阶段视觉伺服（结合三角参数回归与扩散世界模型预测）以及PairUAV数据集，实现了高精度、强泛化性的无人机最后一米视觉自主导航。


<details>
  <summary>Details</summary>
Motivation: 单目视觉引导的无人机最后一米精准导航面临尺度模糊、旋转不连续以及对未知环境泛化性差等挑战，现有方法在大视角变化下容易失效。

Method: 提出DreamNav框架：第一阶段通过三角参数化回归策略预测旋转以减缓优化不稳定性；第二阶段利用预训练世界模型模拟候选动作的未来视觉观测，通过视觉想象选择与目标偏差最小的轨迹。此外，构建了包含480万图像对的PairUAV基准数据集。

Result: 实验表明，DreamNav在准确性和泛化性上均显著优于现有的强视觉伺服和基础模型基线，并实现了对未知场景的零样本迁移。

Conclusion: DreamNav和PairUAV数据集为解决无人机最后一米单目精密导航中的旋转断裂、视角变化和泛化性问题提供了高效的解决方案。
...
TL;DR: 本文提出了首个用于道路施工区域检测与地理定位的RZDG数据集，并开发了基于跟踪器的定位算法，有效解决了高精地图无法实时更新半静态路况的问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶所依赖的高精地图缺乏对道路施工等半静态信息的实时更新，且领域内缺乏公开的评估数据集。

Method: 提出了包含真实和模拟多模态数据的RZDG数据集，并设计了基于AB3DMOT扩展的道路施工区检测与地理定位管线，实现局部到全球坐标的转换。

Result: 实验表明算法在真实和模拟数据上均表现良好，1米误差内真实数据的F1分数为0.597，模拟数据的F1分数为0.665。

Conclusion: 该研究提出的数据集和定位管线能有效检测并定位道路施工区域，为自动驾驶系统提供了可靠的半静态障碍物感知方案。
...
### [338] [The Good, the Bad, and the Brittle: Benchmarking Robustness and Generalisation of Histopathology Foundation Models](https://arxiv.org/abs/2607.04401)
*Dhyey Yajnik,Amina Asif,Fayyaz Minhas*

Main category: cs.CV

TL;DR: 评估了12个病理基础模型的鲁棒性与泛化能力，发现模型参数规模的扩大呈现边际效应递减，未来的临床应用需更关注数据质量和多模态融合。


<details>
  <summary>Details</summary>
Motivation: 探究病理基础模型（PFMs）是否具备足够的临床鲁棒性与泛化性，以及它们的参数规模缩放是否已达到极限。

Method: 使用鲁棒性评估与增强工具箱（REET）在11种临床扰动下测试12个PFMs和ResNet基线，并结合非冗余K折交叉验证（NR-Kfold）和扰动性能指数（PPI）进行评估。

Result: PFMs在鲁棒性和泛化性上均优于传统CNN，但模型缩放收益递减，中等规模模型表现出与大型模型相当或更优的韧性；当训练与测试集相似性降低时，模型准确率会出现系统性下降。

Conclusion: 下一代病理基础模型应优先提升数据质量、多模态信息和领域对齐，而非盲目增加参数量，以实现真正的临床可靠性。
...
### [339] [Event Detection in Videos: A Framework for the Development of New Methods](https://arxiv.org/abs/2607.04372)
...
TL;DR: 本文针对视频事件检测领域中缺乏统一评估标准的问题，提出了一个基于数据集、性能评估和部署场景三大支柱的系统性开发框架。


<details>
  <summary>Details</summary>
Motivation: 现有视频事件检测算法缺乏大规模数据集和严格的性能评估方法，导致算法之间的对比和发展存在偏差。

Method: 提出了一个包含数据集、性能评估及部署场景三个核心要素的视频事件检测方法开发框架。

Result: 构建了一个规范化的框架，使研究人员能够更好地定位和评估其在复杂视频事件检测中的工作。

Conclusion: 该框架有助于推动视频事件检测方法的标准化发展和客观评估。
...
### [340] [CCFM: Collision-Constrained Flow Matching for Safety-Critical Scenario Generation](https://arxiv.org/abs/2607.04451)
*Ke Li,Kaidi Liang,Yuxin Ding,Debojyoti Biswas,Xianbiao Hu,Ruwen Qin*

Main category: cs.CV

TL;DR: 本文提出了CCFM框架，通过引入硬性物理约束，实现了在自动驾驶仿真中精确、可控地生成安全关键的碰撞场景。


<details>
  <summary>Details</summary>
Motivation: 现有的自动驾驶安全关键场景生成方法多采用软引导，无法保证特定碰撞类型在几何与严重程度上的约束，难以生成高可控性的碰撞场景。

Method: 引入碰撞约束流匹配（CCFM）框架，包含启发式碰撞选择器、定义四种碰撞类型的结构化硬约束，以及基于高斯-牛顿流形投影的流匹配采样器。

Result: 在nuScenes和nuPlan数据集上分别实现了46.4%和83.1%的碰撞率，在显著超越基线方法的同时，保持了合理的驾驶行为真实性。

Conclusion: CCFM通过实现安全关键场景生成中的可控碰撞特性，为自动驾驶的安全评估和虚实结合的碰撞数据生成奠定了可靠基础。
...
### [341] [Fields of the Planet: Field Boundary Mapping Beyond 10m](https://arxiv.org/abs/2607.04449)
*Isaac Corley,Caleb Robinson,Jennifer Marcus,Hannah Kerner*

Main category: cs.CV

TL;DR: 本文引入了3米高分辨率的Fields of the Planet (FTP)数据集，显著提升了小农户农田边界提取的精度。


<details>
  <summary>Details</summary>
Motivation: 传统的10米分辨率Sentinel-2图像空间分辨率不足，难以有效识别和分割面积较小的小农户农田地块。

Method: 构建了包含24个国家、超13万个PlanetScope图像对的FTP数据集，并通过将预测结果矢量化来评估全景质量（PQ）、物体F1值及边界误差等指标。

Result: 使用3米分辨率图像使整体全景质量（PQ）从21.0提升至35.5，小于0.5公顷小地块的PQ从5.8提升至15.7，且边界匹配误差从18.6米降至7.4米。

Conclusion: 高分辨率卫星数据能大幅提升小农田边界划分的准确性，对于精细化农业监测和产量评估具有重要价值。
...
### [342] [Spatial Graph Representation and Morphometric Analysis of the Pulmonary Vascular Tree From Computed Tomography Using Multi-Scale Hessian-Based Filter Fusion and TEASAR Skeletonization](https://arxiv.org/abs/2607.04457)
*Piotr Mackiewicz,Jakub Kołyska,Radoslaw Roszczyk*

Main category: cs.CV

TL;DR: 提出了一种无需训练、可解释的确定性管道，用于从CT图像中重建具有几何合理性的肺血管树。


<details>
  <summary>Details</summary>
Motivation: 传统的深度学习分割方法依赖大量标注数据，且临床CT图像受限于空间分辨率、伪影以及血管跨越的多尺度特性，迫切需要一种无需训练且可解释的重建方法。

Method: 融合多尺度Hessian（Frangi和Sato）血管滤镜进行血管检测；通过阈值化和Chan-Vese活动轮廓分割肺实质；采用TEASAR算法提取血管中心线并构建血管图；使用分形维数、Strahler级序、Horton比和Murray定律评估几何合理性。

Result: 重建的血管树分形维数约为2.3，与已知的人类肺血管物理特性一致，同时分支度量的偏差有效反映了由CT分辨率限制引起的末端血管截断现象。

Conclusion: 该方法能够生成符合几何生理特征的肺血管树模型，可为定量肺部成像、血管形态分析及计算肺部建模提供有效支持。
...
### [343] [Flash-BoN: Instant Drafts for Inference-Time Scaling in Diffusion Models](https://arxiv.org/abs/2607.04461)
*Ruchit Rawal,Reza Shirkavand,Sayak Paul,Yuxin Wen,Heng Huang,Yizheng Chen,Tom Goldstein,Gowthami Somepalli*

Main category: cs.CV

TL;DR: 论文提出了 Flash-BoN，一种通过快速生成大量低成本草稿候选并进行高效筛选的文本到图像生成推理加速与优化方法。


<details>
  <summary>Details</summary>
Motivation: 现有的引导搜索方法忽略了验证器的计算开销，导致在实际时间预算下效率不如简单的 Best-of-N (BoN) 采样，因此需要更高效的候选生成和验证机制。

Method: 结合时间步截断、层跳过和激活代理三种加速技术生成低成本草稿候选池，再通过多阶段验证筛选出最佳草稿进行最终的高质量精炼。

Result: 在固定时间预算下，Flash-BoN 在多项基准上显著超越基线（大模型上 AUC 提升 8%），与提示词优化结合可提升 16% AUC，并能加速 RL 后训练收敛。

Conclusion: 在推理时间缩放中，将计算资源用于扩大候选多样性（生成更多廉价草稿）比进行昂贵的中间步骤验证更为有效。
...
### [344] [TrustCLIP: Learning Private Visual Features via Adversarial Reconstruction](https://arxiv.org/abs/2607.04484)
...
TL;DR: 本文提出了 TrustCLIP 框架，通过对抗生成重构攻击来保护图像特征的隐私，同时保持下游任务的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉和视觉语言模型的高级特征容易被生成式模型逆向重构，从而带来严重的隐私泄露风险。

Method: 提出 TrustCLIP 框架，将生成重构器视为显式对手，通过学习一种特征投影来主动降低重构图像的质量，同时保留下游任务所需的关键信息。

Result: 在图像分类和多模态大语言模型任务中，TrustCLIP 均能显著降低生成式逆向重构的图像保真度，且对下游任务的性能几乎没有影响。

Conclusion: TrustCLIP 证明了直接针对生成重构攻击进行优化是保护特征隐私的有效途径，成功在隐私防御和任务实用性之间取得了平衡。
...
### [345] [EVAS: Efficient Multimodal Temporal Forgery Localization via Audio-Visual Synergy and Steered Boundary Calibration](https://arxiv.org/abs/2607.04472)
*Shen Shen,Quan Zhang,Dan Jiang,Ke Zhang*

Main category: cs.CV

TL;DR: 本文提出了EVAS，一种用于长视频中细粒度时序伪造定位的端到端多模态框架。


<details>
  <summary>Details</summary>
Motivation: 在长视频中精准定位稀疏且隐蔽的伪造片段非常困难，尤其是当跨模态信号微弱且在时间上弥散时。

Method: 设计了多阶段视听协同机制、边界感知细化策略（引入无效帧掩码）、解耦训练范式以及轻量级HourglassFFN。

Result: 在三个基准数据集上均取得了最先进的（SOTA）平均定位准确率和平均召回率，并降低了计算开销。

Conclusion: EVAS框架能有效捕获稀疏篡改的高阶语义痕迹并精准校准伪造边界，是解决细粒度时序伪造定位的有效方案。
...
### [346] [Enhancing Facial Expression Recognition in Head-Mounted Displays with Synthetic Data](https://arxiv.org/abs/2607.04490)
*Jianing Deng,Qiang Zhou,Jingtong Hu*

Main category: cs.CV

TL;DR: 本文提出了一种将正面图像合成头戴式摄像头视角图像的数据合成框架，以解决混合现实中面部表情识别数据匮乏的问题。


<details>
  <summary>Details</summary>
Motivation: 头戴式显示器环境下的表情识别面临头戴式摄像头数据稀缺、隐私限制及视角独特导致现有数据集无法直接应用的问题。

Method: 提出一种基于正面图像重建3D纹理网格并渲染HMC视角的研究框架，并引入纹理空间对齐网络（TSAN）以保留面部表情细节。

Result: 实验表明，使用该合成数据集训练的模型性能优于现有数据集训练的模型，并在不同相机配置下展现出更强的泛化能力。

Conclusion: 该方法有效缓解了头戴式设备表情识别的数据短缺问题，为跨设备应用提供了可行的解决方案。
...
### [347] [A non-invasive video-based method for individual identification of wildlife using gait dynamics](https://arxiv.org/abs/2607.04518)
*Muhammad Aamir,Matthew Wijers,Sangyun Shin,Andrew Loveridge,Andrew Markham*

Main category: cs.CV

TL;DR: 本文提出了一种基于视频深度时空表征学习的野生动物步态分析与个体识别自动流水线。


<details>
  <summary>Details</summary>
Motivation: 步态分析在野生动物识别中的应用受到环境多变性和缺乏可扩展识别方法的限制，亟需无创且高效的个体识别技术。

Method: 利用SAM3生成图像掩码以隔离复杂背景，结合ResNet18提取空间特征与VideoPrism进行时间运动建模，并通过余弦相似度对提取的步态特征进行无标记聚类。

Result: 在多物种野生动物视频数据上的实验表明，该方法具有极强的个体内部一致性和清晰的个体间区分度，且得到了量化指标的证实。

Conclusion: 步态动态是野生动物无创个体识别的一种可行途径，展示了基于视频的深度学习流水线在生态监测中的应用潜力。
...
### [348] [QSVideo: Query-Conditioned Semantic Temporal Retrieval for Video Understanding](https://arxiv.org/abs/2607.04559)
*Wei Ao,Lan Wang,Vishnu Naresh Boddeti*

Main category: cs.CV

TL;DR: 提出了QSVideo框架，通过优化检索的相关性、多样性和时序建模，显著提升了视觉语言模型（VLM）在长视频和流式视频理解中的表现。


<details>
  <summary>Details</summary>
Motivation: 长视频中与查询无关的画面会干扰VLM的理解，且现有的检索方法存在相关性偏见、多样性不足和时序崩溃的问题。

Method: 设计了QSVideo框架，包含用于多维度相关性评估的QSRanker、联合优化相关性与多样性的QSRetrieval，以及针对长视频和流式视频的时序对齐策略。

Result: 在长视频和流媒体视频基准测试上，QSVideo在严格的帧数限制下大幅提升了视频VLM的性能。

Conclusion: QSVideo系统性地解决了视频检索中的核心瓶颈，是增强视频VLM理解能力的有效多模态检索方案。
...
### [349] [Geographic Diversity Beats Data Volume for Cross-Domain Generalization in Zero-Label JEPA Driving World Models](https://arxiv.org/abs/2607.04500)
*Santosh Jaiswal*

Main category: cs.CV

TL;DR: 研究表明，在自动驾驶世界模型中，地理多样性对于跨区域零样本泛化至关重要，其提升效果优于单纯增加单一区域的数据量。


<details>
  <summary>Details</summary>
Motivation: 探究在特定地理区域训练的自监督潜在世界模型，能否在零样本情况下泛化到未见过的城市和传感器配置。

Method: 使用基于 JEPA 的世界模型，在 nuPlan 数据集（多城市）上进行训练，并在 Argoverse 2 数据集（未见城市）上进行零样本评估，对比单一与多地理区域训练及不同数据量的效果。

Result: 多地理区域混合训练比单一区域训练的平均惊奇度分数降低了 16.5%；即便单一区域的数据量是混合区域的 3 倍，其泛化表现依然不及混合区域训练的模型。

Conclusion: 地理多样性是预测自动驾驶世界模型跨领域泛化能力的关键因素，其重要性超越了单一源数据量的规模。
...
### [350] [RAF: Reliability-Aware Fusion of Camera, LiDAR, and 4D RADAR for Robust 3D Object Detection in Adverse Weather](https://arxiv.org/abs/2607.04587)
*Heejun Park,Jaeseok Jeong,Kuk-Jin Yoon*

Main category: cs.CV

TL;DR: 本文提出了RAF（可靠性感知融合）框架，通过显式监督相机图像的每像素可靠性，增强了恶劣天气下LiDAR-RADAR-Camera多模态3D目标检测的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在恶劣天气中，LiDAR和RADAR传感器信号稀疏，而相机图像易受雨雪遮挡干扰，且现有的多模态融合方法缺乏对图像区域可靠性的直接监督。

Method: 引入RAF框架显式监督每像素的可靠性估计；保持预训练的LiDAR-RADAR骨干网络冻结，仅训练新增的相机分支、BEV融合编码器和检测头。

Result: 在K-Radar和VoD数据集上，该方法相比LiDAR-RADAR基线取得了显著的精度提升，分别获得高达+6.5 AP_BEV和+7.4 AP_3D的增长。

Conclusion: 通过显式监督并抑制不可靠的视觉线索，可以有效提升恶劣天气下多传感器融合3D目标检测的准确性与鲁棒性。
...
### [351] [Displacement Preserving Relational Distillation for Robust Medical Segmentation](https://arxiv.org/abs/2607.04599)
*Zhicheng Ding,Xinyu Chu,Jung Im Choi,Qing Tian,Tianyu Shi,Xiaoqian Jiang,Lijing Zhu,Qizhen Lan*

Main category: cs.CV

TL;DR: 提出了一种名为DPRD的3D医学图像分割知识蒸馏方法，仅用教师模型约5%的参数和3%的计算量，即可在保持高结构一致性的同时达到极高分割性能。


<details>
  <summary>Details</summary>
Motivation: 3D医学图像分割受限于解剖结构变异和高计算成本。传统的知识蒸馏方法难以保留复杂结构，且易受背景噪声干扰。

Method: 提出位移保持关系蒸馏（DPRD），通过基于向量的对齐来蒸馏潜在的解剖轨迹，并将蒸馏锚定在与任务相关的结构中以防信号稀释。

Result: 在ISLES 2022和AMOS 2022基准上超越了现有方法。在AMOS数据集上实现了85.46%的Dice分数，接近大容量教师模型，且参数量仅为~5%，FLOPs仅为~3%。

Conclusion: 该方法为在资源受限的临床环境中部署高性能医学图像分割模型提供了一种鲁棒且高效的解决方案。
...
### [352] [Integrated Forward-Inverse Network for Lensless Image Reconstruction](https://arxiv.org/abs/2607.04608)
*Donggeon Bae,Jaewoo Jung,Yong Guk Kang,Kyung Chul Lee,Taeyoung Kim,Jongho Kim,Sangjun Byun,Joonsik Park,Seung Ah Lee*

Main category: cs.CV

TL;DR: 本文提出了一种名为IFIN的物理引导神经网络，通过交织正向与逆向过程，显著提升了无透镜成像等计算摄像任务的重建质量。


<details>
  <summary>Details</summary>
Motivation: 无透镜成像虽结构紧凑，但其大范围的点扩散函数导致逆问题严重不适定，且现有深度学习方法难以在网络层级中显式维持数据保真度。

Method: 提出了集成正向-逆向网络（IFIN），在每个尺度上交织可微的正向投影与可学习的逆向更新，实现双向耦合与物理一致的渐进式细化。

Result: 在无透镜成像基准测试及新数据集上取得了最先进的重建效果，并在高斯去模糊和模拟同轴全息重建中表现出优异的泛化性能。

Conclusion: 双向耦合的设计支持物理一致的图像细化与系统约束下的核自适应，表明该交织原理可推广至无透镜成像之外的广泛领域。
...
### [353] [Aperture-aware Dispersion 5-D Light-field Imaging Spectrometer](https://arxiv.org/abs/2607.04635)
*Chenglong Huang,Tao Lv,Jianing Yang,Chongde Zi,Linsen Chen,Xun Cao*

Main category: cs.CV

TL;DR: 提出了一种孔径感知色散光场成像光谱仪（ADLIS），利用双折射晶体相位片实现全空间分辨率的5D光谱光场成像。


<details>
  <summary>Details</summary>
Motivation: 传统的5D光谱光场获取设备体积庞大且昂贵，而现有的单探测器系统面临不同物理维度分辨率相互妥协的局限。

Method: 引入ADLIS系统，采用低成本的双折射石英晶体相位片进行孔径多路复用调制（角-谱编码），并开发了端到端（E2E）联合优化孔径设计与重建的ADLI算法框架。

Result: 通过仿真训练和真实实验验证，该系统在保持全空间分辨率的同时，实现了高鲁棒性、高性能的5D光谱光场重构。

Conclusion: 该研究通过将设计范式从“空间分割”转变为“编码集成”，有效解决了成像系统紧凑性与多维高分辨率之间的矛盾。
...
### [354] [Enhancing Large Multimodal Models in Key Information Extraction via Scene-Aware Document Synthesis](https://arxiv.org/abs/2607.04636)
*Zhipeng Xu,Zulong Chen,Qing Liu,Junhao Ji,Jinxin Hu,Yipeng Yu,Jianqiang Wan,Jun Tang,Zhao Li*

Main category: cs.CV

TL;DR: SAYRE是一个场景感知文档合成框架，通过自动生成高质量训练数据来提升本地部署多模态模型在关键信息抽取（KIE）任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 本地部署的轻量级模型缺乏足够的KIE监督数据，而高性能的云端多模态大模型部署成本又过高。

Method: 利用少量示例学习布局与内容模式以合成数据三元组，并引入误差驱动生成机制，将真实失败案例转化为针对性的困难训练样本。

Result: 显著提升了Qwen3-VL等底座模型在各类KIE任务中的表现，在端侧模型中取得领先性能，并有效减少了复杂表格和条款中的字段级抽取错误。

Conclusion: 场景感知文档合成被证实是一种行之有效的提升实用多模态KIE性能的数据中心（data-centric）方法。
...
### [355] [PixelPilot: Scalable Vision-Language-Action Models for End-to-End Autonomous Driving](https://arxiv.org/abs/2607.04637)
*Pin Tang,Guoqing Wang,Xiangxuan Ren,Zhongdao Wang,Guodongfang Zhao,Bailan,Chao Ma*

Main category: cs.CV

TL;DR: 本文提出了PixelPilot，一种解耦规划与提升的自动驾驶VLA模型，通过在图像平面进行2D规划并结合GRPO优化，提升了跨数据集的泛化与视觉推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA的2D到3D轨迹预测受限于相机参数导致数据扩展性差，且直接在3D空间优化容易使模型忽略视觉场景而依赖自车状态。

Method: 提出了解耦规划与提升的范式（PixelPilot），在图像平面进行传感器无关的2D-to-2D轨迹预测，并在推理时再投影至3D空间；同时使用GRPO通过密集中间奖励进行策略学习。

Result: 在开环和闭环评估中均达到了最先进的（SOTA）性能，验证了其优异的可扩展性和视觉推理能力。

Conclusion: 解耦的规划与提升范式结合密集强化学习奖励，能有效解决自动驾驶中VLA的数据缩放与视觉理解退化问题。
...
### [356] [Learning Flexible Generalization in Video Quality Assessment by Bringing Device and Viewing Condition Distributions](https://arxiv.org/abs/2607.04643)
*Nikolay Safonov,Dmitriy S. Vatolin*

Main category: cs.CV

TL;DR: 本文研究了移动设备在不同观看条件下的视频质量评估，并提出了一个结合设备与环境信息的大型数据集和预测方法。


<details>
  <summary>Details</summary>
Motivation: 现有的视频质量评估（VQA）未充分考虑移动端多变的观看条件和屏幕特性（如环境光、亮度、分辨率）对主观感知质量的影响。

Method: 构建了包含超300台Android设备及环境元数据的大型主观数据集，并提出了一种聚合评分提取策略以及适配具体设备的VQA模型调整方法。

Result: 实验结果表明，引入设备和上下文环境信息能实现更准确和灵活的视频质量预测。

Conclusion: 该工作推进了感知质量模型的发展，弥合了实验室评估与现实世界多样化消费条件之间的差距，为流媒体服务优化提供了新途径。
...
### [357] [Enhancing Video Physical Consistency via Role-aware Joint Training and Modality-decoupled Denoising](https://arxiv.org/abs/2607.04653)
*Guangting Zheng,Haojing Chen,Hao Li,Jingtao Zhang,Zhen Yang,Xiaosong Jia,Xue Yang,Shaofeng Zhang,Yanyong Zhang*

Main category: cs.CV

TL;DR: 提出VPT微调框架，通过角色感知信号和模态解耦去噪策略，在保持视频画质的同时显著提升其物理一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型难以保持长期的物理一致性，且现有引入辅助模态的方法存在未区分实体运动、多模态容量冲突以及推理阶段误差累积的问题。

Method: 引入角色感知信号将实体分类（智能体、受控/被动目标、背景）；采用模态解耦去噪策略及损失权重衰减，将辅助模态作为软约束；设计跨步自引导机制以增强物理动力学。

Result: 在VideoPhy基准上相比Wan2.1-T2V-1.3B实现了SA提升39.4%、PC提升17.9%的显著改进，并在VideoPhy-2上表现出持续的性能提升。

Conclusion: VPT框架通过细化物理角色建模和解耦多模态训练，有效解决了视频生成中的物理失真与推理误差累积问题。
...
### [358] [Learning Structured Visual Compositional Representations for Weakly Supervised Referring Expression Comprehension](https://arxiv.org/abs/2607.04638)
*Lian Xu,Mohammed Bennamoun,Farid Boussaid,Hamid Laga,Yulan Guo,Dan Xu*

Main category: cs.CV

TL;DR: 提出了一种结构化视觉组合表示（SVCR）框架，通过显式建模物体及其相互关系，提升了弱监督指代指称理解的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的弱监督方法将关系隐式编码在单一锚框特征中，导致视觉表示扁平化，无法与结构化的自然语言有效对齐。

Method: 提出SVCR框架，显式建模一元物体嵌入和双标关系嵌入以构建结构化视觉空间，并引入组合对齐机制，在弱监督下实现图文的结构化匹配。

Result: 在RefCOCO、RefCOCO+和RefCOCOg三个基准数据集上均取得了最先进的（SOTA）性能。

Conclusion: 显式的结构化视觉表示和视觉-文本组合对齐对于弱监督指代指称理解（WREC）非常有效。
...
### [359] [DiCE-CIR: Direct Composition Learning for Efficient Zero-Shot Composed Image Retrieval](https://arxiv.org/abs/2607.04665)
*Gwang-Ho Na,Ho-Joong Kim,Seong-Whan Lee*

Main category: cs.CV

TL;DR: 本文提出了一种名为DiCE-CIR的直接组合学习方法，用于解决零样本组合图像检索（ZS-CIR）中现有方法效率低下和训练推理不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于投影的方法需要额外的投影和重编码步骤，增加了训练复杂度，降低了效率，并引入了训练与推理之间的偏差。

Method: 利用大语言模型从大规模图像-文本对中自动构建无标注的组合训练样本，并训练一个轻量级的直接组合模块，以促进目标对齐、编辑一致的语义转换和检索辨别力。

Result: 在CIRCO基准上取得了最先进的（SOTA）性能，在CIRR上达到了极具竞争力的表现，同时保持了极高的计算效率。

Conclusion: DiCE-CIR通过免去复杂的投影步骤，实现了一种高效且高精度的零样本组合图像检索方案。
...
### [360] [GlaKG: A Biomarker-Centric Fundus Knowledge Graph for Explainable Glaucoma Diagnosis and Risk Assessment](https://arxiv.org/abs/2607.04673)
*Cheng Huang,Jia Zhang,Yi Jiang,Yang Liu,Karanjit Kooner,Yadi Liu,Tsengdar Lee,Yang Xie,Wenqi Shi,Guanghua Xiao*

Main category: cs.CV

TL;DR: 提出了一种基于生物标志物的眼底知识图谱（GlaKG），为青光眼诊断和风险分层提供具有临床可解释性的可追溯推理链。


<details>
  <summary>Details</summary>
Motivation: 现有的青光眼自动诊断系统主要依赖不透明的深度学习模型，缺乏临床可解释性。

Method: 构建了融合6种实体、8种关系和11条临床规则的知识图谱 GlaKG，并采用后处理融合框架，将 ResNet50 图像嵌入与归一化知识图谱推理链评分相结合。

Result: 在公开数据集上，二分类青光眼诊断的 F1 值达到 0.9953，四分类风险分层准确率达 0.930；知识图谱特征与生物标志物特征的贡献度接近（51.1% 对 48.9%），且能通过低评分识别边缘病例。

Conclusion: GlaKG 提供了一个临床可审计的推理框架，通过显式展示决策背后的生物标志物证据和规则激活，有效提升了诊断系统的透明度与可信度。
...
### [361] [Reference-Induced Consensus for Selective Posed-Reference Visual Localization](https://arxiv.org/abs/2607.04722)
*Wonseok Kang,Jaehyun Kim,Jeongmin Lee,Tae-Wan Kim*

Main category: cs.CV

TL;DR: RIC-Loc 是一种无需场景训练和 SfM 三维地图点的相机定位方法，利用已知参考帧位姿进行鲁棒一致性估计。


<details>
  <summary>Details</summary>
Motivation: 旨在克服传统定位方法对场景特定训练、预建 SfM 3D 地图或复杂 2D-3D 匹配的依赖。

Method: 使用冻结的 VGGT 预测局部相机位姿、深度及匹配轨迹，通过参考帧诱导 SE(3) 位姿假设并进行一致性估计，同时设计了空间色散和协方差两种评分进行故障检测。

Result: 在多个室内外及低纹理基准测试中，无需场景训练即可实现与结构化定位方法相当的精度，且其故障检测机制显著优于传统的检索评分方法。

Conclusion: RIC-Loc 为基于参考位姿的相机定位提供了一种无需每场景训练的高效、鲁棒的选择性运行方案。
...
### [362] [LILAC: Layer-Wise Independent LoRAs and Cascaded Conditioning for Multi-Concept Customization of Diffusion Models](https://arxiv.org/abs/2607.04801)
*Marian Lupascu,Sebastian Ripa,Mihai Trascau,Mariana-Iuliana Georgescu,Ionut Mironica*

Main category: cs.CV

TL;DR: LILAC通过在推理阶段将独立训练的低秩适配器组合为独立的图像层，解决了多角色文本生成图像中的身份混淆问题，且无需联合训练。


<details>
  <summary>Details</summary>
Motivation: 现有的多主体图像个性化生成方法在共享权重空间中融合适配器时，容易导致身份混淆和风格混杂，且通常需要繁琐的联合重新训练。

Method: 提出LILAC框架，在推理时将适配器作为独立图层进行组合，让每个主体基于已生成主体的冻结复合图进行条件化生成，确保每次仅激活一个适配器以避免参数级干扰。

Result: 该方法无需联合训练即可线性扩展概念数量。在Qwen-Image-Edit上应用时，其ArcFace人脸检测率达到0.861，相比对比方法的0.745有显著提升。

Conclusion: 通过在推理期分层组合适配器而非在参数层面进行融合，可以有效消除多主体生成中的干扰，实现高效、免训练且适配多种骨干网络的多角色图像个性化生成。
...
### [363] [DGSeg: Dynamic Gating of Semantic-Spatial Guided Predictions for Reasoning Segmentation](https://arxiv.org/abs/2607.04779)
*Ruizhe Zeng,Siyu Cao,Lu Zhang,Zhiyong Liu*

Main category: cs.CV

TL;DR: 提出了DGSeg框架，通过解耦并动态融合语义和空间线索，提升了推理分割的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将推理简化为稀疏线索易引入噪声，且直接混合多种线索会导致错误信息干扰最终的分割结果。

Method: 使用MLLM生成互补的语义和空间线索并输入独立分支，随后利用基于相对分支质量监督训练的动态门控模块进行自适应融合。

Result: 在多个基准上表现优异，在ReasonSeg验证集和测试集上分别实现了69.6%和67.3%的gIoU。

Conclusion: 通过解耦和动态门控融合空间与语义线索，能有效减少推理分割中的歧义与噪声，提升模型性能。
...
### [364] [Hybrid Deep Learning for Traceability and Classification of Industrial Slate Tiles](https://arxiv.org/abs/2607.04811)
*Soren Antebi,Stefan Eickeler,Sandra Halscheidt,Rene Schmitz,Michael Muellers,Dirk Hecker,Rafet Sifa*

Main category: cs.CV

TL;DR: 提出一种轻量级混合深度学习方法，结合图像匹配与分类，用于板岩瓦的重识别和产地分类。


<details>
  <summary>Details</summary>
Motivation: 天然板岩瓦的视觉差异导致人工检测成本高且易出错，需提高工业生产效率和质检水平。

Method: 构建了一个集成XFeat特征匹配分支（结合LightGlue）与MobileNetV3分类分支的统一框架，并对双骨干网络的特征进行共享与融合。

Result: 实例匹配性能AUC提升了15.4%，分类准确率较标准MobileNetV3提升了10.9%，并在包含2,610张图像的新工业数据集上进行了有效验证。

Conclusion: 该方法证明了结合匹配与分类的混合模型在工业物体识别与分类任务中的有效性。
...
### [365] [DeGenseGS: Geometrically and Semantically Decoupled Surgical Scene Understanding in 4D Gaussian Splatting](https://arxiv.org/abs/2607.04761)
*Yimo Wang,Bin Kang,Shuojue Yang,Yueming Jin*

Main category: cs.CV

TL;DR: 本文提出了DeGenseGS，一种通过将几何变形与语义演化解耦来实现4D手术场景理解的新框架。


<details>
  <summary>Details</summary>
Motivation: 现有的4D重建方法将视觉语言模型与可变形场硬性耦合，导致手术场景中的语义信息与物理解剖结构之间存在严重的不对齐。

Method: 提出了基于HexPlane的时空纠缠模块以独立建模语义与几何，设计了光栅化原生语义提取机制以减少伪影，并引入了角对齐优化策略以防止语义失真。

Result: 在CholecSeg8k和EndoVis18数据集上的实验表明，该方法达到了最先进的性能，实现了更好的几何完整性和语义-解剖对齐。

Conclusion: DeGenseGS能够应对剧烈的组织变形和拓扑结构变化，实现空间连续的手术场景分割，为自主手术交互提供了支持。
...
### [366] [TGRIP: A Text-Guided Approach to Vehicle Instance Prediction in Autonomous Driving](https://arxiv.org/abs/2607.04812)
*Miguel Antunes-García,Santiago Montiel-Marín,Fabio Sánchez-García,Rodrigo Gutiérrez-Moreno,Rafael Barea,Luis M. Bergasa*

Main category: cs.CV

TL;DR: 本文提出了TGRIP，一种首次将视觉-语言模型生成的语义引导与未来实例预测相结合的自动驾驶框架。


<details>
  <summary>Details</summary>
Motivation: 现有的BEV实例预测主要依赖几何监督，缺乏语义感知，限制了模型在复杂场景下解析歧义和预测特定对象行为的能力。

Method: 采用教师-学生架构，利用视觉-语言基础模型从多相机图像中生成语义增强的BEV图，作为训练时的辅助监督来引导网络学习。

Result: 实验结果表明，TGRIP在nuScenes数据集上超越了现有的前沿（SOTA）模型。

Conclusion: 语义信息的引入对于实现鲁棒的端到端运动预测至关重要。
...
### [367] [HunyuanOCR-1.5: Making Lightweight OCR VLMs Faster and Better](https://arxiv.org/abs/2607.04884)
...
TL;DR: 本文介绍了 HunyuanOCR-1.5，一个轻量级端到端 OCR 视觉语言模型，在保持轻量架构的同时，显著提升了推理速度与多场景 OCR 能力。


<details>
  <summary>Details</summary>
Motivation: 旨在不改变模型骨干网络的前提下，系统性地降低轻量级 OCR 模型在处理长文本/结构化输出时的延迟，并增强其在长尾 OCR 任务上的处理能力。

Method: 采用 DFlash 优化 OCR 解码以加速推理；提出“智能体数据流（Agentic Data Flow）”自动构建数据以修复模型弱点；并升级了预训练与微调方案。

Result: 实现了 6.37 倍的 Transformer 推理加速和 2.14 倍的 vLLM 加速；在 OmniDocBench v1.6 上名列前茅，并在古籍 OCR、图表解析等长尾任务上取得新突破。

Conclusion: HunyuanOCR-1.5 证明了轻量级端到端模型在高效推理、广泛 OCR 能力覆盖及实际部署中的优势，并将开源以促进后续研究。
...
### [368] [Unsupervised Detection of Underground Tunnels in Ground-Penetrating Radar Using Depth-Restricted Reconstruction Scoring](https://arxiv.org/abs/2607.04882)
*Muhammad Junaid,Shoab A. Khan,Nisar Ahmed*

Main category: cs.CV

TL;DR: 本文提出了一种基于探地雷达（GPR）和无监督深度学习的地下偷盗隧道自动检测系统，无需隧道样本即可实现高精度检测。


<details>
  <summary>Details</summary>
Motivation: 传统的管道监测只能在损坏发生后做出反应，而现有的GPR人工识别难以规模化，且由于实际中隧道样本极其稀缺，导致监督学习方法无法应用。

Method: 使用去噪卷积自编码器（CAE）在无异常的地下雷达图上进行无监督训练，通过重构误差识别异常。提出一种“深度限制的top-k异常评分”方法，仅在隧道可能存在的物理深度范围内计算最大重构误差。

Result: 在包含1600个野外测试窗口的数据集上，系统实现了AUC 0.994、F1 0.975、召回率 0.973和精确率 0.976，误报率仅为1.6%，且在训练、评分和阈值校准中均未使用任何隧道标签。

Conclusion: 物理约束的无监督学习方法能够有效克服样本稀缺问题，实现高精度、低误报的地下隧道自动检测，适用于连续的管道走廊监控。
...
### [369] [PAGE: Towards Practical Human-level Gaze Target Estimation](https://arxiv.org/abs/2607.04860)
...
TL;DR: 本文提出了一种名为PaGE的视线估计模型，通过显式建模场景与头部的交互及知识蒸馏，实现了超越人类水平的视线目标估计性能，并支持轻量化部署。


<details>
  <summary>Details</summary>
Motivation: 现有视线估计模型在结合全局场景语义和人体空间推理上面临挑战，导致其性能无法达到人类水平，限制了实际应用。

Method: 提出显式建模场景与头部特征交互的PaGE模型，并采用教师-学生蒸馏策略，利用ViT-H+大模型作为教师，在无监督数据集上训练轻量级的学生模型。

Result: 在多项视线估计任务中达到SOTA，在9个指标中有7个超越了人类，其余2个指标将人机差距缩小了至少60%，且轻量化学生模型保留了大部分性能。

Conclusion: PaGE通过架构改进和创新的蒸馏训练方法，显著提升了视线估计性能，其轻量化版本非常适合部署在机器人和消费级设备上。
...
### [370] [ProCon: Projection-Consistency Memory for Training-Free Anomaly Detection](https://arxiv.org/abs/2607.04894)
*Joongwon Chae,Lihui Luo,Yang Liu,Dongmei Yu,Peiwu Qin,Runming Wang,Ilmoon Chae*

Main category: cs.CV

TL;DR: 本文提出了一种无需训练的异常检测框架ProCon，通过将传统的硬内存检索替换为软投影像重建，显著提升了缺陷定位与检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统的基于记忆库的方法采用硬检索（最近邻查找），容易受到假正常匹配的影响，且无法验证测试斑块是否得到局部正常邻域的稳定支持。

Method: 提出ProCon框架，将测试斑块软投影到邻近的正常记忆向量上，以投影残差作为异常依据；并通过构建扰动层记忆、中位数聚合残差以及层共识融合来稳定残差，无需任何模型训练或微调。

Result: 在MVTec-AD、VisA和Real-IAD数据集上，ProCon取得了极佳的图像级和像素级性能，图像级AUROC分别达到99.8%、99.2%和93.2%。

Conclusion: 研究表明，通过软正常投影替代硬检索，并结合内存与深度共识稳定残差，可以在无需训练的情况下有效提高异常检测的准确性与鲁棒性。
...
### [371] [Virtual Category-Guided Continual Generalized Category Discovery](https://arxiv.org/abs/2607.04984)
*Jiahui Xiong,Qiuxia Lai,Hongsong Wang*

Main category: cs.CV

TL;DR: 本文提出了一种虚拟类别引导的持续广义类别发现方法（C-GCD），通过虚拟类别学习和扩展邻域对比学习，有效解决了无标签流数据中模糊样本导致的伪标签噪声和类别偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有的持续广义类别发现方法在处理模糊的无标签样本时，容易产生错误的伪标签并偏向已知类别，导致学习不够鲁棒。

Method: 引入虚拟类别学习（VCL）将不确定样本分配给临时虚拟类别，并结合扩展邻域对比学习（ENCL）利用扩展邻域关系和自适应边际来增强表征的判别性。

Result: 在 CIFAR-100、Tiny ImageNet 和 ImageNet-100 数据集上的实验结果表明，该方法始终优于现有的最先进方法。

Conclusion: 该方法为持续广义类别发现提供了一种可扩展且有效的解决方案，成功缓解了预测偏差并提高了无标签数据的利用率。
...
### [372] [Geometry-aware Depth-guided Representation Learning for Structure-preserving Low-light Image Enhancement](https://arxiv.org/abs/2607.05005)
*Fang Gao,Jiongkai Qin,Jiabao Wang,Jingfeng Tang,Ming Cheng,Hanbo Zheng,Qingbao Huang,Cheng Wu*

Main category: cs.CV

TL;DR: 提出了一种深度引导的多尺度注意力网络（DMSA-Net），利用几何结构先验来增强低光照图像，并构建了LOL-D数据集。


<details>
  <summary>Details</summary>
Motivation: 现有的低光照图像增强方法主要关注外观恢复，而忽视了利用场景几何结构来保持结构一致性。

Method: 利用Retinex分解模块获取反射率表示并推导深度线索，然后在层次化编解码结构中嵌入多尺度深度引导融合策略，通过深度注意力融合几何与外观特征。

Result: 在多个基准数据集上的实验表明，该方法在有效恢复低光照图像的同时，显著提升了结构保持能力。

Conclusion: DMSA-Net能够实现几何感知的低光图像增强，构建的LOL-D数据集有助于推动相关领域的研究。
...
### [373] [UniSpine-GS: An Efficient Physics-Aware Gaussian Framework for Cross-Modality Multi-view Spine Image Synthesis](https://arxiv.org/abs/2607.04923)
*Qiuhua Chen,Changning Yu,Na Huang,Chao Sun,Bo Du*

Main category: cs.CV

TL;DR: 提出UniSpine-GS，一种高效且感知物理的3D高斯渲染框架，用于多视角脊柱图像的新视角合成，无需显式3D重建。


<details>
  <summary>Details</summary>
Motivation: 三维脊柱评估受限于高昂的三维成像硬件成本，以及不同成像模态间的物理差异对模型泛化性带来的挑战。

Method: 提出UniSpine-GS框架以学习几何感知的无显式重建高斯表示，并引入结构引导的损失重加权策略（SPWM）以提升边界与局部细节。

Result: 在CTSpine3D和新构建的胎儿超声数据集FeSpine3D上，该方法在所有评估指标上均显著优于现有技术。

Conclusion: UniSpine-GS为统一的多视角医学成像提供了一种实用且具有成本效益的解决方案。
...
### [374] [3DMPE: 3D Multi-Perspective Embedding](https://arxiv.org/abs/2607.04898)
*Vahan Huroyan,Md Rahat-uz-Zaman,Stephen Kobourov*

Main category: cs.CV

TL;DR: 本文提出了一种名为3DMPE的无需训练、基于优化的方法，用于从多个部分观测的2D投影中重建3D点云。


<details>
  <summary>Details</summary>
Motivation: 传统的基于学习的重建方法依赖于特定类别的训练数据，且在跨视图存在点缺失和不完整成对距离信息时难以重建一致的3D结构。

Method: 提出3D多视角嵌入（3DMPE）方法，扩展了多视角同时嵌入技术，在固定或可变投影设置下，通过优化算法联合重建3D点云并估计投影图。

Result: 在ShapeNet和Pix3D上的实验表明，该方法在Chamfer距离、EMD和ROA等指标上表现优异，并对噪声、不同视点数量和初始化表现出良好的鲁棒性。

Conclusion: 3DMPE能够仅依赖几何观测有效重建点云，无需类别特定的训练，为多视图三维重建提供了一种有效的免训练解决方案。
...
### [375] [Consistent and Editable: A Balanced Framework for Text-Guided Video Editing](https://arxiv.org/abs/2607.05056)
*Tao Jin,Li Xiao*

Main category: cs.CV

TL;DR: 提出了一种名为EquiEdit的视频编辑框架，通过时间Mamba模块和基于谱变换的噪声注入策略，协调并平衡了视频编辑中的时间一致性与可编辑性。


<details>
  <summary>Details</summary>
Motivation: 现有的文本引导视频编辑方法难以在时间一致性与可编辑性之间取得平衡，这两者在以往的研究中通常呈反比关系。

Method: 提出EquiEdit框架，包含：（1）时间Mamba模块，通过四向时间感知扫描增强帧间一致性；（2）基于谱变换（傅里叶变换）的噪声注入策略，保留初始潜噪声的隐藏结构，在提高编辑灵活性的同时保持对原视频的保真度。

Result: 定性和定量实验表明，该方法在提升视频的时间一致性和可编辑性方面非常有效，同时对输入视频保持了极高的保真度。

Conclusion: EquiEdit成功解决了视频编辑中一致性与可编辑性难以兼顾的权衡问题，实现了两者的协同提升。
...
### [376] [Unsupervised Pixel-Level Semantic Left-Right Understanding of In-the-Wild Images](https://arxiv.org/abs/2607.05006)
*Weikang Wang,Tobias Weißberg,Florian Bernard*

Main category: cs.CV

TL;DR: 提出了一种无监督学习框架，结合三维形状和二维图像，实现了单视图图像中高质量的像素级语义左右预测，并具有跨类别泛化能力。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏三维信息、遮挡、姿态变化和局部缺失等问题，自然图像中的像素级语义左右预测仍然具有挑战性。

Method: 利用无监督学习方法，联合三维形状数据集（主要为人类和四足动物）与多样化的自然图像，来推断单视图图像中的像素级语义左右对应关系。

Result: 在渲染和自然图像数据集上均取得了优于现有最先进方法的密集像素级预测性能，甚至能泛化到汽车或火车等未见过的三维物体类别。

Conclusion: 结合三维几何先验与二维图像的无监督方法能有效解决单视图语义左右预测问题，且表现出极强的泛化性能。
...
### [377] [TimeThink: Reasoning with Time for Video LLMs](https://arxiv.org/abs/2607.05089)
*Handong Li,Longteng Guo,Zikang Liu,Dongze Hao,Yepeng Tang,Zijia Zhao,Jie Jiang,Zhiwei Jin,Chen Chen,Haonan Lu,Jing Liu*

Main category: cs.CV

TL;DR: 本文提出了TimeThink，一个通过分步过程奖励来引导视频大语言模型（Video-LLMs）定位时间证据的强化学习框架。


<details>
  <summary>Details</summary>
Motivation: 现有的视频大模型RL训练主要依赖结果性奖励，缺乏对中间推理步骤中如何发现和定位时间线索的有效指导。

Method: 将时间线索步骤作为优化基元，引入分步时间过程奖励与联合过程-结果优化目标，并构建了包含自动推导时间证据段的TimeThink-RFT-20K数据集。

Result: 在视频推理、时间定位和通用视频理解基准测试中，TimeThink显著提升了定位与推理性能，达到了开源视频RL模型的先进水平。

Conclusion: 通过分步过程奖励显式引导时间证据的发现，能有效增强视频大语言模型的时空推理与定位能力。
...
### [378] [LangLoc: "Tell Me What You See"](https://arxiv.org/abs/2607.05077)
...
TL;DR: LangLoc 是一种利用自然语言描述进行室内细粒度定位的三阶段框架，结合了图注意力网络、几何投影评分和贝叶斯对话模块，实现中位数误差 0.95 米的精准定位。


<details>
  <summary>Details</summary>
Motivation: 基于自然语言的定位轻量且保护隐私，但现有工作仅停留在粗粒度的场景检索阶段，无法解决场景内的精确位姿（2D 位置和朝向）定位问题。

Method: 提出三阶段管线 LangLoc：1) 通过结合 CLIP 语义特征的双分支 GATv2 编码器检索正确场景；2) 利用光线投射物体可见性对密集地面网格评分以估计位姿；3) 通过贝叶斯对话模块提问以消除歧义。此外，贡献了包含 1.3 万个位姿索引描述的数据集。

Result: 场景检索 Top-1 召回率比前人方法提升了 8 个百分点，定位中位数误差达到 0.95 米，并能通过主动对话有效解决位置歧义。

Conclusion: LangLoc 成功填补了从自然语言到细粒度室内定位的空白，证明了结合三维几何推理与主动对话在解决语言定位歧义性方面的有效性。
...
### [379] [RADIANCE: Relative Adaptive Denoising with IP-Adapter for Novel Concept Enhancement](https://arxiv.org/abs/2607.05088)
*Zi-Xiang Ni,Bo-Lun Huang,Teng-Fang Hsiao,Bo-Kai Ruan,Hong-Han Shuai*

Main category: cs.CV

TL;DR: 本文提出了RADIANCE，一种无需训练的闭环反馈框架，通过在去噪过程中动态调整控制尺度，解决了文本生成图像模型在合成罕见组合概念时的缺失与失衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有的文本生成图像扩散模型在处理非寻常的属性-对象配对（罕见概念）时，常因去噪轨迹缺乏组合平衡而导致概念遗漏或语义漂移。

Method: 引入了三个模块：组合相似度监测器（CSM）用于监测中间状态，双向尺度控制器（BSC）用于施加恢复力调整适配器尺度，以及反馈引导调度器（FGS）进行跨时间步协调；并针对多对象场景设计了延迟适配器激活（DAA）和层级交替引导（LAG）。

Result: 在RareBench和T2I-CompBench上的实验表明，RADIANCE在保持竞争性推理延迟的同时，显著提升了生成图像的组合对齐度、视觉质量以及样本成功率。

Conclusion: 通过将推理过程视为闭环反馈控制，RADIANCE在无需任何额外训练的情况下，有效增强了预训练扩散模型的组合图像生成能力。
...
### [380] [Be Indiscrete: The Benefits of Learning Continuous Spine Degeneration Severity Scores](https://arxiv.org/abs/2607.05090)
*Maria Monzon,Andrew Zisserman,Robin Y. Park,Catherine R. Jutzeler,Amir Jamaludin*

Main category: cs.CV

TL;DR: 提出SpineRankNet框架，将腰椎退行性变评估建模为连续严重程度排序问题，而非传统的分类问题。


<details>
  <summary>Details</summary>
Motivation: 传统腰椎MRI评估将序数等级视为离散分类，忽略了误分类严重程度的差异，且无法很好地刻画连续的病变过程。

Method: 引入SpineRankNet框架，使用排序损失从腰椎脊柱MRI中学习标量严重程度评分，并与多分类及序数回归方法进行对比。

Result: 该方法实现了MRI扫描的细粒度排序，能以与分类模型相当的准确率恢复序数等级，并改善了相距较远类别之间的区分度。

Conclusion: 将脊柱退行性变建模为连续排序问题是有效的，能在提供细粒度评分的同时保持分类准确性。
...
### [381] [Semantic Video Communication via Multi-Scale Convolution and Dynamic Routing for Next-Generation Networks](https://arxiv.org/abs/2607.05093)
*Gengtian Shi,Jinze Yu,Chenhao Wu,Shaofei Wang,Eiji Fukuzawa,Junjie Tang,Hiroshi Onoda,Jiang Liu*

Main category: cs.CV

TL;DR: 提出了一种生成式AI辅助的语义视频通信框架，通过多尺度时间编码和胶囊路由机制，在边缘设备上实现高效的视频与文本语义对齐。


<details>
  <summary>Details</summary>
Motivation: 应对视频流量增长带来的带宽限制，解决边缘设备在时序建模和视频-文本语义对齐上面临的效率与鲁棒性挑战。

Method: 采用O(T)复杂度的多尺度时间卷积编码器捕获运动模式，结合胶囊动态路由机制精细化查询对齐，并通过多任务学习目标进行统一优化。

Result: 在ActivityNet Captions数据集上取得了42.9%的Recall@0.5和41.1%的平均IoU，同时保持了适合边缘部署的高计算效率。

Conclusion: 该框架有效克服了边缘部署的计算限制，为面向任务的语义视频通信提供了一种高效且实用的解决方案。
...
### [382] [Green for Go, Red for No: Visual Grounding via Semantic Segmentation for VLA Navigation Policies](https://arxiv.org/abs/2607.05122)
*Adrian Szvoren,Dimitrios Kanoulas,Nilufer Tuptuk*

Main category: cs.CV

TL;DR: 本文首次评估了视觉定位在视觉-语言-动作（VLA）机器人导航中的作用，表明其能显著降低导航误差。


<details>
  <summary>Details</summary>
Motivation: 由于VLA导航模型易受视觉干扰和场景歧义影响，且此前缺乏视觉定位在VLA策略中的实证评估，促使了本研究的开展。

Method: 提出一种基于SegFormer的实时分割定位方法（将可通行和不可通行区域分别标为绿/红），并在Grand Tour数据集上通过OmniVLA评估了仅观察分割和联合观察-目标增强两种方案。

Result: 视觉定位使最远航路点误差降低了27-44%（长指令效果更显著），并将预测路径长度缩短了30%，主要起到轨迹长度正则化器的作用。

Conclusion: 视觉定位是一种无需重新训练模型即可低成本提升VLA导航的有效方法，但无法弥补分布外指令中缺失的训练信号。
...
### [383] [Fully Rotation-Equivariant Spectral-Spatial Learning for Multispectral Object Detection](https://arxiv.org/abs/2607.05148)
*Peng Zhang,Tingfa Xu,Shuaihao Han,Jianan Li*

Main category: cs.CV

TL;DR: 提出FressDet，一种全旋转等变的多光谱目标检测框架，能在大幅减少参数的同时提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有检测器面临离散光谱处理的局限、金字塔层级间谱空间线索的尺度依赖性偏移，以及缺乏针对任意朝向目标的旋转等变几何先验。

Method: 引入三个核心组件：光谱隐式变形（SpeIW）实现连续光谱重采样；旋转等变一致性加权（ReCoW）自适应融合谱空间特征；朝向感知检测头预测多方向目标。

Result: 在三个公开基准数据集上达到最先进（SOTA）性能，且参数量减少了93%。

Conclusion: FressDet能够在旋转扰动下学习到更具判别力和鲁棒性的谱空间特征，具有强有效性和泛化性。
...
### [384] [UNIVERSE: Unified Video Action Models for Autonomous Driving with Flexible Mask-Modulated Modality Generation](https://arxiv.org/abs/2607.05133)
...
TL;DR: 提出UNIVERSE，一个统一视频与动作生成的扩散Transformer模型，通过共享参数和掩码解耦，提升了自动驾驶的轨迹泛化能力与推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有的世界动作模型（WAM）采用分离的级联或双DiT设计，导致视频学到的物理世界动力学难以直接转移到轨迹规划分支，且易使动作模型过拟合。

Method: 设计了统一的单模态调节Diffusion Transformer（single-DiT），共同训练未来视频隐变量和车辆轨迹token；引入“模态解耦可见性掩码”以防止未来目标泄露，并在测试时支持无未来视频生成的纯轨迹推理。

Result: 在NAVSIM上取得91.0 PDMS的优异表现（相比双DiT的89.6），在nuScenes和Bench2Drive上实现强力的零样本迁移，且纯轨迹推理速度提升了4.3倍。

Conclusion: 将视频生成与轨迹规划统一在单个DiT中并结合掩码解耦，能有效将视频表征能力转化为规划泛化力，同时兼顾了计算效率与因果安全性。
...
### [385] [FSDC-DETR: A Frequency-Spatial Domain Collaborative DETR for Small Object Detection](https://arxiv.org/abs/2607.05176)
*Aiwen Liu,Chengguang Zhu,Gang Wang,Dandan Zhu,Haodong Lin,Yan Wang,Huiyu Zhou,Zhiyi Pan*

Main category: cs.CV

TL;DR: 提出一种频域-空域协同检测Transformer（FSDC-DETR），通过协同建模空间与频率表征来提升小目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有检测器受限于空间聚合与隐式频率混叠及截断的耦合，导致小目标检测中关键的高频成分丢失。

Method: 提出FSDC-DETR，核心包含：双分支频域-空域自适应融合（DBFSAF）用于增强频率多样性；分流频域-空域特征融合（SFS-FF）用于跨尺度双向交互；以及频域-空域动态下采样（FSD-Down）以在下采样过程中保留高频响应。

Result: 在VisDrone-DET2019和AITODv2数据集上，AP分别提升了6.4和6.6，其中小目标的AP分别提升了6.8和6.9，达到了SOTA水平。

Conclusion: 频域与空域的显式协同建模能有效减少特征融合过程中的频率退化，显著增强了小目标检测的准确性。
...
### [386] [Claim-Level Rubric Rewards for Video Caption Reinforcement Learning](https://arxiv.org/abs/2607.05150)
*Mingqi Gao,Hongyuan Dong,Yifei Chen,Zhisheng Zhong,Zheng Ruan,Wenjin Hou,Yu Chen,Han Hu,Yansong Tang*

Main category: cs.CV

TL;DR: 本文提出CuRe框架，一种用于密集视频描述的声明级量规奖励设计方法，以解决强化学习中传统奖励机制的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的整篇级奖励易导致奖励作弊且缺乏事实准确性，而基于参考的奖励过度依赖文本对齐，限制了生成的完整性与多样性。

Method: 将奖励建模重构为细粒度的声明级验证，通过结构化量规将生成的描述分解为类别感知的原子声明进行独立验证。

Result: 成功将复杂的整体评估转化为更简单、更可靠的声明级验证，缓解了强化学习中的奖励设计瓶颈。

Conclusion: CuRe通过结构化声明级验证，为密集视频描述提供了一种能更好兼顾事实准确性与生成多样性的强化学习奖励新范式。
...
### [387] [Probing Geospatial SSL Representations with Environmental Signals](https://arxiv.org/abs/2607.05207)
*Rohita Mocharla,Vishal M. Patel*

Main category: cs.CV

TL;DR: 本文通过探究自监督学习（SSL）卫星图像表征与物理环境变量的关联，提出了一种全新的地理空间模型评估方法。


<details>
  <summary>Details</summary>
Motivation: 传统的下游任务评估方法无法有效揭示自监督学习表征中编码的物理环境信息。

Method: 利用ERA5重分析环境变量对DINO、MAE和MoCo模型的表征进行线性探测，并结合内在几何度量进行分析。

Result: 表征级度量能有效区分下游任务表现相似的模型，且环境信号的线性可达性与环境相关任务的性能强相关。

Conclusion: 物理环境关联分析为评估地理空间基础模型提供了超越传统基准的新维度，并发布了ERA5-SSL4EO数据集以支持后续研究。
...
### [388] [An event-driven framework for fly-inspired visual motion detection](https://arxiv.org/abs/2607.05205)
*Qinbing Fu,Jingyu Huang,Yan Xie,Jigen Peng,Yuchao Tang*

Main category: cs.CV

TL;DR: 本文提出了一种结合事件相机与果蝇视叶启发神经网络的计算框架，用于实现高效、低延迟且抗噪的实时前景运动方向检测。


<details>
  <summary>Details</summary>
Motivation: 动态环境下的机器视觉需要快速可靠的运动检测，但事件相机易受噪声干扰，且事件表征与仿生神经处理的有效结合仍有待探索。

Method: 结合时间表面编码进行前端事件表征，利用果蝇视叶启发神经网络进行前景运动方向估计，并引入自底向上的注意力机制以抑制背景运动。

Result: 在真实地面车辆数据集上的评估表明，该方法在与基于帧的模型和优化方法的对比中，展现出高效性、可解释性以及优异的时间性能。

Conclusion: 该框架成功融合了事件驱动视觉的时间优势与仿生神经计算的高效性和可解释性，为嵌入式实时运动检测提供了新方案。
...
### [389] [Causal-RetiGraph: Cross-Cohort Retinal Support and Same-Subject Pathway Analysis for Diabetic Retinopathy](https://arxiv.org/abs/2607.05204)
*Inam Ullah,Imran Razzak,Shoaib Jameel*

Main category: cs.CV

TL;DR: 提出Causal-RetiGraph框架，将视网膜图像特征与系统性疾病通路建模结合，以提高糖尿病视网膜病变的分级和病理通路分析能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视网膜AI虽然分级准确，但未能解决局部病变、血管结构与全身系统性疾病通路之间如何关联的问题。

Method: 引入Causal-RetiGraph框架，通过视网膜图像分支构建X1234图表型，并通过NHANES分支对系统性暴露、协变量及视网膜中介因子进行通路建模。

Result: 该模型实现了0.9055的二分类准确率和0.9711的AUROC，并确定了糖化血红蛋白、尿微量白蛋白等为关键系统性锚点，识别出糖代谢-肾脏和糖代谢-血流动力学通路为主要中介信号。

Conclusion: 视网膜图表型能够有效帮助筛选和确定糖尿病视网膜病变的系统性通路，同时保留了图像特征与同体中介之间的区别。
...
### [390] [GUSH3R: Everyone Everywhere All at Once as Gaussians](https://arxiv.org/abs/2607.05243)
*Keito Abe,Kaede Shiohara,Takashi Otonari,Toshihiko Yamasaki*

Main category: cs.CV

TL;DR: 提出GUSH3R，一个通过单次前向传播即可从单目视频中同时重建动态人体和静态场景的3D高斯泼溅（3DGS）框架。


<details>
  <summary>Details</summary>
Motivation: 现有的前向重建方法受限于非光致真实感表征（如点云或网格），或难以处理非刚性的动态人体，无法高效实现逼真的动态人景重建。

Method: 引入基于3D高斯泼溅（3DGS）的前向框架，在单次前向传播中将动态人体与静态场景一体化重建，确保几何一致性并支持新视角合成。

Result: 在单目人景数据集上的实验表明，该方法在保持高画质新视角合成的同时，相比传统基于优化的方法显著提升了推理效率。

Conclusion: GUSH3R有效解决了在线动态人景重建的效率与画质瓶颈，实现了快速且高质量的单目3D重建。
...
### [391] [A Multimodal Reasoning Typology for Grounding Chart-Image Coherence in Science Communication](https://arxiv.org/abs/2607.05222)
*Avina Nakarmi,Sohom Sen,Xun Song,Sreyashi Samaddar,Aritra Dasgupta*

Main category: cs.CV

TL;DR: 本文提出了一个包含五个级别（R1-R5）的推理差距分类法，用于表征科学文献中图表、图像及其文字说明之间的多模态协同关系和推理需求。


<details>
  <summary>Details</summary>
Motivation: 虽然图表和图像在科学文献中经常共同出现，但以往的计算工作很少表征它们之间的连贯性，需要系统地分析它们如何共同传达科学主张以及读者所需的推理过程。

Method: 基于沟通扎根理论，与神经科学专家合作，从创伤性脑损伤论文中自下而上构建了R1-R5推理差距分类法，并通过领域专家和非专家对视觉语言模型（VLM）描述的评估实验进行了验证。

Result: 该分类法能够成功预测专家与非专家在评估VLM描述时的一致与分歧点，从而准确识别出连贯性依赖于背景知识（而非图表本身）的关键节点。

Conclusion: 该分类法为识别多模态信息关联的成功与失效提供了系统性机制，帮助论文设计者更好地平衡文本与图表关系，弥合专家与非专家在理解科学结论时的认知鸿沟。
...
### [392] [Vision Pretraining for Dense Spatial Perception](https://arxiv.org/abs/2607.05247)
*Zelin Fu,Bin Tan,Changjiang Sun,Shaohui Liu,Kecheng Zheng,Yinghao Xu,Xing Zhu,Yujun Shen,Nan Xue*

Main category: cs.CV

TL;DR: 提出LingBot-Vision，通过掩码边界建模学习空间结构化视觉表征，提升了深度估计和空间感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉大模型侧重于语义不变性，忽视了对实体智能至关重要的详细空间与几何结构理解。

Method: 提出掩码边界建模（Masked Boundary Modeling）自监督范式，动态学习亚像素级边界表征并将其作为掩码目标，进而构建了LingBot-Vision。

Result: 在多项下游任务中表现优异，推动了LingBot-Depth从1.0到2.0的升级，显著提升了深度补全和估计的精度。

Conclusion: 边界建模是一种可扩展的预训练方法，能够超越简单线段提取，有效学习空间结构化的视觉表征。
...
### [393] [Repurposing CLIP to Localize at Pixel Level](https://arxiv.org/abs/2607.05253)
*Jiaxiang Fang,Shiqiang Ma,Jing Wang,Siyu Chen,Fei Guo,Shengfeng He*

Main category: cs.CV

TL;DR: 提出CLIPix框架，通过回溯CLIP的分类过程并引入抗噪与定位嵌入策略，实现了高效的像素级目标分割。


<details>
  <summary>Details</summary>
Motivation: 将CLIP图像级别的开放域定位能力适配到像素级密集预测时，存在全局特征偏差带来的挑战。

Method: 回溯CLIP分类过程以识别目标特定注意力区域，采用抗噪纠正策略精细化定位线索，并利用定位嵌入策略融合定位与细节信息。

Result: 在PASCAL和COCO数据集上的广泛实验表明，CLIPix达到了最先进的（SOTA）性能。

Conclusion: CLIPix在保留CLIP泛化强度的同时，成功释放了其分割任意目标的能力。
...
### [394] [FlowMark: Mask-Guided Video Watermarking](https://arxiv.org/abs/2607.05261)
*Vishal Asnani,Shruti Agarwal,John Collomosse*

Main category: cs.CV

TL;DR: FlowMark是一个利用自动掩码预测器进行鲁棒且无闪烁水印嵌入的端到端视频水印框架。


<details>
  <summary>Details</summary>
Motivation: 传统的基于区域的视频水印需要用户手动提供掩码引导，且难以在保持高视觉质量的同时应对复杂的视频编辑和社交媒体重编码。

Method: 引入专用的掩码预测器网络以自动识别最佳嵌入区域，采用端到端架构，结合区域感知编码、噪声增强训练和内容自适应掩码技术。

Result: 成功在保持高达50.08 dB PSNR的高视觉质量下嵌入128位信息，并在面临各种时间编辑（如帧增删、重采样）和社交平台重编码时保持高恢复率。

Conclusion: FlowMark为视频内容来源追溯、时间真实性验证和完整性保护提供了一种高效且实用的解决方案。
...
### [395] [Learning Probabilistic Embeddings for Unsupervised Action Segmentation](https://arxiv.org/abs/2607.05263)
*Shuai Li,Duc Manh Vu,Juergen Gall*

Main category: cs.CV

TL;DR: 本文针对无监督时序动作分割问题，提出了一种基于概率嵌入和最优传输的方法，以解决传统确定性嵌入易陷入局部最优的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在交替进行伪标签估计和参数优化时使用确定性嵌入，容易使模型训练陷入局部最优解。

Method: 提出学习帧表征的概率嵌入，利用高斯分布对嵌入进行建模，并在估计伪标签之前从该分布中进行采样。

Result: 在多个具有挑战性的数据集上达到了与最先进技术相当或更好的效果，相比确定性基线，MoF提升高达20.7%，F1分数提升高达19.0%。

Conclusion: 引入概率嵌入能够有效缓解无监督时序动作分割中局部最优的问题，显著提高了分割性能。
...
### [396] [SteelBench: Evaluating Vision-Language Models in Real-World Industrial Environments](https://arxiv.org/abs/2607.05264)
...
TL;DR: 本文提出了STEELBENCH，一个针对真实工业CCTV监控环境的视觉语言模型（VLM）诊断性基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有视频基准多基于消费或模拟场景，无法反映真实工业监控中复杂视觉条件（如粉尘、弱光、遮挡）对模型评估的挑战。

Method: 从149小时真实钢厂监控中筛选并高密度标注了1,345个视频片段，并引入了溯源感知审计协议以评估模型辅助标注带来的偏差。

Result: 最先进的VLM在动作识别上的准确率仅为42.6%（远低于人类的84.6%），且未经审计的模型标注会虚高同系模型准确率达17个百分点；37-58%的正确动作预测伴随着错误的安全推理。

Conclusion: 当前VLM在工业监控和安全推理任务上面临巨大挑战，且评估过程中亟需对模型辅助标注进行严格审计。
...
### [397] [ChatImage: Navigating Long-Form LLM Answers through Interactive Images](https://arxiv.org/abs/2607.05290)
*Wencan Jiang,Jiangning Zhang,Yong Liu*

Main category: cs.CV

TL;DR: ChatImage将LLM的密集文本回答转换为包含可点击热区的交互式视觉图像，以优化用户的阅读和交互体验。


<details>
  <summary>Details</summary>
Motivation: LLM生成的长篇线性文本过于密集，使用户难以进行细粒度的检查、导航和重复访问。

Method: 将文本内容结构化并渲染为图像，利用视觉定位模型（如LocateAnything、MiMo-Vision）和SAM进行二次定位，在图像上生成可点击的透明交互热区。

Result: 发布了参考实现和包含30个问题的基准测试，评估结果在交互闭环完成度、视觉对齐及掩膜完整性诊断方面表现优异。

Conclusion: 通过在渲染后定位交互目标，ChatImage增强了视觉内容与点击区域的一致性，有效提升了复杂LLM回答的导航与交互效率。
...
### [398] [Erasing Without Collateral Damage: Precise Concept Removal in Diffusion Models](https://arxiv.org/abs/2607.05274)
*Parth Upman,Nishita Jain,Shreyank N Gowda*

Main category: cs.CV

TL;DR: 本文提出CARE，一种无需训练的闭式概念擦除算子，通过保留子空间感知方向，在精准擦除目标概念的同时，有效减少对相关非目标概念的损害。


<details>
  <summary>Details</summary>
Motivation: 现有的无训练概念擦除方法在擦除目标概念时，容易误伤与其语义相关的非目标概念，因为它们将目标特征与共享的视觉结构混为一谈。

Method: 提出CARE算子，在交叉注意力值空间中，使用基于少量保留概念锚点计算的“保留子空间感知方向”代替原始目标方向，无需微调，仅需极少离线计算，并用收缩参数控制擦除与保留的权衡。

Result: 实验表明，该方法在保持对实例、风格和名人概念高效擦除的同时，能更忠实地保留非目标概念。

Conclusion: CARE提供了一种具有最小扰动特性的概念擦除方案，在投影形式下能保持保留子空间不变，实现了更精准且副作用更小的概念擦除。
...
### [399] [Is the Geometry Doing the Work? An Operating-Point Audit of Hierarchy in Hyperbolic Vision-Language Models](https://arxiv.org/abs/2607.05268)
*Jaeyoung Kim,Eunseok Kim,Dongsuk Jang*

Main category: cs.CV

TL;DR: 本文审计了三种主流的双曲视觉语言模型，发现它们在实际运行中并没有真正利用双曲几何机制，而是退化为了近欧氏表示。


<details>
  <summary>Details</summary>
Motivation: 旨在检验双曲表示模型是否真正利用了其设计的双曲几何（如曲率和锥体机制），因为仅凭曲率参数无法判断其几何特性的实际启用情况。

Method: 开发了一套必要条件诊断工具，对MERU、HyCoCLIP和PHyCLIP等开源模型及其在GRIT数据集上的干预实验进行审计，分析运行点、蕴含锥激活度及层级相关性。

Result: 发现了三个关键失效模式：运行点处于近欧氏区域（均未达到 $\sqrt{c}\rho > 1$）；蕴含锥与遍历机制失效；层级评估结果主要由角度距离驱动而非双曲曲率。并从理论上揭示了模型倾向于走向“低曲率、宽锥体”捷径的坍缩机制。

Conclusion: 现有的双曲视觉语言模型未能真正实例化其几何设计所预期的径向/锥体机制，为此作者提出了一个“五数几何报告”以规范未来的层级表征评估。
...
### [400] [CenSynCMB: Centre Maps and Physics-Guided Synthesis for Microbleed Detection](https://arxiv.org/abs/2607.05325)
...
TL;DR: 提出了CenSynCMB框架，通过物理引导的合成数据和中心引导机制，提升了核磁共振（MRI）中脑微出血（CMBs）的自动检测能力。


<details>
  <summary>Details</summary>
Motivation: 脑微出血（CMB）体积小、分布稀疏，且极易与血管、钙化灶及伪影混淆，导致自动化检测非常困难。

Method: 设计了CenSynCMB框架，结合了3D Attention U-Net、辅助中心图监督、假阴性驱动重赋权，以及基于物理引导的阳性CMB与标记硬负样本的合成技术。

Result: 在VALDO Task 2数据集上取得了最佳的病灶级F1值（74.3%）；在外部AIBL SWI数据集上实现了最高的召回率（88.5%）和F1值（65.0%）。

Conclusion: 该方法适用于大规模未标注MRI队列中脑微出血候选病灶的高效提取，并指出队列特异性校准是实现可靠负荷评估的下一步研究方向。
...
### [401] [Deep Learning for Semen Analysis in Male Infertility: Computer Vision, Multimodal Fusion, and Clinical Translation](https://arxiv.org/abs/2607.05311)
...
TL;DR: 本文综述了人工智能（计算机视觉与深度学习）在精子分析中的应用进展，探讨了从特定图像识别任务向多模态生殖智能的演进及临床转化挑战。


<details>
  <summary>Details</summary>
Motivation: 传统的精液评估方法耗时费力、高度依赖操作者经验且存在较大的主观偏差，因此急需开发客观且可重复的计算化评估方法。

Method: 系统性地综述了基于AI的精子检测、跟踪、分割、形态分类及遗传完整性评估方法，总结了多模态数据融合策略，并探讨了数据稀缺、领域偏移等临床部署障碍。

Result: 梳理了公开数据集与评估指标，指出了算法在实际临床应用中的关键瓶颈（如可解释性、标注不一致和隐私保护），并提出了一个包含技术标准化到临床验证的阶段性转化路线图。

Conclusion: 要将AI驱动的精子分析成功转化为临床决策支持工具，必须克服数据标准化、多中心验证以及监管审批等一系列技术与临床转化挑战。
...
### [402] [WildSplat: Feedforward Gaussian Splatting from Unposed In-the-Wild Images](https://arxiv.org/abs/2607.05347)
*Xiyu Zhang,Jingyu Zhuang,Hongjia Zhai,Zizheng Yan,Jinwei Chen,Guofeng Zhang,Qingnan Fan*

Main category: cs.CV

TL;DR: 本文提出了WildSplat，首个支持在光照多变的野外场景下进行外观条件新视角合成的前馈3D高斯泼溅框架。


<details>
  <summary>Details</summary>
Motivation: 传统前馈3D重建方法在面对光照变化和无位姿的野外图像时，难以处理不一致的光度条件。

Method: 采用双分支架构解耦几何与外观：几何分支预测相机位姿并提取3D结构；外观分支利用全局预调制交叉注意力注入外观特征。同时引入联合多参考训练以防止特征纠缠。

Result: 在稀疏输入的野外新视角合成和外观编辑中，WildSplat超越了现有的优化和前馈方法，实现了SOTA性能。

Conclusion: WildSplat证明了通过显式解耦几何与外观，可以在单次前向传播中高效地实现鲁棒的野外场景重建与外观编辑。
...
### [403] [Beyond Isolated Objects: Relationship-aware Open Vocabulary Scene Understanding via 3D Scene Graph Analysis](https://arxiv.org/abs/2607.05348)
*Xianhao Chen,Jiarui Hu,Yuanbo Yang,Xiyu Zhang,Tengyue Wang,Hujun Bao,Guofeng Zhang,Zhaopeng Cui*

Main category: cs.CV

TL;DR: 本文提出了RelGraphOV，这是一个利用3D场景图和自适应双流上下文图神经网络来增强开放词汇3D场景理解的关系感知框架。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖于独立于上下文的语义表示，未能充分利用物体之间的关系来进行上下文优化。

Method: 利用视觉语言推理构建无需手动标注的关系场景图，并引入自适应门控双流上下文图注意力网络（GAT）来分离并融合几何与语义特征，同时结合分层对比学习目标。

Result: 在ScanNetV2、ScanNet200、ScanNet++和Replica等多个数据集上取得了强大的性能和出色的泛化能力。

Conclusion: 通过引入关系感知的3D场景图和双流特征聚合机制，可以有效提升开放词汇3D场景理解的准确性和泛化性。
...
### [404] [Geometric Reciprocity: Unlocking Self-Supervision for Stereoscopic Video Generation](https://arxiv.org/abs/2607.05354)
*Jingyi Lu,Kai Han*

Main category: cs.CV

TL;DR: 本文提出了一种基于几何互惠定理（GRT）的自监督单目转双目视频合成框架，通过单目视频实现高质量的遮挡区域修复。


<details>
  <summary>Details</summary>
Motivation: 现代DIBR方法中遮挡区域的立体修复是关键瓶颈，且现有训练方法严重依赖稀缺的双目图像或存在领域偏差的合成数据。

Method: 提出了几何互惠定理（GRT），在最近邻DIBR公式下，证明了合成目标视图时的遮挡掩膜等同于从目标回扭曲到源视图时丢失的像素掩膜，从而实现自监督循环一致性学习。

Result: 该方法实现了训练与测试的一致性，支持利用单目视频进行自监督训练，性能显著超越了现有的无训练方法和有监督的最先进方法。

Conclusion: 通过引入GRT，本研究成功实现了首个仅依赖单目视频的自监督立体图像修复框架，有效解决了双目训练数据稀缺的问题。
...
### [405] [ReCal3R: Reliability-Calibrated Learning Rates for Streaming 3D Reconstruction](https://arxiv.org/abs/2607.05356)
*Xinze Li,Yiyuan Wang,Pengxu Chen,Wentao Fan,Weifeng Su,Weisi Lin,Wentao Cheng*

Main category: cs.CV

TL;DR: 提出ReCal3R方法，通过基于可靠性的学习率校准，解决循环三维重建中历史信息被噪声覆盖的问题。


<details>
  <summary>Details</summary>
Motivation: 循环三维重建中的持续更新易导致场景状态损坏，使可靠的历史信息被噪声或模糊的观测所覆盖。

Method: 引入无需训练的可靠性校准学习率方法，通过评估Token可靠性来校准候选学习率，在保守基础率与候选率之间进行插值更新。

Result: 在CUT3R上应用该方法，在长序列的姿态和重建质量上表现优异，绝对轨迹误差（ATE）降低了3.7倍，且未增加运行时间和内存负担。

Conclusion: ReCal3R通过校准Token级别的更新率，有效抑制了不可靠更新，同时保留了对新信息的适应能力，提升了流式三维重建的鲁棒性。
...
### [406] [PixWorld: Unifying 3D Scene Generation and Reconstruction in Pixel Space](https://arxiv.org/abs/2607.05373)
*Sensen Gao,Zhaoqing Wang,Qihang Cao,Dongdong Yu,Changhu Wang,Jia-Wang Bian*

Main category: cs.CV

TL;DR: 提出了PixWorld，一个将3D重建与生成任务统一在像素空间扩散范式下的单一模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将3D重建和生成分离，或在潜空间统一时存在信息丢失、依赖预训练VAE以及缺乏3D几何感知等问题。

Method: 采用像素空间扩散范式，直接在渲染图像上监督扩散过程，并引入基于预训练3D基础模型的几何感知损失，以提供3D结构监督。

Result: PixWorld在生成任务上超越了以往的潜空间方法，并在重建任务上达到了与当前最先进方法相当的水平。

Conclusion: 本研究证实了统一像素空间扩散方法在3D重建与生成双重任务中的优越性和可行性。
