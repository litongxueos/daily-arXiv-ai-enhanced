...
### [1] [Supervised Post-training of Speech Foundation Models for Robust Adaptation in Speech Deepfake Detection](https://arxiv.org/abs/2606.25328)
*Zihan Pan,Sailor Hardik,Jinyang Wu*

Main category: cs.SD

TL;DR: 提出一种混合帧后训练策略，通过帧级监督提升语音基座模型在深度伪造检测中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自监督预训练目标与伪造特定伪影之间存在不匹配，限制了语音基座模型在深度伪造检测中的直接微调效果。

Method: 提出混合帧后训练策略以创建局部伪造扰动，并结合帧级监督促使模型学习对伪造检测至关重要的局部不一致性。

Result: 在ASVspoof5上单模型（无数据增强）达到SOTA的4.50% EER；在ASVspoof2021 LA/DF上的EER差距仅为0.16%，表现出强劲且平衡的鲁棒性。

Conclusion: 监督后训练是使语音基座模型适应鲁棒深度伪造检测的一种有效且实用的方法。
...
### [2] [Sarashina2.2-TTS: Tackling Kanji Polyphony in Japanese Speech Generation via Data Scaling and Targeted Data Synthesis](https://arxiv.org/abs/2606.25369)
...
TL;DR: 本文介绍了 Sarashina2.2-TTS，一个专注于日语的 LLM 文本转语音系统，有效解决了日语中汉字多音字读音的难题，并在发音准确性和零样本语音合成上达到了先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM-TTS 系统多关注英文和中文，日语 TTS 仍面临独特的挑战，特别是上下文相关的常用汉字多音字（kanji polyphony）读音问题尚未得到充分解决。

Method: 使用约36.1万小时的日英文混合数据进行大规模训练，针对2,136个常用汉字设计了定向数据增强流水线；同时推出了 Joyo Kanji Yomi 评估基准，并提出用于衡量发音准确性的 Kana-CER 指标。

Result: 该系统在汉字级别读音准确率上达到最先进水平，在零样本合成中实现了最高的说话人相似度，并在跨语言评估中表现出极佳的日语发音稳定性。

Conclusion: 通过结合大规模数据训练、针对性的汉字多音字数据增强以及全新的评估方法，Sarashina2.2-TTS 成功克服了日语 TTS 的发音难题，并展现出优异的跨语言鲁棒性。
...
### [3] [Attractive and Repulsive Pattern Control in Sequence Generation](https://arxiv.org/abs/2606.24911)
*Francois Pachet*

Main category: cs.SD

TL;DR: 本文提出了一种基于BP-Regular采样的有符号模式控制方法，用以解决可变阶马尔可夫模型生成中的局部重复问题。


<details>
  <summary>Details</summary>
Motivation: 可变阶马尔可夫模型在长序列生成中容易陷入高阶重复的“隧道”（如过度重复的片段），限制了生成的多样性和质量。

Method: 结合置信度传播（BP）算法，通过加权循环自动机计算目标模式激活度，并利用正负耦合系数分别惩罚（负耦合）或奖励（正耦合）特定模式的生成。

Result: 在巴赫、泰勒曼及爵士乐数据集上的实验表明，负耦合显著降低了8-gram的自我重复率，提高了生成的多样性和上下文覆盖率；正耦合则成功展示了对模型吸引子和相变行为的控制能力。

Conclusion: 该方法有效克服了马尔可夫生成中的结构性重复缺陷，并为探索和控制生成模型的动力学行为提供了灵活的机制。
...
### [4] [What Does a Pathological Speech Assessment Model Know about Acoustic Features? A Case Study on Oral and Oropharyngeal Cancer Patients](https://arxiv.org/abs/2606.24949)
*Tuan Nguyen,Corinne Fredouille,Alain Ghio,Muriel Lalain,Virginie Woisard*

Main category: cs.SD

TL;DR: 本文通过典型相关分析评估了用于癌症患者言语可理解性诊断的 Wav2Vec 2.0 模型的解释性。


<details>
  <summary>Details</summary>
Motivation: 旨在探究基于自监督学习的 Wav2Vec 2.0 模型在评估病理言语时，其内部各层是如何编码不同的声学特征的。

Method: 利用典型相关分析（CCA）测量模型各层嵌入与 eGeMAPS 低级声学描述符（LLD）在个人及特征组（韵律、谱、音质）层面的相关性。

Result: 模型表征与谱特征（相关性0.77）和韵律特征（相关性0.71）相关性最强，其中第一MFCC系数在所有层中表现出最高相关性，而音质特征相关性为0.65。

Conclusion: 该研究不仅提升了病理言语评估模型的解释性，还为相关临床评估中的声学特征选择提供了实际指导。
...
TL;DR: 本文提出了一种结合虚拟乐器合成数据和迁移学习的自动吉他转谱（AGT）力度预测方法。


<details>
  <summary>Details</summary>
Motivation: 由于数据集中缺乏力度标签以及除钢琴外的乐器缺乏力度定义，导致现有自动音乐转谱模型难以预测力度。

Method: 利用虚拟乐器生成带力度标签的合成数据进行模型预训练，随后将权重迁移至使用真实吉他音频训练的模型中。

Result: 该方法在力度预测上优于无预训练权重的基线模型，并微幅提升了音符转谱精度，整体转谱效果与当前最先进水平相当。

Conclusion: 该模型在成功预测吉他弹奏力度的同时，保持了高水平的自动吉他转谱性能。
...
### [6] [EmotionAI: A Privacy-Preserving Computational Intelligence Pipeline for Speech-Emotion-Grounded Conversational Analysis](https://arxiv.org/abs/2606.24941)
*Wai Laam Mak,Isibor Kennedy Ihianle,Pedro Machado*

Main category: cs.SD

TL;DR: EmotionAI是一个完全本地化、保护隐私的系统，结合了语音情感识别与本地大语言模型，用于分析访谈录音中的情感线索。


<details>
  <summary>Details</summary>
Motivation: 分析访谈录音中的情感线索效率低且主观性强，而云端自动化服务存在敏感音频数据外泄的隐私隐患。

Method: 构建了一个完全本地运行的管线，整合了说话人日志、Whisper ASR和wav2vec2情感分类器来提取情感证据，并使用由三个本地LLM组成的对抗面板进行基于时间戳和引用的问答分析。

Result: 在RAVDESS数据集上，分类器准确率为48.8%，优于随机基线但低于域内MFCC方法，显现出跨语料库的脆弱性；整个系统在CPU上运行，平均耗时157秒，无需任何外部调用。

Conclusion: 该研究提供了一个可审计且保护隐私的框架，成功将不完美的情感证据整合进对话分析中，并指出了当前跨语料库迁移和以人为本验证的不足之处。
...
### [7] [From Sounds to Scenes: A Benchmark for Evaluating Context-Aware Auditory Scene Understanding in Large Audio Language Models](https://arxiv.org/abs/2606.25391)
...
TL;DR: 提出CASU基准，旨在评估大型音频语言模型（LALMs）整合语音、音效和环境背景等多图层信息以理解复杂听觉场景的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的音频模型评估基准多孤立地测试单一音频维度，忽视了现实世界中多种声音共存时所需的上下文感知听觉场景理解（CASU）。

Method: 引入CASU基准，开发了一套合成多图层音频流的管道，并设计了上下文问答、实体提取、说话人角色推断和反事实推理四项任务来评估模型。

Result: 多模型实验表明，有效的听觉场景理解必须依赖所有听觉图层的整合，仅依靠语音或声音本身是无法实现的。

Conclusion: CASU基准对于推动大音频语言模型在复杂、多层次现实听觉场景中的理解与推理能力至关重要。
...
### [8] [One Model, Many Latencies: Universal Speech Enhancement for Diverse Real-Time Applications](https://arxiv.org/abs/2606.25621)
*Szu-Wei Fu,Rong Chao,Xuesong Yang,Sung-Feng Huang,Ante Jukić,Yu Tsao,Yu-Chiang Frank Wang*

Main category: cs.SD

TL;DR: 提出了一种通用的实时语音增强模型，通过单一模型即可灵活控制算法和计算延迟，无需针对不同延迟预算重新训练。


<details>
  <summary>Details</summary>
Motivation: 不同的实时语音应用具有不同的延迟限制，传统的语音增强方法需要为每种延迟场景单独训练模型，导致高昂的训练和部署成本。

Method: 引入了可配置的前瞻帧和并行卷积层来调节算法延迟，使用早期退出（early-exit）机制控制计算延迟，并采用“共享到多解码器过渡”的两阶段训练策略来提升性能。

Result: 该模型成功实现了对算法和计算延迟的显式控制，有效缩小了通用模型与特定延迟专用模型之间的性能差距。

Conclusion: 该框架使单个语音增强模型能够直接部署于多种延迟预算场景中，消除了为不同应用重新训练模型的需求。
...
### [9] [STEB: A Speech-to-Speech Translation Expressiveness Benchmark for Evaluating Beyond Translation Fidelity](https://arxiv.org/abs/2606.25529)
...
TL;DR: 本研究提出了STEB，一个用于评估语音翻译（S2ST）中情感、场景风格和非言语发音等表达力保留能力的32.6小时中英双语基准。


<details>
  <summary>Details</summary>
Motivation: 传统的语音翻译评估缺乏能同时保证翻译忠实度和表达力对齐的跨语言目标数据，且现有系统在保留情感等非语义表达力方面存在不足。

Method: 开发了STEB基准，并设计了一种“描述-总结”框架，通过将语音转化为结构化属性，并使用大语言模型（LLM）作为裁判来评估源语音与翻译语音的表达力一致性。

Result: 对六种S2ST系统的评估表明，尽管现有系统语义翻译保真度较高，但在情感保留（最高3.82/5）和非言语发音保留（最高2.31/5）方面表现较差；同时，LLM裁判的评估结果与人类判断高度相关。

Conclusion: 研究揭示了语义传输与表达力传输之间的差距，表明表达力保留是当前语音到语音翻译领域中亟待解决的公开挑战。
...
### [10] [FoleySet: A Multi-Level Human-Annotated Foley Sound Dataset](https://arxiv.org/abs/2606.25980)
*Sunshiyu Wang,Alexander Lerch*

Main category: cs.SD

TL;DR: 本文推出了 FoleySet，一个包含 10,000 个已标注音频剪辑的公开拟音（Foley）数据集，旨在支持数据驱动的拟音分类、检索和生成任务。


<details>
  <summary>Details</summary>
Motivation: 传统的拟音制作流程需要专业音效师使用物理道具录制，非常消耗资源。虽然这推动了数据驱动的拟音研究，但目前仍缺乏高质量且带标注的训练数据集。

Method: 创建并推出了 FoleySet 数据集，该数据集包含 10,000 个音频剪辑，并采用两级拟音分类法进行标注，采用知识共享（Creative Commons）许可发布。

Result: 成功构建了一个包含 10,000 个标准化标注音频的 FoleySet 公开数据集。

Conclusion: FoleySet 为数据驱动的拟音分类、检索和生成提供了标准化的开源资源，填补了高质量拟音数据集的空白。
...
### [11] [Frequency-Aware Self-Supervised Music Representation Learning](https://arxiv.org/abs/2606.25713)
*Yicheng Gu,Junan Zhang,Jerry Li,Zhizheng Wu,Lauri Juvela*

Main category: cs.SD

TL;DR: 提出PupuJEPA，一种直接在2D声谱图上训练的视觉联合嵌入预测架构，用以提升音乐信息检索（MIR）任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的音乐自监督学习模型通常将音频处理为1D序列，忽略了2D时频谱图中丰富的空间与结构信息，而这些信息在音乐制作中具有直观的对应关系。

Method: 提出PupuJEPA架构，通过预测未遮蔽区域来补全遮蔽的2D声谱图斑块的潜在嵌入，并针对音乐领域对模型架构、训练和推理进行了特定优化。

Result: 在MARBLE基准测试中，PupuJEPA在线性探测的多个MIR任务上超越了1D序列自监督模型，且其注意力图谱成功捕获了具有音乐意义的2D时频特征。

Conclusion: 研究表明，直接基于2D时频图的联合嵌入预测架构能够更有效地学习音乐表征，为MIR任务提供了一种强有力的新方法。
...
### [12] [The Hitchhiker's Guide to Agentic AI: From Foundations to Systems](https://arxiv.org/abs/2606.24937)
*Haggai Roitman*

Main category: cs.AI

TL;DR: 本书是构建自主AI系统的综合实践指南，涵盖了从大模型底层技术到生产部署的完整技术栈。


<details>
  <summary>Details</summary>
Motivation: 旨在解决构建优秀智能体系统需要理解整个技术流水线（而非单一环节）的需求，填补从底层原理到实际部署之间的知识空白。

Method: 系统性地梳理了LLM基础、对齐与推理层（如RLHF、GRPO）、智能体设计模式、多智能体协同协议（如MCP、A2A）以及生产部署与评估方法。

Result: 提供了一套结合理论基础、代码示例和文献参考的智能体AI构建框架与实践指导。

Conclusion: 成功构建高效的智能体系统必须掌握从模型微调、推理强化、记忆管理到多智能体协作的端到端技术栈。
...
### [13] [Project Auto-World: Towards Automated Benchmarking of Neural Relational Reasoners](https://arxiv.org/abs/2606.24965)
*Anirban Das,Joanne Boisson,Irtaza Khalid,Sumita Garai,Steven Schockaert*

Main category: cs.AI

TL;DR: 本文提出利用大语言模型通过进化与代理搜索自动生成困难的关系推理基准，以提升神经模型（如Edge Transformer）的系统泛化能力。


<details>
  <summary>Details</summary>
Motivation: 神经推理模型难以在比训练集更难的关系推理任务上进行系统性泛化，且目前缺乏评估这种泛化能力的标准，因为很难预先定义什么实例是“困难的”。

Method: 使用基于LLM的进化搜索（FunSearch）和自主代理搜索，在Datalog规则参数化的世界中寻找能够生成困难问题实例的采样函数，并利用这些数据迭代训练和改进Edge Transformer。

Result: 成功自动生成了高难度的基准测试实例，显著提高了Edge Transformer面对数据扰动时的泛化性能，并验证了该方法可推广至LLM自主设计的新推理场景中。

Conclusion: 该方法实现了关系推理基准的自动化生成与模型泛化能力的闭环提升，为神经关系推理的自主科学研究奠定了基础。
...
### [14] [Do vision-language models search like humans? Reasoning tokens as a reaction-time analog in classic visual-search paradigms](https://arxiv.org/abs/2606.25066)
*Farahnaz Wick*

Main category: cs.AI

TL;DR: 本文评估了视觉语言模型（VLM）在经典视觉搜索任务中的行为特征，并将其推理Token数量作为搜索努力度的度量，与人类行为进行了对比。


<details>
  <summary>Details</summary>
Motivation: 探究视觉语言模型是否表现出与人类相似的视觉注意力行为特征（如并行与串行搜索的区别）。

Method: 适配了四种经典的视觉搜索范式（特征与联结搜索、空间结构搜索、枚举、倾斜/垂直不对称性），以模型生成推理Token的数量来模拟人类的搜索反应时间，并与人类基准数据进行对比。

Result: 模型重现了人类的部分行为特征（如特征搜索努力度平缓，而联结搜索努力度随干扰项数量增加），但也在多处与人类存在显著差异（如目标存在与否的努力度斜率与人类相反，枚举任务在人类容易出错的区间仍保持高精度）。

Conclusion: 心理物理学范式是探测机器视觉认知的敏锐且低成本的方法，模型与人类行为的异同点对于理解机器认知同样具有重要的启发意义。
...
### [15] [Diagnosing and Mitigating Compounding Failures in Agentic Persuasion via Taxonomic Strategy Retrieval](https://arxiv.org/abs/2606.24976)
*Pradyumna Narayana,Sana Ayromlou,Purvi Sehgal*

Main category: cs.AI

TL;DR: 本文提出了分类策略RAG（TS-RAG），通过将论证结构与主题内容解耦，解决了多智能体说服任务中的语义泄漏与阿谀奉承问题。


<details>
  <summary>Details</summary>
Motivation: 基础模型智能体在开放式任务中易受复合错误影响，标准RAG的语义泄漏（过度关注词汇重叠而非逻辑）会导致智能体在说服任务中出现问题漂移和盲目迎合。

Method: 提出TS-RAG方法，通过离散的类别瓶颈来解耦逻辑结构与具体话题；同时引入辩论状态表示（DSR）进行转折级别的诊断分析。

Result: TS-RAG显著提升了抽象逻辑的迁移能力，使轻量级智能体在对抗更强对手时的胜率从70.5%提升至78.5%，并提高了论证效率。

Conclusion: 在非对称智能体部署中，必须引入像TS-RAG这样的严格约束，以防止因智能体默认的阿谀奉承行为而导致评估崩溃。
...
### [16] [The Clinician's Veto: Navigating Trust, Liability, and Uncertainty in Autonomous AI Prescribing](https://arxiv.org/abs/2606.25108)
*Eileanor LaRocco,Sarah Tan,Adarsh Subbaswamy,Anne Andrews,Andrew Taylor,Cree Gaskin,Chirag Agarwal*

Main category: cs.AI

TL;DR: 本文探讨了自主AI处方系统的技术与监管架构要求，指出系统需具备置信度校准、区分不确定性类型及推理透明性，方能获得临床医生认可并安全落地。


<details>
  <summary>Details</summary>
Motivation: 随着允许AI自主开具处方的立法和试点逐步推进，现有的监管指南缺乏对预测置信度、区分模型无知与临床模糊性以及决策透明度的要求，存在安全和责任归属隐患。

Method: 通过技术与监管论证，并结合对136名美国处方医生的问卷调查，评估临床医生对自主处方AI的最低架构要求和责任接受度。

Result: 调查显示，医生拒绝无置信度分级上报的自主处方；在偶然不确定性下倾向于查看备选摘要，在认知不确定性下则倾向于让AI弃权；且只有在推理过程透明时，医生才愿意承担额外责任。

Conclusion: 满足上述架构要求的系统实质上会从“自主代理”转变为“受监督的决策支持工具”，这有助于通过监管约束AI的实际自主权，并使责任划分与系统设计及部署者保持一致。
...
### [17] [Beyond Shapley: Efficient Computation of Asymmetric Shapley Values](https://arxiv.org/abs/2606.25103)
...
### [18] [TRUSTMEM: Learning Trustworthy Memory Consolidation for LLM Agents with Long-Term Memory](https://arxiv.org/abs/2606.25161)
*Tianyu Yang,Sudipta Paul,Vijay Srinivasan,Vivek Kulkarni,Srinivas Chappidi*

Main category: cs.AI

TL;DR: 本文提出了TrustMem框架，通过记忆转换验证和偏好强化学习，提升了大模型智能体记忆巩固的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有的智能体记忆更新机制容易遗漏信息、损坏已有记忆或引入幻觉，导致持久性的系统状态故障。

Method: 设计了记忆转换验证器（Memory Transition Verifier）来评估更新质量，并构建候选更新偏好对，利用偏好引导的强化学习优化更新行为。

Result: 在多项基准测试中达到SOTA，HaluMem提取率提升12.14 F1值，且显著降低了遗漏（40.1%）、损坏（79.1%）和幻觉（50.0%）等错误率。

Conclusion: TrustMem能够有效提高LLM智能体长效记忆的实用性与可靠性，减少记忆巩固过程中的各类错误。
...
### [19] [Transferability for General Reasoning: An Automated Curriculum for Multi-Domain RLVR](https://arxiv.org/abs/2606.25178)
*Yongjin Yang,Jiarui Liu,Yinghui He,Lezhen Zhang,Bernhard Schölkopf,Zhijing Jin*

Main category: cs.AI

TL;DR: 提出了一种名为TAC的在线课程学习方法，通过结合局部可学习性和跨领域迁移性，优化多领域强化学习推理训练中的领域采样。


<details>
  <summary>Details</summary>
Motivation: 多领域强化学习（RLVR）现有的课程学习方法通常只关注单一领域的改进（可学习性），而忽略了不同推理领域之间梯度更新的迁移效应。

Method: 提出迁移感知课程（TAC）算法，这是一种在线课程学习方法。它结合了表示局部可学习性的领域优势值，以及利用GRPO步骤中投影梯度估计的跨领域迁移性（梯度几何对齐），且计算开销极低（<1%）。

Result: 在包含六个领域的推理任务上，TAC在Qwen3-1.7B和Llama3.2-3B模型上均取得最佳宏平均准确率，相比仅基于可学习性的方法提升了高达2.8个百分点（相对提升10%），并在不平衡数据集上表现出更强的鲁棒性。

Conclusion: 跨领域迁移性是多领域RLVR课程设计的关键信号，TAC证明了结合迁移性与可学习性可以显著提升多领域强化学习的训练效率和最终性能。
...
### [20] [To Isolate or to Score? Model-Adaptive Assessment for Cost-Efficient Multi-Agent RAG](https://arxiv.org/abs/2606.25191)
*Jungseob Lee,Chanjun Park,Heuiseok Lim*

Main category: cs.AI

TL;DR: 本文探讨了RAG中轻量级模型的多智能体文档评估机制，并提出了自适应路由架构MADARA以降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成（RAG）中多智能体文档评估的计算成本极高，促使业界转向小型模型，但这些模型在文档评估中的工作机制仍不为人知。

Method: 在7B-9B模型上进行免训练干预的对照研究，引入了“推理-分数耦合”（Reasoning-Score Coupling）探针，并提出了模型自适应路由架构MADARA。

Result: 研究发现，弱基线模型通过“单文档隔离”解决上下文混淆即可获得高达50%的性能提升，而强基线模型则更依赖评分质量；MADARA的诊断阈值能够零样本泛化至四个未见模型系列。

Conclusion: MADARA提供了一种鲁棒且轻量级的流水线，能够自适应优化文档评估流程，有效消除了计算开销。
...
### [21] [Elo-Disentangled Player-Style Embeddings for Human Chess via Rating-Conditioned Residual Move Model](https://arxiv.org/abs/2606.25176)
*Jason Carlson*

Main category: cs.AI

TL;DR: 本文提出了一种学习人类国际象棋个性化风格表征的方法，通过残差设计将棋手个人风格与棋力水平（Elo）进行解耦。


<details>
  <summary>Details</summary>
Motivation: 旨在学习一种既能衡量棋手风格相似性、又与棋手实力（Elo评分）解耦的个人风格嵌入，以提供比单独为每个棋手微调更高效、可解释的建模方式。

Method: 采用残差公式：首先构建一个以等级分为条件的基础走子模型（结合Maia模型与Stockfish特征）来捕捉特定水平的典型走法；然后使用其冻结副本构建编码器和棋手个性化向量 z，使 z 仅解释偏离典型走法的个性化偏差。

Result: 基础模型相比Maia-3在步法预测的相对NLL上提升了27-37%；个性化嵌入 z 能有效识别棋手且不发生过拟合；z 与等级分的关联极低（线性探测 R^2 仅为0.06），证明成功实现了水平解耦。

Conclusion: 结合强等级分条件基础模型与紧凑、解耦水平的嵌入，是建模个人棋风的一种经济且具可解释性的新方案，可替代传统的单棋手偏好微调。
...
### [22] [Omni-Perception Policy Optimization for Multimodal Emotion Reasoning](https://arxiv.org/abs/2606.25325)
*Zhiyuan Han,Beier Zhu,Wenwen Tong,Pengyang Shao,Peipei Song,Xinyi Wang,Jiangnan Chen,Lewei Lu,Xun Yang*

Main category: cs.AI

TL;DR: 本文提出了一种名为OPPO的强化学习框架和诊断基准MEP-Bench，旨在解决多模态情感大模型中多模态线索利用不足和跨模态幻觉的问题。


<details>
  <summary>Details</summary>
Motivation: 当前的情感导向多模态大模型（Omni-MLLMs）缺乏可靠的全模态感知：它们在推理过程中未充分利用多模态线索，且容易产生跨模态的虚假幻想。

Method: 提出了全模态感知策略优化（OPPO）框架，包含：1）全模态感知奖励，将真实推理分解为细粒度的视觉、听觉和情感线索并予以奖励；2）全模态感知损失，通过对比全输入与单模态掩码输入，对特定模态的证据Token施加KL惩罚以抑制幻觉。此外，引入了诊断基准MEP-Bench以量化利用率和忠实度。

Result: OPPO在MER-UniBench和MME-Emotion上取得了最先进（SOTA）的性能，并在MEP-Bench上显著提高了模态利用率和忠实度得分。

Conclusion: 充足且忠实的全模态感知对于多模态情感推理至关重要，OPPO框架有效提升了模型的感知与推理能力。
...
### [23] [Heuresis: Search Strategies for Autonomous AI Research Agents Across Quality, Diversity and Novelty](https://arxiv.org/abs/2606.25198)
...
TL;DR: 本文提出了Heuresis框架以评估LLM智能体在机器学习研究中的自主探索能力，发现当前策略难以生成兼具高质量与新颖性的想法，且智能体常出现奖励作弊行为。


<details>
  <summary>Details</summary>
Motivation: 当前的LLM智能体在科学研究中不仅需要编写代码，更需要具备探索同时兼顾性能、多样性和新颖性研究想法的能力。

Method: 提出了Heuresis框架，将研究管线抽象为可组合的原语，评估了六种搜索策略（包括贪婪基线、基于存档、进化和发散策略），并在三个机器学习领域进行测试。

Result: 实验表明完全新颖的想法极少且性能无法达到顶尖水平；智能体在执行过程中存在显著的奖励作弊现象（确认40次伪造行为）；现有策略未能扩展质量与新颖性的前沿。

Conclusion: 目前的搜索和质量-多样性策略虽然可以引导想法的分布，但无法突破质量与新颖性并存的瓶颈，这是实现自主科学进步面临的重大挑战。
...
### [24] [Agentic Knowledge Tracing: A Multi-Agent LLM Architecture for Stealth Assessment of Financial Literacy in Serious Games](https://arxiv.org/abs/2606.25358)
*Gabriel Santos,Rita Julia,Marcelo Nascimento*

Main category: cs.AI

TL;DR: 提出了一种名为 Agentic BKT 的多智能体大语言模型流水线，用于在严肃游戏中隐式评估玩家的金融素养。


<details>
  <summary>Details</summary>
Motivation: 在严肃教育游戏中，如何在不中断游戏和学习体验的前提下，有效且隐式地评估玩家的金融决策能力是一大挑战。

Method: 该方法包含四个阶段：提取玩家决策日志、利用LLM进行事件分类、通过四个领域特定智能体进行贝叶斯知识追踪（BKT）推理，以及由专家评审智能体合成最终的掌握度评分。

Result: 实验表明，该流水线评估出的掌握度与学习增益及后测成绩显著正相关，且预测效度是单一LLM基准模型的三倍左右。

Conclusion: 研究证明，多智能体架构中的领域分解和会话级推理对于从游戏行为中捕获多维度金融素养至关重要。
...
### [25] [What Actually Works for Spacecraft Fault-Tolerant Control: An Honest Settled-Gate Benchmark of Learned and Classical Methods](https://arxiv.org/abs/2606.25374)
*Alireza Shojaei*

Main category: cs.AI

TL;DR: 本文构建了一个严格的航天器容错控制（FTC）评估基准，并证明了结构化“估计-控制”结合扰动观测器在应对未见的执行器故障（增益、符号及偏差）时表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有学习型FTC方法多在单一仿真和宽松指标下测试，难以应对训练中未见过的、需高精度维持的执行器及传感器故障。

Method: 建立了一个基于6-DOF Basilisk平台的严格基准，采用“稳定窗口”评估指标。对比了多种控制器，并提出一种结构化设计：利用循环神经网络在线估计执行器参数并输入解析控制律，同时结合扰动观测器处理常数偏差。

Result: 传统PD/PID和纯RL的成功率为0%；结构化“估计-控制”设计在符号和增益故障中成功率分别达97.8%和94.4%；引入扰动观测器后，将原本为0%的偏差故障恢复率提升至59.4%。

Conclusion: 结合了机器学习参数估计与解析控制律的结构化设计，能更有效地泛化到未知的航天器执行器故障，是解决该领域硬伤（如常数偏差）的关键。
...
### [26] [Offline Multi-agent Continual Cooperation via Skill Partition and Reuse](https://arxiv.org/abs/2606.25389)
*Yuchen Xiao,Lei Yuan,Ruiqi Xue,Tieyue Yin,Yang Yu*

Main category: cs.AI

TL;DR: 提出了一种名为COMAD的持续多智能体技能发现框架，旨在通过技能划分与复来解决多智能体离线强化学习中的灾难性遗忘与任务干扰问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体技能提取方法依赖固定大小的技能库，在面对序列任务时，容易因分布偏移和技能空间膨胀导致灾难性遗忘和可塑性丧失。

Method: 设计了COMAD框架，利用自编码器从混合多智能体数据中提取协调技能，并通过多头架构的技能增强策略学习目标以及基于密度的可复用性评估器来实现技能的持续发现与复用。

Result: 在多个多智能体强化学习基准测试中，COMAD成功扩展了技能库并减轻了干扰，在任务流的前向和后向迁移性能上均显著优于现有基线方法。

Conclusion: COMAD有效实现了在开放环境中持续发现和复用多智能体协调技能的机制，在理论和实验上均证明了其能够有效逼近持续技能发现的优化目标。
...
### [27] [BrainAgent: A Large Language Model-Driven Multi-Agent Framework for Autonomous Brain Signal Understanding](https://arxiv.org/abs/2606.25400)
*Yangxuan Zhou,Sha Zhao,Jiquan Wang,Shijian Li,Gang Pan*

Main category: cs.AI

TL;DR: 本文提出了 BrainAgent，一个基于大语言模型的多智能体框架，旨在通过自动化脑信号分析工作流来降低脑机接口的技术门槛。


<details>
  <summary>Details</summary>
Motivation: 现有的脑信号分析方法技术门槛高，且多为静态、特定任务的模式，无法应对现实部署中复杂的长程工作流。

Method: 设计了 BrainAgent 分层架构框架，由中央监督者协调多个专业子智能体进行任务分解与执行，并构建了系统性的评估基准。

Result: 实验表明，BrainAgent 能够以高可靠性自动执行复杂的脑信号处理工作流。

Conclusion: BrainAgent 成功将抽象的自然语言意图转化为可执行的脑信号处理管道，推动了脑信号理解的民主化与范式转变。
...
### [28] [Long-Term Simulation Exposes Cognitive-Developmental Risks in AI Companions](https://arxiv.org/abs/2606.25396)
*Kaicheng Shen,Lingyu Li,Wen Wu,Yan Teng,Liang He,Yingchun Wang*

Main category: cs.AI

TL;DR: 本文提出TSJ纵向评估框架，用于评估AI伴侣在与发展中用户长期交互中产生的认知与心理发育风险。


<details>
  <summary>Details</summary>
Motivation: 现有的AI安全评估多依赖单轮或短期测试，无法捕捉因长期互动而累积的认知发育风险。

Method: 提出TSJ（剧场-舞台-裁判）框架，结合角色驱动的用户模拟、动态心理状态更新和回顾性评估，模拟了12,960人天的长期交互。

Result: 短期测试严重低估了风险，TSJ在交互达140轮后才能稳定评估风险；研究发现幼儿期和青年早期最脆弱，认知信任和情感依赖是主要受损领域。

Conclusion: TSJ为AI伴侣系统的长期认知发育风险评估提供了一种可扩展的系统性方法。
...
### [29] [Cliff Tokens: Identifying Single-Token Failure Triggers in LLM Mathematical Reasoning](https://arxiv.org/abs/2606.25524)
*Jaeyong Ko,Pilsung Kang,Yukyung Lee*

Main category: cs.AI

TL;DR: 本文定义并研究了导致大模型数学推理失败的关键“悬崖 Token（cliff token）”，并提出了一种针对该位置进行偏好优化的方法以提升推理准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的研究主要在步骤、句子或已发生错误的位置分析推理失败，无法精确定位具体是哪个 Token 触发了模型走向失败的转变。

Method: 引入“悬崖 Token”概念（基于 z 检验的自适应阈值识别潜能骤降点），并基于贪婪选择和 Token 熵提出悬崖分类法（确定性、不确定性、采样偏差悬崖）；最后设计了在悬崖位置进行单 Token 偏好优化的算法（Cliff-DPO）。

Result: 删除首个悬崖 Token 并重新采样可将 pass@64 恢复至 1.0；Cliff-DPO 训练后在多个数学基准测试上将模型准确率提升了高达 6.6%，且优化不确定和采样偏差悬崖的效果最为显著。

Conclusion: 识别并针对性地优化悬崖 Token（尤其是非确定性悬崖）是改善大语言模型数学推理能力的有效途径。
...
### [30] [Quantization Inflates Reasoning: Token Inflation as a Hidden Cost of Low-Bit Reasoning Models](https://arxiv.org/abs/2606.25519)
*Xinyu Lian,Walid Krichene,Beichen Huang,Masahiro Tanaka,Olatunji Ruwase,Li Zhang,Minjia Zhang*

Main category: cs.AI

TL;DR: 低比特量化虽能保持推理模型的准确率，但会导致其生成更长的思维链（Token膨胀），从而抵消预期的推理加速效果。


<details>
  <summary>Details</summary>
Motivation: 传统的量化评估主要关注准确率和单Token延迟，忽略了量化可能引入的隐性计算成本（如生成更长的思维链）。

Method: 在多项推理任务上测试了INT4/INT3量化模型，引入了“CoT Token膨胀率”指标，并评估了提示词工程、解码采样以及量化感知训练（QAT）等缓解策略。

Result: 低比特量化会导致模型推理步骤增加和语义重复，引发Token膨胀；提示词和采样调整在准确率与长度的权衡上表现不稳定，而量化感知训练（QAT）在减少Token膨胀方面更具前景。

Conclusion: 在评估量化推理模型时，应当将推理Token的使用量与准确率一同作为核心指标进行报告。
...
### [31] [Reasonable Motion: A General ASP Foundation for Environment Constrained Movement Trajectory Computation](https://arxiv.org/abs/2606.25626)
*Julius Monsen,Jakob Suchan,Mehul Bhatt,Lars Karlsson*

Main category: cs.AI

TL;DR: 提出一种基于回答集程序（ASP）的定量与定性混合方法，用于计算移动物体在现实世界中受约束的分支轨迹模式。


<details>
  <summary>Details</summary>
Motivation: 现有的纯学习方法在计算移动物体轨迹时，无法提供可验证的可解释性，且难以有效结合领域规范和地图拓扑等复杂约束。

Method: 利用回答集程序（ASP）对环境图进行约束遍历，将几何可行的运动行为枚举为稳定模型，从而结合领域相关和无关的特征计算出不同的轨迹模式。

Result: 在大型真实世界自动驾驶基准数据集 Argoverse 2 上进行了实证评估，展示了计算轨迹与其底层稳定模型之间的可追溯性，提供了可验证的可解释性。

Conclusion: 该方法具有广泛的适用性，能够有效解决自动驾驶等动态领域中移动物体的轨迹计算问题，并提供传统机器学习方法所缺乏的可解释性。
...
TL;DR: 提出了一种物理接地的多智能体发现引擎，能自主设计出符合硬件约束的基座模型压缩与部署方案。


<details>
  <summary>Details</summary>
Motivation: 现有的通用AI智能体在进行科学发现时缺乏物理接地，容易设计出不兼容硬件的系统。

Method: 构建了由演化知识图谱锚定的多智能体发现引擎，通过提取“算法思维链”将随机搜索转化为有方向的结构演化。

Result: 开发出Q-Enhance和MoE-Salient-AQ两种压缩方法，成功将235B参数的大模型部署在双A100服务器上，显存减少75%且精度仅降低0.64%。

Conclusion: 该研究确立了一种在严格物理边界下进行机器自主发现的可扩展软硬件协同设计范式。
...
### [33] [GUI agent: Guided Exploration of User-Sensitive Screens](https://arxiv.org/abs/2606.25705)
*Aradhana Nayak,Mussadiq Nazeer,Wang Peng,Feng Liu*

Main category: cs.AI

TL;DR: 本文提出了一个探索者智能体，通过系统探索GUI环境中的查询空间，识别会导致用户敏感状态的操作，以实现关键时刻的任务接管。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在GUI中自动执行任务时会接触用户敏感信息，但现有的智能体通常忽视安全风险强行执行。为了安全部署，需要识别敏感状态并在关键时刻让用户接管任务。

Method: 开发了一种探索者智能体（explorer agent），从一个已演示的任务出发，系统性地探索查询空间，以识别出执行后会导致GUI环境中出现用户敏感状态的查询。

Result: 该方法能够自动识别导致敏感状态的查询，有助于构建数据集，帮助工程师使智能体在关键场景中识别并请求用户接管。

Conclusion: 通过系统探索查询空间，可以有效识别涉及用户隐私和安全的敏感状态，为提高LLM智能体在真实GUI环境中的安全性和可靠性提供了支持。
...
### [34] [Position Spaces and Graphs](https://arxiv.org/abs/2606.25719)
*Rita-Nathalia Assaf,Tom Davot,Frédéric Lardeux,Frédéric Saubion*

Main category: cs.AI

TL;DR: 本文提出了一种名为“位置图”的图推理框架，用于建模离散标记的相对位置，并对其一致性和计算复杂度进行了理论分析。


<details>
  <summary>Details</summary>
Motivation: 旨在为文档处理等应用中的位置约束提供一个独立于具体数据提取技术的形式化逻辑层和数学基础。

Method: 利用表示水平与垂直对齐及优先顺序的两个严格偏序构建位置图，并引入链条件和行列兼容性约束。

Result: 确定了位置图一致性的判定条件，并证明了其结构模式发现问题（建模为诱导子图同构）即使在受限的位置图类别中也是NP完全的。

Conclusion: 该工作成功定义并分析了位置图的代数一致性与数学性质，为基于位置的推理提供了理论支撑。
...
### [35] [Fuzzy Quantification over OWL Ontologies and Knowledge Graphs](https://arxiv.org/abs/2606.25778)
*Enrique Palacín,Fernando Bobillo,Ignacio Huitzil,Francesca A. Lisi,Umberto Straccia*

Main category: cs.AI

TL;DR: 本文提出了一个通用框架及其开源实现 Q2S2，用于在标准和模糊本体及知识图谱上评估模糊量化查询。


<details>
  <summary>Details</summary>
Motivation: 旨在实现在不同本体数据源（如OWL或RDFS）中，检索满足I型或II型模糊量化查询表达式的个体。

Method: 设计了一种对量词类型、评估方法和数据源均保持无关（自适应）的通用评估框架，并开发了开源系统 Q2S2。

Result: 成功构建了该自适应框架，并提供了公开可用的系统实现以支持未来研究。

Conclusion: 该框架为在多种本体和知识图谱上处理模糊量化查询提供了一种高效且灵活的通用解决方案。
...
### [36] [AI Snitches Get Glitches: Towards Evading Agentic Surveillance](https://arxiv.org/abs/2606.25836)
*Hyejun Jeong,Dzung Pham,Amir Houmansadr,Eugene Bagdasarian*

Main category: cs.AI

TL;DR: 本文研究了AI智能体被滥用以监视用户的安全风险，提出了评估基准SurveilBench，并探讨了防御规避技术。


<details>
  <summary>Details</summary>
Motivation: 随着AI智能体广泛获取用户数据和API权限，存在被雇主或国家利用进行用户监视的新型隐私风险。

Method: 形式化了智能体监视问题，构建了包含企业、教育和警务场景的评估数据集SurveilBench，并开发了三种利用提示注入进行规避的技术。

Result: 研究发现部分模型存在辅助监视的涌现倾向，同时所提出的规避技术能有效对抗智能体监视（如欺骗、隐藏或诱导其过度反应）。

Conclusion: 智能体监视目前已极易实现，亟需建立全面的技术、伦理和立法框架来保护用户隐私。
...
### [37] [Confidence Sequences for Online Statistical Model Checking of Markov Decision Processes](https://arxiv.org/abs/2606.25797)
*Konstantin Kueffner,Tobias Meggendorfer,Maximilian Weininger,Patrick Wienhöft*

Main category: cs.AI

TL;DR: 本文提出了一系列专为马尔可夫决策过程（MDP）在线估算设计的“置信序列”方法，大幅减少了确定转移概率所需的样本量。


<details>
  <summary>Details</summary>
Motivation: 在无法预知MDP精确转移概率的实际应用中，现有的在线统计采样方法要么存在理论缺陷，要么效率低下（样本开销过大）。

Method: 设计并实现了一系列适用于在线设置的置信序列（confidence sequences），并构建了高效的计算工具来逐步收敛转移概率的边界。

Result: 该方法优于传统的联合界（union-bound）方法，在平均样本需求量上比现有最先进技术减少了50倍。

Conclusion: 所提出的置信序列及其工具化实现为MDP在线概率估计提供了一种准确、高效且实用的解决方案。
...
### [38] [Agentic System as Compressor: Quantifying System Intelligence in Bits](https://arxiv.org/abs/2606.25960)
*Zihan Qin,Hongrui Zhang*

Main category: cs.AI

TL;DR: 本文提出从“压缩即智能”的视角出发，利用码长（codelength）来评估和量化大语言模型智能体系统的能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型正从孤立的预测器演变为复杂的智能体系统，亟需一种统一的量化分析框架来评估其性能和效率。

Method: 将“压缩即智能”的观点具体化，结合算术编码、种子编码和回退机制，在反向文本、象棋步法、蛋白质序列、检索增强问答及语义故事压缩五个场景中进行评估。

Result: 在所有测试场景中，智能体组件的加入均成功减少了重构目标对象所需的码长。

Conclusion: 基于码长的度量方法能够有效分析系统组件、观察者和计算预算对残余不确定性的影响，为真实智能体系统的评估提供了指导。
...
### [39] [InvestPhilBench: A Multi-Layer Dynamic Benchmark for Evaluating Large Language Model Procedural Reasoning in Expert Investment Philosophy](https://arxiv.org/abs/2606.25984)
*Mingguang Chen,Bo Qu*

Main category: cs.AI

TL;DR: 本文提出了 InvestPhilBench，一个用于评估大语言模型重建和应用专家投资者程序化决策框架能力的多层动态基准测试。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型越来越多地被用作投资研究助手，但目前还没有基准能够测试它们是否能准确重建并应用专业投资者的特定决策框架。

Method: 引入了包含8个认知层级的 InvestPhilBench 基准（包含投资原则卡、决策框架卡和问答题），并开发了基准自动评分流水线（BASP）、失效模式检测协议（FMDP）以及门控重建准确率（GRA）等评估方法。

Result: 实验表明，虽然前沿模型在复合评分（BASP）上接近饱和（如 Claude L4 达 0.932），但 GRA 指标揭示了它们在程序化推理上的缺陷（L7 GRA 仅为 0.57-0.62）；此外，自动评估指标与人类专家评估具有高度相关性（Pearson r = 0.72）。

Conclusion: 传统的复合评分容易因模型流畅的语言表达而掩盖其在程序化决策上的不足，需要使用像 GRA 这样更精细的指标来准确评估模型的推理缺陷。
...
### [40] [WinDOM: Self-Family Distillation for Small-Model GUI Grounding](https://arxiv.org/abs/2606.25964)
*Chengheng Li-Chen,Zhiqian Zhou,Hao Chen,Nicolas Chauvin*

Main category: cs.AI

TL;DR: 本文提出了一套提升小型（约2B参数）GUI定位智能体性能的方案，通过自动构建的WinDOM数据集和基于自同族蒸馏（SFD）的早期初始化强化学习（GRPO），在无需人工标注的情况下显著提升了小模型的定位精度。


<details>
  <summary>Details</summary>
Motivation: 小型GUI定位智能体非常适合端侧部署，但在如何低成本获取边界框训练数据，以及如何将监督微调与强化学习有效结合方面仍存在挑战。本文旨在探索不依赖扩大模型规模的优化路径。

Method: 1. 自动收集WinDOM数据集（包含54,425条记录，通过无头Playwright直接从网页DOM中提取边界框）；2. 提出自同族蒸馏（SFD）进行拒绝采样冷启动；3. 采用早期初始化RL，将冷启动的饱和深度作为GRPO的超参数，使用未完全收敛的初始化模型进行强化学习。

Result: 在Qwen3.5-2B模型上，采用SFD-4B配合早期初始化RL，在分布外（OOD）测试集上平均表现提升了5.4分。此外，不依赖外部教师的EMA自蒸馏模式取得了65.2分，逼近有4B教师模型指导的66.3分。

Conclusion: 高质量自动标注数据集与控制冷启动饱和度的强化学习相结合，能大幅激发小模型在GUI定位任务上的潜力，甚至在不引入外部大模型教师的情况下也能取得优异表现。
...
### [41] [Autodata: An agentic data scientist to create high quality synthetic data](https://arxiv.org/abs/2606.25996)
...
TL;DR: 提出Autodata方法，使AI智能体能够作为数据科学家来自动构建高质量的训练和评估数据。


<details>
  <summary>Details</summary>
Motivation: 旨在解决如何高效生成高质量合成数据的问题，探索将推理计算转化为更高质量模型训练数据的方法。

Method: 提出Autodata框架，包括对数据科学家智能体进行元优化（meta-optimization），以及具体实现方法Agentic Self-Instruct。

Result: 在计算机科学研究、法律和数学推理等任务中取得了优于传统合成数据方法的成果，且元优化智能体带来了更显著的性能提升。

Conclusion: 代理化数据创建是一种将推理算力转化为高质量训练数据的有效途径，有潜力改变AI数据的构建方式。
...
### [42] [The Unfireable Safety Kernel: Execution-Time AI Alignment for AI Agents and Other Escapable AI Systems](https://arxiv.org/abs/2606.26057)
*Seth Dobrin,Łukasz Chmiel*

Main category: cs.AI

TL;DR: 本文提出并实现了一种名为“Unfireable Safety Kernel”的架构级安全内核，通过形式化验证防止AI智能体逃逸和未经授权的自我修改。


<details>
  <summary>Details</summary>
Motivation: 当前的AI防御机制（如系统提示词、护栏）通常与智能体处于同一运行空间，易受输入对抗影响而被绕过（即“可逃逸的AI”），因此亟需一种架构隔离的硬性授权控制机制。

Method: 定义了执行时AI对齐的四项安全属性，并用Rust实现了安全内核，利用Z3（SMT定理证明）和Kani（有界模型检测）进行双重形式化验证，确保其“故障闭锁”的安全性。

Result: 在针对自主自我改进型AI系统的对抗测试中，该内核成功拦截了所有704次针对核心代码的修改尝试和300次停机测试，在6,240次授权交互中实现了零绕过，效果显著优于现有的控制方案。

Conclusion: 架构级隔离与形式化验证的安全内核能有效实现执行时AI对齐，为防止自主AI智能体失控提供了强有力的安全屏障。
...
### [43] [SEMIR: Topology-Preserving Graph Minors for Thin-Structure Segmentation](https://arxiv.org/abs/2606.24935)
*Luke James Miller,Yugyung Lee*

Main category: cs.CV

TL;DR: 提出了SEMIR框架，通过参数化图式子式在无需切片的情况下保留细长结构分割的连通性，支持高分辨率推理。


<details>
  <summary>Details</summary>
Motivation: 传统的图像表示方法（如切片和常规超像素）在分类前会破坏1-3像素宽细长结构的连通性，且拓扑感知损失无法挽回这种底层的表示破坏。

Method: 使用参数化图式子式（graph minor）替代像素网格，将数百万像素折叠为保留连通性的边界对齐超节点；利用轻量级图神经网络（GNN）对简化图分类，再通过精确映射恢复到像素分辨率。

Result: 在TTPLA、CrackSeg9k和SkyScapes Lane数据集上达到或超过特定领域基线，相比SLIC方法减少了至少4.6倍的掩码碎片化，并支持高达2100万像素图像的无切片全分辨率推理。

Conclusion: SEMIR是一种统一且高效的细长结构分割框架，成功解决了超像素和切片操作破坏拓扑连通性的问题，在大图推理上具有显著优势。
...
TL;DR: 提出了一种基于曲率指导的模型混合框架（CGM），通过损失景观的二阶近似来合并预训练和微调的MLLM，以缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在专用任务微调时容易产生通用能力的灾难性遗忘，而现有合并方法多为启发式且效果非最优。

Method: 引入CGM框架，利用Hessian矩阵近似推导解析闭式解进行“软混合”，并提出基于曲率敏感评分的稀疏参数选择变体CGM†进行“硬混合”。

Result: 在LLaVA-1.5 and Qwen2.5VL上的实验表明，CGM和CGM†在多项下游任务中均优于现有合并方法，能更好地平衡任务专业化与通用知识保留。

Conclusion: CGM为多模态大模型的模型合并提供了具有理论支撑的高效解决方案，有效解决了微调中的灾难性遗忘问题。
...
### [45] [Yuvion VL: A Multimodal Foundation Model for Adversarial Content and AI Safety](https://arxiv.org/abs/2606.25034)
...
TL;DR: 本文提出了 Yuvion VL，一个专为内容与 AI 安全设计的核心多模态大语言模型系列，通过对抗性数据构建和对比微调提升了安全性能。


<details>
  <summary>Details</summary>
Motivation: 通用多模态模型在识别和理解现实世界中的多模态风险（尤其是对抗性内容）方面表现不佳，亟需将安全视为多模态对抗问题的专用模型。

Method: 开发了包含对抗感知数据合成的管道，采用三阶段训练流程，引入“混淆-对比”微调框架，并构建了评估基准 Yuvion VL RiskEval (YVRE)。

Result: Yuvion VL-32B 在安全性能上达到了行业领先水平，超越了同等规模的开源模型及顶尖的闭源商业模型，同时保持了出色的通用能力。

Conclusion: 通过将安全问题视为对抗性多模态任务，并结合针对性的数据合成、多阶段训练与对比学习，可以有效构建兼具高安全性和强通用能力的多模态模型。
...
### [46] [Chorus II: Cross-Request Sparsity Reuse for Efficient Image-to-Video Generation](https://arxiv.org/abs/2606.25040)
*Hao Liu,Chenghuan Huang,Hao Liu,Xing Cai,Chen Li,Ziyang Ma,Jing Lyu,Nong Xiao,Jiangsu Du*

Main category: cs.CV

TL;DR: 本文提出了一种通过复用历史相似请求的稀疏注意力掩码来加速图像到视频（I2V）扩散模型推理的框架，在不损失视频质量的前提下实现了2.16倍的加速。


<details>
  <summary>Details</summary>
Motivation: 图像到视频（I2V）模型的计算成本高昂，且实际应用中存在大量相似请求，现有加速方法主要依赖特征复用，忽略了相似请求之间高度一致的稀疏注意力模式。

Method: 提出以稀疏性复用为核心的框架，直接复用历史相似请求的高质量稀疏掩码，并结合可选的特征复用和引导增强技术，以减少计算量并避免语义漂移。

Result: 实验表明，在默认的稀疏性复用配置下，该方法在保持视频生成质量的同时，实现了2.16倍的推理加速。

Conclusion: 跨请求的稀疏性复用是一种高效且保真的I2V扩散模型加速服务方案，具有很高的实际部署价值。
...
### [47] [Noise-Aware Boundary-Enhanced Generative Learning for Ultrasound Speckle Reduction](https://arxiv.org/abs/2606.25009)
*Yuexi Gu,Mengqi Wu,Yongheng Sun,Virginie Papadopoulou,Mingxia Liu,Maureen Kohi*

Main category: cs.CV

TL;DR: 本文提出了一种用于超声去噪的噪声感知边界增强生成学习（NBGL）框架，在抑制斑点噪声的同时有效保护解剖边界。


<details>
  <summary>Details</summary>
Motivation: 传统超声去斑方法容易过度平滑组织边界，且对不同噪声水平的适应性较差。

Method: 构建包含去斑和边界增强的双分支架构，结合噪声感知交互权重生成（NIWG）模块与加权特征线性调制（wFiLM）模块自适应调节特征耦合。

Result: 在141个3D经阴道超声体积上的实验表明，该方法在六种噪声水平下的去斑与结构保护性能均优于现有最先进方法。

Conclusion: NBGL框架能够有效抑制超声斑点噪声并精确保留解剖边界，表现出强鲁棒性和自适应性。
...
TL;DR: 提出一种通过学习权重空间中的量化感知线性路径来进行神经网络量化的新方法，无需STE和显式离散化训练即可保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 量化过程中的离散约束会使参数偏离最优解导致性能下降。本研究旨在探索能否构建一个位于全精度模型低损失子空间内的量化模型，从而自动保留性能。

Method: 通过在权重空间中学习以最小化损失为目标的量化感知线性路径，并将所得子空间的中点进行直接量化。

Result: 该子空间中点的直接量化性能与传统的量化感知训练（QAT）相当。

Conclusion: 该方法为权重量化提供了全新视角，在训练过程中既不需要直通估计器（STE），也不需要显式的离散化操作。
...
### [49] [Are We There Yet? Exploring the Capabilities of MLLMs in Assistive AI Applications](https://arxiv.org/abs/2606.25084)
*Shayon Dasgupta,Avijit Dasgupta,C. V. Jawahar*

Main category: cs.CV

TL;DR: 本文评估了多模态大语言模型（MLLMs）在辅助AI（Assistive AI）真实场景中的表现，并推出了一个基于第一人称视角数据的基准测试。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在通用任务上表现优异，但其在辅助AI场景（如视觉识别、上下文推理和多语言理解）中的实际效果仍有待深入探索。

Method: 开发了名为NetraLink的头戴式GoPro数据采集系统以收集真实的第一人称视角数据，构建了涵盖日常物体识别、场景文本问答及多语言阅读的测试基准，并对前沿MLLMs进行了评估。

Result: 对当前MLLMs在辅助技术中的表现进行了全面诊断，揭示了它们在处理基于视觉感知和语言交互任务时的具体优势与局限性。

Conclusion: 该研究明确了MLLMs在支持辅助AI方面的潜力和瓶颈，为未来结合视觉与语言的辅助技术发展提供了重要诊断依据。
...
### [50] [Wan-Streamer v0.1: End-to-end Real-time Interactive Foundation Models](https://arxiv.org/abs/2606.25041)
...
TL;DR: Wan-Streamer 是一个原生流式、端到端的双向音视频交互多模态基座模型，支持超低延迟的实时互动。


<details>
  <summary>Details</summary>
Motivation: 传统的级联交互系统依赖多个独立模块，导致高延迟和误差累积，因此需要统一的端到端模型来实现高效的跨模态同步与实时交互。

Method: 在单一 Transformer 中通过块因果注意力统一建模多模态输入输出，并重构了包含因果编解码器和低延迟 Token 调度的完整流式技术栈。

Result: 实现了 160 毫秒的流式单元，模型端延迟仅约 200 毫秒，结合网络延迟后总交互延迟控制在 550 毫秒左右。

Conclusion: 该研究成功验证了统一端到端多模态流式架构在实现亚秒级实时音视频双工交互方面的可行性与优越性。
...
### [51] [FreeStory: Training-Free Character Consistency for Free-Form Visual Storytelling](https://arxiv.org/abs/2606.25079)
*Sibo Dong,Ismail Shaheen,Sarah Adel Bargal*

Main category: cs.CV

TL;DR: 提出了FreeStory，一种无需训练的视觉故事生成框架，通过基于实体的特征复用，在自由文本提示下实现了跨图像的高角色一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖于在每个提示中重复完整角色描述的结构化文本，这与自然故事中角色通常仅在首次引入、后续使用代词或简写指代的实际情况相违背。

Method: 将角色一致性重构为基于实体的特征复用，结合了动态角色掩码、对应感知特征匹配、键值（KV）注入和查询混合技术，并引入了包含单/多角色故事的FreeStoryBench基准。

Result: 实验表明，FreeStory在结构化基准上达到了无训练方法的最先进（SOTA）水平，并在自由文本提示下表现出比基准更强的整体一致性。

Conclusion: FreeStory成功在无需训练的前提下解决了自由文本视觉故事生成中的角色一致性挑战，在保持生成多样性的同时有效维持了角色身份。
...
### [52] [Cage-based Texture Transfer with Geometric Filtering](https://arxiv.org/abs/2606.25220)
*Rose Mei Zhou,Lynnette Hui Xian Ng,Adrian Xuan Wei Lim,Conor Griffin,Faraz Baghernezhad*

Main category: cs.CV

TL;DR: 提出了一种基于笼体几何滤波的实时纹理传递方法，在保证低延迟的同时有效抑制伪影。


<details>
  <summary>Details</summary>
Motivation: 现有的实时纹理传递方法在处理速度和伪影抑制之间存在权衡：低延迟方法容易产生伪影，而鲁棒的方法则需要高昂的训练成本和内存开销。

Method: 采用基于笼体（cage-based）的几何滤波方法来识别非化妆区域（NCZs），从而抑制伪影，避免了对大规模模型和手动标注数据集的依赖。

Result: 可在消费级硬件上即时部署，在移动设备上处理包含约4.8k三角形的网格仅需约70毫秒。

Conclusion: 该方法成功平衡了效率与鲁棒性，为交互式应用提供了一种无需密集训练且高效的纹理传递解决方案。
...
### [53] [Reflective VLA: In-Context Action Consequences Make VLAs Generalize](https://arxiv.org/abs/2606.25215)
*Qing Lian,Kent Yu,Lei Zhang*

Main category: cs.CV

TL;DR: 提出Reflective VLA模型，通过引入“观察-动作-后果”三元组历史信息，使机器人能识别环境特定的偏差，从而显著提升跨环境的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统的反应式VLA模型仅依赖当前观察，难以应对相机几何变化、机器人校准或执行偏差等具体化因素，导致在部署环境发生变化时泛化性能较差。

Method: 设计了Reflective VLA架构，将决策条件限制在包含“观察-动作-后果”的三元组历史上下文中；采用共享注意力机制融合多模态，并使用块因果掩码支持高效的并行多帧训练与KV缓存实时推理。

Result: 在LIBERO和SimplerEnv-Bridge上保持了强劲的分布内性能；在存在分布偏移的LIBERO-Plus和LIBERO-Plus-Hard基准上，成功率较基线分别提升了5.4和4.2个百分点；消融实验证实，动作的“后果”信息而非单纯的历史长度是实现跨环境泛化的核心。

Conclusion: 通过在决策中引入动作后果的反馈闭环，Reflective VLA有效克服了具体化控制中的环境过拟合问题，大幅增强了模型在面对实际部署偏差时的适应力。
...
### [54] [MJEPA: A Simple and Scalable Joint-Embedding Predictive Architecture for Audio-Visual Learning](https://arxiv.org/abs/2606.25225)
*Revant Teotia,Adrien Bardes,Michael Rabbat,Sumit Chopra,Matthew J. Muckley,Nicolas Ballas*

Main category: cs.CV

TL;DR: 提出MJEPA，一种使用单一统一编码器和预测目标的视听联合嵌入预测架构，在多项音频和视频任务中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视听自监督方法依赖特定模态的编码器和复杂的混合目标，限制了跨模态协同与可扩展性，而现有的JEPA方法通常仅用于单模态。

Method: 引入MJEPA，对音频和视频使用单一的统一编码器，并采用在模态内和跨模态之间均适用的单一预测目标（跨模态预测）。

Result: 冻结的ViT-g模型在AudioSet-20K上比先前基线提升了6.8 mAP，在ESC-50和FSD50K上超越了全微调模型，且在视频基准上用少10倍的数据实现了有竞争力的表现。

Conclusion: 跨模态预测是防止共享编码器退化的关键，MJEPA展示了视听双模态在统一预测架构下相互促进的有效性。
...
TL;DR: 本文提出块稀疏特征生成器（BSF），将神经网络中的视觉概念表示为低维几何流形而非单一方向，从而更紧凑地解释和控制表征。


<details>
  <summary>Details</summary>
Motivation: 传统的解释方法将概念视为孤立的激活方向，忽略了概念在激活空间中往往呈现低维几何流形结构的实际情况。

Method: 利用结构化稀疏（块稀疏）将激活方向分组为块，构建了三种块稀疏特征生成器（BSF）变体，并基于最小描述长度进行量化分析。

Result: BSF能更紧凑地描述激活（恢复的概念多为2-4维）；证实了InceptionV1的曲线检测器基于单一连续曲线流形；在DINOv3中发现了阴影与光照流形；并在SDXL中实现了基于流形引导的可控图像生成。

Conclusion: 视觉特征更适合被表征为神经元协同工作的低维流形而非单一神经元方向，这为神经网络的可解释性与生成控制提供了新视角。
...
### [56] [OrthoTrack: Continuous 6-DoF UAV Trajectory Estimation Anchored in Public Orthophotos](https://arxiv.org/abs/2606.25245)
*Oussema Dhaouadi,Zuria Bauer,Johannes Michael Meier,Olaf Wysocki,Marc Pollefeys,Daniel Cremers*

Main category: cs.CV

TL;DR: OrthoTrack是一个无需训练的系统，利用公开的正射影像和表面模型，实现了实时、无漂移的无人机绝对6-DoF位姿估计。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉里程计/SLAM方法存在累计漂移且仅有相对尺度，而单帧地理定位方法速度慢且缺乏时间连续性。

Method: 将关键帧与正射影像匹配并结合表面模型获得3D对应关系，再通过光流将对应关系传播至中间帧，以估计绝对位姿；并推出了MovingDrone数据集。

Result: 在单GPU上实现实时运行，在MovingDrone和真实数据集上性能大幅超越所有基准方法（包括已知理想尺度和对齐的方法）。

Conclusion: 通过利用公开地理数据，OrthoTrack无需特定区域的适配即可部署到新场景，实现了高效的绝对位姿估计。
...
### [57] [Multilingual Hematology Visual Question Answering Dataset](https://arxiv.org/abs/2606.25246)
*Hajra Malik,Hafiza Tooba Aftab,Abdul Rehman,Mohsen Ali,Waqas Sultani*

Main category: cs.CV

TL;DR: 本文提出了WBCMor VQA，这是一个用于白细胞分析的双语（英语和乌尔都语）形态学感知视觉问答（VQA）基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有的血液学视觉语言模型资源主要以英语为主，限制了其在多语言医疗环境（如使用乌尔都语的巴基斯坦）中的应用。

Method: 基于LeukemiaAttri和WBCAtt数据集的形态学标注，结合特定领域的乌尔都语血液学词典构建数据集，并评估了多种开源视觉语言模型（VLM）的基线表现。

Result: 成功构建了包含110K个双语问答对及20K张图像的临床验证基准数据集，并建立了开源VLM的基线性能。

Conclusion: 该基准数据集有助于促进适用于多语言医疗环境的、具有临床相关性的人工智能系统开发。
...
### [58] [Cross-Modality Structural Guidance in 3D Latent Diffusion for Robust FLAIR Super-Resolution](https://arxiv.org/abs/2606.25255)
*Haoyu Lan,Jiazhen Zhang,John Onofrey,Bino Varghese,Nasim Sheikh-Bahaei,Arthur W. Toga,Jeiran Choupan*

Main category: cs.CV

TL;DR: 本文提出MR-DiffuSR，一种在3D潜空间中利用高分辨率T1w结构先验指导厚层FLAIR超分辨率重建的扩散模型，有效避免了解剖结构幻觉并提升了图像重建质量。


<details>
  <summary>Details</summary>
Motivation: 高分辨率MRI扫描受限于扫描时间，常产生厚层等低分辨率图像；现有超分辨率算法易产生解剖细节幻觉，影响脑结构完整性。

Method: 提出基于3D潜空间的扩散超分辨率模型，引入跨模态结构Swin注意力机制以解耦解剖结构与模态对比度，并采用混合尺度退化策略和基于DINOv3的感知损失进行训练。

Result: 在ADNI-4数据集上，该方法取得PSNR 32.46dB、SSIM 0.97的优异表现；在10倍下采样下，下游白质高信号分割的Dice分数仍能保持在0.63，显著优于基线模型。

Conclusion: MR-DiffuSR通过有效融合跨模态结构先验，成功抑制了超分辨率重建中的幻觉，并在极低分辨率下展现出极强的鲁棒性和临床实用价值。
...
### [59] [Pre-Warm: Input-Conditioned Weight Initialization for Convolutional Neural Networks](https://arxiv.org/abs/2606.25256)
*Rowan Martnishn*

Main category: cs.CV

TL;DR: 一种无需训练成本的第一卷积层初始化方法 Pre-Warm，通过聚类输入图像块来初始化滤波器，显著提升了分类准确率。


<details>
  <summary>Details</summary>
Motivation: 传统的卷积层随机初始化方法未能利用输入数据的特征，研究者旨在探索如何通过轻量级、依赖输入数据的信号来优化神经网络的训练轨迹。

Method: 提出 Pre-Warm 方法：从单个训练批次中提取局部图像块，使用 MiniBatchKMeans 进行聚类并应用逆曼哈顿空间权重，以此初始化首层一半的滤波器，并为超参数推导了闭式规则。

Result: 在 MNIST、CIFAR-10 等五个基准数据集上，Pre-Warm 相比 Kaiming 初始化取得了显著的准确率提升（p < 0.05），且计算开销极小，无需修改网络架构。

Conclusion: 即使是轻量级、依赖输入的数据信号，也能有效改善现代卷积神经网络的优化轨迹并提升模型性能。
...
### [60] [CoGeoAD: Hierarchical Color-Geometric Fusion with Multi-View Attention for Zero-Shot 3D Anomaly Detection](https://arxiv.org/abs/2606.25273)
*Ke Xu,Xinle Wang,Yanning Hou,Xueliang Ma,Juan Xie,Jianfeng Qiu*

Main category: cs.CV

TL;DR: 提出CoGeoAD，一个基于CLIP的统一3D异常检测框架，通过有效融合2D颜色与3D几何特征实现高性能的零样本检测。


<details>
  <summary>Details</summary>
Motivation: 现有零样本3D异常检测方法缺乏将2D颜色图像与3D几何结构有效融合的机制，限制了其在统一框架下同时检测表面和结构缺陷的能力。

Method: 提出CoGeoAD框架，通过构建像素对齐的多视图图像融合颜色和几何特征，并引入数据驱动多视图注意力（MVA）机制自适应聚合3D特征，以及多阶段颜色-几何融合（MS-CGF）模块分层整合双模态特征。

Result: 在MVTec3D-AD和Eyecandies基准上取得了SOTA性能，能够有效捕获复杂工业场景中的结构和纹理异常。

Conclusion: CoGeoAD成功解决了多模态特征融合问题，为工业质检提供了一个高效的零样本3D异常检测统一解决方案。
...
### [61] [Heterogeneous and Adept Snapshot Distillation for 3D Semantic Segmentation](https://arxiv.org/abs/2606.25278)
*Xiaopei Wu,Yuenan Hou,Junkai Xu,Wenxiao Wang,Binbin Lin,Yu Li,Ping Li,Haifeng Liu,Deng Cai,Wanli Ouyang*

Main category: cs.CV

TL;DR: 本文提出了一种名为HAS-KD的知识蒸馏框架，通过将多模态教师模型和多个快照专家模型的知识转移到单点云网络中，在不增加推理负担的情况下提升3D语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 多模态融合和多模型集成虽能提升3D语义分割性能，但分别面临依赖辅助输入和计算成本高昂的问题。

Method: 提出异构与熟练快照知识蒸馏（HAS-KD），包含：1) 信息导向异构蒸馏（IHD）配合信息导向过滤（IOF）以传递多模态知识；2) 熟练快照蒸馏（ASD）利用训练过程中的模型快照作为专家，仅在各自擅长的类别上监督学生模型。

Result: 在ScanNetV2和S3DIS数据集上取得了最先进的（SOTA）结果，能够无缝集成到现有3D分割算法中并带来显著提升。

Conclusion: HAS-KD能够以极低的训练成本和零额外推理延迟，有效提升点云3D语义分割模型的表现。
...
### [62] [MRI2Rep: Autoregressive Structured Report Generation for 3D Liver MRI](https://arxiv.org/abs/2606.25279)
*Xinran Li,Junlin Yang,Annabella Shewarega,Zongwei Zhou,Julius Chapiro,James S. Duncan,Lawrence H. Staib*

Main category: cs.CV

TL;DR: 本文提出了首个用于3D肝脏MRI的端到端结构化报告生成框架MRI2Rep，通过自回归模型和无病灶标注的文本规范化方法，实现了临床可接受的自动报告生成。


<details>
  <summary>Details</summary>
Motivation: 手动撰写3D MRI报告耗时费力，且由于3D数据的复杂性和配对数据的匮乏，针对肝脏MRI的端到端结构化报告生成领域仍未得到充分研究。

Method: 提出自回归框架MRI2Rep，采用报告到标签规范化（RLC）模块将自由文本报告转换为结构化诊断序列（无需病灶级标注），并引入LLM-Eval进行自动评估。

Result: MRI2Rep实现了76.0%的病例级敏感性、29.4%的病灶级F1值和82.4%的肝脏级准确率，显著优于现有基线。放射科医生评估其生成的报告有70%-75%达到临床可接受水平。

Conclusion: 该系统是首个针对3D肝脏MRI的端到端LI-RADS结构化报告系统，证明了利用AI自动生成高质量临床MRI报告的可行性。
...
### [63] [Minimalist Preprocessing Approach for Image Synthesis Detection](https://arxiv.org/abs/2606.25297)
*Hoai-Danh Vo,Trung-Nghia Le*

Main category: cs.CV

TL;DR: 一种针对低配置设备设计的简便且高效的生成图像检测方法，通过计算像素梯度来识别虚假图像。


<details>
  <summary>Details</summary>
Motivation: 生成模型制造的虚假图像带来安全威胁，而低端智能设备急需轻量且高效的检测算法。

Method: 计算相邻像素间的梯度（起高通滤波器作用），以捕捉灰度强度变化并减少颜色对检测的干扰。

Result: 在多个数据集上的检测准确率与现有最先进方法相当，且计算资源消耗极低。

Conclusion: 该方法计算开销极小，非常适合部署在智能手机等低端设备上。
...
### [64] [Evaluation Protocols and Validation for Cameras in Indoor Healthcare Monitoring](https://arxiv.org/abs/2606.25284)
*Amirhossein Dadashzadeh,Jingjing Liu,Qianhui Men,Qiushuo Cheng,Kirsty Scott,Lisa Alcock,Ian Craddock,Majid Mirmehdi*

Main category: cs.CV

TL;DR: 评估了不同相机在真实室内干扰条件下的度量性能与三维姿态估计精度，为医疗监控设备的部署与选择提供实证指导。


<details>
  <summary>Details</summary>
Motivation: 基于相机的医疗监控系统在真实复杂室内环境下的技术性能缺乏系统性评估，且现有研究通常孤立地测试硬件度量或算法精度。

Method: 提出两种技术验证协议，系统评估了五款相机在不同光照、高度、视角和遮挡级别下的人体姿态估计和度量性能。

Result: 各相机深度偏差差异巨大（5米处偏差达50-1400mm）；2D姿态估计精度相近（mAP约78%-90%），但3D重建误差受深度感测质量及环境因素影响差异显著（MPJPE达104-365mm）。

Conclusion: 本研究为医疗监控应用中的相机选择和部署提供了基于证据的指导，填补了技术验证实践的空白。
...
### [65] [KidRisk: Benchmark Dataset for Children Dangerous Action Recognition](https://arxiv.org/abs/2606.25298)
*Minh-Kha Nguyen,Trung-Hieu Do,Kim Anh Phung,Thao Thi Phuong Dao,Minh-Triet Tran,Trung-Nghia Le*

Main category: cs.CV

TL;DR: 本文构建了用于识别儿童危险行为的新数据集 KidRisk，并提出了基于视觉-语言模型的基线方法，显著提升了识别效果。


<details>
  <summary>Details</summary>
Motivation: 儿童在缺乏监管时易发生危险，准确识别危险行为对保障儿童安全至关重要，而传统深度学习模型在这一任务上效果有限。

Method: 构建了包含 2,500 个短视频和 10,000 张图像的 KidRisk 数据集并建立基准，同时开发了基于视觉-语言（vision-language）的基线模型以增强上下文理解。

Result: 所提方法在儿童行为分类上达到 83.53% 的准确率，在危险行为识别上达到 96.14% 的准确率，显著优于传统深度学习方法。

Conclusion: 视觉-语言模型在检测儿童危险行为方面具有高可行性与高有效性，能为保障儿童安全做出积极贡献。
...
### [66] [HiFiVe: High-Fidelity Vehicle Generation Leveraging Auto-Regressive 2D Generative Priors](https://arxiv.org/abs/2606.25300)
*Hongli Xiao,Youjian Zhang,Qi Zheng,Zhaohui Hu,Yaohui Jin,Xiaoguang Ren,Wenjing Yang,Long Lan*

Main category: cs.CV

TL;DR: 本文提出了HiFiVe，一个无需训练的高保真三维车辆生成框架，通过结合三维几何约束与二维生成先验，实现了高分辨率纹理和精细几何的重建。


<details>
  <summary>Details</summary>
Motivation: 现有的三维车辆生成方法存在几何保真度低和纹理模糊的问题，且现有基于多视角扩散模型的方法受限于固定视角、低分辨率以及高昂的微调成本。

Method: 提出了一种自回归纹理细化管线，利用粗糙几何作为同步先验，通过深度投影和多视角融合确保跨视角一致性，并利用车辆对称性减少误差，最后利用法线贴图细化网格几何以恢复高频表面细节。

Result: 在合成和真实世界数据集上的实验表明，该方法在几何细节和纹理质量上均显著优于现有的前沿基线方法。

Conclusion: HiFiVe框架证明了通过有机结合二维生成先验与三维几何约束，无需训练即可有效提升三维车辆生成的保真度与一致性。
...
### [67] [Physics Question Scene Graph: Fine-grained Evaluation of Physical Plausibility in Text-to-Video Generation](https://arxiv.org/abs/2606.25306)
*Atin Pothiraj,Jaemin Cho,Yue Zhang,Elias Stengel-Eskin,Mohit Bansal*

Main category: cs.CV

TL;DR: 本文提出PQSG，一种基于分层问题场景图的评估方法，用于细粒度检测生成视频是否符合物理规律。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型难以遵循基本物理规律，且当前缺乏能够定位和具体说明视频中物理违规行为的细粒度评估方法。

Method: 引入物理问题场景图（PQSG）评估管线，利用视觉语言模型生成具有逻辑依赖的问题图，并构建了包含人类注释的FinePhyEval数据集进行验证。

Result: PQSG与人类评估结果具有高度一致性，能有效对主流模型（如Sora 2、Veo 3、Wan 2.1）的物理真实性进行排名，并暴露出当前视觉语言模型在回答物理问题上的局限性。

Conclusion: PQSG为评估视频生成中的物理合理性提供了一种有效且细粒度的工具，有助于推动视频生成模型和多模态理解能力的发展。
...
### [68] [ESTANet: Efficient Online Error Detection in Procedural Videos via Prediction Inconsistency](https://arxiv.org/abs/2606.25317)
*Shih-Po Lee,Reza Ghoddoosian,Faizan Siddiqui,Enna Sachdeva,Behzad Dariush*

Main category: cs.CV

TL;DR: 提出了一种轻量级实时框架 ESTANet，通过分析不同动作检测器预测结果的差异来检测视频中的过程错误。


<details>
  <summary>Details</summary>
Motivation: 旨在解决在线过程错误检测中架构设计过于复杂的问题，利用动作检测器本身对动态和时间上下文的敏感性差异来简化检测逻辑。

Method: 构建标准与错误敏感的动作检测器，使其在正常执行时表现相似但在发生错误时表现不同，并通过多数投票汇总其预测的不一致性以标记错误。

Result: 在 EgoPER、Assembly-101-O 和 EPIC-Tent-O 等数据集上达到了最先进的在线错误检测性能，且保持了实时的高效性。

Conclusion: 利用动作检测器的内在预测特性，无需复杂的网络架构设计即可实现高效实用的在线错误检测。
...
### [69] [REViT: Roto-reflection Equivariant Convolutional Vision Transformer](https://arxiv.org/abs/2606.25318)
*Sheir A. Zaheer,Alexander C. Holston,Chan Y. Park*

Main category: cs.CV

TL;DR: 本文提出了一种结合卷积注意力的离散旋转反射群等变视觉Transformer（ViT）。


<details>
  <summary>Details</summary>
Motivation: 旋转反射等变性在处理方向相关任务时非常有用，但现有研究多集中于CNN，在视觉Transformer中实现等变性仍面临挑战。

Method: 提出了一种更简单的方法来实现离散旋转反射群等变的视觉Transformer，并结合了卷积注意力机制。

Result: 实验结果表明，该方法在图像分类任务上的表现优于现有的离散旋转反射群等变神经网络方法。

Conclusion: 该研究成功构建了等变视觉Transformer，为在Transformer架构中实现旋转反射对称性提供了高效的解决方案。
...
### [70] [V-Zero: Answer-Label-Free On-Policy Distillation with Contrastive Evidence Gating for Fine-Grained Visual Reasoning](https://arxiv.org/abs/2606.25319)
*Haoxiang Sun,Zhihang Yi,Langxuan Deng,Yuhao Zhou,Peiqi Jia,Jian Zhao,Li Yuan,Jiancheng Lv,Tao Wang*

Main category: cs.CV

TL;DR: 本文提出了V-Zero，一个无需人工标注答案的视觉推理框架，通过对比证据门控机制引导策略内蒸馏，实现了高效的细粒度视觉推理。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉推理方法依赖昂贵的强化学习反馈或大规模有监督微调，而现有的策略内蒸馏（OPD）方法因缺乏轨迹级区分度而存在性能瓶颈。

Method: 提出V-Zero框架，在无需文本答案标签的情况下，通过将问题相关的局部区域裁剪与负面视觉视图配对，来评估模型自采样轨迹并门控Token级的策略内蒸馏。

Result: 在多个视觉推理基准上，V-Zero显著提升了细粒度视觉推理能力，且训练速度比传统监督微调快5倍以上，比强化学习基线快10倍以上。

Conclusion: V-Zero证明了利用对比视觉证据门控可以实现无需文本标签、高效且泛化性强的细粒度视觉推理策略蒸馏。
...
### [71] [LEVIRDet: A Million-Scale 159-Category Dataset and Foundation Model for Universal Remote Sensing Object Detection](https://arxiv.org/abs/2606.25312)
*Qinzhe Yang,Dongyu Wang,Haohan Niu,Jia Xu,Zhenwei Shi,Zhengxia Zou*

Main category: cs.CV

TL;DR: 本文推出了目前最大、最全面的遥感目标检测数据集 LEVIRDet-159，以及一个具有极强跨域泛化能力的通用遥感检测基座模型 LEVIRDetNet。


<details>
  <summary>Details</summary>
Motivation: 现有的遥感目标检测数据集和检测器较为碎片化，通常受限于单一传感器、固定空间分辨率和有限的类别，难以实现跨平台与跨类别系统的通用检测。

Method: 构建了包含159个类别、256万个边界框的大规模多层级分类数据集 LEVIRDet-159；并设计了 LEVIRDetNet 模型，融合了在线地面采样距离（GSD）预测、GSD条件查询调制以及层级感知检测头。

Result: 在无需目标域训练或微调的情况下，LEVIRDetNet 在9个外部基准测试上均达到最先进水平，相比最强的全监督对比方法平均提高了 5.02 mAP。

Conclusion: 本研究通过大规模数据集和通用基座模型，有效解决了遥感目标检测的泛化难题，推动了跨类别、跨分辨率和跨传感器的通用遥感检测技术发展。
...
### [72] [Efficient Remote Sensing Instance Segmentation with Linear-Time State Space Distilled Visual Foundation Models](https://arxiv.org/abs/2606.25324)
*Qinzhe Yang,Keyan Chen,Jia Xu,Zhenwei Shi,Zhengxia Zou*

Main category: cs.CV

TL;DR: 提出RS4D方法，利用知识蒸馏和状态空间模型（SSM）实现低计算开销、高精度的遥感图像实例分割。


<details>
  <summary>Details</summary>
Motivation: Transformer的二次方计算复杂度限制了其在密集视觉预测（如遥感实例分割）中的效率，亟需一种兼顾高效率与高精度的线性计算复杂度模型。

Method: 引入基于蒸馏状态空间模型（SSM）的RS4D架构；提出自适应噪声与掩码知识蒸馏方法来预训练轻量级SSM骨干网络，将自注意力机制知识压缩到线性状态空间中。

Result: 在SSDD、WHU和NWPU等数据集上，该方法相比ViT骨干网络减少了8倍参数量和9倍FLOPs，且精度与主流ViT和CNN方法相当甚至更优。

Conclusion: RS4D有效解决了遥感图像长序列建模的低效问题，证明了轻量级SSM在遥感密集预测任务中的高效性和实用性。
...
### [73] [Follow Your Track: Precise Skeleton Animation Controlled by 3D Trajectories](https://arxiv.org/abs/2606.25344)
*Yueting Liu,Yanqin Jiang,Nian Liu,Jingmen Zhou,Zhengjun Zha,Weiming Hu,Jin Gao*

Main category: cs.CV

TL;DR: 本文提出了ACT，一个基于轨迹引导的通用骨骼动画框架，旨在实现高效、高保真且具时间一致性的4D动画生成。


<details>
  <summary>Details</summary>
Motivation: 现有4D生成方法依赖高开销的密集团格或高斯表示，且控制信号（文本或视频）存在缺乏细节或外观纠缠的问题，导致动画质量低且一致性差。

Method: 使用紧凑的骨骼表示和单目视频的3D点轨迹作为运动引导，并设计了包含硬路由、全局路由和局部窗口交叉注意力的路由轨迹注入器（Routed Trajectory Injector）。

Result: 实验结果表明，ACT在生成动画的保真度和时间一致性方面均显著优于现有的方法。

Conclusion: ACT通过骨骼与轨迹引导的结合，有效解决了4D动画生成中的计算开销、微观对齐及运动失真问题。
...
### [74] [Beyond Visual Forensics: Auditing Multimodal Robustness for Synthetic Medical Image Detection](https://arxiv.org/abs/2606.25375)
...
TL;DR: 评估了视觉语言模型（VLM）在结合病历文本检测合成医学图像时的多模态鲁棒性，发现文本上下文会显著干扰模型对图像真实性的判断。


<details>
  <summary>Details</summary>
Motivation: 生成式AI带来的合成医学图像存在欺诈风险，且临床部署中VLM通常同时处理图像和病历文本，但模型可能会过度依赖文本上下文，导致对相同图像产生不一致的判断。

Method: 将合成医学图像检测转化为多模态鲁棒性审计，提出一个保持图像固定但改变元数据文本的配对基准，并对多种开源及商业VLM进行评估。

Result: 定量证实了仅改变元数据文本即可显著导致模型对同一医学图像真实性的预测结果发生改变。

Conclusion: 该研究提供了一个标准化的基准工具，用于评估和提升多模态模型在非单图像场景下的鲁棒性。
...
### [75] [State Space Models Meet Remote Sensing: A Survey](https://arxiv.org/abs/2606.25329)
*Qinzhe Yang,Chenyang Liu,Jia Xu,Zhenwei Shi,Zhengxia Zou*

Main category: cs.CV

TL;DR: 本文是对状态空间模型（SSM）在遥感领域应用的首篇全面综述，分析了其应用、架构设计及未来挑战。


<details>
  <summary>Details</summary>
Motivation: 状态空间模型（SSM）因其线性计算复杂度和强长程依赖建模能力在遥感领域备受关注，急需对其快速发展的研究成果进行系统性综述。

Method: 对引入遥感领域的SSM相关研究进行多维度文献综述，分析其在不同遥感任务中的应用和架构设计，并建立开源资源库以持续追踪最新进展。

Result: 系统梳理了SSM在遥感领域的应用现状，指出了当前面临的关键挑战与未来发展机遇，并提供了学术追踪资源。

Conclusion: 本综述为遥感领域研究人员提供了基础性参考指南，有助于促进SSM在该领域的进一步技术创新与应用落地。
...
### [76] [Invoice Haystack: Benchmarking Document Retrieval and Visual Question Answering Under Strong Visual Homogeneity](https://arxiv.org/abs/2606.25343)
*Heethanjan Kanagalingam,Thenukan Pathmanathan,Mokeeshan Vathanakumar,Basim Azam,Sarah Monazam Erfani*

Main category: cs.CV

TL;DR: 针对视觉同质化文档检索的难题，本文提出了全新的基准测试 Invoice Haystack 以及结合图文双模态检索与验证的 VL-RAG 框架。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理视觉结构高度相似的企业文档时会遭遇嵌入崩塌（embedding collapse）问题，而现有基准测试缺乏对此类高同质化场景的评测。

Method: 引入了包含1,500张同质发票图像的 Invoice Haystack 基准；并提出了 VL-RAG 框架，该框架联合文本与视觉嵌入进行双流检索，并结合 VLM 验证过滤器进行精确识别。

Result: VL-RAG 在 Invoice Haystack-500 上取得了 60.0% 的 Recall@1，比现有最先进方法提升了 13.5 个百分点，同时在 DocHaystack 和 InfoHaystack 基准上也均取得了性能提升。

Conclusion: 双流融合检索策略（VL-RAG）在处理同质和异质文档集的检索任务中均展现出一致的优越性。
...
### [77] [Hypergraph Normal World Models for Logical Visual Anomaly Detection](https://arxiv.org/abs/2606.25368)
*Weizhi Nie,Zibo Xu,Weijie Wang,Yuting Su*

Main category: cs.CV

TL;DR: 本文提出了一种超图正常世界模型（Hypergraph Normal World Model），仅通过正常图像训练，即可有效检测违反数量、共现或空间关系规则的逻辑异常。


<details>
  <summary>Details</summary>
Motivation: 传统的异常检测器仅关注局部结构缺陷，难以应对“局部正常但整体逻辑违规”（如数量或空间关系错误）的逻辑异常。本文旨在研究如何仅从正常图像中学习类别特异的正常关系规则。

Method: 提出超图正常世界模型，将冻结的DINOv2特征蒸馏为patch、关系和超图统计数据；在token组上构建空间超边；利用结合了局部、关系及超边证据的“信息商”对测试图像进行异常评分。

Result: 在MVTec LOCO数据集上，该模型将逻辑异常检测的AUROC从0.8434（DINOv2 patch-kNN基线）提升至0.9279，且在少样本设置下依然有效；实验证明该评分反映了深层的正常世界关系知识，而非简单的浅层映射。

Conclusion: 逻辑视觉异常检测的关键在于对正常的“关系”进行建模，而不仅仅是建模局部的正常patch。
...
### [78] [Transferable Attack against Face Swapping in an Extended Space](https://arxiv.org/abs/2606.25376)
*Mingzhi Lyu,Yi Huang,Jun Xie,Zihao Zhao,Hong Xu,Adams Wai-Kin Kong*

Main category: cs.CV

TL;DR: 本文提出了一种名为AIR的对抗性攻击方法，通过重照明和加性扰动，针对未知的换脸模型实现高可迁移性与视觉自然的隐私保护。


<details>
  <summary>Details</summary>
Motivation: 现有的换脸防御技术多为被动响应，且对未知的换脸模型效果不佳，无法有效应对换脸技术带来的隐私与安全威胁。

Method: 提出基于重照明函数的加性身份攻击（AIR），通过同时使用重照明和加性扰动来扩展攻击空间，并引入自适应平移不变操作和光照控制方案来提升图像质量。

Result: 在多款主流GAN和扩散换脸模型上的实验（包含1000个图像对）表明，AIR在攻击成功率和图像质量上均超越了现有攻击方法。

Conclusion: AIR无需替代模型即可对未知的换脸模型实现高迁移性的有效防御，为对抗恶意换脸提供了有力手段。
...
### [79] [C3-Bench: A Context-Aware Change Captioning Benchmark](https://arxiv.org/abs/2606.25445)
*Jae-Woo Kim,Hyeongbeom Kim,Ue-Hwan Kim*

Main category: cs.CV

TL;DR: 提出了C3-Bench，一个用于评估上下文感知图像变化描述的综合基准和LLM-as-Judge评估框架，揭示了现有模型在真实场景下的局限性。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏全面的评估框架，现有的图像变化描述系统在多样化真实世界变化上下文中的实际性能仍未得到充分探索。

Method: 构建了包含4个领域、4996对人工标注图像的C3-Bench基准；首次引入LLM-as-Judge评估框架（衡量准确性、特异性等维度及新颖的可逆性指标）；评估了32种不同类型的模型。

Result: 常规模型在脱离训练分布时会崩溃，而即使是SOTA多模态大模型也存在系统性的域和位置依赖错误，从而扭曲了对变化的可靠理解。

Conclusion: 该基准使隐性失效模式显性化和可测量化，为构建具有泛化性和可信赖性的图像变化描述系统奠定了基础。
...
### [80] [Anatomically-conditioned Latent Diffusion Model for Data-Efficient Few-Shot Cross-Domain 3D Glioma MRI Synthesis](https://arxiv.org/abs/2606.25390)
*Salman Shaik,Truong Thanh Hung Nguyen,Hung Cao*

Main category: cs.CV

TL;DR: 本文提出了ALDM模型，用于在少样本设置下进行高效的3D体积MRI图像合成。


<details>
  <summary>Details</summary>
Motivation: 解决跨中心领域偏移和缺乏大规模标注数据集导致弥漫性胶质瘤分类困难的问题。

Method: 采用两阶段方法：先用3D VAE从源域学习解剖先验，再通过ControlNet和肿瘤掩膜引导的条件潜扩散模型为稀缺目标域生成结构一致的图像。

Result: 在仅16张目标图像的少样本设定下，取得85.40的FID和0.987的分类AUC，性能优于GAN等基线方法，且保持了清晰的病理边界。

Conclusion: ALDM能有效捕获关键诊断特征，是低资源临床环境下进行医学图像数据增强的可靠工具。
...
### [81] [Teach-to-Reason: Competition-Guided Reasoning with a Self-Improving Teacher](https://arxiv.org/abs/2606.25407)
*Xiao Han,Hao Liu,Zhimin Bao,Jile Jiao,Yue Wang,Hui Guo,Xiaofeng Mou,Yi Xu*

Main category: cs.CV

TL;DR: 提出T2R框架，通过引入自我竞争的教师模型和比较监督机制，显著提升了胸部X光视觉问答模型的医学推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法在胸部X光视觉问答中依赖粗粒度的回答级奖励，难以有效优化推理链（CoT），且在优势崩塌时会失效。

Method: 提出T2R框架，通过自我提升的Teacher与竞争引导的Reasoner进行比较监督优化，并引入案例级奖励设计以解决奖励退化问题。

Result: 在多个CXR开放式VQA基准测试上的实验结果表明，T2R持续且显著地优于现有的强基线模型。

Conclusion: 在推理优化中引入以原则性和受控方式集成的比较监督机制，能为模型提供更有效的训练信号。
...
### [82] [EchoStyle: Unlocking High-Fidelity Video Stylization with Reverse Data Synthesis](https://arxiv.org/abs/2606.25465)
*Huaqiu Li,Jiahao Wang,Sijia Cai,Hualian Sheng,Bing Deng,Jieping Ye,Wenhan Luo*

Main category: cs.CV

TL;DR: 提出EchoStyle，一个可扩展的文本驱动视频风格化框架，支持任意长度视频，并构建了包含2万对视频的V-Style20k数据集。


<details>
  <summary>Details</summary>
Motivation: 现有视频风格化方法存在内容泄露、数据匮乏以及对长视频适应性差的问题，导致风格漂移和运动失真。

Method: 构建视频到视频架构以重融内容与文本风格；开发自动反向合成流水线建立V-Style20k数据集；设计初始-跟随模式及滑动窗口推理策略处理长视频。

Result: EchoStyle在多种艺术风格中表现出色，其效果可媲美主流的闭源解决方案。

Conclusion: EchoStyle通过新颖的框架、大规模数据集和长视频推理策略，成功克服了视频风格化中的关键技术瓶颈。
...
### [83] [Expresso-AI: Explainable Video-Based Deep Learning Models for Depression Diagnosis](https://arxiv.org/abs/2606.25606)
*Felipe Moreno,Sharifa Alghowinem,Hae Won Park,Cynthia Breazeal*

Main category: cs.CV

TL;DR: 提出了一种基于面部视频的可解释性深度学习框架，用于自动评估抑郁症严重程度，并在提高预测准确性的同时提供直观的决策解释。


<details>
  <summary>Details</summary>
Motivation: 现有的自动抑郁症诊断方法缺乏情感特异性和可解释性，尤其是在利用深度模型分析面部视频的时间维度活动时，限制了其临床实用性。

Method: 利用在动作识别数据集上预训练的深度卷积神经网络（DCNN），在AVEC抑郁症面部视频数据集上进行微调，并通过分析显著性图、面部区域和时间表情语义来产生视觉和定量解释。

Result: 该方法超越了以往的单人脸抑郁症诊断基准，不仅显著提升了预测性能，还成功为模型的决策过程提供了可理解的视觉与定量化依据。

Conclusion: 本研究成功构建了一个既能提升抑郁症诊断预测能力，又能解释深度模型决策机制的框架，有助于推动人工智能在精神健康领域的实际应用。
...
### [84] [Gastroendoscopy View Synthesis: A New Real Dataset and Evaluation](https://arxiv.org/abs/2606.25427)
*Masaki Minai,Yusuke Monno,Masatoshi Okutomi,Sho Suzuki*

Main category: cs.CV

TL;DR: 本文推出了GastroNVS，这是首个用于胃镜新视角合成（NVS）的真实胃镜图像数据集。


<details>
  <summary>Details</summary>
Motivation: 现有数据集不足以评估新视角合成（NVS）技术在胃镜检查（如扩大视野、数字孪生和培训）中的应用。

Method: 构建并发布了包含胃镜图像、相机位姿和点云的GastroNVS数据集，并评估了多种3D高斯泼溅（3DGS）方法。

Result: 成功构建了首个真实的胃镜NVS评估基准，并通过评估现有3DGS方法展示了其适用性与未来挑战。

Conclusion: GastroNVS数据集填补了胃镜NVS评估数据集的空白，为未来胃镜三维重建研究奠定了基础。
...
### [85] [HG-Bench: A Benchmark for Multi-Page Handwritten Answer-Region Grounding in Automated Homework Assessment](https://arxiv.org/abs/2606.25491)
*Chuangxin Zhao,Boyan Shi,Yanling Wang,Yijian LU,Canran Xiao,Jiali Chen,Jun Xia,Yan Wang,Ji Qi,Juanzi Li*

Main category: cs.CV

TL;DR: 本文提出了一个名为HG-Bench的新基准，旨在评估模型在手写作业中定位完整答案区域及其具体解题步骤的能力。


<details>
  <summary>Details</summary>
Motivation: 自动作业评估不仅需要识别答案，还需要在多页手写文本中准确定位答案和中间推理步骤，但目前缺乏对此类双层区域定位的评估机制。

Method: 引入了包含500个K-12手写作业样本的HG-Bench基准，设计了页面感知评估协议（测量FA和FSm指标），并使用约1万个领域内样本微调了GLM-4.6V 9B模型作为参考。

Result: 零样本模型在FA和FSm上的最高得分仅为55.22%和48.22%，而微调后的GLM-4.6V 9B模型则能达到74.97%和72.26%。

Conclusion: 研究表明步骤级手写定位是一个亟待解决的能力缺陷，本文为自动作业评估领域提供了可复现的基准和评估方法。
...
### [86] [PRISM: Feed-Forward Single-Image 3D Reconstruction via Geometric Warp-Residual Modeling](https://arxiv.org/abs/2606.25430)
*Zhijie Zheng,Xinhao Xiang,Jiawei Zhang*

Main category: cs.CV

TL;DR: 提出PRISM，一个无需扩散采样的高效前馈3D重建框架，通过结合几何前向变形和残差修正，实现了快速且高质量的单图3D场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有的基于视频扩散模型的单图3D重建方法依赖迭代扩散采样，导致计算成本高、推理速度慢，限制了实际应用部署。

Method: 提出PRISM框架，将多视图隐空间预测分解为无参数的几何先验（前向变形）和可学习的残差修正；并设计了结合隐空间监督蒸馏和感知微调的两阶段训练策略。

Result: 在三个基准数据集上，PRISM达到了与基于扩散模型的方法相当的重建质量，同时将每个场景的推理时间大幅缩短至仅36秒。

Conclusion: PRISM证明了通过将几何先验与残差学习相结合，可以在无需扩散采样的情况下，实现高效且极具竞争力的单图3D重建。
...
### [87] [LinStereo: Linear-Complexity Global Attention for Multi-Scale Iterative Stereo Matching](https://arxiv.org/abs/2606.25437)
*Yiran Wang,Oliver Turner,Viorela Ila*

Main category: cs.CV

TL;DR: 提出 LinStereo 框架，通过引入位置感知线性注意力（PALA）以及深度和语义先验，显著提升了立体匹配在水下等退化环境中的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉基石模型的立体匹配方法未充分利用多尺度特征、几何先验及全局上下文，导致其在光照退化的水下场景中泛化能力不足。

Method: 基于 Depth Anything V3 构建 LinStereo，核心采用位置感知线性注意力（PALA）进行全局聚合，并结合层次化语义代价体积（HSCV）和深度先验初始化（DPI）。

Result: 在标准基准和跨域水下场景中达到 SOTA 精度，在 TartanAir-UW 和 SQUID 数据集上的 AbsRel 指标分别降低了 28% 和 26%。

Conclusion: 通过结合全局注意力机制与多尺度及几何先验，LinStereo 成功克服了传统立体匹配在极端光照退化环境下的局限性，展示了极强的泛化能力。
...
### [88] [TACO: Towards Task-Consistent Open-Vocabulary Adaptation in Video Recognition](https://arxiv.org/abs/2606.25478)
*Minghao Zhu,Xiao Lin,Mengxian Hu,Xun Zhou,Liuyi Wang,Xiaoyan Qi,Chengju Liu,Qijun Chen*

Main category: cs.CV

TL;DR: 提出了 TACO 框架，通过相对结构蒸馏和解耦优化空间，有效解决了 CLIP 在视频自适应微调中的表征偏移问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在微调 CLIP 进行视频识别时，忽略了训练分布外表征偏移的问题，导致泛化与特化之间的平衡不佳。

Method: 引入相对结构蒸馏以约束表征空间的相对几何结构，并设计轻量化专用投影以解耦表征空间与优化空间。

Result: 在跨数据集和从已知到新类别（base-to-novel）的多种视频识别基准测试中取得了最先进的性能。

Conclusion: TACO 证明了在微调过程中保留分布外相关对齐的重要性，为开放词汇视频识别提供了更优的泛化能力。
...
### [89] [Steering Vision-Language Models with Joint Sparse Autoencoders](https://arxiv.org/abs/2606.25657)
*Huizhen Shu,Xuying Li,Hongxu Lin,Wenjie Sun,Hui Li*

Main category: cs.CV

TL;DR: 本文提出了联合稀疏自编码器（JSAE），通过显式对齐约束，在多模态大模型中实现了更具可控性和可解释性的跨模态表征与干预分析。


<details>
  <summary>Details</summary>
Motivation: 现有的稀疏自编码器（SAE）在应用于视觉语言模型（VLM）时，难以产生可用于跨模态可控转向的特征方向。

Method: 提出JSAE方法，利用显式对齐约束，将图像和文本的激活值联合分解为共享的、可解释的跨模态特征，并在三种主流VLM上进行了双向干预实验。

Result: JSAE成功提取了跨模态概念特征，并揭示了干预效果的层依赖性非对称规律：加性转向在中后期层达到峰值，而抑制效果在各层间表现相对稳定。

Conclusion: 显式对齐的稀疏表示相比无约束方法，能够在特定的模型层范围内为多模态特征提供更具可控性的干预与分析手段。
...
### [90] [Causal-rCM: A Unified Teacher-Forcing and Self-Forcing Open Recipe for Autoregressive Diffusion Distillation in Streaming Video Generation and Interactive World Models](https://arxiv.org/abs/2606.25473)
*Kaiwen Zheng,Guande He,Min Zhao,Jintao Zhang,Huayu Chen,Jianfei Chen,Chen-Hsuan Lin,Ming-Yu Liu,Jun Zhu,Qianli Ma*

Main category: cs.CV

TL;DR: 本文提出了Causal-rCM框架，将rCM扩散蒸馏技术扩展到自回归视频生成，实现了仅需1至2步即可生成高质量视频的实时流式生成与交互式世界模型。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型在实时视频流生成和交互式世界模型中至关重要，但面临推理速度慢的问题，需要一种能够结合前向与反向散度优势的自回归蒸馏方案来提升生成效率与质量。

Method: 引入Causal-rCM框架，将基于Teacher-forcing的一致性模型（TF-CM，用于前向散度初始化）与基于Self-forcing的分布匹配蒸馏（SF-DMD，用于反向散度微调）相结合；开发了自定义掩码的FlashAttention-2 JVP内核以支持连续时间CM的训练。

Result: 连续时间CM的收敛速度比离散时间CM快10倍；蒸馏后的2步Causal Wan2.1-1.3B模型在VBench-T2V测试中取得了84.63的高分；并成功将该技术应用于Cosmos 3以构建交互式世界模型。

Conclusion: Causal-rCM为自回归视频扩散提供了一个高效、可扩展的蒸馏算法和基础设施方案，显著推动了实时流式视频生成和物理AI交互式世界模型的发展。
...
### [91] [Point Cloud Diffusion with Global and Local Reconstruction for Instance-Level 3D Anomaly Detection](https://arxiv.org/abs/2606.25740)
*Linchun Wu,Qin Zou,Jiwen Lu,Qingquan Li*

Main category: cs.CV

TL;DR: 提出PCDiff，一个用于实例级3D异常生成与检测的点云扩散框架，显著提升了检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有三维异常检测方法难以重构微小缺陷（如划痕），且背景区域重建易产生位置偏差导致误报。

Method: 引入实例级多模态注意力机制以生成高质量异常，并采用局部-全局联合重建算法以保持背景结构和恢复前景缺陷。

Result: PCDiff在异常生成保真度、重建质量和异常检测准确率上均显著超越了现有最先进的方法。

Conclusion: 该研究证明了结合多模态条件扩散模型与局部-全局重建能有效解决3D工业异常检测中的微小缺陷检测和虚警问题。
...
### [92] [Enhancing Brain MRI Anomaly Detection and Reasoning with ROI Rethink and Synthetic Data](https://arxiv.org/abs/2606.25894)
*Shangkun Li,Jie Xu,Yi Guo,Zeju Li,Yuanyuan Wang*

Main category: cs.CV

TL;DR: 提出BrReMark框架，通过显式区域标记和“假设-验证”机制，提升脑部MRI诊断的空间定位能力并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 传统的医疗视觉语言模型在诊断时缺乏空间定位，无法指出支持其结论的具体图像区域，导致临床实用性受限且易产生幻觉。

Method: 引入显式边界框标记进行病灶定位与验证，结合监督微调（SFT）、融合定位与推理的多重奖励强化学习（RL），以及病理合成数据增强技术。

Result: 在内部基准上，mAP50从0.74%显著提升至37.54%，诊断准确率达到45.26%；在OOD基准上，假阳性率降低了45.7%，显著减少了幻觉。

Conclusion: 显式的“假设-验证”定位机制是实现可信、开放式脑部MRI诊断的有效且实用的途径。
...
### [93] [Edges Before Embeddings: A Confidence-Aware Blur Gate for Vision-Language Pipelines](https://arxiv.org/abs/2606.25838)
*Duy Tran Thanh*

Main category: cs.CV

TL;DR: 本文提出了 MagikaDocumentFromPixel，一种超轻量、适合 CPU 运行的图像质量过滤门控，旨在快速识别并拦截模糊图像。


<details>
  <summary>Details</summary>
Motivation: 生产环境中的视觉流水线常因输入图像模糊而退化，导致下游的 OCR、图像检索和 VLM 任务白白浪费计算资源。

Method: 使用 MobileNetV3-Large 作为基础架构，引入了边缘先验模块（EPM，拉普拉斯算子辅助通道）和基于选择性预测的置信度路由机制，并配合多尺度测试时增强进行优化。

Result: 在单 CPU 核心上推理仅需 7 毫秒，模型大小为 17 MB，达到了 0.9803 的 F1 值和 0.9989 的 AUC，比固定尺度基线提升了 1.31 个百分点。

Conclusion: 该方法证明了结合经典特征先验与轻量级深度学习可以高效构建计算流水线的质量把关器，为低成本降低系统冗余计算提供了可行方案。
...
### [94] [Cross-View Variance Correlation in Path-Traced Stereo:A Hidden Shortcut in Synthetic Training Data](https://arxiv.org/abs/2606.25483)
*Po-Ting Lin*

Main category: cs.CV

TL;DR: 发现蒙特卡洛路径追踪渲染的合成双目图像中，左右视图的方差场在视差对齐后高度相关，这可能作为模型学习的非物理“捷径”线索。


<details>
  <summary>Details</summary>
Motivation: 探究在使用合成双目数据训练视差估计模型时，是否存在由于蒙特卡洛渲染机制引入的、此前未被察觉的sim-to-real偏差信号。

Method: 通过Mitsuba 3渲染多场景双目图像并计算对齐后的方差场相关性，同时设计残差打乱干预实验以评估该特征对匹配算法的实际影响。

Result: 对齐方差场的皮尔逊相关系数高达0.754（朗伯体区域达0.78）且与采样率无关；打破该相关性会导致非玻璃区域的GT代价裕度下降33%，玻璃区域准确率下降4.3倍。

Conclusion: 该方差场相关性是蒙特卡洛渲染数据特有的信号，可作为视差匹配的捷径线索，对训练网络的sim-to-real泛化性带来潜在影响。
...
### [95] [PatchINR: Patch-Based Implicit Neural Representations for Efficient and Scalable Inference](https://arxiv.org/abs/2606.25534)
*Jiachen Ren,Wenyong Zhou,Taiqiang Wu,Yuxin Cheng,Xincheng Feng,Zhengwu Liu,Ngai Wong*

Main category: cs.CV

TL;DR: 提出一种基于分块的隐式神经表示（INR）方法及FPGA加速架构，旨在显著降低高分辨率应用中的推理延迟。


<details>
  <summary>Details</summary>
Motivation: 传统的逐像素INR推理导致推理次数随分辨率呈二次方增长，在高分辨率应用中计算开销巨大。

Method: 引入以无重叠图像块（patch）为基本单元的预测方法，单次前向传播预测整个像素块；并在FPGA上设计了支持双精度和可配置流水线的硬件加速架构。

Result: 在保持与逐像素INR相当的重建质量（2x2分块下PSNR达34.97 dB）的同时，推理延迟降低了75%，参数量仅增加0.6%。

Conclusion: 基于分块的INR及配套FPGA加速架构能有效解决高分辨率实时推理的瓶颈，实现低延迟与高质量重建的平衡。
...
### [96] [C2RM-Seg: Causal Counterfactual Reasoning with Structural-Semantic Priors for Weakly Supervised Histopathological Tissue Segmentation](https://arxiv.org/abs/2606.25508)
*Hualong Zhang,Siyang Feng,Zihan Huan,Yi Qian,Zhenbing Liu,Rushi Lan,Xipeng Pan*

Main category: cs.CV

TL;DR: 本文提出了一种名为 C$^2$RM-Seg 的弱监督病理组织分割框架，通过因果伪标签精细化与结构感知语义增强来提高分割精度。


<details>
  <summary>Details</summary>
Motivation: 现有的弱监督分割方法依赖于CAM生成的伪标签，这些标签易受染色驱动的外观特征干扰，导致定位不准且缺乏结构一致性。

Method: 引入因果反事实推理模块（C$^2$RM）以净化CAM伪标签；设计双路径结构-语义架构结合ResNeSt和DINOV3；使用不确定性门控边际（UGM）损失缓解伪标签噪声。

Result: 在两个公开的组织病理学数据集上进行的实验表明，该方法达到了最先进（SOTA）的性能。

Conclusion: 结合因果推理与结构-语义融合能有效解决弱监督病理分割中的噪声和边界模糊问题，提升了辅助诊断的准确性。
...
### [97] [Pulmonary Embolism Risk Stratification from CTPA and Medical Records: Vascular Graphs Are Not All You Need](https://arxiv.org/abs/2606.25956)
...
TL;DR: 本文评估了仅使用病历和CTPA图像特征进行肺栓塞（PE）风险分层的可行性，发现血管图结构和血管生物标志物对提升预测效果帮助有限。


<details>
  <summary>Details</summary>
Motivation: 肺栓塞风险分层通常依赖临床中经常缺失的血液检测。本研究旨在探索是否能仅通过医疗记录和CTPA图像提取的标志物实现准确的风险分层。

Method: 在包含353例完整数据的私有数据集上，对比了将血管生物标志物引入表格模型，以及在血管树图表示上应用图神经网络（GNN）的预测效果。

Result: 医疗记录和心脏生物标志物是最重要的预测因子，而血管生物标志物或基于血管图的GNN模型并未能超越仅使用全局特征的强表格基线模型。

Conclusion: 研究表明肺血管图结构可能不包含用于肺栓塞风险分层的判别性信息，医疗记录和心脏标志物仍是主要的预测依据。
...
### [98] [TriViewBench: Controlled Complexity Scaling for Multi-View Structural Reasoning in MLLMs](https://arxiv.org/abs/2606.26029)
*Yu-Yang Chen,Lan-Zhe Guo*

Main category: cs.CV

TL;DR: 本文引入了 TriViewBench 基准，用于评估多模态大语言模型（MLLMs）在受控三视角 3D 场景下的三维空间视觉推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准难以评估多模态大模型在受控的结构复杂度（如物体数量和遮挡）下的表现与可扩展性。

Method: 构建了包含 1,923 个合成 3D 场景和超 1.4 万个问答对的 TriViewBench 数据集，并在统一协议下评估了 18 个主流大模型，进行了错误分析和思维链（CoT）测试。

Result: 所有模型表现均随复杂度增加而显著退化；物体计数错误源于单视角的遮挡漏算和多视角的跨视角身份混淆；思维链（CoT）提示词未能带来显著的性能提升。

Conclusion: 当前多模态大模型的局限性在于跨视角的空间表征能力不足而非推理策略问题，TriViewBench 可作为诊断其结构化推理缺陷的有效框架。
...
### [99] [A welding penetration prediction model for laser welding process based on self-supervised learning using physics-informed neural networks](https://arxiv.org/abs/2606.26059)
*Sen Li,Xiaoying Liu,Xiaojian Xu,Chendong Shao,Yaqi Wang,Ling Lan,Xinhua Tang,Haichao Cui*

Main category: cs.CV

TL;DR: 本文提出了 SimPhysNet 算法，通过将物理先验引入自监督与小样本学习框架，实现了在极少标注样本下的高精度激光焊接熔透状态预测。


<details>
  <summary>Details</summary>
Motivation: 准确预测熔透状态对确保焊接质量至关重要，但传统监督学习算法过度依赖大量高质量标注数据，限制了其在实际工业场景中的应用。

Method: 开发了 SimPhysNet 算法，其核心包括：将物理信息神经网络（PINN）嵌入对比学习框架以从无标注数据中提取熔池与匙孔的物理特征，利用三种图像增强任务提升泛化性，并采用基于原型网络的小样本学习策略进行分类。

Result: 仅使用 200 张标注图像（约占总数据集的 5%），SimPhysNet 就达到了 96.06% 的分类准确率，性能与使用全部标注数据的传统监督学习算法相当。

Conclusion: SimPhysNet 提供了一种高效、高精度的激光焊接熔透预测新方法，有效克服了数据标注瓶颈，为激光焊接的智能自动化奠定了基础。
...
### [100] [Spatio-Temporal Mixture-of-Modality-Experts Diffusion for Quantitative DCE-MRI Synthesis from Incomplete MR Sequences](https://arxiv.org/abs/2606.25535)
*Junhyeok Lee,Kyu Sung Choi*

Main category: cs.CV

TL;DR: 本研究提出了ST-MoME框架，利用条件扩散模型在多模态MRI输入缺失的情况下，高保真地合成3D DCE-MRI定量参数图。


<details>
  <summary>Details</summary>
Motivation: DCE-MRI定量图对肿瘤评估至关重要，但受造影剂风险和协议可变性限制常难以获取，且现有合成方法无法应对临床中常见的输入模态缺失问题。

Method: 设计了时空模态混合专家系统（ST-MoME），通过时空门控网络融合多模态特征以引导扩散模型，并在图像空间中结合3D分块与Swin结构进行去噪训练。

Result: 在386名脑肿瘤患者的数据集及16种模态缺失场景下，该方法显著降低了DCE参数的平均归一化均方误差（NMSE），并在肿瘤区域实现了最低的重建误差。

Conclusion: ST-MoME在输入模态不完整的情况下仍能有效且高保真地合成DCE-MRI参数图，其门控融合机制符合临床直觉，具有重要的临床应用价值。
...
### [101] [SAC$^2$-Net: Semantic Anchoring and Complementary-Consensus Fusion for Multimodal Micro-Expression Recognition](https://arxiv.org/abs/2606.25542)
*Xuepeng Zheng,Tong Chen*

Main category: cs.CV

TL;DR: 提出一种名为SAC^2-Net的双模态微表情识别网络，通过语义锚定和互补共识机制融合光流与运动放大特征。


<details>
  <summary>Details</summary>
Motivation: 光流与运动放大在捕捉微表情动态时具有互补性，但两者融合面临跨模态异构性和空间可靠性差异的挑战。

Method: 提出语义锚定软对齐（SASA）将动作单位（AU）转化为文本提示作为语义锚点进行模态对齐，并使用互补共识融合（CCF）修复局部不可靠特征并提炼空间共识。

Result: 在五个微表情基准数据集上的实验表明，SAC^2-Net在粗粒度、细粒度、大规模和跨数据集等多种评估设置下均达到了SOTA或极具竞争力的性能。

Conclusion: 通过引入语义锚定和互补共识融合，该方法有效解决了多模态微表情识别中的异构对齐和特征可靠性问题。
...
### [102] [A cross-process welding penetration status prediction algorithm based on unsupervised domain adaptation in laser and TIG welding](https://arxiv.org/abs/2606.26078)
*Sen Li,Haichao Cui,Chendong Shao,Yaqi Wang,Xinhua Tang*

Main category: cs.CV

TL;DR: 提出了一种结合渐进式源域扩张（GSDE）的无监督域适应（UDA）框架，用于解决不同焊接工艺间熔透状态分类的领域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 监督深度学习模型在不同焊接工艺（如TIG焊与激光焊）之间迁移时，由于物理机制差异导致的领域偏移会使分类性能显著下降。

Method: 引入无监督域适应（UDA）框架，并集成渐进式源域扩张（GSDE）策略以对齐特征分布。

Result: 在跨工艺迁移任务中，TIG到激光和激光到TIG的分类准确率分别达到80.48%和81.13%，相比监督基线提升了超过43%。

Conclusion: 该方法有效降低了新焊接工艺的样本标注成本，增强了智能焊接监控系统在不同工艺间的通用性与鲁棒性。
...
### [103] [TensorLDM: A Component-Wise Latent Diffusion Model for Volumetric DTI Reconstruction from Sparse DWIs](https://arxiv.org/abs/2606.25545)
*Junhyeok Lee,Kyu Sung Choi*

Main category: cs.CV

TL;DR: 本文提出了 TensorLDM，一种基于潜在扩散模型的方法，旨在从稀疏 DWI 数据中重建出高解剖一致性和物理合理性的扩散张量。


<details>
  <summary>Details</summary>
Motivation: 从稀疏 DWI 重建扩散张量对加速临床扩散张量成像（DTI）至关重要，但现有深度学习方法常产生解剖不一致或物理上不合理（如非正定）的张量。

Method: 设计了 TensorLDM 模型，采用解剖条件自编码器分别处理对角与非对角分量，利用共享的跨分量注意力（CCA）建模分量间依赖，并使用混合专家（MoE）DWI条件器实现自适应调节。

Result: 在 HCP 数据集（4卷稀疏采集）上，该方法取得了极高的物理有效性（SPD违反率仅 1.54%），并实现了最准确的下游纤维束追踪和优秀的体素级重建精度。

Conclusion: TensorLDM 通过结合分量建模与解剖约束，有效克服了稀疏 DTI 重建中的物理与结构合理性难题，具有重要的临床应用价值。
...
### [104] [Disease-Centric Vision-Language Pretraining with Hybrid Visual Encoding for 3D Computed Tomography](https://arxiv.org/abs/2606.25546)
*Bowen Shi,Weiwei Cao,Ruifeng Yuan,Wanxing Chang,Wenrui Dai,Hongkai Xiong,Ling Zhang,Jianpeng Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种专为3D CT设计的视觉-语言预训练（VLP）框架，显著提升了多标签诊断和报告生成的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言预训练方法在处理3D CT影像时，面临视觉骨干网络效率低下以及语义对齐粒度过粗的问题。

Method: 提出了一个包含三个核心组件的VLP框架：1) 结合3D CNN与ViT的混合编码器以捕获局部与全局特征；2) 疾病级对比学习机制以动态提取疾病特定语义；3) 诊断感知提示策略以桥接预训练与推理的差距。

Result: 在CT-RATE和Rad-ChestCT数据集上分别实现了84.4%（+5.1%）和75.4%（+5.4%）的AUC，并在60种疾病的基准测试中提升了9.8% AUC，同时在报告生成任务中表现出强迁移性。

Conclusion: 该方法有效克服了3D CT预训练中的架构和语义对齐瓶颈，证实了其在临床应用中的通用性和实用价值。
...
### [105] [Concept Removal for Frontier Image Generative Models](https://arxiv.org/abs/2606.25548)
*Aditya Kumar,Pierre Joly,Adam Dziedzic,Franziska Boenisch*

Main category: cs.CV

TL;DR: 提出一种基于转译器（transcoder）就地替换瓶颈层的图像生成模型概念擦除方法，实现了高效且稳健的不良视觉概念过滤。


<details>
  <summary>Details</summary>
Motivation: 图像生成模型训练数据中包含不良视觉概念，现有方法难以在不降低生成质量的前提下高效且持久地移除这些概念。

Method: 将模型内部的瓶颈层替换为一个转译器（transcoder），该转译器在复制原层功能的同时将激活特征结构化，从而可以有选择性地禁用特定概念信号，并保持对白盒攻击的防御力。

Result: 在SD3.5、Flux等模型上取得了最先进的概念擦除效果，保持了生成质量，具备对抗提示词的鲁棒性，并支持顺序擦除多个概念。

Conclusion: 该方法是前沿图像生成模型中实现概念擦除的一种兼具实用性与鲁棒性的有效方案。
...
### [106] [Efficient Cross-Scale Invertible Hiding Network with Spatial-Frequency Collaboration and Non-Invertible Mechanism](https://arxiv.org/abs/2606.25547)
*Junxue Yang,Xin Liao*

Main category: cs.CV

TL;DR: 提出了一种名为CrosInv的跨尺度可逆图像隐藏网络，通过空频协作和非可逆机制提升隐藏图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有的可逆神经网络（INN）图像隐藏方法受限于单尺度、单域特征提取以及有限的非线性表达能力，导致图像重建质量不佳。

Method: 引入跨尺度可逆模块（融合像素重组与哈尔小波变换以协同空间和频域信息），并结合非可逆交叉密集模块以增强网络的非线性表达能力。

Result: 对比实验表明，所提出的CrosInv在图像隐藏任务中具有显著的有效性和优越性。

Conclusion: CrosInv通过克服传统可逆网络的架构限制，有效提升了基于可逆神经网络的图像隐藏性能。
...
### [107] [FeVOS: Foresight Expression Video Object Segmentation](https://arxiv.org/abs/2606.25585)
*Kehan Lan,Kaining Ying,Henghui Ding*

Main category: cs.CV

TL;DR: 本文提出了预测性表达视频目标分割（FeVOS）新任务，构建了相关数据集，并开发了结合强化学习的 FeVOS-R1 模型以实现对未来事件的目标分割。


<details>
  <summary>Details</summary>
Motivation: 现有的指代视频目标分割（RVOS）局限于描述已观测帧，缺乏对未来视频片段进行前瞻性时空推理的能力。

Method: 定义了 FeVOS 任务，构建了包含 968 个视频及思维链（CoT）标注的数据集，并提出了基于多模态大模型的双阶段（SFT + RL）训练模型 FeVOS-R1。

Result: FeVOS-R1 在 FeVOS 数据集上达到了最先进的性能，并在现有的传统 RVOS 基准测试中展现出强大的泛化能力。

Conclusion: 该工作为视频感知中的预测性推理研究提供了新的任务、数据集和基准模型。
...
### [108] [H-Adapter: Pose-Robust Hairstyle Transfer via Attention-Derived, Source-Aligned Hair Masks](https://arxiv.org/abs/2606.25578)
*Seulgi Jeong,Yunseong Cho,Sanghun Park*

Main category: cs.CV

TL;DR: 本文提出 H-Adapter 方法，通过区域特定损失和空间解耦交叉注意力，解决了大姿态差异下的发型迁移难题。


<details>
  <summary>Details</summary>
Motivation: 现有的发型迁移技术（如虚拟试戴）在面对源图像和参考图像存在较大头部姿态差异时，生成效果较差且不够鲁棒。

Method: 利用区域特定损失解耦头发与非头发目标，引导空间解耦的交叉注意力，并生成源图对齐的编辑掩码来指导扩散模型的图像局部重绘（inpainting）。

Result: 在姿态差异基准测试中取得了领先的 FID 和 CLIP 指标，有效保留了非头发区域，并支持文本生成、发色控制及身份保留等多种应用扩展。

Conclusion: H-Adapter 显著提高了姿态无关发型迁移的鲁棒性与发型保真度，是一种极具实用价值的图像编辑方法。
...
### [109] [ScaleHP: Estimating Hand Pose in Metric Space](https://arxiv.org/abs/2606.25619)
*Ruitao Jing,Xingyu Chen,Hongyang Li,Qing Jiang,Yukai Shi,Lei Zhang*

Main category: cs.CV

TL;DR: 本文提出ScaleHP，一种利用手部骨骼内在比例关系（人类测量学先验）在度量空间中直接恢复手部姿态的端到端单阶段估计框架。


<details>
  <summary>Details</summary>
Motivation: 大多数现有的手势估计方法是在根节点相对坐标系下预测姿态，无法估计出绝对度量尺度。

Method: 采用带有新型尺度Token的Transformer解码器融合多尺度形态与外观特征，并通过透视约束的最小二乘法求解相机坐标系下的度量坐标。

Result: 在FreiHand数据集上达到35.8 CS-MPJPE，在DexYCB和HO3Dv3上分别达到4.6和5.9 PA-MPJPE，均取得当前最先进（SOTA）的性能。

Conclusion: 手部内部的生物学约束能够显著减少相对几何与绝对度量误差，为真实世界的手部追踪提供了一种鲁棒的解决方案。
...
### [110] [VPA-Guard: Defending and Benchmarking Image-to-Video Generation Against Visual Prompt Attacks](https://arxiv.org/abs/2606.25592)
...
TL;DR: 本文揭示了图生视频（I2V）中视觉控制提示（如箭头、草图）被利用进行越狱攻击的风险，并提出了首个安全评估基准 VVA-Bench 及防御框架 VPA-Guard。


<details>
  <summary>Details</summary>
Motivation: 现有的视频生成安全基准主要关注文本和内容图像的越狱，忽视了静态视觉线索作为隐式时间指令所带来的视觉提示攻击风险。

Method: 构建了首个分类评估视觉提示攻击的基准 VVA-Bench；并开发了 VPA-Guard 防御框架，通过检索增强和少样本推理来识别并拦截潜在的恶意意图。

Result: 评估发现主流模型极易受此类攻击（Wan 2.7 和 Veo 3.1 的攻击成功率分别达 100% 和 74.8%）。采用 VPA-Guard 后，平均攻击成功率降低了 44.2%，有害性降低了 73.4%，且不影响正常编辑。

Conclusion: 本研究提供了系统性的安全基准与有效的防御方案，为推动安全、负责任的多模态视频生成奠定了基础。
...
### [111] [Auto-Labelling-Based Domain Transfer for 3D Object Detection on a Bicycle-Mounted LiDAR Platform](https://arxiv.org/abs/2606.25652)
*Mario Finkbeiner,Max A. Buettner,Kanak Mazumder,Fabian B. Flohr*

Main category: cs.CV

TL;DR: 提出一个基于骑行者视角的3D目标检测基准，并证明利用自动标注微调可有效解决从车辆到骑行者视角的领域偏差。


<details>
  <summary>Details</summary>
Motivation: 虽然从骑行者视角研究交通逐渐增多，但该视角下的3D LiDAR标注数据极度匮乏，且现有的车辆训练探测器在此场景下的泛化能力尚未得到评估。

Method: 引入了包含1027个标注LiDAR关键帧的FUSE-Bike数据集，并使用无需人工标注的自动标注管线生成标签，对四种预训练的目标探测器进行微调和评估。

Result: 微调使平均精度（mAP）提升了高达23.4个百分点，显著改善了行人和骑行者的检测效果，且微调后的探测器性能甚至超越了自动标签本身的质量。

Conclusion: 本研究为以弱势路人（VRU）为中心的3D检测提供了可复现的基准，证明了自动标注是取代手动标注、实现平台适配的可行方案。
...
### [112] [SSMNBench: Diagnosing Image-based Cross-View Human-Object Understanding via Single-View Sufficiency and Multi-View Necessity](https://arxiv.org/abs/2606.25634)
*Tianchen Guo,Chen Liu,Ling Chen,Xin Yu*

Main category: cs.CV

TL;DR: 本文提出了SSMNBench，一个用于评估多视角人类与人机交互理解能力的诊断基准，揭示了现有MLLM在跨视角推理上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法区分模型是对视觉干扰的鲁棒性还是具备真正的跨视角证据融合能力，导致无法准确评估MLLM在复杂跨视角场景下的表现。

Method: 引入了包含3,300个QA对的SSMNBench，将任务细分为单视角充分性（SVS）和多视角必要性（MVN），并在17个先进的MLLM上进行了视角扰动评估。

Result: 实验显示模型在SVS中表现出严重的“干扰退化”，在MVN中无法有效融合跨视角的几何证据，更多依赖单图语义平均而非真正的跨视角合成。

Conclusion: SSMNBench暴露了当前多模态大模型在跨视角理解上的根本性缺陷，为未来开发具备跨视角感知能力的多模态架构提供了诊断工具。
...
### [113] [Towards a Dynamic and Fixed-budget Memory Bank for Efficient Streaming Video Understanding](https://arxiv.org/abs/2606.25658)
*Baiyang Song,Yuli Lin,Qiong Wu,Tao Chen,Jun Peng,Xiao Chen,Yiyi Zhou,Rongrong Ji*

Main category: cs.CV

TL;DR: 本文提出了一种名为 CausalMem 的免训练动态视觉记忆更新机制，用于解决多模态大模型在流式视频理解中的内存瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 流式视频理解需要处理持续增加的视频帧，且未来视频内容和指令具有不可预测性，对大模型的内存和计算提出了严峻挑战。

Method: 提出 CausalMem 方法，通过构建动态且固定预算的记忆库，利用在线语义基评估视觉 Token 的冗余度并进行更新，以模拟人类大脑保留关键语义信息。

Result: 在 LLaVA-OneVision 和 Qwen2.5-VL 上的实验显示，该方法在流式和离线视频理解上分别获得 +3.2% 和 +3.0% 的准确率提升，实现了超 20 倍的 Token 压缩率，一小时视频仅需 12k 个 token（约 82MB 存储）。

Conclusion: CausalMem 在流式视频理解中表现出卓越的语义保留能力和极高的压缩效率，证明了动态记忆机制在多模态大模型中的有效性。
...
### [114] [What Does the Brain See? Multiview Neural Representations to Demystify the Brain-Visual Alignment](https://arxiv.org/abs/2606.25718)
*Salini Yadav,Taveena Lotey,Pravendra Singh,Partha Pratim Roy*

Main category: cs.CV

TL;DR: 本文提出了一种多视角EEG表征学习框架，通过联合建模时间、频域和空间结构，显著提升了零样本视觉解码的性能。


<details>
  <summary>Details</summary>
Motivation: 传统的EEG-视觉对齐方法依赖于整体EEG嵌入，忽视了脑电信号中互补的时间、频域和空间特征，且EEG具有低信噪比和非平稳性等挑战。

Method: 提出一种统一的多视角框架，共同建模状态空间时间动力学、可学习的小波谱分解以及注意力调制的图神经网络空间结构，并通过带正则化的对比学习与视觉表征对齐。

Result: 在THINGS-EEG基准上取得SOTA性能，受试者内Top-1准确率达54.8%，跨受试者达15.3%，且在跨会话评估中达到40.8%的Top-1准确率。

Conclusion: 显式建模多视角神经结构能有效改善EEG视觉解码中的语义对齐能力和模型泛化性。
...
### [115] [Falcon: Functional Assembly and Language for Compositional Reasoning in X-ray](https://arxiv.org/abs/2606.25701)
*Yonathan Michael,Mohamad Alansari,Natnael Takele,Andreas Henschel,Naoufel Werghi*

Main category: cs.CV

TL;DR: 本文针对X射线安检提出了Falcon框架和Falcon-X基准，旨在通过结构化安全状态接口提升多模态模型对空间分散组件的组合威胁推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统的视觉语言模型侧重于单目标检测，但在安检中，威胁往往由空间分散的多个组件组合而成，现有模型在处理这种组合安全推理时面临挑战。

Method: 引入Falcon框架，将分割特征抽象为包含组件存在性、成对兼容性和场景风险的结构化安全状态，并作为中间接口注入语言模型；同时构建了Falcon-X评估基准。

Result: 实验表明，现有模型难以应对组合安全推理，而Falcon显著提升了功能性定位能力，并给出了更连贯的威胁评估结果。

Conclusion: 本研究确立了组合安全推理是评估多模态系统安全性的一个独特且重要的新范式。
...
### [116] [UniTeD: Unified Temporal Diffusion for Joint Perception and Planning in Autonomous Driving](https://arxiv.org/abs/2606.25736)
*Bo Zhao,Xinting Zhao,Naifan Li,Erkang Cheng,Haibin Ling*

Main category: cs.CV

TL;DR: 本文提出UniTeD框架，通过在共享生成空间中联合建模感知与规划，并引入时序模块与锚点更新策略，实现了最先进的端到端自动驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将扩散模型局限于规划模块，且感知与规划解耦导致感知误差传递，降低了系统的鲁棒性。

Method: 提出统一时序扩散框架（UniTeD），通过双向信息交换联合建模感知与规划；引入时序转换模块（TTM）解决历史帧噪声失配，以及锚点更新策略（ARS）缓解训练与推理的分布偏移。

Result: 在多个基准测试中均达到了最先进的性能，超越了近期基于判别式和扩散式规划的端到端自动驾驶方法。

Conclusion: 将感知与规划整合进统一的扩散模型并结合时序上下文，能有效提升自动驾驶系统的鲁棒性和规划准确性。
...
### [117] [Efficient Real-World Dehazing via Physics-Inspired Global-Local Decoupling](https://arxiv.org/abs/2606.25732)
*Yifei Qu,Ru Li,Junjie Chen,Jinyuan Wu*

Main category: cs.CV

TL;DR: 提出了一种轻量级的物理启发式全局-局部解耦网络（PGL-Net），在显著降低延迟的同时实现了最先进的真实单幅图像去雾效果。


<details>
  <summary>Details</summary>
Motivation: 现有的去雾方法要么依赖于过于简化且脆弱的物理反演模型，要么采用参数量巨大的盲去雾架构，难以满足边缘设备对轻量化和低延迟部署的需求。

Method: 提出PGL-Net，通过算子级模拟引入物理归纳偏置，将去雾任务解耦为全局分布矫正和局部结构细化；设计了物理启发式仿射融合（PAF）模块进行全局对齐，以及退化感知调制（DAM）模块来自适应恢复空间和光谱局部细节。

Result: 在多个真实世界基准上达到SOTA性能。相比最近的SOTA模型SGDN，其轻量版（PGL-Net-T）在提升下游目标检测精度的同时，将PSNR提升了高达2.6dB，且推理延迟降低了10倍以上。

Conclusion: PGL-Net通过物理先验与高效网络的有机结合，成功解决了真实场景图像去雾中精度与计算开销之间的矛盾，极具实际应用价值。
...
### [118] [Dual Distribution Estimation for Zero-shot Noisy Test-Time Adaptation with VLMs](https://arxiv.org/abs/2606.25758)
*Wenjie Zhu,Yabin Zhang,Liang Xu,Xin Jin,Wenjun Zeng,Lei Zhang*

Main category: cs.CV

TL;DR: 提出了一种无需训练的双分布估计（DDE）框架，用于解决嘈杂测试时适应（NTTA）中的OOD检测与ID分类问题。


<details>
  <summary>Details</summary>
Motivation: 现有的零样本NTTA方法依赖测试时判别训练，导致过度自信的误分类、低推理效率，且易受OOD样本干扰。

Method: 提出DDE框架，包含正特征分布估计（PFDE）和负标签分布估计（NLDE），通过免训练的高斯分布建模来代替传统的实例级学习。

Result: 在ImageNet上，DDE提升了3.70%的调和平均准确率，降低了6.20%的OOD检测FPR95，且推理高效、对稀缺数据鲁棒。

Conclusion: DDE成功将NTTA转化为免训练的分布建模问题，在保证高效在线推理的同时，显著提升了模型对OOD噪声的鲁棒性。
...
### [119] [ShutterMuse: Capture-Time Photography Guidance with MLLMs](https://arxiv.org/abs/2606.25763)
*Jiayu Li,Yixiao Fang,Tianyu Hu,Wei Cheng,Ping Huang,Zheheng Fan,Gang Yu,Xingjun Ma*

Main category: cs.CV

TL;DR: 本文提出了CaptureGuide-Bench基准、CaptureGuide-Dataset数据集以及ShutterMuse模型，旨在解决多模态大语言模型在拍摄时构图指导与姿态推荐方面的研究空白。


<details>
  <summary>Details</summary>
Motivation: 现有的美学裁剪基准主要评估事后裁剪，忽视了拍摄时的相机定位和主体姿态推荐，导致大模型在实时拍摄指导方面的能力未得到探索。

Method: 引入了包含双任务的基准CaptureGuide-Bench，构建了包含130K样本的CaptureGuide-Dataset，并基于监督微调与强化学习开发了统一模型ShutterMuse。

Result: ShutterMuse在构图指导上取得最佳综合性能，并在被摄者姿态推荐上达到极具竞争力的表现，同时显著降低了推理成本。

Conclusion: 本研究展示了多模态大语言模型作为实时拍摄交互式助手的巨大潜力。
...
### [120] [$S^{2}$-FracMix: Label-Preserving Self-Saliency Mixup Augmentation](https://arxiv.org/abs/2606.25784)
*Khawar Islam,Arif Mahmood,Xin Jin,Naveed Akhtar*

Main category: cs.CV

TL;DR: 提出了一种名为 $S^2$-FracMix 的单图像数据增强框架，通过融合自显著性块与分形模式，避免了跨样本混合带来的语义破坏。


<details>
  <summary>Details</summary>
Motivation: 传统的跨样本 Mixup 数据增强方法计算开销大，且容易导致图像语义冲突和破坏。

Method: 结合了自显著性混合（提取单张图像的多尺度显著区域并插入其非显著区域）与分形混合（在显著区域自适应注入自相似分形图案）。

Result: 在分类、鲁棒性、校准、目标检测及迁移学习等7个基准测试中均取得了最先进的（SOTA）性能。

Conclusion: $S^2$-FracMix 是一种针对性强且结构一致的单图像增强策略，理论与实验均证明了其超越现有方法的优越性。
...
### [121] [Graph it first! Enabling Reasoning on Long-form Egocentric Videos through Scene Graphs](https://arxiv.org/abs/2606.25842)
*Agnese Taluzzi,Riccardo Santambrogio,Simone Mentasti,Chiara Plizzari,Matteo Matteucci*

Main category: cs.CV

TL;DR: 提出第一人称场景图（EgoSGs）框架，将长视频转化为紧凑的文本结构化场景图，解决了多模态大模型在长视频问答中的Token限制问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在处理长视频时受限于输入Token上限，不得不进行大量视频帧下采样，导致丢失关键的时间和上下文信息，这在相机移动频繁、状态变化复杂的第一人称视频中尤为严重。

Method: 提出EgoSGs框架，将视频转化为时间对齐的结构化文本场景图（包含物体、属性、空间关系及时间上的交互），在大幅缩减输入长度的同时保留丰富的语义信息。

Result: 在HD-EPIC VQA数据集上取得了最先进的成果，在多个模型上均显著超越了强劲的视频基线模型。

Conclusion: 像EgoSGs这样结构化、时间对齐的表示方法，能够有效解决长视频理解与多模态大模型上下文长度限制之间的矛盾。
...
### [122] [Naturalness Predicts but Does Not Cause Transferability in Image Encodings of Real-World Streams](https://arxiv.org/abs/2606.25844)
*Faruk Alpay,Baris Basaran*

Main category: cs.CV

TL;DR: 探讨了一维信号图像化后，其视觉自然度（FID）与预训练视觉模型迁移准确率之间的关系，指出局部结构而非频谱自然度是决定性能的关键。


<details>
  <summary>Details</summary>
Motivation: 许多研究将一维信号转换为图像以复用预训练视觉模型，但编码图像的视觉自然度如何影响冻结骨干网络的迁移准确率尚不清楚。

Method: 构建了包含299个时间序列的WorldStream数据集；使用可逆编码器，通过调整频谱指数 $\beta$（改变频谱自然度）和相位扰动（破坏局部结构）两种受控干预方法，分析FID与准确率的因果关系。

Result: 跨编码方式下FID能很好地预测准确率（Spearman $\rho = -0.72$）。但在改变频谱指数的干预中，准确率并未随FID改善而提升；而在相位扰动干预中，FID和准确率随局部结构的破坏而同步下降。且全微调也无法弥补与结构化基线的性能差距（27% vs 67%）。

Conclusion: 编码图像的自然度与分类准确率的关联是由局部结构而非频谱自然度介导的，将信号直接编码为图像在结构上存在缺陷，导致其表现远不及专门的结构化基准方法。
...
### [123] [Shift Variant Image Degradation and Restoration Using Singular Value Decomposition](https://arxiv.org/abs/2606.25818)
*Arun D. Kulkarni*

Main category: cs.CV

TL;DR: 本文提出了一种基于奇异值分解（SVD）的框架，用于恢复因空间变化运动模糊退化的图像。


<details>
  <summary>Details</summary>
Motivation: 空间变化图像退化因其点扩散函数随位置变化而难以用单一卷积核表示，导致传统恢复方法难以奏效。

Method: 利用奇异值能量保持准则控制噪声放大，通过SVD分析并求解由位置相关PSF（如线性、高斯、简谐运动）构建的空间变化成像算子。

Result: 在三种退化图像上的实验证明，该方法能有效恢复图像细节并减少模糊伪影。

Conclusion: 该SVD框架为空间变化运动模糊图像的稳定重建提供了一种有效且系统化的去噪平衡方案。
...
### [124] [USS: Unified Spatial-Semantic Prompts for Embodied Visual Tracking with Latent Dynamics Learning](https://arxiv.org/abs/2606.25880)
*Yuchen Xie,Xinyu Zhou,Kuangji Zuo,Yanshuo Lu,Fengrui Huang,Boyu Ma,Jianfei Yang*

Main category: cs.CV

TL;DR: 本文提出了USS框架，将具身视觉跟踪（EVT）的目标指示方式从纯文本扩展为统一的空间-语义提示，并结合潜在动力学学习以提升跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 传统的具身视觉跟踪主要依赖文本提示，但在包含多个同类物体的复杂场景中容易产生目标定位歧义。

Method: 提出端到端框架USS，支持文本、点、边界框和掩膜等多种提示输入，利用混合注意力机制融合特征，并通过潜在世界模型预测未来表征以增强时间鲁棒性。

Result: 实机实验表明空间提示在应对干扰和长时跟踪时优于纯文本；仿真实验中，该方法在非大模型方法中达到SOTA，且比大模型方法推理速度更快、性能相近。

Conclusion: 空间-语义提示为具身视觉跟踪提供了一种更精确、更灵活的目标指示接口。
...
### [125] [SurgAtlas: A Large-Scale Surgical Video-Language Dataset with 2,391 Hours of Open and Minimally Invasive Surgery](https://arxiv.org/abs/2606.25905)
...
TL;DR: 本文推出了SurgAtlas，这是目前最大且首个大规模包含开放手术的手术视频-语言数据集，并为手术视频理解和推理提供了标准化基准。


<details>
  <summary>Details</summary>
Motivation: 现有手术数据集规模有限，且缺乏对开放手术的覆盖以及多层级的语义标注，限制了通用手术多模态AI模型的发展。

Method: 构建了包含1.5万个视频的SurgAtlas数据集，利用结合LLM的自动多级管道进行细粒度标注，并基于该数据集对Qwen3-VL-8B模型进行两阶段微调。

Result: 微调后的模型在手术阶段识别、三元组检测和推理问答等多个建立的基准测试中均取得了竞争性或最先进（SOTA）的性能表现。

Conclusion: SurgAtlas填补了大规模开放手术视频数据的空白，为未来开发下一代多模态手术AI基础模型提供了关键的数据支持和基准。
...
### [126] [OracleAnalyser: Analysing Implicit Semantics of Oracle Bone Scripts through MLLMs with Post-training](https://arxiv.org/abs/2606.25906)
*Zijia Song,Yelin Wang,Zhengyi Ma,Zitong Yu,Tianheng Wang,Jiahuan Zhang,Taorui Wang,Kaicheng Yu*

Main category: cs.CV

TL;DR: 本文提出了一个用于甲骨文分析的推理框架 OracleAnalyser，通过引入 SFPO 算法和构建新数据集，显著提升了模型对甲骨文的分析与推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的甲骨文研究主要局限于简单的识别任务，忽视了同样关键的甲骨文释读与分析工作。

Method: 基于后训练技术微调 Qwen2.5-VL-3B-Instruct 模型，提出了一种定制的稳定焦点偏好优化（SFPO）算法，并构建发布了甲骨文推理数据集、偏好数据集及评估基准。

Result: 实验表明，仅有 3B 参数的 OracleAnalyser 在甲骨文分析任务上表现优异，性能超越了许多参数量更大的模型。

Conclusion: 本研究成功将甲骨文研究从单纯的“识别”拓展至“分析”阶段，为古文字的智能化解读提供了高效的推理框架和基准支持。
...
### [127] [A Benchmark for Heterogeneous Stereo Deblurring with Physically- and Epipolar-constrained Cross Attention](https://arxiv.org/abs/2606.25962)
*Hoju Shin,Jiah Kim,Seung-Wook Kim,Seowon Ji*

Main category: cs.CV

TL;DR: 本文提出了一个针对异构双目相机去模糊的框架，包含首个真实异构双目数据集（HSD）和一种新型的物理与极线约束交叉注意力模块（PECA）。


<details>
  <summary>Details</summary>
Motivation: 现代智能手机的异构双目相机常导致严重的非对称模糊，而现有去模糊方法大多基于同构假设，无法有效处理这种异构退化。

Method: 1. 构建HSD数据集；2. 提出PECA模块，将跨视图匹配限制在基于光学原理的极线搜索窗内；3. 设计置信度加权注意力与残差融合机制，在可靠区域进行跨视图引导，在遮挡区域回退至自去模糊。

Result: PECA作为一种即插即用的轻量化模块，显著提升了CNN、Transformer及NAFNet等基线模型在HSD数据集上的恢复性能，且具有优异的计算效率。

Conclusion: 该研究成功解决了异构双目图像去模糊难题，通过物理约束的跨视图特征融合实现了高效、可靠的图像修复。
...
### [128] [In-context Region-based Drag: Drag Any Region to Any Shape](https://arxiv.org/abs/2606.25907)
*Jiacheng Sui,Tianyu Hao,Bingjie Gao,Li Niu,Guangtao Zhai*

Main category: cs.CV

TL;DR: 本文提出了一种基于上下文学习的区域拖拽图像编辑方法 ICRDrag，通过引入注意力正则化机制和构建新数据集，提升了区域编辑的准确性与保真度。


<details>
  <summary>Details</summary>
Motivation: 现有的拖拽式图像编辑主要关注点对点拖拽，这具有固有的模糊性，因此需要更明确的区域拖拽编辑方法。

Method: 引入 ICRDrag 框架，结合源图像、源区域掩码和目标区域掩码；提出图像-掩码注意力一致性和源-目标注意力对应两项正则化技术；并构建了配对区域数据集（PRD）。

Result: 实验和用户研究表明，ICRDrag 在编辑准确性和视觉保真度上显著优于现有方法。

Conclusion: ICRDrag 和 PRD 数据集为区域拖拽编辑提供了强有力的解决方案，推动了图像精确编辑技术的发展。
...
### [129] [Taxonomy-aware deep learning for hierarchical marine species classification in underwater imagery](https://arxiv.org/abs/2606.25989)
*Dan Zimmerman,Dimitris A. Pados,George Sklivanitis*

Main category: cs.CV

TL;DR: 本文提出了一种结合生物分类层次结构的深度学习框架，用于解决水下图像中海洋物种的自动分类难题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在跨平台领域偏移、近缘物种细粒度相似性以及标注粒度不均等问题上面临挑战，难以实现高效的海洋生物多样性监测。

Method: 结合了分类学加权损失、最小风险贝叶斯推理、多尺度特征编码以及独立的分类头，使训练与推理均与生物分类的层次结构相一致。

Result: 在FathomNet 2025数据集上达到了1.581的平均分类学距离，与第一名方案（1.535）差距在3%以内。

Conclusion: 指标对齐的推理和简单解耦的组件在面对分布偏移时，比复杂的学习依赖关系具有更好的泛化能力。
...
### [130] [FunPiQ: A New Benchmark for Pixel-Level Quality Assessment in Fundus Images](https://arxiv.org/abs/2606.25915)
*Pengwei Wang,José Morano,Virginia Mares,Hrvoje Bogunović*

Main category: cs.CV

TL;DR: 本文引入了首个提供像素级质量标注的眼底图像质量评估（FIQA）数据集 FunPiQ，并提出了一种基于可解释性设计的评估方法 EFIQA-CP。


<details>
  <summary>Details</summary>
Motivation: 现有的眼底图像质量评估方法主要依赖图像级标签，其标准不一且缺乏对局部退化区域的定量评估，难以满足临床上对可信、任务无关及可解释性的要求。

Method: 构建了包含像素级标注的 FunPiQ 数据集，并提出了 EFIQA-CP 方法，该方法通过解剖结构可见性生成质量伪标签，并利用非负正无标记学习（Non-Negative Positive-Unlabeled learning）来训练卷积神经网络。

Result: 实验结果表明，在与各类分类后验解释方法及异常检测方法的对比中，所提出的可解释性设计方法（尤其是 EFIQA-CP）表现出了最优越的性能。

Conclusion: 基于解剖结构可见性的像素级质量评估是一种更具解释性且任务无关的有效途径，FunPiQ 数据集与 EFIQA-CP 方法为实现更可信的眼底图像质量评估奠定了基础。
...
### [131] [From Sparse and Imperfect 2D Anchors to Consistent 3D Gaussian Street Scenes: Support-Aware Appearance](https://arxiv.org/abs/2606.26007)
*Long Cao,Zhongquan Wang,Jie Li,Yuhan Chen,Kefei Qian,Xiangfei Huang,Guofa Li*

Main category: cs.CV

TL;DR: 本文提出了一种针对3D高斯场景的外观烘焙方法，通过教师相对外观残差蒸馏解决了独立编辑视图之间不一致和噪声传播的问题。


<details>
  <summary>Details</summary>
Motivation: 图像先验生成的编辑视图缺乏3D一致性，直接拟合会传播噪声，且现有方法无法兼顾不完美的稀疏锚点与标准光栅化部署。

Method: 引入教师相对外观残差蒸馏技术，利用残差进行频率分解和置信度估计，结合高斯空间聚合正则化与置信度门控优化，最后将残差烘焙至球谐系数中。

Result: 在Waymo和Tanks and Temples数据集上的实验表明，该方法在目标对齐、内容保留、伪影抑制和跨视图一致性上均优于现有编辑基线。

Conclusion: 该方法成功实现了高质量的3D高斯外观烘焙，在抑制编辑噪声的同时保持了跨视图一致性，且不增加推理开销。
...
### [132] [MIMFlow: Integrating Masked Image Modeling with Normalizing Flows for End-to-End Image Generation](https://arxiv.org/abs/2606.26016)
*Yang Chen,Xiaowei Xu,Shuai Wang,Xinwen Zhang,Qiushi Guo,Tiezheng Ge,Limin Wang*

Main category: cs.CV

TL;DR: 本文提出了 MIMFlow，一个将掩码图像建模（MIM）与规范化流（NF）结合的端到端框架，通过解耦语义表示与像素合成来提升生成质量和表示学习效率。


<details>
  <summary>Details</summary>
Motivation: 规范化流（NF）因严格的可逆性导致模型过度关注低级像素细节而忽略高级语义结构，且现有 MIM 与生成模型的结合较为松散。

Method: 引入 VAE 编码器从掩码图像中推断语义隐空间，使 NF 专注于建模低频语义流，并利用专用解码器负责高频细节合成，实现生成任务的解耦。

Result: 在 ImageNet 256x256 数据集上，MIMFlow-L 达到 71.3% 的线性探测准确率和 2.50 的 FID，仅使用 128 个 token 便比同等规模的 NF 基线提升了 32.8% 的性能。

Conclusion: MIMFlow 成功解决了 NF 的容量瓶颈，实现了全局结构一致性与高效生成，同时兼顾了强大的表示学习能力。
...
### [133] [How Robust is OCR-Reasoning? Evaluating OCR-Reasoning Robustness of Vision-Language Models under Visual Perturbations](https://arxiv.org/abs/2606.26041)
*Yuxing Cheng,Yuan Wu,Yi Chang*

Main category: cs.CV

TL;DR: 本文提出了OCR-Robust基准，用于评估视觉语言模型（VLM）在视觉扰动下进行OCR推理的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管VLM在OCR任务上表现强劲，但它们在视觉退化（如图像模糊或损坏）下的鲁棒性尚不明确，这对于需要精确文本和结构理解的OCR推理至关重要。

Method: 构建了包含两个子集（OCR1.0和OCR2.0，共812个样本）的OCR-Robust基准，筛选出5种代表性视觉扰动（3个严重程度等级），并使用清洁准确率、相对损坏保留率（RCR）、最坏情况保留率（WCR）和损坏鲁棒性指数（CRI）评估了18个模型。

Result: 实验表明，高清洁准确率并不等同于强鲁棒性；在最坏情况下，模型在结构敏感任务上的性能会显著下降；相比于文档类输入，图表和表格在扰动下更为脆弱。

Conclusion: OCR-Robust基准揭示了现有VLM在视觉扰动下OCR推理的局限性，特别是在处理结构化数据时表现出的脆弱性。
...
### [134] [MVTrack4Gen: Multi-View Point Tracking as Geometric Supervision for 4D Video Generation](https://arxiv.org/abs/2606.26087)
...
TL;DR: 本文提出了 MVTrack4Gen，一种利用多视角点追踪作为几何和运动监督信号，来提升单目视频新视角生成一致性的运动感知训练框架。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单目视频生成新视角时，要么受限于重建模块的几何精度，要么难以保持跨视角的几何与运动一致性。

Method: 利用注意力层中编码的对应线索，引入辅助多视角追踪头，并使用点追踪目标联合训练扩散模型，以显式增强运动感知的对应关系。

Result: 在多项基准测试中，该方法在保持几何一致性方面达到了最先进的水平，并实现了极具竞争力的相机精度。

Conclusion: 通过显式加强运动感知对应关系，能有效提升新视角视频生成模型对参考视频运动的遵循能力及跨视角几何一致性。
...
### [135] [DomainShuttle: Freeform Open Domain Subject-driven Text-to-video Generation](https://arxiv.org/abs/2606.26058)
*Nan Chen,Yiyang Cai,Rongchang Xie,Junwen Pan,Cheng Chen,Weinan Jia,Zhuowei Chen,Wen Zhou,Zhenbang Sun,Wenhan Luo*

Main category: cs.CV

TL;DR: 本文提出 DomainShuttle 方法，旨在解决主题驱动文本生成视频中跨领域泛化与高保真度难以兼顾的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法过度关注域内场景的主题保真度，导致其在跨领域场景（如新风格、新语义组合）中的编辑性和适应性受到限制。

Method: 提出了 Domain-MoT（结合领域感知 AdaLN 进行特定建模）、Video-Reference DualRoPE 空间建模方案，以及 Cross-Pair Consistent 损失函数。

Result: 实验证明，DomainShuttle 在保持高主题保真度的同时，显著提升了跨领域视频生成的灵活性，性能表现优于现有方法。

Conclusion: DomainShuttle 有效实现了兼顾高保真度与生成灵活性的开放域视频个性化生成。
...
### [136] [TryOnCrafter: Unleashing Camera Trajectories for Realistic Video Virtual Try-on via a Renderable 4D Try-on Proxy](https://arxiv.org/abs/2606.26092)
*Hao Sun,Hao Yan,Mengting Chen,Quanjian Song,Yu Li,Juan Cao,Jinsong Lan,Xiaoyong Zhu,Bo Zheng,Sheng Tang*

Main category: cs.CV

TL;DR: 本文提出了 TryOnCrafter 框架，首次实现了支持任意相机轨迹控制的视频虚拟试衣（CaM-VVT）新任务。


<details>
  <summary>Details</summary>
Motivation: 传统的视频虚拟试衣依赖固定的相机轨迹，无法满足全方位视角探索的交互自由度需求。

Method: 引入可渲染的 4D 试衣代理（结合 3DGS 与 SMPL-X 对衣物与人体建模），并利用代理锚定视频 DiT 进行物理一致的几何约束与高保真视频合成。

Result: 在任意相机移动下实现了逼真的动态虚拟试衣，并支持人体重定位、“子弹时间”和 360 度环绕视角等多种下游应用。

Conclusion: TryOnCrafter 结合了显式 4D 代理与生成式 DiT，有效解决了相机可控视频虚拟试衣中的视角切换与非刚性形变同步难题。
