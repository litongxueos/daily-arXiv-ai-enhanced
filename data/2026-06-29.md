...
### [1] [Advancing Speaker-Based Vocal Effort Classification with WavLM and Data Augmentation in Naturalistic Non-Calibrated Speech Recordings](https://arxiv.org/abs/2606.27543)
*Zahra Omidi,John H. L. Hansen*

Main category: cs.SD

TL;DR: 本文通过引入WavLM模型、数据增强和高斯邻域软标签技术，显著提升了发音努力度分类的准确率，创下了AVID数据集的新SOTA。


<details>
  <summary>Details</summary>
Motivation: 发音努力度属于连续体，相邻类别易混淆，且标注数据稀缺，导致现有自监督学习方法在分类时存在边界预测错误的问题。

Method: 首次将WavLM应用于该任务，系统性研究了多种数据增强策略（如RIR卷积、加性噪声等），并提出高斯邻域软标签来建模连续的发音努力度，辅以渐进式解冻训练。

Result: 数据增强使模型性能提升了0.6%至1.8%，结合渐进式解冻和高斯邻域软标签的WavLM-BASE系统在AVID数据集上取得了78.2%的平均准确率。

Conclusion: 该研究有效缓解了发音努力度分类中的边界混淆与数据稀缺问题，取得了当前最佳性能。
...
### [2] [Learning from Annotation Uncertainty: Entropy-Aware Curriculum for Speech Emotion Recognition](https://arxiv.org/abs/2606.27536)
*Zahra Omidi,John H. L. Hansen*

Main category: cs.SD

TL;DR: 在语音情感识别中，采用基于分布的监督方法比传统硬标签能更好地捕捉评估者之间的分歧与感知不确定性。


<details>
  <summary>Details</summary>
Motivation: 传统的语音情感识别通常采用硬共识标签，这抹杀了评估者之间的分歧和情感的主观不确定性。

Method: 使用WavLM-Base多任务模型在MSP-Podcast 2.0数据集上进行9类情感识别，对比了硬标签训练与基于评估者投票分布的监督训练。

Result: 分布式监督显著改善了与人类投票分布的对齐（降低了JSD/KLD）。硬标签倾向于将模糊语音归为“其他”类，而分布式监督能更好地将不确定性合理分配到具体情感类别中。

Conclusion: 研究结果支持在语音情感识别中超越硬标签，采用能反映听众分歧的分布目标来提升系统对感知不确定性的建模能力。
...
### [3] [Room for Error: Large-Scale Simulation of Over-the-Air Acoustic Attacks](https://arxiv.org/abs/2606.27701)
*Andrew C. Cullen,Neil Marchant,Jiani Xie,Paul Montague,Benjamin I. P. Rubinstein*

Main category: cs.SD

TL;DR: 本文提出了一个高吞吐量的物理模拟框架，用于更准确地评估和提升物理世界中语音对抗攻击的有效性与隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 现有的语音对抗攻击研究难以有效迁移至物理世界，因为它们通常忽略了声学因素（如可探测性和几何结构）的影响，限制了对实际安全风险的理解。

Method: 通过真实世界测试，提出了一个高吞吐量的现实模拟框架，并构建了双形式信噪比（Dual-Form SNR）指标来解耦攻击源的隐蔽性与受害者端攻击的有效性。

Result: 在超过800万次对抗性评估中，表明声学感知使得在Whisper和wav2vec模型上的相对字错率（WER）提升了高达94.5%。

Conclusion: 该框架解决了当前研究中忽略实际声学环境的局限性，为未来在真实物理声学环境中进行可重复、可验证的安全研究奠定了基础。
...
### [4] [A Flexible Encoding Model for Non-Unique Note Alignments](https://arxiv.org/abs/2606.28032)
*Suhit Chiruthapudi,Adam Štefunko,Silvan Peter,Patricia Hu,Jan Hajič,Carlos Eduardo Cancino-Chacón*

Main category: cs.SD

TL;DR: 提出了一种针对Match文件格式的向后兼容扩展，以支持非唯一且语义复杂的符号音乐对齐。


<details>
  <summary>Details</summary>
Motivation: 现有的音乐对齐编码格式仅支持一对一的唯一对应，无法满足排练重复和通奏低音即兴演奏等复杂音乐实践中对灵活、非唯一对齐的需求。

Method: 对Match文件格式进行扩展：引入“虚拟乐谱音符”和“虚拟演奏音符”以支持多重链接编码，并扩展'section'行以包含演奏区域的语义标注。

Result: 通过钢琴排练和通奏低音两个代表性应用场景，展示了该扩展格式在处理复杂对齐关系时的实用性。

Conclusion: 该研究为符号音乐对齐提供了一种灵活、向后兼容且能处理复杂语义关系的编码解决方案。
...
### [5] [Grammar-Guided Hierarchical Parsing for Long-form Audio Activity Recognition](https://arxiv.org/abs/2606.27965)
*Peng Zhang,Qingyu Luo,Philip J. B. Jackson,Wenwu Wang*

Main category: cs.SD

TL;DR: 一种基于语法引导解码的层次化解析方法，仅需事件级监督即可将长音频解析为一致的活动树。


<details>
  <summary>Details</summary>
Motivation: 先前的方法单独建模音频的不同层次，导致层级间不一致且需要多层级的监督标签。

Method: 提出层次活动语法（HAG）以编码层次组成和时序约束，并进行语法引导的解码。

Result: 在MultiAct数据集上提高了时序一致性（Edit分数），并在无需活动和子活动标签的情况下生成了可解释的层次树。

Conclusion: 该方法证明了通过语法约束可以仅从事件级证据有效推导出一致且可解释的长音频层次活动结构。
...
### [6] [From General-Purpose Audio Tagging to Spatially Grounded Sound Event Localization and Detection](https://arxiv.org/abs/2606.27751)
*Stefano Giacomelli,Stefano Damiano,Claudia Rinaldi,Fabio Graziosi,Toon van Waterschoot*

Main category: cs.SD

TL;DR: 本文提出了AT2SELD框架，旨在将预训练的通用音频标注模型扩展应用于空间声源定位与检测任务。


<details>
  <summary>Details</summary>
Motivation: 研究如何在数据、计算和部署受限的条件下，利用预训练音频标注模型中的语义先验来支持空间声源定位与检测。

Method: 结合预训练骨干网络与一阶高保真立体声空间处理，采用多阶段神经网络架构搜索（NAS）优化架构耦合，并结合活动条件监督与门限校准。

Result: 确定了最佳的空间特征接口与架构组件，焦点损失与活动监督有效改善了定位表现，且在STARSS23等多个数据集上验证了跨数据集的泛化能力。

Conclusion: 通用音频标注先验在嵌入空间感知架构并进行针对性校准后，对声源定位与检测（SELD）的设计具有广阔的应用前景。
...
### [7] [DG^VoiC: Speaker Clustering for Fraud Investigation under Real Call-Centre Conditions](https://arxiv.org/abs/2606.28048)
*Muhammad Shakeel Akram,Amal Htait,Abdul Hamid Sadka,Emma Meisingseth,Karishma Jaitly*

Main category: cs.SD

TL;DR: 本文提出了一种名为 DG^VoiC 的语音聚类框架，用于在匿名呼叫中心音频中进行说话人关联，以协助检测保险欺诈。


<details>
  <summary>Details</summary>
Motivation: 保险欺诈检测成本高且操作困难，现有的欺诈检测主要依赖结构化数据或文本，而通话中重复出现的说话人身份这一关键线索未被充分利用。

Method: 该框架结合了敏感信息对齐的匿名化技术、语音预处理、滑动窗口说话人嵌入提取以及基于余弦相似度的聚类方法。

Result: 在真实电话音频数据集上进行评估，该方法达到了 96% AMI、95% ARI、98% 完整度、100% 同质性和 99% V-measure 的优异表现。

Conclusion: 说话人聚类技术能够有效验证说话人一致性并识别跨客户的重复声音，为欺诈调查提供了强有力的辅助信号。
...
TL;DR: 本文提出了“万维模型网（AI-ModelNet）”的概念与架构，旨在实现异构大模型之间的互联、能力共享与协同推理。


<details>
  <summary>Details</summary>
Motivation: 大模型高昂的训练和部署成本推动了轻量化和专用模型的发展，而如何让这些分散的异构模型实现有效交互与协作成为当前的技术瓶颈。

Method: 借鉴互联网的设计理念，提出了AI-ModelNet的系统愿景与分层架构，并通过构建原型系统及多样化应用案例来验证该框架的可行性。

Result: 成功开发了AI-ModelNet原型系统，并通过实际应用案例验证了其在促进模型互联、能力共享和协同推理方面的可行性与有效性。

Conclusion: AI-ModelNet为多模型协同提供了一种新型网络范式，为克服单模型局限性及推动AI模型网络化发展指明了方向。
...
TL;DR: 提出了一种三阶段训练范式，为大语言模型智能体构建具备真实预测能力的内部世界模型，以实现更优的前瞻性规划。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在长程任务中缺乏内部世界模型来模拟未来。直接在预测轨迹上进行微调仅会导致表面的模仿，无法获得真正的预测能力（存在格式-能力差距）。

Method: 提出一种三阶段训练范式：(1) 世界模型智能体中期训练（WM-AMT）注入潜在预测能力；(2) 格式启发微调（FE-SFT）结构化该能力；(3) 前瞻条件强化学习（FC-RL）优化生成模拟的校准度与实用性。

Result: 在搜索和数学推理任务中，该方法一致优于其他训练基线，生成了更具接地气和校准良好的前瞻预测。

Conclusion: 在LLM智能体中构建有效的内部世界模型需要采用“能力优先”的训练管线，而不是简单的格式微调。
...
### [10] [When Does Personality Composition Matter for Multi-Agent LLM Teams?](https://arxiv.org/abs/2606.27443)
*Aryan Keluskar,Amrita Bhattacharjee,Huan Liu*

Main category: cs.AI

TL;DR: 本研究探讨了人格提示对多智能体团队任务性能的影响，发现其效果高度依赖于具体的任务结构。


<details>
  <summary>Details</summary>
Motivation: 虽然人格提示能改变大语言模型的交流风格，但这种行为变化如何影响其在多智能体合作中的客观任务绩效尚未得到系统研究。

Method: 在结构化编程、开放式研究协作和竞争性谈判三个任务域中，操纵前沿LLM的人格特质（如宜人性），并评估其对多智能体团队表现的影响。

Result: 人格效应取决于任务结构：在编程任务中，低宜人性虽改变了交流方式但对任务完成影响较小；但在开放式协作和谈判中，低宜人性会显著降低团队性能。

Conclusion: 设计多智能体系统时需要考虑任务结构与智能体人格的匹配，且人格操纵对任务表现的影响存在局限性。
...
TL;DR: 本文提出了 DysLexLens，这是一个用于分析读写障碍学习者 AI 使用体验的低资源大语言模型框架。


<details>
  <summary>Details</summary>
Motivation: 尽管读写障碍学习者越来越多地使用 AI 工具，但他们的实际体验尚未得到充分研究，且社交语言媒体数据普遍存在噪声和低资源限制。

Method: 开发了 DysLexLens 框架，结合了字典驱动过滤、大语言模型语义分析与知识图谱推理、定量评估（RAGAS 和查询鲁棒性）以及定性验证指南。

Result: 在 Reddit 数据和 30 个测试问题上的实验表明，该框架能有效提取和分析用户体验，并具有推广到其他低资源论坛的潜力。

Conclusion: DysLexLens 提供了一种可证据追溯的端到端方案，能有效从嘈杂社交媒体中挖掘特定人群的 AI 使用体验，促进了该领域的复现性研究。
...
### [12] [Odyssey: Constructing Verifiable Local Truth-Preserving Foundation Models](https://arxiv.org/abs/2606.27593)
*Sridhar Mahadevan*

Main category: cs.AI

TL;DR: 本文介绍了 ODYSSEY，一个用于构建可验证、局部保真基础模型的范畴论框架，利用‘铸造厂’（foundries）架构和阚扩展（Kan extensions）实现知识的结构化融合与验证。


<details>
  <summary>Details</summary>
Motivation: 旨在解决现有基础模型在处理异构知识源时，缺乏局部真理性、可验证性以及系统化知识融合机制的问题。

Method: 提出了 ODYSSEY 框架，采用‘铸造厂’作为知识层构建模块，通过通用铸造厂学习（UFL）的左右阚扩展机制进行约束与推广，并使用 FSQL 和 TICKET 机制进行查询与外部模型认证。

Result: 该框架已完全实现，并在多种场景下通过测试，证明了其在领域构建、人工件重放、层诊断、大模型审查以及因果主张提取等任务中的有效性。

Conclusion: ODYSSEY 成功地将范畴论数学机制应用于基础模型的构建与维护，为可验证和高保真的大模型应用提供了可行的技术路径。
...
TL;DR: 提出MER-R1框架，通过强化学习融合多模态情绪识别中“快思考”与“慢思考”的互补优势，达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 发现显式推理（慢思考）并不必然提升多模态情绪识别准确率；快思考擅长召回，慢思考擅长精确，两者存在互补但缺乏协同优化机制。

Method: 提出MER-R1强化学习框架，采用双目标解耦分别优化召回与精确，并通过快慢置信度校准使慢思考的决策与快思考的直觉对齐。

Result: 在MER-UniBench和MME-Emotion数据集上均取得当前最优（SOTA）表现，使推理真正赋能情绪识别。

Conclusion: 结合快思考的召回导向直觉与慢思考的精确导向选择性，能显著提升情绪识别表现，并在理论上降低了优化过程中的方差干扰。
...
TL;DR: 提出了一种基于符号反馈驱动的迭代自我精炼框架，以提升大语言模型在长程规划中的鲁棒性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在面对复杂的长程决策任务时，容易输出不可行或错误的规划，缺乏安全性和可靠性。

Method: 结合了将逻辑符号转化为自然语言的提示机制、用于识别错误并生成纠错指令的符号验证器，以及用于预测目标可达性的规划识别器。

Result: 实验表明，该框架在长程规划任务中显著提升了生成方案的可行性与正确性。

Conclusion: 该方法有效增强了基于大语言模型规划的可靠性，为构建更具可信度的人工智能系统提供了可行路径。
...
TL;DR: 本文研究了图世界模型（GWM）在长期规划中的rollout误差，并提出了误差感知优化方法。


<details>
  <summary>Details</summary>
Motivation: 在图结构环境中进行规划时，局部预测误差可能随拓扑结构传播，导致长期rollout误差累积，特别是在动态边预测的场景下。

Method: 构建了统一的固定与动态边GWM框架，推导了区分拓扑与模型放大效应的误差界，并提出结合谱正则化、rollout一致性及关键节点加权的Error-Aware GWM。

Result: 实验表明，Error-Aware GWM能有效抑制长期rollout的发散并保持预测精度，且动态边训练对结构演化环境至关重要。

Conclusion: 图世界模型最适用于动态图rollout和智能体规划，而专用图模型在静态或稀疏预测任务中仍具有优势。
...
### [16] [ToE: A Hierarchical and Explainable Claim Verification Framework with Dynamic Multi-source Evidence Retrieval and Aggregation](https://arxiv.org/abs/2606.27736)
*Zhaoqi Wang,Zijian Zhang,Kun Zheng,Zhen Li,Xin Li,Chunlei Li,Jiamou Liu*

Main category: cs.AI

TL;DR: 本文提出ToE框架，通过构建层级证据树和强化学习驱动的检索机制，显著提升了自动事实校验的准确性与对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: AI生成的虚假信息以及生成引擎优化（GEO）投毒威胁了信息生态系统，极易污染大语言模型（LLM）的推理过程。

Method: 提出“证据树”（Tree of Evidence, ToE）框架，结合强化学习多源检索智能体、证据评估智能体和论证树聚合算法进行动态迭代校验，并提供了检索过程的理论误差界限证明。

Result: 在多个数据集和多种LLM上的实验表明，ToE较对比基线提升了4至24个百分点，在应对对抗性投毒输入时优势尤为显著。

Conclusion: ToE证明了结合动态层级推理与强化学习检索在提高自动事实校验准确性、可解释性及抵御对抗攻击方面的有效性。
...
TL;DR: 提出SD-GPS框架，通过将符号求解器作为执行神谕来优化几何问题求解中的自动公式化和定理预测。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号几何求解器存在自动公式化与求解器脱节、定理预测受限于固定规则而易陷僵局的问题。

Method: 采用求解器驱动的自动公式化（基于强化学习提升可执行性）与验证定理提议（在僵局时生成并验证辅助引理）。

Result: 在Geometry3K和PGPS9K数据集上，SD-GPS在各项指标上均优于现有的MLLM和神经符号方法。

Conclusion: 闭环多模态感知与符号执行能显著提升几何推理能力，为神经智能体结合形式化系统提供了可验证的求解方案。
...
TL;DR: 提出了评估多模态大模型在具身智能规划中遵循隐式社会规范的新基准 NormAct，并提出了 NormPerceptor 方法以提升规划的规范符合度与任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的具身智能评估主要关注显式目标的达成，忽略了模型在行动规划中推断并应用隐式社会规范约束的能力。

Method: 引入了将隐式规范嵌入日常任务的基准 NormAct；并提出了 NormPerceptor，一个在规划前推断场景相关规范的上下文条件提示生成器。

Result: 主流模型的目标达成率达 67.3%，但规范合规率仅为 26.4%；而使用 NormPerceptor 后，整体任务成功率从 24.2% 提升至 46.7%。

Conclusion: 具身智能体需要具备主动检测隐式规范、结合视觉证据进行具身落地，并将其整合为行动规划约束的能力。
...
### [19] [ATOD: Annealed Turn-aware On-policy Distillation for Multi-turn Autonomous Agents](https://arxiv.org/abs/2606.27814)
*Qitai Tan,Zefang Zong,Yang Li,Peng Chen*

Main category: cs.AI

TL;DR: 本文提出了一种结合在线策略蒸馏（OPD）和强化学习（RL）的新算法ATOD，旨在提升小型语言模型智能体在长程交互任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 在训练小模型智能体时，OPD前期收敛快但受限于教师水平上限，而RL虽然探索能力强且上限高，但因奖励稀疏导致前期效率低下，两者存在互补空间。

Method: 提出ATOD算法，包含两个核心设计：一是采用退火式的OPD-RL调度机制，由OPD逐步过渡到RL；二是引入轮次级分歧不确定性重加权（T-DUR），以优化长轨迹中的细粒度监督。

Result: 在ALFWorld、WebShop和Search-QA上的实验表明，ATOD的平均成功率比OPD高出3.03个百分点，比GRPO高出23.62个百分点，且超越了教师模型2.16个百分点。

Conclusion: ATOD通过有效结合蒸馏与强化学习，并引入轮次级加权，成功打破了传统蒸馏的性能天花板，实现了高效且突破教师上限的智能体训练。
...
TL;DR: 提出GILP框架，通过结合小型参数化世界模型与大语言模型代理，显著降低规划中的状态幻觉率并提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 旨在解决基于大模型的智能体规划时易产生状态幻觉且难以评估，以及参数化世界模型作为独立规划器能力较弱的问题。

Method: 引入接地迭代语言规划（GILP），利用训练的小型参数化骨干网络提供有效动作和状态预测，与LLM生成的动作进行对比，并通过一致性门控在冲突时触发LLM修正。

Result: 在真实GPT-4o-mini测试中，GILP将幻觉状态率从0.176降至0.035；在模拟实验中，成功率从0.668提升至0.838，仅增加约22%的LLM调用量。

Conclusion: 结合参数化世界模型与LLM推理的混合架构，能以较低的额外调用成本有效减少大模型幻觉，大幅增强其规划能力。
...
TL;DR: 本文提出 RelBall 模型，通过引入模长变换和尾部中心关系球，同时解决了非交换复合关系、语义层级以及复杂一对多关系的建模难题。


<details>
  <summary>Details</summary>
Motivation: 现有的知识图谱补全模型（如 RotatE 和 Rotate3D）无法有效同时处理非交换复合关系、语义层级结构以及多对多等复杂实体映射关系。

Method: 扩展 Rotate3D 并提出 RelBall 模型：采用模长变换来建模层级关系（抽象概念对应小模长，具体实例对应大模长），并引入尾部中心关系球以支持一对多等复杂关系映射。

Result: 在多个基准数据集上的实验结果表明，RelBall 在链接预测任务上取得了相较于多种基线模型更具竞争力的表现。

Conclusion: RelBall 能够全面覆盖多种关系模式，提供具有可解释性的层级表示，并有效解决了复杂多对多关系的建模问题。
...
### [22] [Ontology-Guided Evidence Path Inference for Multi-hop Knowledge Graph Question Answering](https://arxiv.org/abs/2606.28076)
*Yongxue Shan,Meihan Wu,Cundi Fang,Jie Peng,Xiaodong Wang*

Main category: cs.AI

TL;DR: 本文提出了一种名为 OPI 的本体引导证据路径推理框架，用于解决多跳知识图谱问答（KGQA）中搜索空间爆炸和语义约束失效的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多跳 KGQA 方法主要依赖以主题为中心的路径扩展，导致搜索空间因混合类型噪声路径而急剧增长，且检索到的路径往往无法满足复杂问题的语义约束。

Method: 提出 OPI 框架，引入关系中心本体图以捕获类型约束，并设计了结合主题端前缀扩展与答案端终尾匹配的双向检索机制，最后通过迭代细化策略过滤无关证据并重新评估候选答案。

Result: 实验表明，OPI 显著缩小了搜索空间，在 WebQSP 数据集上将 Hit@1/F1 提升了 4.6/5.0 个百分点，在 CWQ 数据集上提升了 8.9/3.3 个百分点，并在 MetaQA 上取得了接近饱和的准确率。

Conclusion: 利用本体引导的路径推理能有效抑制噪声路径并缩小搜索空间，从而大幅提升多跳知识图谱问答的性能。
...
### [23] [Lifted Causal Inference](https://arxiv.org/abs/2606.28024)
*Malte Luttermann,Tanya Braun,Ralf Möller,Marcel Gehrke*

Main category: cs.AI

TL;DR: 本文将提升推理应用于关系领域的因果效应计算，提出了参数化因果因子图（PCFGs）及提升因果推理（LCI）算法，显著提升了推理效率。


<details>
  <summary>Details</summary>
Motivation: 旨在解决在关系领域中高效计算因果效应的问题，并通过利用提升推理来加速传统因果推断方法的计算瓶颈。

Method: 引入了参数化因果因子图（PCFGs）和部分有向参数化因果因子图（PD-PCFGs），并提出了提升因果推理（LCI）算法及其扩展版本。

Result: 实现了在提升层面上高效计算因果效应，相比传统的命题因果推理大幅提高了计算速度，并能有效处理不完全的因果知识。

Conclusion: 提升推理可以成功应用于因果推断，且所提出的 PD-PCFGs 扩展了该方法在较少先验因果知识场景下的适用性。
...
### [24] [JD Oxygen AI Item Center (Oxygen AIIC) V1: An Industrial-Scale LLM/VLM-Centric Solution for Item Understanding, Management, and Applications](https://arxiv.org/abs/2606.28070)
...
TL;DR: 本文介绍了京东构建的Oxygen AIIC平台，这是一个基于多模态大模型（LLM/VLM）构建的工业级商品知识生成与服务平台。


<details>
  <summary>Details</summary>
Motivation: 在海量商品（SKUs）的电商环境下，传统方法难以应对新概念快速涌现、海量SKU高质量知识生成以及多样化下游需求等工业级挑战。

Method: 提出了包含四大支柱的Oxygen AIIC架构：1) 人机协作的本体工程；2) 语义搜索后判别（S2D）的高吞吐知识识别架构；3) 自演进且可控的商品理解大模型；4) 统一的商品数据与服务枢纽，并在华为昇腾NPU上部署运行。

Result: 商品知识生成准确率达94.2%，召回率达82.8%，积累了数千亿条知识资产。应用后，搜索流量覆盖率达到80.4%，信息质量问题减少37%，核心属性自动填充率超过80%。

Conclusion: Oxygen AIIC成功解决了大规模电商场景下的商品知识管理难题，在搜索、推荐和运营等实际业务中实现了显著的效率和质量提升。
...
TL;DR: 本文提出了一种利用深度学习和生成式AI的计算设计合成（CDS）框架，旨在实现高科技系统设计的自动化。
...
Method: 提出计算设计合成（CDS）框架，并结合电子驱动系统设计和空间尺寸问题两个案例研究进行验证。

Result: 成功展示了AI驱动的方法在减少人工干预的情况下进行自主设计的能力。

Conclusion: 该研究标志着工程设计从传统的基于模拟的优化向高度自主设计的根本性转变。
...
TL;DR: 本文提出了串联强化学习（TRL），将串联训练引入可验证奖励强化学习（RLVR）中，以提升强弱模型协同推理的兼容性与可读性。


<details>
  <summary>Details</summary>
Motivation: RLVR虽能提升模型推理能力，但会导致推理过程产生可读性差等漂移现象，且现有的协同训练范式未能在现代RLVR的长推理链中得到验证。

Method: 提出TRL方法，使强模型与冻结的弱模型随机交替生成推理步骤，并使用标准GRPO损失函数对强模型进行训练。

Result: 实验表明，TRL在独立推理能力上与传统GRPO相当，同时显著提高了与弱模型的交接鲁棒性，减少了分布漂移，并生成了对弱模型更易读的思维链。

Conclusion: TRL为解决RLVR中的多模型通信与人类兼容性问题提供了一条行之有效的途径。
...
### [27] [Agent-Native Immune System: Architecture, Taxonomy, and Engineering](https://arxiv.org/abs/2606.28270)
*Bo Shen,Lifeng Chang,Tianyuan Wei,Yunpeng Li,Feng Shi,Yichen Han,Peijie Gao,Shiyi Kuang,Xin Chang,Dehui Li*

Main category: cs.AI

TL;DR: 本文提出了智能体原生免疫系统（ANIS），这是一种直接嵌入智能体认知循环的动态内生防御架构，用于应对运行时的安全威胁。


<details>
  <summary>Details</summary>
Motivation: 现有的外围安全和静态对齐机制位于智能体推理循环之外，无法有效防御内存毒化、工具链操纵等运行时劫持攻击。

Method: 设计了六层免疫塔（L0-L5）、定义了智能体病毒与疫苗的分类法，并构建了基于Harness三元组（Meta, Self, Auto）的持续免疫学习机制。

Result: 成功构建了动态的运行时“执法”防御机制，明确区分了训练期静态对齐与运行期动态免疫，并提出了自身免疫率等评估指标。

Conclusion: 智能体安全必须引入动态的内生免疫机制，未来研究需关注免疫协议标准化、新评估指标及病原体与疫苗的共演化动力学。
...
### [28] [Not All Relations Rotate Alike: Transformation-Aware Decoupling for Viewpoint-Robust 3D Scene Graph Generation](https://arxiv.org/abs/2606.27412)
*Jingjun Sun,Chaowei Wang,Zhirui Liu,Jiaxu Tian,Ming Yang,Yaoxing Wang,Shan Gao*

Main category: cs.CV

TL;DR: 本文提出了TAD框架，通过解耦视角稳定与视角变动的关系推理，提升了三维场景图生成在视角旋转下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的三维场景图生成模型在视角发生偏航角变化时，无法处理方向性关系（随视角变）与语义/接触关系（视角无关）之间的变换异构性问题。

Method: 提出了变换感知解耦（TAD）框架，将关系推理拆分为视角稳定和方向性（视角变动）两个分支，并引入变换特定描述符和组感知辅助监督进行优化。

Result: 在3DSSG数据集上，TAD在无需训练时旋转增强的情况下，实现了针对视角变化的SOTA鲁棒性，并保持了优异的基准性能。

Conclusion: 通过解耦不同变换特性的关系推理，能够有效解决三维场景理解中因视角变化导致的关系预测失真问题。
...
### [29] [SemCityLoc: Aerial 6DoF Localization Using Semantic 3D City Models](https://arxiv.org/abs/2606.27444)
*Jingfeng Mao,Xuyang Chen,Qilin Zhang,Oussema Dhaouadi,Guangming Wang,Brian Sheil,Daniel Cremers,Yan Xia,Olaf Wysocki*

Main category: cs.CV

TL;DR: 提出SemCityLoc系统，通过对齐视觉先验与标准化3D城市模型，实现高精度且可扩展的无人机空中6DoF定位。


<details>
  <summary>Details</summary>
Motivation: 传统的空中定位依赖精确的GNSS或复杂的辐射度3D重建，限制了其在大范围及机载环境下的部署与应用。

Method: 将空中位姿估计重构为结构化表面配准问题，对齐基础模型衍生的视觉语义表面/单目深度与轻量级3D城市建筑模型，并推出SemCityLockeD评估基准。

Result: 相比现有地图定位方法，在城市峡谷环境中召回率提升达36%，平均位置误差从9.89米降低至2.62米。

Conclusion: 语义结构化几何能够提供足够且具扩展性的约束，无需辐射度三维重建即可实现高精度空中定位。
...
### [30] [DMV-Bench: Diagnosing Long-Horizon Multimodal Agents' Visual Memory with Incidental Cue Injection](https://arxiv.org/abs/2606.27499)
*Yujin Tang,Chenming Shang,Ruize Xu,Nikhil Singh*

Main category: cs.CV

TL;DR: 本文推出了首个用于测试多模态智能体视觉记忆的交互式基准DMV-Bench，并提出了基于双重编码理论的DualMem记忆架构。


<details>
  <summary>Details</summary>
Motivation: 现有的智能体记忆研究几乎完全集中在文本层面，缺乏评估智能体在交互式环境中真正记住“所见”图像而非“所写”文本能力的基准。

Method: 开发了DMV-Bench基准，通过无文本线索的商品图像链测试视觉记忆；并提出了DualMem架构，在智能体中并行维护视觉和文本双重编码。

Result: DualMem在不同长度的记忆测试中均显著优于字幕基线及其他三种多模态智能体记忆系统，展示了强大的视觉记忆检索能力。

Conclusion: 结合视觉与文本的并行编码架构（DualMem）能有效解决智能体在交互式任务中的长程视觉记忆与召回难题。
...
### [31] [Fine-tuning a multimodal large language model for clinician-grade autism behavioral scoring from short home videos](https://arxiv.org/abs/2606.27484)
...
TL;DR: 本文提出了一种通过微调多模态大模型（Gemini 2.5 Pro）分析家庭视频行为特征，以实现自闭症谱系障碍（ASD）早期快速辅助诊断的方法。


<details>
  <summary>Details</summary>
Motivation: 自闭症的诊断通常较晚，利用家庭视频等易获取数据进行AI定量诊断，能有效促进早期发现与及时治疗。

Method: 利用低秩适应（LoRA）在400个临床评分的家庭视频上微调Gemini 2.5 Pro，使其能够识别30种关键行为特征，并在99名儿童的数据集上进行验证。

Result: 微调后模型与临床评估的一致性提升了40%，直接诊断的F1分数提升了53%，辅助诊断管线实现了77%的准确率和86%的AUC，表现比肩临床医生。

Conclusion: 微调的多模态大语言模型能够作为高效、可扩展的行为特征提取器，助力自闭症的评估与临床诊断。
...
### [32] [SelectAnyTree: A Promptable Instance Segmentation Model for 3D Forest LiDAR Point Clouds](https://arxiv.org/abs/2606.27491)
...
TL;DR: 提出了一种名为SelectAnyTree的提示式三维点云单木分割模型，仅需少量点击即可高效准确地分割森林点云中的单棵树木。


<details>
  <summary>Details</summary>
Motivation: 三维森林点云数据量大且树冠重叠复杂，人工标注单木实例极其耗时费力，且现有自动预分割方法缺乏高效的交互式微调手段。

Method: 引入了Click-to-query提示编码器（将点击转化为三维特征查询）、冠层高度模型（CHM）引导的首次提示（自动确定树梢），以及状态空间查询解码器（以线性复杂度捕获长距离上下文）。

Result: 在多个森林数据集上表现出强泛化性，单次点击分割IoU达78.2%（比最强基线高出24.8%），能以最少的点击次数和更少的参数及推理时间达到目标精度。

Conclusion: SelectAnyTree成功将提示式分割引入三维森林点云领域，为大规模森林点云的高效交互式单木分割与标注提供了高精度且实用的解决方案。
...
### [33] [Aloe-Vision: Robust Vision-Language Models for Healthcare](https://arxiv.org/abs/2606.27500)
...
TL;DR: 本文推出了开源医疗多模态大模型Aloe-Vision、高质量训练数据集Aloe-Vision-Data以及低污染评估基准CareQA-Vision，在提升医疗多模态能力的同时揭示了模型面对对抗性输入时的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 医疗多模态大模型（LVLMs）的发展受限于高质量医疗多模态数据的缺乏、安全敏感场景下的鲁棒性问题以及评估基准狭窄且可能存在数据污染的问题。

Method: 构建了融合医疗与通用领域的开源数据集Aloe-Vision-Data，基于此训练并开源了7B和72B的Aloe-Vision模型，并基于西班牙医学考试开发了CareQA-Vision评估基准。

Result: Aloe-Vision在保持通用能力的同时显著提升了医疗多模态性能，达到前沿水平；同时评估显示现有模型在面对对抗性和误导性输入时仍存在脆弱性。

Conclusion: 高质量的混合训练数据能产生性能均衡的医疗LVLM，但临床应用仍需克服对抗性鲁棒性等安全可靠性挑战。
...
### [34] [ReWorld: Learning Better Representations for World Action Models](https://arxiv.org/abs/2606.27504)
...
TL;DR: 本文提出了ReWorld，这是首个专为自动驾驶世界动作模型设计的表示学习框架，通过直接优化中间表示提升了生成与规划性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注模型架构设计，忽略了如何高效学习用于规划的世界表示，且标准训练仅监督输出端，导致中间表示只能被间接塑造。

Method: 将中间表示作为直接优化目标：在Video DiT上施加未来预测监督，在Action DiT上进行跨模态对齐，并引入难负样本监督以强化安全关键边界的区分度。

Result: 在nuScenes和NAVSIM上，ReWorld将视频生成FVD提升了23.9%，闭环PDMS从89.1提升至90.4，且从头训练的收敛速度加快了约2倍。

Conclusion: 直接监督和多维度优化中间表示能有效提升自动驾驶世界动作模型的表征能力、规划效果及训练效率。
...
TL;DR: TruEye是一个快速且可解释的检测模型，用于对AI生成或篡改的人像与场景进行细粒度检测与定位。


<details>
  <summary>Details</summary>
Motivation: 现有检测器存在易过拟合、黑盒二分类以及依赖高成本大语言模型（LLM）来生成解释的问题。

Method: 引入掩码条件双流Transformer，分离人类与场景的Token并保持空间对应，结合区域门控交叉注意力和Token级监督，实现无需LLM的组合分类。

Result: 在6个数据集及新构建的FineSyn数据集上，TruEye在准确率和泛化能力上均超越了SOTA方法，且运行速度比基于LLM的方法快100倍以上。

Conclusion: TruEye为检测复杂的AI合成与篡改图像（特别是人-景合成图像）提供了一种高效、高泛化性且具备可解释性的解决方案。
...
### [36] [Structured-Li-GS: Structured 3D Gaussians Splatting with LiDAR Incorporation and Spatial Constraints](https://arxiv.org/abs/2606.27509)
*Huaiyuan Weng,Huibin Li,Chul Min Yeum*

Main category: cs.CV

TL;DR: 本文提出了一种结合LiDAR-惯性-视觉SLAM的轻量级3D高斯泼溅（Structured-Li-GS）框架，仅需较少的高斯数量即可实现高质量的3D重建。


<details>
  <summary>Details</summary>
Motivation: 旨在利用LiDAR提供的高精度密集彩色点云，解决传统3D高斯泼溅中模型体积大、缺乏绝对尺度以及重建精度受限的问题。

Method: 利用下采样点云锚定高斯基元并根据局部几何初始化其参数；设计了结合光度、平坦化、偏移、深度和法线损失的综合训练策略，无需高斯致密化；并开发了硬件同步的LiDAR-相机手持扫描仪进行验证。

Result: 实验表明，Structured-Li-GS在基准数据集和真实世界数据集上，均以更少的高斯基元数量超越了现有的先进方法，实现了高保真度和适中模型大小的重建。

Conclusion: Structured-Li-GS证明了结合LiDAR先验信息的3DGS能够以更低的计算和存储成本实现高精度、带尺度的三维场景重建。
...
### [37] [Tessellating The Earth](https://arxiv.org/abs/2606.27514)
*Daniel Cher,Hamza Iqbal,Eric Xing,Brian Wei,Nathan Jacobs*

Main category: cs.CV

TL;DR: 提出了一种名为TTE的地理位置编码器，通过可学习的球形Voronoi剖分和全局语义标记，实现了动态区域表示和跨地域语义共享。


<details>
  <summary>Details</summary>
Motivation: 现有的地理编码器使用固定基底均匀分配表示能力，在无意义区域（如海洋）与关键区域（如城市）分配了相同资源，缺乏针对性。

Method: 使用可学习的Spherical Voronoi剖分构建地理编码器，使表示能力在训练中动态向判别性区域集中；同时引入全局语义标记，从卫星图像中蒸馏语义知识，允许地理上疏远但环境相似的区域共享语义信息。

Result: 在多项地理空间分类与回归任务中取得了新的SOTA，并在iNaturalist-2018细粒度物种分类中作为地理先验取得了最佳表现。

Conclusion: 通过结合自适应空间剖分与全局语义提炼，TTE成功克服了传统固定基底编码器的限制，显著提升了地理位置编码的效率和效果。
...
### [38] [MemoBench: Benchmarking World Modeling in Dynamically Changing Environments](https://arxiv.org/abs/2606.27537)
...
TL;DR: 介绍了一个名为 MemoBench 的新基准，用于评估视频生成模型在物体“消失再出现”动态场景下的记忆一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的视频评估基准大多只在物体可见时测试一致性，或仅在静态场景下测试遮挡，缺乏对物体在视线外且发生状态改变时的评估能力。

Method: 构建了包含 360 个合成与真实场景剪辑的 MemoBench 基准，并设计了一套结合自动指标与基于 VQA 的四维评估体系。

Result: 通过对 8 个主流前沿视频生成模型的评估，揭示了它们在处理“消失再出现”范式时在记忆一致性方面的关键不足与挑战。

Conclusion: 当前视频生成模型在模拟动态遮挡环境下的记忆一致性仍存在较大局限，MemoBench 为该领域提供了新的评估标准和改进方向。
...
### [39] [Beyond MoCap: Scaling Motion Tokenizers with Synthetic Human Motion for Generative Modeling](https://arxiv.org/abs/2606.27547)
*Yiwen Yan,Wanning He,Yu-Wing Tai*

Main category: cs.CV

TL;DR: 本文通过引入大规模合成数据与重新设计的 VQ-VAE 分词器扩展了动作表示空间，显著提升了人体动作生成的泛化与组合能力。


<details>
  <summary>Details</summary>
Motivation: 现有动作捕捉数据集规模和多样性有限，导致动作生成模型在面对罕见、复杂和高动态动作时泛化能力不足。

Method: 提出了一个利用大规模物理合理合成动作扩展表示空间的框架，并结合了可与扩展动作空间协同缩放的 VQ-VAE 分词器。

Result: 实验表明，使用合成动作进行训练显著提升了动作词汇的覆盖率和组合性，在文本生成动作和动作延续等任务中均取得了一致的性能提升。

Conclusion: 动作生成的主要瓶颈在于表示空间的局限性，而非仅由模型架构决定；通过协同缩放合成动作与表示学习，是实现高表现力动作合成的有效路径。
...
### [40] [Understanding Cross-Rig Generalization in Automotive Perception: a Multi-Rig Benchmark and Rig Variation Metrics](https://arxiv.org/abs/2606.27554)
*Tim Alexander Bader,Tim Dieter Eberhardt,Maximilian Dillitzer,Wilhelm Stork*

Main category: cs.CV

TL;DR: 引入了Plentiful CARLA Camera Rigs基准和几何描述符，以研究和预测相机布局变化对自动驾驶感知模型性能的影响。


<details>
  <summary>Details</summary>
Motivation: 实际应用中自动驾驶车辆的相机布局（位置、方向、FOV和数量）差异巨大，而现有模型多在固定布局下开发，导致了由几何观测变化引起的跨配置域偏差。

Method: 1. 提出Plentiful CARLA Camera Rigs基准，在14种相机布局下渲染相同的驾驶场景；2. 评估多视角感知算法的跨配置迁移能力；3. 提出两个几何描述符：Rig Variance（评估单配置内部多样性）和Rig Contrastive Distance（评估配置间几何差异）。

Result: 实验表明相机几何布局的变化会导致显著的性能波动，且几何配置差异与跨配置迁移的性能下降强相关；Rig Contrastive Distance可有效预测和量化模型在不同相机配置间的迁移难度。

Conclusion: 相机配置的几何差异显著影响多视角感知模型的泛化性能，所提出的基准和几何描述符为分析和解决跨配置域偏差提供了有效工具。
...
### [41] [Large Language Model Teaches Visual Students: Cross-Modality Transfer of Fine-Grained Conceptual Knowledge](https://arxiv.org/abs/2606.27527)
*Thomas Shih-Chao Liang,Zhuoran Yu,Yong Jae Lee*

Main category: cs.CV

TL;DR: 本文提出了 LaViD 框架，这是一种利用仅语言的大模型（LLM）通过多选题机制向仅视觉学生模型蒸馏语义知识的方法。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型拥有广泛的文本概念知识，但如何利用它们来指导和监督其他模态（如视觉）的模型仍未得到充分探索。

Method: 利用 LLM 生成区分不同视觉类别的多选题（MCQs），并将每个类别映射为这些多选题上的软标签分布，以此作为概念特征通过辅助蒸馏损失引导视觉学生模型。

Result: LaViD 在多个细粒度基准上优于利用视觉-语言模型的 MaKD 方法，并在与 DKD 和 MLKD 等视觉蒸馏方法对比时表现出竞争性或更优的性能，同时在 Waterbirds 数据集上显著提升了最差组准确率。

Conclusion: 该研究表明，即使没有图像数据，仅语言的教师模型也能够成功将高阶语义知识转移给视觉模型，并提高其对虚假相关性的鲁棒性。
...
### [42] [Radar Guided Camera Verification for Automatic Emergency Braking Rethinking Object Detection in Radar Camera Fusion](https://arxiv.org/abs/2606.27556)
*Ram Charan Akula,Sivanathan Kandhasamy,Manikandan Ganesan*

Main category: cs.CV

TL;DR: 提出一种轻量级、无需训练的雷达-相机融合障碍物验证方法，用于提升自动紧急制动（AEB）系统的计算效率与可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统AEB系统使用高计算成本的目标检测器来确认雷达检测到的物体，而实际上相机仅需验证障碍物是否存在，无需进行完整的目标识别。

Method: 提出一种基于雷达引导的边缘密度门（edge density gate）方法，在图像的感兴趣区域（ROI）内进行快速验证，该方法无需训练数据、模型权重或GPU加速。

Result: 在真实车辆测试中，该方法减少了高达98.7%的相机搜索空间，每帧ROI平均延迟仅0.121毫秒，AUC达0.898，召回率达0.994，并在33次模拟威胁场景中实现了零漏制动。

Conclusion: 该方法通过显著降低计算开销，提供了一种高效且实用的低功耗雷达-相机融合自动紧急制动解决方案。
...
### [43] [Distribution-based deep multiple instance learning for tumor proportion scoring in NSCLC](https://arxiv.org/abs/2606.27579)
*Krzysztof Pysz,Artur Bartczak,Jarosław Kwiecień,Piotr Krajewski,Witold Dyrka*

Main category: cs.CV

TL;DR: 本文提出了一种结合零膨胀贝塔（ZIBeta）分布的多示例学习（MIL）端到端框架，用以准确预测非小细胞肺癌（NSCLC）的肿瘤比例评分（TPS）。


<details>
  <summary>Details</summary>
Motivation: 评估NSCLC的TPS极度依赖耗时的人工标注且专家资源稀缺；同时，现有的多示例学习方法在处理无表达（零类别）图像时效果不佳。

Method: 提出一种双模型框架：首先通过嵌入提取与多分类网络捕获图像块的病理特征，然后利用MIL模型聚合这些特征来预测代表全片TPS概率分布的ZIBeta参数。

Result: 该方法显著优于传统的线性和岭回归基线模型，在仅使用全片级TPS评分作为标签的情况下，显著提高了预测准确性与模型的可解释性。

Conclusion: 这种基于分布的新型端到端架构能够有效克服零值图像带来的挑战，显著提升了全片TPS预测的精度和解释力。
...
### [44] [Perceptual 3D Simulation With Physical World Modeling](https://arxiv.org/abs/2606.27575)
*Wanhee Lee,Klemen Kotar,Rahul Mysore Venkatesh,Jared Watrous,Daniel L. K. Yamins*

Main category: cs.CV

TL;DR: 提出了 P3Sim，一个在不完整观测和3D变换信号下，通过结合学习推理与显式几何结构来模拟未来场景状态的物理世界建模系统。


<details>
  <summary>Details</summary>
Motivation: 现实世界的系统在预测3D变换后的场景演变时，往往只能依赖不完整和局部的感知输入，缺乏理想模拟器所具备的完整3D几何和动力学信息。

Method: P3Sim 由三个相互作用的组件组成：一个用于多模态场景变量概率推理的学习型物理世界模型、一个提供局部3D变换信号的几何条件模块，以及一个用于跨时间整合预测并保持一致性的持久场景记忆模块。

Result: 该系统成功结合了数据驱动的灵活性与几何归纳偏置，在诸如新视角合成、物体操作和动态场景预测等多种3D变换任务中表现出良好的泛化能力。

Conclusion: P3Sim 作为一种灵活的感知模拟器，有效推进了通用三维场景理解与变换的研究。
...
### [45] [DeLux: Cross-Modal Local Artifact Restoration in Video Using Neuromorphic Data](https://arxiv.org/abs/2606.27576)
*Bartosz Stachowiak,Dariusz Brzezinski*

Main category: cs.CV

TL;DR: 提出了 DeLux，一种利用神经形态事件流作为结构先验来检测和修复 RGB 视频中光照伪影的跨模态恢复方法。


<details>
  <summary>Details</summary>
Motivation: 传统 RGB 相机易受耀斑、眩光、闪烁和过曝等光照伪影影响导致信息丢失，而现有单模态方法难以恢复被严重掩盖的结构细节。

Method: 设计了 DeLux 模块化管线，利用事件相机的超高动态范围和时间分辨率特性作为结构先验，引导 RGB 视频中光照伪影的定位与针对性修复。

Result: 该方法优于仅依赖 RGB 的基线和事件引导的 HDR 模型，在各种伪影类型上实现了超过 0.99 的平均 MS-SSIM，并在真实车载视频中减少了高达 88% 的伪影严重程度。

Conclusion: 跨模态事件引导能够有效抑制局部光照伪影并重建受损区域，研究公开的合成工具和真实数据集将促进相关领域的研究。
...
### [46] [CoIn: Comprehensive 2D-3D Inpainting with Gaussian Splatting Guidance](https://arxiv.org/abs/2606.27584)
*Hana Kim,Minje Kim,Tae-Kyun Kim*

Main category: cs.CV

TL;DR: 提出了 CoIn 框架，通过双向一致性管线连接 2D 修复模型与 3D 高斯泼溅（3DGS），实现灵活的 3D 场景物体移除和插入。


<details>
  <summary>Details</summary>
Motivation: 现有的 3D 场景修复方法高度依赖精确的多视角分割掩码，且通常局限于物体移除任务，无法处理物体插入等更复杂的编辑。

Method: 引入多阶段一致性管线：先利用扩散模型生成初始修复图，再通过参考自适应 3DGS 进行粗糙 3D 重建，随后利用基于 GS 的特征投影引导扩散过程以确保多视角一致性，最后使用纹理增强判别器进行细化。

Result: 实验表明，CoIn 达到了最先进的性能，能够利用灵活的掩码输入，有效处理物体移除和物体插入任务。

Conclusion: 通过建立 2D 与 3D 之间的双向信息流，可以显著提升 3D 场景修复的质量、多视角一致性以及任务灵活性。
...
### [47] [Beyond Points: Spherical Distributional Part Prototypes for Interpretable Classification](https://arxiv.org/abs/2606.27582)
*Duarte Leão,Diogo Pereira Araújo,Catarina Barata,Carlos Santiago*

Main category: cs.CV

TL;DR: 论文提出了 vMFProto，这是一种基于 von Mises-Fisher 分布和最优传输的局部原型网络，有效提升了视觉模型解释的稳定性和质量。


<details>
  <summary>Details</summary>
Motivation: 现代视觉骨干网络的嵌入空间中存在显著的类内变异，导致传统点原型易冗余或不稳定，从而损害了模型的解释质量和鲁棒性。

Method: 提出 vMFProto 框架，将每个类别建模为超球面上的 von Mises-Fisher (vMF) 混合分布；使用熵正则化最优传输进行图像块到原型的分配；采用两阶段训练（OT驱动的原型发现与端到端细化），并结合了图像块级蒸馏和分布感知多样性正则化。

Result: 在 CUB-200-2011、Stanford Dogs 和 Stanford Cars 数据集（基于 DINO 骨干）上的实验表明，vMFProto 在保持竞争性准确率的同时，实现了最先进的解释质量（一致性、稳定性和独特性），并能产生非冗余的局部化证据。

Conclusion: 通过将原型建模为超球面分布并结合最优传输，vMFProto 成功克服了点原型的冗余与不稳定问题，为原型网络提供了一种更稳健的解释方案。
...
### [48] [Qwen-Image-2.0-RL Technical Report](https://arxiv.org/abs/2606.27608)
...
TL;DR: 本文介绍了 Qwen-Image-2.0-RL，一个利用人类反馈强化学习（RLHF）和同策略蒸馏（OPD）提升扩散模型视觉质量和指令遵循能力的后训练管线。


<details>
  <summary>Details</summary>
Motivation: 旨在解决现有扩散模型在视觉质量、指令遵循、美学度及图像编辑精度方面的不足，并建立可靠的奖励评估系统。

Method: 构建了基于视觉语言模型的任务特定复合奖励模型，采用基于 GRPO 的强化学习训练框架，并通过同策略蒸馏（OPD）将不同任务的专门化策略合并到单个学生模型中。

Result: 在 Qwen-Image-Bench 上取得 57.84 的综合评分（相比基座模型提升 2.61 分），在文生图和图像编辑竞技场中的 Elo 评分分别提升了 78 分和 93 分。

Conclusion: 该方法通过强化学习与策略蒸馏，成功且一致地提升了扩散模型的美学质量、提示词遵循度以及图像编辑的准确性。
...
TL;DR: 本文提出了 Fox，一个无需训练的推理期框架，通过诊断和干预注意力机制中的结构性错位来缓解大型视觉语言模型（LVLM）的物体幻觉问题。


<details>
  <summary>Details</summary>
Motivation: LVLM 容易出现物体幻觉。研究发现这是由于在决策关键步骤中，特定注意力头（危险介导者）脱离了视觉证据并锁定在语言先验上，形成了绕过视觉基础的病理快捷路径。

Method: 提出 Fox 框架，包含三个核心部分：使用视觉注意力熵探针无监督定位危险介导者；通过数值对数饱和进行靶向因果干预以切断快捷路径；采用冲突门控协同解码策略平衡干预忠实度与表达流畅度。

Result: 实验表明 Fox 达到了 SOTA 性能，在缓解幻觉方面超越了 SID 算法 29.1%，同时保留了语言生成的丰富性。

Conclusion: 通过在推理阶段对注意力机制中的结构性错位进行靶向因果干预，无需重新训练即可有效解决 LVLM 的物体幻觉问题。
...
### [50] [Explainable AI for Biodiversity Monitoring and Ecological Image Analysis](https://arxiv.org/abs/2606.27667)
*Brinnae Bent,Holly R. Houliston,Jiayi Zhou,Günel Aghakishiyeva,David W. Johnston*

Main category: cs.CV

TL;DR: 本文倡导在生态计算机视觉中引入可解释人工智能（XAI），以提升模型预测的透明度与可信度。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉虽推动了生物多样性监测，但模型决策过程难以审查，存在因伪相关或采样偏差导致错误决策的风险。

Method: 提供了XAI在图像分类、目标检测和图像分割中的应用指南，并通过海豹检测和鲸豚解剖分割两个航空影像案例进行演示。

Result: 展示了解释性方法如何识别生物学线索、揭示由背景和形状混淆导致的假阳性，并指导数据优化与模型改进。

Conclusion: XAI使生态模型的行为更加透明且可进行科学审查，从而为生物多样性保护提供更可靠、可理解和具行动力的证据。
...
### [51] [AI-Generated Image Recognition via Fusion of CNNs and Vision Transformers](https://arxiv.org/abs/2606.27637)
*Xuan-Bach Mai,Hoang-Minh Nguyen-Huu,Quoc-Nghia Nguyen,Hoang-Tung Vu,Minh-Triet Tran,Trung-Nghia Le*

Main category: cs.CV

TL;DR: 本文提出了一种基于融合策略的鲁棒检测方法，用于识别AI生成的图像。


<details>
  <summary>Details</summary>
Motivation: AI生成图像质量的提升使得真假难辨，对数据真实性和可靠性带来挑战，急需有效的检测手段。

Method: 利用融合策略，结合多种检测方法的优势来识别AI图像。

Result: 在CIFAKE数据集上取得了97.32%的准确率。

Conclusion: 该方法能有效区分AI生成图像与真实图像，对合成数据时代的数据认证技术做出了贡献。
...
### [52] [CascadeOcc: Rethinking 3D Occupancy World Models with Cascaded VQ Representations](https://arxiv.org/abs/2606.27644)
*Kyumin Hwang,Wonhyeok Choi,Jaeyeul Kim,Jihun Park,Daehee Park,Sunghoon Im*

Main category: cs.CV

TL;DR: 本文提出了CascadeOcc，一种专为自动驾驶设计的占用世界模型，通过构建时空双重分层机制来预测未来环境并规划轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有的占用世界模型过度依赖外部辅助模态或大语言模型，未能充分挖掘占用表示自身的内在结构潜力。

Method: 在自回归框架中集成级联向量量化（VQ）机制，采用从粗到细的多尺度架构，并引入TimeMixer以捕获多尺度时间依赖，构建时空双重分层机制。

Result: 在4D占用预测和运动规划基准测试中，CascadeOcc在纯视觉方法中取得了领先的性能表现。

Conclusion: 实验证明，优化占用表示的内在结构是替代依赖外部基础模型的强有力且有效的途径。
...
TL;DR: 提出SpatialUAV，一个用于评估多模态模型在无人机空间推理与协同能力的真实低空无人机基准。


<details>
  <summary>Details</summary>
Motivation: 现有的无人机基准缺乏对三维空间推理、多视角协同、动态场景和多样化任务格式的评估。

Method: 构建包含4,331个实例的基准，采用统一的VQA模式，结合检测器辅助、深度监督、人工标注与验证，支持多种输入输出格式。

Result: 现有视觉语言模型性能远低于人类水平，在跨视角关联、结构化定位和几何推理方面存在显著瓶颈。

Conclusion: 该基准为推进低空无人机空间智能提供了有力的实证指导和开源资源。
...
TL;DR: 提出了一种模拟生物海马体记忆机制的新型目标检测框架Hippocampus-DETR，显著提升了检测精度与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前目标检测模型缺乏显式的记忆机制，限制了其在复杂任务中的学习效率与鲁棒性。

Method: 在DETR架构中集成海马体记忆网络（HipNet），通过模拟海马体亚区结构实现模式分离与信息整合，并采用分层策略进行训练。

Result: 实验表明，该模型在目标检测精度上超越主流模型，且在少样本分类、多模态构建和图像修复等任务中展现出优秀的泛化性与高数据效率。

Conclusion: 本研究成功将神经认知机制引入深度学习，为提升模型学习效率和任务鲁棒性提供了一种有效的新途径。
...
### [55] [VLM-Aware Meta-Optic Front-End Design for Frozen Vision-Language Models](https://arxiv.org/abs/2606.27646)
*Chanik Kang,Raphaël Pestourie,Haejun Chung*

Main category: cs.CV

TL;DR: 提出CODA框架，通过将超透镜（meta-optics）的设计直接与冻结的下游视觉模型目标相对齐，显著提升了受限光学系统下的图像识别性能。


<details>
  <summary>Details</summary>
Motivation: 传统机器视觉依赖高质量、高成本的光学系统。微型超透镜虽是替代方案，但受物理效率限制。传统设计关注图像保真度，而非下游的机器识别需求。

Method: 提出联合设计框架CODA，利用可微图像形成和基于麦克斯韦方程模拟的伴随梯度，直接针对冻结的CLIP分类器的交叉熵损失优化超透镜设计，无需图像重建和信号处理（ISP）步骤。

Result: 在ImageNet-100基准上，将CLIP的零样本识别准确率从基线的53.75%提升至65.41%。该设计还能直接迁移应用到SigLIP、DINOv2等多种模型和数据集（CIFAR-100、Food-101）中。

Conclusion: 在受限的超透镜成像场景下，让光学设计直接对齐冻结的视觉模型目标，比追求传统图像保真度指标更能有效提升下游识别性能。
...
### [56] [Denoising ICF Images with Multiplicative Uniform Noise: A Self-Supervised Study Based on the Log-Domain Noisier2Inverse Framework](https://arxiv.org/abs/2606.27635)
*Gyeongha Hwang,Bradley Thomas Wolfe,Naima Naheed*

Main category: cs.CV

TL;DR: 本文提出并评估了Log-Domain Noisier2Inverse自监督去噪框架，用于解决惯性约束聚变（ICF）图像中的乘性均匀噪声问题。


<details>
  <summary>Details</summary>
Motivation: ICF图像常受到特有的乘性均匀噪声污染，且由于缺乏无噪声的真实图像进行监督训练，急需一种高效的自监督去噪方案。

Method: 开发并实现了对数域的Noisier2Inverse框架，理论上证明了其等价于变换域的监督学习；测试了基于均匀噪声加载和固定高斯噪声加载的两种变体，并与经典BM3D及自监督Noise2Self方法进行对比。

Result: 采用均匀噪声加载的变体B表现最佳，PSNR达到21.41 dB，SSIM达到0.8358，比噪声输入基线提升了19.46 dB，显著优于BM3D（4.47 dB）和Noise2Self（4.75 dB）。

Conclusion: 实验表明，Log-Domain Noisier2Inverse自监督框架在无需干净真实数据的情况下，能够有效去除ICF图像中的乘性均匀噪声，性能远超传统及现有自监督方法。
...
### [57] [Improving Adversarial Robustness via Activation Amplification and Attenuation](https://arxiv.org/abs/2606.27784)
*Taïga Gonçalves,Yongsong Huang,Tomo Miyazaki,Shinichiro Omachi*

Main category: cs.CV

TL;DR: 提出轻量级即插即用模块A3，通过联合学习放大和衰减激活信号，以极低的开销显著提升神经网络的对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 对抗攻击源于模型中的非鲁棒特征。现有防御方法通常通过剪枝或掩码减少其影响，而本研究旨在通过简单的激活缩放机制，联合学习放大和衰减这些非鲁棒信号。

Method: 引入A3模块，使用可学习的掩码和缩放因子动态重构激活值。同一组参数可通过翻转缩放符号在“放大”和“衰减”模式间切换。放大的信号作为负参考用于构建对比和排序损失，进而优化衰减模式下的性能。

Result: 实验表明，在放大模式下学习降低预测准确性可以同时提高衰减模式下的对抗鲁棒性。A3在多个基准测试和模型架构上均能有效提升鲁棒性，且计算和内存开销极小。

Conclusion: A3是一种高效、参数量极少的鲁棒性增强模块，证明了通过双向激活缩放来防御对抗攻击的有效性与可行性。
...
TL;DR: 本文提出了Reflect-R1，这是首个用于长视频理解的证据驱动自纠错框架，通过三阶段流水线和阶段解耦强化学习实现了基于客观证据的真实自我纠错。


<details>
  <summary>Details</summary>
Motivation: 现有的长视频理解反思机制缺乏外部客观证据，导致模型容易盲目自信且难以纠正错误，同时在多阶段反思中应用强化学习还存在策略耦合和训练数据匮乏的问题。

Method: 提出了包含直觉、验证和仲裁的三阶段自纠错框架，通过动态检索视觉证据来解决冲突；设计了阶段解耦强化学习算法（SD-GRPO）以独立计算各推理阶段的优势函数；并构建了一个包含12万样本的专用数据集。

Result: 在VideoMME和LongVideoBench等基准测试中取得了最先进的（SOTA）性能，显著提高了模型的真实纠正率，实现了基于客观证据的有效自纠错。

Conclusion: Reflect-R1通过引入外部证据驱动机制和解耦强化学习，成功打破了长视频理解中的幻觉循环，为该领域提供了更可靠的自纠错方案。
...
### [59] [Temporal-Emerged Prompting for Segment Anything in Multiframe Infrared Small Target Detection](https://arxiv.org/abs/2606.27655)
*Yinghui Xing,Donghao Chu,Shizhou Zhang,Di Xu*

Main category: cs.CV

TL;DR: 本文提出了一种名为TEP-SAM的框架，利用时序涌现提示将Segment Anything Model (SAM)应用于低信噪比红外序列中的小目标分割。


<details>
  <summary>Details</summary>
Motivation: 单帧红外图像中目标与背景极难区分，现有方法在低信噪比下容易失效，而目标在时序上会随时间推移逐渐从背景中显现。

Method: 提出TEP-SAM框架，通过联合建模全局运动模式和局部运动偏差来定位目标，并利用运动差异生成时序涌现提示，引导SAM进行非交互式分割。

Result: 实验证明，该方法在严重低信噪比条件和复杂动态背景下均取得了极其显著的分割效果。

Conclusion: 通过将大规模语义预训练与特定任务的时序建模相结合，TEP-SAM成功实现了SAM在多帧红外小目标检测任务中的有效适配。
...
TL;DR: 提出了一种基于视频的从粗到细估算框架，用于自动且高精度地统计帕金森患者转向时的步数。


<details>
  <summary>Details</summary>
Motivation: 传统穿戴式设备在日常连续监测中不够便利，且帕金森患者特有的拖步和运动变异性使基于视频的精确步数统计极具挑战。

Method: 利用3D人体网格恢复提取足部运动信号进行粗略估算，结合光流和交叉注意力机制捕捉细粒度步态动态，并采用多实例学习（MIL）进行步数残差预测。

Result: 在真实世界帕金森病患者转向数据集上的实验表明，该方法在步数估算性能上持续优于现有方法。

Conclusion: 该无源视频框架为帕金森患者提供了一种便捷、准确的转向运动功能评估手段，利于日常连续监测。
...
### [61] [GeoFace: Consistent Multi-View Face Generation with Geometry-Constrained Diffusion](https://arxiv.org/abs/2606.27659)
*Yeji Choi,Jinhyeok Choi,Jaewon Min,Minkyung Kwon,Jin Hyeon Kim,Seungryong Kim*

Main category: cs.CV

TL;DR: GeoFace是一个几何约束的多视角扩散框架，可从单张输入图像生成具有高度跨视角一致性的人脸图像与3D几何。


<details>
  <summary>Details</summary>
Motivation: 现有的多视角扩散模型在生成人脸时，缺乏显式机制来确保不同视角之间共享相同的3D结构，从而导致几何不一致问题。

Method: 提出联合生成多视角RGB图像和3D几何的双流框架，通过共享注意力层交互，并引入几何引导注意力对齐损失，使用FLAME模型推导的规范UV位置图作为跨视角几何约束。

Result: 在RenderMe-360和NeRSemble数据集上的实验证明，GeoFace在视觉质量和跨视角几何一致性上均显著超越现有方法。

Conclusion: 该方法成功解决了多视角人脸生成的几何一致性问题，并为高效的3D人脸重建提供了有力支持。
...
### [62] [MVPruner: Dynamic Token Pruning for Accelerating Multi-view Vision-Language Models in Autonomous Driving](https://arxiv.org/abs/2606.27660)
*Nan Yang,Zhanwen Liu,Linfeng Zhang,Shangyu Xie,Yang Wang,Wenzhuo Zhou,Xiangmo Zhao*

Main category: cs.CV

TL;DR: 本文提出了MVPruner，一种针对自动驾驶中多视角视觉语言模型（VLM）的双阶段自适应Token修剪方法，在几乎不损失精度的情况下大幅提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 多视角VLM由于庞大的视觉Token序列导致计算效率低下，而现有修剪方法使用固定修剪率和静态指标，无法适应多视角间的动态差异和推理过程中的信息需求变化。

Method: 提出双阶段修剪方法MVPruner：第一阶段根据各视角的视觉多样性动态分配预算并保留核心Token；第二阶段根据指令文本引导分配预算和选择Token以实现任务对齐。

Result: 在DriveLM基准测试上，结合MVPruner的DriveMM模型减少了87.3%的FLOPs，预填充阶段提速4.97倍，同时保留了98.5%的准确率。

Conclusion: MVPruner通过将修剪行为与模型的动态信息需求相结合，有效解决了多视角自动驾驶VLM的计算瓶颈，实现了高效与准确的平衡。
...
### [63] [ProMSA:Progressive Multimodal Search Agents for Knowledge-Based Visual Question Answering](https://arxiv.org/abs/2606.27974)
...
TL;DR: 本文提出了 ProMSA，一个针对知识图谱视觉问答（KB-VQA）的渐进式多模态搜索智能体，通过动态自适应的多模态检索和强化学习优化，显著提升了检索与问答准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的 KB-VQA 方法大多采用固定的“先检索再生成”管线，检索器固定且参数静态，在推理过程中缺乏自适应性。

Method: 提出了 ProMSA 智能体，在推理过程中迭代选择图像搜索、文本搜索或停止；训练阶段先通过拒绝采样 SFT 学习工具调用格式，再利用针对生成长度和交互深度进行归一化的强化学习目标（TN-GSPO）进行优化。

Result: 在 E-VQA 和 InfoSeek 数据集上的实验表明，ProMSA 相比于现有的强 RAG 和智能体基线取得了持续的性能提升，改善了检索质量和端到端准确率。

Conclusion: 通过引入自适应的动态多模态检索机制以及专门的强化学习训练方法，可以有效解决 KB-VQA 中静态检索的局限性。
...
### [64] [Joint Transcription and Decryption of Images of Encrypted Handwritten Documents: A Comparison with the Traditional Pipeline](https://arxiv.org/abs/2606.27700)
*Marino Oliveros-Blanco,Lei Kang,Alicia Fornés,Beáta Megyesi*

Main category: cs.CV

TL;DR: 提出一种端到端的历史加密手稿直接图像解密方法，避免转录错误累积。


<details>
  <summary>Details</summary>
Motivation: 传统的两阶段解密方法（先转录再解密）容易受到转录错误传播的影响，导致最终输出不准确。

Method: 提出“直接图像解密”方法，跳过转录阶段，并构建基于Copiale密码的合成数据生成管道进行端到端训练。

Result: 实验表明，联合图像到明文的端到端建模是传统基于转录流水线方法的可行替代方案。

Conclusion: 端到端直接解密能有效规避中间转录阶段的误差传播，为历史文献解密提供了更有效的新路径。
...
TL;DR: 本文提出了一种名为STAG的双流时空网络，通过动态区域（ROI）与动作单元（AU）耦合，有效解决了微表情识别中时空特征割裂及泛化性差的问题。


<details>
  <summary>Details</summary>
Motivation: 微表情动作微弱且短暂，现有方法过度依赖顶点-起止帧，忽视了细粒度的帧间动态，且空间和时间信息的独立建模限制了跨数据集的泛化能力。

Method: 提出STAG框架，通过幅度选择和时间注意力提取光流，利用增强图注意力网络与Transformer分别进行空间和时间建模，并采用双向交叉注意力机制进行特征融合以及AU引导的动态连接。

Result: 在CASME II、4DME、DFME等六个主流数据集上的实验表明，该方法在鲁棒性、泛化性、可解释性和计算效率上均取得了显著提升。

Conclusion: 实验证实了自适应关系推理、AU引导的动态连接以及深度时空特征融合在提升跨数据集微表情识别准确性方面的有效性。
...
### [66] [Multi-Modal Conditioned High-Resolution Transformer for Urban Electromagnetic Field Map Prediction Download PDF](https://arxiv.org/abs/2606.27671)
*Do-Eon Kim,Dongryul Park,Seungyoung Ahn,Namwoo Kang,Seong-heum Kim,Seongsin Kim*

Main category: cs.CV

TL;DR: 提出了一种基于多条件HRFormer的密集预测框架，用于高效、准确地生成城市电磁场强度分布图。


<details>
  <summary>Details</summary>
Motivation: 传统的基于物理模拟的城市电磁场强度预测计算成本高昂，难以满足快速网络规划的需求。

Method: 使用HRFormer作为骨干网络，通过FiLM和交叉注意力机制融合天线参数与辐射方向图，引入发射器相对空间通道以实现坐标一致的测试时增强（TTA），并采用由Masked L1、MS-SSIM和Focal L1组成的复合损失函数进行训练。

Result: 该模型实现了0.0461的测试平均绝对误差（MAE），相比UNet基线提升了25.2%，相比HRFormer单体基线提升了31.8%，且TTA使误差额外降低了6.3%。

Conclusion: 本研究表明，深度学习密集预测框架可以作为物理模拟器的有效替代方案，在大幅降低计算开销的同时实现高精度的城市电磁场预测。
...
### [67] [Two-Stage Cross-Domain Cervical Abnormality Screening with Cytopathological Image Synthesis and Knowledge Distillation](https://arxiv.org/abs/2606.27678)
*Jincheng Li,Yuzhi He,Yihui Zhan,Xinmei Zhang,Yifei Sun,Zelin Liu,Lichi Zhang,Minye Shao,Lili Zhao*

Main category: cs.CV

TL;DR: 提出了一种结合SC-UNSB和双层特征对齐的双阶段框架，用于提升跨域子宫颈细胞检测的性能。


<details>
  <summary>Details</summary>
Motivation: 由于不同机构之间的显著领域偏移以及疾病阶段之间微小的视觉差异，跨域子宫颈细胞病理诊断的模型泛化能力受到严重制约。

Method: 提出一个双阶段框架：第一阶段通过空间连续无配对神经薛定谔桥（SC-UNSB）构建合成中间域以缓解领域偏移；第二阶段在知识蒸馏中采用双层特征对齐策略，逐步对齐浅层结构特征和深层语义表征。

Result: 实验结果表明，该方法有效缓解了领域偏移和类别模糊，显著提升了跨域检测的性能。

Conclusion: 该研究通过构建中间域和促进域不变知识的传递，成功为跨域子宫颈细胞检测提供了一种有效的解决方案。
...
### [68] [Mind the Gap: Quantifying the Domain Gap in Cross-Sensor Diffusion Super-Resolution](https://arxiv.org/abs/2606.28039)
*Dawid Kopeć,Katarzyna Jabłońska,Wojciech Kozłowski,Maciej Zięba*

Main category: cs.CV

TL;DR: 评估了扩散超分辨率模型在卫星图像中从合成到真实跨传感器数据上的性能差距，并提出了新的评估指标。


<details>
  <summary>Details</summary>
Motivation: 高分辨率卫星图像需求增加，但由于缺乏真实的低/高分辨率配对数据，模型通常在合成退化数据上训练，导致应用于真实跨传感器图像时存在域差距。

Method: 使用对齐的Sentinel-2和PlanetScope数据集评估了五种先进的扩散超分辨率架构，并引入了基于Sentinel-2自监督特征的感知指标 LPIPS-Sat。

Result: 在合成数据上训练的模型在真实数据上性能大幅下降，而在真实跨传感器数据上直接训练的模型则面临优化困难，且难以适应物理和辐射的多样性。

Conclusion: 揭示了当前超分辨率模型的关键局限性，强调了未来研究需要将超分辨率重建与领域自适应进行解耦。
...
TL;DR: 本文提出了并行展开近似（PRA）框架，旨在解决像素空间连续自回归生成中的高维误差和训练-推理偏差问题。


<details>
  <summary>Details</summary>
Motivation: 像素空间自回归生成面临单步高维生成误差大以及教师强制训练与实际推理存在偏差导致误差累积的挑战，现有方法无法有效且高效地解决这些问题。

Method: 提出PRA框架，通过生成低维中间状态并利用像素解码器映射回像素空间，同时并行构建模拟推理阶段的反馈输入，以在保持并行训练的同时减少推理偏差。

Result: 在ImageNet-1K上，135M参数的PRA-S和511M参数的PRA-L分别实现了2.58和1.94的FID，刷新了像素空间自回归模型的SOTA，并在图像理解探测任务中表现优异。

Conclusion: PRA框架有效解决了像素空间自回归模型的关键瓶颈，展现了统一像素空间图像生成与理解的巨大潜力。
...
### [70] [Home3D 1.0: A High-Fidelity Image-to-3D Asset Generation System for Interior Design](https://arxiv.org/abs/2606.27923)
*Yiyun Fei,Guoqiu Li,Jin Song,Chuqiao Wu,Delong Wu,Hong Wu,Ziru Zeng,Haohui Chen,YinDong Kong,Jing Li,Qi Wu,Feng Zhang*

Main category: cs.CV

TL;DR: Home3D 1.0 是一个模块化的单图像生成 3D 资产系统，专为室内设计和电商设计，可生成高质量、材质可分解且部件可编辑的 3D 网格。


<details>
  <summary>Details</summary>
Motivation: 旨在解决从单张参考图像高效生成高质量、材质真实（PBR）且可编辑部件的 3D 资产的需求，以服务于室内设计和电子商务领域。

Method: 构建了包含四个模块的系统：Geometry（几何重建）、Texture（纹理生成）、Material（材质分割与 PBR 贴图检索烘焙）和 Parts（生成语义可编辑部件）。

Result: 通过对各模块的独立评估，验证了系统生成高质量 3D 资产的能力，并明确了迈向广泛应用尚存的差距。

Conclusion: Home3D 1.0 成功实现了从单图到结构化、材质可编辑 3D 资产的端到端生成，为相关行业应用提供了有效的技术方案。
...
### [71] [ZooClaw-FashionSigLIP2: Distilled Fine-tuning for Robust Fashion Retrieval](https://arxiv.org/abs/2606.27708)
*Siqiao Xue,Chunxue Xu*

Main category: cs.CV

TL;DR: 提出 ZooClaw-FashionSigLIP2 模型和新的基准测试，通过微调与权重插值解决了时尚检索中通用泛化性与领域特异性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 将通用视觉-语言模型适配到特定检索任务（如时尚检索）时，通常面临目标领域性能提升但模型通用泛化能力下降的权衡困境。

Method: 采用在精选领域数据上进行知识蒸馏的全文微调，随后与基座模型进行权重插值（WiSE-FT）。

Result: 该模型在所有基准测试中均优于包括 LoRA、更大骨干网络（达 10 亿参数）在内的所有基线；同时推出了新的 ZooClaw-Fashion 基准并分析了现有基准的结构偏差。

Conclusion: 该方法有效解决了微调中的泛化性妥协，为时尚检索提供了性能优异的开源模型和更高质量的评估基准。
...
### [72] [OSOR: One-Step Diffusion Inpainting for Effect-Aware Object Removal](https://arxiv.org/abs/2606.28094)
*Qinming Zhou,Chenxi Sun,Deyang Kong,Junhao He,Xiangheng Tang,Peike Yu,Haotian Wu,Leilei Cao,Linfeng Zhang*

Main category: cs.CV

TL;DR: 提出了OSOR，一种高效的单步图像物体擦除方法，能快速且鲁棒地移除物体并处理不精确的掩码。


<details>
  <summary>Details</summary>
Motivation: 传统的扩散模型图像擦除计算成本高、难以实时交互，且在处理阴影/反射等非局部效应和不精确的用户掩码时表现不佳。

Method: 引入了占有率引导的判别器以实现稳定的单步训练，设计了预测擦除区域的alpha head以应对不完美掩码，并提出了语义锚定验证流水线（SAVP）来构建高质量训练数据集。

Result: OSOR在感知质量上超越了强力的多步扩散基线模型，同时在推理速度上实现了4倍至30倍的提升。

Conclusion: OSOR成功实现了高效、效果感知且对掩码鲁棒的单步物体移除，极大地提升了其实用性和运行效率。
...
TL;DR: 本文提出了 PhysisForcing 框架，通过在像素级和语义级共同强化物理一致性监督，显著提升了具身视频生成模型的物理真实性及下游机器人操控性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视频生成模型在模拟物理世界时，常因运动物体形变及交互实体间的时空关联不合理，导致生成不符合物理规律的运动轨迹和交互画面。

Method: 提出 PhysisForcing 框架，结合“像素级轨迹对齐损失”（利用参考点轨迹监督 DiT 特征）和“语义级关系对齐损失”（将 DiT 特征与冻结视频编码器提取的区域间关系对齐）进行联合优化。

Result: 在多个基准测试（R-Bench 等）中显著优于强基线模型，将 Cosmos3-Nano 在 R-Bench 上的表现提升了 9.2%；并在 WorldArena 闭环规划中将成功率从 16.0% 提升至 24.0%。

Conclusion: 通过物理对齐优化视频生成模型，能为机器人操纵提供更强的表征能力，有效提升具身世界模拟器的生成质量和下游决策的成功率。
...
### [74] [BiDeMem: Bidirectional Degradation Memory for Explainable Image Restoration](https://arxiv.org/abs/2606.28112)
*Xinrui Wu,Lichen Huang*

Main category: cs.CV

TL;DR: 本文提出了BiDeMem，一种双向退化记忆模块，用于实现可解释的图像恢复。


<details>
  <summary>Details</summary>
Motivation: 现有的退化感知方法主要依赖PSNR作为单一评估指标，缺乏语义可解释性，容易通过数据捷径而非真正的退化先验来提升性能。

Method: 提出双向退化记忆（BiDeMem）机制，利用图像特征与输入统计量构建查询并检索记忆槽，在推理时辅助图像恢复，在训练时支持前向退化解释。

Result: 在多退化控制变量实验中，BiDeMem优于仅纠正头、密集查询先验等变体（性能提升0.25-0.28 dB），且干预探针显示其能有效提升对退化先验的敏感性。

Conclusion: BiDeMem成功地将退化记忆构建为兼具高性能恢复与可证伪解释能力的双向机制。
...
### [75] [Scene and Human in One World: Reconstruction in a Feedforward Pass](https://arxiv.org/abs/2606.27720)
*Boao Shi,Qiao Feng,Yiming Huang,Lingjie Liu*

Main category: cs.CV

TL;DR: 提出SHOW框架，通过在统一的公制空间中耦合人体网格恢复与3D场景重建，实现了单目视频中高精度、尺度的动态人-景重建。


<details>
  <summary>Details</summary>
Motivation: 单目相机下的动态人-景重建面临尺度模糊、人景不匹配和遮挡干扰等挑战，且现有方法通常将人体和场景重建视为独立任务。

Method: 提出SHOW框架：在统一公制空间中融合人体模型先验与场景几何信息；利用场景几何约束人体网格估计；引入可提示的掩码机制以处理复杂多人物和遮挡场景。

Result: 实验表明，该方法在相机运动、遮挡和杂乱背景等挑战性场景下，显著提升了公制尺度一致性、人景对齐度及重建精度。

Conclusion: SHOW通过联合训练和人-景互导机制，有效解决了单目以人为中心视频中的尺度假定和对齐问题，实现了高质量的协同重建。
...
### [76] [Learning 1-Bit LiDAR-based Localization with Auxiliary Objective](https://arxiv.org/abs/2606.27729)
*Kaijie Yin,Zhiyuan Zhang,Tian Gao,Wentao Zhu,Cheng-zhong Xu,Hui Kong*

Main category: cs.CV

TL;DR: 提出了BiLoc，这是首个用于6-DoF激光雷达定位的双值神经网络（BNN）框架，通过信息瓶颈原理和辅助目标函数显著缓解了二值化带来的信息损失。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统中的激光雷达定位模块计算资源有限，现有的深度学习方法过于庞大，而现有的双值神经网络（BNN）虽然计算轻量，但面临严重的信息丢失和性能下降。

Method: 提出BiLoc框架。从信息瓶颈原理出发重新解释BNN训练，以保留最少但充足的位姿特征；同时引入辅助目标函数，自适应调节二值编码器中的信息保留，从而补偿BNN的表示能力限制和梯度不匹配问题。

Result: 在室外大型激光雷达数据集上的实验表明，BiLoc在基于BNN的激光雷达定位中达到了新的最先进（SOTA）性能。

Conclusion: BiLoc成功证明了双值神经网络在轻量化激光雷达定位任务中的可行性，在大幅降低计算开销的同时有效缓解了性能损失。
...
### [77] [MASS: Motion-Aligned Selective Scan for Refinement in Flow-Based Video Frame Interpolation](https://arxiv.org/abs/2606.27718)
*Jun-Sang Yoo,Seung-Won Jung*

Main category: cs.CV

TL;DR: 本文提出了一种名为MASS的视频插帧框架，将选择性状态空间模型（SSM）的特征扫描从静态网格转为动态运动轨迹，以解决大运动和复杂遮挡下的插帧难题。


<details>
  <summary>Details</summary>
Motivation: 传统的视频插帧方法难以处理模糊的对应关系，且现有的SSM方法受限于静态网格扫描，无法很好地匹配真实的物理运动。

Method: 提出运动对齐选择性扫描（MASS），通过可学习的非线性路径积分逼近弯曲运动轨迹，并利用速度感知SSM动态调整采样步长，最后通过聚合状态引导端到端细化。

Result: 在标准基准测试中取得了极具竞争力的性能，尤其是在大位移和复杂动态场景下达到了最先进的（SOTA）效果。

Conclusion: 研究表明，将SSM的扫描机制从静态空间网格重构为动态运动轨迹，能够显著提升视频插帧在处理复杂及大运动场景时的效果。
...
### [78] [Toward Robust In-Context Segmentation via Concept Guidance](https://arxiv.org/abs/2606.28149)
*Zhigang Chen,Xiawu Zheng,Rongrong Ji*

Main category: cs.CV

TL;DR: 本文提出了CG-ICS框架，通过融合高层语义概念与视觉示例，显著提升了上下文分割（ICS）的鲁棒性与准确性。


<details>
  <summary>Details</summary>
Motivation: 现有上下文分割方法过度依赖低级视觉匹配，忽视了系统的鲁棒性，即模型在面对不同参考图像时输出结果不够稳定。

Method: 引入概念引导的上下文分割（CG-ICS）。使用MLLM提出候选概念，结合SAM3评分函数与树搜索筛选可靠文本概念，配合并行视觉示例路径进行空间定位，共同激活冻结的SAM3进行分割。

Result: 在标准ICS基准测试中，CG-ICS不仅取得了最先进的准确率，还大幅降低了因参考图像变化带来的预测方差，显著提升了系统鲁棒性。

Conclusion: 结合高层语义概念可以有效克服ICS的鲁棒性瓶颈，该方法为实现更稳定可靠的上下文分割提供了新方向。
...
TL;DR: HAT-4D结合人机协同与VLM，首次实现了从单目视频中高质量重建多物体4D物理交互，达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 现有的单目4D重建方法局限于孤立物体，在处理多物体交互的遮挡和复杂动力学时表现不佳，阻碍了具身智能和VLA的数据收集与缩放。

Method: 提出HAT-4D框架，将视觉语言模型（VLM）与多级人机协同反馈机制结合以解决深度歧义和遮挡问题，并构建了MVOIK-4D基准和多维度物理一致性评估协议。

Result: 在多项评估指标上达到SOTA，消融实验证实引入少量人类反馈能显著改善交互重建，且生成的数据能有效提升基线模型的微调性能。

Conclusion: HAT-4D是一个高效且可扩展的单目视频4D交互重建数据引擎，为具身智能的数据规模化提供了可行且低成本的途径。
...
### [80] [SIFT: Self-Imagination Fine-Tuning for Physically Plausible Motion in Video Diffusion Models](https://arxiv.org/abs/2606.27741)
*Ruoyu Wang,Jialun Liu,Huayang Huang,Haibin Huang,Jiepeng Wang,Chi Zhang,Xuelong Li,Yu Wu*

Main category: cs.CV

TL;DR: 提出SIFT微调范式，通过自生成视频训练解决视频扩散模型中的“运动纠缠”与物理失真问题。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型生成的视频常违反物理规律，出现相机与物体运动耦合的“运动纠缠”现象，这源于数据偏差和重构式训练设计。

Method: 提出自想象微调（SIFT）范式，让模型学习自身生成的视频以打破重构捷径，并辅以运动感知判别监督和渐进式难例重放。

Result: 实验表明，该方法拓宽了运动空间覆盖，显著提高了生成视频的物理真实性、运动解耦性及可控性。

Conclusion: 通过摆脱传统的重构训练，SIFT能有效改善视频生成中的运动质量和物理合理性。
...
### [81] [Panoramic Scene Analysis: A Survey from Distortion-Aware Engineering to Sphere-Native Foundation Modeling](https://arxiv.org/abs/2606.27745)
*Qinfeng Zhu,Lei Fan*

Main category: cs.CV

TL;DR: 本文是对全景场景分析领域的系统性综述，梳理了其从传统投影适配到几何感知基础模型的研究脉络，并指出了当前评估协议的缺陷与未来方向。


<details>
  <summary>Details</summary>
Motivation: 全景图像的球形几何畸变破坏了标准视觉架构的假设，且现有方法在实现严格的球形等变性与完全复用透视预训练权重之间存在结构性冲突。

Method: 从“架构设计”（算子与球形几何的交互）和“训练范式”（跨领域知识迁移）两个正交维度对现有文献进行分类与系统性梳理。

Result: 指出当前方法无法同时兼顾球形等变性与预训练权重复用，并揭示了现有评估协议在极地鲁棒性、接缝一致性等方面的五个系统性缺陷。

Conclusion: 提出了面向通用全景智能的六点研究路线图，并提供了开源资源库以促进该领域的发展。
...
TL;DR: 提出DEFAR框架，通过利用曝光偏差自身的方向和频率反馈信号，来纠正流匹配模型在推理过程中的偏差。


<details>
  <summary>Details</summary>
Motivation: 流匹配模型存在训练与推理不一致导致的曝光偏差，而现有方法依赖静态约束，未利用偏差自身包含的动态纠偏信号。

Method: 引入DEFAR框架，包含抗漂移整流（ADR）以学习纠偏方向，以及频率补偿（FC）以利用偏差自反馈强化缺失的低频成分。

Result: 在CIFAR-10、CelebA和ImageNet等多个数据集上的实验表明，DEFAR性能优于现有基线，且具良好的扩展性与鲁棒性。

Conclusion: 通过模拟推理并利用偏差自身的定向和频率自适应信号，可以有效提升流匹配模型的自纠正能力和生成质量。
...
### [83] [Learning Topology-Aware Representations via Test-Time Adaptation for Anomaly Segmentation](https://arxiv.org/abs/2606.28268)
*Ali Zia,Usman Ali,Abdul Rehman,Umer Ramzan,Kang Han,Muhammad Faheem,Shahnawaz Qureshi,Wei Xiang*

Main category: cs.CV

TL;DR: 本文提出了一种名为TopoTTA的测试时自适应框架，通过引入拓扑数据分析来提升异常分割的几何与结构相干性。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时自适应方法在异常分割中过于依赖像素级启发式规则，忽略了复杂的空间几何关系，且难以在噪声和纹理变化下保持结构一致性。

Method: 利用持续同调（Persistent Homology）对异常评分图应用多级立方复形过滤，从而推导出鲁棒的拓扑伪标签，以指导轻量级测试时分类器进行自适应，而无需重新训练骨干网络。

Result: 在六个标准基准数据集上的实验表明，该方法相比最先进的无监督异常检测方法，F1分数平均提升了15%，尤其在处理复杂几何结构的异常时提升最显著。

Conclusion: 将拓扑推理融入测试时自适应，为实现结构感知泛化提供了一种有效路径，成功连接了几何学习与鲁棒自适应。
...
### [84] [An Embedded Real-Time License Plate Recognition System for Complex Traffic Scenes](https://arxiv.org/abs/2606.27772)
...
TL;DR: 提出了一种适用于发展中国家复杂交通场景、基于FPGA加速的轻量级嵌入式实时车牌识别系统及SL-LPR数据集。


<details>
  <summary>Details</summary>
Motivation: 旨在解决发展中国家复杂、非结构化交通场景下，如何在低成本和资源受限的嵌入式平台上实现实时的车牌识别。

Method: 采用轻量级CNN进行车牌检测与字符识别，引入SL-LPR数据集，并利用Brevitas进行低比特量化，通过FINN框架在FPGA（Xilinx Kria KV260）上实现加速。

Result: 模型检测mAP达到93.6%，识别准确率达87.88%，在嵌入式KV260平台上端到端运行速度可达11.5 FPS。

Conclusion: 研究证明了所提系统在资源受限的嵌入式设备上针对复杂交通场景实现实时、高效车牌识别的有效性。
...
### [85] [PixelU: A U-Shaped Transformer for Efficient End-to-End Pixel Diffusion](https://arxiv.org/abs/2606.27760)
*Zipeng Guo,Lichen Ma,Yu He,Xiaolong Fu,Jingling Fu,Junshi Huang,Yan Li*

Main category: cs.CV

TL;DR: 本文提出了 PixelU，一个专为像素空间设计的极简单阶段 U 型扩散 Transformer，通过去除复杂的辅助解码器，实现了高效的高质量图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有的像素空间扩散模型难以同时建模高频细节和低频语义，且过度依赖复杂的像素解码器来解决 $v$-prediction 带来的优化难题。

Method: 采用 $x$-prediction 范式，舍弃辅助解码器，利用无成本的跳跃连接直接传递高频空间细节；同时引入恒定通道空间下采样机制，使主干网络专注于建模低频语义。

Result: 在 ImageNet 256×256 和 512×512 数据集上分别取得了 1.63 和 1.92 的 FID 成绩，且计算成本仅为基线模型 JiT-G 的约 1/3。

Conclusion: PixelU 证明了通过解耦频率和简化结构可以有效提升像素空间扩散模型的效率与效果，为端到端扩散模型提供了一种简单而强大的新范式。
...
### [86] [ModaFlow: Modality-Aware Flow Matching for High-Fidelity Virtual Try-On](https://arxiv.org/abs/2606.27773)
*Xiangyu Sai,Meysam Madadi,Sergio Escalera,Yong Xu*

Main category: cs.CV

TL;DR: ModaFlow是一个基于模态感知流匹配的虚拟试衣框架，通过结合视觉与文本引导，实现了高保真度的衣物与人体对齐。


<details>
  <summary>Details</summary>
Motivation: 现有方法在面对衣物与身体的大幅度变形时，难以同时保留精细的衣物语义并适应多样的人体几何结构。

Method: 提出ModaFlow框架，采用模态感知引导方案（分离视觉与文本嵌入处理）、两种新型正则化损失（余弦相似度与感知流判别），以及用于模拟遮挡的掩码操纵策略。

Result: 在配对和非配对基准测试中，FID指标分别降低了约30%和20%，取得了最先进的定性和定量评估结果。

Conclusion: ModaFlow通过优化多模态引导和流场准确性，有效提升了虚拟试衣的语义保真度、变形适应性以及在无配对设定下的推理鲁棒性。
...
### [87] [MindFlow: Harmonizing Cognitive Semantics and Acoustic Dynamics for Facial Animation Generation in Dyadic Conversations](https://arxiv.org/abs/2606.27779)
*Hejia Chen,Haoxian Zhang,Xu He,Xiaoqiang Liu,Pengfei Wan,Shoulong Zhang,Shuai Li*

Main category: cs.CV

TL;DR: 本文提出了 MindFlow，一个双通路的生成框架，用于在双人对话中生成兼具深层语义理解和精细运动控制的逼真面部动画。


<details>
  <summary>Details</summary>
Motivation: 现有的双人对话面部动画生成方法在对话上下文的语义理解和精确的动态控制上面临挑战，难以协调高层认知意图与底层运动反射。

Method: 提出了受神经科学启发的 MindFlow 框架。其腹侧模块采用 Chunk-State 方法将音频建模为情感状态链以捕捉语义细节；背侧模块利用条件自回归流匹配网络和选择性声音注入器来生成高保真面部动作。

Result: 实验结果表明，MindFlow 在语义合理性和动作自然度上均显著优于现有的最先进基线方法。

Conclusion: 通过将深度语义推理与精细控制解耦并协同生成，该双通路框架为双人对话面部动画提供了一种更有效且鲁棒的解决方案。
...
### [88] [TRUST: Efficient Abdominal Trauma Recognition via Image-to-Ultrasound-Video Transfer Learning](https://arxiv.org/abs/2606.27777)
*Enguang Wang,Hao Zhou,Shuo Gao,Tuo Liu,Guangquan Zhou*

Main category: cs.CV

TL;DR: 本文提出了一种名为TRUST的扫描感知参数高效图像到视频迁移学习框架，用于高效、可靠的腹部超声视频分析。


<details>
  <summary>Details</summary>
Motivation: 超声检查受医生操作影响存在显著的时空和语义差异，且易受斑点噪声干扰，导致现有视频理解方法效果受限。

Method: 提出了包含跨频协同适配器（CFCA）、多粒度运动感知模块（MGMA）和视觉查询语义聚合模块（VQSA）的TRUST框架，从频域、运动动态和视觉-文本对齐三方面进行优化。

Result: 在超声创伤数据集上的实验表明，TRUST的性能超越现有最先进方法9.63%，且具备更高的计算效率。

Conclusion: TRUST框架通过显式建模细粒度的时空变化，为临床超声视频的可靠理解与泛化提供了有效的解决方案。
...
### [89] [Text as Illumination: Spatial Contrastive Retinex Learning for Language-guided Medical Image Segmentation](https://arxiv.org/abs/2606.27794)
*Jian Shi,Cheng Zhen,Pingping Zhang,Rui Xu,Yanan Lv,Yili Ma,Huan Bi,Haojie Li,Huchuan Lu*

Main category: cs.CV

TL;DR: 本文提出了TIRNet，一种受Retinex理论启发的新型网络，通过将文本嵌入视为语义光照来增强语言引导医学图像分割（LMIS）中的语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有LMIS方法依赖隐式交互或粗粒度监督，缺乏显式和细粒度的约束，导致临床文本与分割输出之间存在语义不匹配的问题。

Method: 提出了TIRNet框架，包含两个核心模块：Retinex启发文本调制模块（RTMB）用于增强前景并抑制背景，一致性细节补偿模块（CDCB）用于恢复高频细节；并设计了多尺度光照监督损失（MSIS-Loss）以实现精确的跨模态对齐。

Result: 在MosMedData+和QaTa-COV19数据集上的广泛实验表明，TIRNet取得了最先进（SOTA）的分割性能。

Conclusion: TIRNet通过将文本转化为语义光照并引入多尺度光照监督，成功解决了跨模态对齐中的语义不一致问题，显著提升了医学图像分割的准确性。
...
### [90] [Scalable and Differentiable Point-Cloud Registration Using Maximum Mean Discrepancy](https://arxiv.org/abs/2606.27818)
*Rixon Crane,Fahira Afzal Maken,Nicholas Lawrance,Stanislav Funiak,Kasra Khosoussi,Ming Xu,Russell Tsuchida*

Main category: cs.CV

TL;DR: 提出MMD-Reg，一种基于最大均值差异（MMD）的无对应关系、可微且具有线性计算复杂度的点云配准方法。


<details>
  <summary>Details</summary>
Motivation: 解决传统点云配准方法在初始对齐差、部分重叠等复杂工况下计算复杂度高，且难以作为可微模块融入端到端深度学习模型的问题。

Method: 将配准建模为基于随机傅里叶特征近似MMD的非线性最小二乘问题，采用Levenberg-Marquardt算法求解，并通过隐函数定理实现解的可微性，使其可作为优化层嵌入神经网络（如Set Transformer）。

Result: 实验表明，Neural MMD-Reg在监督和无监督训练下均优于近期学习型方法，且独立版MMD-Reg在精度和可扩展性上超越了传统的非学习型配准方法。

Conclusion: MMD-Reg是一种高效、可微且易于扩展的点云配准方法，能够有效结合传统优化与端到端深度学习。
...
### [91] [Video-MME-Logical: A Controlled Diagnostic Benchmark for Video Temporal-Logical Reasoning](https://arxiv.org/abs/2606.27828)
*Hohin Kwan,Hongyu Li,Ray Zhang,Manyuan Zhang,Xianghao Kong,Anyi Rao,Jiahao Xie,Si Liu*

Main category: cs.CV

TL;DR: 本文提出了 Video-MME-Logical，这是一个专为评估多模态大语言模型（MLLMs）视频时序逻辑推理能力而设计的受控基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有的视频基准测试常将时序逻辑推理能力与场景复杂度或静态识别混淆，无法有效孤立并评估模型在动态视觉变化中的推理能力。

Method: 设计了包含5种时序逻辑操作和25个细粒度任务的基准，支持难度控制和中间状态诊断，通过改变时间跨度和推理复杂度来评估模型。

Result: 实验显示现有主流多模态大模型与人类之间存在巨大差距，且即使使用50万样本进行监督微调，也无法完全消除这一推理差距。

Conclusion: Video-MME-Logical 是一个可扩展的测试平台，为分析和提升多模态大模型在时序逻辑推理方面的能力提供了重要工具。
...
### [92] [CSD: Content-aware Speculative Decoding for Efficient Image Generation](https://arxiv.org/abs/2606.27829)
*Mingcheng Wang,Junbo Qiao,Yunchen Li,Lingfu Jiang,Wei Li,Jie Hu,Jiao Xie,Zhou Yu,Xinghao Chen,Guixu Zhang,Shaohui Lin*

Main category: cs.CV

TL;DR: 本文提出了一种内容感知投机解码（CSD）算法，旨在加速自回归图像生成模型的推理速度并保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 在图像生成领域，传统投机解码算法面临接受率低的问题，而直接放宽接受条件又会导致图像质量退化。

Method: 引入基于熵的概率松弛机制和最优重采样策略，动态调整不同区域的Token接受概率，并结合分布对齐滤波器以确保输出分布与目标模型一致。

Result: 在Lumina-mGPT和Janus-Pro上的实验结果表明，该方法在显著提升推理效率的同时，有效保证了图像的生成质量。

Conclusion: CSD通过利用图像不同区域的信息不确定性，成功实现了自回归图像生成加速与生成质量之间的平衡。
...
### [93] [ScaLe-INR: Scale and Learn Implicit Neural Representations](https://arxiv.org/abs/2606.27862)
...
TL;DR: 提出ScaLe-INR，一种通过多分支架构和方向坐标缩放解决隐式神经表示（INR）中谱偏差和信息交叉干扰的新方法。


<details>
  <summary>Details</summary>
Motivation: 隐式神经表示（INR）在捕捉多尺度现象时存在谱偏差和信息交叉干扰，高频权重更新会破坏低频结构的近似。

Method: 引入ScaLe-INR多分支架构，利用傅里叶逆缩放定理进行方向坐标缩放以扩展带宽，并提出方向边缘引导损失（Directional Edge Guidance Loss）以实现分支间的功能解耦。

Result: 在多项任务中超越现有SOTA方法，其中图像重建提升5.16 dB，图像去噪提升0.65 dB，音频重建达到50.02 dB，3D重建达到0.999 IoU。

Conclusion: ScaLe-INR成功消除了谱交叉干扰，加速了收敛，并在复杂的多尺度拓扑上实现了高保真的信号重建。
...
TL;DR: 提出了OrthoTryOn框架，通过正交子空间投影和Fisher引导的负向引导，解决了统一服装生成任务中的任务冲突与负迁移问题。


<details>
  <summary>Details</summary>
Motivation: 在统一服装生成（如虚拟试衣与服装重建）中，朴素的参数共享会导致严重的任务间梯度冲突和性能下降。

Method: 引入正交子空间投影（OSP）将任务特征映射到去相关的坐标系，并使用Fisher引导的负向引导（FNG）在推理时排斥易混淆任务的生成轨迹。

Result: 该方法克服了统一训练的性能退化，表现超越了独立训练的专有模型，在多个基准上达到SOTA且具有良好的泛化性。

Conclusion: OrthoTryOn有效缓解了多任务扩散模型中的语义干扰，实现了高效且高质量的统一服装生成。
...
### [95] [A Multi-Attribute Latent Space for Visual Analysis of Watches](https://arxiv.org/abs/2606.27897)
*Kai Lawonn,Tobias Günther,Monique Meuschke*

Main category: cs.CV

TL;DR: 本文介绍了一个用于探索大型手表收藏的交互式可视化分析系统，该系统结合了视觉与语义属性来辅助用户进行开放式探索。


<details>
  <summary>Details</summary>
Motivation: 传统的目录和电商界面仅支持元数据过滤，无法有效支持用户对视觉相似性、设计风格以及美学与功能混合标准的探索。

Method: 利用U-Net分割表盘，Vision Transformer预测手表类型，CIELAB表征颜色，梯度描述符提取表盘结构，并通过扩展的UMAP算法融合多属性 neighborhood graphs，最终构建了包含空间导航和示例检索的交互式界面。

Result: 通过运行时间测试和用户定性评估，证实该系统能有效支持手表的发现与对比，但也暴露出在可扩展性和搜索验证方面的一些局限性。

Conclusion: 该研究为多属性潜在空间可视化提供了有效的设计方案，并为异构视觉集合的探索推导出了具体的设计启示。
...
### [96] [Long-Term Prediction of Local and Global Human Motion with Occlusion Recovery](https://arxiv.org/abs/2606.27900)
*Qiaoyue Yang,Sven Heutger,Christopher Niemann,Magnus Jung,Ayoub Al-Hamadi,Sven Wachsmuth*

Main category: cs.CV

TL;DR: 提出一种基于时空注意力的非自回归Transformer模型，能够同时预测局部人体姿态和全局空间运动，并具备处理关节缺失和变长输入的能力。


<details>
  <summary>Details</summary>
Motivation: 传统的自回归人体运动预测方法容易随时间累积误差，且通常仅局限于局部姿态预测，忽略了全局运动。

Method: 采用非自回归的Transformer架构，利用时空注意力机制进行局部与全局运动的多任务训练，并设计了缺失关节修复和变长历史输入处理机制以增强实用性。

Result: 成功实现了一个能克服误差累积、同时预测局部姿态和全局轨迹的模型，并且在应对关节遮挡和不同长度历史输入时表现出更强的鲁棒性。

Conclusion: 非自回归时空Transformer能有效解决传统方法的误差累积问题，为真实场景下的局部与全局人体运动预测提供了可行的解决方案。
...
### [97] [There and Back Again: A Flexible-Frame Transformer for Multi-Exposure Fusion](https://arxiv.org/abs/2606.27905)
*Lishen Qu,Yao Liu,Shihao Zhou,Jie Liang,Hui Zeng,Lei Zhang,Jufeng Yang*

Main category: cs.CV

TL;DR: 本文提出了FreeMEF，这是首个用于多曝光融合（MEF）的柔性帧Transformer，能够在不改变架构或重新训练的情况下处理任意数量的输入图像。


<details>
  <summary>Details</summary>
Motivation: 传统的MEF方法仅适用于固定数量的输入帧，导致在处理不同帧数时需要维护多个模型，降低了实际部署的效率。

Method: 提出了包含两个核心模块的FreeMEF：1) 循环状态空间模块（RSSM），用于顺序融合任意长度序列的特征；2) 全局特征引导块（GFGB），结合极值感知混合注意力和仿射注入前馈网络，优化对比度和亮度。

Result: 在三个基准数据集上的实验结果表明，该方法在定量和定性指标上均优于现有的先进（SOTA）方法。

Conclusion: FreeMEF成功实现了灵活的、不限帧数的多曝光图像融合，显著提升了MEF模型在实际应用中的部署效率和适应性。
...
### [98] [A Unified Framework for Vision Transformers Equivariant to Discrete Subgroups of $\mathrm{O}(2)$](https://arxiv.org/abs/2606.27864)
*Tīkun Ông,Georg Bökman*

Main category: cs.CV

TL;DR: 本文提出了一种等变于 $\mathrm{O}(2)$ 任意离散子群的视觉 Transformer（ViT）框架，并在数据稀缺场景下实现了更高的识别准确率。


<details>
  <summary>Details</summary>
Motivation: 标准的视觉 Transformer 模型缺乏对平面对称性（如旋转和翻转）的显式编码能力。

Method: 构建了等变于 $\mathrm{O}(2)$ 任意离散子群的 ViT 核心组件，提供了理论表达能力保证，并设计了基于六边形分块的 $D_6$ 等变模型。

Result: 在 PatternNet 航拍图像数据集的测试中，等变模型在参数量相同且数据稀缺的情况下取得了比普通模型更好的识别准确率。

Conclusion: 等变性能够有效改善 Transformer 模型的视觉识别性能，证明了在视觉模型中引入离散对称群的价值。
...
### [99] [Controllable Histopathology Image Synthesis with Training-free Structural Initialization and Textural Modulation](https://arxiv.org/abs/2606.27935)
*Yuheng Qiu,Jingyi Luo,Chenfei Ye,Ting Ma,Jianfeng Cao*

Main category: cs.CV

TL;DR: 本文提出了一种名为CHIS的即插即用框架，可在无需标注训练的情况下，引导预训练扩散模型生成符合结构掩码且保留参考纹理风格的组织病理学图像。


<details>
  <summary>Details</summary>
Motivation: 深度学习在病理图像分析中受限于高昂的专家标注成本，而现有的图像合成方法在训练阶段仍需要一定数量的标注数据来施加结构约束。

Method: 提出CHIS框架，包含两个阶段：一是结构初始化阶段，在频域中融合先验掩码的相位与高斯噪声的振幅；二是纹理调制阶段，在反向扩散过程中通过小波分解自适应调节不同层级的粗细粒度纹理。

Result: 实验结果表明，CHIS在图像生成保真度上具有显著优越性，并能大幅提升下游细胞分割任务的性能。

Conclusion: CHIS成功实现了仅利用无标注图像预训练的扩散模型，生成结构对齐且纹理逼真的病理图像，有效缓解了数据稀缺和标注负担问题。
...
### [100] [Understanding How MLLMs Describe Artworks Using Token Activation Maps](https://arxiv.org/abs/2606.27947)
*Nicola Fanelli,Pasquale De Marinis,Raffaele Scaringi,Eva Cetinic,Gennaro Vessio,Giovanna Castellano*

Main category: cs.CV

TL;DR: 本文评估了多模态大语言模型在描述艺术品时的视觉定位能力，揭示了其视觉推理的透明度与局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型能流畅描述艺术品，但其背后的视觉推理过程仍不透明，无法确定其生成内容是基于特定画布区域还是文本先验。

Method: 利用标记激活图（TAM）为生成的标记生成热力图以隔离视觉证据，分析了五类语义标记的定位模式，并与开放词汇分割方法进行了对比。

Result: 研究发现视觉定位因标记语义而异；模型在艺术家归因上的准确率显著高于标题预测，且后者更易出现幻觉。

Conclusion: 该研究揭示了模型在艺术理解中的视觉定位规律与局限，并开源了代码和实验配置以促进后续研究。
...
### [101] [Latent Visual Diffusion Reasoning with Monte Carlo Tree Search](https://arxiv.org/abs/2606.27988)
*Xirui Teng,Nan Xi,Junsong Yuan*

Main category: cs.CV

TL;DR: 提出了LVDR框架，通过结合关键点引导的蒙特卡洛树搜索，实现高精度且具可解释性的细粒度动作质量评估。


<details>
  <summary>Details</summary>
Motivation: 现有的动作质量评估模型大多是黑盒，缺乏显式呈现其决策背后视觉推理过程的能力。

Method: 提出潜视觉扩散推理（LVDR）框架，融合关键点引导的蒙特卡洛树搜索（MCTS）来模拟和可视化潜在的视觉推理过程。

Result: 在体育和手术领域的四个数据集上取得了有竞争力的定量性能，并生成了指向最终预测的可解释视觉推理轨迹。

Conclusion: LVDR在提升技能评估准确性的同时，成功揭示了决策背后的关键推理序列，增强了模型的可解释性。
...
### [102] [Directing the World: Fast Autoregressive Video Generation with Compositional Human-Camera Control](https://arxiv.org/abs/2606.27964)
*Haoyuan Wang,Yabo Chen,Haibin Huang,Chi Zhang,Xuelong Li*

Main category: cs.CV

TL;DR: 本文提出“Directing the World”框架，通过解耦人体动作与相机轨迹控制，实现稳定且高质量的长视频自回归生成。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归视频生成在长时域生成中易积累误差，且在结合人类动作和相机轨迹等异构控制时，容易破坏视频先验，导致可控性与视觉质量失衡。

Method: 提出快慢记忆训练策略以稳定长视频学习；设计t引导动态投影与运动无分类器引导（Motion-CFG）进行精确的动作对齐；采用两阶段模块解耦动作与相机控制，并构建了高质量的多模态数据集。

Result: 实验表明，该方法能够实现稳定的长视频生成，具备高精度可控性、优异的视觉质量，并支持多角色动作与相机视角的协同控制。

Conclusion: 解耦控制学习与针对性的训练策略能有效克服自回归视频生成在复杂控制下的退化问题，为交互式世界模型的构建提供了新方案。
...
### [103] [Curriculum-guided Change Detection Training: Toward Accurate Serac Fall Monitoring](https://arxiv.org/abs/2606.28012)
*Arthur Dérédel,Carlos Crispim-Junior,Pierre Lemaire,Johan Berthet,Laure Tougne Rodet*

Main category: cs.CV

TL;DR: 本文提出了一种用于变化检测的课程学习框架，通过由易到难地输入训练样本来提高模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的变化检测方法大多采用均匀采样进行训练，忽视了样本难易度的差异，容易引入噪声梯度并限制模型性能。

Method: 设计了两种难度度量指标：反映成像条件差异的太阳角度差（SAG）和反映外观相似度的结构相似性指数（SSIM），据此在训练中逐步引入困难样本。

Result: 在SeracFallDet基准上的实验表明，该方法在像素级和对象级变化检测上均优于传统的均匀采样策略。

Conclusion: 课程学习能显著提升变化检测模型的鲁棒性，且该框架具有即插即用的通用性，可推广至多种现有架构。
...
### [104] [TempAct: Advancing Temporal Plausibility in Autoregressive Video Generation via Planner-Executor RL](https://arxiv.org/abs/2606.28016)
*Jing Wang,Xiangxin Zhou,Jiajun Liang,Kaiqi Liu,Wanyun Pang,Zhenyu Xie,Tianyu Pang,Xiaodan Liang*

Main category: cs.CV

TL;DR: 本文提出了 TempAct，一个结合了规划器与执行器的层次化强化学习框架，用以解决自回归视频生成中时间指令遵循不准确的问题。


<details>
  <summary>Details</summary>
Motivation: 传统的自回归视频扩散模型在处理逐步指令时，容易出现反应延迟、语义混淆和误差传播，且现有的监督微调和蒸馏方法无法直接优化动作顺序与提示词转换的正确性。

Method: 引入基于强化学习的 TempAct 框架，利用 LLM 规划器探索可执行的分段步骤提示，并设计分层小组探索与分层奖励机制（结合全局规划反馈和局部步骤遵循奖励），共同优化规划与自回归扩散执行。

Result: 在 Self-Forcing 和 LongLive 基准上的实验结果表明，TempAct 在保持高视频视觉质量的前提下，显著提升了视频生成的时间一致性和时序指令遵循能力。

Conclusion: 通过将自回归视频生成建模为规划与执行的层次化强化学习问题，能够有效缓解时序指令错位并提升长视频生成的时空合理性。
...
### [105] [HumanMoveVQA: Can Video MLLMs reason about human movement in videos?](https://arxiv.org/abs/2606.27999)
*Pulkit Gera,Faegheh Sardari,Asmar Nadeem,Valentina Bono,Padraig Boulton,Adrian Hilton,Armin Mustafa*

Main category: cs.CV

TL;DR: 本文提出了首个用于评估和提升多模态大模型对人类全局三维运动轨迹和朝向理解能力的基准测试 HumanMoveVQA。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大模型在视频理解中无法有效捕捉人类在空间和时间上的全局运动信息（如轨迹和朝向变化），且缺乏相应的评估基准。

Method: 构建了基于首帧锚定世界坐标系的 HumanMoveVQA 基准，通过多阶段管线将 2D 视频转化为 3D 一致运动轨迹，生成了超 1 万个结构化问答对。

Result: 评估表明主流前沿模型在深度运动理解上存在明显短板，但通过使用本文提出的世界一致性监督数据微调开源模型，可以显著提升其性能。

Conclusion: HumanMoveVQA 为开发下一代具备运动感知能力的视频理解模型奠定了严谨的几何学基础。
...
### [106] [EMOSH: Expressive Motion and Shape Disentanglement for Human Animation](https://arxiv.org/abs/2606.28026)
*Dongbin Zhang,Hao Liu,Binquan Dai,Kangjie Chen,Chuming Wang,Chen Li,Jing Lyu,Haoqian Wang*

Main category: cs.CV

TL;DR: 本文提出了一种名为 EMOSH 的新型高保真可控人体动画生成框架，解决了表现力与几何解耦之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 现有方法在表现力与解耦性之间存在权衡：2D姿态控制方法容易导致驱动源的体型泄露，而基于3D先验的方法则难以捕获细腻的面部表情和复杂手势。

Method: 引入显式解耦形状和姿态的表达性人体模型（EHM）及运动追踪器，提出粗到细混合运动注入策略以精细控制表情和手势，并使用空间对齐条件机制提升身份一致性。

Result: 实验表明，EMOSH 在自驱动和跨驱动场景中均优于先前方法，能够生成具有生动表情的高保真视频，同时有效防止体型泄露。

Conclusion: EMOSH 成功克服了现有可控人体视频生成中动作与体型纠缠以及表情缺失的局限性，为高保真数字人驱动提供了有效方案。
...
### [107] [RPM-Distill: Physiology-guided Adaptive Cross-modal Distillation for Robust Remote Physiological Measurement](https://arxiv.org/abs/2606.28089)
*Jiyao Wang,Qingyong Hu,Duoxun Tang,Xiao Yang,Kaishun Wu,Jiangbo Yu*

Main category: cs.CV

TL;DR: 提出RPM-Distill框架，在训练阶段利用雷达信号引导视频生理测量模型，实现更鲁棒的纯视频生理指标估算。


<details>
  <summary>Details</summary>
Motivation: 视频生理测量易受光照和运动干扰，而鲁棒的雷达设备在实际推理中难以普及部署，因此需要一种仅在训练时利用雷达辅助的方案。

Method: 利用雷达与视频在频域的共性，进行生理结构化频谱蒸馏（包括锚定峰值、匹配背景及保持频谱形态），并引入频谱策略网络动态调整蒸馏权重以防止负迁移。

Result: 在复杂场景和跨数据集测试中，该方法相比单模态基线实现了81%的MAE降低和21%的相关性提升。

Conclusion: 通过跨模态频域蒸馏，在无需增加推理成本的前提下，显著增强了视频生理测量的鲁棒性。
...
### [108] [ReScene: Structured Indoor Scene Reconstruction from Multi-View Captures](https://arxiv.org/abs/2606.28060)
*Haoran Xu,Lechao Zhang,Daoguo Dong,Yan Gao,Xin Tan*

Main category: cs.CV

TL;DR: 本文提出了ReScene框架，通过多视图几何先验构建物理一致且适用于仿真环境的3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有的多视图3D场景重建方法存在单视角偏差、物理不一致以及依赖特定硬件等问题，关键瓶颈在于跨视图关系融合与物理合理的场景组装。

Method: 提出了包含两个核心组件的ReScene框架：依据语义一致性和3D覆盖度筛选视角的HierView，以及融合多帧关系预测与几何先验进行物理一致场景组装的Relation-Aware Assembly。

Result: 在ScanNet数据集上取得SOTA性能，倒角距离降低17%，LPIPS降低26%，运行速度提升高达10倍，并构建了高质量的具身视觉问答数据集。

Conclusion: ReScene成功解决了物理一致性场景重建的难题，为具身智能的下游任务提供了高效且高质量的3D场景重建方案。
...
### [109] [TextDS: Parameter-Efficient Representation Alignment for Scene Text Detection under Distribution Shifts](https://arxiv.org/abs/2606.28077)
*Boyuan Chen,Zichen Dang,Chuang Yang,Lap-pui Chau,Yi Wang*

Main category: cs.CV

TL;DR: 提出了一种名为TextDS的高效场景文本检测框架，旨在解决真实世界部署中的分布偏移和图像降质问题。


<details>
  <summary>Details</summary>
Motivation: 现有的场景文本检测器在面对跨域变化和真实世界图像降质时评估受限，且高度依赖大规模场景文本预训练。

Method: 提出了基于视觉基座模型的双编码器设计，引入了具有动态早退机制的逐步LoRA自适应（SWLoRA），以及用于双分支对齐与融合的公共子空间融合（CSF）机制。

Result: 仅需4.9M可训练参数，TextDS在跨领域和恶劣成像条件下均实现了具有竞争力的场景文本检测性能与强鲁棒性。

Conclusion: TextDS是一个高效且鲁棒的框架，证明了无需大规模场景文本预训练也能有效应对分布偏移挑战。
...
### [110] [AirGroundBench: Probing Spatial Intelligence in Multimodal Large Models under Heterogeneous Multi-View Embodied Collaboration](https://arxiv.org/abs/2606.28049)
*Haotian Li,Yida Wang,Leyuan Wang,Jinshan Lai,Keyang Wang,Zonghao Guo,Qiang Ma,Liuyu Xiang,Jianwei Hu,Zhaofeng He*

Main category: cs.CV

TL;DR: 本文提出了 AirGroundBench，这是一个用于评估无人机-无人车协同中多视角空间智能的基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有基准多关注单视角感知，缺乏对空地协同场景中多尺度、非对称遮挡及参考系不一致等空间理解能力的系统评估。

Method: 构建了包含11个仿真环境、1021对空地观测、约62000个问答实例和115个导航片段的基准 AirGroundBench，并对13个代表性多模态大模型在不同视角配置下进行评估。

Result: 模型在基础空间感知上表现尚可，但在跨视角对齐、变换推理及序列决策中遇到瓶颈，双视角表现虽优于单视角但仍显著落后于人类。

Conclusion: 几何一致性是当前具身多模态大模型的主要限制，跨视角空间智能仍有巨大提升空间。
...
### [111] [Diffusion Model Attribution via Spectral Coupling of Denoiser Responses](https://arxiv.org/abs/2606.28092)
*Pragati Shuddhodhan Meshram,Varun Chandrasekaran*

Main category: cs.CV

TL;DR: 本文提出了一种名为谱去噪签名（SDS）的非侵入式归属方法，通过分析扩散模型在去噪过程中的谱几何特征来识别其来源。


<details>
  <summary>Details</summary>
Motivation: 将生成的图像准确归属到其源扩散模型非常困难，因为不同模型可能收敛到相似的输出分布，且现有方法易在架构相似或共享自动编码器的模型上失效。

Method: 利用频控扰动探测候选模型的去噪行为，提取其在空间频段上重新分配能量的独特谱几何特征，作为该模型的稳定指纹。

Result: SDS在8种不同的扩散模型上达到了约99.9%的分类准确率，在跨域提示词偏移下达到96.2%的准确率，显著优于现有的非侵入式基线方法。

Conclusion: 谱几何特征可以作为扩散模型归属的原理性且实用的基础，为图像来源验证和知识产权保护提供了有效手段。
...
### [112] [Cross-view Multimodal Vision-Based Assessment Framework for Traditional Chinese Medicine Rehabilitation Training](https://arxiv.org/abs/2606.28104)
*Francis Xiatian Zhang,Hao Yao,Shengxuan Chen,Hong Zhu,Hongxiao Jia,Sisi Zheng,Hubert P. H. Shum*

Main category: cs.CV

TL;DR: 提出CME-AQA框架，通过融合第一和第三人称双视角视频，提升针灸和推拿等复杂中医康复训练动作评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的单视角骨骼评估方法难以应对中医手法（如针灸、推拿）中密集的手部自遮挡和复杂的动作交互。

Method: 设计了跨视角、多模态评估框架CME-AQA，融合视觉与姿态信息，并在训练中结合双视角视频；同时构建了TCM-AQA61-A和TCM-AQA61-T两个双视角数据集。

Result: 在关键评分任务上，加权F1值比最优对比方法提升了10%以上，降低了定量指标的平均绝对误差，并在CPR数据集上表现出良好的泛化能力。

Conclusion: CME-AQA有效提升了结构化中医康复训练的评估准确性，为临床技能评估提供了更便捷高效的方案。
...
### [113] [Monocular Avatar Reconstruction via Cascaded Diffusion Priors and UV-Space Differentiable Shading](https://arxiv.org/abs/2606.28144)
...
TL;DR: 本文提出了一种高效的数据利用框架，通过级联的低秩适应（LoRA）技术，利用预训练扩散模型的强 generative 先验，从单张野外图像中重建出高质量、可重新光照的3D化身。


<details>
  <summary>Details</summary>
Motivation: 单张图像重建可重光照3D化身面临高质量PBR数据稀缺，以及难以从固有材质中解耦光照的挑战。

Method: 在UV空间中使用级联LoRA（包括Inpainting LoRA和Light-Homogenization LoRA）进行纹理补全与去光照，引入交叉本征注意力机制合成像素级对齐的PBR贴图，并施加UV空间可微BRDF着色损失以确保物理合理性。

Result: 仅需不到100个真实3D扫描数据进行训练，即可生成优于现有技术的4K分辨率高保真PBR资产，表现出极强的泛化能力。

Conclusion: 该方法通过高效利用预训练扩散模型的先验知识，成功解决了解耦和数据稀缺问题，为高质量3D化身重建提供了一种实用且高效的解决方案。
...
### [114] [EchoSonar-R: A Multi-View Reasoning-Enabled Model for Disease Classification and Report Generation in Echocardiography](https://arxiv.org/abs/2606.28164)
*Darya Taratynova,Ahmed Aly,Numan Saeed,Mohammad Yaqub*

Main category: cs.CV

TL;DR: EchoSonar-R是一个多视角推理视觉语言模型，用于超声心动图的疾病分类与报告生成。


<details>
  <summary>Details</summary>
Motivation: 现有模型在解释超声心动图时缺乏显式的诊断推理和空间定位解剖证据，限制了临床信任度。

Method: 结合时空视频编码器与结构感知心脏检测器，并采用监督微调（SFT）与基于任务特定奖励的组相对策略优化（GRPO）两阶段强化学习框架进行训练。

Result: 在私有数据集和MIMICEchoQA上宏均衡准确率分别提升17.1%和6.1%，临床忠实度（GREEN）达0.800，并能提供基于视觉证据的可解释推理过程。

Conclusion: EchoSonar-R通过多视角推理和空间解剖定位，有效提升了超声心动图自动诊断的准确性、报告生成质量及临床可信度。
...
### [115] [RSICCLLM: A Multimodal Large Language Model for Remote Sensing Image Change Captioning](https://arxiv.org/abs/2606.28266)
*Yelin Wang,Zijia Song,Shuo Ye,Chuanguang Yang,Miaoyu Wang,Yong Xu,Zhulin An,Yongjun Xu,Zitong Yu*

Main category: cs.CV

TL;DR: 本文提出了RSICCLLM，这是首个针对遥感图像变化描述（RSICC）任务的大型视觉语言模型后训练框架，仅需7B参数即可实现卓越性能。


<details>
  <summary>Details</summary>
Motivation: 传统的深度学习架构在RSICC任务中受限于模型容量，且由于数据稀缺以及对细粒度变化理解的高要求，直接迁移通用大语言模型面临挑战。

Method: 设计了数据生成范式并构建了指令数据集RSICI与基准；引入了差异感知监督微调以感知时空差异；提出了双否定偏好优化（DNPO）方法及偏好数据集RSICP以进一步优化模型。

Result: 实验结果表明，RSICCLLM在各项指标上表现优异，仅以7B的参数量便超越了参数量大得多的模型。

Conclusion: RSICCLLM是一个针对遥感图像变化描述行之有效的后训练框架，为大模型在特定遥感领域的应用提供了新的思路。
...
### [116] [StructSplat: Generalizable 3D Gaussian Splatting from Uncalibrated Sparse Views](https://arxiv.org/abs/2606.28321)
*Jia-Chen Zhao,Beiqi Chen,Xinyang Chen,Guangcong Wang,Liqiang Nie*

Main category: cs.CV

TL;DR: StructSplat是一个无需相机参数、直接从非标定图像中进行前向泛化3D高斯重建的框架。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖于逐场景优化或已知相机位姿，且将几何与外观耦合在一起，限制了重建的保真度和泛化能力。

Method: 采用结构化表示解耦几何、语义和纹理，引入像素对齐特征注入以精确建模纹理，结合语义感知先验提升全局一致性，并设计相机对齐策略以提高泛化性。

Result: 在DL3DV数据集上达到28.045 PSNR，比AnySplat提升了5.67 dB；在ACID和RealEstate10K的跨数据集评估中，分别比AnySplat提升了1.94 dB和1.72 dB。

Conclusion: StructSplat通过结构化表示和相机对齐策略，成功实现了无需相机校准的高质量、强泛化性的三维高斯重建。
...
### [117] [PerceptionRubrics: Calibrating Multimodal Evaluation to Human Perception](https://arxiv.org/abs/2606.28322)
...
TL;DR: 本文提出了PerceptionRubrics，一个基于细粒度规则的评估框架，通过原子级审计解决多模态模型在实际应用中的感知脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 旨在弥合现有基准测试分数饱和与模型实际脆弱性之间的差距，解决传统整体语义匹配无法有效检测细粒度感知错误的问题。

Method: 构建了包含1,038张信息密集图像和超12,000个规则的数据集，利用循环同行评审生成标准描述，并将其蒸馏为“必须正确”与“易错”的双流规则，采用门控评分机制进行严格评估。

Result: 评估发现模型在复杂约束下存在可靠性缺陷，开源与闭源模型在感知上存在8%的持续差距，且该门控评估指标比传统基准更符合人类评估的严谨性。

Conclusion: 严格的感知保真度是实现可靠生成的关键前提，PerceptionRubrics为多模态评估提供了一种更具人类对齐度且严谨的新方案。
