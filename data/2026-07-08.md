...
### [1] [Escaping the Procrustean Bed: Groupwise Orthogonal Connectors for Audio-Language Models](https://arxiv.org/abs/2607.06014)
*Ho-Lam Chung,Ke-Han Lu,Yi-Cheng Lin,Guan-Ting Lin,Yiming Chen,Hung-yi Lee*

Main category: cs.SD

TL;DR: 提出ORCA方法，通过分组和约束查询输出解决语音-语言模型连接器中的表示崩塌和信息丢失问题。


<details>
  <summary>Details</summary>
Motivation: 现有语音-语言模型的Q-Former连接器存在缺陷，即输出向量会崩塌至单一方向，导致说话人身份、性别和语调等副语言信息丢失。

Method: 提出ORCA方法，将查询（queries）分组并约束其输出指向不同方向，以逆转向量崩塌。

Result: ORCA在SAKURA多步推理上比4B基线提升26.4分达到75.2%，优于8B的Audio Flamingo-3（49.0%）；在连接器层面，查询冗余减少12倍，跨说话人方差提升75倍。

Conclusion: ORCA有效克服了语音连接器的表示崩塌，显著保留了副语言特征并提升了模型的推理表现。
...
### [2] [Determinantal point process sampling for bioacoustic active learning](https://arxiv.org/abs/2607.06063)
*Hugo Magaldi,Gabriel Dubus*

Main category: cs.SD

TL;DR: 本文提出了一种名为 CARE-DPP 的批处理主动学习方法，用于高效减少生态声学数据的标注成本并提升分类器性能。


<details>
  <summary>Details</summary>
Motivation: 生态声学监测产生的大量音频数据需要耗费极高的标注成本，亟需高效的主动学习方法来优化样本选择。

Method: 结合类别平衡的预测不确定性与嵌入空间新颖性，利用行列式点过程（DPP）确保样本非冗余，并采用动态退火策略与自适应采集调度。

Result: 在 BirdSet 和 ATBFL 数据集上，该方法取得了 0.50 的平均开发 AULC（macro mAP），显著优于官方基线 CoreSet（0.46）。

Conclusion: CARE-DPP 是一种高效率的生物声学主动学习方法，其中 DPP 批次多样化和自适应调度对性能提升起到了关键作用。
...
### [3] [Music I Care About: Automated Multimodal Benchmarking of LLM Music Perception Skills on (Almost) Any Music](https://arxiv.org/abs/2607.06015)
*Tomáš Sourada,Katia Vendrame,Jan Hajič*

Main category: cs.SD

TL;DR: 本文提出了 MusICA-MetaBench，一个能从用户数据中自动生成多模态（音频、乐谱、符号）音乐理解评测基准的框架。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型的音乐评估基准存在评估成本高、缺乏真正的音乐感知测试以及不支持跨模态系统对比等局限性。

Method: 利用结构化符号表示（如 MusicXML）和预定义的问题模板，自动构建针对音频、图像和符号模态的选择题，并在 ChoraleBricks 数据集上进行验证。

Result: 确定了确保统计可靠性的基准规模，并通过与纯文本和白噪音基线对比，证实了该基准确实能测试模型的音乐感知能力。

Conclusion: MusICA-MetaBench 实现了多模态大模型音乐感知能力的跨模态评估，提供了一种高效、按需的音乐基准测试新范式。
...
### [4] [From Textural Counterpoint to Feature Encoding: A Multi-Dimensional Machine Representation Study of Haydn's "The Lark" Integrating Electroacoustic Analysis](https://arxiv.org/abs/2607.05902)
*Yakun Liu,Zhiyu Jin,Hai Luan,Dong Liu,Xiaonan Li*

Main category: cs.SD

TL;DR: 本文提出了一种结合古典音乐形态学、电声测量和机器表征的人机协同作曲新方法，旨在赋予AI音乐生成模型角色感知与动态交互能力。


<details>
  <summary>Details</summary>
Motivation: 现有的深度音乐生成模型在多声部交互中缺乏角色感知能力，而室内乐的声部动态交互逻辑为解决这一问题提供了研究蓝图。

Method: 提出“古典形态定性分析-电声定量测量-机器表征重建”的跨学科路径：分析海顿四重奏的对位形态，利用DAW工具量化声学特征，并引入事件时间戳与“角色感知编码”重建机器特征表征。

Result: 成功打通了从古典分析、电子音乐映射到AI符号生成的逻辑闭环，为构建具备“社交属性”与“他者意识”的人机协同音乐系统奠定了理论基础。

Conclusion: 该研究通过融合音乐学与计算机科学的新机制，克服了传统机械网格量化的局限，为开发具有深度交互能力的智能音乐生成系统开辟了新途径。
...
### [5] [Fréchet Distance Loss on Speech Representations for Text-to-Speech Synthesis](https://arxiv.org/abs/2607.06027)
*Ho-Lam Chung,Kuan-Po Huang,Bo-Ru Lu,Hung-yi Lee*

Main category: cs.SD

TL;DR: 提出了一种名为SR-FD的训练时分布正则化损失，通过匹配生成语音与参考语音的特征分布，显著提升了少步数流匹配TTS模型的语音可懂度。


<details>
  <summary>Details</summary>
Motivation: 现有的少步数扩散和流匹配TTS模型主要依靠局部目标训练，无法保证采样语音符合高质量语音的全局分布，导致少步数合成时可懂度（WER）较差。

Method: 提出语音表示Fréchet距离损失（SR-FD）。在微调期间使用与部署相同的少步数采样器合成语音，并匹配该语音与参考目标在冻结的Whisper和CTC特征上的均值和协方差，该方法无需判别器，且不增加推理时的计算开销。

Result: 在Seed-TTS英文任务上，4步采样的SR-FD微调将字错率（WER）从基线的2.2279%降低至1.4147%（相对降低36.5%），甚至优于原始的10步采样基线（1.7366%），同时保持了说话人相似度和语音质量。

Conclusion: SR-FD是一种有效的分布正则化方法，可在不增加推理开销的情况下，显著提高少步数TTS模型的语音可懂度。
...
### [6] [BlueMagpie-TTS: A Token-Efficient Tokenizer, Language Model, and TTS for Taiwanese-Accent Code-Switching Speech](https://arxiv.org/abs/2607.06054)
...
TL;DR: 本文提出 BlueMagpie-TTS 系统，通过引入适配台湾语境的分词器 PangolinTokenizer 和语言模型 Barbet，显著提升了台湾国语及中英夹杂文本的语音合成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的TTS系统在台湾国语上表现不佳，存在口音偏离、分词过度切分以及中英夹杂处发音退化等问题，其根源在于文本端缺乏对台湾本地语境的适配。

Method: 从文本端进行自底向上适配：1) 训练基于台湾语境数据的字节级BPE分词器 PangolinTokenizer；2) 构建十亿参数的繁体中文语言模型 Barbet 作为文本语义前端；3) 提出 BlueMagpie-TTS，通过学习到的桥接模块将 Barbet 连接到固定的预训练声学模型 VoxCPM2 上。

Result: PangolinTokenizer 实现了最低的字符Token率；Barbet 在14项公共评估任务中表现最优；BlueMagpie-TTS 在台湾本地测试集上将字错率（CER）降至4.81%（相对降低58.0%），词错率（WER）降至5.36%（相对降低63.9%）；盲听测试中65.6%的主流意见更偏好该系统。

Conclusion: 通过在文本前端进行本地化语境适配，可以在不改变声学模型的前提下，大幅提升特定地区口音（如台湾国语）的语音合成质量及中英夹杂发音的准确性。
...
### [7] [Learning-based Physics-Constrained Neural Kernel for Sound Field Estimation With Source-Position-Dependent Directional Weighting](https://arxiv.org/abs/2607.06274)
*Mattia Marella,Shoichi Koyama*

Main category: cs.SD

TL;DR: 提出一种基于学习和物理约束的神经核方法，用于提高声场估计在未知声源位置上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的核回归声场估计方法通常针对单次快照测量进行优化，容易导致严重的过拟合和较差的泛化性能。

Method: 引入一种与声源位置相关的隐式神经表示（INR）来建模方向权重函数，使核函数能捕获通用的方向模式并泛化至未见声源位置。

Result: 实验表明，该方法估计的方向权重函数能匹配目标声场的方向性，在声场估计性能上优于基于快照的方法。

Conclusion: 结合声源位置相关INR的物理约束神经核方法能有效提升声场估计的泛化能力和准确性。
...
### [8] [Flow Matching-Based Speech Source Separation with Best-of-N Biometric Sampling](https://arxiv.org/abs/2607.06088)
...
TL;DR: 提出了一种基于条件流匹配的单通道语音分离方法，结合冻结的说话人编码器解决了排列歧义和长音频分块对齐问题。


<details>
  <summary>Details</summary>
Motivation: 单通道语音分离在实际部署中面临源排列歧义、生成模型采样变异性以及长录音分块推理困难等挑战。

Method: 提出一种基于条件流匹配（CFM）的方法来产生有序的双源输出，并在训练和推理阶段复用冻结的说话人编码器来进行源排序、最佳候选选择（best-of-N）和分块通道对齐。

Result: 在Libri2Mix基准测试中，该Transformer U-Net变体在客观分离指标上具有竞争力，并在下游自动语音识别（ASR）和说话人确认（SV）任务中均取得了最低的错误率。

Conclusion: 该方法有效克服了单通道语音分离的部署难题，显著提升了下游语音识别和说话人确认的性能。
...
### [9] [Designing Maintainable Hybrid Generative Systems: A Quantum-Inspired Approach to Automated Music Harmony Generation](https://arxiv.org/abs/2607.06296)
*Josef Pavlicek*

Main category: cs.SD

TL;DR: 本文提出了一种结合量子启发算法与规则优化的混合生成架构，用于自动旋律和声生成。


<details>
  <summary>Details</summary>
Motivation: 旨在平衡自动音乐和声生成中的生成灵活性与结构控制，并在无需训练语料的情况下提高系统的可维护性。

Method: 结合了在重叠旋律上下文上的量子启发式候选探索方法，以及显式的基于规则的优化层。

Result: 该方法生成的和声能够保留调性结构和终止式行为，支持多种有效的和声实现，且优化层提高了结构的连贯性、稳定性和可预测性。

Conclusion: 研究证明，在信息系统开发背景下，可以系统地设计和评估透明且可控的混合生成系统。
...
### [10] [InsideSSL: Understanding Self-Supervised Speech Representations using a Model-Centric Perspective](https://arxiv.org/abs/2607.06392)
*Samir Sadok,Xavier Alameda-Pineda*

Main category: cs.SD

TL;DR: 本文提出了 InsideSSL 分析框架，用于深入探究语音自监督学习（SSL）模型的内部层级动态。


<details>
  <summary>Details</summary>
Motivation: 尽管语音自监督模型在多项任务中表现优异，但其内部的层级表征演变和动态机制仍不清晰。

Method: 提出了 InsideSSL 框架：首先从熵（压缩）、曲率（几何）和扰动鲁棒性三个维度进行任务无关分析；其次引入跨层生成兼容性矩阵（GCM）并结合线性探针来评估功能迁移与下游任务的关联。

Result: 研究表明不同的训练目标会引导不同的声学压缩和流形展开模式，揭示了稳定的语音核心和深层语义剪枝等现象，并证实了层拓扑结构如何决定音素、音高和说话人信息的编码。

Conclusion: InsideSSL 框架为理解语音自监督模型提供了系统性的模型中心视角，成功将模型的层级拓扑特征与下游任务表现联系起来。
...
### [11] [From Graphs to Gradients: Physics-Inspired Structural Attribution for Cyber-Physical IoT Systems and Beyond](https://arxiv.org/abs/2607.05563)
*Spyridon Evangelatos,Christos Diou,Georgios Th. Papadopoulos,Evangelos Markakis,Panagiotis Sarigiannidis*

Main category: cs.AI

TL;DR: 提出一种基于统计物理能量模型的解释性框架，无需重建有向因果图即可为复杂物联网系统提供准确的依赖关系归因。


<details>
  <summary>Details</summary>
Motivation: 在具有反馈回路和部分可观测性的复杂网络物理系统中，重建显式的有向因果结构通常是不切实际的，而传统的解释方法仅能显示相关性而非因果性。

Method: 受统计力学启发，通过无向的、基于能量的表示来建模变量依赖关系，并通过分析能量景观的变化来评估各个组件的影响，支持对微扰效应的推理。

Result: 在工业物联网测试集上的仿真实验表明，该方法在归因准确性、鲁棒性和可扩展性上均显著优于现有的基于图的方法。

Conclusion: 该框架为混合高维系统提供了可靠且具依赖感知的解释，能够有效支持人类解释及下游预测和诊断任务，并可推广至其他复杂系统。
...
### [12] [Prompt-to-Paper: Agentic AI System for Bioinformatics](https://arxiv.org/abs/2607.05456)
*Ramsha Kamran,Maheera Amjad,Zartasha Mustansar,Arsalan Shaukat,Salma Sherbaz,Muhammad U. S. Khan*

Main category: cs.AI

TL;DR: 提出 Prompt-to-Paper 多智能体框架，通过运行真实实验和闭环质量优化，低成本生成高可靠性的学术论文。


<details>
  <summary>Details</summary>
Motivation: 现有的自动文献生成系统存在引用不可靠、实验结果造假以及缺乏标准化质量评估等严重缺陷。

Method: 结合检索增强生成（RAG）定位引用、自主代码智能体执行真实计算实验，并采用八维自动质量评分器进行闭环迭代修正。

Result: 在5个案例研究中成功生成了排版规范且无错误引用的PDF，质量评分平均提高17.96分，人类评审均分达7.0/10，单篇论文生成成本仅0.31美元。

Conclusion: Prompt-to-Paper 框架证明了通过多智能体协作，能以极低成本生成具备真实实验支撑和严谨引用的高质量学术论文。
...
### [13] [Foundation Models for Automatic CAD Generation](https://arxiv.org/abs/2607.05573)
*J de Curtò,Victoria Guillén,I. de Zarzà*

Main category: cs.AI

TL;DR: 本文评估了基础模型在自动机械零件CAD生成中的表现，并提出了结合解析几何反馈与视觉语言模型反馈的LLMForge框架。


<details>
  <summary>Details</summary>
Motivation: 旨在探索如何利用先进的LLM和VLM技术，从自然语言规范中自动且高质地生成参数化3D CAD机械零件。

Method: 引入了LLMForge框架，该框架包含两种迭代优化机制：基于解析几何指标反馈的IterTracer，以及基于VLM语义视觉推理反馈的IterVision，并在97个设计问题的基准数据集上对7个基础模型进行了评估。

Result: 在IterTracer机制下，领先模型的网格生成成功率达到98.97%；在IterVision机制下，最先进模型实现了100%的水密网格生成，但在旋转对称几何体（如圆柱体）的识别上仍存在局限。

Conclusion: 研究表明紧凑型指令微调模型在CAD生成上可媲美大型模型，同时指出视觉语义反馈在提升几何质量上的潜力以及在处理对称性几何时的挑战。
...
TL;DR: 本文提出了 FirstResearch 框架，通过引入结构化的“研究问题证书”来提高大语言模型（LLM）生成的科学研究问题的可审计性。


<details>
  <summary>Details</summary>
Motivation: 现有的科学探索 LLM 系统提出的研究问题难以审计，往往缺乏具体机制、可证伪性或假设，导致科学家难以对其进行有效评估。

Method: 提出了 FirstResearch 框架，其核心是“研究问题证书”（Research Question Certificate），该证书记录了定义、假设、机制模型、矛盾冲突、可证伪假设、决定性测试和失败更新规则。

Result: 在十个研究主题上，FirstResearch 的表现显著优于 AI co-scientist 和 Agent Laboratory 等基准系统，在 Gemini 评估中获得 4.86/5 的高分，且消融实验证实证书机制是提升性能的关键。

Conclusion: 显式推导约束（如研究问题证书）是使 LLM 生成的科学问题更具可审计性的一种极具前景的机制。
...
TL;DR: 本文提出了CSTutorBench，一个用于评估语言模型在图形化编程（VEX VR）教育中作为AI导师表现的基准测试。


<details>
  <summary>Details</summary>
Motivation: 在K-12教育中部署大模型存在隐私和成本担忧，而小语言模型（SLM）虽然是替代方案，但在缺乏特定领域数据（如图形化编程）时，难以选择合适的模型。

Method: 开发了包含17个场景化问题的CSTutorBench，基于教学法研究制定评分细则，并采用人机协同的LLM-as-judge流程进行评估。

Result: 对11个模型的测试表明，模型在语气等表面标准上表现较好，但在避免直接泄露答案和结合学生调试历史等深度教学行为上较差；模型家族和微调方法比参数量更能决定教学质量；提示词优化改善了多数模型的表现。

Conclusion: 针对特定教学情境且基于教学法设计的基准测试，对于在教育部署中选择合适的小语言模型具有重要价值。
...
### [16] [Narrative World Model: Narratology-Grounded Writer Memory for Long-Form Fiction](https://arxiv.org/abs/2607.05577)
*Mohammad Saifullah,Thomas Kornmaier,Taaha Kazi,Vasu Sharma,Aditya Sanjiv Kanade,Aanand Kumar Yadav*

Main category: cs.AI

TL;DR: 本文提出了叙事世界模型 (NWM)，一种通过叙事学驱动的时间状态图和混合检索来解决长篇小说多跳叙事问答的作家记忆系统。


<details>
  <summary>Details</summary>
Motivation: 现有的通用检索和智能体记忆系统无法捕获长篇小说写作中所必需的叙事结构（如信息状态、事件顺序等），导致无法有效回答复杂的叙事问题。

Method: 引入叙事世界模型 (NWM)，将基于叙事学的有类型时间状态图与查询条件混合检索相结合，并在控制阅读器变量的条件下进行多跳问答基准测试。

Result: NWM 在多跳叙事问答上显著优于强时间知识图谱基线 Graphiti/Zep、GraphRAG 以及扁平检索，其优势主要源于其叙事学结构表示和查询条件检索。

Conclusion: 针对长篇故事的记忆系统需要从叙事学角度进行结构设计，仅靠传统的实体与事实表示无法满足复杂的叙事推理需求。
...
### [17] [Memory in the Loop: In-Process Retrieval as ExtendedWorking Memory for Language Agents](https://arxiv.org/abs/2607.05690)
*Yusuf Khan,Carlo Lipizzi*

Main category: cs.AI

TL;DR: 本文研究了将智能体记忆库存放于进程内以实现每步循环读写（in-loop memory）的可行性，通过降低延迟使其成为智能体的“扩展工作内存”。


<details>
  <summary>Details</summary>
Motivation: 传统的智能体记忆库多为网络端存储，高延迟（数十至数百毫秒）限制了智能体在每个推理步骤中频繁读写记忆的能力。

Method: 提出采用进程内存储（in-process store）代替网络存储，将响应时间缩短至微秒级，并配合本地小型嵌入模型以消除网络嵌入延迟。

Result: 进程内存储使操作延迟降至80-165微秒，结合本地嵌入后仅需约40微秒；冗余动作显著减少（从7.2次降至0次），召回率从0/5提升至3.6-4.8/5。

Conclusion: 极低延迟的存储使记忆库能够无缝融入智能体每步循环中，消除了检索的延迟惩罚，而记忆遗失的瓶颈也从存储性能转移到了智能体的读取策略上。
...
TL;DR: 提出了Akashic，一种针对大模型Agent的低开销记忆系统，通过分块建模和软硬件协同布局提升了长上下文处理效率和任务准确率。


<details>
  <summary>Details</summary>
Motivation: LLM Agent在多轮交互中累积的长上下文导致Prefill开销过高、易超出上下文限制，且冗余信息会干扰任务，降低服务效率和输出质量。

Method: 设计了Akashic系统，引入MemAttention机制将上下文分块并建模跨分块语义关系；同时采用软硬件协同设计的内存布局，协同定位可能被共同检索的分块以减少I/O开销。

Result: 在4种工作负载和3种模型大小的测试中，Akashic将任务准确率提升了高达10.2个百分点，吞吐量提升高达1.21倍，可持续请求率提升高达1.88倍。

Conclusion: Akashic通过创新的内存管理和检索优化，有效解决了长上下文LLM Agent的性能与效率瓶颈。
...
### [19] [ArtisanCAD: An Industrial-Level CAD Agent with Expert-Grounded Knowledge Distillation](https://arxiv.org/abs/2607.05750)
...
TL;DR: 本文提出了一种基于技能引导和专家知识蒸馏的工业CAD智能体，利用CAD中间表示（CAD-IR）将模糊的文本提示转化为可执行且可编辑的B-Rep模型。


<details>
  <summary>Details</summary>
Motivation: 现有的文本生成CAD方法在面对模糊或高层设计意图的提示时表现不佳，且未能有效利用工业工作流中的专家程序化知识。

Method: 引入了CAD中间表示（CAD-IR），并开发了一个智能体系统。该系统能够检索专家技能、实例化和修改CAD-IR、通过CATIA-MCP后端执行程序，并利用多视角视觉反馈进行迭代优化。

Result: 在Text2CAD基准测试中，该方法将平均倒角距离（Chamfer Distance）从14.83降低至9.88；在四个复杂汽车零部件上，成功将专家CATIA记录蒸馏为可复用技能，生成了可编辑的B-Rep模型。

Conclusion: CAD-IR能有效弥合模糊文本意图与可执行CAD构建之间的差距，实现了高精度、生产级别的工业CAD自动建模。
...
### [20] [Synthetic Consumer Insight Generation with Large Language Models](https://arxiv.org/abs/2607.05761)
*Stephen L. France,Pia. A. Albinsson*

Main category: cs.AI

TL;DR: 本文研究了使用大语言模型（LLM）生成投影技术下的虚拟消费者数据的可行性，并将其与真实人类响应进行了多维度对比。


<details>
  <summary>Details</summary>
Motivation: 数据驱动的营销高度依赖消费者数据，但传统收集方法成本高、耗时且难以规模化，因此需要探索利用LLM生成合成数据作为替代方案。

Method: 测试了不同LLM、提示策略和温度设置在多项投影任务中的表现，并与人类关于城市旅游感知的真实响应进行对比，采用语言学度量、多样性指标、主题模型和高频词分析等方法进行评估。

Result: LLM与人类响应在宏观主题和关联上表现出显著的重合，但在语言风格、句法结构以及多样性的生成机制上存在明显差异。

Conclusion: LLM可有效辅助生成虚拟消费者数据，但使用时需优化模型与提示词选择，并警惕其在表达风格和多样性方面的局限性。
...
TL;DR: 本文系统性地梳理了27篇评估文献，构建了首个大语言模型（LLM）智能体局限性的统一分类法，揭示了六大核心失效类别。


<details>
  <summary>Details</summary>
Motivation: 现有的智能体基准测试进展掩盖了反复出现的系统性失效模式，缺乏对智能体在工具使用、规划和安全等领域局限性的统一整合与分类。

Method: 对涉及19个基准测试的27篇文献进行综合分析，通过迭代归纳将报告的错误类型按智能体推理至行动管线的阶段划分为六大失效集群。

Result: 确定了工具调用、规划失败、长上下文退化、多智能体协同失效、安全漏洞及测量有效性六大失效集群；发现失效会随任务长度非线性累加，且辅助框架无法稳定提高可靠性。

Conclusion: LLM智能体在复杂、长流程任务中仍面临严重的非线性失效挑战，单项子任务的成功无法保证端到端的可靠性，未来研究需关注这些系统性局限。
...
TL;DR: 本文介绍了一种名为 AgenticAI-Supervisor 的强化学习 Gym 环境，旨在通过可验证的执行结果和多维奖励塑形来评估和优化大语言模型智能体的多步决策能力。


<details>
  <summary>Details</summary>
Motivation: 传统的静态评估方法无法有效捕获大语言模型智能体在多步决策过程中的表现。

Method: 引入了解耦环境创建与执行的 API/UI 驱动 Gym 环境，采用可验证的执行结果、多维奖励塑形以及严格的内部状态验证来防止奖励作弊。

Result: 通过客户支持智能体的案例研究，验证了该平台能够为模型优化提供持续的闭环反馈。

Conclusion: 该平台为智能体评估提供了高保真追踪和防作弊机制，未来将拓展至计算机使用、工具调用及自动边缘情况生成等前沿领域。
...
TL;DR: 本文提出了NapMem框架，将长期用户记忆构建为多粒度的结构化动作空间，并通过强化学习训练智能体主动检索和利用记忆。


<details>
  <summary>Details</summary>
Motivation: 现有的个性化对话系统通常将长期记忆视为被动检索的上下文，限制了智能体根据需求主动筛选和组合不同粒度记忆的能力。

Method: 构建了一个连接原始对话、记忆记录、主题和用户画像的多粒度记忆金字塔，并利用记忆工具强化学习（RL）训练智能体自主选择和导航这些记忆工具。

Result: 在PersonaMem-v2、LongMemEval和LoCoMo等多个基准测试中，NapMem在记忆密集型任务上表现优异，且未损害智能体的通用推理与工具使用能力。

Conclusion: 长期用户记忆的有效利用得益于将结构化存储与针对不同粒度记忆的主动学习策略相结合。
...
### [24] [Controlling Tool Use with Heading-Specific Activation Steering](https://arxiv.org/abs/2607.05790)
*Yuqi Chen,Vincent Siu,Yang Liu,Dawn Song,Chenguang Wang*

Main category: cs.AI

TL;DR: 本文研究了如何通过操纵引导向量（steering vectors）来控制大语言模型的工具调用行为，发现这能实现有效的双向控制，但其内部几何表征呈现非线性且高度工具特异的特征。


<details>
  <summary>Details</summary>
Motivation: 工具增强型LLM常过度使用工具，且工具在推理时仅存在于上下文中，因此探究模型内部是否存在可操纵的工具调用决策表征具有重要意义。

Method: 从五个开源模型中提取标题锚点位置的引导向量，在三个领域中进行双向因果控制实验，并对激活模式进行几何分析。

Result: 引导向量成功实现了对工具调用行为的双向控制，有效抑制了不必要的工具使用；但几何分析表明这种控制并非基于简单的线性结构，且不同工具之间的特征重叠度较低。

Conclusion: 工具使用决策具有可操纵的表征，但其几何特性具有非参数化特征，与参数化概念不同，其几何不规则性与因果控制力之间的关系仍是开放性问题。
...
### [25] [TurnOPD: Making On-Policy Distillation Turn-Aware for Efficient Long-Horizon Agent Training](https://arxiv.org/abs/2607.05804)
*Yuhang Zhou,Kai Zheng,Haoling Li,Dengyun Peng,Can Xu,Jingjing Chen*

Main category: cs.AI

TL;DR: 提出了 TurnOPD，一种针对长程智能体策略蒸馏的轮次级预算策略，显著提高了训练效率和准确率。


<details>
  <summary>Details</summary>
Motivation: 传统的策略内蒸馏（OPD）在长程任务中效率低下，原因在于全长度生成在尾部轮次上浪费了计算资源，且轨迹级KL损失导致浅层Token过度训练而深层决策轮次训练不足。

Method: 引入 TurnOPD，包含两个控制器：自适应生成深度预算（利用探针统计数据动态调整生成长度）和渐进式轮次归一化损失预算（将KL权重从Token级别逐步调整为轮次均衡监督）。

Result: 在 ALFWorld、WebShop 和 Multi-Hop Search 等基准测试中，TurnOPD 在相同的训练时间预算下实现了更高的验证准确率，并拓展了准确率与时间效率的边界。

Conclusion: TurnOPD 通过轮次级别的预算控制，有效解决了长程智能体蒸馏中的资源浪费和训练不均衡问题，提供了一种更高效的智能体训练框架。
...
### [26] [Uncovering Latent Depression Severity for Binary Depression Detection via Advantage-weighting Ranking](https://arxiv.org/abs/2607.05901)
*Manning Gao,Tingyi Liu,Leheng Zhang,Haifeng Hu,Yuncheng Jiang,Sijie Mai*

Main category: cs.AI

TL;DR: 本研究提出了一种新型多模态框架，通过引入“二值优势权重排序损失”来优化潜在空间分布，从而提升了自动抑郁症检测的性能。


<details>
  <summary>Details</summary>
Motivation: 利用视听数据进行自动抑郁症检测时，面临着难以解耦重叠的特征分布以及难以建立鲁棒决策边界的挑战。

Method: 提出了一个包含时间编码器和相互Transformer的细粒度多模态框架，并设计了“二值优势权重排序损失”（包含优势权重分离和优势权重紧凑性两个机制）以优化潜空间分布。

Result: 在D-vlog和LMVD数据集上的实验表明，该模型通过优先处理困难样本对，成功重建了潜在的序关系结构，并取得了最先进的（SOTA）性能。

Conclusion: 通过动态加权困难样本并最小化类内方差，所提方法能有效解决特征重叠问题，为抑郁症检测提供了更鲁棒的决策边界。
...
### [27] [StateFuse: Deterministic Conflict-Preserving Memory for Multi-Agent Systems](https://arxiv.org/abs/2607.05844)
*Sergey Volkov,Yang Li,Ye Luo*

Main category: cs.AI

TL;DR: StateFuse是一种冲突感知的复制内存契约，它通过保留和暴露内存冲突而非直接覆盖，使智能体能够进行更安全的弃权和纠错。


<details>
  <summary>Details</summary>
Motivation: 智能体系统在不同分支和副本中容易积累冲突的观测数据，但现有的内存层通常通过难以检查的覆盖规则直接消除这些分歧。

Method: 提出基于标准OpSet/CRDT合并的StateFuse，定义了包含不可变历史、显式冲突对象、精确与语义纠正句柄以及投影时解析的语义层。

Result: 实验表明，StateFuse在准确率上与基线持平，但能保持冲突可见。保留歧义使智能体能进行更安全的弃权和纠错，且语义句柄在缺失精确标识符时十分关键。

Conclusion: StateFuse适用于作为一种更安全的公共内存契约，用于冲突显露、弃权和可审计的纠错，而非用于提升通用准确率。
...
TL;DR: 本文提出Onnes系统，将物理数字孪生与多智能体LLM结合，实现了稀释制冷机的高精度实时故障诊断。


<details>
  <summary>Details</summary>
Motivation: 传统的稀释制冷机故障诊断依赖阈值报警，只能报告存在异常而无法定位具体故障原因。

Method: 构建了基于物理模型和真实噪声指纹的数字孪生模拟器Onnes，驱动多智能体LLM层，并通过对比少样本演示与监督学习分类器进行故障诊断评估。

Result: 引入少样本演示和自一致性投票后，LLM分类准确率从0.685提升至0.990，媲美监督分类器（0.985），且在真实硬件测试中达到100%的故障召回率。

Conclusion: 物理数字孪生与LLM智能体的结合，无需参数更新即可实现高效、准确的低温设备故障实时监控与诊断。
...
TL;DR: 提出 SearchEyes 框架，通过统一知识图谱骨干构建模拟搜索世界，解决多模态搜索智能体在多跳推理中的结构脱节问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态搜索智能体训练中，数据、环境和奖励信号各自独立构建，导致元数据丢失、搜索环境不可复现以及强化学习奖励稀疏。

Method: 利用类型化知识图谱构建模拟搜索世界，提出感知-知识链（PKC）来采样多跳路径，并提出跳锚定策略优化（HaPO）实现无需独立过程奖励模型的步骤级信用分配。

Result: 在六个多模态知识密集型基准测试中达到最先进水平，其中 SearchEyes-27B 比最强的开源基线平均提高了 6.2 个百分点。

Conclusion: 通过知识图谱统一数据、环境与奖励机制，能有效克服多跳推理中的结构性脱节，显著提升多模态搜索智能体的性能。
...
TL;DR: 本文介绍了 PCBWorld，一个基于 KiCad 引擎的开源交互式 PCB 布线环境及基准测试集，旨在推动强化学习和大语言模型在 PCB 布线任务中的应用。


<details>
  <summary>Details</summary>
Motivation: 尽管 PCB 布线任务至关重要，但目前基于学习的方法仍然落后于基于规则的布线器，缺乏能够提供实时设计规则检查（DRC）反馈的交互式仿真环境。

Method: 构建了基于 KiCad EDA 引擎的 PCBWorld 环境，支持智能体利用引擎原生操作和 DRC 反馈进行布线；同时推出了包含合成和真实数据源的 PCBWorld-Bench 基准，并提供 8 种评估指标。

Result: 实验表明，PCBWorld 智能体表现优于传统的网格动作 RL 和开环 LLM 基线，且在合成数据上训练的 RL 策略能够零样本迁移至真实电路板，性能接近基于规则的布线器。

Conclusion: 基于引擎驱动的交互式方法（如 PCBWorld）为提高 RL 和 LLM 智能体的 PCB 自动布线能力提供了有效且极具前景的基础框架。
...
### [31] [Integrating knowledge graphs and multilingual scholarly corpora for domain-adaptive LLMs in SSH](https://arxiv.org/abs/2607.05956)
*Adam Faci,Alessio Miaschi,Anne Combe,Pascal Cuxac,Francesca Frontini,Nicolas Larrousse,Stéphane Pouyllau*

Main category: cs.AI

TL;DR: 本文介绍了一个在LLMs4EU项目和ALT-EDIC基础设施下，针对人文社科（SSH）研究调整基础大模型的在研案例，重点探讨了其评估框架与合规性。


<details>
  <summary>Details</summary>
Motivation: 将大语言模型整合到人文社科研究中，在学科多样性、多语言文献获取及结果评估方面面临着方法论、认识论和监管等方面的严峻挑战。

Method: 将模型适配嵌入研究基础设施中，并采用双重评估框架：包含检索、摘要、可追溯性和幻觉检测的独立定量基准测试，以及数字人文专家的定性评估。

Result: 开发了支持问答、文献对比分析和文献综述的领域敏感型生成式人工智能应用，并将其置于结构化的法律与伦理合规框架中。

Conclusion: 通过将模型适配与研究基础设施及合规框架相结合，领域敏感和符合监管的生成式AI能够在保持可靠性和学术责任的前提下支持人文社科研究。
...
### [32] [Auto-DSM Under the Lens: A Black-Box Evaluation Framework for LLM-Based DSM Generation](https://arxiv.org/abs/2607.05985)
*Niels Potters,Theo Hofman*

Main category: cs.AI

TL;DR: 本文提出了一个黑盒评估框架，用于系统性地评估大语言模型（LLM）从技术文档中自动生成设计结构矩阵（DSM）的能力。


<details>
  <summary>Details</summary>
Motivation: 由于现有的自动DSM生成流程多为闭源，缺乏公开且可复现的标准化评估方法。

Method: 提出了一种将生成DSM与真实值进行对比的评估框架，结合结构指标、分类指标及稳定性度量，并定义了综合质量得分（Q），在虚拟抽象系统和真实冰箱分解两个数据集上进行了受控实验。

Result: LLM在结构良好的输入下能生成合理的DSM并具有高复现性，但对歧义、不一致的依赖定义和提示词设计十分敏感，存在系统性的幻觉和拒绝失败问题。

Conclusion: 该框架为审计自动DSM生成流程提供了透明的基准，并为将LLM驱动的系统分解方法整合到基于模型的系统工程（MBSE）中奠定了基础。
...
### [33] [AgoraSim: A Hybrid Agent-Based Modeling Framework](https://arxiv.org/abs/2607.05999)
*Chung-Chi Chen*

Main category: cs.AI

TL;DR: 本文介绍了 AgoraSim，一个用于场景导向社会反应分析的混合基于智能体建模（ABM）框架。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLM）智能体模拟的输出容易被过度解读为预测，且难以与显式的社会动力学进行对比。

Method: 开发了 AgoraSim 框架，将文本或多模态工件转化为可编辑的 ABM 配置，支持混合运行多种智能体（LLM、视觉、传统等），并采用统一的结构化决策对象规范交互。

Result: 该框架能够将混合智能体场景与经典参考动力学进行对比，并通过本地 UI、Python SDK 和 API 提供了场景轨迹检查和建模假设对比功能。

Conclusion: AgoraSim 有效地帮助用户评估智能体模拟的社会动力学，识别需要实证验证的关键案例。
...
### [34] [PolyWorkBench: Benchmarking Multilingual Long-Horizon LLM Agents](https://arxiv.org/abs/2607.06008)
*Hongliang Li,Yijin Liu,Zhiwei Zhang,Zihe Liu,Xinyue Lou,Jinan Xu,Fandong Meng,Kaiyu Huang*

Main category: cs.AI

TL;DR: 本文提出了 PolyWorkBench，一个用于评估大语言模型（LLM）智能体在多语言长程工作流中表现的基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准测试多局限于单语言设置，而真实世界应用通常涉及多语言输入和输出，多语言性对智能体执行的影响尚未得到充分研究。

Method: 构建了包含5个领域、67个任务的基准测试 PolyWorkBench，并提出一个结合结构化评分、可执行验证和基于LLM语义评估的混合评估框架。

Result: 最先进的LLM智能体在多语言工作流设置下相比单语言基准出现显著的性能下降，多语言性在推理和执行步骤中带来了复合性的负面影响。

Conclusion: 未来智能体的评估与开发需要将语言多样性与程序化决策进行联合建模。
...
### [35] [Information Limits and Attractor Dynamics in Economies of Frontier LLM Agents: A Pre-Registered Test](https://arxiv.org/abs/2607.06001)
*Cheng Qian*

Main category: cs.AI

TL;DR: 本文报告了一项针对前沿LLM智能体（Claude Opus 4.8）小型经济体的预注册双部分实验，定量验证了信息论财富增长定律，但否定了关于群体失调的均值场残差标度律预测。


<details>
  <summary>Details</summary>
Motivation: 旨在通过定量实验验证多智能体系统中关于市场耦合下财富增长的信息论预测，以及群体行为控制中均值场模型的适用性。

Method: 利用Claude Opus 4.8智能体构建小型模拟经济体，实施预注册实验：一部分测试同注分彩（parimutuel）耦合经济体中的财富增长与信息关系，另一部分测试激励与控制杠杆下的群体响应。

Result: 验证了财富增长差距等于信息差异的定律，最知情的智能体几乎吸收了全部财富；但否定了均值场模型的平滑响应假设，智能体群体的目标离散度塌缩，且对控制杠杆呈双稳态的阶跃响应。

Conclusion: 实验证实了信息论在预测智能体市场行为中的精确性，但表明目前的LLM群体行为不符合平滑均值场模型的噪声维持分散假设，表现出更显著的突变与双稳态特征。
...
TL;DR: 提出一种名为Efficiency的奖励密度启发式算法，在动态车辆路径问题中，其求解质量比肩先进的元启发式算法，但计算速度提升了2到3个数量级。


<details>
  <summary>Details</summary>
Motivation: 旨在解决结合了车辆路径（VRP）与定向运动（OP）的动态在线多车任务分配问题，以应对实时重规划中对计算效率的极高要求。

Method: 设计了一种基于奖励密度的“Efficiency”启发式算法，并在无人机任务分配和城市出租车调度两个场景下，将其与4种传统构造算法及3种经典元启发式算法（ALNS、GA、SA）进行了对比评估。

Result: 在所有测试配置下，该算法在解的质量上匹配了最优的元启发式算法，同时规划时间减少了2至3个数量级，在“收益-计算”前沿上达成了帕累托支配。

Conclusion: 在动态且有时间约束的路由环境中，精心设计的贪婪启发式算法能够以极低的计算成本匹配复杂搜索算法的效果，是实时在线部署的更优选择。
...
TL;DR: 提出了一种“正规博弈”（proper betting）策略，成功将预测准确性与交易盈利性进行形式化统一，并在实际部署中实现了高收益。


<details>
  <summary>Details</summary>
Motivation: 传统理论中预测准确性与交易利润的等价性仅适用于特定自动做市商（AMM）设计，但在主流的限价订单簿市场中，该等价性失效，导致准确的预测者也可能亏损。

Method: 基于严格正规评分规则（strictly proper scoring rule），设计了一种仅依赖于预测概率和市场价格的“正规博弈”策略，并证明了期望利润的分解定理。

Result: 在数千次AI模型预测的实证中，该策略是唯一能稳定将准确率转化为利润的策略；在Kalshi平台的实盘部署中，实现了80.33%的投资回报率（Sharpe比率3.35）。

Conclusion: 解决了预测市场中准确性与盈利性脱节的难题，为理性预测者在真实市场中获得稳健收益提供了理论保证和实用策略。
...
TL;DR: 本文提出了一个将好奇心视为生态系统的概念性框架，用于研究单智能体和多智能体的探索行为。


<details>
  <summary>Details</summary>
Motivation: 旨在为好奇心生态学研究以及未来多智能体AI发现系统的设计提供理论框架。

Method: 构建了一个双层框架，分别模拟了单智能体动态调整的提问策略，以及多智能体在共享知识空间中的协同探索行为。

Result: 提出了一个能够追踪查询量、主题多样性、冗余度和可重用知识等指标的动态好奇心生态系统模型。

Conclusion: 该框架为从生态学视角理解好奇心提供了新途径，有助于未来多智能体科研发现系统的设计。
...
TL;DR: 提出IGRPO框架，通过基于信息增益自适应分配探索预算，显著提升了大模型智能体在长程搜索任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在长程搜索中盲目分配rollout预算，导致在低价值的中间状态上浪费了大量计算资源。

Method: 提出基于信息增益的Rollout策略优化（IGRPO），根据节点信息量自适应分配树状探索预算，并通过诱导出的限制性教师分布引导策略学习。

Result: 在七个搜索增强的问答（QA）基准测试中，IGRPO在相同预算约束下均显著且一致地优于现有强基准方法。

Conclusion: 验证了结合自适应树状探索与信息增益引导策略优化的有效性，为提升长程搜索智能体的效率提供了新途径。
...
TL;DR: 本文介绍了一种名为 TOFFEE 的系统，通过蒙特卡洛树搜索（MCTS）合成高质量数据智能体轨迹，以提升其在异构环境中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有数据智能体在未知和异构的企业数据环境中难以泛化，迫切需要高质量的分析轨迹数据用于监督微调（SFT）和上下文学习（ICL）。

Method: 提出 TOFFEE 系统，利用蒙特卡洛树搜索（MCTS）、自适应模型选择和跨任务前缀复用技术，并结合任务池构建、轨迹探索器及学习代价模型来合成轨迹。

Result: 成功在异构环境中为复杂分析任务生成了可扩展的轨迹数据，并演示了在智能体微调和增强推理两个端到端场景中的应用。

Conclusion: TOFFEE 验证了通过 MCTS 合成轨迹的可行性，为提升数据智能体在复杂异构环境下的泛化与推理能力提供了有效的系统级解决方案。
...
TL;DR: 本文提出了一套理论框架，通过引入结构张力、离线循环和推理期可塑性三种机制，将大语言模型的应用层认知协议下沉至原生元架构中。


<details>
  <summary>Details</summary>
Motivation: 当前的LLM本质上是无状态的，高阶认知架构只能通过提示词工程等应用层手段进行模拟，缺乏原生的自适应认知与演化机制。

Method: 提出了三种互锁机制：基于流形拓扑冲突的内生损失函数（结构张力）、用于消化结构冲突的沙盒化自我处理循环（离线循环），以及在不修改预训练权重下重构上下文拓扑的机制（推理期可塑性）。

Result: 该机制使不同的模型实例能演化出独特的拓扑结构，在保持可审计性、可逆性等治理约束的同时，构建出异构的智能生态。

Conclusion: 研究重新将“治理”而非单独的“能力”定义为架构智能的核心标准，为构建安全且多样化的自适应智能体提供了理论路径。
...
TL;DR: 本文提出了 SkillReranker，一个用于智能体自适应技能选择的推理期重排框架。


<details>
  <summary>Details</summary>
Motivation: 在大规模技能库中，模糊的语义匹配以及现有方法对任务难度和技能适用性动态变化的忽视，导致难以准确选择目标技能。

Method: 通过对任务和技能进行语义分解构建有向无环执行图（DAG）以建立任务-技能对应关系，划分出子任务区间，并利用交叉编码器对候选技能进行评分与重排。

Result: 在 ALFWorld 和 ScienceWorld 数据集上结合三种大模型的实验表明，该方法显著提高了任务成功率，减少了环境交互步骤并降低了 Token 消耗。

Conclusion: SkillReranker 能够有效解决智能体在复杂场景下的技能选择难题，提升任务执行效率与经济性。
...
TL;DR: 提出了DT-Guard，一种采用“有推理训练、无推理推理”范式的内容安全防护模型，兼顾了低延迟与高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有安全防护方法在轻量分类模型（高效但难处理复杂隐蔽风险）与推理决策模型（准确但推理延迟高）之间存在权衡。

Method: 将安全判定构建为“意图-类别-安全”的渐进式决策；训练时引入推理轨迹监督，推理时仅输出结构化标签；并提出“滚轮引导的渐进式硬样本优化（RG-PHO）”来提升困难样本的鲁棒性。

Result: 在提示侧和响应侧安全基准上分别取得0.886和0.870的F1值，4B参数的模型以0.878的平均F1值超越了8B参数的强基线模型。

Conclusion: 推理监督可以被有效内化到低延迟的安全判别中，成功实现了高效且鲁棒的大模型安全防护。
...
TL;DR: 本文通过引入无监督字典学习为端到端自动驾驶模型提供事后可解释性，并通过概念级干预纠正错误决策，提升了驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶模型的高复杂性和不透明性增加了模型学习到异常或错误行为的风险。

Method: 将无监督字典学习作为事后可解释性模块集成到自动驾驶模型中，提出逐步框架提取语义概念，并进行概念级别的靶向干预。

Result: 成功分解出具有语义信息的驾驶概念，揭示了决策逻辑，并通过概念干预修正了驾驶决策，实现了可衡量的性能提升。

Conclusion: 可解释性方法能有效降低模型黑盒性，揭示并纠正错误行为，从而最终提升自动驾驶模型的表现。
...
TL;DR: TopoBrick是一个免训练的零样本建筑物联网预测框架，通过利用建筑知识图谱和拓扑感知采样来提高预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有预测方法通常将建筑传感器视为孤立的时间序列或依赖固定协变量，忽略了其物理拓扑、空间层级和运行上下文等结构化关联。

Method: 使用建筑知识图谱构建紧凑的结构骨架，利用智能体拓扑采样器筛选目标特定的外部变量，并根据部署时的可用性（历史已知与未来已知）对变量进行分类组织。

Result: 在三栋真实建筑的实验中，TopoBrick的性能超越了强零样本基础模型，并达到了与完全训练的建筑特定模型相当的水平；消融实验证实拓扑感知采样比随机或固定步数选择更可靠。

Conclusion: 通过结合建筑拓扑知识，无需训练的零样本预测框架能够有效利用空间和物理关联，显著提升暖通空调（HVAC）和气象驱动等物联网传感器的预测效果。
...
TL;DR: 本文旨在厘清AI领域中“世界模型”的概念，提供了其科学定义、关键技术讨论以及发展的分阶段路线图。


<details>
  <summary>Details</summary>
Motivation: 尽管“世界模型”在强化学习和具身智能等领域被广泛应用，但对于其本质定义、预测目标及构建方法，学术界目前缺乏统一共识。

Method: 本文采用观点评述的形式，系统性地梳理并给出了世界模型的科学定义，分析了其关键技术维度，并设计了分阶段的发展路线图。

Result: 确立了世界模型的概念框架，深入探讨了其核心技术特征，并为有效构建世界模型提供了战略指导。

Conclusion: 明确的定义与分阶段的发展路线图，对于未来推动物理AI和具身智能中世界模型的研究与应用具有重要指导意义。
...
TL;DR: 本文提出了用于解释布尔模型的声明式查询语言 ExplAIner 及其优化变体 Opt-FOIL，实现了多种解释查询的统一表达与高效计算。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏统一的声明式语言来表达和分析各种可解释性（XAI）查询，且现有语言 FOIL 存在表达力受限和计算复杂度过高的问题。

Method: 设计了具有扩展词汇和分层结构的查询语言 ExplAIner，以及用于计算极小化解释的子集 Opt-FOIL，并评估了它们在布尔模型（如可分解电路）上的复杂度。

Result: ExplAIner 能表达多种主流解释查询，其评估可通过有限次调用 SAT 求解器完成；Opt-FOIL 能够以多项式次 SAT 调用计算极小化解释。

Conclusion: 该框架为形式化可解释性人工智能（Formal XAI）提供了一种兼具强表达力与算法可行性的统一查询工具。
...
TL;DR: 评估了Nimblemind多智能体系统（nMAS）在从胃活检病理报告中提取幽门螺杆菌感染及相关胃炎信息的表现，结果显示其具有98.61%的高准确率，并能显著缩短临床审查时间。


<details>
  <summary>Details</summary>
Motivation: 幽门螺杆菌感染的相关证据分散在非结构化的自由文本报告中，需要复杂的上下文和否定词理解，导致传统关键字搜索效果不佳且人工审查难以规模化。

Method: 使用54份胃活检病理报告，评估了基于字段名称驱动和证据链接的nMAS多智能体系统，提取四个临床二分类字段，并与MiniMax M2.5模型进行对比。

Result: nMAS正确分类了216个决策中的213个（准确率98.61%），与对比模型表现相当，但nMAS额外提供了源句子定位以支持可追溯性。估算显示，其可将1,000份报告的审查时间从83.3小时降至1.4小时，节省约6100美元的工时价值。

Conclusion: nMAS的主要贡献在于工作流集成和可追溯性，未来需要通过更大规模的多机构研究来评估其证据跨度准确性、临床验证时间及泛化能力。
...
### [49] [Danus: Orchestrating Mathematical Reasoning Agents with Fact-Graph Memory](https://arxiv.org/abs/2607.06447)
...
TL;DR: 本文提出了Danus，一个基于共享事实图的协同编排系统，旨在解决多Agent在研究级数学推理中的并行证明与内存管理问题。


<details>
  <summary>Details</summary>
Motivation: 在大规模、长程的数学推理中，协同多个Agent进行并行证明搜索并保持中间结论的有序与可靠十分困难。

Method: 设计了Danus系统。它以共享事实图为全局内存，由负责规划的主Agent、进行并行搜索的Worker Agent以及负责验证数学主张的无状态验证器组成。

Result: 通过代数几何、奇异性理论和组合数学的六个研究级案例，证明了Danus能够成功构建出长篇且详细的数学证明。

Conclusion: 基于事实图的编排机制为扩展数学推理Agent以解决长程研究问题提供了一条有效途径。
...
### [50] [A Physics-Informed Neural Network Framework for Elastodynamic Wave Propagation in Bimaterial Systems](https://arxiv.org/abs/2607.06479)
*Sonal Ankush Chibire,Jenn-Terng Gau,Bo Zhang*

Main category: cs.AI

TL;DR: 本文提出了一种结合物理信息神经网络（PINN）与有限元分析的方法，用于模拟双材料系统中的瞬态弹性动力学波动传播。


<details>
  <summary>Details</summary>
Motivation: 旨在克服传统数值方法在计算高变率固体力学和冲击工程中异质结构波动传播时的计算瓶颈，建立高效的代理模型。

Method: 开发了PINN框架，将轴对称线性弹性动力学方程、初始条件、边界及双材料界面条件融入损失函数，并结合有限元模拟数据进行约束训练。

Result: 模型准确预测了波在界面的反射与透射、位移及应力应变演化，并对未见时间步和不同材料属性表现出良好的泛化外推能力。

Conclusion: 结合物理信息神经网络与显式有限元分析的框架，为异质固体弹性动力学波动分析提供了一种鲁棒且高效的代理建模方法。
...
TL;DR: 本文提出利用LLM智能体的内部隐藏状态早期预测任务失败，并构建了可显著节省算力的中断级联机制。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在多步任务中常陷入注定失败的轨迹，导致在显现失败前浪费大量推理算力。

Method: 利用隐藏激活状态上的轻量级逐轮探针，结合无分布校准的逐轮中断级联，以优化推理终止决策。

Result: 在TextCraft任务上，该机制能满足90%-97%的召回率目标，并在90%的目标下分别节省了47.1%（Qwen-2.5-7B）和37.2%（Llama-3.2-3B）的推理算力，显著优于仅使用行为特征的方法。

Conclusion: 智能体的内部状态比外在行为更早且更有效地反映失败迹象，结合合理的级联设计可在实际部署中安全、大幅地降低算力开销。
...
### [52] [Multi-Agent Deep Reinforcement Learning for Multi Objective Battery Management in Dairy Farms](https://arxiv.org/abs/2607.06489)
*Marcos Eduardo Cruz Victorio,Karl Mason*

Main category: cs.AI

TL;DR: 提出一种结合差分进化与多智能体深度强化学习的控制系统，用于优化爱尔兰乳制品行业的分布式可再生能源管理，可提升达18%的套利利润并确保电网合规。


<details>
  <summary>Details</summary>
Motivation: 爱尔兰乳制品行业在整合可再生能源和碳减排方面潜力巨大，但现有的分布式发电控制研究主要集中在住宅和商业领域，缺乏针对乳制品行业的研究。

Method: 提出一种双层多目标优化控制系统：上层利用动态定价，下层基于多智能体深度强化学习（DRL）进行电池管理，并结合差分进化算法在农村配电回路中进行仿真模拟。

Result: 仿真表明，该控制框架较传统规则模型提升了18%的能量套利利润，在不显著增加成本的前提下提高了分布式发电的利用率，且电压变化符合爱尔兰电网标准。

Conclusion: 该研究提出的控制框架能有效促进乳制品行业的可再生能源整合，实现了经济效益、能源利用率和电网安全合规性的多目标平衡。
...
### [53] [RMISC: A Large-scale Real-world Multivariate Corpus for Time Series Foundation Models](https://arxiv.org/abs/2607.06504)
*Qian Sun,Yong-Ming Tian,Jia-Wei Huang,Cheng Feng,Shao-Qun Zhang*

Main category: cs.AI

TL;DR: 本文评估了真实世界多元时间序列数据对时间序列基础模型（TSFM）零样本泛化能力的影响，并构建了大规模真实世界数据集 RMISC。


<details>
  <summary>Details</summary>
Motivation: 现有模型大多基于合成数据预训练，难以捕获真实世界复杂的时序动态与跨变量关系，需探究真实数据对模型性能的提升作用。

Method: 构建了包含1420亿时间点的真实多元时间序列语料库 RMISC，并基于不同数据源预训练了四种 TSFM，在分布内和分布外基准上评估其零样本泛化能力。

Result: 实验表明，引入真实世界的多元数据能显著提升单变量和多变量 TSFM 的零样本泛化性能。

Conclusion: 真实世界的多元数据对于开发更强大的时间序列基础模型至关重要。
...
TL;DR: FreqDepthKV 是一种无需重训的推理期 KV 缓存压缩方法，通过将相邻层 KV 状态分解为共享的低频深度分量和稀疏的高频残差，实现 3.9 倍有效压缩并保持高精度。


<details>
  <summary>Details</summary>
Motivation: 长文本 LLM 推理受限于 KV 缓存的显存和带宽成本，而激进的压缩会破坏检索和多步推理所需的层特定关键信息。

Method: 提出 FreqDepthKV 方法，将相邻层 KV 状态因式分解为共享低频深度分量和稀疏高频残差，并利用轻量级在线探针根据注意力对数贡献将注意力头自适应分配到共享、残差或精确缓存模式。

Result: 在 32k 窗口下，该方法几乎无损地接近全量 KV 性能（58.3 EM, 63.0 F1等），同时实现 70.4 tokens/s 的解码吞吐量、降低首字延迟（TTFT）至 2.06 秒，降低峰值 KV 显存至 6.2 GB（3.9倍压缩）。

Conclusion: FreqDepthKV 在显著降低显存预算和提升推理效率的同时，有效保留了长文本任务的准确性，证明了基于深度频率分解的自适应压缩的可行性。
...
### [55] [FootsiesGym: A Fighting Game Benchmark for Two-Player Zero-Sum Imperfect-Information Games](https://arxiv.org/abs/2607.06514)
*Chase McDonald,Nathan Tsang,Wesley N. Kerr*

Main category: cs.AI

TL;DR: FootsiesGym 是一个开源、高吞吐量的强化学习环境，基于 2D 格斗游戏设计，旨在研究格斗游戏中的复杂博弈与策略互动。


<details>
  <summary>Details</summary>
Motivation: 格斗游戏研究缺乏一个既能保留核心策略互动（如循环和非传递性博弈），又足够轻量、易于在标准硬件上进行高效分析的实验平台。

Method: 基于极简格斗游戏《Footsies》构建了 FootsiesGym，开发了支持高吞吐量训练的向量化模拟器，并对多种强化学习算法进行了基准测试。

Result: 成功实现了一个高效、可复现的强化学习环境，并提供了多种 RL 算法的基准测试数据，代码已完全开源。

Conclusion: FootsiesGym 为研究非完美信息零和博弈中的复杂战略互动提供了一个简单且高效的平台，开辟了新的研究方向。
...
### [56] [Bridging Physical Reasoning and Task Generalization via Visual Action Outcome Reasoning Alignment](https://arxiv.org/abs/2607.06522)
*Han-Jun Ko,Jr-Jen Chen,Haobo Yuan,Hsin-Ying Lee,Tiancheng Shen,Ming-Hsuan Yang,Yu-Chiang Frank Wang*

Main category: cs.AI

TL;DR: 本文提出VAORA，通过视觉对齐和视觉-动作对齐双重奖励设计，解决了多模态模型在物理交互推理中的幻觉与动作失调问题。


<details>
  <summary>Details</summary>
Motivation: 多模态语言模型在未见物理交互任务中泛化困难，主要由于推理过程产生物理幻觉，且推理与实际执行的动作存在偏差。

Method: 引入视觉对齐奖励（锚定视觉上下文）和视觉-动作对齐奖励（立足动作结果），并利用预训练专家估计成功概率以提供平滑稠密奖励。

Result: 在PHYRE和Virtual Tool基准测试中，VAORA在面对新任务和未见环境时均取得了显著的性能提升。

Conclusion: VAORA能有效抑制幻觉推理，缩小推理与动作之间的差距，实现具身且具泛化性的物理智能。
...
### [57] [DepthWeave-KV: Token-Adaptive Cross-Layer Residual Factorization for Long-Context KV Cache Compression](https://arxiv.org/abs/2607.06523)
*Anna Cordoba,Adam Puente Tercero,Nerea Angulo Hijo,Mar Linares Tercero,Julia Barrientos,Ainhoa Miranda,Jesus Olivera*

Main category: cs.AI

TL;DR: 提出 DepthWeave-KV，一种针对长文本大模型推理的令牌自适应 KV 缓存压缩方法，在大幅降低内存占用的同时保持高精度检索与生成。


<details>
  <summary>Details</summary>
Motivation: 长文本推理受到 KV 缓存内存带宽和容量的限制，而现有均匀压缩方法在处理不同重要性的层或令牌时容易导致检索性能下降。

Method: 采用跨深度残差因子化与令牌条件深度路由器，结合在线误差追踪对关键令牌分配更高的重构秩，并设计了融合 CUDA 算子以优化解码内存流量。

Result: 在多项长文本基准测试中表现优异，实现了高达 8.3 倍的 KV 内存缩减，并在 64K 上下文下达到每秒 72.8 令牌的生成速度。

Conclusion: DepthWeave-KV 在无需微调原模型的情况下，通过精细化的自适应压缩有效解决了长文本推理的 KV 缓存瓶颈问题。
...
### [58] [The Large Cancer Assistant (LCA): A Model-Agnostic Orchestration Framework for Scalable Clinical Decision Support in Oncology](https://arxiv.org/abs/2607.06531)
*Ghassen Marrakchi,Basarab Matei*

Main category: cs.AI

TL;DR: 本文提出了一种名为大癌症助手（LCA）的模型无关、事后编排框架，旨在解决多模态肿瘤学AI模型设计过于单一且不灵活的问题。


<details>
  <summary>Details</summary>
Motivation: 当前的肿瘤学多模态深度学习模型设计过于紧密耦合，限制了数据摄入、临床路由和AI推理的灵活性与可扩展性。

Method: 提出了基于算法不渗透性原则的7元组架构LCA，结合几何深度学习标准化数据，并通过癌症切换模块和标准化中间有效载荷（SIP）实现核心AI与医院IT系统的隔离。

Result: PoC验证显示，该框架运行开销极低，在更换AI模型时保持路由不变，且在数据异常时实现了100%的辅助数据请求召回率。

Conclusion: LCA通过解耦数据摄入与特征推理，提供了一个高适应性的模块化编排基础，其SIP设计为未来电子病历（EMR）的互操作性奠定了基础。
...
### [59] [Rethinking Indic AI from a Lens of Cultural Heritage Preservation](https://arxiv.org/abs/2607.06544)
*Aparna Madva,Sharath Srivatsa,Srinath Srinivasa,Tulika Saha*

Main category: cs.AI

TL;DR: 本文探讨了人工智能对印度语言和文化的影响，回顾了印度语支自然语言处理（Indic NLP）的发展历程，并提出了一种名为“文化感知”（Culture Sensing）的新研究方向。


<details>
  <summary>Details</summary>
Motivation: 人工智能的发展可能使弱势语言和多元文化世界观面临被边缘化或同质化的风险，且印度语言具有复杂的结构与社会语言学特征，给AI基础模型的构建带来独特挑战。

Method: 通过对印度语支NLP发展史进行纵向文献调研，分析现有基础模型在解决资源与表示差距上的作用，并提出基于诠释学推理的“文化感知”研究框架。

Result: 系统阐述了印度语言的独特挑战，评估了当前基础模型的应对表现，并确立了“文化感知”在解决低资源语言性能公平性及生成文化相关内容方面的可行路径。

Conclusion: 本文为未来印度语支NLP的研究指明了方向，有助于推动开发更具鲁棒性、包容性且契合文化背景的印度语支AI基础模型。
...
TL;DR: 本文提出了一个用于复杂图像创建和编辑的视觉工具使用数据集CanvasCraft，以及一个能够多轮交互并协调多种工具的智能体CanvasAgent。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态工具智能体主要针对感知和搜索进行了优化，缺乏在大规模可执行图像创建轨迹上的监督，难以应对需要多种工具协同的复杂图像编辑任务。

Method: 引入了包含14万条可执行轨迹的数据集CanvasCraft，并设计了CanvasAgent。CanvasAgent先通过监督微调（SFT）学习轨迹，再使用GRPO算法配合结合了结果和过程信号的混合奖励进行强化学习优化。

Result: 实验评估了最终图像质量和轨迹行为，验证了CanvasAgent和CanvasCraft数据集在复杂多工具图像创建工作流中的有效性。

Conclusion: CanvasCraft数据集和CanvasAgent智能体有效推动了多模态智能体从“感知型推理”向“操作型视觉创建”的转变。
...
### [61] [A Task-Driven Evaluation of UAV Detection and Tracking under Synthetic Fog](https://arxiv.org/abs/2607.05467)
*Amir Pouladi,Vesal Ahsani,Haijun Li,Homayoun Najjaran,Afzal Suleman*

Main category: cs.CV

TL;DR: 本文提出了一个任务驱动的评估框架，系统分析了合成雾和图像去雾对无人机目标检测与跟踪性能的影响。


<details>
  <summary>Details</summary>
Motivation: 雾天会严重降低远距离无人机的可见度，且实际采集和标注雾天无人机图像十分困难，需要研究去雾算法对下游实际感知任务的影响。

Method: 利用单目深度估计和大气散射模型生成合成雾，对比了经典、CNN及Transformer去雾方法，并在多种训练策略下测试了去雾对检测和追踪性能的提升效果。

Result: 雾天显著降低了检测与追踪性能；包含雾天的训练能最有效提升模型鲁棒性，而测试期去雾在检测器仅用晴天数据训练时效果最好；去雾质量的提升并不等同于下游任务性能的等比例提升。

Conclusion: 去雾算法的质量评估不应局限于图像指标，而应与下游的检测和跟踪性能进行联合评估。
...
### [62] [Binocular Gaze Estimation with Single Camera and Single Light Source](https://arxiv.org/abs/2607.05473)
*Tongbing Huang,Yang Fu,Yunfei Wang,Zhaocan Wang*

Main category: cs.CV

TL;DR: 提出了一种仅使用单个相机和单个光源的视线估计方法，通过引入“虚拟光源”技术实现在简化硬件条件下的视线追踪。


<details>
  <summary>Details</summary>
Motivation: 在移动设备等应用场景中，需要减少视线追踪系统的硬件组件（尤其是光源数量）以节省空间和功耗。

Method: 引入与真实光源对称的“虚拟光源”并估算图像中的“虚拟闪烁”，利用双瞳距离与闪烁距离的关系，结合改进归一化因子的多项式回归算法进行视线估计。

Result: 该方法达到了可接受的视线估计精度，虽然相比双真实光源系统性能有所下降，但验证了新型归一化因子在单闪烁系统中的实用性。

Conclusion: 采用单相机和单光源的视线追踪方案在物理上是可行且实用的，为便携式设备的视线估计提供了一种有效的低成本替代方案。
...
### [63] [Light-Omni: Reflex over Reasoning in Agentic Video Understanding with Long-Term Memory](https://arxiv.org/abs/2607.05511)
*Chang Nie,Jiaju Wei,Junlan Feng,Chaoyou Fu,Caifeng Shan*

Main category: cs.CV

TL;DR: 本文提出Light-Omni，一种通过双重上下文状态实现反射式和轻量级长视频理解的多模态智能体框架。


<details>
  <summary>Details</summary>
Motivation: 现有的视频智能体依赖于迭代推理进行动作控制和证据聚合，导致计算成本和延迟过高，这主要是由于缺乏全局上下文和检索语义不匹配造成的。

Method: 设计了双重上下文状态：1) 全局状态，通过分层合并片段内存构建多模态脚本；2) 参数化隐状态，基于全局上下文直接驱动动作并生成检索嵌入，避免了迭代推理。

Result: 在多个视频基准测试中，Light-Omni超越了M3-Agent，平均准确率提升2.4%，速度提升12.1倍，GPU内存效率提升2.6倍，并能有效增强现有的大型多模态模型（MLLM）。

Conclusion: Light-Omni证明了通过构建全局和隐式上下文状态，可以在无需重度迭代推理的情况下，实现高效、低延迟且准确的视频理解和智能体决策。
...
### [64] [Ground3D-LMM: Fine-Grained 3D Point Grounding and Spatial Reasoning with LMM](https://arxiv.org/abs/2607.05493)
*Amol Harsh,Zongyan Han,Jean Lahoud,Ye Liu,Rao Muhammad Anwer,Hisham Cholakkal,Salman Khan,Fahad Khan*

Main category: cs.CV

TL;DR: 本文提出了 Ground3D-LMM，一个能够同时进行 3D 空间定位和物理度量（如尺寸、距离）的统一 3D 多模态大模型。


<details>
  <summary>Details</summary>
Motivation: 现有的 3D 多模态模型要么在对话中缺乏明确的 3D 定位，要么无法进行交互式且感知度量的对话。

Method: 引入了 3D 定位度量任务，开发了 Ground3D-LMM 模型，并基于 ScanNet 和 ScanNet++ 构建了包含约 250 万个问答对的大规模数据集。

Result: 实验结果表明，Ground3D-LMM 在多个数据集和任务上表现优异，为具备定位与度量感知的 3D 对话理解提供了强有力的基线。

Conclusion: 该工作成功填补了 3D 空间定位与物理度量对话的空白，开源的模型和数据集将推动 3D 场景理解的发展。
...
### [65] [Rendering-Aware Bayesian 3D Gaussian Splatting with Native Uncertainty and Adaptive Complexity Control](https://arxiv.org/abs/2607.05522)
*Gaoxiang Jia,Vikram Appia,Junzhou Huang,Xinlei Wang*

Main category: cs.CV

TL;DR: 提出了一种贝叶斯3D高斯泼溅（3DGS）框架，通过Normal-Inverse-Wishart后验实现原生不确定性估计，有效提升了主动视角选择任务的性能。


<details>
  <summary>Details</summary>
Motivation: 传统的3DGS依赖点估计，缺乏原生不确定性评估和复杂度控制，限制了其在稀疏视角或有限预算场景（如主动视点选择）下的应用。

Method: 使用正态-逆威沙特（NIW）后验来追踪高斯均值和协方差，并结合狄利克雷过程进行组件控制，通过对后验几何进行采样与重新渲染来生成预测不确定性。

Result: 在主动视点选择任务中，相比集成基线，该方法将PSNR提升了+0.453 dB，且覆盖误差比深度集成低10倍，训练时间仅增加1.6%，成本仅为集成的三分之一。

Conclusion: 该方法证明了贝叶斯3DGS是一种实用且高效的概率场景表示，能够为主动视角选择等决策导向型任务提供可靠的不确定性度量。
...
### [66] [Statistical Adversaries: Natural Backdoor-like Features in Vision Datasets](https://arxiv.org/abs/2607.05516)
*Paul K. Mandal,Pavan Reddy,Tristan Malatynski*

Main category: cs.CV

TL;DR: 研究揭示了视觉数据中自然存在的统计信号（统计对抗样本）可以像后门触发器一样误导模型，且无需恶意投毒。


<details>
  <summary>Details</summary>
Motivation: 探索非恶意植入的、自然存在于数据中的统计信号是否会成为类似后门的对抗性触发器。

Method: 分析ImageNet中与标签强相关的模式，通过统计控制消除随机相关性，并评估其对不同模型预测的影响。

Result: 这些统计对抗信号能够直接、可预测地改变模型预测，且具有跨模型架构的迁移性。

Conclusion: 普通数据集即使未被投毒也存在可利用的对抗隐患，数据集审计应将这些伪结构视为潜在的攻击面。
...
### [67] [Multi-Teacher Contrastive Distillation for Edge-Efficient Pathology Foundation Models](https://arxiv.org/abs/2607.05533)
*Tim Lenz,Maurice Heide,Marco Gustav,Nic G. Reitsam,Jakob Nikolas Kather*

Main category: cs.CV

TL;DR: 本研究提出MuCoDi蒸馏框架，将大型病理学基础模型（PFMs）的特征提炼至轻量级边缘端网络中，在大幅降低参数量和计算成本的同时保持了接近大模型的性能。


<details>
  <summary>Details</summary>
Motivation: 计算病理学基础模型（PFM）由于体积庞大且推理成本高昂，难以在本地病理科室及边缘设备上进行实际部署。

Method: 提出MuCoDi框架，利用源自MoCo v3的对比蒸馏目标，将多个主流PFM（如Virchow2、UNI2等）的特征蒸馏到轻量级的MobileOne和RepViT学生网络中。

Result: RepViT基础的学生网络在参数量减少数个数量级的情况下，取得了与教师模型相近的性能（如MuCoEdge-R1.0仅需6.4M参数即可达到70.9% AUROC）。在树莓派5上，轻量级学生模型实现了比大模型高出605倍的推理加速。

Conclusion: 该方法证明了高质量的病理学表征可以被成功迁移至紧凑的边缘设备上，为病理基础模型的实际本地部署提供了可行方案。
...
### [68] [Harnessing Generative Image Models for Training-Free Primitive Shape Abstraction](https://arxiv.org/abs/2607.05568)
*Gregor Kobsik,Tim Elsner,Leif Kobbelt*

Main category: cs.CV

TL;DR: 提出一种无需训练的方法，通过多视图渲染和预训练生成模型，将3D形状分解并拟合为紧凑的几何基元。


<details>
  <summary>Details</summary>
Motivation: 旨在探索能否在不进行微调的情况下，直接利用预训练视觉生成模型来解决3D几何基元拟合在跨类别和旋转不变性上的局限性。

Method: 通过多视图渲染，结合视觉语言模型（VLM）进行语义分析和生成模型进行分割掩码绘制，最后将分割结果投影回3D空间并优化拟合超二次体基元。

Result: 在HumanPrim和Toys4K数据集上取得了最低的Chamfer距离（平均每个物体仅需5-9个基元），并证实当前的性能瓶颈主要在于语义分割而非基元拟合。

Conclusion: 该无需训练的框架具有优秀的类别无关和旋转不变特性，且其准确率上限将随着未来生成式图像模型的提升而持续增长。
...
### [69] [Hierarchical Classification via Cascading Feature Elimination: Application to Human Phenotype Ontology-Aligned Facial Phenotyping (FaceMesh2HPO)](https://arxiv.org/abs/2607.05585)
...
TL;DR: FaceMesh2HPO是一个结合3D面部网格与层次化PointNet的模型，旨在实现与人类表型本体（HPO）对齐的面部表型分类以辅助临床诊断。


<details>
  <summary>Details</summary>
Motivation: 旨在通过对与HPO对齐的面部表型描述符进行分类，来支持和辅助临床诊断。

Method: 利用2D图像生成含有478个关键点的3D面部网格，并训练了基于PointNet的层次化分类与特征消除管道。

Result: 模型实现了约0.55至0.89的AUROC，父节点的分类性能优于具体的叶子节点，且外部验证显示出不同的泛化能力。

Conclusion: 层次化3D面部几何建模能实现可解释的表型分类，但稀有表型性能受限，未来需提升数据多样性与特征选择。
...
### [70] [Patch Knowledge Transfer for Efficient AI-Generated Image Quality Assessment](https://arxiv.org/abs/2607.05605)
*Jiquan Yuan*

Main category: cs.CV

TL;DR: 本文提出了一种基于知识蒸馏的补丁知识转移（PKT）框架，旨在协同优化AI生成图像质量评估的精度与推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有的生成图像质量评估方法在计算成本（复杂特征提取导致无法实时推理）与评估精度（简单缩放导致精度不足）之间难以兼顾。

Method: 设计了一种双模型架构，通过多级知识蒸馏机制，用包含局部-全局混合处理的教师模型监督仅采用全局处理的学生模型，实现知识转移。

Result: 在4个AIGIQA数据库上的实验表明，该方法在保持与教师模型相当性能的前提下，减少了67.7%的计算成本，实现了效率与精度的最佳平衡。

Conclusion: PKT框架成功突破了生成图像质量评估中高精度与高效率无法兼得的瓶颈，为大规模实时图像评估提供了有效的解决方案。
...
### [71] [Taxlifier: Leveraging Disease Taxonomy for Enhanced Multi-Label Classification in Chest Radiography](https://arxiv.org/abs/2607.05628)
*Mohammad S. Majdi,Jeffrey J. Rodriguez*

Main category: cs.CV

TL;DR: 本文提出了两种基于层级关系的多标签分类方法，显著提升了胸部X光图像多病变分类的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 胸部X光图像中多种病变并存且视觉特征重叠，导致传统自动分类系统难以准确识别。

Method: 设计了基于损失（loss-based）和基于对数几率（logit-based）的两种分层多标签分类技术，利用疾病分类学中的层级关系优化模型训练和概率预测。

Result: 在CheXpert、PADCHEST和NIH数据集上，Logit和Loss方法相比基线分别将分类准确率提升了12%和11%，AUC和F1分数也均有显著改善。

Conclusion: 结合特定领域的层级知识不仅能显著提高分类性能，还能为临床决策支持提供更具可解释性的输出。
...
### [72] [Cross-Contextual Vision-Language Adaptation with LoRA for Personalized Severe Adverse Event Detection in Clinical Wound Monitoring](https://arxiv.org/abs/2607.05625)
*Aditi Naiknaware,Jian Sun,Aminreza Khandan,Shengyang Huang,Sean Dow,Bijan Najafi,Salimeh Sekeh*

Main category: cs.CV

TL;DR: 本文提出了一个用于自动伤口监测和严重不良事件（SAE）检测的多模态框架，结合了跨上下文LoRA融合与分布外（OOD）检测机制。


<details>
  <summary>Details</summary>
Motivation: 临床伤口监控至关重要，但现有视觉语言模型（VLM）缺乏领域特定的接地能力来整合图像与临床信息，且无法有效检测偏离训练分布的异常（SAE）病例。

Method: 使用基于冻结BiomedCLIP的双流LoRA框架，引入跨上下文LoRA融合机制来交互临床语义与视觉描述；提出一种结合语义、视觉典型性及对齐分数的伤口特异性OOD检测框架，并引入时间漂移惩罚以捕捉愈合动态。

Result: 在纵向伤口数据集上的实验表明，该系统在伤口愈合评估和SAE（如感染、恶化等）检测上均表现出优异的性能。

Conclusion: 该研究展示了语义丰富且具备时间感知能力的视觉语言系统在临床伤口监测和早期风险识别中的巨大潜力。
...
### [73] [VEIL: How Visual Encoding Hijacking Induces Bias In Vision Models](https://arxiv.org/abs/2607.05641)
*Suranjana Sooraj,Xuyang Chen,Madhumitha Venkatesan,Dongyu Liu*

Main category: cs.CV

TL;DR: 本文探讨了将时间序列渲染为图表进行分类时，图表设计如何影响CNN模型的学习表征。


<details>
  <summary>Details</summary>
Motivation: 尽管将时间序列转化为图表进行分类日益普遍，但目前尚不清楚模型是学习了底层的时序模式，还是依赖于图表设计所引入的视觉线索。

Method: 提出了VEIL框架，通过相似性、可迁移性和归因的互补分析，系统研究图表编码对学习表征的影响，并评估了注意力引导训练的缓解效果。

Result: 研究发现图表设计选择会塑造学习到的表征，而注意力引导训练在编码敏感性明显时能减轻该效应，但在无明显信号时效果有限或起到反作用。

Conclusion: 图表设计选择会显著改变模型特征，因此基于图表的时间序列分类应被视为表征和测量问题，而非单纯的建模决定。
...
### [74] [Recovering Cloud Microstructures with Cascaded Diffusion Inversion](https://arxiv.org/abs/2607.05637)
*Hanan Gani,Guy Pulik,Daniel Rosenfeld,Duncan Watson-Parris,Salman Khan*

Main category: cs.CV

TL;DR: 本文提出了一种双阶段扩散超分辨率框架，用于提升多光谱卫星云图分辨率，以清晰观测用于人工增雨决策的微细云结构。


<details>
  <summary>Details</summary>
Motivation: 现有卫星空间分辨率不足以捕捉细微云特征，且针对自然图像的传统超分辨率方法难以推广至卫星多光谱云图。

Method: 提出一种双阶段扩散模型进行4倍超分辨率重建：第一阶段利用真实配对数据学习退化处理与对齐，第二阶段通过自监督内部降级精细化结构与纹理合成。

Result: 实验结果表明，该方法在重建精度和视觉质量上均优于现有的Transformer和扩散模型基线，能更有效捕捉对流塔和云隙等微细结构。

Conclusion: 该研究为改进云微物理分析提供了实用方案，是利用人工智能助力气候和可持续发展的重要一步。
...
### [75] [REVIVE: A Multi-Modal Framework for Vandalism Detection and Recovery in Autonomous Vehicles](https://arxiv.org/abs/2607.05649)
*Abdullah Tariq Choudhry,Tapadhir Das*

Main category: cs.CV

TL;DR: 本文提出了REVIVE框架，旨在检测并恢复自动驾驶车辆中因故意遮挡攻击（VOA）而受损的相机图像，从而保障下游视觉感知任务的性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆面临物理遮挡攻击（如涂鸦、遮挡等）的威胁，虽然目前可以检测到此类破坏，但如何有效恢复受损图像以维持感知系统运转的研究仍较为匮乏。

Method: 开发了REVIVE框架，整合了二进制检测、多类别遮挡模式识别、基于EfficientNet的U-Net分割，以及由BLIP引导的Stable Diffusion修补、直接像素替换和自适应中值滤波组成的类型感知恢复机制，并引入了异步恢复与质量门控机制。

Result: 实验表明，在对齐参考条件下，直接像素替换将YOLOv8l的目标检测召回率从0.588提升至0.967，F1分数提升至0.970；质量门控机制可防止恢复恶化，使召回率保持在0.608以上。

Conclusion: REVIVE框架提供了一种应对自动驾驶相机物理遮挡的高效恢复方案，确保了在遭受攻击后系统的感知能力不会进一步恶化。
...
### [76] [Clustered Codebook Quantization for 2D Gaussian-based Image Compression](https://arxiv.org/abs/2607.05667)
*Runze Cheng,Yicheng Zhan,Josef Spjut,Kaan Akşit*

Main category: cs.CV

TL;DR: 提出一种基于高斯基元的图像压缩方法CGVQ，通过在量化前将高斯参数分组来提高率失真性能，在保持视觉质量的同时减少了20%的bpp。


<details>
  <summary>Details</summary>
Motivation: 高斯图像表示中每个基元存储大量浮点参数，导致在高保真度目标下率失真效率下降。

Method: 提出聚类引导矢量量化（CGVQ），在量化前将高斯参数进一步划分为同质组，以实现更高的压缩效率和准确的参数重建。

Result: 实验表明，与基线相比，CGVQ在保持同等视觉质量的情况下，将bpp降低了20%。

Conclusion: CGVQ通过对高斯参数进行分组和矢量量化，有效改善了基于高斯表示的图像压缩率失真性能。
...
### [77] [Scene Graph Thinking: Reinforcing Structured Visual Reasoning for Multimodal Large Language Models](https://arxiv.org/abs/2607.05716)
*Zhiwei Yang,Yuanchen Wu,Nan Zhang,Yucong Meng,Ke Yan,Shouhong Ding*

Main category: cs.CV

TL;DR: 提出了Scene Graph Thinking (SaGe)架构，通过显式场景图表示来提升多模态大模型的细粒度和结构化推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有模型大多只关注孤立物体，忽视了对于高效目标导航等视觉密集型任务至关重要的结构化关系。

Method: 开发自动数据引擎将图文语料转化为场景图并构建120K推理数据集，再通过两阶段后训练（监督微调和基于节点代理图奖励的强化微调）使模型实现图对齐推理。

Result: 在八个多模态基准测试中均取得了显著的性能提升，表现出强大的细粒度感知与推理能力。

Conclusion: SaGe通过结合精细场景图数据与图对齐训练范式，有效增强了多模态大模型在复杂视觉任务中的结构化探索与推理能力。
...
### [78] [Association Restoration Test: Revealing Restorable Shortcuts after Unlearning](https://arxiv.org/abs/2607.05726)
*Amy Lu,Changxiu Ji*

Main category: cs.CV

TL;DR: 本文提出了一种名为 ART 的后验诊断方法，用于评估分类器中残留的快捷关联（shortcut associations）是否仍能被功能性地恢复。


<details>
  <summary>Details</summary>
Motivation: 现有的评估方法（如输出鲁棒性或表征探测）无法确定在进行关联去学习后，分类器中残留的快捷关联是否仍能被其分类头功能性地利用。

Method: 提出关联恢复测试（ART），该方法通过估计类条件关联方向、放大残留特征成分，并使用原始分类头对修改后的特征进行评估。

Result: 在 Waterbirds、CelebA、SpuCoDogs 等多个数据集上的实验表明，输出指标、表征探测和 ART 刻画了快捷方式缓解的不同维度，证明了看似被消除的关联仍可被功能性恢复。

Conclusion: 研究结果表明有必要采用“恢复感知”的评估方法，并推动针对学习到的关联（而非单一类别或概念）的去学习与快捷方式缓解方法的发展。
...
### [79] [Robust Face Super-Resolution and Recognition Through Multi-Feature Aggregation in Diffusion Models](https://arxiv.org/abs/2607.05702)
*Marcelo dos Santos,Rayson Laroca,João Carlos Raposo Neves,David Menotti*

Main category: cs.CV

TL;DR: 提出FASR++，一种基于扩散模型的人脸超分辨率算法，利用多张辅助低质量图像提升监控场景下的面部识别率并减少身份畸变。


<details>
  <summary>Details</summary>
Motivation: 监控环境下的图像由于低分辨率和遮挡等问题导致人脸识别困难，而现有超分辨率算法易导致身份失真，需引入多帧图像的辅助信息以增强鲁棒性。

Method: 引入FASR++算法，该算法基于扩散模型，利用参考低分辨率图像及多张辅助低质量图像提取的特征引导重建，无需显式软属性或梯度引导。

Result: 在两个标准数据集上取得了最先进的（SOTA）人脸识别、验证结果，以及优异的图像质量指标（PSNR、SSIM、LPIPS）。

Conclusion: FASR++作为预处理步骤，能有效生成高质量面部图像，并在保持身份特征的同时显著提升人脸识别系统的性能。
...
### [80] [SAMPLe: SAM-based Optimizer for Prompt Learning in VLMs](https://arxiv.org/abs/2607.05727)
*Hossein Rajoli,Fatemeh Lotfi,Niloufar Alipour Talemi,Hossein Kashiani,Xiaolong Ma,Fatemeh Afghah*

Main category: cs.CV

TL;DR: 本文提出SAMPLe，一种即插即用的敏锐度感知优化器，旨在解决视觉语言模型提示学习中性能与泛化性不可兼得的困境。


<details>
  <summary>Details</summary>
Motivation: 提示学习（Prompt Learning）在视觉语言模型（如CLIP）中容易出现过拟合，导致在已知分布上准确率高但对未知数据的泛化能力下降。

Method: 提出SAMPLe优化器，通过考虑损失景观（loss landscape）的平坦度，并在每步优化中动态适应局部曲率和梯度属性，平衡探索与开发以抑制过拟合。

Result: 将SAMPLe应用于CoOp、CoCoOp、MaPLe等多种提示学习框架，实验表明其表现一致优于现有优化器，有效保留了模型的泛化潜力。

Conclusion: SAMPLe是一种鲁棒且与模型无关的优化解决方案，能有效克服提示学习中的泛化瓶颈。
...
### [81] [Optimized Adaptive Loop Filter in Versatile Video Coding](https://arxiv.org/abs/2607.05737)
*Meng Xuewei,Zhang Jiaqi,Jia Chuanmin,Zhang Xinfeng,Wang Shanshe,Ma Siwei*

Main category: cs.CV

TL;DR: 本文提出了一种针对VVC标准的优化自适应环路滤波（ALF）框架，在极低编码性能损失下显著降低了计算复杂度和图像缓存访问次数。


<details>
  <summary>Details</summary>
Motivation: VVC中的自适应环路滤波（ALF，包括GALF和CCALF）虽然能有效减少压缩伪影，但其编码复杂度高且需要多次图像缓存访问，导致外部内存带宽压力大，不利于软硬件设计。

Method: 提出了一个优化的ALF框架，主要包括：1) GALF与CCALF的并行设计；2) GALF的自适应参数决策；3) 通过估算滤波失真而无需进行实际滤波操作的单通（one-pass）CCALF方案。

Result: 与VTM-8.0相比，该方法在随机接入（RA）配置下将图像缓存访问次数从152次减少至1次，并节省了约25%的ALF模块计算时间，且编码性能变化微乎其微。

Conclusion: 该研究成功解决了ALF模块的高内存访问和高复杂度问题，部分优化算法已被VVC标准参考软件采纳。
...
### [82] [ARMS: Anchor-Relational Motion Streaming for Seamless Solo-Social Motion Transitions](https://arxiv.org/abs/2607.05733)
*Huakun Liu,Qing Yu,Kent Fujiwara,Hideaki Uchiyama,Kiyoshi Kiyokawa*

Main category: cs.CV

TL;DR: 本文提出了ARMS框架，通过统一的因果生成过程实现单人运动与多人交互的无缝切换和长时序动作生成。


<details>
  <summary>Details</summary>
Motivation: 现有动作生成方法多局限于固定长度和静态社交配置，无法有效处理单人与交互状态之间的动态过渡，且难以进行长时序渐进式生成。

Method: 采用动态非对称表示解耦单人运动与人际对齐，利用因果关系扩散模型进行渐进式片段生成，并通过模式感知关系门控动态控制跨智能体的连接。

Result: 实验表明，该方法在状态过渡的平滑度和社交协同性上显著优于现有基准，并在人际交互基准测试中取得了极具竞争力的结果。

Conclusion: ARMS框架成功解决了长时序中单人动作与复杂交互动作的连续生成难题，为社交场景下的动作合成提供了有效方案。
...
### [83] [Image2Sim: Scaling Embodied Navigation via Generative Neural Simulator](https://arxiv.org/abs/2607.05765)
*Zihan Wang,Seungjun Lee,Yinghao Xu,Gim Hee Lee*

Main category: cs.CV

TL;DR: Image2Sim是一个实时神经模拟框架，能从RGB-D序列构建高质量交互式三维环境，用于具身导航训练。


<details>
  <summary>Details</summary>
Motivation: 现有的具身导航训练环境面临真实数据难以规模化扩展、合成模拟器存在严重虚实差距（sim-to-real gap）的问题。

Method: 解耦3D空间锚定与观测合成，采用前馈特征高斯模型构建3D空间，并使用几何感知单步像素流模型进行高保真全景渲染。

Result: 成功构建了近2万个交互式场景并合成超1000万个导航训练样本，显著提升了导航模型在基准测试及真实世界零样本迁移中的性能。

Conclusion: 可扩展的神经模拟能够为大规模具身导航模型的训练提供切实可行的实践基石。
...
TL;DR: 本文提出Legato 2，一种新型的乐谱光学识别（OMR）流水线，通过按行顺序处理图像来提取符号标记和语义知识。


<details>
  <summary>Details</summary>
Motivation: 传统的OMR方法将整页图像无差别处理，难以扩展到长输入，且无法生成包含标题和注释等嵌入文本的符号转录。

Method: 结合系统级分割与自回归视觉语言模型（vision-LM），模拟人类阅读习惯进行逐行（system-by-system）顺序识别。

Result: 在多个数据集上超越了先前的最先进技术（SOTA），并证明了其转录结果能辅助前沿语言模型更好地理解复杂乐谱文档。

Conclusion: Legato 2在OMR和下游乐谱理解领域均树立了新的性能标杆。
...
### [85] [Benchmarking the Robustness of Autonomous Driving to Environmental Illusions: A Lane Perception Perspective](https://arxiv.org/abs/2607.05783)
*Tianyuan Zhang,Xianglong Liu,Aishan Liu,Lu Wang,Yitong Zhang,Peng Yue,Mingchuan Zhang,Siyuan Liang,Dacheng Tao*

Main category: cs.CV

TL;DR: 本文提出了首个评估自动驾驶在环境幻觉（如阴影、反光等）下车道感知鲁棒性的基准LanEvil++，并提出了有效的防御方法MIDA。


<details>
  <summary>Details</summary>
Motivation: 现实中的环境幻觉（如阴影、反光和胎迹）会干扰自动驾驶系统的视觉感知并带来安全风险，但现有研究对此关注不足。

Method: 构建了LanEvil++基准（包含14种幻觉、利用CARLA生成的3D场景及多模态数据集），评估了传统车道线检测和视觉语言模型，并提出了多模态幻觉防御方法（MIDA）。

Result: 环境幻觉显著降低了感知性能，导致车道检测准确率下降5.27%，其中阴影影响最严重；所提MIDA方法成功将传统模型和视觉语言模型的鲁棒性分别提升了4.23%和3.82%。

Conclusion: 环境幻觉对自动驾驶的安全决策构成严重威胁，本研究提出的基准与防御方法为提升系统在复杂现实环境下的鲁棒性提供了重要支持。
...
### [86] [DeSeG: Decoupling Semantic Intent and Geometric Constraints for Physically Plausible Human-Scene Interaction](https://arxiv.org/abs/2607.05787)
*Jiakun Li,Zhe Li,Wenqiang Wu,Zheng Chang,Mingqi Gao,Jinyu Yang,Feng Zheng*

Main category: cs.CV

TL;DR: 本文提出了一种名为DeSeG的分层框架，旨在通过解耦语义意图与几何约束，解决人类-场景交互合成中的物理穿透与语义偏离问题。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在合成人类-场景交互时存在“语义-几何纠缠”问题，导致空间几何约束覆盖语义意图，并产生身体与场景穿透等物理不合理现象。

Method: 提出DeSeG框架，包含用于解耦文本指令与空间轨迹的残差语义规划器，以及在扩散目标中引入可微排斥势场的物理正则化扩散执行器。

Result: 在Lingo数据集上的实验表明，DeSeG达到了领先性能，比现有基线方法减少了47%的场景穿透，并提高了29%的语义对齐度。

Conclusion: 通过显式解耦语义意图与几何约束，并引入物理碰撞正则化，能够有效生成物理上可行且语义一致的人类-场景交互动作。
...
### [87] [Segmentation before Answering: Pixel Grounding for MLLM Visual Reasoning](https://arxiv.org/abs/2607.05798)
*Yake Wei,Yuan Wang,Fengyun Rao,Jing Lyu,Di Hu*

Main category: cs.CV

TL;DR: 本文提出SegAnswer方法，通过将多模态大模型的关注区域从边界框转为像素级分割掩码，提升了视觉语言推理的精确度。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大模型在视觉推理中通常使用边界框（bounding box）进行局部放大，这容易引入冗余的背景和干扰物体。

Method: 提出“先分割后回答”（SegAnswer）框架，将局部放大单元从边界框提升为像素级分割掩码，以隔离目标区域并更自然地与模型视觉Token结构对齐。

Result: 在超分辨率感知、通用感知和幻觉评估等多项基准测试中均取得了一致的性能提升，并在分割任务中展现出可靠的像素定位能力。

Conclusion: SegAnswer通过精细的像素级分割有效过滤了背景干扰，证实了高精度像素接地对提升多模态推理能力的有效性。
...
### [88] [TRIG: Trajectory-Rig Decoupled Metric Geometry Learning](https://arxiv.org/abs/2607.05801)
*Lizhou Liao,Wentao Xu,Handong Wang,Lirong Yang,Shuai Yang,Weiwei Liu,Chang Huang*

Main category: cs.CV

TL;DR: 本文提出TRIG框架，通过将相机位姿解耦为自我轨迹和相机阵列两部分，提升了多相机自动驾驶系统的度量几何学习性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型将随时间变化的车辆运动与静态的相机阵列几何耦合在一起，限制了车辆端几何先验的有效利用。

Method: 引入轨迹-相机阵列解耦的位姿编码与监督机制，并结合稀疏时空注意力机制以降低计算复杂度并保留几何推理能力。

Result: 在五个自动驾驶基准数据集上，该方法在位姿估计、度量深度预测和3D重建任务中均达到了SOTA性能。

Conclusion: 解耦车辆轨迹与相机阵列拓扑的建模方式，能够显著提高自动驾驶中多相机系统的几何感知与计算效率。
...
### [89] [Complementary Roles of Image Classification and Vessel Segmentation in AI-Based Screening for Retinopathy of Prematurity Plus Disease in a Kenyan Preterm Cohort](https://arxiv.org/abs/2607.05825)
...
TL;DR: 本研究评估了多种深度学习模型在肯尼亚早产儿视网膜病变（ROP）Plus病变检测中的表现，发现结合图像分类与血管分割的集成模型效果最佳。


<details>
  <summary>Details</summary>
Motivation: 中低收入国家缺乏专业的眼科医生，且Plus病变诊断主观性强，目前针对非洲人群的自动筛查研究和证据十分有限。

Method: 分析了121名肯尼亚早产儿的眼底图像，利用嵌套交叉验证评估了包括图像分类器、多实例学习以及分割分类耦合在内的11种模型配置。

Result: 结合分类与分割的概率集成模型取得了最佳的平衡性能，敏感度为0.692，特异性为0.914，平衡准确率为0.803，优于单一分类器。

Conclusion: 分类和血管分割方法在ROP检测中具有互补性，建议未来非洲的ROP AI筛查系统采用结合两者的混合工作流并进行多中心验证。
...
### [90] [Realistic Compound-Lens Defocus Blur Synthesis](https://arxiv.org/abs/2607.05837)
*Yunkyu Lee,Woohyeok Kim,Sunghyun Cho*

Main category: cs.CV

TL;DR: 本文提出了一种用于合成多镜头真实离焦去模糊数据集的管线，并构建了大规模数据集CLDefocus，提升了去模糊模型的跨设备泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习去模糊方法受限于训练数据集的光学多样性和真实性，导致模型在不同相机和镜头上的性能显著下降。

Method: 集成基于Debye CZT传播的波动光学PSF计算、带遮挡处理的深度感知离焦渲染，以及辐射线性空间下的模糊合成与相机ISP模拟。

Result: 成功构建了CLDefocus数据集，实验证明在该数据集上训练的模型在跨设备泛化表现上显著优于使用现有真实或合成数据集训练的模型。

Conclusion: 本研究表明，通过物理建模合成的多样化镜头离焦数据能有效克服现有真实数据集的缺陷与局限性，是提升去模糊算法泛化性能的关键。
...
TL;DR: 提出AVA-VLM，一种基于人类“由粗到细”推理策略的自适应视觉注意力视觉语言模型，用于高效且可靠的工地监控。


<details>
  <summary>Details</summary>
Motivation: 现有的工地监控VLM在操作范围、低分辨率输入的可靠性以及推理效率上存在局限性。

Method: 设计了AVA-VLM框架，使模型能先分析低分辨率全局图，并在需要时自适应请求高分辨率局部裁剪；同时引入了区域感知思维链数据集。

Result: 在远距离和低分辨率条件下显著提升了模型可靠性，并大幅减少了视觉Token的使用量。

Conclusion: 通过模拟人类“变焦”检查的机制，AVA-VLM成功平衡了工地监控中的检测精度与推理效率。
...
### [92] [Breaking Spurious Correlations via Generative Randomization and Cross-Variant Self-Supervised Learning](https://arxiv.org/abs/2607.05850)
*Suraj Yadav,Anjaneya Sharma,Siddharth Yadav*

Main category: cs.CV

TL;DR: 提出了一种结合生成式干预与自监督对比学习的双阶段框架，用以解决模型因背景伪相关而在分布偏移下失效的问题。


<details>
  <summary>Details</summary>
Motivation: ERM训练的模型易依赖背景等伪相关特征，且现有的生成对抗增强方法仍保留了背景敏感的表征，无法实现真正的背景不变性。

Method: 利用零样本分割与扩散模型生成同一物体的多背景变体，通过跨变体自监督对比学习对齐物体表征，最后通过ERM预热和GroupDRO微调模型。

Result: 在分布偏移基准测试中表现优异，Waterbirds上达到92.5%，MetaShift达到81.7%，NICO++达到87.4%的最差组准确率。

Conclusion: 该方法通过显式鼓励模型学习背景无关的物体中心表征，有效提升了模型在分布偏移环境下的泛化能力和鲁棒性。
...
### [93] [Few-Medoids: An Embarrassingly Simple Coreset Selection Method for Few-Shot Knowledge Distillation](https://arxiv.org/abs/2607.05891)
*Cemil-Andrei Dilmac,Florinel-Alin Croitoru,Radu Tudor Ionescu*

Main category: cs.CV

TL;DR: 本文提出了一种极简的核心集（coreset）选择策略 Few-Medoids，通过选择靠近类别质心的样本，在少样本知识蒸馏任务中持续超越随机选择和其他方法。


<details>
  <summary>Details</summary>
Motivation: 在少样本知识蒸馏（KD）场景下，现有的核心集样本选择策略往往难以超越简单的随机选择基线。

Method: 提出了 Few-Medoids 方法，该方法通过计算并选择每个类别中距离质心（平均图像）最近的样本来构建核心集。

Result: 在4个图像分类数据集以及包含CNN和Transformer的3个师生模型对上的实验表明，Few-Medoids 一致且显著地优于随机选择以及其他代表性的核心集选择策略。

Conclusion: Few-Medoids 极其简单且效果显著，可作为未来核心集选择研究中常用基线方法（如 herding 或 k-center Greedy）的直接替代方案。
...
### [94] [GaussFusion: Towards Multimodal 3D Gaussian Pretraining](https://arxiv.org/abs/2607.05906)
*Zhixuan You,Jihua Zhu,Yiding Sun,Zihao Guo,Haozhe Cheng,Dongxu Zhang,Lin Chen,Hainan Luo*

Main category: cs.CV

TL;DR: 本文提出了 GaussFusion，一个结合多模态监督与改进掩码策略的 3D Gaussian 表示预训练框架，提升了其在下游任务中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的 3D 高斯预训练方法主要捕捉局部结构，语义监督有限，且难以适应高斯基元的非均匀分布。

Method: 将图像和文本监督（跨模态语义对齐）引入掩码高斯建模，并设计了高斯显著性引导的多尺度空洞掩码（GSHM）来捕获多尺度空间依赖关系。

Result: 在下游任务中展现出更好的迁移性，在 ModelNet40 和 ScanObjectNN 数据集上分别超越 Gaussian-MAE 达 0.61% 和 3.85%。

Conclusion: GaussFusion 证明了多模态语义对齐与自适应掩码策略在提升 3D 高斯表示学习中的有效性。
...
### [95] [Harrison.Rad 1.5 Technical Report: A radiology foundation model that can draft reports from images, priors and clinical context](https://arxiv.org/abs/2607.05880)
*Suneeta Mall,Vladimir Nekrasov,Ashnil Kumar,Sajith Karunasena,Aiden Nibali,Alix Bird,Mateo Diaz Shine,Jarrel Seah*

Main category: cs.CV

TL;DR: 本文介绍了一种专为放射科设计的多模态大语言模型 Harrison.Rad 1.5 (HR1.5)，它能生成高质量的放射学报告，并在模拟放射科医师考试中达到通过标准。


<details>
  <summary>Details</summary>
Motivation: 医学影像需求快速增长导致报告积压，需要减少放射科医生撰写报告所需的时间和精力。

Method: 通过三阶段管线进行训练（领域自适应、基于~600万实例的对比视觉编码训练、多轮视觉问答微调），并使用结合本体匹配和极性矛盾检测的评分框架以及多维度基准进行评估。

Result: HR1.5在多项公开和内部基准测试中取得最高准确率，且是唯一通过模拟 FRCR 2B 考试标准的研究系统。

Conclusion: HR1.5 展示了放射学多模态大模型在自动生成临床报告和辅助诊断方面的巨大潜力，并为临床质量评估提供了新框架。
...
### [96] [Progressive Reasoning with Primitive Correction for Compositional Zero-Shot Learning](https://arxiv.org/abs/2607.05911)
*Ziyi Chen,Haoyan Shi,Sunhan Xu,Congyan Lang*

Main category: cs.CV

TL;DR: 本文提出了PRPC框架，通过渐进式推理与双向基元纠错，有效解决了组合零样本学习中的错误传播和上下文缺失问题。


<details>
  <summary>Details</summary>
Motivation: 现有的组合零样本学习（CZSL）方法要么独立预测属性和物体而忽略了上下文依赖，要么采用单向条件建模，这容易导致预测错误在步骤间传播。

Method: 提出基于多模态大模型的渐进式推理与基元纠错（PRPC）框架，将CZSL转化为结构化的问答式思维链推理，并引入基于GRPO的强化学习后训练以提供步骤级奖励。

Result: 在三个主要的CZSL基准数据集上进行的广泛实验表明，PRPC取得了当前最先进（SOTA）的性能表现。

Conclusion: 实验结果验证了渐进式推理和双向纠错机制对于实现鲁棒的组合泛化能力的有效性。
...
### [97] [PolicyShiftGuard: Benchmarking and Improving Policy-Adaptive Image Guardrails](https://arxiv.org/abs/2607.05910)
*Mingyang Song,Luxin Xu,Haoyu Sun,Minzhou Pan,Yu Cheng,Bo Li*

Main category: cs.CV

TL;DR: 本文研究了策略自适应的图像安全护栏，提出了基准测试 PolicyShiftBench 和训练方案 PolicyShiftGuard，显著改善了模型在动态策略下的安全判定能力。


<details>
  <summary>Details</summary>
Motivation: 现有的图像安全检测主要依赖固定策略，而实际应用中安全定义常随产品和政策调整而变化，需要模型具备策略自适应能力。

Method: 引入了 PolicyShiftBench 基准，并开发了 PolicyShiftGuard 模型，采用随机策略监督微调（RP-SFT）和边界对策略自适应（BP-Adapt）两阶段方法进行训练。

Result: 实验表明，PolicyShiftGuard 7B 在 PolicyShiftBench 上取得了 SOTA 性能（76.9 F1 和 72.1 PSS），并表现出良好的泛化能力和低延迟。

Conclusion: 匹配的通过/阻止边界对对于稳定策略自适应至关重要，PolicyShiftGuard 有效克服了现有模型在策略偏移时的脆弱性。
...
### [98] [NegROI: Click-Centric Uncertainty-Guided Refinement with Scene-Conditioned Negative Prompts for Robust Interactive 3D Segmentation](https://arxiv.org/abs/2607.05955)
*Shuheng Zhang,Feng Wu*

Main category: cs.CV

TL;DR: 本文提出了NegROI框架，通过多分辨率局部精细化和场景条件负向提示，提升了交互式3D点云分割的精度和跨数据集鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的交互式3D分割方法受限于粗糙的体素分辨率，导致边界模糊，且容易受到复杂背景结构干扰产生假阳性，尤其在面临跨数据集的密度和尺度变化时泛化性较差。

Method: 提出NegROI框架，采用点击中心的局部ROI多分辨率细化（结合不确定性驱动的选择性细化），并引入场景条件负向提示（结合多样性正则化与边界感知硬负样本挖掘）来建模复杂的背景模式。

Result: 在ScanNet、S3DIS和KITTI等主流数据集上的实验表明，该方法显著提高了点击效率，减少了假阳性，并展现出优于现有先进方法的跨数据集鲁棒性。

Conclusion: 通过将局部多分辨率精细化与场景负向提示相结合，NegROI有效克服了交互式3D分割中的边界模糊和背景干扰问题，实现了高效且鲁棒的分割。
...
### [99] [Decoupled Single-Mask Annotation Noise Detection via Cross-Sectional Patch Self-Consistency](https://arxiv.org/abs/2607.05965)
*Yinheng Zhu,Xiaowei Xu*

Main category: cs.CV

TL;DR: 本文提出一种基于横截面图像块自一致性的解耦框架，用于检测和审计血管CT单掩膜标注中的噪声，并揭示了系统性标注偏差。


<details>
  <summary>Details</summary>
Motivation: 血管CT数据集通常仅有单次标注，导致标注噪声普遍存在。现有去噪方法要么需要高成本的多人标注融合，要么与网络训练深度耦合，无法对标注错误的位置和原因进行显式审计。

Method: 提出一种利用横截面块自一致性的解耦检测框架。该方法沿血管中心线提取正交横截面块，通过向量搜索检索相似邻域，并基于统计掩膜差异计算块级噪声得分，从而为每个标记区域提供显式的图像-掩膜证据。

Result: 在冠状动脉CT数据集上的实验表明，所检测的噪声能有效提升训练鲁棒性，并揭示了系统性标注偏差：横向和倾斜血管的错误率是轴向对齐结构的5.1倍，且错误率与血管截面积及亮度相关。

Conclusion: 该方法通过利用管状解剖结构的重复性，实现了一种无需与训练耦合、可审计的血管标注噪声检测方法，有助于数据集质量评估和鲁棒性训练。
...
### [100] [Propose and Attend: Training-free MLLM Grounding Confidence via Multi-Token Localized Attention](https://arxiv.org/abs/2607.05978)
*Daniel Shalam,Emanuel Ben Baruch,Avi Ben Cohen,Tal Remez*

Main category: cs.CV

TL;DR: 提出了一种无需训练的后处理评分方法MTLA，通过分析预测区域内的注意力权重来有效评估并减少多模态大模型的定位幻觉。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLM）在定位物体或事件时极易产生幻觉，且模型自身的Token概率无法有效衡量定位的准确性。

Method: 提出多Token局部注意力（MTLA），这是一种无需训练的方法，通过在预测的局部区域内计算注意力并聚合所有预测Token的得分来评估定位可信度。

Result: 在三种模态和多个模型上，MTLA将幻觉检测的AUROC提升了+7至+38，并将8B通用模型在零样本COCO检测中的AP值从20.4翻倍提升至37.0。

Conclusion: MTLA提供了一种简单、免训练且跨模态通用的解决方案，能显著缓解多模态大模型的定位幻觉，缩小与监督学习检测器的差距。
...
### [101] [SpecTrack: Spectral Prompt Guided Adaptive Experts for Multispectral Object Tracking](https://arxiv.org/abs/2607.05988)
*Xingyu Tan,Yunrong Qin,Mengjie Hu*

Main category: cs.CV

TL;DR: 提出一种名为 SpecTrack 的多光谱/超光谱目标跟踪器，通过自适应分配光谱-空间计算复杂度，实现了精度与效率的平衡。


<details>
  <summary>Details</summary>
Motivation: 传统的跟踪器采用固定容量的计算路径处理所有搜索区域，忽略了不同帧和目标状态下跟踪难度的显著差异。

Method: 提出 SpecTrack 框架，核心包含谱自适应混合专家（SAMoE）模块以提供多级容量专家池，通过光谱提示路由器动态引导专家选择，并结合共享全局专家提供通用上下文。

Result: 在 MUST、MSITrack 和 HOTC20 数据集上达到最先进或极具竞争力的 AUC 表现（如 SpecTrack-L384 达到 65.2%、51.9% 和 72.6%），且在 RGB 数据集 GOT-10k 上展现出良好的泛化性（79.3% AO）。

Conclusion: SpecTrack 通过搜索区域级的自适应复杂度分配，有效解决了多光谱目标跟踪中精度与效率的权衡问题，并具备跨模态的泛化能力。
...
### [102] [SparseCtrl-HOI: Sparse Temporal Control for Human-Object Interaction Video Generation](https://arxiv.org/abs/2607.05994)
*Shenbo Xie,Mingrui Cai,Xu Yang,Yifei Liu,Changxing Ding*

Main category: cs.CV

TL;DR: 提出了一种名为 SparseCtrl-HOI 的稀疏时间控制框架，仅需少量关键帧即可生成高质量的人手与物体交互（HOI）视频。


<details>
  <summary>Details</summary>
Motivation: 现有HOI视频生成方法依赖密集的逐帧姿态引导，导致标注成本高昂且限制了动作合成的多样性。

Method: 引入时间控制旋转位置编码（TiRoPE）定位关键帧，利用多模态大模型（MLLM）注入运动先验以生成合理的中间帧过渡，并构建了 SparseHOI-5K 数据集。

Result: 实验表明，该方法在显著降低标注成本的前提下，合成了更优质的电商直播交互视频。

Conclusion: SparseCtrl-HOI 有效解决了HOI视频生成中的高标注成本与动作多样性不足的问题，为电商直播视频合成提供了高效的新方案。
...
### [103] [OBBSeg: Irregular Lesion Segmentation under Oriented Bounding Box Annotations](https://arxiv.org/abs/2607.06007)
*Jun Wei,Xinchang Liu,Yu Liu,Chuhua Yang,Shuhui Wang,Hui Huang*

Main category: cs.CV

TL;DR: 提出了一种基于有向边界框（OBB）引导的弱监督医学图像分割方法 OBBSeg，实现了接近全监督的分割性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像的像素级标注成本极高，而现有的弱监督方法约束不足，难以有效处理细长或各向异性的病灶。

Method: 引入OBB以联合编码空间范围和方向；设计了可微的Mask-to-OBB损失以消除矩形偏差；并结合了PAFE和DBFE两个提示驱动的语义引导模块来增强前景并抑制背景干扰。

Result: 在5种成像模态的13个数据集上的实验表明，OBBSeg不仅显著超越了现有的弱监督方法，还取得了与全监督方法相媲美的分割精度。

Conclusion: OBBSeg证明了有向边界框作为一种高效、可扩展的弱监督手段，在医学图像分割中具有巨大的应用潜力。
...
### [104] [Unlearnable Faces: Privacy Protection Surviving Extraction Pipeline](https://arxiv.org/abs/2607.05996)
*Byunghoon Oh,Sunghwan Park,Jaewoo Lee*

Main category: cs.CV

TL;DR: 本文提出了 LPID，一种通过将人脸提取（裁剪与缩放）过程融入优化目标来保护个人照片免受未经授权人脸识别的鲁棒不可学习样本生成方法。


<details>
  <summary>Details</summary>
Motivation: 现有的不可学习样本在面对攻击者对人脸图像进行裁剪和缩放等提取操作时容易失效。

Method: 将提取过程构建到不可学习样本的优化目标中，使用可微提取模型在特定频段和人脸区域内优化扰动，并支持按相册进行重新优化。

Result: LPID 在各种设置下均实现了最低的攻击者准确率（在人脸提取下将攻击者准确率降至 10% 以下），同时保持了高隐蔽性（32.7 dB PSNR 和 0.161 LPIPS）。

Conclusion: LPID 证明了将变换过程融入防御机制能有效提升对抗人脸提取的鲁棒性，为用户隐私提供了更强的保护。
...
### [105] [Structured Data Extraction from Real Estate Documents using Clustering, Classification, and Large Language Models](https://arxiv.org/abs/2607.06012)
*Muhammad Assad Shehbaz,Carlos Francisco Moreno-García*

Main category: cs.CV

TL;DR: 本文提出了一个自动分类和提取异构房地产问卷文档中结构化数据的端到端管道。


<details>
  <summary>Details</summary>
Motivation: 丰富的房产级别信息通常隐藏在格式异构的附件问卷文档中，目前缺乏在大规模下自动处理和提取这些信息的系统。

Method: 开发了端到端管道：首先将文档分类为三类（纯文本、扫描件和特殊字符），然后利用大语言模型 DeepSeek R1 从符合条件的文档中提取35个预定义属性并输出JSON，最后通过余弦相似度和K-Means聚类进行数据验证。

Result: 成功处理了2781份文档并生成2766条唯一房产记录。验证表明，余弦相似度匹配的Jaccard一致性得分达0.82，K-Means聚类的轮廓系数为0.2088。

Conclusion: 该方法在大规模异构房产文档的结构化数据提取中具有可行性和高可靠性。
...
### [106] [KOAL: Knowledge-Driven Prostate Cancer Grading with Ordinal-Aware Learning](https://arxiv.org/abs/2607.06019)
*Zheng Guo,Jiaqi Cui,Haocheng Xiong,Jize Han,Bo Liu,Qianwen Zhang,Rui Chen,Yan Wang*

Main category: cs.CV

TL;DR: 提出了KOAL框架，通过结合临床上下文、大模型专家知识和层级序数约束，提升了前列腺癌Gleason评分（GGG）的无创预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略了非图像信息（如年龄、PSA及报告先验），且将GGG简化为扁平分类，忽视了其首要和次要病理模式的层级结构。

Method: 提出KOAL框架，包含三个模块：临床上下文调制（CCM）动态融合临床变量；知识引导原型对齐（KGPA）利用大模型提取专家知识作为语义锚点；层级序数感知约束（HOC）解耦预测并利用可微生物逻辑映射层（DBML）确保病理分级一致性。

Result: 在公开PI-CAI和内部数据集上的实验表明，KOAL的预测效果显著优于现有的最先进方法。

Conclusion: KOAL框架通过整合临床上下文、专家先验和病理学层级逻辑，为前列腺癌的无创精准分级提供了有效的新方案。
...
### [107] [Why does Deep Learning Improve Visual SLAM?](https://arxiv.org/abs/2607.06023)
*Giovanni Cioffi,Davide Scaramuzza*

Main category: cs.CV

TL;DR: 本文通过对照实验探讨了深度学习视觉SLAM系统的成功要素，发现其优异性能主要源于学习到的2D数据关联和不确定性估计，而非其循环架构。


<details>
  <summary>Details</summary>
Motivation: 尽管基于深度学习的视觉SLAM系统性能优异，但目前尚不清楚具体是哪些组件（学习到的2D数据关联、不确定性估计，还是循环架构本身）决定了其成功。

Method: 通过进行受控的实证研究，系统地评估和对比了2D数据关联、不确定性估计以及循环架构对DL-based V-SLAM系统性能的贡献。

Result: 研究表明，深度学习V-SLAM系统的成功主要取决于学习到的2D数据关联和不确定性，而非其循环架构。

Conclusion: 该研究强调了在V-SLAM设计中，对数据关联和不确定性组件采用基于学习的范式的必要性，同时表明循环架构并不是取得成功的决定性因素。
...
### [108] [MSA-DCNN: A Data-Efficient Multi-Scale Deformable CNN for Medical Image Classification](https://arxiv.org/abs/2607.06083)
*Hamza Hussaini,Shahana Bano,Eyad Elyan,Carlos Francisco Moreno-García*

Main category: cs.CV

TL;DR: 提出MSA-DCNN框架，通过结合自适应多尺度采样与自蒸馏，提升了在标签稀缺和分布偏差情况下的医学图像分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分类方法难以同时应对多尺度形态变化和标注样本稀缺的挑战，且现有工作多是孤立地解决这些问题。

Method: 设计了MSA-DCNN框架，引入自适应多尺度采样、尺度内显著性细化、可学习的跨尺度融合以及辅助自蒸馏机制。

Result: 在多个公开基准和外部测试集上，该方法在参数量更少的前提下，在准确率、F1分数和AUC指标上均优于或相当于主流的ViT、CNN及半监督基准模型。

Conclusion: MSA-DCNN为解决数据受限的医学图像分类任务提供了一个高效且实用的基础框架。
...
### [109] [EcoVision: AI-Powered Drone Imaging for Salt Marsh Vegetation Monitoring and Dominance Mapping](https://arxiv.org/abs/2607.06105)
*Innocent Onyenonachi,Peter J. Lawerance,Nadia Kanwal*

Main category: cs.CV

TL;DR: 介绍了一种名为EcoVision的AI驱动无人机盐沼植物监测系统，用于高精度分类和优势度评估。


<details>
  <summary>Details</summary>
Motivation: 旨在解决盐沼生态系统监测中，如何高效、高精度地识别特定植物物种并将其转化为生态学可解释指标的问题。

Method: 结合Transformer语义分割、基于ConvNeXt的细粒度物种分类以及2x2m网格优势度评分，利用无人机RGB图像和公开数据集进行训练与测试。

Result: 语义分割平均IoU达0.56（像素准确率0.96），物体级分类F1值达0.99，优势度估算与实地样方调查的绝对差异低于8%。

Conclusion: EcoVision系统为可扩展的盐沼高分辨率监测奠定了实用基础，展示了AI在转化像素预测为生态评估指标方面的应用潜力。
...
### [110] [VendorBench-100: A Unified Cross-Paradigm Benchmark for Deepfake Image Detection](https://arxiv.org/abs/2607.06254)
*Sharayu N. Deshmukh,Md Rashidunnabi,Nelton Tiago Gemo,Kurundkar G. D.,Mahamune M. R.,Nilesh K. Deshmukh*

Main category: cs.CV

TL;DR: 本文推出了 VendorBench-100 基准测试，首次在统一框架下评估了 36 个跨范式的 Deepfake 检测模型，并揭示了模型排序能力与实际阈值决策质量之间的关键分歧。


<details>
  <summary>Details</summary>
Motivation: 商业 API、零样本视觉大模型和开源检测器这三类 Deepfake 检测范式缺乏统一的评估协议，导致难以进行直接对比。

Method: 开发了 VendorBench-100 基准，基于包含 100 张涵盖 8 类挑战性边缘场景的对抗性图像集，使用统一输出格式，并结合 MCC 和 ROC-AUC 指标进行评估。

Result: 商业 API 的中位数性能最优，其次是视觉大模型和开源检测器；更重要的是，研究发现模型的 ROC-AUC（排序能力）与 MCC（操作点质量）之间存在持续的分歧。

Conclusion: 强评分判别力并不等同于可靠的默认阈值决策，这种指标间的不一致性是本基准的核心发现，该研究开源了全部框架以促进可复现的后续研究。
...
### [111] [Token-Based Dual-view Fusion and Adaptation of Large Vision Models for Breast Cancer Classification](https://arxiv.org/abs/2607.06309)
*Aysan Ghayouri Pirsoltan,Shima Babakordi,Mohammad Reza Mohammadi*

Main category: cs.CV

TL;DR: 提出一种以 Token 为中心的双视角学习框架，通过在冻结的 Vision Transformer 中引入专用融合 Token 和多深度交互，实现了乳腺钼靶 CC 和 MLO 视角的高效融合。


<details>
  <summary>Details</summary>
Motivation: 现有乳腺多视角学习方法多依赖特征级聚合或单阶段交叉注意力，容易混淆特定视角与共享的表示，且限制了深层特征的交互。

Method: 将跨视角交互重新表述为结构化的 Token 级通信。利用专用融合 Token 在多个 Transformer 深度进行双向交叉注意力交互，并在后续层中进行层次化传播，同时保持各视角的独立结构。

Result: 在 VinDr-Mammo 和 CMMD 数据集上取得显著改进。在 VinDr-Mammo BI-RADS 分类任务中达到 50.40% 的 F1 分数和 0.8090 的 AUC，在二分类设置下比传统双视角融合基线提升了 0.10 AUC。

Conclusion: 实验和消融研究证明，基于 Token 的融合和多深度交互设计能够有效整合双视角互补信息，显著提升乳腺癌分类的性能。
...
### [112] [PVCap: Towards Accurate 3D Dense Captioning via PseudoCap and VoxelCapNet](https://arxiv.org/abs/2607.06097)
...
TL;DR: 提出了一种名为PVCap的3D密集描述（3D dense captioning）方法，通过PseudoCap数据增强技术和VoxelCapNet网络架构显著提升了描述性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏空间布局的多样性增强，难以有效描述物体间的空间关系，且骨干与检测网络设计过于简单，限制了语义信息的提取。

Method: 提出PVCap方法，包含：1）PseudoCap：利用实例级随机混合技术生成具有多样化布局的伪帧，并通过教师-学生框架获取伪描述标签；2）VoxelCapNet：一种基于体素特征的高效密集描述网络。

Result: 在ScanRefer和Nr3D基准测试中，该方法在CIDEr@0.5IoU指标上分别超越当前最先进方法11.41%和13.99%。

Conclusion: PVCap通过增强空间布局多样性并优化网络架构，有效解决了3D密集描述中的数据瓶颈与特征提取不足问题，为未来研究提供了一个强有力的基线。
...
### [113] [RoME: Robust Mixture of Low-Rank Experts against Multiple Adversarial Perturbations](https://arxiv.org/abs/2607.06109)
*Woo Jae Kim,Kyle Min,Suhyeon Ha,Joonsung Jeon,Sung-eui Yoon*

Main category: cs.CV

TL;DR: 本文提出RoME框架，通过低阶专家混合与改进的门控机制，显著提升了模型对多种及未知对抗扰动的防御能力。


<details>
  <summary>Details</summary>
Motivation: 多扰动对抗训练面临不同威胁之间的鲁棒性权衡，而传统MoE方法存在专家特征捕获冗余以及门控路由与威胁无关的挑战。

Method: 提出RoME方法，将专家设计为共享主干的低阶加性更新，并引入双尺度门控机制与威胁引导的门控多样化策略。

Result: 实验表明，RoME在联合鲁棒性和自然准确率上均优于现有最先进方法，并显著提升了对未见威胁的鲁棒性。

Conclusion: RoME通过结合低阶参数高效专家和改进的门控设计，成功克服了多威胁对抗训练中的权衡瓶颈，实现了更强的多扰动防御。
...
### [114] [Analysis-by-Proxy: Localization Signals in VLMs Operating as Condition Encoders](https://arxiv.org/abs/2607.06445)
*Yoav Baron,Sara Dorfman,Roni Paiss,Daniel Cohen-Or,Or Patashnik*

Main category: cs.CV

TL;DR: 本文研究了视觉语言模型（VLM）在图像编辑中作为条件编码器时定位能力下降的问题，并提出了“通过代理分析”框架来揭示原因。


<details>
  <summary>Details</summary>
Motivation: VLM在单次前向传播的条件编码模式下，无法充分发挥其自回归生成中展现出的强大空间定位能力，导致图像编辑在多实体场景中定位不准。

Method: 提出了“通过代理分析”（Analysis-by-Proxy）框架，通过在VLM的中间表示上训练一个轻量级、可解释的代理模型来执行辅助定位任务。

Result: 发现定位信号无法可靠地传播到现有图像编辑管道常用的预定义层，而是隐藏在随输入提示词变化的中间表示中。

Conclusion: 揭示了现有编辑管道中条件提取策略的根本缺陷，为未来更合理的条件架构设计奠定了基础。
...
### [115] [WebRetriever: A Large-Scale Comprehensive Benchmark for Efficient Web Agent Evaluation](https://arxiv.org/abs/2607.06118)
...
TL;DR: 本文提出了WebRetriever基准和NavEval评估框架，旨在解决现有Web智能体评估在规模、领域多样性和细粒度交互评估上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有Web智能体基准规模有限、领域单一，且现有的LLM-as-Judge评估方法无法捕获细粒度的交互语义，且忽视了真实部署中的关键需求。

Method: 引入了包含800个网站和1550个任务的大规模基准WebRetriever，提出了利用丰富交互上下文的评估框架NavEval，并建立了三种互补的评估协议。

Result: NavEval在与人类判断的一致性上达到了前沿水平，实验表明仅靠导航成功率不足以预测实际应用效果，不同协议间存在显著性能差异。

Conclusion: WebRetriever和NavEval为Web智能体提供了更全面、细粒度的评估诊断，为推动实用Web智能体研究奠定了坚实基础。
...
### [116] [Tuning-Free Latent Diffusion Models for Ultrahigh-Resolution Image Editing](https://arxiv.org/abs/2607.06136)
*Wanglong Lu,Lingming Su,Kaijie Shi,Minglun Gong,Xiaogang Jin,Hanli Zhao,Xianta Jiang*

Main category: cs.CV

TL;DR: 提出 UltraDiffEdit，一种无需微调的图像编辑框架，可在单张 GPU 上实现高达 8K 分辨率的高质量图像编辑。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型图像编辑方法通常受限于 1K 以下的分辨率，无法满足现代移动设备高达 8K 的实际超高分辨率编辑需求。

Method: 引入多尺度渐进式编辑策略，并结合多块编码、全局-局部一致性去噪以及基于分块的混合采样技术，以保留细节并确保语义一致性。

Result: 实验证明，UltraDiffEdit 仅使用单张 NVIDIA RTX 3090 GPU 即可处理高达 8K 分辨率的图像，具有极高的编辑质量和灵活性。

Conclusion: 该框架成功将潜在扩散模型扩展至超高分辨率，有效解决了计算资源限制与高分辨率图像细节保持之间的矛盾。
...
### [117] [Prompt-Adapter Context Routing for Parameter-Efficient Multi-Shot Long Video Extrapolation](https://arxiv.org/abs/2607.06481)
*Anna Córdoba,Adam Puente Tercero,Nerea Angulo Hijo,Mar Linares Tercero,Julia Barrientos,Ainhoa Miranda,Jesús Olivera*

Main category: cs.CV

TL;DR: PACR-Video是一个用于多镜头长视频外推的参数高效框架，通过轻量级时序适配器与递归提示词路由，在无需全局微调的情况下实现长视频的一致性生成。


<details>
  <summary>Details</summary>
Motivation: 旨在解决长视频生成中，如何在不进行全参数微调的前提下，跨多镜头维持角色实体、场景结构、视觉风格和故事因果进展的一致性问题。

Method: 冻结文本生成视频的扩散Transformer，并引入受镜头角色提示词引导的低秩时序适配器；构建递归提示词库进行动态路由，并采用镜头局部/故事全局的联合优化目标与适配器组合调度机制。

Result: 在六个多镜头和长视频基准测试中，PACR-Video在图像质量、语义对齐、身份一致性、时序平滑度、运动稳定性及人类偏好等多项指标上均显著优于现有基线方法。

Conclusion: 紧凑的提示词路由与轻量级的时序适配机制能够为稳定、可控的长视频外推任务提供充足的生成与对齐能力。
...
### [118] [AirflowAttack: Thermal-Airflow Adversarial Perturbations against Infrared Remote-Sensing Vision-Language Models](https://arxiv.org/abs/2607.06485)
*Cong Su,Jiaju Han,Xuemeng Sun,Chengyin Hu,Qike Zhang,Jiujiang Guo,Yiwei Wei,Jiahuan Long*

Main category: cs.CV

TL;DR: 提出首个针对红外遥感视觉语言模型的对抗攻击方法 AirflowAttack，利用热气流作为物理扰动先验，暴露出模型的严重漏洞。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLM）正被部署在安全关键的红外遥感成像中，但其对抗鲁棒性尚未得到评估。

Method: 提出 AirflowAttack，通过轻量级生成器合成与输入无关的扰动，并将其正则化为符合物理规律的热气流湍流模式。

Result: 在多款CLIP主干网络上实现48.5%的平均攻击成功率（ASR）；将六种先进VLM的分类准确率降低多达38.2%，并诱导模型将扰动误判为真实的温度梯度等热特征。

Conclusion: 该研究构建了包含11个模型和4项任务的基准，揭示了当前红外VLM生态系统中存在的关键安全脆弱性。
...
### [119] [What Images Cannot Say: Language-Guided Olfactory Representation Learning](https://arxiv.org/abs/2607.06402)
*Eleftherios Tsonis,Xi Wang,Vicky Kalogeiton*

Main category: cs.CV

TL;DR: 提出了SCENT框架，利用语言作为语义桥梁来对齐视觉和嗅觉信号。


<details>
  <summary>Details</summary>
Motivation: 由于许多嗅觉线索源自图像像素中无法直接看到的上下文环境因素，导致将气味信号与图像直接对齐面临巨大挑战。

Method: 利用视觉语言模型（VLM）生成场景描述以提供语义指导，训练气味编码器将电子鼻信号映射到共享嵌入空间，并引入语言引导的潜空间分解技术来区分物体特有气味与环境背景气味。

Result: 在New York Smells数据集上取得了最先进的气味-图像和气味-文本双向检索性能，并能够解耦复杂的气味混合物。

Conclusion: 上下文语义信息对于多模态学习中嗅觉感知的落地至关重要，为未来的多模态嗅觉研究奠定了基础。
...
### [120] [Enhanced Seam Segmentation for Automated Welding Robot in Construction Through Transfer Learning: Addressing Limitations of Bilateral Segmentation Network](https://arxiv.org/abs/2607.06150)
*Keonvin Park,Yong Ann Voeurn,Hyeokjun Kweon,Doyun Lee*

Main category: cs.CV

TL;DR: 本文提出了一种基于改进BiSeNetV2的反射鲁棒性焊缝分割框架，在不增加计算复杂度的前提下显著提升了强反射环境下的分割精度与稳定性。


<details>
  <summary>Details</summary>
Motivation: 自主焊接机器人在施工中面临强光、镜面反射和细窄焊缝导致的分割性能下降问题，亟需一种高鲁棒性的感知方法。

Method: 基于BiSeNetV2骨干网络，通过迁移学习和混合交叉熵-Lovász损失（Cross-Entropy--Lovász loss）进行优化，在不增加模型复杂度的前提下提升抗反射干扰能力。

Result: 该方法实现了81.76%的Joint IoU和90.73%的mIoU，在保持参数量和计算量不变的情况下，比基线提升了22.36%的Joint IoU，并成功修复了96.33%的严重零IoU失效案例。

Conclusion: 该框架为含有反射金属表面的机器人焊接提供了一种实用且轻量化的实时视觉感知解决方案。
...
TL;DR: 本文提出了AEGIS，一种通过在推理阶段动态引导关键注意力头来防御文本生成图像模型免受视觉同义词攻击（VSA）的新型防御机制。


<details>
  <summary>Details</summary>
Motivation: 现有的静态防御方法无法有效应对视觉同义词攻击（VSA），此类攻击通过隐式视觉关联绕过文本过滤，导致现有防御面临安全与实用性无法兼顾的困境。

Method: 通过机制分析发现VSA与显式不安全提示在特定的稀疏注意力头处发生语义收敛，据此提出AEGIS，在推理阶段对这些易受攻击的注意力头实施相似度感知排斥。

Result: 实验表明AEGIS在SD 1.4上将暴力/裸露VSA的攻击成功率降至0.00/0.03，对域外攻击的成功率控制在0.09以下，且能有效保留良性图像的保真度，并成功迁移至SD 2.1和FLUX.1。

Conclusion: AEGIS通过动态追踪和引导生成过程中的不安全语义，成功平衡了图像生成模型的安全防护与实用性保留。
...
### [122] [RFHNet: Relational and Frequency-Aware Hashing Network for Large-Scale Fine-Grained Food Image Retrieval](https://arxiv.org/abs/2607.06148)
*Junsong Wang,Weiqing Min,Guorui Sheng,Tao Yao,Lili Wang,Shuqiang Jiang*

Main category: cs.CV

TL;DR: 本文提出了一种用于细粒度食物图像检索的级联分层哈希网络RFHNet。


<details>
  <summary>Details</summary>
Motivation: 现有的哈希检索方法在细粒度食物场景中表现不佳，难以捕获微妙的局部语义和频率敏感的视觉线索。

Method: 设计了由细粒度关系建模（FRM）、多频调制融合（MFMF）和分层语义协同（HSS）组成的RFHNet架构。

Result: 在六个基准数据集上，RFHNet在12位哈希码下的平均精度（mAP）提升了4.44%至17.20%，显著优于现有方法。

Conclusion: RFHNet能够有效提升大规模视觉食物检索的性能，对智能餐饮等应用具有重要价值。
...
### [123] [ELSA3D: Elastic Semantic Anchoring for Unified 3D Understanding and Generation](https://arxiv.org/abs/2607.06565)
*Tianjiao Yu,Xinzhuo Li,Yifan Shen,Onkar Susladkar,Yuanzhe Liu,Xiaona Zhou,Ismini Lourentzou*

Main category: cs.CV

TL;DR: ELSA3D 通过弹性语义锚定将语言和多尺度几何推理进行结构化关联，显著提升了三维生成与理解的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的统一3D基础模型将文本和3D Token简单平铺拼接，导致粗粒度结构线索与细粒度几何细节混淆，缺乏明确的多尺度跨模态交互。

Method: 引入ELSA3D，使用尺度感知八叉树分词器表征几何，通过稀疏的跨模态“锚点Token”将语义路由至最相关的3D尺度进行特征融合，并采用轻量级路由器实现计算与推理的弹性分配。

Result: 在图像生成3D、文本生成3D及3D描述任务上均达到SOTA性能，超越了强基线模型，且相比非弹性版本减少了约一半的FLOPs和推理延迟。

Conclusion: 研究表明，在匹配的抽象尺度上联合构建语言和几何推理，能使跨模态能力精准集中，从而同时实现高精度与高效率的3D多模态交互。
...
### [124] [Revisiting Scene Graph Generation from the Perspective of Detector-Conditioned Reachability](https://arxiv.org/abs/2607.06176)
*Runfeng Qu,Pia K Bideau,Ole Hall,Julie Ouerfelli-Ethier,Klaus Obermayer,Olaf Hellwich*

Main category: cs.CV

TL;DR: 本文提出了一种名为Dual-SGG的双查询方法，通过融合基于检测器和基于查询的推理机制来提升场景图生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有场景图生成方法主要分为基于检测器和基于查询两类，但两者因推理机制不同而导致的预测差异此前缺乏系统分析，且具有互补性。

Method: 设计了对照实验来分析预测行为差异，并提出了一种融合两种推理机制的双查询（Dual-SGG）设计方法。

Result: 在Visual Genome、Open Images v6和GQA-200数据集上的广泛实验证明了该方法的有效性。

Conclusion: 通过双查询设计结合基于检测器和基于查询的方法，可以有效利用两者的互补预测优势，从而提升场景图生成的表现。
...
### [125] [MoWorld: A Flash World Model](https://arxiv.org/abs/2607.06216)
...
TL;DR: MoWorld是一个高性价比、高性能的实时交互世界模型（Flash World Model），在NPU上可达到50 FPS的推理速度，且推理成本仅为现有模型的30%-50%。


<details>
  <summary>Details</summary>
Motivation: 现有的世界模型推理效率低下且成本高昂，难以在没有高端GPU支持的情况下满足实时交互和大规模实际部署的需求。

Method: 提出了一套端到端框架，包含：1) 基于3D原生数据引擎的数据生成；2) 课程跨帧预训练策略；3) 旨在降低扩散训练成本的去噪步蒸馏算法；4) 针对NPU优化的混合精度并行推理框架。

Result: MoWorld首次在NPU上实现了高达50 FPS的实时交互，同时将平均推理成本降低至现有世界模型的30%-50%，并保持了电影级的视觉质量。

Conclusion: MoWorld证明了在非高端硬件（如NPU）上实现高效、低成本实时世界模型的可行性，为世界模型的大规模工业落地奠定了实用基础。
...
### [126] [High-Resolution Artwork Outpainting with Global Blueprint Guidance and Layout Control](https://arxiv.org/abs/2607.06162)
*Junha Kim,Hyunjoon Park,Donghyeon Cho*

Main category: cs.CV

TL;DR: 提出了一种全局蓝图引导的双阶段扩散框架，用于实现高分辨率、布局可控且高效并行的图像外绘。


<details>
  <summary>Details</summary>
Motivation: 现有高分辨率外绘方法存在全局规划缺失导致结构不稳定、缺乏空间控制力以及顺序生成导致的高推理延迟等问题。

Method: 采用双阶段法：第一阶段利用布局适配器生成低分辨率全局蓝图以确立全局结构；第二阶段引入全局引导，并行合成高分辨率局部图像块。

Result: 在艺术品数据集上，该方法显著提升了视觉质量与语义一致性，大幅降低了推理延迟，并支持显式的布局控制。

Conclusion: 该方法通过蓝图引导和并行合成，成功解决了高分辨率图像外绘在一致性、可控性与效率方面的瓶颈。
...
### [127] [WING: A Window-Prior-Based Generative Network with Gated Inception for Cross-Modality CT Synthesis](https://arxiv.org/abs/2607.06234)
*Siyuan Mei,Yan Xia,Yipeng Sun,Siming Bayer,Zirong Li,Chengze Ye,Daiqi Liu,Fuxin Fan,Yixing Huang,Andreas Maier*

Main category: cs.CV

TL;DR: 提出了一种基于窗口先验的生成网络（WING），通过将CT合成任务分解为多窗口预测，有效解决了直接回归高动态范围CT值时导致重要结构模糊的问题。


<details>
  <summary>Details</summary>
Motivation: 直接回归CT值受到其高动态范围和长尾分布的限制，容易导致稀疏但临床上重要的解剖结构在生成过程中被平滑和忽略。

Method: 将CT回归目标重构为多个具有平滑分布的窗口化表示，并提出WING网络，该网络包含：1）用于生成多窗口预测的门控Inception生成器；2）用于融合窗口输出并细化细节的Transformer；3）用于增强真实感的联合对抗训练目标。

Result: 在MRI-to-CT和CBCT-to-CT的基准测试中均达到了最先进的性能，并且能够使用单个模型支持多解剖部位的CT图像合成。

Conclusion: 利用CT强度的窗口可分性先验，将生成任务转化为多窗口预测与结构化融合，是解决跨模态医学图像合成中高动态范围问题的有效方法。
...
### [128] [MobileWan: Closing the Quality Gap for Mobile Video Diffusion](https://arxiv.org/abs/2607.06173)
...
TL;DR: 本文推出了MobileWan，通过循环重构和结构化压缩，首次将5B参数的服务器级视频扩散模型成功部署在商用移动设备上。


<details>
  <summary>Details</summary>
Motivation: 现有移动视频扩散模型因参数量小（0.4-1.8B）导致生成质量受限，而直接在移动设备上部署大规模视频扩散模型面临严峻的内存和算力挑战。

Method: 结合Wan2.2-5B模型，采用循环蒸馏框架将生成转化为块状自回归过程并引入因果线性注意力；提出基于二进制门控的可学习注意力头剪枝与蒸馏微调；使用采样步数蒸馏和内存优化的VAE解码。

Result: 在移动端仅需20秒即可端到端生成5秒、480x832分辨率（16 FPS）的视频，VBench得分达到83.79，创下移动端视频生成的新SOTA。

Conclusion: 通过合理的架构重构和压缩，无需缩减模型规模即可在受限的移动硬件上实现高质量的服务器级视频生成。
...
### [129] [Structured-Condensed Prompt Tuning in Vision-Language Models for Fine-grained Image Recognition](https://arxiv.org/abs/2607.06185)
*Xinda Liu,Qinyu Zhang,Weiqing Min,Guohua Geng,Shuqiang Jiang*

Main category: cs.CV

TL;DR: 本文提出SCPT方法，通过引入语义关系编码（SRE）和语义凝聚损失（ScLoss）来建模类别间的结构关系，显著提升了视觉语言模型在细粒度图像识别中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的提示微调方法将类别标签视为孤立实体，忽略了类别间丰富的语义关系和层级依赖，导致视觉语言模型在区分高度相似的细粒度类别时效果不佳。

Method: 提出结构化凝聚提示微调（SCPT），包含两个核心组件：语义关系编码（SRE）用于显式建模类别间的拓扑关系；语义凝聚损失（ScLoss）用于抑制冗余监督并提取判别性语义特征。

Result: 在14个细粒度基准数据集上的实验表明，SCPT在少样本和基类到新类泛化设置下均达到了SOTA性能，有效缓解了语义模糊问题。

Conclusion: 通过在提示学习中引入结构化的语义建模，可以显著增强视觉语言模型的语义对齐和细粒度分类能力。
...
### [130] [MAC-XA: Multi-view Anatomy-Correspondence Fusion for Coronary Stenosis Reporting from X-ray Angiography](https://arxiv.org/abs/2607.06268)
*Chen Jia,Baochang Zhang,Fatia Kusuma Dewi,Amir Yousefi,Heribert Schunkert,Reza Ghotbi,Nassir Navab*

Main category: cs.CV

TL;DR: 本文提出了一种基于对齐约束的冠状动脉X射线造影多视图报告生成方法，通过合成数据引入几何对应监督，显著提升了狭窄评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 单视图建模因3D血管拓扑导致的重叠和变形而不完整，而多视图融合由于真实数据中跨视图对齐无法显式监督，导致融合缺乏解剖一致性。

Method: 将多视图狭窄报告重新表述为对齐约束的聚合问题，引入可控合成造影生成策略以获取几何层面的对应监督，并设计解剖对应模块在融合前显式对齐跨视图特征。

Result: 在合成数据实验以及向真实造影的零样本迁移中，该对齐约束设计在对应一致性和结构化狭窄报告生成上均优于单视图和传统多视图融合方法。

Conclusion: 引入解剖对应关系的显式对齐约束能够有效解决多视图造影推理中的对齐缺失问题，提升冠状动脉造影的自动化诊断水平。
...
### [131] [AlayaWorld: Long-Horizon and Playable Video World Generation](https://arxiv.org/abs/2607.06291)
...
TL;DR: 本文推出了AlayaWorld，这是一个用于构建交互式生成式世界（Generative Worlds）的开源全栈框架。


<details>
  <summary>Details</summary>
Motivation: 传统的游戏开发流程成本高昂且难以定制，而现有的视频世界模型缺乏统一且高效的开源开发与部署框架。

Method: 设计并实现了AlayaWorld模块化架构，统一了数据准备、模型训练、推理加速及部署流程，支持实时且多样化的用户交互操作。

Result: 发布了可复现的流水线、参考实现、评估工具和详细文档，成功支持了实时开放式的交互生成。

Conclusion: AlayaWorld为生成式世界模型的未来研究和工业级实时应用提供了实用的开源基础。
...
### [132] [EeveeDark: A Binary Neural Framework for Low-Light Video Enhancement via Event-Guided Sensor-Level Fusion](https://arxiv.org/abs/2607.06217)
*Onur Eker,Erkut Erdem,Aykut Erdem*

Main category: cs.CV

TL;DR: EeveeDark是一个利用二值神经网络（BNN）高效融合RAW图像与事件流的轻量级低光视频增强框架。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的环境下，传统的低光视频增强方法难以在恢复质量和计算效率之间取得平衡。

Method: 引入EeveeDark框架，结合RAW的空间细节与事件流的时间精度。采用二值神经网络结构降低计算开销，并设计了模态特定二值编码器、轻量化融合块及事件引导跳跃门控机制。

Result: 实验表明，该方法在合成和真实数据集上均优于现有的二值化方法，并在性能与效率的权衡上相比全精度模型更具优势。

Conclusion: EeveeDark成功证明了结合多模态数据与二值神经网络在实现高效、高质量低光视频增强方面的可行性与优越性。
...
### [133] [PhyMRI-SR: Toward Physics-Aware MRI Image Super-Resolution](https://arxiv.org/abs/2607.06238)
*Lihua Wei,Huatong Gao,Jia Gong,Zhiyu Tan,Hao Li,Jun Liu,Zhihua Ren*

Main category: cs.CV

TL;DR: 本文提出了一种物理感知的动态分辨率MRI超分辨率方法，通过改进的2D高斯泼溅结合物理约束模型，实现了高保真度的重建。


<details>
  <summary>Details</summary>
Motivation: 传统MRI超分辨率方法将重建视为固定的映射，忽略了空间分辨率与信噪比（SNR）之间的物理耦合关系，无法有效处理实际采集中的动态分辨率输入。

Method: 将重建视作分辨率无关的渲染问题，引入2D高斯泼溅（2D GS）；提出结合解剖结构与成像系统的先验高斯表示；采用物理约束信号建模预测组织参数；利用元学习框架解决真实配对数据稀缺问题。

Result: 在动态分辨率数据集和标准基准测试上均取得了最先进的（SOTA）重建性能。

Conclusion: 该方法有效克服了动态分辨率重建的挑战，具有高保真度，在临床应用中展现出强烈的实用潜力。
...
### [134] [Synthetic-to-Real Translation for Class-Agnostic Motion Prediction](https://arxiv.org/abs/2607.06319)
...
TL;DR: 本文提出了一种从合成到真实场景的运动预测（SRMP）知识迁移框架，通过物体属性先验和新构建的Motion4D数据集来解决领域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 获取真实世界的运动标注数据成本高昂，而现有的运动回归方法极易受到合成与真实数据之间领域偏移的影响。

Method: 提出了包含物体感知运动预测和物体辅助运动增强的迁移框架，并开发了一个基于物理的管线来生成首个用于SRMP研究的合成4D LiDAR数据集（Motion4D）。

Result: 实验表明，该方法有效缩小了领域差距，并在真实场景的运动预测中取得了优异的表现。

Conclusion: 结合物体属性先验和专门的合成数据集，能够有效实现鲁棒的合成到真实场景的运动知识迁移。
...
### [135] [Straight-Path Flow Matching for Incomplete Multi-View Clustering](https://arxiv.org/abs/2607.06281)
*Yiteng Yuan,Junyan Wang,Zheyuan Liu,Hong Jia,Lei Fan,Zhulin Tao,Lianbo Guo*

Main category: cs.CV

TL;DR: 本文提出了一种基于流匹配（Flow-Matching）的端到端不完整多视角聚类框架，通过确定性ODE流代替扩散模型以实现更佳的缺失视角重建与聚类。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散模型的生成式不完整多视角聚类方法，因其随机去噪和无聚类倾向的噪声初始化，无法与聚类目标有效对齐。

Method: 引入流匹配框架，在配对视角表征之间建立线性插值路径，并融合聚类级与熵对齐机制以确保跨视角聚类一致性。

Result: 在标准不完整多视角聚类基准测试上进行了广泛实验，证明该方法取得了当前最先进（SOTA）的性能。

Conclusion: 确定性ODE流比基于扩散的随机轨迹在传输机制上更契合聚类目标，能更有效地保持类别条件分布和聚类一致性。
...
### [136] [FADRA: Frequency-Aware Diffusion with Residual Adaptation for Video Face Restoration](https://arxiv.org/abs/2607.06389)
*Jin Jiang,Jia Wang,Panwen Hu,Weiran Zhao,Shengcai Liao*

Main category: cs.CV

TL;DR: 本文提出了FADRA，一种基于频域感知和迭代残差自适应的视频人脸修复（VFR）扩散框架，旨在平衡空间保真度与时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的视频人脸修复方法在复杂退化下难以同时兼顾空间细节的真实性与时间维度的连贯性。

Method: 结合预训练视频扩散模型，引入轻量化LoRA和低质量像素对齐特征融合模块；设计了重复残差自适应头（RRAH）以进行逐步残差细化；并采用频域感知损失来加强对视觉敏感频段的监督。

Result: 实验结果表明，与现有最先进的方法相比，FADRA能够恢复出更好的面部结构，并生成时间上更一致的视频，在定量指标和视觉质量上均有明显提升。

Conclusion: FADRA通过有效地将强大的生成先验与迭代残差自适应及频域监督相结合，成功解决了视频人脸修复中的细节丢失和时间抖动问题。
...
### [137] [Generalized Synthetic Image Detection with Enhanced RGB-Noise Representation Learning](https://arxiv.org/abs/2607.06354)
*Zhen Li,Gang Cao,Tian Zhang,Lifang Yu,Shaowei Weng*

Main category: cs.CV

TL;DR: 本文提出了一种名为 RNSIDNet 的新型图像检测框架，通过融合 RGB 语义与噪声特征，有效提升了对生成式 AI 图像的泛化检测能力。


<details>
  <summary>Details</summary>
Motivation: 现有的合成图像检测方法过度依赖单域表征和传统的二分类优化，导致其在面对跨模型泛化和现实世界图像退化时表现不佳。

Method: 设计了双分支架构，利用 FiLM 模块使 CLIP 提取的全局 RGB 语义动态调节 Bayar 卷积捕获的高频噪声特征，并引入硬样本感知对比学习（HSCL）策略来拉大真伪图像的表征间距。

Result: 在八个公开基准数据集上的实验表明，该方法达到了最先进的性能（SOTA），在泛化能力、鲁棒性和计算效率上均表现优异。

Conclusion: RNSIDNet 成功证明了结合 RGB 与高频噪声的多域表征学习以及针对性的对比学习优化，是解决合成图像泛化检测难题的有效途径。
...
### [138] [Visual graphs for image classification: does the structure affect performance?](https://arxiv.org/abs/2607.06295)
*Alessandra Ibba*

Main category: cs.CV

TL;DR: 本文系统对比了在固定GCN架构下的不同图构建技术，探讨其对视觉任务性能的影响。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型难以完全编码图像的内在空间、拓扑和语义结构，而图神经网络在视觉任务中的应用及图构建技术尚未得到系统研究。

Method: 在固定的三层图卷积网络（GCN）框架下，对现有的图构建技术进行系统性的比较和实证研究。

Result: 展示了图网络结构对模型性能的具体影响，并对图构建前的计算准备阶段做出了方法学上的贡献。

Conclusion: 网络结构显著影响视觉GCN的性能，本研究为图利用前的计算阶段提供了重要的方法学指导。
...
### [139] [HoloCount: A Holistic Visual Counting Benchmark for MLLMs](https://arxiv.org/abs/2607.06420)
*Jinhong Deng,Limeng Qiao,Guanglu Wan*

Main category: cs.CV

TL;DR: 本文提出了HoloCount，这是一个用于全面评估多模态大语言模型（MLLMs）视觉计数能力的三层层级结构诊断基准测试集。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在数量精度上存在严重的数值幻觉，且现有的计数基准过于简化，无法评估模型在逻辑约束或对抗条件下的复杂失效模式。

Method: 引入HoloCount基准，从三个维度进行评估：语义计数（原子/属性枚举）、分析计数（空间/集合推理）以及鲁棒性测试（高密度/语言偏差等对抗场景）。

Result: 对20多个主流大模型评估发现，当任务从基本感知转向复杂的分析推理和对抗场景时，即使是顶级模型的性能也会出现显著下降。

Conclusion: 该研究系统地揭示了当前多模态大模型在计数能力上的局限性，并为开发更可靠的视觉计数与多模态系统提供了改进方向。
...
### [140] [PIPBench: A Profile-Inclusive Framework for Personalized Image Generation Evaluation](https://arxiv.org/abs/2607.06440)
*Yuhang Wu,Shuxiang Zhang,Wee Hian Ching,Chi Zhang,Miao Liu*

Main category: cs.CV

TL;DR: 本文提出了首个包含用户画像的个性化图像生成评估基准 PIPBench，用于评估模型与用户隐式视觉偏好的对齐能力。


<details>
  <summary>Details</summary>
Motivation: 目前的文本到图像模型虽然表现优异，但无法适应用户的个体审美偏好和历史偏好历史。

Method: 引入了评估基准 PIPBench，并设计了一种结合心理学和人口统计学特征的数据构建管线，用于真实用户数据采集和智能体数据生成。

Result: 通过在 PIPBench 上评估代表性方法，揭示了现有技术在个性化图像生成中的关键局限性。

Conclusion: 该研究为个性化文本到图像合成指明了新的挑战与机遇，推动了该领域的进一步发展。
...
### [141] [Bridging Diffusion Pruning and Step Distillation with Teacher-Aligned Repair](https://arxiv.org/abs/2607.06335)
*Jincheng Ying,Li Wenlin,Minghui Xu,Yinhao Xiao*

Main category: cs.CV

TL;DR: 本文提出了一种结合剪枝与单步蒸馏的扩散模型压缩方法，显著降低了推理成本。


<details>
  <summary>Details</summary>
Motivation: 旨在解决扩散模型压缩中剪枝后需要长时间微调，以及单步蒸馏对初始学生模型要求高的问题。

Method: 引入了“教师对齐修复阶段”作为剪枝与单步蒸馏之间的桥梁，通过噪声真实图像隐空间对齐剪枝模型与教师模型。

Result: 在ImageNet-512上，20%剪枝的单步生成器参数减少至98.8M且仅需1次评估，FID达到3.12（优于基线的3.53）；30%剪枝的单步生成器参数减少至88.0M，FID为4.26。

Conclusion: 通过教师对齐修复，成功用单步蒸馏替代了剪枝后的重训练，实现了高效且高质量的单步扩散模型生成。
...
### [142] [Temporal Modeling of Optically Variable Devices in Identity Documents](https://arxiv.org/abs/2607.06408)
*Glen Pouliquen,Joseph Chazalon,Guillaume Chiron,Oriol Ramos Terrades,Thierry Géraud,Ahmad Montaser Awal*

Main category: cs.CV

TL;DR: 提出了一种利用自监督和时序建模验证身份证件中动态全息图（OVD）的安全系统，能有效防御未知的伪造攻击。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视了OVD的时序动态特性，易受欺骗攻击，且视频的逐帧标注成本高，导致传统监督学习难以应用。

Method: 设计了两种在无攻击样本的自监督设定下训练的新方案，通过序列建模和异常检测来验证开放场景下OVD的动态变化。

Result: 在公开数据集上取得了超越前沿方法的效果，且完全符合工业部署的实际限制条件。

Conclusion: 研究表明时序动态建模对于抵御复杂的物理攻击至关重要，并证实了序列建模与异常检测在全息防伪验证中的巨大潜力。
...
### [143] [VaseMuseum: Digital Intelligent Museum for Ancient Greek Pottery](https://arxiv.org/abs/2607.06374)
*Jiazi Wang,Nonghai Zhang,Qiushi Xie,Zeyu Zhang,Yufeng Chen,Yang Zhao,Ling Shao,Hao Tang*

Main category: cs.CV

TL;DR: 本文提出了VaseMuseum，一个针对古希腊陶器数字化博物馆的轻量级多模态智能体框架，旨在提高VLM在文物交互中的可靠性与准确性。


<details>
  <summary>Details</summary>
Motivation: VLM在文化遗产领域应用时，难以将视觉证据与专业知识有效结合，且在信息不完整或模糊时容易产生虚假自信的幻觉回答。

Method: 提出VaseMuseum框架，通过VaseAgent实现多模态感知与3D推理，结合源头级与响应级控制筛选可靠证据，并采用免训练的GRPO风格选择机制优化生成结果。

Result: 实验显示，该框架在数字化博物馆模拟中显著提升了引文有效性，减少了知识密集型查询的幻觉，并在歧义情况下表现出更高的中立性。

Conclusion: 通过模块化多模态智能体与推理期可靠性控制，VaseMuseum有效解决了VLM在专业文物检索与交互中的知识接地和幻觉问题。
...
### [144] [XRFormer: Multiscale Tokenization for XRF Representation Learning](https://arxiv.org/abs/2607.06424)
*Sofiane Daimellah,Sylvie Le Hégarat-Mascle,Clotilde Boust*

Main category: cs.CV

TL;DR: 提出了XRFormer，一种专为X射线荧光（XRF）光谱设计的Transformer架构，通过多尺度分词和自监督预训练，显著提升了颜料识别与解混的性能和参数效率。


<details>
  <summary>Details</summary>
Motivation: XRF光谱包含复杂的元素峰、宽结构和背景变化，现有基于机器学习的模型难以有效处理这些复杂的1D信号。

Method: 引入了结合局部性和多分辨率偏差的多尺度卷积分词器，并设计了掩码光谱建模（MSM）和物理启发的峰值存在预测（PPP）两种自监督预训练方法。

Result: XRFormer在颜料识别上超越了ViT、SpectralFormer和1D-CNN；在解混任务中以不足一半的参数量（1.5M vs 3.37M）实现了鲁棒的丰度估计，且MSM and PPP带来了持续的性能增益。

Conclusion: 多尺度、模态感知的分词机制是数据受限条件下，构建高效、轻量化Transformer-based XRF光谱分析模型的有效途径。
...
### [145] [Verification of Dynamic Holographic Behavior in Identity Documents](https://arxiv.org/abs/2607.06466)
*Glen Pouliquen,Joseph Chazalon,Guillaume Chiron,Thierry Géraud,Ahmad Montaser Awal*

Main category: cs.CV

TL;DR: 本文提出了一种针对身份证件全息图防伪的远程验证方法、一个全新的动态攻击数据集MIDV-DynAttack以及相关基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有全息图自动验证方法缺乏评估动态欺诈的公开数据集，且难以泛化到未知的攻击类型。

Method: 构建了包含静态与动态攻击的MIDV-DynAttack数据集，并提出了一种基于动态行为和外观分析且无需动态攻击样本训练的验证方法。

Result: 所提方法取得了新的最先进性能，基准测试也揭示了现有方法在应对动态攻击时的不足。

Conclusion: 结合动态特征分析能有效提升全息图防伪验证的泛化与检测能力，新数据集为该领域的研究奠定了基础。
...
### [146] [Point as Skeleton: Accumulated Point Cloud Enhanced Autoregressive Generation for Closed-Loop Autonomous Driving Simulation](https://arxiv.org/abs/2607.06516)
...
TL;DR: 本文提出了 Point as Skeleton，一个用于闭环自动驾驶评估的生成式传感器模拟框架，旨在平衡真实视觉保真度与闭环交互性。


<details>
  <summary>Details</summary>
Motivation: 现有的自动驾驶仿真方法在闭环交互性（如 CARLA）与真实世界视觉保真度（如 nuScenes）之间存在难以调和的权衡关系。

Method: 提出自回归生成框架，并引入 Reset-and-Roll 机制以适应滚动扩散推理，同时利用点云骨架解耦前景和背景，提供外观与几何线索以稳定误差累积。

Result: 在 nuScenes 和 nuPlan 数据集上的实验表明，该方法在闭环运行过程中显著提升了自回归生成的质量。

Conclusion: Point as Skeleton 成功展示了其在实现高视觉保真度、闭环交互式自动驾驶仿真方面的巨大潜力。
...
### [147] [EgoPolice: A Benchmark for Egocentric Video Understanding in High-Stakes Police Body-Worn Camera Footage](https://arxiv.org/abs/2607.06468)
...
TL;DR: 本文介绍了EgoPolice，一个源自真实警民互动执法记录仪视频的第一人称视角（egocentric）数据集，用于评估视频模型在复杂、高风险场景下的感知能力。


<details>
  <summary>Details</summary>
Motivation: 警务行为研究和视频自动审查需要处理第一人称视角视频，但这些视频包含剧烈运动、密集交互和罕见的高风险事件，现有模型在此类场景中的表现亟待评估和提升。

Method: 构建了包含逐秒精细动作标注的EgoPolice数据集，设计了动作分类和多项选择问答（MCQA）两项任务，并对多种开源和闭源视频大模型进行了基准测试。

Result: 实验表明，即使是像Gemini 2.5 Pro这样先进的视频模型，在准确预测“拔枪（Weapon Out）”等高风险动作方面依然面临巨大挑战。

Conclusion: EgoPolice不仅是一个极具挑战性的视频感知基准，还为在大规模执法记录仪视频中高效识别关键事件、辅助下游人工审查奠定了基础。
...
### [148] [Andha-Dhun: A First Look at Audio Descriptions in Hindi](https://arxiv.org/abs/2607.06457)
*Ritabrata Chakraborty,Divy Kala,Nisheeth Bhooshan Gupta,Ganji Sreeram,Pailla Balakrishna Reddy,Makarand Tapaswi*

Main category: cs.CV

TL;DR: 本文对印地语视频无障碍旁白（AD）进行了首次系统性研究，并推出了首个由人类撰写的印地语无障碍旁白数据集 Andha-Dhun。


<details>
  <summary>Details</summary>
Motivation: 为了满足视障群体的需求以及印度官方的强制规定，亟需将视频无障碍旁白扩展到英语以外的印度本地语言，但目前该领域尚属空白。

Method: 收集了8部完整电影的印地语无障碍旁白构建数据集；探索了“从英文视频描述直接生成”和“将英文旁白翻译为印地语”两种方法；并采用困惑度和大模型裁判指标进行评估。

Result: 直接机器翻译无法很好地转换文化背景，且容易引入瑕疵并降低内容多样性；即使是人工翻译的旁白也存在局限性。

Conclusion: 印地语无障碍旁白的关键在于针对视障受众进行文化和内容的本土化改编，而非对英文源内容的机械式翻译。
...
### [149] [A VLM-Enhanced Framework for Comprehensive Traffic Sign Condition Assessment Integrating Daytime Visual Performance and Nighttime Retroreflectivity Evaluation](https://arxiv.org/abs/2607.06478)
*Linlin Zhang,Neema Jakisa Owor,Xiang Yu,Abby Watts,Yaw Adu-Gyamfi*

Main category: cs.CV

TL;DR: 本文提出了一种结合白天视觉多模态模型（VLM）与夜间LiDAR反光率分析的交通标志综合评估框架，以替代传统的人工检测方法。


<details>
  <summary>Details</summary>
Motivation: 传统检测方法主观性强、劳动强度大且存在安全隐患，设备成本高昂，且以往研究极少将白天视觉要素与夜间逆反射性能结合进行全面评估。

Method: 利用三款微调的VLM进行白天多维度视觉评估（结合情感分析与CLIP评分），并通过LiDAR校准评估夜间逆反射表现，最终构建综合标志状况指数（SCI）。

Result: LLaVA和Qwen模型性能优于InternVL，双向余弦相似度达到0.67-0.76；在462个测试标志中，成功识别出68个需要立即更换的低反光度标志。

Conclusion: 该研究为交通标志的全面维护和状况评估提供了一种高性价比、自动化的替代方案。
...
### [150] [Unsupervised Domain Adaptation for Calcification Classification in Mammography Across Multi-Site Datasets](https://arxiv.org/abs/2607.06549)
...
TL;DR: 本文提出了一种结合无监督域自适应与Swin Transformer V2的乳腺钙化分类框架，有效提升了跨中心乳腺钼靶图像诊断的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习辅助诊断系统在乳腺癌诊断中表现良好，但跨中心数据集的域偏移限制了模型在未知域上的泛化能力。

Method: 提出双模块框架：1) 基于风格迁移（AdaIN和CycleGAN）的无监督域自适应模块，用于生成特定厂商/技术的训练样本；2) 基于Swin Transformer V2的监督分类模块。

Result: 在外部验证中，该框架将EMBED数据集的AUC从0.68提升至0.72，将Duke数据集的AUC从0.68提升至0.73。

Conclusion: 域自适应技术能有效减少域偏移，提高跨中心乳腺钙化分类模型的泛化能力。
...
### [151] [Mitigating Domain Shift in Conditioned Floor Plan Generation: Synthetic Pre-training for Data-Efficient Adaptation](https://arxiv.org/abs/2607.06483)
*Matthieu Ospici,Arnaud Gueze,Luc Bourrat,Adrien Bernhardt*

Main category: cs.CV

TL;DR: 本文评估了户型图生成模型的跨领域鲁棒性，并提出了一种利用物理约束的合成数据进行预训练的方法，以提升模型的泛化与微调性能。


<details>
  <summary>Details</summary>
Motivation: 不同地区的户型图受建筑文化和空间限制影响差异巨大，获取新标注数据成本高昂，且现有模型极易受领域偏移影响，此前缺乏相关鲁棒性研究。

Method: 在三个公开数据集上评估了现有模型；提出一种程序化方法来生成具有物理约束（如不重叠房间、有效门放置等）但几何形状高度扰动的合成训练数据集，并以此进行预训练和微调。

Result: 实验表明现有模型跨领域性能下降显著；而使用所提合成数据预训练不仅提升了零样本跨领域表现，还在微调时将目标领域适应性能提升了高达40%。

Conclusion: 结合物理约束与几何多样性的合成数据预训练，是解决户型图生成模型领域适应和鲁棒性问题的有效途径。
...
### [152] [From RGB Generation to Dense Field Readout: Pixel-Space Dense Prediction with Text-to-Image Models](https://arxiv.org/abs/2607.06553)
*Zanyi Wang,Xin Lin,Haodong Li,Dengyang Jiang,Yijiang Li,Pengtao Xie*

Main category: cs.CV

TL;DR: 本文提出 ReChannel，一种无需目标端 VAE 解码器、直接将预训练 DiT 的 token 映射为任务特定像素块的密集预测方法。


<details>
  <summary>Details</summary>
Motivation: 现有生成式密集预测方法依赖 VAE 潜空间和解码器来重建目标，这引入了不必要的生成开销，且限制了像素级精度。

Method: 保留输入端 VAE 编码器并弃用输出端解码器，使用任务 LoRA 适配冻结的 DiT，并通过一个超轻量级的 token 本地线性头直接输出像素级预测结果。

Result: 在多项任务上达到 SOTA 性能，在 4B 参数规模下，相比传统的潜空间解码方案，准确度更高且推理速度提升 2.48 倍。

Conclusion: 密集感知任务可以高效利用生成式预训练先验，而无需绑定其传统的生成式输出接口。
...
### [153] [CAIRN: Cross-Room 3D Scene Understanding with Topology-Aware Large Multimodal Models](https://arxiv.org/abs/2607.06534)
*He Liang,Chenyang Ma,Yiming Zhang,Sangyun Shin,Andrew Markham,Niki Trigoni,Yuhang He*

Main category: cs.CV

TL;DR: 本文提出了CAIRN，一种具有拓扑感知能力的3D大语言模型，旨在解决复杂多房间3D场景的理解与推理问题。


<details>
  <summary>Details</summary>
Motivation: 现有的3D-LLM局限于简单的单房间3D场景，无法对真实世界中包含多个互联房间和多样化物体类别的家庭环境进行有效推理。

Method: 提出了CAIRN模型和CAIRN-MR基准。CAIRN通过图神经网络丰富物体的房间局部关系，引入学习型房间Token，并使用带几何偏差的分层注意力掩码来对齐场景拓扑结构。

Result: CAIRN在CAIRN-MR基准的各项任务中（定位、描述、多任务问答等）均大幅超越先前的3D-LLM，并在五个单房间基准上保持了竞争力。

Conclusion: 结合场景层次结构和拓扑感知的设计能显著提升3D-LLM在复杂多房间环境下的空间理解和跨房间推理能力。
...
### [154] [MonoIR-RS: Infrared Remote Sensing Vision-Language Learning with CLIP and VLM Adaptation](https://arxiv.org/abs/2607.06552)
*Jiaju Han,Ma Yaqi,Yahui Chai,Xuemeng Sun,Xin Li,Qike Zhang,Yingying Zhao,Xiang Chen,Luwei Yang,Chengyin Hu,Jiahuan Long*

Main category: cs.CV

TL;DR: 本文提出了MonoIR-RS，一个旨在将红外遥感图像与语言对齐的大规模数据集和基准，有效解决了红外视觉语言理解不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有遥感多模态模型主要关注可见光语义，忽视了红外图像特有的灰度结构、目标对比度和光照不变性等特征，导致红外领域的视觉语言理解未得到充分探索。

Method: 构建了包含60万张合成红外图像及近6万条红外感知文本标注的MonoIR-RS数据集（去除RGB颜色描述），并对5个CLIP和6个VLM基准模型进行了对比适应性微调和指令微调。

Result: 实验表明，合成的红外图像比单纯的灰度转换更接近真实热成像；微调后CLIP的平均召回率提升高达12.8%，VLM在生成描述时的红外特征覆盖率达到100%，且残留的RGB颜色泄露降至接近零。

Conclusion: MonoIR-RS通过隔离红外模态，为将红外遥感信息与语言对齐提供了一个受控且可复现的基准测试平台。
...
### [155] [Vision as Unified Multimodal Generation](https://arxiv.org/abs/2607.06560)
...
TL;DR: SenseNova-Vision将各种计算机视觉任务统一为多模态生成，通过单一模型实现多任务的通用处理。


<details>
  <summary>Details</summary>
Motivation: 消除传统计算机视觉中对特定任务架构和预测头的依赖，探索将视觉能力整合进通用基础模型的统一途径。

Method: 构建SenseNova-Vision Corpus数据集，并在预训练多模态模型上进行端到端的指令微调，无需任何特定任务的结构修改。

Result: 在目标检测、OCR、分割、深度估计等多种任务上，该单一模型达到了与各领域专用系统相媲美的性能。

Conclusion: 统一多模态生成是构建通用视觉基础模型的一条极具可扩展性的可行路线。
...
### [156] [ProxyPose: 6-DoF Pose Tracking via Video-to-Video Translation](https://arxiv.org/abs/2607.06555)
*Ruihang Zhang,Felix Taubner,Pooja Ravi,Kiriakos N. Kutulakos,David B. Lindell*

Main category: cs.CV

TL;DR: 提出ProxyPose，将单目视频中的6-DoF位姿跟踪重塑为视频到视频的转换任务，仅需视频和首帧的单个标记像素即可实现高精度跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有的6-DoF位姿跟踪方法通常需要3D模型、深度图等额外输入，且在面对无纹理、透明、反射或变形表面时面临巨大挑战。

Method: 利用微调的视频扩散模型将输入视频转换为“代理视频”（展示相同局部刚体运动的彩色多面体），然后通过经典位姿求解器从该代理视频中恢复6-DoF轨迹。

Result: 在无需额外输入且仅在合成数据上微调的情况下，实现了最先进的6-DoF位姿跟踪精度，并成功泛化至人脸跟踪、相机位姿估计和野外挑战性场景。

Conclusion: ProxyPose通过将复杂的物理跟踪问题转化为视频生成问题，有效克服了材质、遮挡和变形等传统跟踪难点，展示了强大的泛化能力。
