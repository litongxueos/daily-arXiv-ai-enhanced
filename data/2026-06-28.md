...
### [1] [Simulation-based inference for rapid Bayesian parameter estimation in epidemiological models: a comparison with MCMC](https://arxiv.org/abs/2606.27286)
...
TL;DR: 本文评估了基于模拟的推断（SBI）在流行病学模型校准中的应用，表明其在保持高精度的同时显著缩短了计算时间。


<details>
  <summary>Details</summary>
Motivation: 传统的马尔可夫链蒙特卡罗（MCMC）方法在对复杂的流行病学模型进行贝叶斯校准时计算成本过高，难以满足实时分析的需求。

Method: 引入基于神经网络后验估计的模拟推断（SBI）方法，对SECIR流行病模型进行校准，并与传统的MCMC方法在不同时间窗口和复杂度下进行对比。

Result: SBI在推断结果上与MCMC高度一致，且计算速度显著提升：在31天窗口中将运行时间从1000秒缩短至60-70秒，在201天窗口中从19000秒缩短至157秒。

Conclusion: SBI为流行病学模型的快速贝叶斯校准提供了一种高效的计算框架，能够有效支持近实时的疫情推断和爆发分析。
...
### [2] [EO-WM: A Physically Informed World Model for Probabilistic Earth Observation Forecasting](https://arxiv.org/abs/2606.27277)
*Junwei Luo,Shuai Yuan,Zhenya Yang,Yansheng Li,Zhe Liu,Hengshuang Zhao*

Main category: cs.AI

TL;DR: 本文提出了EO-WM，一个用于多光谱地球观测预测的视频扩散Transformer模型，能够更准确地响应气象变化。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能有效应对预测中的不确定性，且将天气变量视为无差别的调节信号，导致无法准确评估预测对天气强迫变化的响应。

Method: 引入物理启发式调节框架（区分气候基线与天气异常，并累积异常胁迫信号），并设计了极端夏季基准和季节匹配对基准两个诊断基准。

Result: 实验表明，EO-WM将NDVI下降幅度的预测误差降低了5.63%，方向命中率提高了7.80%，同时在标准像素级指标上保持竞争力。

Conclusion: EO-WM通过更精细的天气调节机制和全新的诊断基准，显著提升了地球观测中天气驱动陆表动态预测的准确性和响应保真度。
...
### [3] [Prompt Injection in Automated Résumé Screening with Large Language Models: Single and Multi-Injection Settings](https://arxiv.org/abs/2606.27287)
*Preet Baxi,Jiannan Xu,Jane Yi Jiang,Stefanus Jasin*

Main category: cs.AI

TL;DR: 本文研究了在大型语言模型（LLM）简历筛选中，求职者利用提示词注入进行操纵的效果及其对招聘公平性的影响。


<details>
  <summary>Details</summary>
Motivation: LLM广泛应用于招聘筛选，促使求职者尝试通过在简历中嵌入微妙的提示词注入来操纵算法评估，亟需评估其有效性与潜在风险。

Method: 采用受控实验方法，在候选人质量同质或异质、以及不同提示词注入比例的多种设定下，测试LLM的排名反应。

Result: 当简历质量同质且仅少数人使用注入时，该手段能显著提升排名，但随普及率增加效果迅速崩溃；在异质场景下，它偶尔能使低质量候选人反超高质量候选人。

Conclusion: LLM筛选系统在操纵行为罕见且候选人差异较小时最易受攻击，这为AI辅助招聘的公平性带来了隐患。
...
### [4] [On-board Remote-Sensing Foundation Models for Unsupervised Change Detection of Disaster Events](https://arxiv.org/abs/2606.27018)
*S. Ramírez-Gallego*

Main category: cs.CV

TL;DR: 本文提出了一种基于遥感基础模型（RSFM）和特征金字塔网络（FPN）的无监督变化检测方法，无需训练即可实现高效的高分辨率异常检测。


<details>
  <summary>Details</summary>
Motivation: 旨在解决轨道应用中受限的算力和电力资源问题，通过无需昂贵标注的无监督方法，实现高效的灾害监测与异常检测。

Method: 结合了ResNet (RSFM)与未训练的FPN架构，利用其固有先验，通过检测连续轨道扫过时潜空间中的细微语义变化来进行异常识别（免训练）。

Result: 与以往需要训练或基于图像块的方法相比，该方法实现了高效的图像级生成和更高分辨率的映射，同时大幅减少了开发工作量。

Conclusion: 通过使用RSFM替代定制模型，该方法在消除定制训练需求的同时，保证了在不同地形和传感器上的高泛化性能。
